...
export function answerAwaiting(
  ...
  switch (awaiting.kind) {
    ...
    case 'chooseFromZone': {
      ...
      // D521 - the Ring-bearer pick: a creature I control (the prompt's rule - a derived creature carries a power), the
      // biggest first (the evasion and the loot are worth most on the body that attacks); the host refuses anything else.
      if (awaiting.zone === 'battlefield' && awaiting.pick === 'ringBearer') {
        const mine = myPermanents(view, me).filter((c) => c.power !== null && c.toughness !== null);
        const cards = [...mine].sort((a, b) => (b.power ?? 0) - (a.power ?? 0)).slice(0, awaiting.count).map((c) => c.instanceId);
        if (cards.length < (awaiting.min ?? awaiting.count)) return fault('noIntentForAwaiting', `asked for a Ring-bearer among ${mine.length} creatures`);
        return act({ t: 'AnswerChooseFromZone', player: me, cards }, `bear the Ring with ${cards.length} for ${awaiting.label}`);
      }
