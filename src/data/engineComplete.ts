...
export function linesUnaccounted(
  ...
  for (const [i, text] of lines.entries()) {
    ...
    // D546 - an embalm / eternalize line whose copy the vocabulary read is the engine's.
    if (face.activated.some((a) => a.embalm?.effects !== undefined && a.embalm.line === line)) continue;
    ...
    // D408 - an alternative cost the engine CHARGES at cast (elected by `CastSpell.alternative`).
