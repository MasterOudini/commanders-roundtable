...
export function tier3NotesFor(card: CardData, faceIndex = 0): Tier3Note[] {
  ...
  // ⚠️ D363 disclaimed a `{S}` cost here and D364 took the note away: the engine
  // charges it now, from mana a snow source made (CR 107.4s), so saying otherwise
  // would be the lie in the other direction - warning a player off a card the app
  // plays. The boundary that REMAINS is the hand tool: mana added with the Tier-3
  // control is ordinary mana, so `{S}` cannot be paid off it. That is a limit of the
  // TOOL rather than of the card, which is why it is not a note on every snow card.
  ...
  for (const ability of abilities) {
    ...
    if (ability.ninjutsu !== undefined) continue;
    ...
    if (ability.sacrificesSelf || ability.sacrificeCost || ability.discardCost || ability.tapCost || ability.removeCounterCost || ability.returnCost || ability.returnsSelf || ability.putCounterCost) {
      add(
  ...
  for (const raw of card.keywords) {
    ...
    if (raw.trim().toLowerCase() === 'ninjutsu' && abilities.some((a) => a.ninjutsu !== undefined)) continue;
