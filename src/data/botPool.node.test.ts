...
describe.skipIf(!HAVE_DB)('the bot pool, measured', () => {
  ...
  test('D90 does not reproduce, and the vocabulary keeps moving it: 948 auto, 1,832 assisted (D199, D343)', () => {
    expect.soft(r.spells.auto).toBe(1332);
    expect.soft(r.spells.assisted).toBe(2000);
    expect.soft(r.spells.autoAnyFace).toBe(1340);
  });
  ...
  test('enchantments execute now - 393 after the trigger heads; planeswalkers and battles still none', () => {
    ...
    // enchantments were.
    expect.soft(r.poolByType['enchantment'] ?? 0).toBe(491);
