...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 7711 fixtures: 7543 taken by name, 168 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(7711);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(168);
  });
