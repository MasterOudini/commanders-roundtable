...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 5603 fixtures: 5448 taken by name, 148 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(5603);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(148);
  });
