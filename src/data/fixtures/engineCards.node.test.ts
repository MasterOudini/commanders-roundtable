...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 387 fixtures: 348 taken by name, 39 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(387);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(39);
  });
