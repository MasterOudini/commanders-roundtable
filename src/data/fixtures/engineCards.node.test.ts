...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 1,628 fixtures: 1,541 taken by name, 87 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(1628);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(87);
  });
