...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 600 fixtures: 538 taken by name, 62 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(600);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(62);
  });
