...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 2,067 fixtures: 1,974 taken by name, 93 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(2067);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(93);
  });
