...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 1,927 fixtures: 1,837 taken by name, 90 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(1927);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(90);
  });
