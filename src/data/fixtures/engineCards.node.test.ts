...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 578 fixtures: 518 taken by name, 60 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(578);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(60);
  });
