...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 9067 fixtures: 8801 taken by name, 266 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(9067);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(266);
  });
