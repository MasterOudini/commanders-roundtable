...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 8535 fixtures: 8300 taken by name, 235 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(8535);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(235);
  });
