...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 7989 fixtures: 7816 taken by name, 173 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(7989);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(173);
  });
