...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 4650 fixtures: 4510 taken by name, 133 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(4650);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(133);
  });
