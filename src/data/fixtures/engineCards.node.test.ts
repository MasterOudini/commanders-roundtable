...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 9303 fixtures: 9029 taken by name, 274 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(9303);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(274);
  });
