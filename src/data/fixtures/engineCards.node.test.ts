...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 2,393 fixtures: 2,285 taken by name, 101 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(2393);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(101);
  });
