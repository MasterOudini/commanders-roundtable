...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 7799 fixtures: 7630 taken by name, 169 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(7799);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(169);
  });
