...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 537 fixtures: 479 taken by name, 58 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(537);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(58);
  });
