...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 1,571 fixtures: 1,485 taken by name, 86 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(1571);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(86);
  });
