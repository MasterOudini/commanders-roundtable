...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 7849 fixtures: 7679 taken by name, 170 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(7849);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(170);
  });
