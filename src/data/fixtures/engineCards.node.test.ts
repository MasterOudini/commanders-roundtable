...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 7352 fixtures: 7181 taken by name, 164 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(7352);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(164);
  });
