...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 7114 fixtures: 6951 taken by name, 156 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(7114);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(156);
  });
