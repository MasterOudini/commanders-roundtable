...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 2,574 fixtures: 2,463 taken by name, 104 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(2574);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(104);
  });
