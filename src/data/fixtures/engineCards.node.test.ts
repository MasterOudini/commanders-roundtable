...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 1,473 fixtures: 1,390 taken by name, 83 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(1473);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(83);
  });
