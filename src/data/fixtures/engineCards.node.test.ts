...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 3860 fixtures: 3732 taken by name, 121 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(3860);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(121);
  });
