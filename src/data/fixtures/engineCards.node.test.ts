...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 1,397 fixtures: 1,317 taken by name, 80 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(1397);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(80);
  });
