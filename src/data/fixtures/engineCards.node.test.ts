...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 556 fixtures: 497 taken by name, 59 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(556);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(59);
  });
