...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 3456 fixtures: 3340 taken by name, 109 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(3456);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(109);
  });
