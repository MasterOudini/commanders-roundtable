...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 646 fixtures: 582 taken by name, 64 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(646);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(64);
  });
