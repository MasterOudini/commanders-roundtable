...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 6579 fixtures: 6418 taken by name, 154 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(6579);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(154);
  });
