...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 7916 fixtures: 7744 taken by name, 172 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(7916);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(172);
  });
