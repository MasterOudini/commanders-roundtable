...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 8340 fixtures: 8112 taken by name, 228 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(8340);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(228);
  });
