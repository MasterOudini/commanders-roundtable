...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 4193 fixtures: 4061 taken by name, 125 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(4193);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(125);
  });
