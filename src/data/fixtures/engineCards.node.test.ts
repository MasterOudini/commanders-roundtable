...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 7582 fixtures: 7410 taken by name, 165 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(7582);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(165);
  });
