...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 8666 fixtures: 8430 taken by name, 236 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(8666);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(236);
  });
