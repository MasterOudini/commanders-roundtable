...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 8754 fixtures: 8492 taken by name, 262 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(8754);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(262);
  });
