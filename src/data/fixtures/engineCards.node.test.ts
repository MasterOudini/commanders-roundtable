...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 670 fixtures: 602 taken by name, 68 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(670);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(68);
  });
