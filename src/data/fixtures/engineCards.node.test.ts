...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 181 fixtures: 172 taken by name, 9 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(181);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(9);
  });
