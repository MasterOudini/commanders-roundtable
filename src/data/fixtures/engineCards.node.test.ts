...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 4576 fixtures: 4437 taken by name, 132 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(4576);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(132);
  });
