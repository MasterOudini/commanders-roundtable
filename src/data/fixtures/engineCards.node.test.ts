...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 8941 fixtures: 8678 taken by name, 263 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(8941);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(263);
  });
