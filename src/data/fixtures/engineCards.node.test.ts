...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 1,668 fixtures: 1,580 taken by name, 88 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(1668);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(88);
  });
