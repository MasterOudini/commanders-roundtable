...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 3792 fixtures: 3672 taken by name, 113 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(3792);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(113);
  });
