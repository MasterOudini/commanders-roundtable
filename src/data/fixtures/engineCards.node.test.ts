...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 3838 fixtures: 3712 taken by name, 119 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(3838);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(119);
  });
