...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 8039 fixtures: 7864 taken by name, 175 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(8039);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(175);
  });
