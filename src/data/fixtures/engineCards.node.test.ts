...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 232 fixtures: 213 taken by name, 19 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(232);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(19);
  });
