...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 410 fixtures: 366 taken by name, 44 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(410);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(44);
  });
