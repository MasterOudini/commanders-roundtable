...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 7867 fixtures: 7696 taken by name, 171 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(7867);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(171);
  });
