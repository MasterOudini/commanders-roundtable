...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 3769 fixtures: 3650 taken by name, 112 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(3769);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(112);
  });
