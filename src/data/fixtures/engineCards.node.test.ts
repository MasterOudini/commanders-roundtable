...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 4177 fixtures: 4047 taken by name, 123 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(4177);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(123);
  });
