...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 7232 fixtures: 7067 taken by name, 158 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(7232);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(158);
  });
