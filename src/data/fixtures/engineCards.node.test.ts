...
describe.skipIf(!HAVE_DB)('the committed engine fixtures still match the real cards', () => {
   ...
   */
  test('there are 8257 fixtures: 8031 taken by name, 226 tokens pinned by printing', () => {
    expect(ENGINE_CARDS).toHaveLength(8257);
    expect(ENGINE_CARDS.filter((c) => c.layout === 'token')).toHaveLength(226);
  });
