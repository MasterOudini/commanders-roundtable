...
describe.skipIf(!HAVE_DB)('what each primitive is worth', () => {
  ...
  test('the first four primitives multiply what is scriptable by 3', () => {
    ...
    expect(steps.map((s) => s.unlocked)).toEqual([1298, 1397, 3190, 5104, 6316]);
...
replacement split: ${JSON.stringify(split)}  (tapped LANDS: ${tappedLands})`);
  ...
  test('what a script can express today, and what the engine still runs', () => {
    ...
    expect(steps.map((s) => s.unlocked)).toEqual([1298, 1397]);
    expect(r.complete).toBe(3755);
  });
});
