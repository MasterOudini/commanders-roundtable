...
describe.skipIf(!HAVE_DB)('what each primitive is worth', () => {
  ...
  test('the build order is what it is', () => {
    expect.soft({
      ...
      // 1,761 → 1,774.
      // M6.4hp (D383): the SCOPED BOARD EFFECT, 7,974 -> 8,035 - and only 27 of the 61 are the
  ...
  test('the first four primitives multiply what is scriptable by 3', () => {
    ...
    expect.soft(steps.map((s) => s.unlocked)).toEqual([1946, 1961, 3258, 4722, 5860]);
...
replacement split: ${JSON.stringify(split)}  (tapped LANDS: ${tappedLands})`);
    ...
    // D441 - 19 -> 2: the seventeen reveal lands are the engine's own (an asked enters-tapped condition).
    expect.soft(tappedLands).toBe(2);
  });
  ...
  test('the layer6 bucket is four different things', () => {
    ...
    expect.soft(split).toEqual({ grant: 545, anthem: 105, restriction: 171, conditional: 88, unclaimed: 0 });
    ...
    expect.soft(temporary).toBe(386);
  });
  ...
  test('the effect:token bucket, by who asks and by what is asked for', () => {
    ...
    expect.soft(byOwner).toEqual({ spell: 275, permanent: 489 });
  ...
  test('what a script can express today, and what the engine still runs', () => {
    ...
    expect.soft(steps.map((s) => s.unlocked)).toEqual([1946, 1961]);
    expect.soft(r.complete).toBe(12425);
  });
});
...
describe('a "you may" line is only `optional` if that is all it needs', () => {
  ...
  test('and when it is not, the line says what it is really waiting on', () => {
    // ⚠️ THIS LINE HAS MOVED TWICE, and both moves are the test working. D357 read the search
    // and it went `effect:search` -> `optional`: the yes/no was all that was left. D359 reads
    // the yes/no too - `you may search` is one sentence to the vocabulary now, with a decline
    // stage in front of it - so nothing is missing and the row is `scriptable`.
    expect.soft(
  ...
  test('and an unreadable one is residue rather than a tick', () => {
    // D436 - the graveyard-card exile that stood here is read now (the graveyard-card target); a shuffle-in is not.
    expect.soft(of('When this creature enters, you may have each player shuffle their graveyard into their library.')).toBe('unclassified');
  });
});
...
describe('a spell face is scriptable by the seam unless the line is structural (D191)', () => {
  const of = (text: string, spellFace: boolean): Primitive =>
    primitiveFor({ text, kind: 'sentence', raw: text }, 'Test Card', spellFace);

  test('an unreadable-but-expressible spell line is scriptable — Fruition, the shipped proof', () => {
    expect.soft(of('You gain 1 life for each tapped Forest on the battlefield.', true)).toBe('scriptable');
  });

  test('the SAME line on a permanent face is untouched — nothing moves for permanents', () => {
    expect.soft(of('You gain 1 life for each tapped Forest on the battlefield.', false)).toBe('unclassified');
  });
  ...
  test('a structural line never comes out scriptable, spell face or not', () => {
    ...
    // Randomness in resolution: a die roll is not wired through the seam (D534 - the coin flip is: its RNG draw rides
    // `rngAfter`, so the example is the die now).
    expect.soft(of('Roll a six-sided die.', true)).not.toBe('scriptable');
  ...
  test('a RULES row caught above keeps its claim — only the residue spills', () => {
    // ⚠️ THE POINT IS UNCHANGED AND THE EXAMPLE HAS MOVED TWICE. A RULES row caught above still
    // keeps its claim instead of spilling to `scriptable` - but the search is no longer an
    // example of one, because D357 gave it a verb and D359 gave it its `you may`. The line
    // below is: a `may` over a payload nothing reads, on a spell face, is `optional` and does
    // not spill.
    expect.soft(
      of('You may return target creature card from your graveyard to your hand.', true),
    ).toBe('optional');
    // ⚠️ And the line that moved, kept here as the proof that it moved: the same sentence that
    // was `optional` for D357 and D358 is `scriptable` now, on a spell face too.
    expect.soft(
