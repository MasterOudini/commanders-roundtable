  ...
  for (let i = 0; i < count; i++) {
    ...
    if (i % 4 === 0) {
      out.push({
        ...
        requiresTap: true,
        conditional: false,
        flexibilityRank: 0, snow: false, restriction: null
      });
    } else if (i % 4 === 1) {
      out.push({
        card: `c${i}`,
        ...
        requiresTap: true,
        conditional: false,
        flexibilityRank: 2, snow: false, restriction: null
      });
    } else if (i % 4 === 2) {
      out.push({
        card: `c${i}`,
        ...
        requiresTap: true,
        conditional: false,
        flexibilityRank: 4, snow: false, restriction: null
      });
    } else {
      out.push({
        card: `c${i}`,
        ...
        requiresTap: true,
        conditional: false,
        flexibilityRank: 6, snow: false, restriction: null
      });
    }
  }
  return out;
...
function input(sources: ManaSource[]): SolveInput {
  return { pool: EMPTY_POOL, poolSnow: EMPTY_POOL, poolRestricted: [], sources, lifeAvailable: 40, eventCount: 1 };
}
