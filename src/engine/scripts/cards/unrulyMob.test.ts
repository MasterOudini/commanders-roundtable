// `Unruly Mob` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNRULY_MOB_SCRIPT } from './unrulyMob';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unruly Mob";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Unruly Mob", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNRULY_MOB_SCRIPT]),
  });
  holdEverywhere(g);
