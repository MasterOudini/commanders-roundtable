// `Trinket Mage` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trinket Mage", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRINKET_MAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Trinket Mage", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for an artifact card with mana value 1 or less, reveal that card, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
