// `Spikeshot Goblin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIKESHOT_GOBLIN_SCRIPT } from './spikeshotGoblin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spikeshot Goblin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spikeshot Goblin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPIKESHOT_GOBLIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spikeshot Goblin", () => {
  test("{R}, {T}: the vocabulary resolves \"~ deals damage equal to its power to any target.\"", () => {
