// `Charmed Sleep` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHARMED_SLEEP_SCRIPT } from './charmedSleep';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Charmed Sleep";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Charmed Sleep", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHARMED_SLEEP_SCRIPT]),
  });
  holdEverywhere(g);
