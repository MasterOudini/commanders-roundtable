// `Obelisk of Alara` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OBELISK_OF_ALARA_SCRIPT } from './obeliskOfAlara';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Obelisk of Alara";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([OBELISK_OF_ALARA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Obelisk of Alara"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OBELISK_OF_ALARA_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 4) {
    ...
    must(g.submit({ t: 'ActivateAbility', player: 'p1', card: self, abilityIndex: 4 }));
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'chooseTargets', 20_000);
    must(g.submit({ t: 'ChooseTargets', player: 'p1', targets: [{ kind: 'card', id: no }] }));
    settle(g);
    }
...
describe("Obelisk of Alara", () => {
  test("{1}{W}, {T}: 5 life is gained", () => {
    const { g, self, life0 } = armed(0);
    expect(g.state.players.p1?.life).toBe(life0 + 5);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{1}{U}, {T}: a card is drawn, then one discarded", () => {
    const { g, self, hand0, gy0 } = armed(1);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0);
    expect((g.state.zones.graveyard.p1 ?? []).length).toBe(gy0 + 1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{1}{B}, {T}: the declared creature is pumped until end of turn", () => {
    const { g, self, no } = armed(2);
    expect(pt(g, no)).toEqual([3, 0]);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{1}{R}, {T}: it deals 3 damage to the declared player", () => {
    const { g, self, p2life0 } = armed(3);
    expect(g.state.players.p2?.life).toBe(p2life0 - 3);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{1}{G}, {T}: the declared creature is pumped until end of turn", () => {
    const { g, self, no } = armed(4);
    expect(pt(g, no)).toEqual([9, 6]);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });
