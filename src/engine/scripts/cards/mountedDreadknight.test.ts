// `Mounted Dreadknight` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOUNTED_DREADKNIGHT_SCRIPT } from './mountedDreadknight';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mounted Dreadknight";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MOUNTED_DREADKNIGHT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mounted Dreadknight"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOUNTED_DREADKNIGHT_SCRIPT]),
  });
  holdEverywhere(g);
