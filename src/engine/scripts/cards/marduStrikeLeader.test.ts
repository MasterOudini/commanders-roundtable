// `Mardu Strike Leader` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARDU_STRIKE_LEADER_SCRIPT } from './marduStrikeLeader';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mardu Strike Leader";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mardu Strike Leader"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARDU_STRIKE_LEADER_SCRIPT]),
