// `Warren Instigator` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Warren Instigator", "Mons's Goblin Raiders", "Mons's Goblin Raiders"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WARREN_INSTIGATOR_SCRIPT]),
  ...
  if (which === 0) {
    { const inHand = (g.state.zones.hand.p1 ?? []).some((id) => nameOf(g, id) === "Mons's Goblin Raiders");
      const inLib = (g.state.zones.library.p1 ?? []).find((id) => nameOf(g, id) === "Mons's Goblin Raiders");
      if (!inHand) { expect(inLib, "Mons's Goblin Raiders is dealt for the hand put").toBeDefined(); must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: inLib as InstanceId, to: { kind: 'hand', player: 'p1' } })); } }
...
describe("Warren Instigator", () => {
  test("Whenever this creature deals damage to an opponent: the vocabulary resolves \"Put a Goblin creature card from your hand onto the battlefield.\"", () => {
    ...
    { const ask = g.state.priority.awaiting;
      ...
      const found = (g.state.zones.hand.p1 ?? []).find((id) => nameOf(g, id) === "Mons's Goblin Raiders");
      expect(found, "Mons's Goblin Raiders is in hand for the put").toBeDefined();
