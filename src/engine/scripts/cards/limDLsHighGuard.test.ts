// `Lim-Dûl's High Guard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIM_DLS_HIGH_GUARD_SCRIPT } from './limDLsHighGuard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lim-Dûl's High Guard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lim-Dûl's High Guard", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIM_DLS_HIGH_GUARD_SCRIPT]),
  });
  holdEverywhere(g);
