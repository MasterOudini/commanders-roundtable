// `Cabal Executioner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CABAL_EXECUTIONER_SCRIPT } from './cabalExecutioner';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cabal Executioner";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cabal Executioner"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CABAL_EXECUTIONER_SCRIPT]),
...
describe("Cabal Executioner", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Target player sacrifices a creature of target player's choice.\"", () => {
    const { g } = armed(0);
