// `Merfolk Seer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERFOLK_SEER_SCRIPT } from './merfolkSeer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Merfolk Seer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merfolk Seer", "Island", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERFOLK_SEER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Merfolk Seer", () => {
  test("When this creature dies: the vocabulary resolves \"You may pay {1}{U}. If you do, draw a card.\" - the price is declined", () => {
  ...
  test("When this creature dies: the vocabulary resolves \"You may pay {1}{U}. If you do, draw a card.\" - the price is paid", () => {
    const { g, payL0_p1_0, payL0_p1_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
