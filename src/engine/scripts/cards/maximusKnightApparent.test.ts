// `Maximus, Knight Apparent` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAXIMUS_KNIGHT_APPARENT_SCRIPT } from './maximusKnightApparent';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Maximus, Knight Apparent";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Maximus, Knight Apparent", "Sol Ring", "+2 Mace"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAXIMUS_KNIGHT_APPARENT_SCRIPT]),
...
describe("Maximus, Knight Apparent", () => {
  test("When Maximus enters: the vocabulary resolves \"Search your library for an Equipment card with mana value 2, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "+2 Mace" && g.state.cards[id as InstanceId]?.zone.kind === 'hand' && g.state.cards[id as InstanceId]?.zone.player === 'p1');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "+2 Mace");
      expect(found, "+2 Mace is not in the library for the search to find").toBeDefined();
  ...
  test("{1}, Sacrifice an artifact: the vocabulary resolves \"You get {E}{E}.\"", () => {
    const { g, energy0, fodder0 } = armed(1);
