// `Collector's Vault` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COLLECTORS_VAULT_SCRIPT } from './collectorsVault';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Collector's Vault";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Collector's Vault"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COLLECTORS_VAULT_SCRIPT]),
