// `Terra Stomper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TERRA_STOMPER_SCRIPT } from './terraStomper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Terra Stomper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Terra Stomper"], ["Cyclops of One-Eyed Pass", "Counterspell"]],
    scripts: createRegistry([TERRA_STOMPER_SCRIPT]),
  });
  holdEverywhere(g);
