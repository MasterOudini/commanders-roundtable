// `Monstrous Carabid` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MONSTROUS_CARABID_SCRIPT } from './monstrousCarabid';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Monstrous Carabid";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Monstrous Carabid", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MONSTROUS_CARABID_SCRIPT]),
  });
  holdEverywhere(g);
