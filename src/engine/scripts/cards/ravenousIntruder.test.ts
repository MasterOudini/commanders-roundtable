// `Ravenous Intruder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAVENOUS_INTRUDER_SCRIPT } from './ravenousIntruder';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ravenous Intruder";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RAVENOUS_INTRUDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ravenous Intruder", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAVENOUS_INTRUDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ravenous Intruder", () => {
  test("Sacrifice an artifact: it gets +2/+2 until end of turn", () => {
