// `Stromgald Cabal` - counter on "Counter target white spell": the adjective is the parser's and the
// validator's (D294). Generated from one table row (D295).

import { STROMGALD_CABAL } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(STROMGALD_CABAL, "{T}, Pay 1 life: Counter target white spell.");
const TEXT = PRINTED;

export const STROMGALD_CABAL_SCRIPT: CardScript = {
  oracleId: STROMGALD_CABAL.oracleId,
  name: STROMGALD_CABAL.name,
  activated: [
    {
      ref: `${STROMGALD_CABAL.oracleId}#a0`,
      text: TEXT,
