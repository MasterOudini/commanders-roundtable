// `Fire Bowman` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIRE_BOWMAN_SCRIPT } from './fireBowman';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fire Bowman";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fire Bowman"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIRE_BOWMAN_SCRIPT]),
  });
  holdEverywhere(g);
