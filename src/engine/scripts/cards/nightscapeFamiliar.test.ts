// `Nightscape Familiar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIGHTSCAPE_FAMILIAR_SCRIPT } from './nightscapeFamiliar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nightscape Familiar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nightscape Familiar", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIGHTSCAPE_FAMILIAR_SCRIPT]),
  });
  holdEverywhere(g);
