// `Gift of Strands` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GIFT_OF_STRANDS_SCRIPT } from './giftOfStrands';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gift of Strands";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GIFT_OF_STRANDS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gift of Strands", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GIFT_OF_STRANDS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gift of Strands", () => {
  test("When this Aura enters: scry 2 asks, and the cards stay on top", () => {
