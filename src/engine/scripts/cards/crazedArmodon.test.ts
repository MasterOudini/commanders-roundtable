// `Crazed Armodon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRAZED_ARMODON_SCRIPT } from './crazedArmodon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crazed Armodon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crazed Armodon", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRAZED_ARMODON_SCRIPT]),
...
describe("Crazed Armodon", () => {
  test("{G}: the vocabulary resolves \"~ gets +3/+0 and gains trample until end of turn. Destroy this creature at the beginning of the next end step.\"", () => {
