// `Web Up` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WEB_UP_SCRIPT } from './webUp';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Web Up"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WEB_UP_SCRIPT]),
  });
  holdEverywhere(g);
