// `Goblin Raider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_RAIDER_SCRIPT } from './goblinRaider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Raider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Raider", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_RAIDER_SCRIPT]),
  });
  holdEverywhere(g);
