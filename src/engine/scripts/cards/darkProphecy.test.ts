// `Dark Prophecy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DARK_PROPHECY_SCRIPT } from './darkProphecy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dark Prophecy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dark Prophecy", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DARK_PROPHECY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dark Prophecy", () => {
  test("Whenever a creature you control dies: a card is drawn and 1 life lost", () => {
