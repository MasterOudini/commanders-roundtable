// `Fang Skulkin` - the named target is accepted and the pump lands; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { FANG_SKULKIN_SCRIPT } from './fangSkulkin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fang Skulkin";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Fang Skulkin"], ["Vampire Nighthawk", "Grizzly Bears"]],
    scripts: createRegistry([FANG_SKULKIN_SCRIPT]),
  });
  holdEverywhere(g);
