// `Geyser Glider` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GEYSER_GLIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Geyser Glider"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GEYSER_GLIDER_SCRIPT]),
  });
  holdEverywhere(g);
