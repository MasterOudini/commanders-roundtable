// `Cave Tiger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAVE_TIGER_SCRIPT } from './caveTiger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cave Tiger";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CAVE_TIGER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cave Tiger"], ["Cyclops of One-Eyed Pass", "Memnite"]],
    scripts: createRegistry([CAVE_TIGER_SCRIPT]),
  });
  holdEverywhere(g);
