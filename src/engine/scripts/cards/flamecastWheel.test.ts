// `Flamecast Wheel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLAMECAST_WHEEL_SCRIPT } from './flamecastWheel';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flamecast Wheel";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flamecast Wheel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLAMECAST_WHEEL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Flamecast Wheel", () => {
  test("{5}, {T}, Sacrifice this artifact: it deals 3 damage to the declared creature", () => {
