// `Thraben Doomsayer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THRABEN_DOOMSAYER_SCRIPT } from './thrabenDoomsayer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thraben Doomsayer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([THRABEN_DOOMSAYER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thraben Doomsayer", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THRABEN_DOOMSAYER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thraben Doomsayer", () => {
  ...
  test("`Thraben Doomsayer` - as long as you have 5 or less life: absent while it is unmet, read once it holds", () => {
    const { g, self, no, yes, condOff } = armed(1);
