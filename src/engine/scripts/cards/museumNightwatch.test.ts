// `Museum Nightwatch` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MUSEUM_NIGHTWATCH_SCRIPT } from './museumNightwatch';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Museum Nightwatch";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Museum Nightwatch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MUSEUM_NIGHTWATCH_SCRIPT]),
