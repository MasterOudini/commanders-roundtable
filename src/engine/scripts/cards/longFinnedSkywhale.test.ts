// `Long-Finned Skywhale` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LONG_FINNED_SKYWHALE_SCRIPT } from './longFinnedSkywhale';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Long-Finned Skywhale";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Long-Finned Skywhale", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LONG_FINNED_SKYWHALE_SCRIPT]),
  });
  holdEverywhere(g);
