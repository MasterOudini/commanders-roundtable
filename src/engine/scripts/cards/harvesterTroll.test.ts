// `Harvester Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HARVESTER_TROLL_SCRIPT } from './harvesterTroll';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Harvester Troll";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Harvester Troll", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HARVESTER_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Harvester Troll", () => {
  test("When this creature enters: the vocabulary resolves \"You may sacrifice a creature or land. If you do, put two +1/+1 counters on this creature.\" - the price is declined", () => {
  ...
  test("When this creature enters: the vocabulary resolves \"You may sacrifice a creature or land. If you do, put two +1/+1 counters on this creature.\" - the price is paid", () => {
