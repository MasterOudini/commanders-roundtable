// `Goblin Burrows` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_BURROWS_SCRIPT } from './goblinBurrows';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Burrows";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOBLIN_BURROWS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Burrows"], ["Cyclops of One-Eyed Pass", "Mons's Goblin Raiders"]],
    scripts: createRegistry([GOBLIN_BURROWS_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtA1_0 = put(g, 'p2', "Mons's Goblin Raiders");
  settle(g);
...
describe("Goblin Burrows", () => {
  test("{1}{R}, {T}: the vocabulary resolves \"Target Goblin creature gets +2/+0 until end of turn.\"", () => {
    const { g, self, vtA1_0 } = armed(0);
    expect(pt(g, vtA1_0)).toEqual([3, 1]);
