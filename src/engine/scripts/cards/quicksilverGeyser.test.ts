// `Quicksilver Geyser` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { QUICKSILVER_GEYSER_SCRIPT } from './quicksilverGeyser';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Quicksilver Geyser";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Quicksilver Geyser"], ["Grizzly Bears", "Vampire Nighthawk", "Forest"]],
    scripts: createRegistry([QUICKSILVER_GEYSER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Quicksilver Geyser", () => {
  test('the named targets are accepted and the effect lands', () => {
    ...
    expect(g.state.cards[targets[1] as InstanceId]?.zone.kind).toBe('hand');
  });
