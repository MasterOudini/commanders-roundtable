// `Street Riot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STREET_RIOT_SCRIPT } from './streetRiot';
import { advanceUntil, deps, holdEverywhere, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Street Riot";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; yes: InstanceId; condOff: boolean };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STREET_RIOT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Street Riot", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STREET_RIOT_SCRIPT]),
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, yes, condOff };
}

describe("Street Riot", () => {
  test("`Street Riot` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
    const { g, no, yes, condOff } = armed(0);
