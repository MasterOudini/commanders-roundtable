// `Claim of Erebos` - the permanent it is attached to HAS a quoted
...
import { CLAIM_OF_EREBOS } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(CLAIM_OF_EREBOS, "Enchant creature\nEnchanted creature has \"{1}{B}, {T}: Target player loses 2 life.\"");
const LINES = PRINTED.split('\n');

const GRANT = grantedActivated("{1}{B}, {T}: Target player loses 2 life.", `${CLAIM_OF_EREBOS.oracleId}#g1`, CLAIM_OF_EREBOS.name);
const VOCAB = vocabularyEffects("Target player loses 2 life.", CLAIM_OF_EREBOS.name);
const VOCAB_T = vocabularyTargets("Target player loses 2 life.");

export const CLAIM_OF_EREBOS_SCRIPT: CardScript = {
  oracleId: CLAIM_OF_EREBOS.oracleId,
  name: CLAIM_OF_EREBOS.name,
  statics: [
    {
      abilityId: 'grant-1',
