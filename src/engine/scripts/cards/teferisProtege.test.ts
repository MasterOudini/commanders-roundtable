// `Teferi's Protege` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEFERIS_PROTEGE_SCRIPT } from './teferisProtege';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Teferi's Protege";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Teferi's Protege"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEFERIS_PROTEGE_SCRIPT]),
  });
  holdEverywhere(g);
