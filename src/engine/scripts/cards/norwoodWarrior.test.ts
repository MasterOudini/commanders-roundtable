// `Norwood Warrior` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NORWOOD_WARRIOR_SCRIPT } from './norwoodWarrior';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Norwood Warrior";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NORWOOD_WARRIOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Norwood Warrior"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NORWOOD_WARRIOR_SCRIPT]),
  });
  holdEverywhere(g);
