// `Yahenni, Undying Partisan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { YAHENNI_UNDYING_PARTISAN_SCRIPT } from './yahenniUndyingPartisan';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Yahenni, Undying Partisan";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([YAHENNI_UNDYING_PARTISAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Yahenni, Undying Partisan", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YAHENNI_UNDYING_PARTISAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Yahenni, Undying Partisan", () => {
  test("Whenever a creature an opponent controls dies: it gets 1 +1/+1 counter", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(1);
  });

  test("Sacrifice another creature: it gains indestructible until end of turn", () => {
