// `Ravenous Daggertooth` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAVENOUS_DAGGERTOOTH_SCRIPT } from './ravenousDaggertooth';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ravenous Daggertooth";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ravenous Daggertooth", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([RAVENOUS_DAGGERTOOTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ravenous Daggertooth", () => {
  test("Enrage — Whenever this creature is dealt damage: 2 life is gained", () => {
