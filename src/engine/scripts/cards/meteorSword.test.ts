// `Meteor Sword` - every printed ability proven in its own game: the cost's mark, the pump
...
import { METEOR_SWORD_SCRIPT } from './meteorSword';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Meteor Sword";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([METEOR_SWORD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Meteor Sword", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([METEOR_SWORD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Meteor Sword", () => {
  test("When this Equipment enters: the vocabulary resolves \"Destroy target permanent.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
  });

  test("Equipped creature gets +3/+3.: the creature it is attached to reads it", () => {
