// `Corpses of the Lost` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CORPSES_OF_THE_LOST_SCRIPT } from './corpsesOfTheLost';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Corpses of the Lost";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CORPSES_OF_THE_LOST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Corpses of the Lost", "Skeletal Snake", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CORPSES_OF_THE_LOST_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Skeletal Snake");
...
describe("Corpses of the Lost", () => {
  test("Skeletons you control get +1/+0 and have haste.: only the Skeleton reads it", () => {
  ...
  test("At the beginning of your end step: the vocabulary resolves \"You may pay 1 life. If you do, return this enchantment to its owner's hand.\" - the price is declined", () => {
    const { g } = armed(2);
  ...
  test("At the beginning of your end step: the vocabulary resolves \"You may pay 1 life. If you do, return this enchantment to its owner's hand.\" - the price is paid", () => {
