// `Sixth Sense` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SIXTH_SENSE_SCRIPT } from './sixthSense';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sixth Sense";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sixth Sense", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIXTH_SENSE_SCRIPT]),
...
describe("Sixth Sense", () => {
  test("Enchanted creature has \"Whenever this creature deals combat damage to a player, you may draw a card.\": the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
  });

  test("Enchanted creature has \"Whenever this creature deals combat damage to a player: a card is drawn [combatDamagePlayer]", () => {
