// `Marang River Skeleton` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARANG_RIVER_SKELETON_SCRIPT } from './marangRiverSkeleton';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Marang River Skeleton";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Marang River Skeleton", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARANG_RIVER_SKELETON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Marang River Skeleton", () => {
