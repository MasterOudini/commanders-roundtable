// `Summon: Fat Chocobo` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUMMON_FAT_CHOCOBO_SCRIPT } from './summonFatChocobo';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Summon: Fat Chocobo";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUMMON_FAT_CHOCOBO_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Summon: Fat Chocobo", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUMMON_FAT_CHOCOBO_SCRIPT]),
...
describe("Summon: Fat Chocobo", () => {
  test("I — Wark — Create a 2/2 green Bird creature token with \"Whenever a land you control enters: the vocabulary resolves \"Create a 2/2 green Bird creature token with \\\"Whenever a land you control enters, this token gets +1/+0 until end of turn.\\\"\"", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("II: its controller's creatures gain trample until end of turn", () => {
