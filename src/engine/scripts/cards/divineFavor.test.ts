// `Divine Favor` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: gain 3 life); gets +1/+3;
...
import { DIVINE_FAVOR_SCRIPT } from './divineFavor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Divine Favor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DIVINE_FAVOR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([DIVINE_FAVOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Divine Favor", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: gain 3 life); gets +1/+3; Cyclops of One-Eyed Pass is untouched", () => {
