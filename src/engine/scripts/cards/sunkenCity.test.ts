// `Sunken City` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUNKEN_CITY_SCRIPT } from './sunkenCity';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunken City";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; yes: InstanceId; payL0_p1_0: InstanceId; payL0_p1_1: InstanceId };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUNKEN_CITY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sunken City", "Coral Eel", "Island", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUNKEN_CITY_SCRIPT]),
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Coral Eel");
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const payL0_p1_0 = put(g, 'p1', "Island");
  const payL0_p1_1 = put(g, 'p1', "Island");
  settle(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, yes, payL0_p1_0, payL0_p1_1 };
}

describe("Sunken City", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this enchantment unless you pay {U}{U}.\" - the price is declined", () => {
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this enchantment unless you pay {U}{U}.\" - the price is paid", () => {
  ...
  test("Blue creatures get +1/+1.: only the blue creature reads it", () => {
