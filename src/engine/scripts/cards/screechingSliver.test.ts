// `Screeching Sliver` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCREECHING_SLIVER_SCRIPT } from './screechingSliver';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SCREECHING_SLIVER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Screeching Sliver", "Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCREECHING_SLIVER_SCRIPT]),
...
describe("Screeching Sliver", () => {
  test("All Slivers have \"{T}: only the Sliver reads it", () => {
    const { g, no } = armed(0);
    expect(pt(g, no)).toEqual([5, 2]);
  });

  test("All Slivers have \"{T}: the vocabulary resolves \"Target player mills a card.\"", () => {
    const { g, yes, p2gy0 } = armed(1);
