// `Cateran Overlord` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cateran Overlord", "Grizzly Bears", "Infernal Grasp", "Rampart Crawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CATERAN_OVERLORD_SCRIPT]),
...
describe("Cateran Overlord", () => {
  ...
  test("{6}, {T}: the vocabulary resolves \"Search your library for a Mercenary permanent card with mana value 6 or less, put it onto the battlefield, then shuffle.\"", () => {
    const { g, self, board0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Rampart Crawler" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Rampart Crawler");
      expect(found, "Rampart Crawler is not in the library for the search to find").toBeDefined();
