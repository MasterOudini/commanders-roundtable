// `Leonin Snarecaster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LEONIN_SNARECASTER_SCRIPT } from './leoninSnarecaster';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Leonin Snarecaster";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Leonin Snarecaster"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LEONIN_SNARECASTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Leonin Snarecaster", () => {
  test("When this creature enters: the vocabulary resolves \"Tap target creature.\"", () => {
    const { g, no } = armed(0);
