// `Spike Rogue` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIKE_ROGUE_SCRIPT } from './spikeRogue';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spike Rogue";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; cnt0: number; pt0: [number | null, number | null]; donor0: InstanceId };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPIKE_ROGUE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spike Rogue", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPIKE_ROGUE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 2) {
    cnt0 = g.state.cards[self]?.counters["+1/+1"] ?? 0;
    must(g.submit({ t: 'ManualAddMana', player: 'p1', target: 'p1', symbol: 'C', amount: 2 }));
    must(g.submit({ t: 'ActivateAbility', player: 'p1', card: self, abilityIndex: 1, removeCounter: [donor0] }));
    settle(g);
    }
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, cnt0, pt0, donor0 };
}

describe("Spike Rogue", () => {
  ...
  test("{2}, Remove a +1/+1 counter from a creature you control: it gets 1 +1/+1 counter", () => {
