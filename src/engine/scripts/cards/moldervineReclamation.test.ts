// `Moldervine Reclamation` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOLDERVINE_RECLAMATION_SCRIPT } from './moldervineReclamation';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Moldervine Reclamation";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Moldervine Reclamation", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOLDERVINE_RECLAMATION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Moldervine Reclamation", () => {
  test("Whenever a creature you control dies: the vocabulary resolves \"You gain 1 life and draw a card.\"", () => {
