// `Outland Colossus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OUTLAND_COLOSSUS_SCRIPT } from './outlandColossus';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Outland Colossus";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([OUTLAND_COLOSSUS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Outland Colossus", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([OUTLAND_COLOSSUS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Outland Colossus", () => {
  test("Renown 6 (When this creature deals combat damage to a player: dealing combat damage to a player, it gets 6 +1/+1 counters and becomes renowned", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.counters['+1/+1'] ?? 0).toBe(6);
    expect(g.state.cards[self]?.renowned).toBe(true);
    expect(pt(g, self)).toEqual([12, 12]);
  });
