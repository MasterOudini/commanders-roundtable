// `Ogre's Cleaver` - equipped to Grizzly Bears the creature gets +5/+0; the Cyclops is untouched;
...
import { OGRES_CLEAVER_SCRIPT } from './ogresCleaver';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ogre's Cleaver";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([OGRES_CLEAVER_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([OGRES_CLEAVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ogre's Cleaver", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +5/+0; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([7, 2]);
