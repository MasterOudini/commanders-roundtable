// `Shuko` - equipped to Grizzly Bears the creature gets +1/+0; the Cyclops is untouched;
...
import { SHUKO_SCRIPT } from './shuko';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHUKO_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SHUKO_SCRIPT]),
  });
  holdEverywhere(g);
