// `Skyshroud Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKYSHROUD_TROLL_SCRIPT } from './skyshroudTroll';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skyshroud Troll";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skyshroud Troll", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKYSHROUD_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
