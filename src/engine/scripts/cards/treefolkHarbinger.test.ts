// `Treefolk Harbinger` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Treefolk Harbinger", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TREEFOLK_HARBINGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Treefolk Harbinger", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a Treefolk or Forest card, reveal it, then shuffle and put that card on top.\"", () => {
    const { g } = armed(0);
