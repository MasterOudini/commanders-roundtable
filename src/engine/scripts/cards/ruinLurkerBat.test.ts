// `Ruin-Lurker Bat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUIN_LURKER_BAT_SCRIPT } from './ruinLurkerBat';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ruin-Lurker Bat";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ruin-Lurker Bat", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUIN_LURKER_BAT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ruin-Lurker Bat", () => {
  test("At the beginning of your end step: scry 1 asks, and the cards stay on top", () => {
