// `Drowned` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DROWNED_SCRIPT } from './drowned';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drowned", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DROWNED_SCRIPT]),
  });
  holdEverywhere(g);
