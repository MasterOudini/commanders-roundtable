// `Vault Guardsman` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VAULT_GUARDSMAN_SCRIPT } from './vaultGuardsman';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vault Guardsman";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vault Guardsman"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VAULT_GUARDSMAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vault Guardsman", () => {
  test("When this creature enters: the vocabulary resolves \"Exile target artifact or creature an opponent controls until this creature leaves the battlefield.\"", () => {
