// `Retreat to Hagra` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RETREAT_TO_HAGRA_SCRIPT } from './retreatToHagra';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Retreat to Hagra";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RETREAT_TO_HAGRA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Retreat to Hagra"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RETREAT_TO_HAGRA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Retreat to Hagra", () => {
  test("Landfall — Whenever a land you control enters: choosing \"Target creature gets +1/+0 and gains deathtouch until end of turn.\", pumpTarget", () => {
    const { g, no } = armed(0);
    expect(pt(g, no)).toEqual([6, 2]);
    expect(kw(g, no).has("deathtouch")).toBe(true);
  });

  test("Landfall — Whenever a land you control enters: choosing \"Each opponent loses 1 life and you gain 1 life.\", drain", () => {
