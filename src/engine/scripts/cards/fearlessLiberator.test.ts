// `Fearless Liberator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FEARLESS_LIBERATOR_SCRIPT } from './fearlessLiberator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fearless Liberator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fearless Liberator", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FEARLESS_LIBERATOR_SCRIPT]),
...
describe("Fearless Liberator", () => {
  test("Boast — {2}{R}: 1 token made - refused unless this creature attacked this turn", () => {
