// `Viashino Slaughtermaster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VIASHINO_SLAUGHTERMASTER_SCRIPT } from './viashinoSlaughtermaster';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Viashino Slaughtermaster";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VIASHINO_SLAUGHTERMASTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Viashino Slaughtermaster", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIASHINO_SLAUGHTERMASTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Viashino Slaughtermaster", () => {
  test("{B}{G}: it gets +1/+1 until end of turn", () => {
