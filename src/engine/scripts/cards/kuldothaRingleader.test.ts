// `Kuldotha Ringleader` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KULDOTHA_RINGLEADER_SCRIPT } from './kuldothaRingleader';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kuldotha Ringleader";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KULDOTHA_RINGLEADER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kuldotha Ringleader", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KULDOTHA_RINGLEADER_SCRIPT]),
  });
  holdEverywhere(g);
