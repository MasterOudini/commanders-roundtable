// `Michelangelo, Game Master` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MICHELANGELO_GAME_MASTER_SCRIPT } from './michelangeloGameMaster';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Michelangelo, Game Master";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Michelangelo, Game Master", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MICHELANGELO_GAME_MASTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Michelangelo, Game Master", () => {
  test("Disappear — At the beginning of your end step: it gets 1 +1/+1 counter", () => {
