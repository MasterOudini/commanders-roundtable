// `Honored Heirloom` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HONORED_HEIRLOOM_SCRIPT } from './honoredHeirloom';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Honored Heirloom";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Honored Heirloom"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([HONORED_HEIRLOOM_SCRIPT]),
  });
  holdEverywhere(g);
