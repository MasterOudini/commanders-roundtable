// `Cephalid Inkshrouder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CEPHALID_INKSHROUDER_SCRIPT } from './cephalidInkshrouder';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cephalid Inkshrouder";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CEPHALID_INKSHROUDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cephalid Inkshrouder", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CEPHALID_INKSHROUDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cephalid Inkshrouder", () => {
  test("Discard a card: it gains shroud until end of turn and can't be blocked this turn", () => {
