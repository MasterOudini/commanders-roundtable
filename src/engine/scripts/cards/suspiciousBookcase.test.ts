// `Suspicious Bookcase` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUSPICIOUS_BOOKCASE_SCRIPT } from './suspiciousBookcase';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Suspicious Bookcase";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Suspicious Bookcase"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUSPICIOUS_BOOKCASE_SCRIPT]),
  });
  holdEverywhere(g);
