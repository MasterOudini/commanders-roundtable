// `Wild Nacatl` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WILD_NACATL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wild Nacatl", "Mountain", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WILD_NACATL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wild Nacatl", () => {
  test("`Wild Nacatl` - as long as you control a Mountain: absent while it is unmet, read once it holds", () => {
  ...
  test("`Wild Nacatl` - as long as you control a Plains: absent while it is unmet, read once it holds", () => {
