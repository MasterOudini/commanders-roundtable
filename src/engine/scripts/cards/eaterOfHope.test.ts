// `Eater of Hope` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EATER_OF_HOPE_SCRIPT } from './eaterOfHope';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Eater of Hope";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Eater of Hope", "Grizzly Bears", "Infernal Grasp", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EATER_OF_HOPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Eater of Hope", () => {
  test("{B}, Sacrifice another creature: it regenerates from the Bolt", () => {
  ...
  test("{2}{B}, Sacrifice two other creatures: the vocabulary resolves \"Destroy target creature.\"", () => {
