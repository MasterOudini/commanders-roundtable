// `Barbed Batterfist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BARBED_BATTERFIST_SCRIPT } from './barbedBatterfist';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Barbed Batterfist";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BARBED_BATTERFIST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Barbed Batterfist", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BARBED_BATTERFIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Barbed Batterfist", () => {
  ...
  test("Equipped creature gets +1/-1.: the creature it is attached to reads it", () => {
    ...
    expect(pt(g, bearsB)).toEqual([3, 1]);
  });
