// `Giant Spectacle` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+1 and has menace;
...
import { GIANT_SPECTACLE_SCRIPT } from './giantSpectacle';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Giant Spectacle";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GIANT_SPECTACLE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GIANT_SPECTACLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Giant Spectacle", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+1 and has menace; Cyclops of One-Eyed Pass is untouched", () => {
