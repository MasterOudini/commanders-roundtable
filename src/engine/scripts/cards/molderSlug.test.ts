// `Molder Slug` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOLDER_SLUG_SCRIPT } from './molderSlug';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Molder Slug";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Molder Slug"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOLDER_SLUG_SCRIPT]),
...
describe("Molder Slug", () => {
  test("At the beginning of each player's upkeep: the vocabulary resolves \"Target player sacrifices an artifact of target player's choice.\"", () => {
    const { g } = armed(0);
