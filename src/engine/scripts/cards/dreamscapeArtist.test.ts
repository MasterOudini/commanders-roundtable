// `Dreamscape Artist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DREAMSCAPE_ARTIST_SCRIPT } from './dreamscapeArtist';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dreamscape Artist";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; fodder0: InstanceId[]; disc0: InstanceId[] };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dreamscape Artist", "Forest", "Grizzly Bears", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DREAMSCAPE_ARTIST_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    ...
    must(g.submit({ t: 'ActivateAbility', player: 'p1', card: self, abilityIndex: 0, sacrifice: fodder0.slice(0, 1), discard: disc0 }));
    settle(g);
    }
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, fodder0, disc0 };
}

describe("Dreamscape Artist", () => {
  test("{2}{U}, {T}, Discard a card, Sacrifice a land: the vocabulary resolves \"Search your library for up to two basic land cards, put them onto the battlefield, then shuffle.\"", () => {
    const { g, self, board0, fodder0, disc0 } = armed(0);
