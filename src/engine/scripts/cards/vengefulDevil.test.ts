// `Vengeful Devil` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VENGEFUL_DEVIL_SCRIPT } from './vengefulDevil';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vengeful Devil";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vengeful Devil", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VENGEFUL_DEVIL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vengeful Devil", () => {
  test("Morbid — {T}: it deals 1 damage to the declared creature - refused unless if a creature died this turn", () => {
