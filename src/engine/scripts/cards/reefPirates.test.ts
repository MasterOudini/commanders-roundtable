// `Reef Pirates` - every printed ability proven in its own game: the cost's mark, the pump
...
import { REEF_PIRATES_SCRIPT } from './reefPirates';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Reef Pirates";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Reef Pirates"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([REEF_PIRATES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Reef Pirates", () => {
  test("Whenever this creature deals damage to an opponent: the vocabulary resolves \"Target player mills a card.\"", () => {
