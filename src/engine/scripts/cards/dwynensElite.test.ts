// `Dwynen's Elite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DWYNENS_ELITE_SCRIPT } from './dwynensElite';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dwynen's Elite";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dwynen's Elite", "Llanowar Elves"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DWYNENS_ELITE_SCRIPT]),
  });
  holdEverywhere(g);
