// `Mesmeric Trance` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mesmeric Trance", "Grizzly Bears", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MESMERIC_TRANCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mesmeric Trance", () => {
  test("{U}, Discard a card: a card is drawn", () => {
    const { g, hand0, disc0 } = armed(0);
