// `Corpse Connoisseur` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Corpse Connoisseur", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CORPSE_CONNOISSEUR_SCRIPT]),
...
describe("Corpse Connoisseur", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a creature card, put that card into your graveyard, then shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Grizzly Bears" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
