// `Triton Shorestalker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRITON_SHORESTALKER_SCRIPT } from './tritonShorestalker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Triton Shorestalker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Triton Shorestalker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRITON_SHORESTALKER_SCRIPT]),
  });
  holdEverywhere(g);
