// `Kiora's Follower` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KIORAS_FOLLOWER_SCRIPT } from './kiorasFollower';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kiora's Follower";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kiora's Follower"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KIORAS_FOLLOWER_SCRIPT]),
  });
  holdEverywhere(g);
