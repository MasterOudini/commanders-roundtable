// `Curse of Chains` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CURSE_OF_CHAINS_SCRIPT } from './curseOfChains';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Curse of Chains";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Curse of Chains", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CURSE_OF_CHAINS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Curse of Chains", () => {
  test("At the beginning of each upkeep: the enchanted creature is tapped", () => {
