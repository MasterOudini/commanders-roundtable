// `Consuming Fervor` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { CONSUMING_FERVOR_SCRIPT } from './consumingFervor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Consuming Fervor";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Consuming Fervor","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONSUMING_FERVOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Consuming Fervor", () => {
  test("grants At the beginning of your upkeep, put a -1/-1 counter on this creature. to Grizzly Bears, and to nothing outside the scope", () => {
