// `Farhaven Elf` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Farhaven Elf", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FARHAVEN_ELF_SCRIPT]),
  });
  holdEverywhere(g);
