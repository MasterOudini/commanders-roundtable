// `Ulvenwald Hydra` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ULVENWALD_HYDRA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ulvenwald Hydra", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ULVENWALD_HYDRA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ulvenwald Hydra", () => {
  test("Ulvenwald Hydra's power and toughness are each equal to the number of lands you control.: its power and toughness follow the count", () => {
    const { g, self, cda0 } = armed(0);
    expect(pt(g, self)).toEqual([cda0 + 1, cda0 + 1]);
  });

  test("When this creature enters: the vocabulary resolves \"Search your library for a land card, put it onto the battlefield tapped, then shuffle.\"", () => {
