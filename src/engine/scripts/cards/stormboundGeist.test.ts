// `Stormbound Geist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORMBOUND_GEIST_SCRIPT } from './stormboundGeist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stormbound Geist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stormbound Geist", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORMBOUND_GEIST_SCRIPT]),
  });
  holdEverywhere(g);
