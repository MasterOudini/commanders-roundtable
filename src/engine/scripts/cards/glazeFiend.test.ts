// `Glaze Fiend` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GLAZE_FIEND_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glaze Fiend", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLAZE_FIEND_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Glaze Fiend", () => {
  test("Whenever another artifact you control enters: it gets +2/+2 until end of turn", () => {
