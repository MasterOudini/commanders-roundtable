// `Tattermunge Maniac` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TATTERMUNGE_MANIAC_SCRIPT } from './tattermungeManiac';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tattermunge Maniac";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tattermunge Maniac", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TATTERMUNGE_MANIAC_SCRIPT]),
  });
  holdEverywhere(g);
