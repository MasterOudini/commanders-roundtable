// `Rathi Fiend` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rathi Fiend", "Rampart Crawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RATHI_FIEND_SCRIPT]),
...
describe("Rathi Fiend", () => {
  test("When this creature enters: the vocabulary resolves \"Each player loses 3 life.\"", () => {
    const { g, life0, p2life0 } = armed(0);
    expect(g.state.players.p1?.life).toBe(life0 + -3);
    expect(g.state.players.p2?.life).toBe(p2life0 + -3);
  });

  test("{3}, {T}: the vocabulary resolves \"Search your library for a Mercenary permanent card with mana value 3 or less, put it onto the battlefield, then shuffle.\"", () => {
