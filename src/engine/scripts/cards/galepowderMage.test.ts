// `Galepowder Mage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GALEPOWDER_MAGE_SCRIPT } from './galepowderMage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Galepowder Mage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Galepowder Mage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GALEPOWDER_MAGE_SCRIPT]),
...
describe("Galepowder Mage", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Exile another target creature. Return that card to the battlefield under its owner's control at the beginning of the next end step.\"", () => {
    const { g, no } = armed(0);
