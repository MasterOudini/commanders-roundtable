// `Phyrexian Colossus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHYREXIAN_COLOSSUS_SCRIPT } from './phyrexianColossus';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phyrexian Colossus";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phyrexian Colossus", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([PHYREXIAN_COLOSSUS_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 2) {
    ...
    must(g.submit({ t: 'DeclareBlockers', player: 'p2', blocks: [{ blocker: no, attacker: self }, { blocker: wc1, attacker: self }, { blocker: wc2, attacker: self }] }));
    settle(g);
    }
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, refused, bearsB };
}

describe("Phyrexian Colossus", () => {
  ...
  test("Pay 8 life: it untaps", () => {
    const { g, self, life0 } = armed(1);
    expect(g.state.cards[self]?.tapped).toBe(false);
    expect(g.state.players.p1?.life).toBe(life0 - 8);
  });

  test("This creature can't be blocked except by three or more creatures.: the block is refused", () => {
