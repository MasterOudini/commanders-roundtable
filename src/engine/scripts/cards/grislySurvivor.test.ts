// `Grisly Survivor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRISLY_SURVIVOR_SCRIPT } from './grislySurvivor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grisly Survivor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GRISLY_SURVIVOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grisly Survivor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRISLY_SURVIVOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Grisly Survivor", () => {
  test("Whenever you cycle or discard a card: it gets +2/+0 until end of turn", () => {
