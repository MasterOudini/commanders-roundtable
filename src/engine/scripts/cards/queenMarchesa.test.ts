// `Queen Marchesa` - every printed ability proven in its own game: the cost's mark, the pump
...
import { QUEEN_MARCHESA_SCRIPT } from './queenMarchesa';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Queen Marchesa";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Queen Marchesa"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([QUEEN_MARCHESA_SCRIPT]),
  ...
  if (which === 1) {
    if (g.state.monarch !== null) must(g.submit({ t: 'ManualSetMonarch', player: 'p1', target: null }));
    ...
    if (g.state.monarch !== "p2") must(g.submit({ t: 'ManualSetMonarch', player: 'p1', target: "p2" }));
      settle(g);
  ...
  if (which === 1) {
    advanceUntil(g, (s) => s.turn.turnNumber === 7 && s.turn.step === 'upkeep', 60000);
    settle(g);
    }
...
describe("Queen Marchesa", () => {
  test("When Queen Marchesa enters: its controller becomes the monarch", () => {
    const { g } = armed(0);
    expect(g.state.monarch).toBe('p1');
  });

  test("At the beginning of your upkeep: 1 token made", () => {
