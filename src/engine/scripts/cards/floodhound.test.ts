// `Floodhound` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLOODHOUND_SCRIPT } from './floodhound';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Floodhound";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Floodhound"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLOODHOUND_SCRIPT]),
  });
  holdEverywhere(g);
