// `Thistledown Players` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THISTLEDOWN_PLAYERS_SCRIPT } from './thistledownPlayers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thistledown Players";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thistledown Players"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THISTLEDOWN_PLAYERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thistledown Players", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Untap target nonland permanent.\"", () => {
    const { g, no } = armed(0);
