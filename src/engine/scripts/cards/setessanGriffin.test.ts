// `Setessan Griffin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SETESSAN_GRIFFIN_SCRIPT } from './setessanGriffin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Setessan Griffin";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SETESSAN_GRIFFIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Setessan Griffin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SETESSAN_GRIFFIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Setessan Griffin", () => {
  test("{2}{G}{G}: it gets +2/+2 until end of turn", () => {
