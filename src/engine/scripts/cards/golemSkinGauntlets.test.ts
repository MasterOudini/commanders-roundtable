// `Golem-Skin Gauntlets` - the pump is COUNTED: zero of them is no bonus, and each one moves the
// derived power and toughness by the printed delta. Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { GOLEM_SKIN_GAUNTLETS_SCRIPT } from './golemSkinGauntlets';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Golem-Skin Gauntlets";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOLEM_SKIN_GAUNTLETS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; subject: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GOLEM_SKIN_GAUNTLETS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Golem-Skin Gauntlets", () => {
  test("each Swiftfoot Boots attached counted moves the P/T by [1, 0], and one leaving takes it back", () => {
