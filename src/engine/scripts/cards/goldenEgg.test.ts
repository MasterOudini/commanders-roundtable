// `Golden Egg` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOLDEN_EGG_SCRIPT } from './goldenEgg';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Golden Egg";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Golden Egg"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOLDEN_EGG_SCRIPT]),
  });
  holdEverywhere(g);
