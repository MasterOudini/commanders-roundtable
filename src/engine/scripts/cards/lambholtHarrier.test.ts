// `Lambholt Harrier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LAMBHOLT_HARRIER_SCRIPT } from './lambholtHarrier';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lambholt Harrier";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lambholt Harrier"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LAMBHOLT_HARRIER_SCRIPT]),
  });
  holdEverywhere(g);
