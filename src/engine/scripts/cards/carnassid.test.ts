// `Carnassid` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CARNASSID_SCRIPT } from './carnassid';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Carnassid";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Carnassid", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CARNASSID_SCRIPT]),
  });
  holdEverywhere(g);
