// `Rathi Intimidator` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rathi Intimidator", "Rampart Crawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RATHI_INTIMIDATOR_SCRIPT]),
...
describe("Rathi Intimidator", () => {
  test("{2}, {T}: the vocabulary resolves \"Search your library for a Mercenary permanent card with mana value 2 or less, put it onto the battlefield, then shuffle.\"", () => {
