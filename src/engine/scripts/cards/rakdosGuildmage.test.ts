// `Rakdos Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAKDOS_GUILDMAGE_SCRIPT } from './rakdosGuildmage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rakdos Guildmage";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; tok0: readonly InstanceId[]; disc0: InstanceId[] };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RAKDOS_GUILDMAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rakdos Guildmage", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAKDOS_GUILDMAGE_SCRIPT]),
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, tok0, disc0 };
}

describe("Rakdos Guildmage", () => {
  test("{3}{B}, Discard a card: the declared creature is pumped until end of turn", () => {
    const { g, no, disc0 } = armed(0);
    expect(pt(g, no)).toEqual([3, 0]);
    for (const c of disc0) expect(g.state.cards[c]?.zone.kind).toBe('graveyard');
  });

  test("{3}{R}: the vocabulary resolves \"Create a 2/1 red Goblin creature token with haste. Exile it at the beginning of the next end step.\"", () => {
    const { g, board0, tok0 } = armed(1);
