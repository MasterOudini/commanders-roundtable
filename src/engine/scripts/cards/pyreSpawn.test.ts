// `Pyre Spawn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PYRE_SPAWN_SCRIPT } from './pyreSpawn';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pyre Spawn";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pyre Spawn"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PYRE_SPAWN_SCRIPT]),
  });
  holdEverywhere(g);
