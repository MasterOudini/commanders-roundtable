// `Ashiok's Adept` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASHIOKS_ADEPT_SCRIPT } from './ashioksAdept';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ashiok's Adept";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ashiok's Adept", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASHIOKS_ADEPT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ashiok's Adept", () => {
  test("Heroic — Whenever you cast a spell that targets this creature: the vocabulary resolves \"Each opponent discards a card.\"", () => {
    const { g } = armed(0);
    let qdL0 = 0;
