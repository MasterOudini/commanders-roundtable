// `Echoing Truth` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ECHOING_TRUTH_SCRIPT } from './echoingTruth';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Echoing Truth";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Echoing Truth", "Grizzly Bears"], ["Grizzly Bears", "Forest", "Grizzly Bears"]],
    scripts: createRegistry([ECHOING_TRUTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Echoing Truth", () => {
  test('the named target is accepted and the effect lands', () => {
    ...
    expect(g.state.cards[extras[0] as InstanceId]?.zone.kind).toBe('hand');
    expect(g.state.cards[extras[1] as InstanceId]?.zone.kind).toBe('hand');
  });
