// `Wild Wanderer` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wild Wanderer", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WILD_WANDERER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wild Wanderer", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
