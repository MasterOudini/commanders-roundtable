// `Heart of Light` - a CONTINUOUS prevention effect (CR 615, D385): a `PreventionDef` the
...
import { HEART_OF_LIGHT } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(HEART_OF_LIGHT, "Enchant creature (Target a creature as you cast this. This card enters attached to that creature.)\nPrevent all damage that would be dealt to and dealt by enchanted creature.");
const LINES = PRINTED.split(String.fromCharCode(10));

export const HEART_OF_LIGHT_SCRIPT: CardScript = {
  oracleId: HEART_OF_LIGHT.oracleId,
  name: HEART_OF_LIGHT.name,
  prevention: [
    {
      abilityId: 'prevent-1',
