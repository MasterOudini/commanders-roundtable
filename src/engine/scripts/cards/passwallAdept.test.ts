// `Passwall Adept` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PASSWALL_ADEPT_SCRIPT } from './passwallAdept';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Passwall Adept";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Passwall Adept"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PASSWALL_ADEPT_SCRIPT]),
  });
  holdEverywhere(g);
