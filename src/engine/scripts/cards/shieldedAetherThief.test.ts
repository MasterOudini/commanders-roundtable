// `Shielded Aether Thief` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHIELDED_AETHER_THIEF_SCRIPT } from './shieldedAetherThief';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shielded Aether Thief";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shielded Aether Thief"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHIELDED_AETHER_THIEF_SCRIPT]),
...
describe("Shielded Aether Thief", () => {
  test("Whenever this creature blocks: the vocabulary resolves \"You get {E}.\"", () => {
    const { g, energy0 } = armed(0);
    expect(g.state.players.p1?.energy).toBe(energy0 + 1);
  });

  test("{T}, Pay {E}{E}{E}: a card is drawn", () => {
    const { g, self, energy0, hand0 } = armed(1);
