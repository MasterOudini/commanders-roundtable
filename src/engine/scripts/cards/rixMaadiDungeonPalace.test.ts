// `Rix Maadi, Dungeon Palace` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIX_MAADI_DUNGEON_PALACE_SCRIPT } from './rixMaadiDungeonPalace';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rix Maadi, Dungeon Palace";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rix Maadi, Dungeon Palace"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIX_MAADI_DUNGEON_PALACE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rix Maadi, Dungeon Palace", () => {
  test("{1}{B}{R}, {T}: the vocabulary resolves \"Each player discards a card.\"", () => {
    ...
    { const qb = [...g.log].reverse().find((x) => x.body.t === 'CardsMoved' && x.body.moves.some((m) => m.reason === 'discard'));
      ...
      qdA1 = qm.filter((m) => m.from.kind === 'hand' && m.from.player === 'p1').length;
    }
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 0 - qdA1);
