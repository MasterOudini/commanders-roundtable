// `Kjeldoran Dead` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KJELDORAN_DEAD_SCRIPT } from './kjeldoranDead';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kjeldoran Dead";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kjeldoran Dead", "Lightning Bolt", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KJELDORAN_DEAD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kjeldoran Dead", () => {
  test("When this creature enters: the vocabulary resolves \"Sacrifice a creature.\"", () => {
  ...
  test("{B}: it regenerates from the Bolt", () => {
