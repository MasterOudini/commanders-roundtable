// `Sandworm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SANDWORM_SCRIPT } from './sandworm';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sandworm"], ["Cyclops of One-Eyed Pass", "Forest", "Forest"]],
    scripts: createRegistry([SANDWORM_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtL1_etb_0 = put(g, 'p2', "Forest");
  settle(g);
...
describe("Sandworm", () => {
  test("When this creature enters: the vocabulary resolves \"Destroy target land. Its controller may search their library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
