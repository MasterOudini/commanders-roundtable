// `Jace's Sanctum` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JACES_SANCTUM_SCRIPT } from './jacesSanctum';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jace's Sanctum";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jace's Sanctum", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JACES_SANCTUM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Jace's Sanctum", () => {
  test("Whenever you cast an instant or sorcery spell: scry 1 asks, and the cards stay on top", () => {
