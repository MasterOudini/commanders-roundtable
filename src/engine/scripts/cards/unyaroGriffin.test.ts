// `Unyaro Griffin` - counters a held spell of the named colour and refuses one of another:
// the adjective is the parser's and the validator's (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { UNYARO_GRIFFIN_SCRIPT } from './unyaroGriffin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unyaro Griffin";
...
function armed(which: 'right' | 'wrong'): { g: Game; self: InstanceId; spell: InstanceId; stackId: string; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Unyaro Griffin"], ["Pyretic Ritual", "Grizzly Bears"]],
    scripts: createRegistry([UNYARO_GRIFFIN_SCRIPT]),
  });
  holdEverywhere(g);
