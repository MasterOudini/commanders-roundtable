// `Shivan Gorge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHIVAN_GORGE_SCRIPT } from './shivanGorge';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shivan Gorge";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shivan Gorge"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHIVAN_GORGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shivan Gorge", () => {
  test("{2}{R}, {T}: it deals 1 damage to each opponent", () => {
