// `Rook Turret` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROOK_TURRET_SCRIPT } from './rookTurret';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rook Turret";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rook Turret", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROOK_TURRET_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rook Turret", () => {
  test("Whenever another artifact you control enters: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(0);
