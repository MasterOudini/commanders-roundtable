// `Scalding Cauldron` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCALDING_CAULDRON_SCRIPT } from './scaldingCauldron';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scalding Cauldron";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scalding Cauldron"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCALDING_CAULDRON_SCRIPT]),
  });
  holdEverywhere(g);
