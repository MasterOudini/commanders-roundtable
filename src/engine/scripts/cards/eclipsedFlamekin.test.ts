// `Eclipsed Flamekin` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Eclipsed Flamekin", "Island", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ECLIPSED_FLAMEKIN_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const lookPicksL0_etb = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Island" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL0_etb.length, "Island is dealt for the look").toBeGreaterThan(0);
...
describe("Eclipsed Flamekin", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top four cards of your library. You may reveal an Elemental, Island, or Mountain card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
    ...
    { const shown = (g.state.zones.library.p1 ?? []).filter((id) => g.state.cards[id]?.revealedTo.includes('p1'));
      const found = shown.find((id) => nameOf(g, id) === "Island");
      expect(found, "Island is not among the revealed cards").toBeDefined();
