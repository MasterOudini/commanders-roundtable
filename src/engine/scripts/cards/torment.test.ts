// `Torment` - cast on Grizzly Bears it attaches and the enchanted creature gets -3/+0;
...
import { TORMENT_SCRIPT } from './torment';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TORMENT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([TORMENT_SCRIPT]),
  });
  holdEverywhere(g);
