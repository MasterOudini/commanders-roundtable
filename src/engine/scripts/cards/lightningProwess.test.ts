// `Lightning Prowess` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { LIGHTNING_PROWESS_SCRIPT } from './lightningProwess';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning Prowess";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Lightning Prowess","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIGHTNING_PROWESS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lightning Prowess", () => {
  test("grants {T}: This creature deals 1 damage to any target. to Grizzly Bears, and to nothing outside the scope", () => {
