// `Fauna Shaman` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fauna Shaman", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FAUNA_SHAMAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fauna Shaman", () => {
  test("{G}, {T}, Discard a creature card: the vocabulary resolves \"Search your library for a creature card, reveal it, put it into your hand, then shuffle.\"", () => {
