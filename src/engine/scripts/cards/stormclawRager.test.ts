// `Stormclaw Rager` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORMCLAW_RAGER_SCRIPT } from './stormclawRager';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stormclaw Rager";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stormclaw Rager", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORMCLAW_RAGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stormclaw Rager", () => {
  test("{1}, Sacrifice another creature or artifact: the vocabulary resolves \"Put a +1/+1 counter on this creature and draw a card.\"", () => {
