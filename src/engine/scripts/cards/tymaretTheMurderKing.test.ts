// `Tymaret, the Murder King` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TYMARET_THE_MURDER_KING_SCRIPT } from './tymaretTheMurderKing';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tymaret, the Murder King";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tymaret, the Murder King", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TYMARET_THE_MURDER_KING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Tymaret, the Murder King", () => {
  test("{1}{R}, Sacrifice another creature: it deals 2 damage to the declared player", () => {
    const { g, p2life0, fodder0 } = armed(0);
  ...
  test("{1}{B}, Sacrifice a creature: it returns from the graveyard to hand", () => {
