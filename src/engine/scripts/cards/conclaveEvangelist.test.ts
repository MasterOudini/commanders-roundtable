// `Conclave Evangelist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONCLAVE_EVANGELIST_SCRIPT } from './conclaveEvangelist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Conclave Evangelist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Conclave Evangelist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONCLAVE_EVANGELIST_SCRIPT]),
...
describe("Conclave Evangelist", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Create a token that's a copy of this creature.\"", () => {
