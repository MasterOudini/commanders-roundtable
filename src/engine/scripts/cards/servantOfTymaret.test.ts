// `Servant of Tymaret` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SERVANT_OF_TYMARET_SCRIPT } from './servantOfTymaret';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Servant of Tymaret";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Servant of Tymaret", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SERVANT_OF_TYMARET_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Servant of Tymaret", () => {
  test("Inspired — Whenever this creature becomes untapped: the vocabulary resolves \"Each opponent loses 1 life. You gain life equal to the life lost this way.\"", () => {
