// `Professional Wrestler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PROFESSIONAL_WRESTLER_SCRIPT } from './professionalWrestler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Professional Wrestler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Professional Wrestler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([PROFESSIONAL_WRESTLER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Professional Wrestler", () => {
