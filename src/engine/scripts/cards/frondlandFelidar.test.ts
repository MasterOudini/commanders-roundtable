// `Frondland Felidar` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { FRONDLAND_FELIDAR_SCRIPT } from './frondlandFelidar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frondland Felidar";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Frondland Felidar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FRONDLAND_FELIDAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Frondland Felidar", () => {
  test("grants {1}, {T}: Tap target creature. to itself, and to nothing outside the scope", () => {
