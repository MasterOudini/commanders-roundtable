// `Herald of Anguish` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HERALD_OF_ANGUISH_SCRIPT } from './heraldOfAnguish';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Herald of Anguish";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HERALD_OF_ANGUISH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Herald of Anguish", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HERALD_OF_ANGUISH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Herald of Anguish", () => {
  test("At the beginning of your end step: the vocabulary resolves \"Each opponent discards a card.\"", () => {
    const { g } = armed(0);
    let qdL2 = 0;
    ...
    { const qb = [...g.log].reverse().find((x) => x.body.t === 'CardsMoved' && x.body.moves.some((m) => m.reason === 'discard'));
      ...
      qdL2 = qm.filter((m) => m.from.kind === 'hand' && m.from.player === 'p1').length;
    }
    expect(qdL2, 'p1 was not in the scope').toBe(0);
  });

  test("{1}{B}, Sacrifice an artifact: the declared creature is pumped until end of turn", () => {
