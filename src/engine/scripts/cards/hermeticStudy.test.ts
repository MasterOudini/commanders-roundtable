// `Hermetic Study` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { HERMETIC_STUDY_SCRIPT } from './hermeticStudy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hermetic Study";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Hermetic Study","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HERMETIC_STUDY_SCRIPT]),
  });
  holdEverywhere(g);
