// `Nimbus Wings` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+2 and has flying;
...
import { NIMBUS_WINGS_SCRIPT } from './nimbusWings';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nimbus Wings";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NIMBUS_WINGS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([NIMBUS_WINGS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Nimbus Wings", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+2 and has flying; Cyclops of One-Eyed Pass is untouched", () => {
