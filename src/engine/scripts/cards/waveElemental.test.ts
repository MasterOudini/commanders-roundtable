// `Wave Elemental` - every declared pick is accepted and it is tapped; a permanent the clause
// excludes is refused (D299). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { WAVE_ELEMENTAL_SCRIPT } from './waveElemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wave Elemental";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; target2: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Wave Elemental"], ["Grizzly Bears", "Lady Caleria", "Vampire Nighthawk"]],
    scripts: createRegistry([WAVE_ELEMENTAL_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const target = put(g, 'p2', "Grizzly Bears");
  const target2 = put(g, 'p2', "Lady Caleria");
...
describe("Wave Elemental", () => {
  test("each of Grizzly Bears and Lady Caleria is accepted and it is tapped", () => {
