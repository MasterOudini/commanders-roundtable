// `Enraged Huorn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ENRAGED_HUORN_SCRIPT } from './enragedHuorn';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Enraged Huorn";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Enraged Huorn"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ENRAGED_HUORN_SCRIPT]),
