// `Ondu War Cleric` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ONDU_WAR_CLERIC_SCRIPT } from './onduWarCleric';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ondu War Cleric";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ondu War Cleric", "Expedition Envoy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ONDU_WAR_CLERIC_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ondu War Cleric", () => {
  test("Cohort — {T}, Tap an untapped Ally you control: 2 life is gained", () => {
    const { g, self, life0, tapper0 } = armed(0);
