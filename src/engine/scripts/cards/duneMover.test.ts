// `Dune Mover` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dune Mover", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUNE_MOVER_SCRIPT]),
  });
  holdEverywhere(g);
