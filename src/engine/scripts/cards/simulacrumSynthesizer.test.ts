// `Simulacrum Synthesizer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SIMULACRUM_SYNTHESIZER_SCRIPT } from './simulacrumSynthesizer';
import { CONSTRUCT_TOKEN008D3B2E_SCRIPT } from './constructToken008d3b2e';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Simulacrum Synthesizer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Simulacrum Synthesizer", "Grizzly Bears", "Sliver Construct"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIMULACRUM_SYNTHESIZER_SCRIPT, CONSTRUCT_TOKEN008D3B2E_SCRIPT]),
  ...
  if (which === 1) {
    bears = put(g, 'p1', "Sliver Construct");
...
describe("Simulacrum Synthesizer", () => {
  ...
  test("Whenever another artifact you control with mana value 3 or greater enters: the vocabulary resolves \"Create a 0/0 colorless Construct artifact creature token with \\\"This token gets +1/+1 for each artifact you control.\\\"\"", () => {
