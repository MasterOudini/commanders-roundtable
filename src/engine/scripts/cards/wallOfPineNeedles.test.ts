// `Wall of Pine Needles` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WALL_OF_PINE_NEEDLES_SCRIPT } from './wallOfPineNeedles';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wall of Pine Needles";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wall of Pine Needles", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WALL_OF_PINE_NEEDLES_SCRIPT]),
  });
  holdEverywhere(g);
