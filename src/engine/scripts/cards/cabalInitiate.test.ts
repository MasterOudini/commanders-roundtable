// `Cabal Initiate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CABAL_INITIATE_SCRIPT } from './cabalInitiate';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cabal Initiate";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CABAL_INITIATE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cabal Initiate", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CABAL_INITIATE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cabal Initiate", () => {
  ...
  test("Threshold — This creature gets +1/+2 as long as there are seven or more cards in your graveyard.: with seven cards in the graveyard it reads the pump", () => {
