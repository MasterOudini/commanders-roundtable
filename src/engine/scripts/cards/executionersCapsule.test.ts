// `Executioner's Capsule` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { EXECUTIONERS_CAPSULE_SCRIPT } from './executionersCapsule';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Executioner's Capsule";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Executioner's Capsule"], ["Grizzly Bears", "Vampire Nighthawk"]],
    scripts: createRegistry([EXECUTIONERS_CAPSULE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Executioner's Capsule", () => {
