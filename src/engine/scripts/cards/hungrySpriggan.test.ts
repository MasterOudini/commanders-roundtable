// `Hungry Spriggan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HUNGRY_SPRIGGAN_SCRIPT } from './hungrySpriggan';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hungry Spriggan";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HUNGRY_SPRIGGAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hungry Spriggan"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HUNGRY_SPRIGGAN_SCRIPT]),
  });
  holdEverywhere(g);
