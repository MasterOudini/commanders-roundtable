// `One with Nature` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["One with Nature", "Grizzly Bears", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ONE_WITH_NATURE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("One with Nature", () => {
  test("Whenever enchanted creature deals combat damage to a player: the vocabulary resolves \"Search your library for a basic land card, put that card onto the battlefield tapped, then shuffle.\"", () => {
