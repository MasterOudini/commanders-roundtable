// `Shinen of Fury's Fire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHINEN_OF_FURYS_FIRE_SCRIPT } from './shinenOfFurysFire';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shinen of Fury's Fire";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHINEN_OF_FURYS_FIRE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shinen of Fury's Fire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHINEN_OF_FURYS_FIRE_SCRIPT]),
...
describe("Shinen of Fury's Fire", () => {
  test("Channel — {R}, Discard this card: the declared creature is pumped until end of turn", () => {
