// `Epiphany Storm` - the permanent it is attached to HAS a quoted
...
import { EPIPHANY_STORM } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(EPIPHANY_STORM, "Enchant creature\nEnchanted creature has \"{R}, {T}, Discard a card: Draw a card.\"");
const LINES = PRINTED.split('\n');

const GRANT = grantedActivated("{R}, {T}, Discard a card: Draw a card.", `${EPIPHANY_STORM.oracleId}#g1`, EPIPHANY_STORM.name);
const VOCAB = vocabularyEffects("Draw a card.", EPIPHANY_STORM.name);
const VOCAB_T = vocabularyTargets("Draw a card.");

export const EPIPHANY_STORM_SCRIPT: CardScript = {
  oracleId: EPIPHANY_STORM.oracleId,
  name: EPIPHANY_STORM.name,
  statics: [
    {
      abilityId: 'grant-1',
