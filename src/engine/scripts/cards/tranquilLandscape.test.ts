// `Tranquil Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tranquil Landscape", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRANQUIL_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Tranquil Landscape", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Forest, Plains, or Island card, put it onto the battlefield tapped, then shuffle.\"", () => {
