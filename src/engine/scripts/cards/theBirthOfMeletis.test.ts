// `The Birth of Meletis` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["The Birth of Meletis", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THE_BIRTH_OF_MELETIS_SCRIPT]),
...
describe("The Birth of Meletis", () => {
  test("I — Search your library for a basic Plains card: the vocabulary resolves \"Search your library for a basic Plains card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
  ...
  test("II — Create a 0/4 colorless Wall artifact creature token with defender.: 1 token made", () => {
    const { g, board0 } = armed(1);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("III — You gain 2 life.: 2 life is gained", () => {
