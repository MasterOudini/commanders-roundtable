// `Slither Blade` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLITHER_BLADE_SCRIPT } from './slitherBlade';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slither Blade";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slither Blade", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLITHER_BLADE_SCRIPT]),
  });
  holdEverywhere(g);
