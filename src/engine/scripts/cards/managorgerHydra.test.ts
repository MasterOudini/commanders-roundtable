// `Managorger Hydra` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MANAGORGER_HYDRA_SCRIPT } from './managorgerHydra';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Managorger Hydra";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Managorger Hydra", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MANAGORGER_HYDRA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Managorger Hydra", () => {
  test("Whenever a player casts a spell: it gets 1 +1/+1 counter", () => {
