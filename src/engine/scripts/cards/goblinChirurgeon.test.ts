// `Goblin Chirurgeon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_CHIRURGEON_SCRIPT } from './goblinChirurgeon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Chirurgeon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Chirurgeon", "Raging Goblin", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_CHIRURGEON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Chirurgeon", () => {
  test("Sacrifice a Goblin: the declared creature regenerates from the Bolt", () => {
