// `Raven Clan War-Axe` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RAVEN_CLAN_WAR_AXE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Raven Clan War-Axe", "Grizzly Bears", "Eivor, Battle-Ready"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAVEN_CLAN_WAR_AXE_SCRIPT]),
...
describe("Raven Clan War-Axe", () => {
  test("When this Equipment enters: the vocabulary resolves \"Search your library and/or graveyard for a card named Eivor, Battle-Ready, reveal it, and put it into your hand. If you search your library this way, shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Eivor, Battle-Ready" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Eivor, Battle-Ready");
      expect(found, "Eivor, Battle-Ready is not in the library for the search to find").toBeDefined();
  ...
  test("Equipped creature gets +2/+0 and has trample.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([4, 2]);
    expect(kw(g, bearsB).has("trample")).toBe(true);
  });
