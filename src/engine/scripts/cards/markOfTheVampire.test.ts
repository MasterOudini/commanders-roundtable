// `Mark of the Vampire` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2 and has lifelink;
...
import { MARK_OF_THE_VAMPIRE_SCRIPT } from './markOfTheVampire';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mark of the Vampire";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MARK_OF_THE_VAMPIRE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([MARK_OF_THE_VAMPIRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mark of the Vampire", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+2 and has lifelink; Cyclops of One-Eyed Pass is untouched", () => {
