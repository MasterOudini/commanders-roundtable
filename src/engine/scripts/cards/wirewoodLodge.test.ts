// `Wirewood Lodge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WIREWOOD_LODGE_SCRIPT } from './wirewoodLodge';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wirewood Lodge";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wirewood Lodge"], ["Cyclops of One-Eyed Pass", "Devoted Hero"]],
    scripts: createRegistry([WIREWOOD_LODGE_SCRIPT]),
...
describe("Wirewood Lodge", () => {
  test("{G}, {T}: the vocabulary resolves \"Untap target Elf.\"", () => {
