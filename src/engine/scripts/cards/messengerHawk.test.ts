// `Messenger Hawk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MESSENGER_HAWK_SCRIPT } from './messengerHawk';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Messenger Hawk";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MESSENGER_HAWK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Messenger Hawk", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MESSENGER_HAWK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Messenger Hawk", () => {
  test("When this creature enters: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("`Messenger Hawk` - as long as you've drawn two or more cards this turn: absent while it is unmet, read once it holds", () => {
