// `Valor in Akros` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VALOR_IN_AKROS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Valor in Akros", "Coral Eel", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VALOR_IN_AKROS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Valor in Akros", () => {
  test("Whenever a creature you control enters: its controller's creatures get +1/+1 until end of turn", () => {
