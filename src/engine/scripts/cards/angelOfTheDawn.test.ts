// `Angel of the Dawn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ANGEL_OF_THE_DAWN_SCRIPT } from './angelOfTheDawn';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Angel of the Dawn";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ANGEL_OF_THE_DAWN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Angel of the Dawn", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ANGEL_OF_THE_DAWN_SCRIPT]),
  });
  holdEverywhere(g);
