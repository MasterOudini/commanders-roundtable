// `Berserkers of Blood Ridge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BERSERKERS_OF_BLOOD_RIDGE_SCRIPT } from './berserkersOfBloodRidge';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Berserkers of Blood Ridge";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Berserkers of Blood Ridge", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BERSERKERS_OF_BLOOD_RIDGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Berserkers of Blood Ridge", () => {
