// `Ghitu Journeymage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHITU_JOURNEYMAGE_SCRIPT } from './ghituJourneymage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghitu Journeymage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghitu Journeymage", "Fugitive Wizard"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHITU_JOURNEYMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ghitu Journeymage", () => {
  test("When this creature enters: it deals 2 damage to each opponent", () => {
