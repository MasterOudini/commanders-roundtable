// `Sangrite Backlash` - cast on Grizzly Bears it attaches and the enchanted creature gets +3/-3;
...
import { SANGRITE_BACKLASH_SCRIPT } from './sangriteBacklash';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sangrite Backlash";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SANGRITE_BACKLASH_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SANGRITE_BACKLASH_SCRIPT]),
  });
  holdEverywhere(g);
