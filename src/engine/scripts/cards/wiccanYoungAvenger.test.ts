// `Wiccan, Young Avenger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WICCAN_YOUNG_AVENGER_SCRIPT } from './wiccanYoungAvenger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wiccan, Young Avenger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wiccan, Young Avenger", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WICCAN_YOUNG_AVENGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wiccan, Young Avenger", () => {
  test("Whenever you cast a noncreature spell: the vocabulary resolves \"Exile the top card of your library. Until your next end step, you may play that card.\"", () => {
    const { g } = armed(0);
