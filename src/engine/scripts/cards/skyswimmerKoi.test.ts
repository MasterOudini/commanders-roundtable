// `Skyswimmer Koi` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKYSWIMMER_KOI_SCRIPT } from './skyswimmerKoi';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skyswimmer Koi";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skyswimmer Koi", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKYSWIMMER_KOI_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skyswimmer Koi", () => {
  test("Whenever an artifact you control enters: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(0);
