// `Ramosian Sky Marshal` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ramosian Sky Marshal", "Children of Korlis"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAMOSIAN_SKY_MARSHAL_SCRIPT]),
...
describe("Ramosian Sky Marshal", () => {
  test("{7}, {T}: the vocabulary resolves \"Search your library for a Rebel permanent card with mana value 6 or less, put it onto the battlefield, then shuffle.\"", () => {
