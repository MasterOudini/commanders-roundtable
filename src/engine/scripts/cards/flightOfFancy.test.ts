// `Flight of Fancy` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: draw 2); has flying;
...
import { FLIGHT_OF_FANCY_SCRIPT } from './flightOfFancy';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flight of Fancy";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([FLIGHT_OF_FANCY_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FLIGHT_OF_FANCY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Flight of Fancy", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: draw 2); has flying; Cyclops of One-Eyed Pass is untouched", () => {
