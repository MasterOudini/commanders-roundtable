// `Wolverine Riders` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WOLVERINE_RIDERS_SCRIPT } from './wolverineRiders';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wolverine Riders";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wolverine Riders", "Grizzly Bears", "Devoted Hero"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WOLVERINE_RIDERS_SCRIPT]),
...
describe("Wolverine Riders", () => {
  test("At the beginning of each upkeep: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("Whenever another Elf you control enters: the vocabulary resolves \"You gain life equal to target creature's toughness.\"", () => {
