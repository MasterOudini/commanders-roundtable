// `Blessing of Leeches` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLESSING_OF_LEECHES_SCRIPT } from './blessingOfLeeches';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blessing of Leeches";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blessing of Leeches", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLESSING_OF_LEECHES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Blessing of Leeches", () => {
  test("At the beginning of your upkeep: 1 life is lost", () => {
    const { g, life0 } = armed(0);
    expect(g.state.players.p1?.life).toBe(life0 - 1);
  });

  test("{0}: the enchanted creature regenerates from the Bolt", () => {
