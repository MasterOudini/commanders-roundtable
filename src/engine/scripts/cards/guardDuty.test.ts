// `Guard Duty` - cast on Grizzly Bears it attaches and the enchanted creature has defender;
...
import { GUARD_DUTY_SCRIPT } from './guardDuty';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Guard Duty";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([GUARD_DUTY_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GUARD_DUTY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Guard Duty", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has defender; Cyclops of One-Eyed Pass is untouched", () => {
