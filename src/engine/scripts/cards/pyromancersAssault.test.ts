// `Pyromancer's Assault` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PYROMANCERS_ASSAULT_SCRIPT } from './pyromancersAssault';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pyromancer's Assault";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pyromancer's Assault", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PYROMANCERS_ASSAULT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pyromancer's Assault", () => {
  test("Whenever you cast your second spell each turn: the vocabulary resolves \"This enchantment deals 2 damage to any target.\"", () => {
    const { g, no } = armed(0);
