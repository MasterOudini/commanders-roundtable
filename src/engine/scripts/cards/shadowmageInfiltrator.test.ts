// `Shadowmage Infiltrator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHADOWMAGE_INFILTRATOR_SCRIPT } from './shadowmageInfiltrator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shadowmage Infiltrator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shadowmage Infiltrator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHADOWMAGE_INFILTRATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shadowmage Infiltrator", () => {
