// `Mad Ratter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAD_RATTER_SCRIPT } from './madRatter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mad Ratter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mad Ratter", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAD_RATTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mad Ratter", () => {
  test("Whenever you draw your second card each turn: 2 tokens made", () => {
