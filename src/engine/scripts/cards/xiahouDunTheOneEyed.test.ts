// `Xiahou Dun, the One-Eyed` - every printed ability proven in its own game: the cost's mark, the pump
...
import { XIAHOU_DUN_THE_ONE_EYED_SCRIPT } from './xiahouDunTheOneEyed';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Xiahou Dun, the One-Eyed";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Xiahou Dun, the One-Eyed", "Walking Corpse"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([XIAHOU_DUN_THE_ONE_EYED_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Xiahou Dun, the One-Eyed", () => {
  test("Sacrifice Xiahou Dun: the vocabulary resolves \"Return target black card from your graveyard to your hand.\" - refused unless during your turn, before attackers are declared", () => {
    const { g, self, hand0, condRefused, vtA0_0 } = armed(0);
