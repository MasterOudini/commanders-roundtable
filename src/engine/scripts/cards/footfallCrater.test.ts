// `Footfall Crater` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { FOOTFALL_CRATER_SCRIPT } from './footfallCrater';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Footfall Crater";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Footfall Crater","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOOTFALL_CRATER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Footfall Crater", () => {
  test("grants {T}: Target creature gains trample and haste until end of turn. to Forest, and to nothing outside the scope", () => {
