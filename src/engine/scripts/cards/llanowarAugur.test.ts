// `Llanowar Augur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LLANOWAR_AUGUR_SCRIPT } from './llanowarAugur';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Llanowar Augur";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LLANOWAR_AUGUR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Llanowar Augur"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LLANOWAR_AUGUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Llanowar Augur", () => {
  test("Sacrifice this creature: the declared creature is pumped until end of turn - refused unless during your upkeep", () => {
