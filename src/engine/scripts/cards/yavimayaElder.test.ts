// `Yavimaya Elder` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Yavimaya Elder", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YAVIMAYA_ELDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Yavimaya Elder", () => {
  test("When this creature dies: the vocabulary resolves \"Search your library for up to two basic land cards, reveal them, put them into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
