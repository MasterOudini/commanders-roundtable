// `Northern Paladin` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { NORTHERN_PALADIN_SCRIPT } from './northernPaladin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Northern Paladin";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Northern Paladin"], ["Vampire Nighthawk", "Grizzly Bears"]],
    scripts: createRegistry([NORTHERN_PALADIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Northern Paladin", () => {
  test("Vampire Nighthawk is a legal target and it is destroyed", () => {
