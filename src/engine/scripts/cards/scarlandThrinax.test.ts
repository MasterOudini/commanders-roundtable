// `Scarland Thrinax` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCARLAND_THRINAX_SCRIPT } from './scarlandThrinax';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scarland Thrinax";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scarland Thrinax", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCARLAND_THRINAX_SCRIPT]),
  });
  holdEverywhere(g);
