// `Gravetiller Wurm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRAVETILLER_WURM_SCRIPT } from './gravetillerWurm';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gravetiller Wurm";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GRAVETILLER_WURM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gravetiller Wurm", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRAVETILLER_WURM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gravetiller Wurm", () => {
  test("Morbid — This creature enters with four +1/+1 counters on it if a creature died this turn.: cast, it enters with 4 +1/+1 counters", () => {
