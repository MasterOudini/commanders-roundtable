// `Origin Spellbomb` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORIGIN_SPELLBOMB_SCRIPT } from './originSpellbomb';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Origin Spellbomb";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Origin Spellbomb", "Grizzly Bears", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORIGIN_SPELLBOMB_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Origin Spellbomb", () => {
  test("{1}, {T}, Sacrifice this artifact: 1 token made", () => {
    const { g, self, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 - 1);
    expect(g.state.cards[self]?.zone.kind).toBe('graveyard');
  });

  test("When this artifact is put into a graveyard from the battlefield: the vocabulary resolves \"You may pay {W}. If you do, draw a card.\" - the price is declined", () => {
  ...
  test("When this artifact is put into a graveyard from the battlefield: the vocabulary resolves \"You may pay {W}. If you do, draw a card.\" - the price is paid", () => {
    const { g, payL1_p1_0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
