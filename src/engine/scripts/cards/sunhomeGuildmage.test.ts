// `Sunhome Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUNHOME_GUILDMAGE_SCRIPT } from './sunhomeGuildmage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunhome Guildmage";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUNHOME_GUILDMAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sunhome Guildmage", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUNHOME_GUILDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sunhome Guildmage", () => {
  test("{1}{R}{W}: its controller's creatures get +1/+0 until end of turn", () => {
    const { g, no, yes } = armed(0);
    expect(pt(g, yes)).toEqual([3, 1]);
    expect(pt(g, no)).toEqual([5, 2]);
  });

  test("{2}{R}{W}: 1 token made", () => {
