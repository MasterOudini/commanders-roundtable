// `Benalish Sleeper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BENALISH_SLEEPER_SCRIPT } from './benalishSleeper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Benalish Sleeper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Benalish Sleeper", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BENALISH_SLEEPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Benalish Sleeper", () => {
  test("When this creature enters: the vocabulary resolves \"Each player sacrifices a creature of their choice.\"", () => {
