// `Sling-Gang Lieutenant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLING_GANG_LIEUTENANT_SCRIPT } from './slingGangLieutenant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sling-Gang Lieutenant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sling-Gang Lieutenant", "Raging Goblin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLING_GANG_LIEUTENANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sling-Gang Lieutenant", () => {
  test("When this creature enters: 2 tokens made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 2 + 1);
  });

  test("Sacrifice a Goblin: the vocabulary resolves \"Target player loses 1 life and you gain 1 life.\"", () => {
    const { g, life0, p2life0, fodder0 } = armed(1);
