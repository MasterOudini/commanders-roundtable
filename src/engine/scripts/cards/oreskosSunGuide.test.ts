// `Oreskos Sun Guide` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORESKOS_SUN_GUIDE_SCRIPT } from './oreskosSunGuide';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Oreskos Sun Guide";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Oreskos Sun Guide"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORESKOS_SUN_GUIDE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Oreskos Sun Guide", () => {
  test("Inspired — Whenever this creature becomes untapped: 2 life is gained", () => {
