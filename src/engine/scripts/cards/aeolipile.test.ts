// `Aeolipile` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AEOLIPILE_SCRIPT } from './aeolipile';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aeolipile";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aeolipile"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AEOLIPILE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aeolipile", () => {
  test("{1}, {T}, Sacrifice this artifact: it deals 2 damage to the declared creature", () => {
