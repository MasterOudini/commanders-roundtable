// `Resistance Squad` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RESISTANCE_SQUAD_SCRIPT } from './resistanceSquad';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Resistance Squad";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Resistance Squad", "Eager Cadet"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RESISTANCE_SQUAD_SCRIPT]),
  });
  holdEverywhere(g);
