// `Prickleboar` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PRICKLEBOAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prickleboar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRICKLEBOAR_SCRIPT]),
  ...
  if (which === 0) {
    ...
    settle(g);
    condOff = pt(g, self)[0] === 3 && pt(g, self)[1] === 3 && !kw(g, self).has("firstStrike");
...
describe("Prickleboar", () => {
  test("`Prickleboar` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
