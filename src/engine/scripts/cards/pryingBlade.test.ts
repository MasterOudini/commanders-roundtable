// `Prying Blade` - equipped to Grizzly Bears the creature gets +1/+0; (on the equipped creature dealing combat damage to a player: 1 token); the Cyclops is untouched;
...
import { PRYING_BLADE_SCRIPT } from './pryingBlade';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prying Blade";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PRYING_BLADE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([PRYING_BLADE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Prying Blade", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+0; (on the equipped creature dealing combat damage to a player: 1 token); the Cyclops is untouched", () => {
