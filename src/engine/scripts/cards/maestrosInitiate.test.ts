// `Maestros Initiate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAESTROS_INITIATE_SCRIPT } from './maestrosInitiate';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Maestros Initiate";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Maestros Initiate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAESTROS_INITIATE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Maestros Initiate", () => {
  test("{4}{U/R}, Exile this card from your graveyard: the vocabulary resolves \"Draw two cards, then discard a card.\"", () => {
    ...
    { const qb = [...g.log].reverse().find((x) => x.body.t === 'CardsMoved' && x.body.moves.some((m) => m.reason === 'discard'));
      ...
      qdA0 = qm.filter((m) => m.from.kind === 'hand' && m.from.player === 'p1').length;
    }
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 2 - qdA0);
