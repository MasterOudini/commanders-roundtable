// `Sporeback Wolf` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPOREBACK_WOLF_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sporeback Wolf"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPOREBACK_WOLF_SCRIPT]),
...
describe("Sporeback Wolf", () => {
  test("`Sporeback Wolf` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
