// `Phantom Ninja` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHANTOM_NINJA_SCRIPT } from './phantomNinja';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phantom Ninja";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phantom Ninja", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHANTOM_NINJA_SCRIPT]),
  });
  holdEverywhere(g);
