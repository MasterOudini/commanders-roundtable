// `Ethereal Elk` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ethereal Elk", "Vivien, Nature's Avenger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ETHEREAL_ELK_SCRIPT]),
...
describe("Ethereal Elk", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library and/or graveyard for a card named Vivien, Nature's Avenger, reveal it, and put it into your hand. If you search your library this way, shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Vivien, Nature's Avenger" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Vivien, Nature's Avenger");
      expect(found, "Vivien, Nature's Avenger is not in the library for the search to find").toBeDefined();
