// `Fallowsage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FALLOWSAGE_SCRIPT } from './fallowsage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fallowsage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fallowsage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FALLOWSAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fallowsage", () => {
  test("Whenever this creature becomes tapped: a card is drawn", () => {
