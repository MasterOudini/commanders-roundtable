// `Racecourse Fury` - the permanent it is attached to HAS a quoted
...
import { RACECOURSE_FURY } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(RACECOURSE_FURY, "Enchant land\nEnchanted land has \"{T}: Target creature gains haste until end of turn.\"");
const LINES = PRINTED.split('\n');

const GRANT = grantedActivated("{T}: Target creature gains haste until end of turn.", `${RACECOURSE_FURY.oracleId}#g1`, RACECOURSE_FURY.name);
const VOCAB = vocabularyEffects("Target creature gains haste until end of turn.", RACECOURSE_FURY.name);
const VOCAB_T = vocabularyTargets("Target creature gains haste until end of turn.");

export const RACECOURSE_FURY_SCRIPT: CardScript = {
  oracleId: RACECOURSE_FURY.oracleId,
  name: RACECOURSE_FURY.name,
  statics: [
    {
      abilityId: 'grant-1',
