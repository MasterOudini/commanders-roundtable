// `Viashino Skeleton` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VIASHINO_SKELETON_SCRIPT } from './viashinoSkeleton';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Viashino Skeleton";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Viashino Skeleton", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIASHINO_SKELETON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Viashino Skeleton", () => {
  test("{1}{B}, Discard a card: it regenerates from the Bolt", () => {
