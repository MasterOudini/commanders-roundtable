// `Rubblebelt Recluse` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUBBLEBELT_RECLUSE_SCRIPT } from './rubblebeltRecluse';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rubblebelt Recluse";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rubblebelt Recluse", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUBBLEBELT_RECLUSE_SCRIPT]),
  });
  holdEverywhere(g);
