// `Frontier Guide` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frontier Guide", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FRONTIER_GUIDE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Frontier Guide", () => {
  test("{3}{G}, {T}: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
