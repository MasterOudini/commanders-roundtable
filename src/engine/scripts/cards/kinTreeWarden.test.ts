// `Kin-Tree Warden` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KIN_TREE_WARDEN_SCRIPT } from './kinTreeWarden';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kin-Tree Warden";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kin-Tree Warden", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KIN_TREE_WARDEN_SCRIPT]),
  });
  holdEverywhere(g);
