// `Nantuko Husk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NANTUKO_HUSK_SCRIPT } from './nantukoHusk';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nantuko Husk";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NANTUKO_HUSK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nantuko Husk", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NANTUKO_HUSK_SCRIPT]),
  });
  holdEverywhere(g);
