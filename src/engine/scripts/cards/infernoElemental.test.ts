// `Inferno Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INFERNO_ELEMENTAL_SCRIPT } from './infernoElemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Inferno Elemental";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Inferno Elemental"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INFERNO_ELEMENTAL_SCRIPT]),
...
describe("Inferno Elemental", () => {
  test("Whenever this creature blocks or becomes blocked by a creature: the vocabulary resolves \"~ deals 3 damage to target creature.\"", () => {
    const { g, no } = armed(0);
