// `Lumberknot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LUMBERKNOT_SCRIPT } from './lumberknot';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lumberknot";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lumberknot"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LUMBERKNOT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lumberknot", () => {
  test("Whenever a creature dies: it gets 1 +1/+1 counter", () => {
