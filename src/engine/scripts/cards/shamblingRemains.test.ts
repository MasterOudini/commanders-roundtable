// `Shambling Remains` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHAMBLING_REMAINS_SCRIPT } from './shamblingRemains';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shambling Remains";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shambling Remains", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHAMBLING_REMAINS_SCRIPT]),
