// `Quandrix Apprentice` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Quandrix Apprentice", "Pyretic Ritual", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([QUANDRIX_APPRENTICE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const lookPicksL0_castInstantSorcery = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Forest" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL0_castInstantSorcery.length, "Forest is dealt for the look").toBeGreaterThan(0);
    for (const id of lookPicksL0_castInstantSorcery) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: id, to: { kind: 'library', player: 'p1' }, placement: 'top' }));
...
describe("Quandrix Apprentice", () => {
  test("Magecraft — Whenever you cast or copy an instant or sorcery spell: the vocabulary resolves \"Look at the top three cards of your library. You may reveal a land card from among them and put that card into your hand. Put the rest on the bottom of your library in any order.\"", () => {
    const { g } = armed(0);
