// `Savage Silhouette` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { SAVAGE_SILHOUETTE_SCRIPT } from './savageSilhouette';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Savage Silhouette";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Savage Silhouette","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SAVAGE_SILHOUETTE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Savage Silhouette", () => {
  test("grants {1}{G}: Regenerate this creature. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and the body hits the RECIPIENT', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{1}{G}");
    ...
    settle(g);
    expect(g.state.regenerationShields[host] ?? 0).toBe(1);
