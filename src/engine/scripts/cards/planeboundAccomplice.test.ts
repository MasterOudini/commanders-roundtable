// `Planebound Accomplice` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Planebound Accomplice", "Jace Reawakened", "Jace Reawakened"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PLANEBOUND_ACCOMPLICE_SCRIPT]),
  ...
  if (which === 0) {
    { const inHand = (g.state.zones.hand.p1 ?? []).some((id) => nameOf(g, id) === "Jace Reawakened");
      const inLib = (g.state.zones.library.p1 ?? []).find((id) => nameOf(g, id) === "Jace Reawakened");
      if (!inHand) { expect(inLib, "Jace Reawakened is dealt for the hand put").toBeDefined(); must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: inLib as InstanceId, to: { kind: 'hand', player: 'p1' } })); } }
...
describe("Planebound Accomplice", () => {
  test("{R}: the vocabulary resolves \"You may put a planeswalker card from your hand onto the battlefield. Sacrifice it at the beginning of the next end step.\"", () => {
    ...
    { const ask = g.state.priority.awaiting;
      ...
      const found = (g.state.zones.hand.p1 ?? []).find((id) => nameOf(g, id) === "Jace Reawakened");
      expect(found, "Jace Reawakened is in hand for the put").toBeDefined();
    ...
    { const boardD = onBoard(g);
      ...
      expect(g.state.cards[self]?.zone.kind, "sacrificeSelf at the step").toBe('graveyard');
    }
