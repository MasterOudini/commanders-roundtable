// `Fleet-Footed Monk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLEET_FOOTED_MONK_SCRIPT } from './fleetFootedMonk';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fleet-Footed Monk";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fleet-Footed Monk", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLEET_FOOTED_MONK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fleet-Footed Monk", () => {
  test("This creature can't be blocked by creatures with power 2 or greater.: the block is refused", () => {
