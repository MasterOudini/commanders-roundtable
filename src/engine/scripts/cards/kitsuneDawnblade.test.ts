// `Kitsune Dawnblade` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KITSUNE_DAWNBLADE_SCRIPT } from './kitsuneDawnblade';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kitsune Dawnblade";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kitsune Dawnblade"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KITSUNE_DAWNBLADE_SCRIPT]),
  });
  holdEverywhere(g);
