// `Blind Creeper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLIND_CREEPER_SCRIPT } from './blindCreeper';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blind Creeper";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLIND_CREEPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blind Creeper", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLIND_CREEPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Blind Creeper", () => {
  test("Whenever a player casts a spell: it gets -1/-1 until end of turn", () => {
