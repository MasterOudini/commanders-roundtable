// `Biotech Specialist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BIOTECH_SPECIALIST_SCRIPT } from './biotechSpecialist';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Biotech Specialist";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([BIOTECH_SPECIALIST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Biotech Specialist", "Lotus Petal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BIOTECH_SPECIALIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Biotech Specialist", () => {
  ...
  test("Whenever you sacrifice an artifact: it deals 2 damage to the declared player", () => {
