// `Omen of the Hunt` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Omen of the Hunt", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OMEN_OF_THE_HUNT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Omen of the Hunt", () => {
  test("When this enchantment enters: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
  ...
  test("{2}{G}, Sacrifice this enchantment: scry 2 asks, and the cards stay on top", () => {
