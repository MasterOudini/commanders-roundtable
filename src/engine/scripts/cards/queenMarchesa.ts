// `Queen Marchesa` - a etb trigger monarch, a upkeep trigger token
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { QUEEN_MARCHESA } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(QUEEN_MARCHESA, "Deathtouch, haste\nWhen Queen Marchesa enters, you become the monarch.\nAt the beginning of your upkeep, if an opponent is the monarch, create a 1/1 black Assassin creature token with deathtouch and haste.");
const LINES = PRINTED.split('\n');
const TOKEN_L2 = tokenRef("Assassin|1/1|B|Creature|deathtouch|haste");

// "as long as an opponent is the monarch" - read off the state, the PRINTED faces, the turn record, the life totals and the live combat; never derived (D317, D398).
function ifCond2Of(ctx: ScriptCtx, self: InstanceId): boolean {
  const me = ctx.query.controllerOf(self);
  if (me === null) return false;
  return ctx.state.monarch !== null && ctx.state.monarch !== me;
}


export const QUEEN_MARCHESA_SCRIPT: CardScript = {
  oracleId: QUEEN_MARCHESA.oracleId,
  name: QUEEN_MARCHESA.name,
  triggers: [
    {
      abilityId: 'etb-1',
