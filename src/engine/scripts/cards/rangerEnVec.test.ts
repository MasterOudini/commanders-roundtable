// `Ranger en-Vec` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RANGER_EN_VEC_SCRIPT } from './rangerEnVec';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ranger en-Vec";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ranger en-Vec", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RANGER_EN_VEC_SCRIPT]),
  });
  holdEverywhere(g);
