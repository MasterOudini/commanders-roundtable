// `Stagecoach Security` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STAGECOACH_SECURITY_SCRIPT } from './stagecoachSecurity';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stagecoach Security";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STAGECOACH_SECURITY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stagecoach Security", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STAGECOACH_SECURITY_SCRIPT]),
...
describe("Stagecoach Security", () => {
  test("When this creature enters: its controller's creatures get +1/+1 and gain vigilance until end of turn", () => {
