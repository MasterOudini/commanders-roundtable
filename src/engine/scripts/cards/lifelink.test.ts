// `Lifelink` - cast on Grizzly Bears it attaches and the enchanted creature has lifelink;
...
import { LIFELINK_SCRIPT } from './lifelink';
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([LIFELINK_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([LIFELINK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lifelink", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has lifelink; Cyclops of One-Eyed Pass is untouched", () => {
