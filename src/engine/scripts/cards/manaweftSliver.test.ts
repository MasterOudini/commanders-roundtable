// `Manaweft Sliver` - the granted MANA ability proven end to end (D372): the recipient HAS the
...
import { MANAWEFT_SLIVER_SCRIPT } from './manaweftSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Manaweft Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Manaweft Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MANAWEFT_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
