// `Agna Qel'a` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AGNA_QELA_SCRIPT } from './agnaQela';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Agna Qel'a";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Agna Qel'a"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AGNA_QELA_SCRIPT]),
  });
  holdEverywhere(g);
