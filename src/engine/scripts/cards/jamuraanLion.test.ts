// `Jamuraan Lion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JAMURAAN_LION_SCRIPT } from './jamuraanLion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jamuraan Lion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jamuraan Lion"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JAMURAAN_LION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Jamuraan Lion", () => {
  test("{W}, {T}: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
