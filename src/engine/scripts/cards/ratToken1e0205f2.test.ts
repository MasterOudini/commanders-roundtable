// `Rat` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([RAT_TOKEN1E0205F2_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: RAT_1E0205F2_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === RAT_1E0205F2_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
...
describe("Rat", () => {
  test("This token can't block.: the block is refused", () => {
