// `Syndicate Heavy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SYNDICATE_HEAVY_SCRIPT } from './syndicateHeavy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Syndicate Heavy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Syndicate Heavy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SYNDICATE_HEAVY_SCRIPT]),
  });
  holdEverywhere(g);
