// `Chandler` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { CHANDLER_SCRIPT } from './chandler';
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Chandler"], ["Grapeshot Catapult", "Grizzly Bears"]],
    scripts: createRegistry([CHANDLER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chandler", () => {
  test("Grapeshot Catapult is a legal target and it is destroyed", () => {
