// `Lifesmith` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIFESMITH_SCRIPT } from './lifesmith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lifesmith";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lifesmith", "Sol Ring", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIFESMITH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lifesmith", () => {
  test("Whenever you cast an artifact spell: the vocabulary resolves \"You may pay {1}. If you do, you gain 3 life.\" - the price is declined", () => {
  ...
  test("Whenever you cast an artifact spell: the vocabulary resolves \"You may pay {1}. If you do, you gain 3 life.\" - the price is paid", () => {
