// `Kor Firewalker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KOR_FIREWALKER_SCRIPT } from './korFirewalker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kor Firewalker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kor Firewalker", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KOR_FIREWALKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kor Firewalker", () => {
  test("Whenever a player casts a red spell: 1 life is gained", () => {
