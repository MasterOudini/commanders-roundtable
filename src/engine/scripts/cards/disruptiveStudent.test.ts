// `Disruptive Student` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DISRUPTIVE_STUDENT_SCRIPT } from './disruptiveStudent';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Disruptive Student";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Disruptive Student"], ["Cyclops of One-Eyed Pass", "Divination", "Forest"]],
    scripts: createRegistry([DISRUPTIVE_STUDENT_SCRIPT]),
  });
  holdEverywhere(g);
