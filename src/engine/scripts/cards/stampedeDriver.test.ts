// `Stampede Driver` - the printed cost buys the pump until end of turn; it ends at cleanup;
// replay equal (D301). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { STAMPEDE_DRIVER_SCRIPT } from './stampedeDriver';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stampede Driver";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STAMPEDE_DRIVER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stampede Driver", "Coral Eel", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STAMPEDE_DRIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stampede Driver", () => {
  test("{1}{G}, {T}, Discard a card: its controller's creatures get +1/+1 and gain trample until end of turn", () => {
