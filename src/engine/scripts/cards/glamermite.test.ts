// `Glamermite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GLAMERMITE_SCRIPT } from './glamermite';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Glamermite";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glamermite", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLAMERMITE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Glamermite", () => {
  test("When this creature enters: choosing \"Tap target creature.\", tapTarget", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.tapped).toBe(true);
  });

  test("When this creature enters: choosing \"Untap target creature.\", the vocabulary resolves \"Untap target creature.\"", () => {
    const { g, no } = armed(1);
