// `Wizard Replica` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WIZARD_REPLICA_SCRIPT } from './wizardReplica';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wizard Replica";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wizard Replica"], ["Cyclops of One-Eyed Pass", "Divination", "Forest", "Forest"]],
    scripts: createRegistry([WIZARD_REPLICA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wizard Replica", () => {
  test("{U}, Sacrifice this creature: the vocabulary resolves \"Counter target spell unless its controller pays {2}.\" - the price is declined", () => {
  ...
  test("{U}, Sacrifice this creature: the vocabulary resolves \"Counter target spell unless its controller pays {2}.\" - the price is paid", () => {
    const { g, self, payA0_p2_0, payA0_p2_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
