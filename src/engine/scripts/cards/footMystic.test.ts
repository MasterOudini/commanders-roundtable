// `Foot Mystic` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FOOT_MYSTIC_SCRIPT } from './footMystic';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Foot Mystic";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Foot Mystic", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOOT_MYSTIC_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Foot Mystic", () => {
  test("Disappear — When this creature enters: 1 token made", () => {
