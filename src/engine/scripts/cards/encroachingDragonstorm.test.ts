// `Encroaching Dragonstorm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ENCROACHING_DRAGONSTORM_SCRIPT } from './encroachingDragonstorm';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Encroaching Dragonstorm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Encroaching Dragonstorm", "Grizzly Bears", "Dragonborn Looter", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ENCROACHING_DRAGONSTORM_SCRIPT]),
...
describe("Encroaching Dragonstorm", () => {
  test("When this enchantment enters: the vocabulary resolves \"Search your library for up to two basic land cards, put them onto the battlefield tapped, then shuffle.\"", () => {
  ...
  test("When a Dragon you control enters: the vocabulary resolves \"Return this enchantment to its owner's hand.\"", () => {
    const {  } = armed(1);
  });
