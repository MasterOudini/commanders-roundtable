// `Ceremonial Guard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CEREMONIAL_GUARD_SCRIPT } from './ceremonialGuard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ceremonial Guard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ceremonial Guard"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([CEREMONIAL_GUARD_SCRIPT]),
...
describe("Ceremonial Guard", () => {
  test("When this creature attacks or blocks: the vocabulary resolves \"Destroy ~ at end of combat.\" [attacks]", () => {
    ...
    { const boardD = onBoard(g);
      ...
      expect(g.state.cards[self]?.zone.kind, "destroy at the step").toBe('graveyard');
    }
    expect(g.state.delayedTriggers.length, 'every entry fired').toBe(0);
  });

  test("When this creature attacks or blocks: the vocabulary resolves \"Destroy ~ at end of combat.\" [blocks]", () => {
