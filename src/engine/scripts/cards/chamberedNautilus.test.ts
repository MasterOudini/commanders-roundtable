// `Chambered Nautilus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHAMBERED_NAUTILUS_SCRIPT } from './chamberedNautilus';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chambered Nautilus";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chambered Nautilus"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHAMBERED_NAUTILUS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chambered Nautilus", () => {
  test("Whenever this creature becomes blocked: a card is drawn", () => {
