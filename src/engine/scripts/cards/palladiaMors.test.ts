// `Palladia-Mors` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PALLADIA_MORS_SCRIPT } from './palladiaMors';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Palladia-Mors";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Palladia-Mors", "Plains", "Mountain", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PALLADIA_MORS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Palladia-Mors", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice ~ unless you pay {R}{G}{W}.\" - the price is declined", () => {
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice ~ unless you pay {R}{G}{W}.\" - the price is paid", () => {
