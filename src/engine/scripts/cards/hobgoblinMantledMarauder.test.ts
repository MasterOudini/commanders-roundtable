// `Hobgoblin, Mantled Marauder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOBGOBLIN_MANTLED_MARAUDER_SCRIPT } from './hobgoblinMantledMarauder';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hobgoblin, Mantled Marauder";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HOBGOBLIN_MANTLED_MARAUDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hobgoblin, Mantled Marauder"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOBGOBLIN_MANTLED_MARAUDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hobgoblin, Mantled Marauder", () => {
  test("Whenever you discard a card: it gets +2/+0 until end of turn", () => {
