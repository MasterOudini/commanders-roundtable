// `Surveilling Sprite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SURVEILLING_SPRITE_SCRIPT } from './surveillingSprite';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Surveilling Sprite";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Surveilling Sprite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SURVEILLING_SPRITE_SCRIPT]),
  });
  holdEverywhere(g);
