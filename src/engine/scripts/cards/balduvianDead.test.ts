// `Balduvian Dead` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BALDUVIAN_DEAD_SCRIPT } from './balduvianDead';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Balduvian Dead";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; tok0: readonly InstanceId[]; exg0: InstanceId[] };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Balduvian Dead", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BALDUVIAN_DEAD_SCRIPT]),
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, tok0, exg0 };
}

describe("Balduvian Dead", () => {
  test("{2}{R}, Exile a creature card from your graveyard: the vocabulary resolves \"Create a 3/1 black and red Graveborn creature token with haste. Sacrifice it at the beginning of the next end step.\"", () => {
    const { g, board0, tok0, exg0 } = armed(0);
