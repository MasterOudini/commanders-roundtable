// `Rumble Arena` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUMBLE_ARENA_SCRIPT } from './rumbleArena';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rumble Arena";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rumble Arena"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUMBLE_ARENA_SCRIPT]),
  });
  holdEverywhere(g);
