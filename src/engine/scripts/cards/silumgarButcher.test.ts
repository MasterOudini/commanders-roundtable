// `Silumgar Butcher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SILUMGAR_BUTCHER_SCRIPT } from './silumgarButcher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Silumgar Butcher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Silumgar Butcher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SILUMGAR_BUTCHER_SCRIPT]),
...
describe("Silumgar Butcher", () => {
  test("When this creature exploits a creature: the vocabulary resolves \"Target creature gets -3/-3 until end of turn.\"", () => {
    const { g, no } = armed(0);
