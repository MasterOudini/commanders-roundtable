// `Lounge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOUNGE_SCRIPT } from './lounge';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lounge"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOUNGE_SCRIPT]),
  });
  holdEverywhere(g);
