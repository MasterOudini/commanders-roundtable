// `Markov Purifier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARKOV_PURIFIER_SCRIPT } from './markovPurifier';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Markov Purifier";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Markov Purifier", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARKOV_PURIFIER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Markov Purifier", () => {
  test("At the beginning of your end step: the vocabulary resolves \"You may pay {2}. If you do, draw a card.\" - the price is declined", () => {
    const { g, payL1_p1_0, payL1_p1_1 } = armed(0);
  ...
  test("At the beginning of your end step: the vocabulary resolves \"You may pay {2}. If you do, draw a card.\" - the price is paid", () => {
    const { g, payL1_p1_0, payL1_p1_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
