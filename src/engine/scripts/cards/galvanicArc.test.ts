// `Galvanic Arc` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GALVANIC_ARC_SCRIPT } from './galvanicArc';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Galvanic Arc";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([GALVANIC_ARC_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Galvanic Arc", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GALVANIC_ARC_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Galvanic Arc", () => {
  test("When this Aura enters: it deals 3 damage to the declared creature", () => {
    const { g, no } = armed(0);
