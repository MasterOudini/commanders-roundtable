// `Faerie` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FAERIE_TOKEN0F9A993F_SCRIPT } from './faerieToken0f9a993f';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([FAERIE_TOKEN0F9A993F_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: FAERIE_0F9A993F_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === FAERIE_0F9A993F_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
