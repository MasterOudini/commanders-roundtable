// `Mind's Eye` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MINDS_EYE_SCRIPT } from './mindsEye';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mind's Eye";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mind's Eye", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MINDS_EYE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mind's Eye", () => {
  test("Whenever an opponent draws a card: the vocabulary resolves \"You may pay {1}. If you do, draw a card.\" - the price is declined", () => {
  ...
  test("Whenever an opponent draws a card: the vocabulary resolves \"You may pay {1}. If you do, draw a card.\" - the price is paid", () => {
    const { g, payL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
