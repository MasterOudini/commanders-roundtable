// `Screaming Shield` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCREAMING_SHIELD_SCRIPT } from './screamingShield';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SCREAMING_SHIELD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Screaming Shield", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCREAMING_SHIELD_SCRIPT]),
...
describe("Screaming Shield", () => {
  test("Equipped creature gets +0/+3 and has \"{2}, {T}: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([2, 5]);
  });

  test("Equipped creature gets +0/+3 and has \"{2}, {T}: the vocabulary resolves \"Target player mills three cards.\"", () => {
    const { g, p2gy0, bearsB } = armed(1);
