// `Alaborn Cavalier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ALABORN_CAVALIER_SCRIPT } from './alabornCavalier';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Alaborn Cavalier";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Alaborn Cavalier"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ALABORN_CAVALIER_SCRIPT]),
  });
  holdEverywhere(g);
