// `Wanderer's Twig` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wanderer's Twig", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WANDERERS_TWIG_SCRIPT]),
  });
  holdEverywhere(g);
