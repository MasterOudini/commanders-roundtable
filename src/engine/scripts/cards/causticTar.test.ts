// `Caustic Tar` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { CAUSTIC_TAR_SCRIPT } from './causticTar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Caustic Tar";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Caustic Tar","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAUSTIC_TAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Caustic Tar", () => {
  test("grants {T}: Target player loses 3 life. to Forest, and to nothing outside the scope", () => {
