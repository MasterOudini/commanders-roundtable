// `Skullclamp` - equipped to Grizzly Bears the creature gets +1/-1; (on the equipped creature dying: draw 2); the Cyclops is untouched;
...
import { SKULLCLAMP_SCRIPT } from './skullclamp';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skullclamp";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SKULLCLAMP_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SKULLCLAMP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skullclamp", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/-1; (on the equipped creature dying: draw 2); the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([3, 1]);
  ...
  test("the equipped creature dying: (on the equipped creature dying: draw 2)", () => {
    const { g, self, host } = board();
    equip(g, self, host);
