// `Waylaying Pirates` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WAYLAYING_PIRATES_SCRIPT } from './waylayingPirates';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Waylaying Pirates";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Waylaying Pirates", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WAYLAYING_PIRATES_SCRIPT]),
