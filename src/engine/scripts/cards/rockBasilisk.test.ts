// `Rock Basilisk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROCK_BASILISK_SCRIPT } from './rockBasilisk';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rock Basilisk";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rock Basilisk"], ["Cyclops of One-Eyed Pass", "Grizzled Leotau"]],
    scripts: createRegistry([ROCK_BASILISK_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const gift2 = put(g, 'p2', "Grizzled Leotau");
  settle(g);
