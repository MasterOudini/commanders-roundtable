// `Satyr` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SATYR_TOKEN49BDAE00_SCRIPT } from './satyrToken49bdae00';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([SATYR_TOKEN49BDAE00_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: SATYR_49BDAE00_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === SATYR_49BDAE00_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
