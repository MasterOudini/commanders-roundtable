// `Sibsig Icebreakers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SIBSIG_ICEBREAKERS_SCRIPT } from './sibsigIcebreakers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sibsig Icebreakers";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sibsig Icebreakers"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIBSIG_ICEBREAKERS_SCRIPT]),
  });
  holdEverywhere(g);
