// `Vrock` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VROCK_SCRIPT } from './vrock';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vrock", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VROCK_SCRIPT]),
...
describe("Vrock", () => {
  test("Toxic Spores — At the beginning of your end step: each opponent loses 3 life", () => {
