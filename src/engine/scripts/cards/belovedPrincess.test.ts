// `Beloved Princess` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BELOVED_PRINCESS_SCRIPT } from './belovedPrincess';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Beloved Princess";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Beloved Princess", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BELOVED_PRINCESS_SCRIPT]),
  });
  holdEverywhere(g);
