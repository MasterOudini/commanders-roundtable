// `Ant-Man's Air Force` - every declared pick is accepted and the pump lands; a permanent the clause
// excludes is refused (D299). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ANT_MANS_AIR_FORCE_SCRIPT } from './antMansAirForce';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ant-Man's Air Force";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ANT_MANS_AIR_FORCE_SCRIPT]));
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Ant-Man's Air Force"], ["Grizzly Bears", "Forest"]],
    scripts: createRegistry([ANT_MANS_AIR_FORCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ant-Man's Air Force", () => {
  test("Grizzly Bears is accepted and the pump lands", () => {
    ...
    settle(g);
    expect(pt(g, target)).toEqual([1, 2]);
  });
