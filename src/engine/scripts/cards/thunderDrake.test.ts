// `Thunder Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THUNDER_DRAKE_SCRIPT } from './thunderDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thunder Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thunder Drake", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THUNDER_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
