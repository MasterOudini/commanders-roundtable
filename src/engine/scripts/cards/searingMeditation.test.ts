// `Searing Meditation` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEARING_MEDITATION_SCRIPT } from './searingMeditation';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Searing Meditation";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Searing Meditation", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEARING_MEDITATION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Searing Meditation", () => {
  test("Whenever you gain life: the vocabulary resolves \"You may pay {2}. If you do, this enchantment deals 2 damage to any target.\" - the price is declined", () => {
    const { g, no, payL0_p1_0, payL0_p1_1 } = armed(0);
  ...
  test("Whenever you gain life: the vocabulary resolves \"You may pay {2}. If you do, this enchantment deals 2 damage to any target.\" - the price is paid", () => {
    const { g, no, payL0_p1_0, payL0_p1_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
