// `Jabari's Banner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JABARIS_BANNER_SCRIPT } from './jabarisBanner';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jabari's Banner";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([JABARIS_BANNER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jabari's Banner"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JABARIS_BANNER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Jabari's Banner", () => {
  test("{1}, {T}: the declared creature is pumped until end of turn", () => {
    const { g, self, no } = armed(0);
    expect(pt(g, no)).toEqual([5, 2]);
    expect(kw(g, no).has("flanking")).toBe(true);
