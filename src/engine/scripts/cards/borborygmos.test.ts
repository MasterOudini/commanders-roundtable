// `Borborygmos` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BORBORYGMOS_SCRIPT } from './borborygmos';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Borborygmos";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Borborygmos", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BORBORYGMOS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Borborygmos", () => {
  test("Whenever Borborygmos deals combat damage to a player: each of its controller's creatures gets 1 +1/+1 counter", () => {
