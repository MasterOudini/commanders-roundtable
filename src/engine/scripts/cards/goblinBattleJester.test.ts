// `Goblin Battle Jester` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_BATTLE_JESTER_SCRIPT } from './goblinBattleJester';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Battle Jester";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Battle Jester", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_BATTLE_JESTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Battle Jester", () => {
  test("Whenever you cast a red spell: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
    const { g, no } = armed(0);
