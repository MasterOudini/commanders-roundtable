// `Clinging Darkness` - cast on Grizzly Bears it attaches and the enchanted creature gets -4/-1;
...
import { CLINGING_DARKNESS_SCRIPT } from './clingingDarkness';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Clinging Darkness";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CLINGING_DARKNESS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([CLINGING_DARKNESS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Clinging Darkness", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets -4/-1; Cyclops of One-Eyed Pass is untouched", () => {
