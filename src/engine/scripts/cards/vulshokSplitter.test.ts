// `Vulshok Splitter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VULSHOK_SPLITTER_SCRIPT } from './vulshokSplitter';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vulshok Splitter";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VULSHOK_SPLITTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vulshok Splitter", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VULSHOK_SPLITTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vulshok Splitter", () => {
  test("For Mirrodin! (When this Equipment enters: a token is made and it attaches to it", () => {
    ...
    if (germ !== null) expect(pt(g, germ)).toEqual([4, 2]);
  });
