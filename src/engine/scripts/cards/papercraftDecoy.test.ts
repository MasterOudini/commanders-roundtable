// `Papercraft Decoy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PAPERCRAFT_DECOY_SCRIPT } from './papercraftDecoy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Papercraft Decoy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Papercraft Decoy", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PAPERCRAFT_DECOY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Papercraft Decoy", () => {
  test("When this creature leaves the battlefield: the vocabulary resolves \"You may pay {2}. If you do, draw a card.\" - the price is declined", () => {
  ...
  test("When this creature leaves the battlefield: the vocabulary resolves \"You may pay {2}. If you do, draw a card.\" - the price is paid", () => {
    const { g, payL0_p1_0, payL0_p1_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
