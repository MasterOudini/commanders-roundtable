// `Grasping Scoundrel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRASPING_SCOUNDREL_SCRIPT } from './graspingScoundrel';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grasping Scoundrel";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GRASPING_SCOUNDREL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grasping Scoundrel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRASPING_SCOUNDREL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Grasping Scoundrel", () => {
  test("`Grasping Scoundrel` - as long as it's attacking: absent while it is unmet, read once it holds", () => {
