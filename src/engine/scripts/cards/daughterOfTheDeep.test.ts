// `Daughter of the Deep` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAUGHTER_OF_THE_DEEP_SCRIPT } from './daughterOfTheDeep';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Daughter of the Deep";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Daughter of the Deep", "Divination"], ["Cyclops of One-Eyed Pass", "Merfolk of the Pearl Trident"]],
    scripts: createRegistry([DAUGHTER_OF_THE_DEEP_SCRIPT]),
...
describe("Daughter of the Deep", () => {
  test("Whenever you draw your second card each turn: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("{U}, {T}: the vocabulary resolves \"Target Merfolk can't be blocked this turn.\"", () => {
