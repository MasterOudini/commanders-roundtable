// `Undead Warchief` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([UNDEAD_WARCHIEF_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Undead Warchief", "Scathe Zombies"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNDEAD_WARCHIEF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Undead Warchief", () => {
  test("Zombie creatures you control get +2/+1.: only the Zombie reads it", () => {
