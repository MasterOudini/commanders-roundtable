// `Borderland Marauder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BORDERLAND_MARAUDER_SCRIPT } from './borderlandMarauder';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Borderland Marauder";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BORDERLAND_MARAUDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Borderland Marauder"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BORDERLAND_MARAUDER_SCRIPT]),
  });
  holdEverywhere(g);
