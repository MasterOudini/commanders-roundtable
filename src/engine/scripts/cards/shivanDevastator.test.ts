// `Shivan Devastator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHIVAN_DEVASTATOR_SCRIPT } from './shivanDevastator';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shivan Devastator";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHIVAN_DEVASTATOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shivan Devastator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHIVAN_DEVASTATOR_SCRIPT]),
  });
  holdEverywhere(g);
