// `Phyrexian Ghoul` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHYREXIAN_GHOUL_SCRIPT } from './phyrexianGhoul';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phyrexian Ghoul";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PHYREXIAN_GHOUL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phyrexian Ghoul", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHYREXIAN_GHOUL_SCRIPT]),
  });
  holdEverywhere(g);
