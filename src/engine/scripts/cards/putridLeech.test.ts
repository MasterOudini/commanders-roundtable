// `Putrid Leech` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PUTRID_LEECH_SCRIPT } from './putridLeech';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Putrid Leech";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PUTRID_LEECH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Putrid Leech", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PUTRID_LEECH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Putrid Leech", () => {
  test("Pay 2 life: it gets +2/+2 until end of turn", () => {
