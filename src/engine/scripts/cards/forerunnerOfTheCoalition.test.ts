// `Forerunner of the Coalition` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FORERUNNER_OF_THE_COALITION_SCRIPT } from './forerunnerOfTheCoalition';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Forerunner of the Coalition";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Forerunner of the Coalition", "Grizzly Bears", "Swab Goblin", "Cloud Pirates"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FORERUNNER_OF_THE_COALITION_SCRIPT]),
...
describe("Forerunner of the Coalition", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a Pirate card, reveal it, then shuffle and put that card on top.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Cloud Pirates" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Cloud Pirates");
      expect(found, "Cloud Pirates is not in the library for the search to find").toBeDefined();
  ...
  test("Whenever another Pirate you control enters: each opponent loses 1 life", () => {
