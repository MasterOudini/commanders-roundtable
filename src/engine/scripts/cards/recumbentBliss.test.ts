// `Recumbent Bliss` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RECUMBENT_BLISS_SCRIPT } from './recumbentBliss';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Recumbent Bliss";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Recumbent Bliss", "Grizzly Bears", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RECUMBENT_BLISS_SCRIPT]),
  });
  holdEverywhere(g);
