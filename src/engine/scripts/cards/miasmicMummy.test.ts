// `Miasmic Mummy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MIASMIC_MUMMY_SCRIPT } from './miasmicMummy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Miasmic Mummy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Miasmic Mummy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIASMIC_MUMMY_SCRIPT]),
  });
  holdEverywhere(g);
