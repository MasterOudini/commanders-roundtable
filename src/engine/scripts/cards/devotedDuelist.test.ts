// `Devoted Duelist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEVOTED_DUELIST_SCRIPT } from './devotedDuelist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Devoted Duelist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Devoted Duelist", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEVOTED_DUELIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Devoted Duelist", () => {
  test("Flurry — Whenever you cast your second spell each turn: it deals 1 damage to each opponent", () => {
