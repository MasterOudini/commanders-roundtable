// `Aether Channeler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AETHER_CHANNELER_SCRIPT } from './aetherChanneler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aether Channeler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aether Channeler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AETHER_CHANNELER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aether Channeler", () => {
  test("When this creature enters: choosing \"Create a 1/1 white Bird creature token with flying.\", token [etb 1]", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("When this creature enters: choosing \"Return another target nonland permanent to its owner's hand.\", the vocabulary resolves \"Return another target nonland permanent to its owner's hand.\" [etb 2]", () => {
    const { g, no } = armed(1);
    expect(g.state.cards[no]?.zone.kind).toBe('hand');
  });

  test("When this creature enters: choosing \"Draw a card.\", draw [etb 3]", () => {
