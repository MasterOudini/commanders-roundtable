// `Yavimaya's Embrace` - every printed ability proven in its own game: the cost's mark, the pump
...
import { YAVIMAYAS_EMBRACE_SCRIPT } from './yavimayasEmbrace';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Yavimaya's Embrace";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([YAVIMAYAS_EMBRACE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Yavimaya's Embrace", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YAVIMAYAS_EMBRACE_SCRIPT]),
...
describe("Yavimaya's Embrace", () => {
  test("Enchanted creature gets +2/+2 and has trample.: the creature it is attached to reads it", () => {
