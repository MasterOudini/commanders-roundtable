// `Dark Favor` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: lose 1 life); gets +3/+1;
...
import { DARK_FAVOR_SCRIPT } from './darkFavor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dark Favor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DARK_FAVOR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([DARK_FAVOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dark Favor", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: lose 1 life); gets +3/+1; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    const life0 = g.state.players.p1?.life ?? 0;
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([5, 3]);
