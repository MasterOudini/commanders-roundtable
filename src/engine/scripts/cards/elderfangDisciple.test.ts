// `Elderfang Disciple` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ELDERFANG_DISCIPLE_SCRIPT } from './elderfangDisciple';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Elderfang Disciple";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elderfang Disciple"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELDERFANG_DISCIPLE_SCRIPT]),
  });
  holdEverywhere(g);
