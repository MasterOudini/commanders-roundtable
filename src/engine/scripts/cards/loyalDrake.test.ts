// `Loyal Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOYAL_DRAKE_SCRIPT } from './loyalDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Loyal Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Loyal Drake"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOYAL_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Loyal Drake", () => {
  test("Lieutenant — At the beginning of combat on your turn: a card is drawn", () => {
