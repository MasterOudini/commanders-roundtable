// `Nightsquad Commando` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIGHTSQUAD_COMMANDO_SCRIPT } from './nightsquadCommando';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nightsquad Commando";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nightsquad Commando", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIGHTSQUAD_COMMANDO_SCRIPT]),
  });
  holdEverywhere(g);
