// `Vermiculos` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VERMICULOS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vermiculos", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VERMICULOS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vermiculos", () => {
  test("Whenever an artifact enters: it gets +4/+4 until end of turn", () => {
