// `Wingblade Disciple` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WINGBLADE_DISCIPLE_SCRIPT } from './wingbladeDisciple';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wingblade Disciple";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wingblade Disciple", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WINGBLADE_DISCIPLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wingblade Disciple", () => {
  test("Flurry — Whenever you cast your second spell each turn: 1 token made", () => {
