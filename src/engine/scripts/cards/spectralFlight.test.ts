// `Spectral Flight` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2 and has flying;
...
import { SPECTRAL_FLIGHT_SCRIPT } from './spectralFlight';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spectral Flight";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPECTRAL_FLIGHT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SPECTRAL_FLIGHT_SCRIPT]),
  });
  holdEverywhere(g);
