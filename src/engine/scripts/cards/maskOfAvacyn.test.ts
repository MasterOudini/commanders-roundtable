// `Mask of Avacyn` - equipped to Grizzly Bears the creature gets +1/+2 and has hexproof; the Cyclops is untouched;
...
import { MASK_OF_AVACYN_SCRIPT } from './maskOfAvacyn';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mask of Avacyn";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MASK_OF_AVACYN_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([MASK_OF_AVACYN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mask of Avacyn", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+2 and has hexproof; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([3, 4]);
