// `War Oracle` - a renownDamage trigger renown
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { WAR_ORACLE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(WAR_ORACLE, "Lifelink (Damage dealt by this creature also causes you to gain that much life.)\nRenown 1 (When this creature deals combat damage to a player, if it isn't renowned, put a +1/+1 counter on it and it becomes renowned.)");
const LINES = PRINTED.split('\n');

export const WAR_ORACLE_SCRIPT: CardScript = {
  oracleId: WAR_ORACLE.oracleId,
  name: WAR_ORACLE.name,
  triggers: [
    {
