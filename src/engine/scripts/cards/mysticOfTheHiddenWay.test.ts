// `Mystic of the Hidden Way` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MYSTIC_OF_THE_HIDDEN_WAY_SCRIPT } from './mysticOfTheHiddenWay';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mystic of the Hidden Way";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mystic of the Hidden Way", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MYSTIC_OF_THE_HIDDEN_WAY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mystic of the Hidden Way", () => {
