// `Agent 13, Sharon Carter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AGENT13_SHARON_CARTER_SCRIPT } from './agent13SharonCarter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Agent 13, Sharon Carter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Agent 13, Sharon Carter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AGENT13_SHARON_CARTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Agent 13, Sharon Carter", () => {
  test("Whenever a creature you control attacks alone: 1 token made", () => {
