// `Vineweft` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VINEWEFT_SCRIPT } from './vineweft';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VINEWEFT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vineweft", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VINEWEFT_SCRIPT]),
  });
  holdEverywhere(g);
