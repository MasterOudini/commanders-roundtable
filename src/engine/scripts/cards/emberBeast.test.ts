// `Ember Beast` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EMBER_BEAST_SCRIPT } from './emberBeast';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ember Beast";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ember Beast", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EMBER_BEAST_SCRIPT]),
  });
  holdEverywhere(g);
