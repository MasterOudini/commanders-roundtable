// `Spine of Ish Sah` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPINE_OF_ISH_SAH_SCRIPT } from './spineOfIshSah';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spine of Ish Sah";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spine of Ish Sah", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPINE_OF_ISH_SAH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spine of Ish Sah", () => {
  test("When this artifact enters: the vocabulary resolves \"Destroy target permanent.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
  });

  test("When this artifact is put into a graveyard from the battlefield: it returns to its owner's hand", () => {
