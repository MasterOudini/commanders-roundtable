// `Blazing Specter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLAZING_SPECTER_SCRIPT } from './blazingSpecter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blazing Specter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blazing Specter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLAZING_SPECTER_SCRIPT]),
  });
  holdEverywhere(g);
