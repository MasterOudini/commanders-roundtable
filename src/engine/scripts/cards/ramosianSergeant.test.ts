// `Ramosian Sergeant` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ramosian Sergeant", "Children of Korlis"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAMOSIAN_SERGEANT_SCRIPT]),
...
describe("Ramosian Sergeant", () => {
  test("{3}, {T}: the vocabulary resolves \"Search your library for a Rebel permanent card with mana value 2 or less, put it onto the battlefield, then shuffle.\"", () => {
