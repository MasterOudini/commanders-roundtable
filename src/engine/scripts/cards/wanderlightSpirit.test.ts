// `Wanderlight Spirit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WANDERLIGHT_SPIRIT_SCRIPT } from './wanderlightSpirit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wanderlight Spirit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wanderlight Spirit", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WANDERLIGHT_SPIRIT_SCRIPT]),
  });
  holdEverywhere(g);
