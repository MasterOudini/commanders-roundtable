// `Tidecaller Mentor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TIDECALLER_MENTOR_SCRIPT } from './tidecallerMentor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tidecaller Mentor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tidecaller Mentor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TIDECALLER_MENTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Tidecaller Mentor", () => {
  test("Threshold — When this creature enters: the vocabulary resolves \"Return up to one target nonland permanent to its owner's hand.\"", () => {
    const { g, no } = armed(0);
