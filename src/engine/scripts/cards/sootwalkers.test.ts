// `Sootwalkers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOOTWALKERS_SCRIPT } from './sootwalkers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sootwalkers";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sootwalkers", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Devoted Hero"]],
    scripts: createRegistry([SOOTWALKERS_SCRIPT]),
