// `Foreboding Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Foreboding Landscape", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOREBODING_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Foreboding Landscape", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Swamp, Forest, or Island card, put it onto the battlefield tapped, then shuffle.\"", () => {
