// `Loxodon Partisan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOXODON_PARTISAN_SCRIPT } from './loxodonPartisan';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Loxodon Partisan";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LOXODON_PARTISAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Loxodon Partisan", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOXODON_PARTISAN_SCRIPT]),
  });
  holdEverywhere(g);
