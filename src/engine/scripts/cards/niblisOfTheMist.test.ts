// `Niblis of the Mist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIBLIS_OF_THE_MIST_SCRIPT } from './niblisOfTheMist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Niblis of the Mist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Niblis of the Mist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIBLIS_OF_THE_MIST_SCRIPT]),
  });
  holdEverywhere(g);
