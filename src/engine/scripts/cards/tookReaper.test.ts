// `Took Reaper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TOOK_REAPER_SCRIPT } from './tookReaper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Took Reaper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Took Reaper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TOOK_REAPER_SCRIPT]),
