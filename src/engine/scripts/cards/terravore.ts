// `Terravore` - a static cdaCount
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { TERRAVORE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(TERRAVORE, "Trample\nTerravore's power and toughness are each equal to the number of land cards in all graveyards.");
const LINES = PRINTED.split('\n');

// "land cards in all graveyards", read off the printed faces (a count is not a characteristic, CR 604.3).
...
export const TERRAVORE_SCRIPT: CardScript = {
  oracleId: TERRAVORE.oracleId,
  name: TERRAVORE.name,
  statics: [
    {
      abilityId: 'cda-1',
