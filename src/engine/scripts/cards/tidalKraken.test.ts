// `Tidal Kraken` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TIDAL_KRAKEN_SCRIPT } from './tidalKraken';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tidal Kraken";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tidal Kraken", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TIDAL_KRAKEN_SCRIPT]),
  });
  holdEverywhere(g);
