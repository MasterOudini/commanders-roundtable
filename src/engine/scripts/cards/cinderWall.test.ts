// `Cinder Wall` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CINDER_WALL_SCRIPT } from './cinderWall';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cinder Wall";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cinder Wall"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([CINDER_WALL_SCRIPT]),
...
describe("Cinder Wall", () => {
  test("When this creature blocks: the vocabulary resolves \"Destroy ~ at end of combat.\"", () => {
