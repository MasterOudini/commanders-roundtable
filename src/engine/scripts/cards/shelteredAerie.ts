// `Sheltered Aerie` - the permanent it is attached to HAS a quoted
...
import { SHELTERED_AERIE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(SHELTERED_AERIE, "Enchant land\nEnchanted land has \"{T}: Add two mana of any one color.\"");
const LINES = PRINTED.split('\n');

const GRANT = grantedMana("{T}: Add two mana of any one color.", SHELTERED_AERIE.name);

export const SHELTERED_AERIE_SCRIPT: CardScript = {
  oracleId: SHELTERED_AERIE.oracleId,
  name: SHELTERED_AERIE.name,
  statics: [
    {
      abilityId: 'grant-1',
