// `Blisterspit Gremlin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLISTERSPIT_GREMLIN_SCRIPT } from './blisterspitGremlin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blisterspit Gremlin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blisterspit Gremlin", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLISTERSPIT_GREMLIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Blisterspit Gremlin", () => {
  test("{1}, {T}: it deals 1 damage to each opponent", () => {
    const { g, self, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 - 1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("Whenever you cast a noncreature spell: it untaps", () => {
