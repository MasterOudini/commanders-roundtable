// `Bearer of Glory` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BEARER_OF_GLORY_SCRIPT } from './bearerOfGlory';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bearer of Glory";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BEARER_OF_GLORY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bearer of Glory", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BEARER_OF_GLORY_SCRIPT]),
...
describe("Bearer of Glory", () => {
  test("`Bearer of Glory` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
