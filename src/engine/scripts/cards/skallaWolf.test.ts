// `Skalla Wolf` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skalla Wolf", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKALLA_WOLF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skalla Wolf", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top five cards of your library. You may reveal a green card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
