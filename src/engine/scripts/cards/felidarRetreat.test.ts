// `Felidar Retreat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FELIDAR_RETREAT_SCRIPT } from './felidarRetreat';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Felidar Retreat";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([FELIDAR_RETREAT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Felidar Retreat", "Runeclaw Bear"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FELIDAR_RETREAT_SCRIPT]),
...
describe("Felidar Retreat", () => {
  test("Landfall — Whenever a land you control enters: choosing \"Create a 2/2 white Cat Beast creature token.\", token [landfall 1]", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("Landfall — Whenever a land you control enters: choosing \"Put a +1/+1 counter on each creature you control. Those creatures gain vigilance until end of turn.\", the vocabulary resolves \"Put a +1/+1 counter on each creature you control. Those creatures gain vigilance until end of turn.\" [landfall 2]", () => {
    ...
    expect(kw(g, ownBear).has("vigilance"), "gained vigilance").toBe(true);
  });
