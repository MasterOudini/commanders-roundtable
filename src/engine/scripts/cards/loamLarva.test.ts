// `Loam Larva` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Loam Larva", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOAM_LARVA_SCRIPT]),
  });
  holdEverywhere(g);
