// `Codex Shredder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CODEX_SHREDDER_SCRIPT } from './codexShredder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Codex Shredder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Codex Shredder", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CODEX_SHREDDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Codex Shredder", () => {
  test("{T}: the vocabulary resolves \"Target player mills a card.\"", () => {
    const { g, self, p2gy0 } = armed(0);
    expect((g.state.zones.graveyard.p2 ?? []).length).toBe(p2gy0 + 1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{5}, {T}, Sacrifice this artifact: the vocabulary resolves \"Return target card from your graveyard to your hand.\"", () => {
