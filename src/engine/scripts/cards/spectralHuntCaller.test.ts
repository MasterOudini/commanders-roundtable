// `Spectral Hunt-Caller` - the printed cost buys the pump until end of turn; it ends at cleanup;
// replay equal (D301). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SPECTRAL_HUNT_CALLER_SCRIPT } from './spectralHuntCaller';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spectral Hunt-Caller";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPECTRAL_HUNT_CALLER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spectral Hunt-Caller", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPECTRAL_HUNT_CALLER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spectral Hunt-Caller", () => {
  test("{5}{G}: its controller's creatures get +1/+1 and gain trample until end of turn", () => {
