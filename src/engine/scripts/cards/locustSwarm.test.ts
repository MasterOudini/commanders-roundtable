// `Locust Swarm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOCUST_SWARM_SCRIPT } from './locustSwarm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Locust Swarm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Locust Swarm", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOCUST_SWARM_SCRIPT]),
  });
  holdEverywhere(g);
