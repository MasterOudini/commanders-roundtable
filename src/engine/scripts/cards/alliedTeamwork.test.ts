// `Allied Teamwork` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ALLIED_TEAMWORK_SCRIPT } from './alliedTeamwork';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Allied Teamwork";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ALLIED_TEAMWORK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Allied Teamwork", "Makindi Aeronaut"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ALLIED_TEAMWORK_SCRIPT]),
  });
  holdEverywhere(g);
