// `Unrooted Ancestor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNROOTED_ANCESTOR_SCRIPT } from './unrootedAncestor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unrooted Ancestor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Unrooted Ancestor", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNROOTED_ANCESTOR_SCRIPT]),
...
describe("Unrooted Ancestor", () => {
  test("{1}, Sacrifice another creature: the vocabulary resolves \"~ gains indestructible until end of turn. Tap it.\"", () => {
