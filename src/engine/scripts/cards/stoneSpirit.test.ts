// `Stone Spirit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STONE_SPIRIT_SCRIPT } from './stoneSpirit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stone Spirit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stone Spirit", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Ornithopter"]],
    scripts: createRegistry([STONE_SPIRIT_SCRIPT]),
