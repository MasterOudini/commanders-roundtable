// `Salvaging Station` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SALVAGING_STATION_SCRIPT } from './salvagingStation';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Salvaging Station";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Salvaging Station", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SALVAGING_STATION_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtA0_0 = put(g, 'p1', "Sol Ring", 'graveyard');
  settle(g);
...
describe("Salvaging Station", () => {
  test("{T}: the vocabulary resolves \"Return target noncreature artifact card with mana value 1 or less from your graveyard to the battlefield.\"", () => {
  ...
  test("Whenever a creature dies: the vocabulary resolves \"Untap this artifact.\"", () => {
    const {  } = armed(1);
  });
