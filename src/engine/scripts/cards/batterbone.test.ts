// `Batterbone` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BATTERBONE_SCRIPT } from './batterbone';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Batterbone";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BATTERBONE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Batterbone", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BATTERBONE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Batterbone", () => {
  test("Living weapon (When this Equipment enters: a token is made and it attaches to it", () => {
    ...
    if (germ !== null) expect(pt(g, germ)).toEqual([1, 1]);
  });

  test("Equipped creature gets +1/+1 and has vigilance and lifelink.: the creature it is attached to reads it", () => {
