// `Apothecary Geist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { APOTHECARY_GEIST_SCRIPT } from './apothecaryGeist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Apothecary Geist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Apothecary Geist", "Bile Urchin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([APOTHECARY_GEIST_SCRIPT]),
  });
  holdEverywhere(g);
