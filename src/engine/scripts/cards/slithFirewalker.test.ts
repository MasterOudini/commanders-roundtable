// `Slith Firewalker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLITH_FIREWALKER_SCRIPT } from './slithFirewalker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slith Firewalker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slith Firewalker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLITH_FIREWALKER_SCRIPT]),
  });
  holdEverywhere(g);
