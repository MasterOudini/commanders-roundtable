// `Hieromancer's Cage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HIEROMANCERS_CAGE_SCRIPT } from './hieromancersCage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hieromancer's Cage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hieromancer's Cage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HIEROMANCERS_CAGE_SCRIPT]),
  });
  holdEverywhere(g);
