// `Tattered Haunter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TATTERED_HAUNTER_SCRIPT } from './tatteredHaunter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tattered Haunter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tattered Haunter", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TATTERED_HAUNTER_SCRIPT]),
  });
  holdEverywhere(g);
