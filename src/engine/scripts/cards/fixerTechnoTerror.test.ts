// `Fixer, Techno Terror` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIXER_TECHNO_TERROR_SCRIPT } from './fixerTechnoTerror';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fixer, Techno Terror";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fixer, Techno Terror", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIXER_TECHNO_TERROR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fixer, Techno Terror", () => {
  test("{T}, Pay 2 life: a card is drawn - refused unless if an artifact entered under your control this turn", () => {
