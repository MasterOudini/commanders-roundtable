// `Ghost Hounds` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHOST_HOUNDS_SCRIPT } from './ghostHounds';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghost Hounds";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GHOST_HOUNDS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghost Hounds"], ["Cyclops of One-Eyed Pass", "Thraben Standard Bearer"]],
    scripts: createRegistry([GHOST_HOUNDS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ghost Hounds", () => {
  test("Whenever this creature blocks or becomes blocked by a white creature: it gains firstStrike until end of turn", () => {
