// `Ondu Champion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ONDU_CHAMPION_SCRIPT } from './onduChampion';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ondu Champion";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ONDU_CHAMPION_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ondu Champion", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ONDU_CHAMPION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ondu Champion", () => {
  test("Rally — Whenever this creature or another Ally you control enters: its controller's creatures gain trample until end of turn", () => {
