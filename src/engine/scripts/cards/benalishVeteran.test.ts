// `Benalish Veteran` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BENALISH_VETERAN_SCRIPT } from './benalishVeteran';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Benalish Veteran";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BENALISH_VETERAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Benalish Veteran"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BENALISH_VETERAN_SCRIPT]),
  });
  holdEverywhere(g);
