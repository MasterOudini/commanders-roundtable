// `Mu Yanling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MU_YANLING_SCRIPT } from './muYanling';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mu Yanling";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mu Yanling", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MU_YANLING_SCRIPT]),
...
describe("Mu Yanling", () => {
  test("+2: the vocabulary resolves \"Target creature can't be blocked this turn.\"", () => {
  ...
  test("−3: 2 cards are drawn", () => {
    const { g, self, hand0, refused } = armed(1);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 2);
    expect(refused).toBe(true);
    expect(g.state.cards[self]?.counters['loyalty'] ?? 0).toBe(2);
  });

  test("−10: the vocabulary resolves \"Tap all creatures your opponents control. You take an extra turn after this one.\"", () => {
    const { g, self, no, refused } = armed(2);
    expect(g.state.cards[no]?.tapped, 'the witness tapped').toBe(true);
