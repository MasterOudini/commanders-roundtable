// `Valley Dasher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VALLEY_DASHER_SCRIPT } from './valleyDasher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Valley Dasher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Valley Dasher", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VALLEY_DASHER_SCRIPT]),
  });
  holdEverywhere(g);
