// `Wurm's Tooth` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WURMS_TOOTH_SCRIPT } from './wurmsTooth';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wurm's Tooth";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wurm's Tooth", "Grizzly Bears", "Norwood Ranger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WURMS_TOOTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wurm's Tooth", () => {
  test("Whenever a player casts a green spell: 1 life is gained", () => {
