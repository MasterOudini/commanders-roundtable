// `Implement of Ferocity` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IMPLEMENT_OF_FEROCITY_SCRIPT } from './implementOfFerocity';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Implement of Ferocity";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Implement of Ferocity", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IMPLEMENT_OF_FEROCITY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Implement of Ferocity", () => {
  test("{G}, Sacrifice this artifact: the declared creature gets 1 +1/+1 counter", () => {
