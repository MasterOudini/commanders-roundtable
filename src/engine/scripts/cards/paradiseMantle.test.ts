// `Paradise Mantle` - the granted MANA ability proven end to end (D372): the recipient HAS the
...
import { PARADISE_MANTLE_SCRIPT } from './paradiseMantle';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Paradise Mantle";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Paradise Mantle","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PARADISE_MANTLE_SCRIPT]),
  });
  holdEverywhere(g);
