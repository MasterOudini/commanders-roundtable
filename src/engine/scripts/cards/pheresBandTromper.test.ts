// `Pheres-Band Tromper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHERES_BAND_TROMPER_SCRIPT } from './pheresBandTromper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pheres-Band Tromper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pheres-Band Tromper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHERES_BAND_TROMPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pheres-Band Tromper", () => {
  test("Inspired — Whenever this creature becomes untapped: it gets 1 +1/+1 counter", () => {
