// `Maro` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MARO_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Maro"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARO_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Maro", () => {
  test("Maro's power and toughness are each equal to the number of cards in your hand.: its power and toughness follow the count", () => {
