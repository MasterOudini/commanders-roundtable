// `Goblin Heelcutter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_HEELCUTTER_SCRIPT } from './goblinHeelcutter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Heelcutter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Heelcutter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_HEELCUTTER_SCRIPT]),
...
describe("Goblin Heelcutter", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
    const { g, no } = armed(0);
