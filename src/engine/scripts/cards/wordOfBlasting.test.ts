// `Word of Blasting` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { WORD_OF_BLASTING_SCRIPT } from './wordOfBlasting';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Word of Blasting";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; hosts: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Word of Blasting"], ["Wall of Omens", "Grizzly Bears"]],
    scripts: createRegistry([WORD_OF_BLASTING_SCRIPT]),
  });
  holdEverywhere(g);
  const targets: InstanceId[] = [];
  const hosts: InstanceId[] = [];
  targets.push(put(g, 'p2', "Wall of Omens"));
