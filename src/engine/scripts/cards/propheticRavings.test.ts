// `Prophetic Ravings` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { PROPHETIC_RAVINGS_SCRIPT } from './propheticRavings';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prophetic Ravings";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Prophetic Ravings","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PROPHETIC_RAVINGS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Prophetic Ravings", () => {
  test("grants {T}, Discard a card: Draw a card. to Grizzly Bears, and to nothing outside the scope", () => {
