// `Pawn of Ulamog` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PAWN_OF_ULAMOG_SCRIPT } from './pawnOfUlamog';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pawn of Ulamog";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pawn of Ulamog", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PAWN_OF_ULAMOG_SCRIPT]),
...
describe("Pawn of Ulamog", () => {
  test("Whenever this creature or another nontoken creature you control dies: the vocabulary resolves \"Create a 0/1 colorless Eldrazi Spawn creature token. It has \\\"Sacrifice this token: Add {C}.\\\"\" [dies]", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 - 1);
  });

  test("Whenever this creature or another nontoken creature you control dies: the vocabulary resolves \"Create a 0/1 colorless Eldrazi Spawn creature token. It has \\\"Sacrifice this token: Add {C}.\\\"\" [anotherCreatureDies]", () => {
    const { g, board0 } = armed(1);
    expect(onBoard(g)).toBe(board0 + 1 + 1 - 1);
  });
