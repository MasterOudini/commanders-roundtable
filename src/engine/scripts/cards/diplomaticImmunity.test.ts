// `Diplomatic Immunity` - cast on Grizzly Bears it attaches and the enchanted creature has shroud;
...
import { DIPLOMATIC_IMMUNITY_SCRIPT } from './diplomaticImmunity';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Diplomatic Immunity";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([DIPLOMATIC_IMMUNITY_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([DIPLOMATIC_IMMUNITY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Diplomatic Immunity", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has shroud; Cyclops of One-Eyed Pass is untouched", () => {
