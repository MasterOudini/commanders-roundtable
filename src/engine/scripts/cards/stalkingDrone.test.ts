// `Stalking Drone` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STALKING_DRONE_SCRIPT } from './stalkingDrone';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stalking Drone";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STALKING_DRONE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stalking Drone", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STALKING_DRONE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stalking Drone", () => {
  test("{C}: it gets +1/+2 until end of turn", () => {
