// `Tangle Hulk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TANGLE_HULK_SCRIPT } from './tangleHulk';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tangle Hulk";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tangle Hulk", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TANGLE_HULK_SCRIPT]),
  });
  holdEverywhere(g);
