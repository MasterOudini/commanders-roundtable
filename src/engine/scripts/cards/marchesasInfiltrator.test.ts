// `Marchesa's Infiltrator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARCHESAS_INFILTRATOR_SCRIPT } from './marchesasInfiltrator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Marchesa's Infiltrator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Marchesa's Infiltrator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARCHESAS_INFILTRATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Marchesa's Infiltrator", () => {
