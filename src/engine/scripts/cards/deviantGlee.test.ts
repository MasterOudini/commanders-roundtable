// `Deviant Glee` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { DEVIANT_GLEE_SCRIPT } from './deviantGlee';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deviant Glee";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Deviant Glee","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEVIANT_GLEE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deviant Glee", () => {
  test("grants {R}: This creature gains trample until end of turn. to Grizzly Bears, and to nothing outside the scope", () => {
    ...
    expect([d.power, d.toughness]).toEqual([4, 3]);
  });
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and the body hits the RECIPIENT', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{R}");
