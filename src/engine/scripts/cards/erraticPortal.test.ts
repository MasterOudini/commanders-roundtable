// `Erratic Portal` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ERRATIC_PORTAL_SCRIPT } from './erraticPortal';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Erratic Portal";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; payA0_p2_0: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Erratic Portal"], ["Cyclops of One-Eyed Pass", "Forest"]],
    scripts: createRegistry([ERRATIC_PORTAL_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, payA0_p2_0 };
}

describe("Erratic Portal", () => {
  test("{1}, {T}: the vocabulary resolves \"Return target creature to its owner's hand unless its controller pays {1}.\" - the price is declined", () => {
    const { g, self, no, payA0_p2_0 } = armed(0);
  ...
  test("{1}, {T}: the vocabulary resolves \"Return target creature to its owner's hand unless its controller pays {1}.\" - the price is paid", () => {
    const { g, self, no, payA0_p2_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
