// `Deadeye Tormentor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEADEYE_TORMENTOR_SCRIPT } from './deadeyeTormentor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deadeye Tormentor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deadeye Tormentor", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEADEYE_TORMENTOR_SCRIPT]),
  });
  holdEverywhere(g);
