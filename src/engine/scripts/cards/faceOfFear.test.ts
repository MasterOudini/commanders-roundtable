// `Face of Fear` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FACE_OF_FEAR_SCRIPT } from './faceOfFear';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Face of Fear";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FACE_OF_FEAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Face of Fear", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FACE_OF_FEAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Face of Fear", () => {
  test("{2}{B}, Discard a card: it gains fear until end of turn", () => {
