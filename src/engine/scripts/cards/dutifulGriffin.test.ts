// `Dutiful Griffin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DUTIFUL_GRIFFIN_SCRIPT } from './dutifulGriffin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dutiful Griffin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dutiful Griffin", "Hissing Miasma", "Hissing Miasma"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUTIFUL_GRIFFIN_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = [put(g, 'p1', "Hissing Miasma"), put(g, 'p1', "Hissing Miasma")];
  settle(g);
...
describe("Dutiful Griffin", () => {
  test("{2}{W}, Sacrifice two enchantments: it returns from the graveyard to hand", () => {
