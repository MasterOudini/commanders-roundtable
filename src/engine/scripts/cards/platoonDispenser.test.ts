// `Platoon Dispenser` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PLATOON_DISPENSER_SCRIPT } from './platoonDispenser';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Platoon Dispenser";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Platoon Dispenser", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PLATOON_DISPENSER_SCRIPT]),
