// `Pious Interdiction` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: gain 2 life); cannot attack or block;
...
import { PIOUS_INTERDICTION_SCRIPT } from './piousInterdiction';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pious Interdiction";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([PIOUS_INTERDICTION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pious Interdiction", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: gain 2 life); cannot attack or block; Cyclops of One-Eyed Pass is untouched", () => {
