// `Ragnar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAGNAR_SCRIPT } from './ragnar';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ragnar", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAGNAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ragnar", () => {
  test("{G}{W}{U}, {T}: the declared creature regenerates from the Bolt", () => {
