// `Burning Anger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BURNING_ANGER_SCRIPT } from './burningAnger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { legalActions } from '../../legal';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Burning Anger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Burning Anger", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BURNING_ANGER_SCRIPT]),
...
describe("Burning Anger", () => {
  ...
  test("Enchanted creature has \"{T}: the vocabulary resolves \"~ deals damage equal to its power to any target.\"", () => {
