// `Chalk Outline` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHALK_OUTLINE_SCRIPT } from './chalkOutline';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chalk Outline";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chalk Outline", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHALK_OUTLINE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chalk Outline", () => {
  test("Whenever one or more creature cards leave your graveyard: the vocabulary resolves \"Create a 2/2 white and blue Detective creature token, then investigate.\"", () => {
