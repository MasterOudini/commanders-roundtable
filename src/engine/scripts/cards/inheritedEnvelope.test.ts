// `Inherited Envelope` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INHERITED_ENVELOPE_SCRIPT } from './inheritedEnvelope';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Inherited Envelope";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Inherited Envelope"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INHERITED_ENVELOPE_SCRIPT]),
...
describe("Inherited Envelope", () => {
  test("When this artifact enters: the vocabulary resolves \"The Ring tempts you.\"", () => {
