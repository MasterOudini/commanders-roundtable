// `Fleeting Aven` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLEETING_AVEN_SCRIPT } from './fleetingAven';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fleeting Aven";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([FLEETING_AVEN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fleeting Aven"], ["Cyclops of One-Eyed Pass", "Lonely Sandbar"]],
    scripts: createRegistry([FLEETING_AVEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fleeting Aven", () => {
  test("Whenever a player cycles a card: the vocabulary resolves \"Return this creature to its owner's hand.\"", () => {
    const {  } = armed(0);
  });
