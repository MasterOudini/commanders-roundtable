// `Lion Vulture` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LION_VULTURE_SCRIPT } from './lionVulture';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lion Vulture";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lion Vulture"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LION_VULTURE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lion Vulture", () => {
  test("At the beginning of your end step: the vocabulary resolves \"Put a +1/+1 counter on this creature and draw a card.\"", () => {
