// `Runed Servitor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUNED_SERVITOR_SCRIPT } from './runedServitor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Runed Servitor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Runed Servitor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUNED_SERVITOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Runed Servitor", () => {
  test("When this creature dies: the vocabulary resolves \"Each player draws a card.\"", () => {
