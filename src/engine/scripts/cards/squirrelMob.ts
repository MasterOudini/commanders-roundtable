// `Squirrel Mob` - a VARIABLE PUMP (D387): the delta is COUNTED at each derive
...
import { SQUIRREL_MOB } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(SQUIRREL_MOB, "This creature gets +1/+1 for each other Squirrel on the battlefield.");
...
function countOf(ctx: ScriptCtx, self: InstanceId, applied: InstanceId): number {
  ...
  for (const inst of Object.values(ctx.state.cards)) {
    ...
    if (!face) continue;
    if (!face.typeLine.subtypes.includes("Squirrel")) continue;
    n++;
  }
  return n;
}

export const SQUIRREL_MOB_SCRIPT: CardScript = {
  oracleId: SQUIRREL_MOB.oracleId,
  name: SQUIRREL_MOB.name,
  statics: [
    {
      abilityId: 'pump-0',
