// `Rejuvenation Chamber` - every printed ability proven in its own game: the cost's mark, the pump
...
import { REJUVENATION_CHAMBER_SCRIPT } from './rejuvenationChamber';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rejuvenation Chamber";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rejuvenation Chamber"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([REJUVENATION_CHAMBER_SCRIPT]),
...
describe("Rejuvenation Chamber", () => {
  test("{T}: 2 life is gained", () => {
