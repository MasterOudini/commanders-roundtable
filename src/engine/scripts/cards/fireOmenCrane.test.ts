// `Fire-Omen Crane` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIRE_OMEN_CRANE_SCRIPT } from './fireOmenCrane';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fire-Omen Crane";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fire-Omen Crane"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIRE_OMEN_CRANE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fire-Omen Crane", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"~ deals 1 damage to target creature an opponent controls.\"", () => {
    const { g, no } = armed(0);
