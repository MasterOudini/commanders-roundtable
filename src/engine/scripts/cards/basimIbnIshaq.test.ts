// `Basim Ibn Ishaq` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BASIM_IBN_ISHAQ_SCRIPT } from './basimIbnIshaq';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Basim Ibn Ishaq";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Basim Ibn Ishaq", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BASIM_IBN_ISHAQ_SCRIPT]),
...
describe("Basim Ibn Ishaq", () => {
  test("Whenever you cast a historic spell: the vocabulary resolves \"Draw a card. ~ can't be blocked this turn.\"", () => {
    const { g, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1 - 1);
  });

  test("Whenever Basim Ibn Ishaq deals combat damage to a player: it gets 1 +1/+1 counter", () => {
    const { g, self } = armed(1);
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(1);
  });

  test('the once-per-turn rider rides the def (D492)', () => {
    expect(BASIM_IBN_ISHAQ_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("castSpell-0") && t.oncePerTurn === true), 'the def of line 0 carries oncePerTurn').toBeDefined();
  });
