// `Oran-Rief Survivalist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORAN_RIEF_SURVIVALIST_SCRIPT } from './oranRiefSurvivalist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Oran-Rief Survivalist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Oran-Rief Survivalist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORAN_RIEF_SURVIVALIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Oran-Rief Survivalist", () => {
  test("Whenever this creature or another Ally you control enters: it gets 1 +1/+1 counter", () => {
