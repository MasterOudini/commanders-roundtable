// `Silkweaver Elite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SILKWEAVER_ELITE_SCRIPT } from './silkweaverElite';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Silkweaver Elite";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Silkweaver Elite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SILKWEAVER_ELITE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Silkweaver Elite", () => {
  test("Revolt — When this creature enters: a card is drawn", () => {
