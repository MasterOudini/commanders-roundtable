// `Proud Mentor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PROUD_MENTOR_SCRIPT } from './proudMentor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Proud Mentor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Proud Mentor", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PROUD_MENTOR_SCRIPT]),
