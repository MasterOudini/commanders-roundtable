// `Waterknot` - a etb trigger tapAttached, a static attachedNoUntap
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { WATERKNOT } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(WATERKNOT, "Enchant creature\nWhen this Aura enters, tap enchanted creature.\nEnchanted creature doesn't untap during its controller's untap step.");
const LINES = PRINTED.split('\n');

export const WATERKNOT_SCRIPT: CardScript = {
  oracleId: WATERKNOT.oracleId,
  name: WATERKNOT.name,
  triggers: [
    {
      abilityId: 'etb-1',
