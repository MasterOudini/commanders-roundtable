// `Dwarven Demolition Team` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { DWARVEN_DEMOLITION_TEAM_SCRIPT } from './dwarvenDemolitionTeam';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dwarven Demolition Team";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Dwarven Demolition Team"], ["Wall of Omens", "Grizzly Bears"]],
    scripts: createRegistry([DWARVEN_DEMOLITION_TEAM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dwarven Demolition Team", () => {
