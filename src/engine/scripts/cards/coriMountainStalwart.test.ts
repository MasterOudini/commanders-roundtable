// `Cori Mountain Stalwart` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CORI_MOUNTAIN_STALWART_SCRIPT } from './coriMountainStalwart';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cori Mountain Stalwart";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cori Mountain Stalwart", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CORI_MOUNTAIN_STALWART_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cori Mountain Stalwart", () => {
  test("Flurry — Whenever you cast your second spell each turn: the vocabulary resolves \"~ deals 2 damage to each opponent and you gain 2 life.\"", () => {
