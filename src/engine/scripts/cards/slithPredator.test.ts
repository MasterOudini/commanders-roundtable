// `Slith Predator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLITH_PREDATOR_SCRIPT } from './slithPredator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slith Predator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slith Predator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLITH_PREDATOR_SCRIPT]),
  });
  holdEverywhere(g);
