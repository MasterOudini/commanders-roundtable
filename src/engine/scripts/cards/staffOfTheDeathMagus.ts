// `Staff of the Death Magus` — "Whenever you cast a black spell or a Swamp
// you control enters, you gain 1 life." One printed line, THREE defs: the
// color cast, the card entry, the token entry (a token Swamp is printed
// nowhere as exempt). The first of the five-Staff family. D252.

import { STAFF_OF_THE_DEATH_MAGUS } from '../../../data/fixtures/engineCards';
...
function landQualifies(ctx: ScriptCtx, self: InstanceId, entrant: InstanceId): boolean {
  ...
  return ctx.derive(entrant).typeLine.subtypes.includes('Swamp');
}
...
export const STAFF_OF_THE_DEATH_MAGUS_SCRIPT: CardScript = {
  oracleId: STAFF_OF_THE_DEATH_MAGUS.oracleId,
  name: STAFF_OF_THE_DEATH_MAGUS.name,
  triggers: [
    {
      abilityId: 'cast',
      text: TEXT,
      ...
      label: () => 'Staff of the Death Magus — you gain 1 life',
