// `Hungry Mist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HUNGRY_MIST_SCRIPT } from './hungryMist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hungry Mist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hungry Mist", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HUNGRY_MIST_SCRIPT]),
  });
  holdEverywhere(g);
