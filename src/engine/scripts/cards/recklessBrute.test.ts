// `Reckless Brute` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RECKLESS_BRUTE_SCRIPT } from './recklessBrute';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Reckless Brute";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Reckless Brute", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RECKLESS_BRUTE_SCRIPT]),
  });
  holdEverywhere(g);
