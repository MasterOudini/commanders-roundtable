// `Selkie Hedge-Mage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SELKIE_HEDGE_MAGE_SCRIPT } from './selkieHedgeMage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Selkie Hedge-Mage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Selkie Hedge-Mage", "Forest", "Forest", "Island", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SELKIE_HEDGE_MAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Selkie Hedge-Mage", () => {
  test("When this creature enters: 3 life is gained", () => {
    const { g, life0 } = armed(0);
    expect(g.state.players.p1?.life).toBe(life0 + 3);
  });

  test("When this creature enters: the vocabulary resolves \"Return target tapped creature to its owner's hand.\"", () => {
    const { g, no } = armed(1);
