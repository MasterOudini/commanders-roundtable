// `Evolving Wilds` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Evolving Wilds", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EVOLVING_WILDS_SCRIPT]),
  });
  holdEverywhere(g);
