// `Deep-Slumber Titan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEEP_SLUMBER_TITAN_SCRIPT } from './deepSlumberTitan';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deep-Slumber Titan";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; gob: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deep-Slumber Titan", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([DEEP_SLUMBER_TITAN_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, gob };
}

describe("Deep-Slumber Titan", () => {
  ...
  test("Whenever this creature is dealt damage: it untaps", () => {
