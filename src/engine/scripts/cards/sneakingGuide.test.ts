// `Sneaking Guide` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SNEAKING_GUIDE_SCRIPT } from './sneakingGuide';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sneaking Guide";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sneaking Guide"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([SNEAKING_GUIDE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sneaking Guide", () => {
  test("{2}, {T}: the vocabulary resolves \"Target creature with power 2 or less can't be blocked this turn.\"", () => {
