// `Rushwood Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUSHWOOD_ELEMENTAL_SCRIPT } from './rushwoodElemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rushwood Elemental";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rushwood Elemental"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUSHWOOD_ELEMENTAL_SCRIPT]),
  });
  holdEverywhere(g);
