// `Expedition Map` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Expedition Map", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EXPEDITION_MAP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Expedition Map", () => {
  test("{2}, {T}, Sacrifice this artifact: the vocabulary resolves \"Search your library for a land card, reveal it, put it into your hand, then shuffle.\"", () => {
