// `Rusted Slasher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUSTED_SLASHER_SCRIPT } from './rustedSlasher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rusted Slasher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rusted Slasher", "Sol Ring", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUSTED_SLASHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rusted Slasher", () => {
  test("Sacrifice an artifact: it regenerates from the Bolt", () => {
