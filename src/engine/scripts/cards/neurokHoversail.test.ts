// `Neurok Hoversail` - equipped to Grizzly Bears the creature has flying; the Cyclops is untouched;
...
import { NEUROK_HOVERSAIL_SCRIPT } from './neurokHoversail';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Neurok Hoversail";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([NEUROK_HOVERSAIL_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([NEUROK_HOVERSAIL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Neurok Hoversail", () => {
  test("equipped to Grizzly Bears: attached, and the creature has flying; the Cyclops is untouched", () => {
