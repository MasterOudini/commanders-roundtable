// `Nettle Sentinel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NETTLE_SENTINEL_SCRIPT } from './nettleSentinel';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nettle Sentinel";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nettle Sentinel", "Grizzly Bears", "Norwood Ranger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NETTLE_SENTINEL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Nettle Sentinel", () => {
  ...
  test("Whenever you cast a green spell: it untaps", () => {
