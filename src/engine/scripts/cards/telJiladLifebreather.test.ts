// `Tel-Jilad Lifebreather` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEL_JILAD_LIFEBREATHER_SCRIPT } from './telJiladLifebreather';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tel-Jilad Lifebreather";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tel-Jilad Lifebreather", "Forest", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEL_JILAD_LIFEBREATHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Tel-Jilad Lifebreather", () => {
  test("{G}, {T}, Sacrifice a Forest: the declared creature regenerates from the Bolt", () => {
    const { g, self, no, fodder0 } = armed(0);
