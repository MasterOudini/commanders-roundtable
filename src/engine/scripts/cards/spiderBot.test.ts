// `Spider-Bot` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spider-Bot", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPIDER_BOT_SCRIPT]),
  });
  holdEverywhere(g);
