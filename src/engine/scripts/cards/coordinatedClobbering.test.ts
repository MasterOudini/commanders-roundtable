// `Coordinated Clobbering` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { COORDINATED_CLOBBERING_SCRIPT } from './coordinatedClobbering';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Coordinated Clobbering";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Coordinated Clobbering", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears"], ["Vampire Nighthawk"]],
    scripts: createRegistry([COORDINATED_CLOBBERING_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  settle(g);
  must(g.submit({ t: 'ManualSetTapped', player: 'p1', cards: [wrong], tapped: true }));
...
describe("Coordinated Clobbering", () => {
  test('the named targets are accepted and the effect lands', () => {
    ...
    settle(g);
    expect(g.state.cards[targets[0] as InstanceId]?.tapped).toBe(true);
    expect(g.state.cards[targets[1] as InstanceId]?.tapped).toBe(true);
    expect(g.state.cards[targets[2] as InstanceId]?.zone.kind).toBe('graveyard');
  });

  test("Grizzly Bears (tapped) is refused where the clause excludes it (D294)", () => {
