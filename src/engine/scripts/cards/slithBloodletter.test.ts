// `Slith Bloodletter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLITH_BLOODLETTER_SCRIPT } from './slithBloodletter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slith Bloodletter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slith Bloodletter", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLITH_BLOODLETTER_SCRIPT]),
  });
  holdEverywhere(g);
