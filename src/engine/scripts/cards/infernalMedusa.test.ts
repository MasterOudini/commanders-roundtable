// `Infernal Medusa` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INFERNAL_MEDUSA_SCRIPT } from './infernalMedusa';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Infernal Medusa";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Infernal Medusa"], ["Cyclops of One-Eyed Pass", "Axebane Stag"]],
    scripts: createRegistry([INFERNAL_MEDUSA_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const gift2 = put(g, 'p2', "Axebane Stag");
  settle(g);
  ...
  if (which === 1) {
    ...
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'declareBlockers', 20_000);
    must(g.submit({ t: 'DeclareBlockers', player: 'p2', blocks: [{ blocker: gift2, attacker: self }] }));
    settle(g);
    }
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, gift2 };
}

describe("Infernal Medusa", () => {
  test("Whenever this creature blocks a creature: the vocabulary resolves \"Destroy target creature at end of combat.\"", () => {
  ...
  test("Whenever this creature becomes blocked by a non-Wall creature: the vocabulary resolves \"Destroy target creature at end of combat.\"", () => {
    const { g, gift2 } = armed(1);
    expect(g.state.delayedTriggers.length, 'the payload is armed, not run').toBe(1);
    {
