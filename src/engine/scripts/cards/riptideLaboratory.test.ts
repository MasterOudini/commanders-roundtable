// `Riptide Laboratory` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIPTIDE_LABORATORY_SCRIPT } from './riptideLaboratory';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Riptide Laboratory";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Riptide Laboratory", "Fugitive Wizard"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIPTIDE_LABORATORY_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtA1_0 = put(g, 'p1', "Fugitive Wizard");
  settle(g);
...
describe("Riptide Laboratory", () => {
  test("{1}{U}, {T}: the vocabulary resolves \"Return target Wizard you control to its owner's hand.\"", () => {
    const { g, self, hand0, board0, vtA1_0 } = armed(0);
