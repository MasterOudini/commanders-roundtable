// `Skewer Slinger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKEWER_SLINGER_SCRIPT } from './skewerSlinger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skewer Slinger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skewer Slinger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKEWER_SLINGER_SCRIPT]),
...
describe("Skewer Slinger", () => {
  test("Whenever this creature blocks or becomes blocked by a creature: the vocabulary resolves \"~ deals 1 damage to target creature.\"", () => {
    const { g, no } = armed(0);
