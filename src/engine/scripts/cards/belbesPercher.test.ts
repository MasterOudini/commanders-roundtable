// `Belbe's Percher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BELBES_PERCHER_SCRIPT } from './belbesPercher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Belbe's Percher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Belbe's Percher", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BELBES_PERCHER_SCRIPT]),
  });
  holdEverywhere(g);
