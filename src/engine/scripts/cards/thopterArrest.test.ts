// `Thopter Arrest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THOPTER_ARREST_SCRIPT } from './thopterArrest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thopter Arrest";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thopter Arrest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THOPTER_ARREST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thopter Arrest", () => {
  test("When this enchantment enters: the vocabulary resolves \"Exile target artifact or creature an opponent controls until this enchantment leaves the battlefield.\"", () => {
