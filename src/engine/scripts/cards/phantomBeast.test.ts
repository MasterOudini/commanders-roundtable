// `Phantom Beast` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHANTOM_BEAST_SCRIPT } from './phantomBeast';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phantom Beast";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phantom Beast", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHANTOM_BEAST_SCRIPT]),
  });
  holdEverywhere(g);
