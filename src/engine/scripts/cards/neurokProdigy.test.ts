// `Neurok Prodigy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NEUROK_PRODIGY_SCRIPT } from './neurokProdigy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Neurok Prodigy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Neurok Prodigy", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NEUROK_PRODIGY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Neurok Prodigy", () => {
  test("Discard an artifact card: it returns to its owner's hand", () => {
