// `Brackwater Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRACKWATER_ELEMENTAL_SCRIPT } from './brackwaterElemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brackwater Elemental";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brackwater Elemental"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([BRACKWATER_ELEMENTAL_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const gift2 = put(g, 'p2', "Grizzly Bears");
  settle(g);
...
describe("Brackwater Elemental", () => {
  test("When this creature attacks or blocks: the vocabulary resolves \"Sacrifice it at the beginning of the next end step.\" [attacks]", () => {
  ...
  test("When this creature attacks or blocks: the vocabulary resolves \"Sacrifice it at the beginning of the next end step.\" [blocks]", () => {
