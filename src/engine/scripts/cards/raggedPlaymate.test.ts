// `Ragged Playmate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAGGED_PLAYMATE_SCRIPT } from './raggedPlaymate';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ragged Playmate";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ragged Playmate"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([RAGGED_PLAYMATE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ragged Playmate", () => {
  test("{1}, {T}: the vocabulary resolves \"Target creature with power 2 or less can't be blocked this turn.\"", () => {
