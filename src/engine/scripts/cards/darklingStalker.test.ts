// `Darkling Stalker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DARKLING_STALKER_SCRIPT } from './darklingStalker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Darkling Stalker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DARKLING_STALKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Darkling Stalker", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DARKLING_STALKER_SCRIPT]),
  });
  holdEverywhere(g);
