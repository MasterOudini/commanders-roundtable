// `Riptide Survivor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIPTIDE_SURVIVOR_SCRIPT } from './riptideSurvivor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Riptide Survivor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Riptide Survivor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIPTIDE_SURVIVOR_SCRIPT]),
...
describe("Riptide Survivor", () => {
  test("When this creature is turned face up: the vocabulary resolves \"Discard two cards, then draw three cards.\"", () => {
    ...
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 3 - qdL1);
  });
