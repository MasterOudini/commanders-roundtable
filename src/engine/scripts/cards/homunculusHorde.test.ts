// `Homunculus Horde` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOMUNCULUS_HORDE_SCRIPT } from './homunculusHorde';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Homunculus Horde";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Homunculus Horde", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOMUNCULUS_HORDE_SCRIPT]),
...
describe("Homunculus Horde", () => {
  test("Whenever you draw your second card each turn: the vocabulary resolves \"Create a token that's a copy of this creature.\"", () => {
