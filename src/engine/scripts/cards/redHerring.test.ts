// `Red Herring` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RED_HERRING_SCRIPT } from './redHerring';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Red Herring";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Red Herring", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RED_HERRING_SCRIPT]),
  });
  holdEverywhere(g);
