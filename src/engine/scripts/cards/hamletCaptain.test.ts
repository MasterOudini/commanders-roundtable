// `Hamlet Captain` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HAMLET_CAPTAIN_SCRIPT } from './hamletCaptain';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hamlet Captain";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HAMLET_CAPTAIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hamlet Captain", "Jasmine Boreal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HAMLET_CAPTAIN_SCRIPT]),
...
describe("Hamlet Captain", () => {
  test("Whenever this creature attacks or blocks: other Human creatures you control get +1/+1 until end of turn [attacks]", () => {
    const { g, no, yes } = armed(0);
    expect(pt(g, yes)).toEqual([5, 6]);
    expect(pt(g, no)).toEqual([5, 2]);
  });

  test("Whenever this creature attacks or blocks: other Human creatures you control get +1/+1 until end of turn [blocks]", () => {
