// `Nightblade Brigade` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIGHTBLADE_BRIGADE_SCRIPT } from './nightbladeBrigade';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nightblade Brigade";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nightblade Brigade"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIGHTBLADE_BRIGADE_SCRIPT]),
