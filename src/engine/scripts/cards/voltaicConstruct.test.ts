// `Voltaic Construct` - the named target is accepted and it untaps; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { VOLTAIC_CONSTRUCT_SCRIPT } from './voltaicConstruct';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Voltaic Construct";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Voltaic Construct"], ["Grapeshot Catapult", "Grizzly Bears"]],
    scripts: createRegistry([VOLTAIC_CONSTRUCT_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const target = put(g, 'p2', "Grapeshot Catapult");
...
describe("Voltaic Construct", () => {
  test("Grapeshot Catapult (tapped) is a legal target and it untaps", () => {
