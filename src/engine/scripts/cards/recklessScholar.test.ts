// `Reckless Scholar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RECKLESS_SCHOLAR_SCRIPT } from './recklessScholar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Reckless Scholar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Reckless Scholar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RECKLESS_SCHOLAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Reckless Scholar", () => {
  test("{T}: the vocabulary resolves \"Target player draws a card, then discards a card.\"", () => {
