// `Stormwatch Eagle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORMWATCH_EAGLE_SCRIPT } from './stormwatchEagle';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stormwatch Eagle";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stormwatch Eagle", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORMWATCH_EAGLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stormwatch Eagle", () => {
  test("Sacrifice a land: it returns to its owner's hand", () => {
