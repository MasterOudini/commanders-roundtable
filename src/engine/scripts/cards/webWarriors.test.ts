// `Web-Warriors` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WEB_WARRIORS_SCRIPT } from './webWarriors';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Web-Warriors";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Web-Warriors", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WEB_WARRIORS_SCRIPT]),
  });
  holdEverywhere(g);
