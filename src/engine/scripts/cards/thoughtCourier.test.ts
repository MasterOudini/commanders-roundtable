// `Thought Courier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THOUGHT_COURIER_SCRIPT } from './thoughtCourier';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thought Courier";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thought Courier"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THOUGHT_COURIER_SCRIPT]),
  });
  holdEverywhere(g);
