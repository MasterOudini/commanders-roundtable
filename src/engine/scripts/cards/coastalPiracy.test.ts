// `Coastal Piracy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COASTAL_PIRACY_SCRIPT } from './coastalPiracy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Coastal Piracy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Coastal Piracy", "Memnite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COASTAL_PIRACY_SCRIPT]),
...
describe("Coastal Piracy", () => {
  test("Whenever a creature you control deals combat damage to an opponent: a card is drawn", () => {
