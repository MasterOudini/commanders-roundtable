// `Skyshroud Poacher` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skyshroud Poacher", "Devoted Hero"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKYSHROUD_POACHER_SCRIPT]),
...
describe("Skyshroud Poacher", () => {
  test("{3}, {T}: the vocabulary resolves \"Search your library for an Elf permanent card, put it onto the battlefield, then shuffle.\"", () => {
    const { g, self, board0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Devoted Hero" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
