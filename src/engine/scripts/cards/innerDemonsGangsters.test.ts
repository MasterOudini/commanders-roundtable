// `Inner Demons Gangsters` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INNER_DEMONS_GANGSTERS_SCRIPT } from './innerDemonsGangsters';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Inner Demons Gangsters";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([INNER_DEMONS_GANGSTERS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Inner Demons Gangsters", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INNER_DEMONS_GANGSTERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Inner Demons Gangsters", () => {
  test("Discard a card: it gets +1/+0 and gains menace until end of turn", () => {
