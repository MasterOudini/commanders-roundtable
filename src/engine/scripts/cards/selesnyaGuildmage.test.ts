// `Selesnya Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SELESNYA_GUILDMAGE_SCRIPT } from './selesnyaGuildmage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Selesnya Guildmage";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SELESNYA_GUILDMAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Selesnya Guildmage", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SELESNYA_GUILDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Selesnya Guildmage", () => {
  test("{3}{G}: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("{3}{W}: its controller's creatures get +1/+1 until end of turn", () => {
