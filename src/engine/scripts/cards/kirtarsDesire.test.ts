// `Kirtar's Desire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KIRTARS_DESIRE_SCRIPT } from './kirtarsDesire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kirtar's Desire";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kirtar's Desire", "Grizzly Bears", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KIRTARS_DESIRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kirtar's Desire", () => {
  ...
  test("Threshold — Enchanted creature can't block as long as there are seven or more cards in your graveyard.: the creature it is attached to is held back", () => {
