// `Fledgling Griffin` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FLEDGLING_GRIFFIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fledgling Griffin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLEDGLING_GRIFFIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fledgling Griffin", () => {
  test("Landfall — Whenever a land you control enters: it gains flying until end of turn", () => {
