// `Wei Night Raiders` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WEI_NIGHT_RAIDERS_SCRIPT } from './weiNightRaiders';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wei Night Raiders";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wei Night Raiders"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WEI_NIGHT_RAIDERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wei Night Raiders", () => {
  test("Whenever this creature deals damage to an opponent: the vocabulary resolves \"Target player discards a card.\"", () => {
    const { g } = armed(0);
