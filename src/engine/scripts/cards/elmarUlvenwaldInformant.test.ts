// `Elmar, Ulvenwald Informant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ELMAR_ULVENWALD_INFORMANT_SCRIPT } from './elmarUlvenwaldInformant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Elmar, Ulvenwald Informant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elmar, Ulvenwald Informant", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELMAR_ULVENWALD_INFORMANT_SCRIPT]),
...
describe("Elmar, Ulvenwald Informant", () => {
  test("Whenever you cast your second spell each turn: the vocabulary resolves \"Untap target creature, then investigate.\"", () => {
