// `Ravos, Soultender` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAVOS_SOULTENDER_SCRIPT } from './ravosSoultender';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ravos, Soultender";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RAVOS_SOULTENDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ravos, Soultender", "Coral Eel", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAVOS_SOULTENDER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Coral Eel");
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtL2_upkeep_0 = put(g, 'p1', "Grizzly Bears", 'graveyard');
  settle(g);
...
describe("Ravos, Soultender", () => {
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Return target creature card from your graveyard to your hand.\"", () => {
    const { g, hand0, vtL2_upkeep_0 } = armed(1);
    expect(g.state.cards[vtL2_upkeep_0]?.zone.kind).toBe('hand');
