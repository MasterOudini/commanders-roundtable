// `Claustrophobia` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLAUSTROPHOBIA_SCRIPT } from './claustrophobia';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Claustrophobia";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Claustrophobia", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLAUSTROPHOBIA_SCRIPT]),
  });
  holdEverywhere(g);
