// `+2 Mace` - equipped to Grizzly Bears the creature gets +2/+2; the Cyclops is untouched;
...
import { PLUS2_MACE_SCRIPT } from './plus2Mace';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PLUS2_MACE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([PLUS2_MACE_SCRIPT]),
  });
  holdEverywhere(g);
