// `Omen of the Sun` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OMEN_OF_THE_SUN_SCRIPT } from './omenOfTheSun';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Omen of the Sun";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Omen of the Sun"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OMEN_OF_THE_SUN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Omen of the Sun", () => {
  test("When this enchantment enters: the vocabulary resolves \"Create two 1/1 white Human Soldier creature tokens and you gain 2 life.\"", () => {
    const { g, life0, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 2 + 1);
    expect(g.state.players.p1?.life).toBe(life0 + 2);
  });

  test("{2}{W}, Sacrifice this enchantment: scry 2 asks, and the cards stay on top", () => {
