// `Mai, Jaded Edge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAI_JADED_EDGE_SCRIPT } from './maiJadedEdge';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mai, Jaded Edge";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mai, Jaded Edge", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAI_JADED_EDGE_SCRIPT]),
...
describe("Mai, Jaded Edge", () => {
  test("Exhaust — {3}: the vocabulary resolves \"Put a double strike counter on ~.\"", () => {
