// `Cloud Pirates` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLOUD_PIRATES_SCRIPT } from './cloudPirates';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cloud Pirates";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cloud Pirates", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLOUD_PIRATES_SCRIPT]),
  });
  holdEverywhere(g);
