// `Phantasmal Shieldback` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHANTASMAL_SHIELDBACK_SCRIPT } from './phantasmalShieldback';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phantasmal Shieldback";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phantasmal Shieldback", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHANTASMAL_SHIELDBACK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Phantasmal Shieldback", () => {
  test("When this creature becomes the target of a spell or ability: it is sacrificed", () => {
