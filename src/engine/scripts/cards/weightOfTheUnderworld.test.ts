// `Weight of the Underworld` - cast on Grizzly Bears it attaches and the enchanted creature gets -3/-2;
...
import { WEIGHT_OF_THE_UNDERWORLD_SCRIPT } from './weightOfTheUnderworld';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Weight of the Underworld";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WEIGHT_OF_THE_UNDERWORLD_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([WEIGHT_OF_THE_UNDERWORLD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Weight of the Underworld", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets -3/-2; Cyclops of One-Eyed Pass is untouched", () => {
