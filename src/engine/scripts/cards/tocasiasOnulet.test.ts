// `Tocasia's Onulet` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TOCASIAS_ONULET_SCRIPT } from './tocasiasOnulet';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tocasia's Onulet";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tocasia's Onulet"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TOCASIAS_ONULET_SCRIPT]),
...
describe("Tocasia's Onulet", () => {
  test("When this creature leaves the battlefield: 2 life is gained", () => {
