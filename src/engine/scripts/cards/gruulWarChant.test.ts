// `Gruul War Chant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRUUL_WAR_CHANT_SCRIPT } from './gruulWarChant';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gruul War Chant";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GRUUL_WAR_CHANT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gruul War Chant", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRUUL_WAR_CHANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gruul War Chant", () => {
  test("Attacking creatures you control get +1/+0 and have menace.: only the attacking creature reads it", () => {
