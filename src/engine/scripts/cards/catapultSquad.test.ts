// `Catapult Squad` — damage on my ATTACKING creature: the right target is
// accepted, a creature that stayed home is refused (D291). Generated from one table row (D292).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { CATAPULT_SQUAD_SCRIPT } from './catapultSquad';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Catapult Squad";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; soldier: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [[CARD, BEARS, BEARS, 'Vampire Nighthawk', 'Forest', 'Thraben Standard Bearer'], [BEARS, BEARS]],
    scripts: createRegistry([CATAPULT_SQUAD_SCRIPT]),
  });
  const self = put(g, 'p1', CARD);
  const att = put(g, 'p1', BEARS);
  const home = put(g, 'p1', BEARS);
  const soldier = put(g, 'p1', 'Thraben Standard Bearer');
  settle(g);
  ...
  must(g.submit({ t: 'ActivateAbility', player: 'p1', card: self, abilityIndex: 0, tap: [self, soldier] }));
  return { g, self, target: att, wrong: home, soldier };
}

describe("Catapult Squad", () => {
  test('the attacking creature is a legal target and the effect lands', () => {
    const { g, target, soldier } = armed();
    must(g.submit({ t: 'ChooseTargets', player: 'p1', targets: [{ kind: 'card', id: target }] }));
    settle(g);
    expect(g.state.cards[target]?.zone.kind).toBe('graveyard');
    expect(g.state.cards[soldier]?.tapped).toBe(true);
  });
