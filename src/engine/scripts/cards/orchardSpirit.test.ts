// `Orchard Spirit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORCHARD_SPIRIT_SCRIPT } from './orchardSpirit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Orchard Spirit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Orchard Spirit", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORCHARD_SPIRIT_SCRIPT]),
