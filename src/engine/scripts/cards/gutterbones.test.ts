// `Gutterbones` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GUTTERBONES_SCRIPT } from './gutterbones';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gutterbones";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gutterbones"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GUTTERBONES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gutterbones", () => {
  test("{1}{B}: it returns from the graveyard to hand - refused unless during your turn and only if an opponent lost life this turn", () => {
