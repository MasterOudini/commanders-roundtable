// `Hagra Crocodile` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HAGRA_CROCODILE_SCRIPT } from './hagraCrocodile';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hagra Crocodile";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HAGRA_CROCODILE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hagra Crocodile", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HAGRA_CROCODILE_SCRIPT]),
  });
  holdEverywhere(g);
