// `Spawnwrithe` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPAWNWRITHE_SCRIPT } from './spawnwrithe';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spawnwrithe";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spawnwrithe"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPAWNWRITHE_SCRIPT]),
