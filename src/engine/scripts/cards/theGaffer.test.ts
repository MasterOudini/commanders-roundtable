// `The Gaffer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THE_GAFFER_SCRIPT } from './theGaffer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "The Gaffer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["The Gaffer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THE_GAFFER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("The Gaffer", () => {
  test("At the beginning of each end step: a card is drawn", () => {
