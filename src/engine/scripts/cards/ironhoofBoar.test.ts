// `Ironhoof Boar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IRONHOOF_BOAR_SCRIPT } from './ironhoofBoar';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ironhoof Boar";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([IRONHOOF_BOAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ironhoof Boar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IRONHOOF_BOAR_SCRIPT]),
...
describe("Ironhoof Boar", () => {
  test("Channel — {1}{R}, Discard this card: the declared creature is pumped until end of turn", () => {
    const { g, self, no } = armed(0);
    expect(pt(g, no)).toEqual([8, 3]);
