// `Niblis of the Urn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIBLIS_OF_THE_URN_SCRIPT } from './niblisOfTheUrn';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Niblis of the Urn";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Niblis of the Urn"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIBLIS_OF_THE_URN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Niblis of the Urn", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Tap target creature.\"", () => {
    const { g, no } = armed(0);
