// `Scepter of Fugue` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCEPTER_OF_FUGUE_SCRIPT } from './scepterOfFugue';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scepter of Fugue";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scepter of Fugue"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCEPTER_OF_FUGUE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Scepter of Fugue", () => {
  test("{1}{B}, {T}: the vocabulary resolves \"Target player discards a card.\" - refused unless during your turn", () => {
