// `Stronghold Rats` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STRONGHOLD_RATS_SCRIPT } from './strongholdRats';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stronghold Rats";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stronghold Rats"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STRONGHOLD_RATS_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    ...
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'chooseFromZone' || (s.turn.turnNumber === 3 && s.turn.phase === 'postcombatMain'), 20_000);
    }
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0 };
}

describe("Stronghold Rats", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Each player discards a card.\"", () => {
    ...
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 0 - qdL1);
  });
