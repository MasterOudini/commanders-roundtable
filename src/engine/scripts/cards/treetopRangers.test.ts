// `Treetop Rangers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TREETOP_RANGERS_SCRIPT } from './treetopRangers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Treetop Rangers";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Treetop Rangers", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TREETOP_RANGERS_SCRIPT]),
