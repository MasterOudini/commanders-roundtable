// `Cabal Archon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CABAL_ARCHON_SCRIPT } from './cabalArchon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cabal Archon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cabal Archon", "Acolyte of Xathrid"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CABAL_ARCHON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cabal Archon", () => {
  test("{B}, Sacrifice a Cleric: the vocabulary resolves \"Target player loses 2 life and you gain 2 life.\"", () => {
