// `Skyward Eye Prophets` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skyward Eye Prophets", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKYWARD_EYE_PROPHETS_SCRIPT]),
...
describe("Skyward Eye Prophets", () => {
  test("{T}: the vocabulary resolves \"Reveal the top card of your library. If it's a land card, put it onto the battlefield. Otherwise, put it into your hand.\"", () => {
