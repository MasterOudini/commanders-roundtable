// `Slashing Tiger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLASHING_TIGER_SCRIPT } from './slashingTiger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slashing Tiger";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SLASHING_TIGER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slashing Tiger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLASHING_TIGER_SCRIPT]),
  });
  holdEverywhere(g);
