// `Plague Mare` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PLAGUE_MARE_SCRIPT } from './plagueMare';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Plague Mare";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PLAGUE_MARE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Plague Mare", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Devoted Hero"]],
    scripts: createRegistry([PLAGUE_MARE_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const gift2 = put(g, 'p2', "Devoted Hero");
  settle(g);
...
describe("Plague Mare", () => {
  test("This creature can't be blocked by white creatures.: the block by white creatures is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("When this creature enters: the vocabulary resolves \"Creatures your opponents control get -1/-1 until end of turn.\"", () => {
    const { g, no } = armed(1);
