// `Trespasser il-Vec` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRESPASSER_IL_VEC_SCRIPT } from './trespasserIlVec';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trespasser il-Vec";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TRESPASSER_IL_VEC_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trespasser il-Vec", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRESPASSER_IL_VEC_SCRIPT]),
  });
  holdEverywhere(g);
