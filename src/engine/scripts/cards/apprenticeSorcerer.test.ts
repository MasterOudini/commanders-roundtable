// `Apprentice Sorcerer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { APPRENTICE_SORCERER_SCRIPT } from './apprenticeSorcerer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Apprentice Sorcerer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Apprentice Sorcerer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([APPRENTICE_SORCERER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Apprentice Sorcerer", () => {
  test("{T}: it deals 1 damage to the declared creature - refused unless during your turn, before attackers are declared", () => {
