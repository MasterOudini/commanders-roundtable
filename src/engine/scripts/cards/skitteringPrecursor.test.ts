// `Skittering Precursor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKITTERING_PRECURSOR_SCRIPT } from './skitteringPrecursor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skittering Precursor";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([SKITTERING_PRECURSOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skittering Precursor", "Lotus Petal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKITTERING_PRECURSOR_SCRIPT]),
...
describe("Skittering Precursor", () => {
  test("Whenever you sacrifice a nontoken permanent: the vocabulary resolves \"Create a 0/1 colorless Eldrazi Spawn creature token with \\\"Sacrifice this token: Add {C}.\\\"\"", () => {
