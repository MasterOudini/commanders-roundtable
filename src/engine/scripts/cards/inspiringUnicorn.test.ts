// `Inspiring Unicorn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INSPIRING_UNICORN_SCRIPT } from './inspiringUnicorn';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Inspiring Unicorn";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([INSPIRING_UNICORN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Inspiring Unicorn", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INSPIRING_UNICORN_SCRIPT]),
  });
  holdEverywhere(g);
