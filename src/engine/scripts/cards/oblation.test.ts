// `Oblation` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { OBLATION_SCRIPT } from './oblation';
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Oblation"], ["Grizzly Bears", "Forest"]],
    scripts: createRegistry([OBLATION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Oblation", () => {
  test('the named target is accepted and the effect lands', () => {
    ...
    expect((g.state.zones.hand.p2 ?? []).length - handBefore.p2).toBe(2);
  });
