// `Syndicate Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SYNDICATE_GUILDMAGE_SCRIPT } from './syndicateGuildmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Syndicate Guildmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Syndicate Guildmage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SYNDICATE_GUILDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Syndicate Guildmage", () => {
  test("{1}{W}, {T}: the vocabulary resolves \"Tap target creature with power 4 or greater.\"", () => {
    const { g, self, no } = armed(0);
    expect(g.state.cards[no]?.tapped).toBe(true);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{4}{B}, {T}: the vocabulary resolves \"~ deals 2 damage to target opponent or planeswalker.\"", () => {
    const { g, self, p2life0 } = armed(1);
