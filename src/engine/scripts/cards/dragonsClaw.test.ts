// `Dragon's Claw` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRAGONS_CLAW_SCRIPT } from './dragonsClaw';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dragon's Claw";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dragon's Claw", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRAGONS_CLAW_SCRIPT]),
  });
  holdEverywhere(g);
