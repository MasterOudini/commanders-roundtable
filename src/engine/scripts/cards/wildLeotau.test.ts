// `Wild Leotau` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WILD_LEOTAU_SCRIPT } from './wildLeotau';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wild Leotau";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wild Leotau", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WILD_LEOTAU_SCRIPT]),
  });
  holdEverywhere(g);
