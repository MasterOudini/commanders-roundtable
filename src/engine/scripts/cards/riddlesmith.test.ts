// `Riddlesmith` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIDDLESMITH_SCRIPT } from './riddlesmith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Riddlesmith";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Riddlesmith", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIDDLESMITH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Riddlesmith", () => {
  test("Whenever you cast an artifact spell: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(0);
