// `Totem-Guide Hartebeest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TOTEM_GUIDE_HARTEBEEST_SCRIPT } from './totemGuideHartebeest';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Totem-Guide Hartebeest";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Totem-Guide Hartebeest", "Aboshan's Desire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TOTEM_GUIDE_HARTEBEEST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Totem-Guide Hartebeest", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for an Aura card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Aboshan's Desire" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Aboshan's Desire");
      expect(found, "Aboshan's Desire is not in the library for the search to find").toBeDefined();
