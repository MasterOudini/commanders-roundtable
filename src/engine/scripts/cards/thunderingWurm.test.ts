// `Thundering Wurm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THUNDERING_WURM_SCRIPT } from './thunderingWurm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thundering Wurm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thundering Wurm", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THUNDERING_WURM_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const priceL0_p1_0 = put(g, 'p1', "Forest", 'hand');
  settle(g);
...
describe("Thundering Wurm", () => {
  test("When this creature enters: the vocabulary resolves \"Sacrifice it unless you discard a land card.\" - the price is declined", () => {
  ...
  test("When this creature enters: the vocabulary resolves \"Sacrifice it unless you discard a land card.\" - the price is paid", () => {
