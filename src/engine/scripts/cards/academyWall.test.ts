// `Academy Wall` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ACADEMY_WALL_SCRIPT } from './academyWall';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Academy Wall";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Academy Wall", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ACADEMY_WALL_SCRIPT]),
...
describe("Academy Wall", () => {
  test("Whenever you cast an instant or sorcery spell: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(0);
  ...
  test('the once-per-turn rider rides the def (D492)', () => {
    expect(ACADEMY_WALL_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("castInstantSorcery-1") && t.oncePerTurn === true), 'the def of line 1 carries oncePerTurn').toBeDefined();
  });
