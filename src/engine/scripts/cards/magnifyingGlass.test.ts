// `Magnifying Glass` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAGNIFYING_GLASS_SCRIPT } from './magnifyingGlass';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Magnifying Glass";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Magnifying Glass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAGNIFYING_GLASS_SCRIPT]),
  });
  holdEverywhere(g);
