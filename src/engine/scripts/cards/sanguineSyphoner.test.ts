// `Sanguine Syphoner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SANGUINE_SYPHONER_SCRIPT } from './sanguineSyphoner';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sanguine Syphoner";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sanguine Syphoner"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SANGUINE_SYPHONER_SCRIPT]),
  });
  holdEverywhere(g);
