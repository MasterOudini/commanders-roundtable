// `Twinblade Assassins` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TWINBLADE_ASSASSINS_SCRIPT } from './twinbladeAssassins';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Twinblade Assassins";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Twinblade Assassins", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TWINBLADE_ASSASSINS_SCRIPT]),
  });
  holdEverywhere(g);
