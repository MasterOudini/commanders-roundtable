// `Seal Away` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEAL_AWAY_SCRIPT } from './sealAway';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Seal Away";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seal Away"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEAL_AWAY_SCRIPT]),
  });
  holdEverywhere(g);
