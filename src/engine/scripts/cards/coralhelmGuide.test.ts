// `Coralhelm Guide` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CORALHELM_GUIDE_SCRIPT } from './coralhelmGuide';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Coralhelm Guide";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Coralhelm Guide"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CORALHELM_GUIDE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Coralhelm Guide", () => {
  test("{4}{U}: the vocabulary resolves \"Target creature can't be blocked this turn.\"", () => {
    const { g, no } = armed(0);
