// `Tumble Magnet` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TUMBLE_MAGNET_SCRIPT } from './tumbleMagnet';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tumble Magnet";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TUMBLE_MAGNET_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tumble Magnet"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TUMBLE_MAGNET_SCRIPT]),
...
describe("Tumble Magnet", () => {
  ...
  test("{T}, Remove a charge counter from this artifact: the vocabulary resolves \"Tap target artifact or creature.\"", () => {
