// `Reassembling Skeleton` - every printed ability proven in its own game: the cost's mark, the pump
...
import { REASSEMBLING_SKELETON_SCRIPT } from './reassemblingSkeleton';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Reassembling Skeleton";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Reassembling Skeleton"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([REASSEMBLING_SKELETON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Reassembling Skeleton", () => {
  test("{1}{B}: it returns from the graveyard to the battlefield tapped", () => {
