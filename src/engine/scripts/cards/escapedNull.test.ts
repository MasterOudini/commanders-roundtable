// `Escaped Null` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ESCAPED_NULL_SCRIPT } from './escapedNull';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Escaped Null";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ESCAPED_NULL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Escaped Null"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ESCAPED_NULL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Escaped Null", () => {
  test("Whenever this creature blocks or becomes blocked: it gets +5/+0 until end of turn", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([6, 2]);
  });
