// `Millstone` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MILLSTONE_SCRIPT } from './millstone';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Millstone";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Millstone"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MILLSTONE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Millstone", () => {
  test("{2}, {T}: the vocabulary resolves \"Target player mills two cards.\"", () => {
