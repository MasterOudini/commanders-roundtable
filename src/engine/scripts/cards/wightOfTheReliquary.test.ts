// `Wight of the Reliquary` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WIGHT_OF_THE_RELIQUARY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wight of the Reliquary", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WIGHT_OF_THE_RELIQUARY_SCRIPT]),
...
describe("Wight of the Reliquary", () => {
  test("This creature gets +1/+1 for each creature card in your graveyard.: it gets +1/+1 for each creature card in your graveyard", () => {
    const { g, self, cda0, cdaT0 } = armed(0);
    expect(pt(g, self)).toEqual([cda0 + 1, cdaT0 + 1]);
  });

  test("{T}, Sacrifice another creature: the vocabulary resolves \"Search your library for a land card, put it onto the battlefield tapped, then shuffle.\"", () => {
