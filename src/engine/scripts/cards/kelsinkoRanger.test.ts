// `Kelsinko Ranger` - the named target is accepted and the pump lands; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { KELSINKO_RANGER_SCRIPT } from './kelsinkoRanger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kelsinko Ranger";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Kelsinko Ranger"], ["Grizzly Bears", "Vampire Nighthawk"]],
    scripts: createRegistry([KELSINKO_RANGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kelsinko Ranger", () => {
  test("Grizzly Bears is a legal target and the pump lands", () => {
    ...
    settle(g);
    const d = deps(createRegistry([KELSINKO_RANGER_SCRIPT]));
