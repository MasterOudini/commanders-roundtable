// `Perilous Myr` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PERILOUS_MYR_SCRIPT } from './perilousMyr';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Perilous Myr";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Perilous Myr"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PERILOUS_MYR_SCRIPT]),
  });
  holdEverywhere(g);
