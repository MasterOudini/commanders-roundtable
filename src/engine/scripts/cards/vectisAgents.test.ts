// `Vectis Agents` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VECTIS_AGENTS_SCRIPT } from './vectisAgents';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vectis Agents";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VECTIS_AGENTS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vectis Agents"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VECTIS_AGENTS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vectis Agents", () => {
  test("{U}{B}: it gets -2/+0 until end of turn and can't be blocked this turn", () => {
