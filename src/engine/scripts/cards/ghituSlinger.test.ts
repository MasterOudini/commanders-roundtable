// `Ghitu Slinger` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghitu Slinger", "Forest", "Forest", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHITU_SLINGER_SCRIPT]),
  });
  holdEverywhere(g);
