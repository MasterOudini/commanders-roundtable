// `Quandrix Cultivator` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Quandrix Cultivator", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([QUANDRIX_CULTIVATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Quandrix Cultivator", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a basic Forest or Island card, put it onto the battlefield, then shuffle.\"", () => {
