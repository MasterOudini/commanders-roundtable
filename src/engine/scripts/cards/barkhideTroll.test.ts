// `Barkhide Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BARKHIDE_TROLL_SCRIPT } from './barkhideTroll';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Barkhide Troll";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BARKHIDE_TROLL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Barkhide Troll"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BARKHIDE_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Barkhide Troll", () => {
  ...
  test("{1}, Remove a +1/+1 counter from this creature: it gains hexproof until end of turn", () => {
    const { g, self, cnt0, pt0 } = armed(1);
    expect(pt(g, self)).toEqual([(pt0[0] ?? 0) + 0 - 1, (pt0[1] ?? 0) + 0 - 1]);
