// `Liliana's Steward` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LILIANAS_STEWARD_SCRIPT } from './lilianasSteward';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Liliana's Steward";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Liliana's Steward"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LILIANAS_STEWARD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Liliana's Steward", () => {
  test("{T}, Sacrifice this creature: the vocabulary resolves \"Target opponent discards a card.\"", () => {
