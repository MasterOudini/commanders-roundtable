// `Stone Catapult` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STONE_CATAPULT_SCRIPT } from './stoneCatapult';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stone Catapult";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stone Catapult"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STONE_CATAPULT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stone Catapult", () => {
  test("{T}: the vocabulary resolves \"Destroy target tapped nonblack creature.\" - refused unless during your turn, before attackers are declared", () => {
