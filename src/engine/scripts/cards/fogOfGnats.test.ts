// `Fog of Gnats` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FOG_OF_GNATS_SCRIPT } from './fogOfGnats';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fog of Gnats";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fog of Gnats", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOG_OF_GNATS_SCRIPT]),
  });
  holdEverywhere(g);
