// `Indomitable Will` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+2;
...
import { INDOMITABLE_WILL_SCRIPT } from './indomitableWill';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Indomitable Will";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([INDOMITABLE_WILL_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([INDOMITABLE_WILL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Indomitable Will", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+2; Cyclops of One-Eyed Pass is untouched", () => {
