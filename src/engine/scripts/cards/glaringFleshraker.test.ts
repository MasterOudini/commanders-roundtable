// `Glaring Fleshraker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GLARING_FLESHRAKER_SCRIPT } from './glaringFleshraker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Glaring Fleshraker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glaring Fleshraker", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLARING_FLESHRAKER_SCRIPT]),
...
describe("Glaring Fleshraker", () => {
  test("Whenever you cast a colorless spell: the vocabulary resolves \"Create a 0/1 colorless Eldrazi Spawn creature token with \\\"Sacrifice this token: Add {C}.\\\"\"", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("Whenever another colorless creature you control enters: it deals 1 damage to each opponent", () => {
