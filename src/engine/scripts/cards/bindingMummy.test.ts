// `Binding Mummy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BINDING_MUMMY_SCRIPT } from './bindingMummy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Binding Mummy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Binding Mummy", "Grizzly Bears", "Gutter Skulk"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BINDING_MUMMY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Binding Mummy", () => {
  test("Whenever another Zombie you control enters: the vocabulary resolves \"Tap target artifact or creature.\"", () => {
    const { g, no } = armed(0);
