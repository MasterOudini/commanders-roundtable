// `Spire Tracer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIRE_TRACER_SCRIPT } from './spireTracer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spire Tracer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spire Tracer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPIRE_TRACER_SCRIPT]),
