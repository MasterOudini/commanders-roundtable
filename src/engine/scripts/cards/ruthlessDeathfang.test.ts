// `Ruthless Deathfang` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUTHLESS_DEATHFANG_SCRIPT } from './ruthlessDeathfang';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ruthless Deathfang";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([RUTHLESS_DEATHFANG_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ruthless Deathfang", "Blood Pet"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUTHLESS_DEATHFANG_SCRIPT]),
...
describe("Ruthless Deathfang", () => {
  test("Whenever you sacrifice a creature: the vocabulary resolves \"Target opponent sacrifices a creature of their choice.\"", () => {
    const { g } = armed(0);
