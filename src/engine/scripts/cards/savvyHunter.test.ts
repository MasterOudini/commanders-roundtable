// `Savvy Hunter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SAVVY_HUNTER_SCRIPT } from './savvyHunter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';
import { TOKEN_TABLE } from '../../../data/tokenTable';

const CARD = "Savvy Hunter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Savvy Hunter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SAVVY_HUNTER_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: TOKEN_TABLE["Food|/||Artifact|"]?.printingId ?? '', count: 2 }));
...
describe("Savvy Hunter", () => {
  test("Whenever this creature attacks or blocks: 1 token made", () => {
  ...
  test("Sacrifice two Foods: a card is drawn", () => {
