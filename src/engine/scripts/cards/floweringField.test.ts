// `Flowering Field` - the carrier proven end to end (D367/D368) with a D384 payload: the recipient
...
import { FLOWERING_FIELD_SCRIPT } from './floweringField';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flowering Field";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Flowering Field","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLOWERING_FIELD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Flowering Field", () => {
  test("grants {T}: Prevent the next 1 damage that would be dealt to any target this turn. to Forest, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and the body hits the RECIPIENT', () => {
    ...
    // CR 601.2 - the cost was paid when the target was answered, not at the submit (D169).
    // CR 602.2b - the tap is the RECIPIENT's, paid as part of the cost.
    expect(g.state.cards[host]?.tapped).toBe(true);
    expect(g.state.preventionShields.length).toBeGreaterThan(0);
