// `Unbender Tine` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNBENDER_TINE_SCRIPT } from './unbenderTine';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unbender Tine";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Unbender Tine"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNBENDER_TINE_SCRIPT]),
  });
  holdEverywhere(g);
