// `Goblin Bushwhacker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_BUSHWHACKER_SCRIPT } from './goblinBushwhacker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Bushwhacker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOBLIN_BUSHWHACKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Bushwhacker", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_BUSHWHACKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Bushwhacker", () => {
  test("When this creature enters: its controller's creatures get +1/+0 and gain haste until end of turn", () => {
