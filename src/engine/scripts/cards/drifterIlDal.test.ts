// `Drifter il-Dal` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRIFTER_IL_DAL_SCRIPT } from './drifterIlDal';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drifter il-Dal";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drifter il-Dal", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRIFTER_IL_DAL_SCRIPT]),
  });
  holdEverywhere(g);
