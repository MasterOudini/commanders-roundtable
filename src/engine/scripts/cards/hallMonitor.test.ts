// `Hall Monitor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HALL_MONITOR_SCRIPT } from './hallMonitor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hall Monitor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hall Monitor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HALL_MONITOR_SCRIPT]),
  });
  holdEverywhere(g);
