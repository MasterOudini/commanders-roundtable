// `Taoist Mystic` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TAOIST_MYSTIC_SCRIPT } from './taoistMystic';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Taoist Mystic";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Taoist Mystic", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Wei Scout"]],
    scripts: createRegistry([TAOIST_MYSTIC_SCRIPT]),
...
describe("Taoist Mystic", () => {
  test("This creature can't be blocked by creatures with horsemanship.: the block by creatures with horsemanship is refused", () => {
