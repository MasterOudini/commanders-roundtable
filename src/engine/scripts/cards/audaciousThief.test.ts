// `Audacious Thief` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AUDACIOUS_THIEF_SCRIPT } from './audaciousThief';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Audacious Thief";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Audacious Thief"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AUDACIOUS_THIEF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Audacious Thief", () => {
  test("Whenever this creature attacks: a card is drawn and 1 life lost", () => {
