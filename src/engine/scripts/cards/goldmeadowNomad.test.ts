// `Goldmeadow Nomad` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOLDMEADOW_NOMAD_SCRIPT } from './goldmeadowNomad';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goldmeadow Nomad";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goldmeadow Nomad"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOLDMEADOW_NOMAD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goldmeadow Nomad", () => {
  test("{W}, Exile this card from your graveyard: 1 token made", () => {
