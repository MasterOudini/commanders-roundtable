// `Leshrac's Rite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LESHRACS_RITE_SCRIPT } from './leshracsRite';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Leshrac's Rite";
...
function lw(g: Game, id: InstanceId): readonly string[] {
  const d = deps(createRegistry([LESHRACS_RITE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Leshrac's Rite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LESHRACS_RITE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Leshrac's Rite", () => {
  test("Enchanted creature has swampwalk. (It can't be blocked as long as defending player controls a Swamp.): the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(lw(g, bearsB)).toContain("Swamp");
  });
