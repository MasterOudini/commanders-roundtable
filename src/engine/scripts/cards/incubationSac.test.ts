// `Incubation Sac` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INCUBATION_SAC_SCRIPT } from './incubationSac';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Incubation Sac";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([INCUBATION_SAC_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Incubation Sac"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INCUBATION_SAC_SCRIPT]),
...
describe("Incubation Sac", () => {
  test("This artifact enters with three oil counters on it.: cast, it enters with 3 oil counters", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.zone.kind).toBe('battlefield');
    expect(g.state.cards[self]?.counters["oil"] ?? 0).toBe(3);
  });

  test("{4}, {T}, Remove an oil counter from this artifact: 1 token made", () => {
