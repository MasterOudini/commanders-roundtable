// `Moria Marauder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MORIA_MARAUDER_SCRIPT } from './moriaMarauder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Moria Marauder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Moria Marauder", "Mons's Goblin Raiders", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MORIA_MARAUDER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const stillFix0 = put(g, 'p1', "Mons's Goblin Raiders");
  settle(g);
...
describe("Moria Marauder", () => {
  test("Whenever a Goblin or Orc you control deals combat damage to a player: the vocabulary resolves \"Exile the top card of your library. You may play that card this turn.\"", () => {
    const { g } = armed(0);
