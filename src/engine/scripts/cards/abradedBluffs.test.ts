// `Abraded Bluffs` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ABRADED_BLUFFS_SCRIPT } from './abradedBluffs';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Abraded Bluffs";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Abraded Bluffs"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ABRADED_BLUFFS_SCRIPT]),
  });
  holdEverywhere(g);
