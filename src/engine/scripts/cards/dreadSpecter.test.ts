// `Dread Specter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DREAD_SPECTER_SCRIPT } from './dreadSpecter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dread Specter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dread Specter"], ["Cyclops of One-Eyed Pass", "Staunch Shieldmate"]],
    scripts: createRegistry([DREAD_SPECTER_SCRIPT]),
