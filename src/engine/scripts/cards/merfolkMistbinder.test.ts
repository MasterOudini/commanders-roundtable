// `Merfolk Mistbinder` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MERFOLK_MISTBINDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merfolk Mistbinder", "Merfolk of the Pearl Trident"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERFOLK_MISTBINDER_SCRIPT]),
  });
  holdEverywhere(g);
