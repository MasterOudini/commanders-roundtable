// `Lance` - cast on Grizzly Bears it attaches and the enchanted creature has firstStrike;
...
import { LANCE_SCRIPT } from './lance';
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([LANCE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([LANCE_SCRIPT]),
  });
  holdEverywhere(g);
