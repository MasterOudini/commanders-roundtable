// `Hurloon Shaman` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HURLOON_SHAMAN_SCRIPT } from './hurloonShaman';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hurloon Shaman";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hurloon Shaman", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HURLOON_SHAMAN_SCRIPT]),
  });
  holdEverywhere(g);
