// `Chub Toad` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHUB_TOAD_SCRIPT } from './chubToad';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chub Toad";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHUB_TOAD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chub Toad"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHUB_TOAD_SCRIPT]),
  });
  holdEverywhere(g);
