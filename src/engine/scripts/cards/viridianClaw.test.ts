// `Viridian Claw` - equipped to Grizzly Bears the creature gets +1/+0 and has firstStrike; the Cyclops is untouched;
...
import { VIRIDIAN_CLAW_SCRIPT } from './viridianClaw';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Viridian Claw";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VIRIDIAN_CLAW_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([VIRIDIAN_CLAW_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Viridian Claw", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+0 and has firstStrike; the Cyclops is untouched", () => {
