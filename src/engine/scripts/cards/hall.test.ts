// `Hall` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hall"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HALL_SCRIPT]),
  });
  holdEverywhere(g);
