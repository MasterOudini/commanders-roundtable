// `Keldon Mantle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KELDON_MANTLE_SCRIPT } from './keldonMantle';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Keldon Mantle";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KELDON_MANTLE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Keldon Mantle", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KELDON_MANTLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Keldon Mantle", () => {
  ...
  test("{R}: the enchanted creature is pumped until end of turn", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([3, 2]);
  });

  test("{G}: the enchanted creature is pumped until end of turn", () => {
    const { g, self, bearsB } = armed(2);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([2, 2]);
