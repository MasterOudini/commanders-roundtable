// `Lightning Cloud` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIGHTNING_CLOUD_SCRIPT } from './lightningCloud';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning Cloud";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lightning Cloud", "Grizzly Bears", "Crimson Kobolds", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIGHTNING_CLOUD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lightning Cloud", () => {
  test("Whenever a player casts a red spell: the vocabulary resolves \"You may pay {R}. If you do, this enchantment deals 1 damage to any target.\" - the price is declined", () => {
  ...
  test("Whenever a player casts a red spell: the vocabulary resolves \"You may pay {R}. If you do, this enchantment deals 1 damage to any target.\" - the price is paid", () => {
    const { g, no, payL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
    ...
    expect(g.state.cards[no]?.damage ?? 0).toBe(1);
  });
