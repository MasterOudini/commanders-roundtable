// `Umbral Mantle` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { UMBRAL_MANTLE_SCRIPT } from './umbralMantle';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Umbral Mantle";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Umbral Mantle","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UMBRAL_MANTLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Umbral Mantle", () => {
  test("grants {3}, {Q}: This creature gets +2/+2 until end of turn. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and the body hits the RECIPIENT', () => {
    ...
    // ⚠️ TAPPED FIRST: an untap is observable only on a tapped permanent (D128), and a
    // `{Q}` cost is OFFERED only while the permanent is tapped (legal.ts reads requiresUntap).
    must(g.submit({ t: 'ManualSetTapped', player: 'p1', cards: [host], tapped: true }));
    settle(g);
    const granted = offers(g, host).filter((x) => x.t === 'ActivateAbility' && x.grantRef !== undefined);
    expect(granted).toHaveLength(1);
    const offer = granted[0];
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{3}, {Q}");
