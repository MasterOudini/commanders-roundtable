// `Kraul Swarm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KRAUL_SWARM_SCRIPT } from './kraulSwarm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kraul Swarm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kraul Swarm", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KRAUL_SWARM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kraul Swarm", () => {
  test("{2}{B}, Discard a creature card: it returns from the graveyard to hand", () => {
