// `Marrow Bats` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARROW_BATS_SCRIPT } from './marrowBats';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Marrow Bats";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Marrow Bats", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARROW_BATS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Marrow Bats", () => {
  test("Pay 4 life: it regenerates from the Bolt", () => {
