// `Crested Sunmare` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRESTED_SUNMARE_SCRIPT } from './crestedSunmare';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crested Sunmare";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CRESTED_SUNMARE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crested Sunmare", "Armored Warhorse"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRESTED_SUNMARE_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Armored Warhorse");
...
describe("Crested Sunmare", () => {
  test("Other Horses you control have indestructible.: only the Horse reads it", () => {
