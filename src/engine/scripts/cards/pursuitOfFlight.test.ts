// `Pursuit of Flight` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { PURSUIT_OF_FLIGHT_SCRIPT } from './pursuitOfFlight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pursuit of Flight";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Pursuit of Flight","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PURSUIT_OF_FLIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pursuit of Flight", () => {
  test("grants {U}: This creature gains flying until end of turn. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and the body hits the RECIPIENT', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{U}");
