// `Mikokoro, Center of the Sea` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MIKOKORO_CENTER_OF_THE_SEA_SCRIPT } from './mikokoroCenterOfTheSea';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mikokoro, Center of the Sea";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mikokoro, Center of the Sea"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIKOKORO_CENTER_OF_THE_SEA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mikokoro, Center of the Sea", () => {
  test("{2}, {T}: the vocabulary resolves \"Each player draws a card.\"", () => {
    const { g, self, hand0, p2hand0 } = armed(0);
