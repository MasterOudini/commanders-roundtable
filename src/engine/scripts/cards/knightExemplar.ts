// `Knight Exemplar` - a layer-6 anthem: "Other Knight creatures you control get +1/+1 and have indestructible. (Damage and effects that say "destroy" don't destroy them.)". A StaticDef in the shape of the
...
import { KNIGHT_EXEMPLAR } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(KNIGHT_EXEMPLAR, "First strike (This creature deals combat damage before creatures without first strike.)\nOther Knight creatures you control get +1/+1 and have indestructible. (Damage and effects that say \"destroy\" don't destroy them.)");
const TEXT = PRINTED.split('\n')[1] as string;

export const KNIGHT_EXEMPLAR_SCRIPT: CardScript = {
  oracleId: KNIGHT_EXEMPLAR.oracleId,
  name: KNIGHT_EXEMPLAR.name,
  statics: [
    {
      abilityId: 'anthem',
      text: TEXT,
      ...
      appliesTo: (ctx, self, candidate, chars) => {
        ...
        if (!chars.typeLine.subtypes.includes("Knight")) return false;
        return true;
      },
      modify: (chars) => {
