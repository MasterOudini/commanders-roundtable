// `Gnarlwood Dryad` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GNARLWOOD_DRYAD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gnarlwood Dryad", "Grizzly Bears", "Forest", "Sol Ring", "Hissing Miasma"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GNARLWOOD_DRYAD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gnarlwood Dryad", () => {
  test("`Gnarlwood Dryad` - as long as there are four or more card types among cards in your graveyard: absent while it is unmet, read once it holds", () => {
