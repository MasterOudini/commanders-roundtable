// `Gorehorn Raider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOREHORN_RAIDER_SCRIPT } from './gorehornRaider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gorehorn Raider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gorehorn Raider", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOREHORN_RAIDER_SCRIPT]),
  });
  holdEverywhere(g);
