// `Amugaba` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AMUGABA_SCRIPT } from './amugaba';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Amugaba", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AMUGABA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Amugaba", () => {
  test("{2}{U}, Discard a card: it returns to its owner's hand", () => {
