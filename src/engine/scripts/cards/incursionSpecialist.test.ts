// `Incursion Specialist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INCURSION_SPECIALIST_SCRIPT } from './incursionSpecialist';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Incursion Specialist";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([INCURSION_SPECIALIST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Incursion Specialist", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INCURSION_SPECIALIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Incursion Specialist", () => {
  test("Whenever you cast your second spell each turn: it gets +2/+0 until end of turn and can't be blocked this turn", () => {
