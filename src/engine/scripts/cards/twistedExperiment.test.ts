// `Twisted Experiment` - cast on Grizzly Bears it attaches and the enchanted creature gets +3/-1;
...
import { TWISTED_EXPERIMENT_SCRIPT } from './twistedExperiment';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Twisted Experiment";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TWISTED_EXPERIMENT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([TWISTED_EXPERIMENT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Twisted Experiment", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +3/-1; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([5, 1]);
