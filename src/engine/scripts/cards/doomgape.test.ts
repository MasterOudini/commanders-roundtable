// `Doomgape` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DOOMGAPE_SCRIPT } from './doomgape';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Doomgape", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DOOMGAPE_SCRIPT]),
...
describe("Doomgape", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice a creature. You gain life equal to that creature's toughness.\"", () => {
    const { g, life0, board0, queueFodderL1_upkeep } = armed(0);
