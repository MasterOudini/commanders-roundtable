// `Taurean Mauler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TAUREAN_MAULER_SCRIPT } from './taureanMauler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Taurean Mauler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Taurean Mauler"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([TAUREAN_MAULER_SCRIPT]),
  });
  holdEverywhere(g);
