// `Null Group Biological Assets` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NULL_GROUP_BIOLOGICAL_ASSETS_SCRIPT } from './nullGroupBiologicalAssets';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Null Group Biological Assets";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; condOff: boolean; priceL1_p1_0: InstanceId };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NULL_GROUP_BIOLOGICAL_ASSETS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Null Group Biological Assets", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NULL_GROUP_BIOLOGICAL_ASSETS_SCRIPT]),
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, condOff, priceL1_p1_0 };
}

describe("Null Group Biological Assets", () => {
  test("`Null Group Biological Assets` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"You may discard a card. If you do, draw a card.\" - the price is declined", () => {
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"You may discard a card. If you do, draw a card.\" - the price is paid", () => {
    const { g, priceL1_p1_0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
