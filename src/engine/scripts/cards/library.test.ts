// `Library` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIBRARY_SCRIPT } from './library';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Library"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIBRARY_SCRIPT]),
  });
  holdEverywhere(g);
