// `Captain Sisay` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Captain Sisay", "Abstergo Entertainment"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAPTAIN_SISAY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Captain Sisay", () => {
  test("{T}: the vocabulary resolves \"Search your library for a legendary card, reveal that card, put it into your hand, then shuffle.\"", () => {
