// `Thriving Turtle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THRIVING_TURTLE_SCRIPT } from './thrivingTurtle';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thriving Turtle";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thriving Turtle"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THRIVING_TURTLE_SCRIPT]),
