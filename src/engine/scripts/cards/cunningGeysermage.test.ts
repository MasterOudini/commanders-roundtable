// `Cunning Geysermage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CUNNING_GEYSERMAGE_SCRIPT } from './cunningGeysermage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cunning Geysermage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cunning Geysermage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CUNNING_GEYSERMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cunning Geysermage", () => {
  test("When this creature enters: the vocabulary resolves \"Return up to one other target creature to its owner's hand.\"", () => {
    const { g, no } = armed(0);
