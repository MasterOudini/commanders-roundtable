// `Artificer's Assistant` — the historic-cast filter (Jhoira's, D183) paying
// a SCRY instead of a draw: an artifact cast asks, a vanilla creature cast
// does not.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ARTIFICERS_ASSISTANT_SCRIPT } from './artificersAssistant';
...
function board(): Game {
  const g = startedGame({
    players: 2,
    decks: [["Artificer's Assistant", 'Sol Ring', 'Grizzly Bears'], ['Grizzly Bears']],
    scripts: createRegistry([ARTIFICERS_ASSISTANT_SCRIPT]),
  });
  put(g, 'p1', "Artificer's Assistant");
  settle(g);
  holdEverywhere(g);
...
describe("Artificer's Assistant", () => {
  test('an ARTIFACT cast raises the scry; the answer clears it', () => {
    const g = board();
  ...
  test('a NON-historic cast pays nothing', () => {
    const g = board();
