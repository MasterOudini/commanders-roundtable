// `Somberwald Vigilante` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOMBERWALD_VIGILANTE_SCRIPT } from './somberwaldVigilante';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Somberwald Vigilante";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Somberwald Vigilante"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOMBERWALD_VIGILANTE_SCRIPT]),
...
describe("Somberwald Vigilante", () => {
  test("Whenever this creature becomes blocked by a creature: the vocabulary resolves \"~ deals 1 damage to target creature.\"", () => {
    const { g, no } = armed(0);
