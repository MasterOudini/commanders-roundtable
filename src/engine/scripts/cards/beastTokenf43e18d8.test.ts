// `Beast` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BEAST_TOKENF43E18D8_SCRIPT } from './beastTokenf43e18d8';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([BEAST_TOKENF43E18D8_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: BEAST_F43E18D8_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === BEAST_F43E18D8_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
