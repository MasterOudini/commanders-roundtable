// `Ardent Soldier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ARDENT_SOLDIER_SCRIPT } from './ardentSoldier';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ardent Soldier";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ARDENT_SOLDIER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ardent Soldier"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARDENT_SOLDIER_SCRIPT]),
  });
  holdEverywhere(g);
