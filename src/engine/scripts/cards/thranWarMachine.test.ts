// `Thran War Machine` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thran War Machine", "Grizzly Bears", "Forest", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THRAN_WAR_MACHINE_SCRIPT]),
  });
  holdEverywhere(g);
