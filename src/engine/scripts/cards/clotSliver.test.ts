// `Clot Sliver` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { CLOT_SLIVER_SCRIPT } from './clotSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Clot Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Clot Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLOT_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Clot Sliver", () => {
  test("grants {2}: Regenerate this permanent. to Metallic Sliver, and to nothing outside the scope", () => {
