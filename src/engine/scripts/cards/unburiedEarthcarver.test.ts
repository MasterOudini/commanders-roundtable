// `Unburied Earthcarver` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNBURIED_EARTHCARVER_SCRIPT } from './unburiedEarthcarver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unburied Earthcarver";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Unburied Earthcarver", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNBURIED_EARTHCARVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Unburied Earthcarver", () => {
  test("{2}, Sacrifice another creature: it gets 1 +1/+1 counter", () => {
