// `Zephyr Net` - cast on Grizzly Bears it attaches and the enchanted creature has defender, flying;
...
import { ZEPHYR_NET_SCRIPT } from './zephyrNet';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zephyr Net";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([ZEPHYR_NET_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ZEPHYR_NET_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zephyr Net", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has defender, flying; Cyclops of One-Eyed Pass is untouched", () => {
