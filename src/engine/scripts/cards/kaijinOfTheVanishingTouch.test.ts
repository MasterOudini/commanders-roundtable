// `Kaijin of the Vanishing Touch` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KAIJIN_OF_THE_VANISHING_TOUCH_SCRIPT } from './kaijinOfTheVanishingTouch';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kaijin of the Vanishing Touch";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kaijin of the Vanishing Touch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KAIJIN_OF_THE_VANISHING_TOUCH_SCRIPT]),
...
describe("Kaijin of the Vanishing Touch", () => {
  test("Whenever this creature blocks a creature: the vocabulary resolves \"Return target creature to its owner's hand at end of combat.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.delayedTriggers.length, 'the payload is armed, not run').toBe(1);
    {
      ...
      settle(g);
      expect(g.state.cards[no]?.zone.kind, 'bounced at the step').toBe('hand');
    }
