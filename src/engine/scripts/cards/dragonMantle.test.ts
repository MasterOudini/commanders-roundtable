// `Dragon Mantle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRAGON_MANTLE_SCRIPT } from './dragonMantle';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DRAGON_MANTLE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dragon Mantle", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRAGON_MANTLE_SCRIPT]),
...
describe("Dragon Mantle", () => {
  test("When this Aura enters: a card is drawn", () => {
    const { g, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1 - 1);
  });

  test("Enchanted creature has \"{R}: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
  });

  test("Enchanted creature has \"{R}: the vocabulary resolves \"~ gets +1/+0 until end of turn.\"", () => {
    const { g, bearsB } = armed(2);
