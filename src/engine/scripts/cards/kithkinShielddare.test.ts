// `Kithkin Shielddare` — pump on their BLOCKING creature: the right target is
// accepted, a creature that is attacking, not blocking, is refused (D291). Generated from one table row (D292).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { KITHKIN_SHIELDDARE_SCRIPT } from './kithkinShielddare';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kithkin Shielddare";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([KITHKIN_SHIELDDARE_SCRIPT]),
  });
...
describe("Kithkin Shielddare", () => {
  test('the blocking creature is a legal target and the effect lands', () => {
    ...
    settle(g);
    const d = deps(createRegistry([KITHKIN_SHIELDDARE_SCRIPT]));
