// `Duskmantle Operative` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DUSKMANTLE_OPERATIVE_SCRIPT } from './duskmantleOperative';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Duskmantle Operative";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Duskmantle Operative", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUSKMANTLE_OPERATIVE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Duskmantle Operative", () => {
  test("This creature can't be blocked by creatures with power 4 or greater.: the block is refused", () => {
