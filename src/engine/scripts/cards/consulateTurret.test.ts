// `Consulate Turret` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONSULATE_TURRET_SCRIPT } from './consulateTurret';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Consulate Turret";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Consulate Turret"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONSULATE_TURRET_SCRIPT]),
...
describe("Consulate Turret", () => {
  test("{T}: the vocabulary resolves \"You get {E}.\"", () => {
    const { g, self, energy0 } = armed(0);
    expect(g.state.players.p1?.energy).toBe(energy0 + 1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{T}, Pay {E}{E}{E}: it deals 2 damage to the declared player", () => {
    const { g, self, energy0, p2life0 } = armed(1);
