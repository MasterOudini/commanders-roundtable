// `Ironhoof Ox` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IRONHOOF_OX_SCRIPT } from './ironhoofOx';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ironhoof Ox";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ironhoof Ox", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([IRONHOOF_OX_SCRIPT]),
  });
  holdEverywhere(g);
