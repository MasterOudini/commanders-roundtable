// `Paradise Druid` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PARADISE_DRUID_SCRIPT } from './paradiseDruid';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Paradise Druid";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PARADISE_DRUID_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Paradise Druid"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PARADISE_DRUID_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Paradise Druid", () => {
  test("`Paradise Druid` - as long as it's untapped: absent while it is unmet, read once it holds", () => {
