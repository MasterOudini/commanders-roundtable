// `Metathran Soldier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { METATHRAN_SOLDIER_SCRIPT } from './metathranSoldier';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Metathran Soldier";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Metathran Soldier", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([METATHRAN_SOLDIER_SCRIPT]),
  });
  holdEverywhere(g);
