// `History of Benalia` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HISTORY_OF_BENALIA_SCRIPT } from './historyOfBenalia';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "History of Benalia";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HISTORY_OF_BENALIA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["History of Benalia", "Kithkin Billyrider"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HISTORY_OF_BENALIA_SCRIPT]),
...
describe("History of Benalia", () => {
  test("I: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("III — Knights you control get +2/+1 until end of turn.: Knight creatures you control get +2/+1 until end of turn", () => {
