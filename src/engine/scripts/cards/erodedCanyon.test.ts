// `Eroded Canyon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ERODED_CANYON_SCRIPT } from './erodedCanyon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Eroded Canyon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Eroded Canyon"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ERODED_CANYON_SCRIPT]),
  });
  holdEverywhere(g);
