// `Postmortem Professor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { POSTMORTEM_PROFESSOR_SCRIPT } from './postmortemProfessor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Postmortem Professor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Postmortem Professor", "Grizzly Bears", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([POSTMORTEM_PROFESSOR_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const exg0 = [put(g, 'p1', "Giant Growth", 'graveyard')];
  settle(g);
  const self = put(g, 'p1', CARD, [2].includes(which) ? 'graveyard' : 'battlefield');
  settle(g);
...
describe("Postmortem Professor", () => {
  test("This creature can't block.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("Whenever this creature attacks: each opponent loses 1 life and 1 is gained", () => {
    const { g, life0, p2life0 } = armed(1);
    expect(g.state.players.p2?.life).toBe(p2life0 - 1);
    expect(g.state.players.p1?.life).toBe(life0 + 1);
  });

  test("{1}{B}, Exile an instant or sorcery card from your graveyard: it returns from the graveyard to the battlefield", () => {
    const { g, self, exg0 } = armed(2);
