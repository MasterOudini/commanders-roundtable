// `Invisible Stalker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INVISIBLE_STALKER_SCRIPT } from './invisibleStalker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Invisible Stalker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Invisible Stalker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INVISIBLE_STALKER_SCRIPT]),
  });
  holdEverywhere(g);
