// `Torch Slinger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TORCH_SLINGER_SCRIPT } from './torchSlinger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Torch Slinger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Torch Slinger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TORCH_SLINGER_SCRIPT]),
  });
  holdEverywhere(g);
