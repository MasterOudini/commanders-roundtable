// `Memory Erosion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MEMORY_EROSION_SCRIPT } from './memoryErosion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Memory Erosion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Memory Erosion"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([MEMORY_EROSION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Memory Erosion", () => {
  test("Whenever an opponent casts a spell: the vocabulary resolves \"Target player mills two cards.\"", () => {
