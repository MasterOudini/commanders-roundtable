// `Engine Rat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ENGINE_RAT_SCRIPT } from './engineRat';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Engine Rat";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Engine Rat"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ENGINE_RAT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Engine Rat", () => {
  test("{5}{B}: each opponent loses 2 life", () => {
