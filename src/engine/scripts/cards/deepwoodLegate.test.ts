// `Deepwood Legate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEEPWOOD_LEGATE_SCRIPT } from './deepwoodLegate';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deepwood Legate";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DEEPWOOD_LEGATE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deepwood Legate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEEPWOOD_LEGATE_SCRIPT]),
