// `Hit-Monkey` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HIT_MONKEY_SCRIPT } from './hitMonkey';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hit-Monkey";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hit-Monkey"], ["Cyclops of One-Eyed Pass", "Counterspell"]],
    scripts: createRegistry([HIT_MONKEY_SCRIPT]),
  });
  holdEverywhere(g);
