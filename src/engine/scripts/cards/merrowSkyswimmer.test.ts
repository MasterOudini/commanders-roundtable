// `Merrow Skyswimmer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERROW_SKYSWIMMER_SCRIPT } from './merrowSkyswimmer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Merrow Skyswimmer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merrow Skyswimmer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERROW_SKYSWIMMER_SCRIPT]),
