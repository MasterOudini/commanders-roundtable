// `Steelgaze Griffin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STEELGAZE_GRIFFIN_SCRIPT } from './steelgazeGriffin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Steelgaze Griffin";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STEELGAZE_GRIFFIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Steelgaze Griffin", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STEELGAZE_GRIFFIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Steelgaze Griffin", () => {
  test("Whenever you draw your second card each turn: it gets +2/+0 until end of turn", () => {
