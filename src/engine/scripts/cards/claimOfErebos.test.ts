// `Claim of Erebos` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { CLAIM_OF_EREBOS_SCRIPT } from './claimOfErebos';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Claim of Erebos";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Claim of Erebos","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLAIM_OF_EREBOS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Claim of Erebos", () => {
  test("grants {1}{B}, {T}: Target player loses 2 life. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and runs the payload', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{1}{B}, {T}");
