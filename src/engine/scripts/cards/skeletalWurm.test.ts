// `Skeletal Wurm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKELETAL_WURM_SCRIPT } from './skeletalWurm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skeletal Wurm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skeletal Wurm", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKELETAL_WURM_SCRIPT]),
  });
  holdEverywhere(g);
