// `Fresh-Faced Recruit` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FRESH_FACED_RECRUIT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fresh-Faced Recruit"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FRESH_FACED_RECRUIT_SCRIPT]),
...
describe("Fresh-Faced Recruit", () => {
  test("`Fresh-Faced Recruit` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
