// `Frontline War-Rager` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FRONTLINE_WAR_RAGER_SCRIPT } from './frontlineWarRager';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frontline War-Rager";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frontline War-Rager", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FRONTLINE_WAR_RAGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Frontline War-Rager", () => {
  test("At the beginning of your end step: it gets 1 +1/+1 counter", () => {
