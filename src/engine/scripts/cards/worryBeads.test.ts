// `Worry Beads` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WORRY_BEADS_SCRIPT } from './worryBeads';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Worry Beads";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Worry Beads"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WORRY_BEADS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Worry Beads", () => {
  test("At the beginning of each player's upkeep: the vocabulary resolves \"Target player mills a card.\"", () => {
