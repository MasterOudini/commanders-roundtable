// `Warden of the Woods` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WARDEN_OF_THE_WOODS_SCRIPT } from './wardenOfTheWoods';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Warden of the Woods";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Warden of the Woods"], ["Cyclops of One-Eyed Pass", "Giant Growth"]],
    scripts: createRegistry([WARDEN_OF_THE_WOODS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Warden of the Woods", () => {
  test("Whenever this creature becomes the target of a spell or ability an opponent controls: 2 cards are drawn", () => {
