// `Angry Rabble` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ANGRY_RABBLE_SCRIPT } from './angryRabble';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Angry Rabble";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Angry Rabble", "Grizzly Bears", "Amphin Cutthroat"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ANGRY_RABBLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Angry Rabble", () => {
  test("Whenever you cast a spell with mana value 4 or greater: it deals 1 damage to each opponent", () => {
    const { g, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 - 1);
  });

  test("{5}{R}: it gets 2 +1/+1 counters", () => {
