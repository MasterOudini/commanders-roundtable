// `Cloud Spirit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLOUD_SPIRIT_SCRIPT } from './cloudSpirit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cloud Spirit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cloud Spirit", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLOUD_SPIRIT_SCRIPT]),
  });
  holdEverywhere(g);
