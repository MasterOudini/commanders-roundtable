// `Dunland Crebain` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DUNLAND_CREBAIN_SCRIPT } from './dunlandCrebain';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dunland Crebain";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dunland Crebain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUNLAND_CREBAIN_SCRIPT]),
...
describe("Dunland Crebain", () => {
  test("When this creature enters: the vocabulary resolves \"Amass Orcs 2.\"", () => {
