// `Hekma Sentinels` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HEKMA_SENTINELS_SCRIPT } from './hekmaSentinels';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hekma Sentinels";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HEKMA_SENTINELS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hekma Sentinels"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HEKMA_SENTINELS_SCRIPT]),
  });
  holdEverywhere(g);
