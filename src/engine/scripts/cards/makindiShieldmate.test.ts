// `Makindi Shieldmate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAKINDI_SHIELDMATE_SCRIPT } from './makindiShieldmate';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Makindi Shieldmate";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Makindi Shieldmate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAKINDI_SHIELDMATE_SCRIPT]),
  });
  holdEverywhere(g);
