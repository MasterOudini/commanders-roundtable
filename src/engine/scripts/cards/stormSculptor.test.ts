// `Storm Sculptor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORM_SCULPTOR_SCRIPT } from './stormSculptor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Storm Sculptor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Storm Sculptor", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORM_SCULPTOR_SCRIPT]),
  });
  holdEverywhere(g);
