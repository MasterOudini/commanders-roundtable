// `Smelt-Ward Minotaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SMELT_WARD_MINOTAUR_SCRIPT } from './smeltWardMinotaur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Smelt-Ward Minotaur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Smelt-Ward Minotaur", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SMELT_WARD_MINOTAUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Smelt-Ward Minotaur", () => {
  test("Whenever you cast an instant or sorcery spell: the vocabulary resolves \"Target creature an opponent controls can't block this turn.\"", () => {
    const { g, no } = armed(0);
