// `Old Flitterfang` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OLD_FLITTERFANG_SCRIPT } from './oldFlitterfang';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Old Flitterfang";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([OLD_FLITTERFANG_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Old Flitterfang", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OLD_FLITTERFANG_SCRIPT]),
  });
  holdEverywhere(g);
