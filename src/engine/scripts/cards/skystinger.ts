// `Skystinger` - a blocks trigger pumping itself
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { SKYSTINGER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(SKYSTINGER, "Reach\nWhenever this creature blocks a creature with flying, this creature gets +5/+0 until end of turn.");
const LINES = PRINTED.split('\n');

export const SKYSTINGER_SCRIPT: CardScript = {
  oracleId: SKYSTINGER.oracleId,
  name: SKYSTINGER.name,
  triggers: [
    {
      abilityId: 'blocks-1',
      ...
      label: () => "Skystinger - it pumped until end of turn",
