// `Flood Plain` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flood Plain", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLOOD_PLAIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Flood Plain", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a Plains or Island card, put it onto the battlefield, then shuffle.\"", () => {
