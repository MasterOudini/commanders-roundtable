// `Ajani's Mantra` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AJANIS_MANTRA_SCRIPT } from './ajanisMantra';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ajani's Mantra";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ajani's Mantra"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AJANIS_MANTRA_SCRIPT]),
  });
  holdEverywhere(g);
