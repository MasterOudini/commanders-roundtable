// `Darba` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DARBA_SCRIPT } from './darba';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Darba", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DARBA_SCRIPT]),
  });
  holdEverywhere(g);
