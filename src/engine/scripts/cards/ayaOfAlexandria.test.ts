// `Aya of Alexandria` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AYA_OF_ALEXANDRIA_SCRIPT } from './ayaOfAlexandria';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aya of Alexandria";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aya of Alexandria", "Memnite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AYA_OF_ALEXANDRIA_SCRIPT]),
...
describe("Aya of Alexandria", () => {
  test("Whenever a historic creature you control deals combat damage to a player: 1 token made", () => {
