// `Headhunter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HEADHUNTER_SCRIPT } from './headhunter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Headhunter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Headhunter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HEADHUNTER_SCRIPT]),
  });
  holdEverywhere(g);
