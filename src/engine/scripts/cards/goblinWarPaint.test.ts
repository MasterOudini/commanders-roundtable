// `Goblin War Paint` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2 and has haste;
...
import { GOBLIN_WAR_PAINT_SCRIPT } from './goblinWarPaint';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin War Paint";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOBLIN_WAR_PAINT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GOBLIN_WAR_PAINT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin War Paint", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+2 and has haste; Cyclops of One-Eyed Pass is untouched", () => {
