// `Into the Wilds` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Into the Wilds", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INTO_THE_WILDS_SCRIPT]),
  ...
  if (which === 0) {
    const lookPicksL0_upkeep = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Forest" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL0_upkeep.length, "Forest is dealt for the look").toBeGreaterThan(0);
    for (const id of lookPicksL0_upkeep) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: id, to: { kind: 'library', player: 'p1' }, placement: 'top' }));
...
describe("Into the Wilds", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Look at the top card of your library. If it's a land card, you may put it onto the battlefield.\"", () => {
    const { g } = armed(0);
