// `Sluggishness` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLUGGISHNESS_SCRIPT } from './sluggishness';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sluggishness";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sluggishness", "Grizzly Bears", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLUGGISHNESS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sluggishness", () => {
  test("Enchanted creature can't block.: the creature it is attached to is held back", () => {
