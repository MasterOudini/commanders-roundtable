// `Thopter Shop` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THOPTER_SHOP_SCRIPT } from './thopterShop';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thopter Shop";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thopter Shop", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THOPTER_SHOP_SCRIPT]),
...
describe("Thopter Shop", () => {
  test("Whenever one or more artifact creatures you control die: a card is drawn", () => {
    const { g, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1);
  });

  test("{2}{W}, {T}: 1 token made", () => {
  ...
  test('the once-per-turn rider rides the def (D492)', () => {
    expect(THOPTER_SHOP_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("anotherCreatureDies-0") && t.oncePerTurn === true), 'the def of line 0 carries oncePerTurn').toBeDefined();
  });
