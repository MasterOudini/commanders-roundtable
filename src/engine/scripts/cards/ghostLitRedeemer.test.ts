// `Ghost-Lit Redeemer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHOST_LIT_REDEEMER_SCRIPT } from './ghostLitRedeemer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghost-Lit Redeemer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghost-Lit Redeemer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHOST_LIT_REDEEMER_SCRIPT]),
