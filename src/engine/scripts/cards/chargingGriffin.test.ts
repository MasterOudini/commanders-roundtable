// `Charging Griffin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHARGING_GRIFFIN_SCRIPT } from './chargingGriffin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Charging Griffin";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHARGING_GRIFFIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Charging Griffin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHARGING_GRIFFIN_SCRIPT]),
  });
  holdEverywhere(g);
