// `Atsushi, the Blazing Sky` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ATSUSHI_THE_BLAZING_SKY_SCRIPT } from './atsushiTheBlazingSky';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Atsushi, the Blazing Sky";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Atsushi, the Blazing Sky"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ATSUSHI_THE_BLAZING_SKY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Atsushi, the Blazing Sky", () => {
  test("When Atsushi dies: choosing \"Exile the top two cards of your library. Until the end of your next turn, you may play those cards.\", the vocabulary resolves \"Exile the top two cards of your library. Until the end of your next turn, you may play those cards.\" [dies 1]", () => {
    const { g } = armed(0);
  ...
  test("When Atsushi dies: choosing \"Create three Treasure tokens.\", token [dies 2]", () => {
