// `Firewing Phoenix` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIREWING_PHOENIX_SCRIPT } from './firewingPhoenix';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Firewing Phoenix";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Firewing Phoenix"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIREWING_PHOENIX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Firewing Phoenix", () => {
  test("{1}{R}{R}{R}: it returns from the graveyard to hand", () => {
