// `Core Prowler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CORE_PROWLER_SCRIPT } from './coreProwler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Core Prowler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Core Prowler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CORE_PROWLER_SCRIPT]),
  });
  holdEverywhere(g);
