// `Armor Sliver` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { ARMOR_SLIVER_SCRIPT } from './armorSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Armor Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Armor Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARMOR_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Armor Sliver", () => {
  test("grants {2}: This creature gets +0/+1 until end of turn. to Metallic Sliver, and to nothing outside the scope", () => {
