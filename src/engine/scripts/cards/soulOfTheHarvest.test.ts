// `Soul of the Harvest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOUL_OF_THE_HARVEST_SCRIPT } from './soulOfTheHarvest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Soul of the Harvest";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soul of the Harvest", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOUL_OF_THE_HARVEST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Soul of the Harvest", () => {
  test("Whenever another nontoken creature you control enters: a card is drawn", () => {
