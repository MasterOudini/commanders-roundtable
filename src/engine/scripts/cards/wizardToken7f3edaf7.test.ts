// `Wizard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WIZARD_TOKEN7F3EDAF7_SCRIPT } from './wizardToken7f3edaf7';
...
type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; vtA0_0: InstanceId; payA0_p2_0: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [[], ["Cyclops of One-Eyed Pass", "Divination", "Forest"]],
    scripts: createRegistry([WIZARD_TOKEN7F3EDAF7_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: WIZARD_7F3EDAF7_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === WIZARD_7F3EDAF7_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, vtA0_0, payA0_p2_0 };
}

describe("Wizard", () => {
  test("{1}, Sacrifice this creature: the vocabulary resolves \"Counter target noncreature spell unless its controller pays {1}.\" - the price is declined", () => {
    ...
    expect(g.state.cards[self], 'a token off the battlefield ceases to exist (CR 704.5d)').toBeUndefined();
  });

  test("{1}, Sacrifice this creature: the vocabulary resolves \"Counter target noncreature spell unless its controller pays {1}.\" - the price is paid", () => {
    const { g, self, payA0_p2_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
