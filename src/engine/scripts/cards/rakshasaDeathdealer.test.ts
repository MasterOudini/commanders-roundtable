// `Rakshasa Deathdealer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAKSHASA_DEATHDEALER_SCRIPT } from './rakshasaDeathdealer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rakshasa Deathdealer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RAKSHASA_DEATHDEALER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rakshasa Deathdealer", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAKSHASA_DEATHDEALER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rakshasa Deathdealer", () => {
  test("{B}{G}: it gets +2/+2 until end of turn", () => {
