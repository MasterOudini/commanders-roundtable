// `Endless One` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ENDLESS_ONE_SCRIPT } from './endlessOne';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Endless One";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ENDLESS_ONE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Endless One"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ENDLESS_ONE_SCRIPT]),
  });
  holdEverywhere(g);
