// `Talons of Falkenrath` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { TALONS_OF_FALKENRATH_SCRIPT } from './talonsOfFalkenrath';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Talons of Falkenrath";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Talons of Falkenrath","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TALONS_OF_FALKENRATH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Talons of Falkenrath", () => {
  test("grants {1}{R}: This creature gets +2/+0 until end of turn. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and the body hits the RECIPIENT', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{1}{R}");
