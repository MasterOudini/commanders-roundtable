// `Wormhole Serpent` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WORMHOLE_SERPENT_SCRIPT } from './wormholeSerpent';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wormhole Serpent";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wormhole Serpent"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WORMHOLE_SERPENT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wormhole Serpent", () => {
  test("{3}{U}: the vocabulary resolves \"Target creature can't be blocked this turn.\"", () => {
    const { g, no } = armed(0);
