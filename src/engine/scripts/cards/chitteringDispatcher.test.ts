// `Chittering Dispatcher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHITTERING_DISPATCHER_SCRIPT } from './chitteringDispatcher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chittering Dispatcher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chittering Dispatcher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHITTERING_DISPATCHER_SCRIPT]),
...
describe("Chittering Dispatcher", () => {
  test("When this creature leaves the battlefield: the vocabulary resolves \"Create a 0/1 colorless Eldrazi Spawn creature token with \\\"Sacrifice this token: Add {C}.\\\"\"", () => {
