// `Stromkirk Captain` - the anthem reaches the permanent its scope names and not the other;
// it ends when the source leaves; replay equal (D300). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { STROMKIRK_CAPTAIN_SCRIPT } from './stromkirkCaptain';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stromkirk Captain";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STROMKIRK_CAPTAIN_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Stromkirk Captain", "Driver of the Dead", "Coral Eel"], ["Grizzly Bears"]],
    scripts: createRegistry([STROMKIRK_CAPTAIN_SCRIPT]),
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Driver of the Dead");
...
describe("Stromkirk Captain", () => {
  test("Driver of the Dead is reached, Coral Eel is not", () => {
