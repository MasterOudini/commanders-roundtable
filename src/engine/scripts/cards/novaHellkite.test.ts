// `Nova Hellkite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NOVA_HELLKITE_SCRIPT } from './novaHellkite';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nova Hellkite";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nova Hellkite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NOVA_HELLKITE_SCRIPT]),
...
describe("Nova Hellkite", () => {
  test("When this creature enters: the vocabulary resolves \"~ deals 1 damage to target creature an opponent controls.\"", () => {
    const { g, no } = armed(0);
