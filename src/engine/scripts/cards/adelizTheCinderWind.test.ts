// `Adeliz, the Cinder Wind` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ADELIZ_THE_CINDER_WIND_SCRIPT } from './adelizTheCinderWind';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Adeliz, the Cinder Wind";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ADELIZ_THE_CINDER_WIND_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Adeliz, the Cinder Wind", "Fugitive Wizard", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ADELIZ_THE_CINDER_WIND_SCRIPT]),
...
describe("Adeliz, the Cinder Wind", () => {
  test("Whenever you cast an instant or sorcery spell: Wizard creatures you control get +1/+1 until end of turn", () => {
