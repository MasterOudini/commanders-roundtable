// `Lupinflower Village` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lupinflower Village", "Aven Envoy", "Aven Envoy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LUPINFLOWER_VILLAGE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    ...
    settle(g);
    const lookPicksA2 = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Aven Envoy" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksA2.length, "Aven Envoy is dealt for the look").toBeGreaterThan(0);
...
describe("Lupinflower Village", () => {
  test("{1}{W}, {T}, Sacrifice this land: the vocabulary resolves \"Look at the top six cards of your library. You may reveal a Bat, Bird, Mouse, or Rabbit card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    ...
    { const shown = (g.state.zones.library.p1 ?? []).filter((id) => g.state.cards[id]?.revealedTo.includes('p1'));
      const found = shown.find((id) => nameOf(g, id) === "Aven Envoy");
      expect(found, "Aven Envoy is not among the revealed cards").toBeDefined();
