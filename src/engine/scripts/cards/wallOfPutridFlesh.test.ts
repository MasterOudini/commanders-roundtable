// `Wall of Putrid Flesh` - the continuous prevention effect proven both ways (D385): the damage the line
// covers never arrives and nothing is spent; damage outside the line lands. Generated from one row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { WALL_OF_PUTRID_FLESH_SCRIPT } from './wallOfPutridFlesh';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wall of Putrid Flesh";
...
function armed(): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wall of Putrid Flesh", "Grizzly Bears", "Spark Spray"], ["Cyclops of One-Eyed Pass", "Holy Strength"]],
    scripts: createRegistry([WALL_OF_PUTRID_FLESH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wall of Putrid Flesh", () => {
  test("combat damage from the enchanted attacker is prevented, and nothing is spent", () => {
    const a = armed();
    advanceUntil(a.g, (s) => s.turn.turnNumber === 4 && s.turn.phase === 'precombatMain' && s.priority.player === 'p2' && s.priority.awaiting === null, 40_000);
    // The attacker is ENCHANTED first: a quiet Aura of the opponent's, CAST (D269) onto the Cyclops.
    const hs = put(a.g, 'p2', "Holy Strength", 'hand');
    mana(a.g, 'p2', ['W']);
    must(a.g.submit({ t: 'CastSpell', player: 'p2', card: hs, targets: [{ kind: 'card', id: a.no }] }));
    settle(a.g);
