// `Arbor Elf` - the named target is accepted and it untaps; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ARBOR_ELF_SCRIPT } from './arborElf';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Arbor Elf";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Arbor Elf", "Forest", "Island"], ["Grizzly Bears"]],
    scripts: createRegistry([ARBOR_ELF_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const target = put(g, 'p1', "Forest");
  const wrong = put(g, 'p1', "Island");
  settle(g);
...
describe("Arbor Elf", () => {
  ...
  test("Island (untapped) is refused (D294)", () => {
    const { g, wrong } = armed();
