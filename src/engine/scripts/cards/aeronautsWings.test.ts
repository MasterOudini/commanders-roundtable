// `Aeronaut's Wings` - equipped to Grizzly Bears the creature gets +1/+0 and has flying; the Cyclops is untouched;
...
import { AERONAUTS_WINGS_SCRIPT } from './aeronautsWings';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aeronaut's Wings";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([AERONAUTS_WINGS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([AERONAUTS_WINGS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aeronaut's Wings", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+0 and has flying; the Cyclops is untouched", () => {
