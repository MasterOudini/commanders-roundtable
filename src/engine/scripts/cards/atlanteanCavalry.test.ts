// `Atlantean Cavalry` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ATLANTEAN_CAVALRY_SCRIPT } from './atlanteanCavalry';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Atlantean Cavalry";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Atlantean Cavalry", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ATLANTEAN_CAVALRY_SCRIPT]),
  });
  holdEverywhere(g);
