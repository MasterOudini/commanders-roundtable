// `Drudge Reavers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRUDGE_REAVERS_SCRIPT } from './drudgeReavers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drudge Reavers";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drudge Reavers", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRUDGE_REAVERS_SCRIPT]),
  });
  holdEverywhere(g);
