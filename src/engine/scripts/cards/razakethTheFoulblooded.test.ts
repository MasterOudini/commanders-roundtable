// `Razaketh, the Foulblooded` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAZAKETH_THE_FOULBLOODED_SCRIPT } from './razakethTheFoulblooded';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Razaketh, the Foulblooded";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Razaketh, the Foulblooded", "Grizzly Bears", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAZAKETH_THE_FOULBLOODED_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Razaketh, the Foulblooded", () => {
  test("Pay 2 life, Sacrifice another creature: the vocabulary resolves \"Search your library for a card, put that card into your hand, then shuffle.\"", () => {
    const { g, life0, fodder0 } = armed(0);
