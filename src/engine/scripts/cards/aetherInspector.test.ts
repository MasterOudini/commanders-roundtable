// `Aether Inspector` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AETHER_INSPECTOR_SCRIPT } from './aetherInspector';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aether Inspector";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aether Inspector"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AETHER_INSPECTOR_SCRIPT]),
...
describe("Aether Inspector", () => {
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"You may pay {E}{E}. If you do, create a 1/1 colorless Servo artifact creature token.\" - the price is declined", () => {
    const { g } = armed(1);
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"You may pay {E}{E}. If you do, create a 1/1 colorless Servo artifact creature token.\" - the price is paid", () => {
    const { g, energy0, board0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
