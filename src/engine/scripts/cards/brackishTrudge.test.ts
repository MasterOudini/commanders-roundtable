// `Brackish Trudge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRACKISH_TRUDGE_SCRIPT } from './brackishTrudge';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brackish Trudge";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brackish Trudge"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRACKISH_TRUDGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Brackish Trudge", () => {
  test("{1}{B}: it returns from the graveyard to hand - refused unless if you gained life this turn", () => {
