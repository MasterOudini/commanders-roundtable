// `Strider Harness` - equipped to Grizzly Bears the creature gets +1/+1 and has haste; the Cyclops is untouched;
...
import { STRIDER_HARNESS_SCRIPT } from './striderHarness';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Strider Harness";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STRIDER_HARNESS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([STRIDER_HARNESS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Strider Harness", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+1 and has haste; the Cyclops is untouched", () => {
