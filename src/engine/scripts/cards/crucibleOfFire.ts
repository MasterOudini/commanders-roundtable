// `Crucible of Fire` - a layer-6 anthem: "Dragon creatures you control get +3/+3". A StaticDef in the shape of the
...
import { CRUCIBLE_OF_FIRE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(CRUCIBLE_OF_FIRE, "Dragon creatures you control get +3/+3.");
const TEXT = PRINTED;

export const CRUCIBLE_OF_FIRE_SCRIPT: CardScript = {
  oracleId: CRUCIBLE_OF_FIRE.oracleId,
  name: CRUCIBLE_OF_FIRE.name,
  statics: [
    {
      abilityId: 'anthem',
      text: TEXT,
      ...
      appliesTo: (ctx, self, candidate, chars) => {
        ...
        if (!chars.typeLine.subtypes.includes("Dragon")) return false;
        return true;
      },
      modify: (chars) => {
