// `Abyssal Gatekeeper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ABYSSAL_GATEKEEPER_SCRIPT } from './abyssalGatekeeper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Abyssal Gatekeeper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Abyssal Gatekeeper", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ABYSSAL_GATEKEEPER_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const queueFodderL0_dies = put(g, 'p1', "Grizzly Bears");
  settle(g);
...
describe("Abyssal Gatekeeper", () => {
  test("When this creature dies: the vocabulary resolves \"Each player sacrifices a creature of their choice.\"", () => {
