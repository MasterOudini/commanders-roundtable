// `Aspect of Gorgon` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+3 and has deathtouch;
...
import { ASPECT_OF_GORGON_SCRIPT } from './aspectOfGorgon';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aspect of Gorgon";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ASPECT_OF_GORGON_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ASPECT_OF_GORGON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aspect of Gorgon", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+3 and has deathtouch; Cyclops of One-Eyed Pass is untouched", () => {
