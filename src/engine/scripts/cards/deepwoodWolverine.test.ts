// `Deepwood Wolverine` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEEPWOOD_WOLVERINE_SCRIPT } from './deepwoodWolverine';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deepwood Wolverine";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DEEPWOOD_WOLVERINE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deepwood Wolverine"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEEPWOOD_WOLVERINE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deepwood Wolverine", () => {
  test("Whenever this creature becomes blocked: it gets +2/+0 until end of turn", () => {
