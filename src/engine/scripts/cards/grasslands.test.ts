// `Grasslands` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grasslands", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRASSLANDS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Grasslands", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a Forest or Plains card, put it onto the battlefield, then shuffle.\"", () => {
