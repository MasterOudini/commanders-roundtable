// `Candlelight Vigil` - cast on Grizzly Bears it attaches and the enchanted creature gets +3/+2 and has vigilance;
...
import { CANDLELIGHT_VIGIL_SCRIPT } from './candlelightVigil';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Candlelight Vigil";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CANDLELIGHT_VIGIL_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([CANDLELIGHT_VIGIL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Candlelight Vigil", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +3/+2 and has vigilance; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([5, 4]);
