// `Crossroads Watcher` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CROSSROADS_WATCHER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crossroads Watcher", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CROSSROADS_WATCHER_SCRIPT]),
  });
  holdEverywhere(g);
