// `Ogre Slumlord` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OGRE_SLUMLORD_SCRIPT } from './ogreSlumlord';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ogre Slumlord";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([OGRE_SLUMLORD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ogre Slumlord", "Muck Rats", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OGRE_SLUMLORD_SCRIPT]),
...
describe("Ogre Slumlord", () => {
  test("Whenever another nontoken creature dies: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("Rats you control have deathtouch.: only the Rat reads it", () => {
