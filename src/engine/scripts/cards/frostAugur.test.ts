// `Frost Augur` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frost Augur", "Snow-Covered Forest", "Snow-Covered Forest", "Snow-Covered Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FROST_AUGUR_SCRIPT]),
  ...
  if (which === 0) {
    ...
    settle(g);
    const lookPicksA0 = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Snow-Covered Forest" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksA0.length, "Snow-Covered Forest is dealt for the look").toBeGreaterThan(0);
...
describe("Frost Augur", () => {
  test("{S}, {T}: the vocabulary resolves \"Look at the top card of your library. If it's a snow card, you may reveal it and put it into your hand.\"", () => {
    ...
    { const shown = (g.state.zones.library.p1 ?? []).filter((id) => g.state.cards[id]?.revealedTo.includes('p1'));
      const found = shown.find((id) => nameOf(g, id) === "Snow-Covered Forest");
      expect(found, "Snow-Covered Forest is not among the revealed cards").toBeDefined();
