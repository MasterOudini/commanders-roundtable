// `Drey Keeper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DREY_KEEPER_SCRIPT } from './dreyKeeper';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drey Keeper";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DREY_KEEPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drey Keeper", "Squirrelanoids"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DREY_KEEPER_SCRIPT]),
...
describe("Drey Keeper", () => {
  test("When this creature enters: 2 tokens made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 2 + 1);
  });

  test("{3}{B}: Squirrel creatures you control get +1/+0 and gain menace until end of turn", () => {
