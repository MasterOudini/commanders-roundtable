// `Cleric of Chill Depths` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLERIC_OF_CHILL_DEPTHS_SCRIPT } from './clericOfChillDepths';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cleric of Chill Depths";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cleric of Chill Depths"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLERIC_OF_CHILL_DEPTHS_SCRIPT]),
...
describe("Cleric of Chill Depths", () => {
  test("Whenever this creature blocks a creature: the vocabulary resolves \"Target creature doesn't untap during its controller's next untap step.\"", () => {
    const { g, no } = armed(0);
