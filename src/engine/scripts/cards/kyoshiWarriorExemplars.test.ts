// `Kyoshi Warrior Exemplars` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KYOSHI_WARRIOR_EXEMPLARS_SCRIPT } from './kyoshiWarriorExemplars';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kyoshi Warrior Exemplars";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KYOSHI_WARRIOR_EXEMPLARS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kyoshi Warrior Exemplars", "Coral Eel", "Forest", "Forest", "Forest", "Forest", "Forest", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KYOSHI_WARRIOR_EXEMPLARS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kyoshi Warrior Exemplars", () => {
  test("Whenever this creature attacks: its controller's creatures get +2/+2 until end of turn", () => {
