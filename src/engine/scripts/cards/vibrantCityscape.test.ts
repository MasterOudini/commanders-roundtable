// `Vibrant Cityscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vibrant Cityscape", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIBRANT_CITYSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
