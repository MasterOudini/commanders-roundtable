// `Kyren Glider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KYREN_GLIDER_SCRIPT } from './kyrenGlider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kyren Glider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kyren Glider", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KYREN_GLIDER_SCRIPT]),
  });
  holdEverywhere(g);
