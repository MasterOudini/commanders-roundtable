// `Springjaw Trap` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPRINGJAW_TRAP_SCRIPT } from './springjawTrap';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Springjaw Trap";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Springjaw Trap"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPRINGJAW_TRAP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Springjaw Trap", () => {
  test("{4}, {T}, Sacrifice this artifact: it deals 3 damage to the declared creature", () => {
