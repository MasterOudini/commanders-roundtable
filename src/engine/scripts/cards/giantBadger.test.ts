// `Giant Badger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GIANT_BADGER_SCRIPT } from './giantBadger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Giant Badger";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GIANT_BADGER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Giant Badger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GIANT_BADGER_SCRIPT]),
  });
  holdEverywhere(g);
