// `Myr Propagator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MYR_PROPAGATOR_SCRIPT } from './myrPropagator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Myr Propagator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Myr Propagator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MYR_PROPAGATOR_SCRIPT]),
...
describe("Myr Propagator", () => {
  test("{3}, {T}: the vocabulary resolves \"Create a token that's a copy of this creature.\"", () => {
