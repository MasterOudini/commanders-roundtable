// `Zombie Musher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZOMBIE_MUSHER_SCRIPT } from './zombieMusher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zombie Musher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zombie Musher", "Lightning Bolt", "Snow-Covered Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZOMBIE_MUSHER_SCRIPT]),
  });
  holdEverywhere(g);
