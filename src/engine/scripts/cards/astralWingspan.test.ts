// `Astral Wingspan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASTRAL_WINGSPAN_SCRIPT } from './astralWingspan';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Astral Wingspan";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ASTRAL_WINGSPAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Astral Wingspan", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASTRAL_WINGSPAN_SCRIPT]),
  });
  holdEverywhere(g);
