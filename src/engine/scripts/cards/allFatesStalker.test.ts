// `All-Fates Stalker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ALL_FATES_STALKER_SCRIPT } from './allFatesStalker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "All-Fates Stalker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["All-Fates Stalker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ALL_FATES_STALKER_SCRIPT]),
...
describe("All-Fates Stalker", () => {
  test("When this creature enters: the vocabulary resolves \"Exile up to one target non-Assassin creature until this creature leaves the battlefield.\"", () => {
