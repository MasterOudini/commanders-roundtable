// `Ceremonial Knife` - the carrier proven end to end (D367/D368) with a D384 payload: the recipient
...
import { CEREMONIAL_KNIFE_SCRIPT } from './ceremonialKnife';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ceremonial Knife";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Ceremonial Knife","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CEREMONIAL_KNIFE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ceremonial Knife", () => {
  test("grants Whenever this creature deals combat damage, create a Blood token. to Grizzly Bears, and to nothing outside the scope", () => {
    ...
    expect([d.power, d.toughness]).toEqual([3, 2]);
  });
