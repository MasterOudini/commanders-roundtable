// `Deepfathom Skulker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEEPFATHOM_SKULKER_SCRIPT } from './deepfathomSkulker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deepfathom Skulker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deepfathom Skulker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEEPFATHOM_SKULKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deepfathom Skulker", () => {
  ...
  test("{3}{C}: the vocabulary resolves \"Target creature can't be blocked this turn.\"", () => {
    const { g, no } = armed(1);
