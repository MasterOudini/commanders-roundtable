// `Chocobo Racetrack` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHOCOBO_RACETRACK_SCRIPT } from './chocoboRacetrack';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chocobo Racetrack"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHOCOBO_RACETRACK_SCRIPT, BIRD_TOKEN1FBC471D_SCRIPT]),
...
describe("Chocobo Racetrack", () => {
  test("Landfall — Whenever a land you control enters: the vocabulary resolves \"Create a 2/2 green Bird creature token with \\\"Whenever a land you control enters, this token gets +1/+0 until end of turn.\\\"\"", () => {
