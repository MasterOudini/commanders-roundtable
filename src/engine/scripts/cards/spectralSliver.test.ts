// `Spectral Sliver` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { SPECTRAL_SLIVER_SCRIPT } from './spectralSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spectral Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Spectral Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPECTRAL_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spectral Sliver", () => {
  test("grants {2}: This creature gets +1/+1 until end of turn. to Metallic Sliver, and to nothing outside the scope", () => {
