// `Dreamcaller Siren` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DREAMCALLER_SIREN_SCRIPT } from './dreamcallerSiren';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dreamcaller Siren";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dreamcaller Siren", "Grizzly Bears", "Cloud Pirates"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DREAMCALLER_SIREN_SCRIPT]),
...
describe("Dreamcaller Siren", () => {
  test("This creature can block only creatures with flying.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("When this creature enters: the vocabulary resolves \"Tap up to two target nonland permanents.\"", () => {
    const { g, no } = armed(1);
