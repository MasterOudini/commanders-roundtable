// `Vithian Stinger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VITHIAN_STINGER_SCRIPT } from './vithianStinger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vithian Stinger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vithian Stinger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VITHIAN_STINGER_SCRIPT]),
