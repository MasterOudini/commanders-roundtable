// `Lightning-Core Excavator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIGHTNING_CORE_EXCAVATOR_SCRIPT } from './lightningCoreExcavator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning-Core Excavator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lightning-Core Excavator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIGHTNING_CORE_EXCAVATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lightning-Core Excavator", () => {
  test("{5}, {T}, Sacrifice this creature: it deals 3 damage to the declared creature", () => {
