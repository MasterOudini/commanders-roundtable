// `Norwood Riders` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NORWOOD_RIDERS_SCRIPT } from './norwoodRiders';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Norwood Riders";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Norwood Riders", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([NORWOOD_RIDERS_SCRIPT]),
  });
  holdEverywhere(g);
