// `Town Sentry` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TOWN_SENTRY_SCRIPT } from './townSentry';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Town Sentry";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TOWN_SENTRY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Town Sentry"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TOWN_SENTRY_SCRIPT]),
  });
  holdEverywhere(g);
