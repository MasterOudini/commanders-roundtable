// `Stalwart Aven` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STALWART_AVEN_SCRIPT } from './stalwartAven';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stalwart Aven";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STALWART_AVEN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stalwart Aven"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STALWART_AVEN_SCRIPT]),
  });
  holdEverywhere(g);
