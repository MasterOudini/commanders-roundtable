// `Borderland Ranger` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Borderland Ranger", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BORDERLAND_RANGER_SCRIPT]),
  });
  holdEverywhere(g);
