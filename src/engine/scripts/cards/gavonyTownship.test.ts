// `Gavony Township` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GAVONY_TOWNSHIP_SCRIPT } from './gavonyTownship';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gavony Township";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gavony Township", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GAVONY_TOWNSHIP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gavony Township", () => {
  test("{2}{G}{W}, {T}: each of its controller's creatures gets 1 +1/+1 counter", () => {
