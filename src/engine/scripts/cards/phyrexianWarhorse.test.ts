// `Phyrexian Warhorse` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHYREXIAN_WARHORSE_SCRIPT } from './phyrexianWarhorse';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phyrexian Warhorse";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PHYREXIAN_WARHORSE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phyrexian Warhorse", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHYREXIAN_WARHORSE_SCRIPT]),
  });
  holdEverywhere(g);
