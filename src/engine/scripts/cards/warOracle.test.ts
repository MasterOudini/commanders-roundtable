// `War Oracle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WAR_ORACLE_SCRIPT } from './warOracle';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "War Oracle";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WAR_ORACLE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["War Oracle"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WAR_ORACLE_SCRIPT]),
  });
  holdEverywhere(g);
