// `White Tiger, Amulet Keeper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WHITE_TIGER_AMULET_KEEPER_SCRIPT } from './whiteTigerAmuletKeeper';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "White Tiger, Amulet Keeper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["White Tiger, Amulet Keeper", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WHITE_TIGER_AMULET_KEEPER_SCRIPT]),
...
describe("White Tiger, Amulet Keeper", () => {
  test("{3}{G}, Exile this card from your graveyard: the vocabulary resolves \"Draw a card. You may put a land card from your hand onto the battlefield.\"", () => {
