// `Setessan Training` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: draw 1); gets +1/+0 and has trample;
...
import { SETESSAN_TRAINING_SCRIPT } from './setessanTraining';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Setessan Training";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SETESSAN_TRAINING_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SETESSAN_TRAINING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Setessan Training", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: draw 1); gets +1/+0 and has trample; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    const hand0 = (g.state.zones.hand.p1 ?? []).length;
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([3, 2]);
    expect(kw(g, host).has("trample")).toBe(true);
    expect(pt(g, other)).toEqual([5, 2]);
    expect(kw(g, other).has("trample")).toBe(false);
