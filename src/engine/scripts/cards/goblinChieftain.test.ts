// `Goblin Chieftain` - the anthem reaches the permanent its scope names and not the other;
// it ends when the source leaves; replay equal (D300). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { GOBLIN_CHIEFTAIN_SCRIPT } from './goblinChieftain';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Chieftain";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOBLIN_CHIEFTAIN_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Chieftain", "Beetleback Chief", "Coral Eel"], ["Grizzly Bears"]],
    scripts: createRegistry([GOBLIN_CHIEFTAIN_SCRIPT]),
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Beetleback Chief");
...
describe("Goblin Chieftain", () => {
  test("Beetleback Chief is reached, Coral Eel is not", () => {
