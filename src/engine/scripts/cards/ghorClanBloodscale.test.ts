// `Ghor-Clan Bloodscale` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHOR_CLAN_BLOODSCALE_SCRIPT } from './ghorClanBloodscale';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghor-Clan Bloodscale";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GHOR_CLAN_BLOODSCALE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghor-Clan Bloodscale", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHOR_CLAN_BLOODSCALE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ghor-Clan Bloodscale", () => {
  test("{3}{G}: it gets +2/+2 until end of turn", () => {
