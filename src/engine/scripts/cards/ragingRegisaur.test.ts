// `Raging Regisaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAGING_REGISAUR_SCRIPT } from './ragingRegisaur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Raging Regisaur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Raging Regisaur"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAGING_REGISAUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Raging Regisaur", () => {
  test("Whenever this creature attacks: it deals 1 damage to the declared creature", () => {
    const { g, no } = armed(0);
