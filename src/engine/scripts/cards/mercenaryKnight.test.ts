// `Mercenary Knight` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERCENARY_KNIGHT_SCRIPT } from './mercenaryKnight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mercenary Knight";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mercenary Knight", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERCENARY_KNIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mercenary Knight", () => {
  test("When this creature enters: the vocabulary resolves \"Sacrifice it unless you discard a creature card.\" - the price is declined", () => {
  ...
  test("When this creature enters: the vocabulary resolves \"Sacrifice it unless you discard a creature card.\" - the price is paid", () => {
