// `Zephyr Spirit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZEPHYR_SPIRIT_SCRIPT } from './zephyrSpirit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zephyr Spirit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zephyr Spirit"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZEPHYR_SPIRIT_SCRIPT]),
...
describe("Zephyr Spirit", () => {
  test("When this creature blocks: it returns to its owner's hand", () => {
