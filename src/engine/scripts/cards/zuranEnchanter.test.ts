// `Zuran Enchanter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZURAN_ENCHANTER_SCRIPT } from './zuranEnchanter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zuran Enchanter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zuran Enchanter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZURAN_ENCHANTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zuran Enchanter", () => {
  test("{2}{B}, {T}: the vocabulary resolves \"Target player discards a card.\" - refused unless during your turn", () => {
