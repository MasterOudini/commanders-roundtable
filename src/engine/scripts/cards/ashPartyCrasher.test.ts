// `Ash, Party Crasher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASH_PARTY_CRASHER_SCRIPT } from './ashPartyCrasher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ash, Party Crasher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ash, Party Crasher", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASH_PARTY_CRASHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ash, Party Crasher", () => {
  test("Celebration — Whenever Ash attacks: it gets 1 +1/+1 counter", () => {
