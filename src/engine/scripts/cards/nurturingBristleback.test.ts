// `Nurturing Bristleback` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NURTURING_BRISTLEBACK_SCRIPT } from './nurturingBristleback';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nurturing Bristleback";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nurturing Bristleback"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NURTURING_BRISTLEBACK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Nurturing Bristleback", () => {
