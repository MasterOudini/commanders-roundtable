// `Rootrunner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROOTRUNNER_SCRIPT } from './rootrunner';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rootrunner";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rootrunner"], ["Cyclops of One-Eyed Pass", "Forest"]],
    scripts: createRegistry([ROOTRUNNER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rootrunner", () => {
  test("{G}{G}, Sacrifice this creature: the vocabulary resolves \"Put target land on top of its owner's library.\"", () => {
