// `Wei Ambush Force` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WEI_AMBUSH_FORCE_SCRIPT } from './weiAmbushForce';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wei Ambush Force";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WEI_AMBUSH_FORCE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wei Ambush Force"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WEI_AMBUSH_FORCE_SCRIPT]),
  });
  holdEverywhere(g);
