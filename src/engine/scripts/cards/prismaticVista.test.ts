// `Prismatic Vista` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prismatic Vista", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRISMATIC_VISTA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Prismatic Vista", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield, then shuffle.\"", () => {
