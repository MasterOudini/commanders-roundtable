// `Hystrodon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HYSTRODON_SCRIPT } from './hystrodon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hystrodon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hystrodon"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HYSTRODON_SCRIPT]),
  });
  holdEverywhere(g);
