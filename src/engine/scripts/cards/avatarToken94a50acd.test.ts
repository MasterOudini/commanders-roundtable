// `Avatar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AVATAR_TOKEN94A50ACD_SCRIPT } from './avatarToken94a50acd';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([AVATAR_TOKEN94A50ACD_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: AVATAR_94A50ACD_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === AVATAR_94A50ACD_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
...
describe("Avatar", () => {
  test("Whenever this creature attacks: it deals 3 damage to each opponent", () => {
