// `Haunted Cloak` - equipped to Grizzly Bears the creature has vigilance, trample, haste; the Cyclops is untouched;
...
import { HAUNTED_CLOAK_SCRIPT } from './hauntedCloak';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Haunted Cloak";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([HAUNTED_CLOAK_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([HAUNTED_CLOAK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Haunted Cloak", () => {
  test("equipped to Grizzly Bears: attached, and the creature has vigilance, trample, haste; the Cyclops is untouched", () => {
