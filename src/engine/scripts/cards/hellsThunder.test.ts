// `Hell's Thunder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HELLS_THUNDER_SCRIPT } from './hellsThunder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hell's Thunder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hell's Thunder"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HELLS_THUNDER_SCRIPT]),
