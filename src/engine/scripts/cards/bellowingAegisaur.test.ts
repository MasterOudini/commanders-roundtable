// `Bellowing Aegisaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BELLOWING_AEGISAUR_SCRIPT } from './bellowingAegisaur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bellowing Aegisaur";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; yes: InstanceId; gob: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bellowing Aegisaur", "Coral Eel", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([BELLOWING_AEGISAUR_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, yes, gob };
}

describe("Bellowing Aegisaur", () => {
  test("Enrage — Whenever this creature is dealt damage: each of its controller's creatures gets 1 +1/+1 counter", () => {
