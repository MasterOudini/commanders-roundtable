// `Elvish Handservant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ELVISH_HANDSERVANT_SCRIPT } from './elvishHandservant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Elvish Handservant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elvish Handservant", "Grizzly Bears", "Highland Giant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELVISH_HANDSERVANT_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  const bearsSpell = put(g, 'p1', "Highland Giant", 'hand');
...
describe("Elvish Handservant", () => {
  test("Whenever a player casts a Giant spell: it gets 1 +1/+1 counter", () => {
