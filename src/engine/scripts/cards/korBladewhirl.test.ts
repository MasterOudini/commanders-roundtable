// `Kor Bladewhirl` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KOR_BLADEWHIRL_SCRIPT } from './korBladewhirl';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kor Bladewhirl";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KOR_BLADEWHIRL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kor Bladewhirl", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KOR_BLADEWHIRL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kor Bladewhirl", () => {
  test("Rally — Whenever this creature or another Ally you control enters: its controller's creatures gain firstStrike until end of turn", () => {
