// `Bubble Matrix` - the continuous prevention effect proven both ways (D385): the damage the line
// covers never arrives and nothing is spent; damage outside the line lands. Generated from one row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { BUBBLE_MATRIX_SCRIPT } from './bubbleMatrix';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bubble Matrix";
...
function armed(): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bubble Matrix", "Grizzly Bears", "Spark Spray", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BUBBLE_MATRIX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bubble Matrix", () => {
  ...
  test("a Lightning Bolt at the opponent is outside the line and lands", () => {
    const a = armed();
    const bolt = put(a.g, 'p1', 'Lightning Bolt', 'hand');
    mana(a.g, 'p1', ['R']);
    must(a.g.submit({ t: 'CastSpell', player: 'p1', card: bolt, targets: [{ kind: 'player', id: 'p2' }] }));
    settle(a.g);
    expect(a.g.state.players.p2?.life).toBe(a.p2life0 - 3);
  });
