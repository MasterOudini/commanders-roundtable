// `Honored Knight-Captain` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HONORED_KNIGHT_CAPTAIN_SCRIPT } from './honoredKnightCaptain';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Honored Knight-Captain";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Honored Knight-Captain", "Accorder's Shield"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HONORED_KNIGHT_CAPTAIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Honored Knight-Captain", () => {
  test("When this creature enters: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("{4}{W}{W}, Sacrifice this creature: the vocabulary resolves \"Search your library for an Equipment card, put it onto the battlefield, then shuffle.\"", () => {
