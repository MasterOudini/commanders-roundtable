// `Scorch Rider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCORCH_RIDER_SCRIPT } from './scorchRider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scorch Rider";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SCORCH_RIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scorch Rider"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCORCH_RIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Scorch Rider", () => {
  test("When this creature enters: it gains haste until end of turn", () => {
