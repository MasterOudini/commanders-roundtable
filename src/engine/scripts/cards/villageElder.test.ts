// `Village Elder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VILLAGE_ELDER_SCRIPT } from './villageElder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Village Elder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Village Elder", "Forest", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VILLAGE_ELDER_SCRIPT]),
  });
  holdEverywhere(g);
