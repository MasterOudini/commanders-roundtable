// `Pulse Tracker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PULSE_TRACKER_SCRIPT } from './pulseTracker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pulse Tracker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pulse Tracker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PULSE_TRACKER_SCRIPT]),
  });
  holdEverywhere(g);
