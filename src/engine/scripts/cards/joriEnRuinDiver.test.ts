// `Jori En, Ruin Diver` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JORI_EN_RUIN_DIVER_SCRIPT } from './joriEnRuinDiver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jori En, Ruin Diver";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jori En, Ruin Diver", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JORI_EN_RUIN_DIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Jori En, Ruin Diver", () => {
  test("Whenever you cast your second spell each turn: a card is drawn", () => {
