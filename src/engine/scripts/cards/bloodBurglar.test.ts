// `Blood Burglar` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLOOD_BURGLAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blood Burglar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOOD_BURGLAR_SCRIPT]),
...
describe("Blood Burglar", () => {
  test("`Blood Burglar` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
