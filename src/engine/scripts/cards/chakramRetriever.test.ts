// `Chakram Retriever` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHAKRAM_RETRIEVER_SCRIPT } from './chakramRetriever';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chakram Retriever";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chakram Retriever", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHAKRAM_RETRIEVER_SCRIPT]),
...
describe("Chakram Retriever", () => {
  test("Whenever you cast a spell during your turn: the vocabulary resolves \"Untap target creature.\"", () => {
    const { g, no } = armed(0);
