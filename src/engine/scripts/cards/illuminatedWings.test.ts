// `Illuminated Wings` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ILLUMINATED_WINGS_SCRIPT } from './illuminatedWings';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Illuminated Wings";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([ILLUMINATED_WINGS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Illuminated Wings", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ILLUMINATED_WINGS_SCRIPT]),
  });
  holdEverywhere(g);
