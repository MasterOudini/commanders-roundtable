// `Fiery Islet` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIERY_ISLET_SCRIPT } from './fieryIslet';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fiery Islet";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fiery Islet"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIERY_ISLET_SCRIPT]),
  });
  holdEverywhere(g);
