// `Wandering Musicians` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WANDERING_MUSICIANS_SCRIPT } from './wanderingMusicians';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wandering Musicians";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WANDERING_MUSICIANS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wandering Musicians", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WANDERING_MUSICIANS_SCRIPT]),
  });
  holdEverywhere(g);
