// `Basilisk Collar` - equipped to Grizzly Bears the creature has deathtouch, lifelink; the Cyclops is untouched;
...
import { BASILISK_COLLAR_SCRIPT } from './basiliskCollar';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Basilisk Collar";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([BASILISK_COLLAR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([BASILISK_COLLAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Basilisk Collar", () => {
  test("equipped to Grizzly Bears: attached, and the creature has deathtouch, lifelink; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(kw(g, host).has("deathtouch")).toBe(true);
    expect(kw(g, host).has("lifelink")).toBe(true);
    expect(kw(g, other).has("deathtouch")).toBe(false);
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("deathtouch")).toBe(false);
