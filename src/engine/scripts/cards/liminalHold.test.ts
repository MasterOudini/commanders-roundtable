// `Liminal Hold` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIMINAL_HOLD_SCRIPT } from './liminalHold';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Liminal Hold";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Liminal Hold"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIMINAL_HOLD_SCRIPT]),
  });
  holdEverywhere(g);
