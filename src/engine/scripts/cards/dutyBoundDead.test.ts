// `Duty-Bound Dead` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DUTY_BOUND_DEAD_SCRIPT } from './dutyBoundDead';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Duty-Bound Dead";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Duty-Bound Dead", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUTY_BOUND_DEAD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Duty-Bound Dead", () => {
  test("{3}{B}: it regenerates from the Bolt", () => {
