// `Zuo Ci, the Mocking Sage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZUO_CI_THE_MOCKING_SAGE_SCRIPT } from './zuoCiTheMockingSage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zuo Ci, the Mocking Sage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zuo Ci, the Mocking Sage", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Wei Scout"]],
    scripts: createRegistry([ZUO_CI_THE_MOCKING_SAGE_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const gift2 = put(g, 'p2', "Wei Scout");
  settle(g);
...
describe("Zuo Ci, the Mocking Sage", () => {
  test("Zuo Ci can't be blocked by creatures with horsemanship.: the block by creatures with horsemanship is refused", () => {
