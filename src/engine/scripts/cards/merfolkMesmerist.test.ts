// `Merfolk Mesmerist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERFOLK_MESMERIST_SCRIPT } from './merfolkMesmerist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Merfolk Mesmerist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merfolk Mesmerist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERFOLK_MESMERIST_SCRIPT]),
  });
  holdEverywhere(g);
