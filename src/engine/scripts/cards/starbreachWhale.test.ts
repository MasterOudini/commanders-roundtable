// `Starbreach Whale` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STARBREACH_WHALE_SCRIPT } from './starbreachWhale';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Starbreach Whale";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Starbreach Whale"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STARBREACH_WHALE_SCRIPT]),
