// `Escape Tunnel` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Escape Tunnel", "Forest"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([ESCAPE_TUNNEL_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtA1_0 = put(g, 'p2', "Grizzly Bears");
  settle(g);
...
describe("Escape Tunnel", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
  ...
  test("{T}, Sacrifice this land: the vocabulary resolves \"Target creature with power 2 or less can't be blocked this turn.\"", () => {
    const { g, self, vtA1_0 } = armed(1);
    if (g.state.cards[vtA1_0]?.zone.kind === 'battlefield') expect(g.state.untilEndOfTurn.some((m) => m.card === vtA1_0 && m.cantBeBlocked === true), 'cannot be blocked this turn').toBe(true);
    expect(g.state.cards[self]?.zone.kind).toBe('graveyard');
  });
