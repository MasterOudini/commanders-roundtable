// `Yellow Scarves General` - every printed ability proven in its own game: the cost's mark, the pump
...
import { YELLOW_SCARVES_GENERAL_SCRIPT } from './yellowScarvesGeneral';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Yellow Scarves General";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Yellow Scarves General", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YELLOW_SCARVES_GENERAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Yellow Scarves General", () => {
