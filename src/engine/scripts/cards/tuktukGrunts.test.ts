// `Tuktuk Grunts` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TUKTUK_GRUNTS_SCRIPT } from './tuktukGrunts';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tuktuk Grunts";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tuktuk Grunts"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TUKTUK_GRUNTS_SCRIPT]),
  });
  holdEverywhere(g);
