// `Sure-Footed Infiltrator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SURE_FOOTED_INFILTRATOR_SCRIPT } from './sureFootedInfiltrator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sure-Footed Infiltrator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sure-Footed Infiltrator", "Triton Shorethief"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SURE_FOOTED_INFILTRATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sure-Footed Infiltrator", () => {
  test("Tap another untapped Rogue you control: the vocabulary resolves \"~ can't be blocked this turn.\"", () => {
    const { g, tapper0 } = armed(0);
