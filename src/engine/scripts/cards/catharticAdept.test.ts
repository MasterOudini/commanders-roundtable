// `Cathartic Adept` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CATHARTIC_ADEPT_SCRIPT } from './catharticAdept';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cathartic Adept";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cathartic Adept"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CATHARTIC_ADEPT_SCRIPT]),
  });
  holdEverywhere(g);
