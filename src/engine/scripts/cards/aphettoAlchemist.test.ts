// `Aphetto Alchemist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { APHETTO_ALCHEMIST_SCRIPT } from './aphettoAlchemist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aphetto Alchemist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aphetto Alchemist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([APHETTO_ALCHEMIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aphetto Alchemist", () => {
  test("{T}: the vocabulary resolves \"Untap target artifact or creature.\"", () => {
