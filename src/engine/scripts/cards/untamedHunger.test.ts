// `Untamed Hunger` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+1 and has menace;
...
import { UNTAMED_HUNGER_SCRIPT } from './untamedHunger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Untamed Hunger";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([UNTAMED_HUNGER_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([UNTAMED_HUNGER_SCRIPT]),
  });
  holdEverywhere(g);
