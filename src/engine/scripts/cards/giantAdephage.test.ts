// `Giant Adephage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GIANT_ADEPHAGE_SCRIPT } from './giantAdephage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Giant Adephage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Giant Adephage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GIANT_ADEPHAGE_SCRIPT]),
