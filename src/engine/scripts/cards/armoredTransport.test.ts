// `Armored Transport` - the continuous prevention effect proven both ways (D385): the damage the line
// covers never arrives and nothing is spent; damage outside the line lands. Generated from one row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { ARMORED_TRANSPORT_SCRIPT } from './armoredTransport';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Armored Transport";
...
function armed(): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Armored Transport", "Grizzly Bears", "Spark Spray"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARMORED_TRANSPORT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Armored Transport", () => {
  test("combat damage from the blocking Cyclops is prevented, and nothing is spent", () => {
    ...
    must(a.g.submit({ t: 'DeclareAttackers', player: 'p1', attackers: [{ card: a.self, defender: { kind: 'player', id: 'p2' } }] }));
    advanceUntil(a.g, (s) => s.priority.awaiting?.kind === 'declareBlockers', 20_000);
    must(a.g.submit({ t: 'DeclareBlockers', player: 'p2', blocks: [{ blocker: a.no, attacker: a.self }] }));
