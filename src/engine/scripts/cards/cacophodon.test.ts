// `Cacophodon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CACOPHODON_SCRIPT } from './cacophodon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cacophodon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cacophodon", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([CACOPHODON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cacophodon", () => {
  test("Enrage — Whenever this creature is dealt damage: the vocabulary resolves \"Untap target permanent.\"", () => {
    const { g, no } = armed(0);
