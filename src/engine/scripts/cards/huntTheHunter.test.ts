// `Hunt the Hunter` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { HUNT_THE_HUNTER_SCRIPT } from './huntTheHunter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hunt the Hunter";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Hunt the Hunter", "Grizzly Bears"], ["Grizzly Bears", "Vampire Nighthawk"]],
    scripts: createRegistry([HUNT_THE_HUNTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hunt the Hunter", () => {
  test('the named targets are accepted and the effect lands', () => {
    ...
    settle(g);
    expect(g.state.cards[targets[1] as InstanceId]?.zone.kind).toBe('graveyard');
    expect(g.state.cards[targets[0] as InstanceId]?.zone.kind).toBe('battlefield');
  });

  test("Vampire Nighthawk is refused where the clause excludes it (D294)", () => {
