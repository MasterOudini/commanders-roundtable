// `Gorgon Flail` - equipped to Grizzly Bears the creature gets +1/+1 and has deathtouch; the Cyclops is untouched;
...
import { GORGON_FLAIL_SCRIPT } from './gorgonFlail';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gorgon Flail";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GORGON_FLAIL_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GORGON_FLAIL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gorgon Flail", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+1 and has deathtouch; the Cyclops is untouched", () => {
