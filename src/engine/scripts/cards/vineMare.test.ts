// `Vine Mare` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VINE_MARE_SCRIPT } from './vineMare';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vine Mare";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vine Mare", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Muck Rats"]],
    scripts: createRegistry([VINE_MARE_SCRIPT]),
