// `Shu Farmer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHU_FARMER_SCRIPT } from './shuFarmer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shu Farmer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shu Farmer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHU_FARMER_SCRIPT]),
  });
  holdEverywhere(g);
