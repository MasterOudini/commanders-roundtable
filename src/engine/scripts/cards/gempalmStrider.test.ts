// `Gempalm Strider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GEMPALM_STRIDER_SCRIPT } from './gempalmStrider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gempalm Strider";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([GEMPALM_STRIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gempalm Strider", "Willow Elf"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GEMPALM_STRIDER_SCRIPT]),
...
describe("Gempalm Strider", () => {
  test("When you cycle this card: Elf creatures get +2/+2 until end of turn", () => {
