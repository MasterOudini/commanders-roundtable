// `Lumengrid Sentinel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LUMENGRID_SENTINEL_SCRIPT } from './lumengridSentinel';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lumengrid Sentinel";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lumengrid Sentinel", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LUMENGRID_SENTINEL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lumengrid Sentinel", () => {
  test("Whenever an artifact you control enters: the vocabulary resolves \"Tap target permanent.\"", () => {
    const { g, no } = armed(0);
