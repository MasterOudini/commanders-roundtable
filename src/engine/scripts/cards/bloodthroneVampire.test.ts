// `Bloodthrone Vampire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODTHRONE_VAMPIRE_SCRIPT } from './bloodthroneVampire';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodthrone Vampire";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLOODTHRONE_VAMPIRE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodthrone Vampire", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODTHRONE_VAMPIRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bloodthrone Vampire", () => {
  test("Sacrifice a creature: it gets +2/+2 until end of turn", () => {
