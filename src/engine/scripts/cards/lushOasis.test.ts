// `Lush Oasis` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LUSH_OASIS_SCRIPT } from './lushOasis';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lush Oasis";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lush Oasis"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LUSH_OASIS_SCRIPT]),
  });
  holdEverywhere(g);
