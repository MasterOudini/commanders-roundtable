// `Spider-Byte, Web Warden` - every declared pick is accepted and it returns to hand; a permanent the clause
// excludes is refused (D299). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SPIDER_BYTE_WEB_WARDEN_SCRIPT } from './spiderByteWebWarden';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spider-Byte, Web Warden";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Spider-Byte, Web Warden"], ["Grizzly Bears", "Forest"]],
    scripts: createRegistry([SPIDER_BYTE_WEB_WARDEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spider-Byte, Web Warden", () => {
