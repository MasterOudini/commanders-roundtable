// `Seshiro the Anointed` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SESHIRO_THE_ANOINTED_SCRIPT } from './seshiroTheAnointed';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Seshiro the Anointed";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SESHIRO_THE_ANOINTED_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seshiro the Anointed", "Skeletal Snake", "Grizzly Bears", "Skeletal Snake"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SESHIRO_THE_ANOINTED_SCRIPT]),
  ...
  const stillFix0 = put(g, 'p1', "Skeletal Snake");
  settle(g);
...
describe("Seshiro the Anointed", () => {
  test("Other Snake creatures you control get +2/+2.: only the Snake reads it", () => {
    const { g, no, yes } = armed(0);
    expect(pt(g, yes)).toEqual([4, 3]);
    expect(pt(g, no)).toEqual([5, 2]);
  });

  test("Whenever a Snake you control deals combat damage to a player: a card is drawn", () => {
