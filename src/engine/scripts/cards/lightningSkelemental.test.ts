// `Lightning Skelemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIGHTNING_SKELEMENTAL_SCRIPT } from './lightningSkelemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning Skelemental";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lightning Skelemental"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIGHTNING_SKELEMENTAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lightning Skelemental", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Target player discards two cards.\"", () => {
    const { g } = armed(0);
  ...
  test("At the beginning of the end step: it is sacrificed", () => {
