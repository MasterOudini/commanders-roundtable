// `Knight Luminary` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KNIGHT_LUMINARY_SCRIPT } from './knightLuminary';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Knight Luminary";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Knight Luminary"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KNIGHT_LUMINARY_SCRIPT]),
