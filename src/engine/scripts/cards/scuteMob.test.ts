// `Scute Mob` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCUTE_MOB_SCRIPT } from './scuteMob';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scute Mob";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scute Mob", "Forest", "Forest", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCUTE_MOB_SCRIPT]),
...
describe("Scute Mob", () => {
  test("At the beginning of your upkeep: it gets 4 +1/+1 counters", () => {
