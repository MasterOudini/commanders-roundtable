// `Spike Colony` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIKE_COLONY_SCRIPT } from './spikeColony';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spike Colony";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPIKE_COLONY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spike Colony"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPIKE_COLONY_SCRIPT]),
  });
  holdEverywhere(g);
