// `Staff of Domination` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STAFF_OF_DOMINATION_SCRIPT } from './staffOfDomination';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Staff of Domination";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Staff of Domination", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STAFF_OF_DOMINATION_SCRIPT]),
  });
  holdEverywhere(g);
