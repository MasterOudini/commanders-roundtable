// `Thicket Basilisk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THICKET_BASILISK_SCRIPT } from './thicketBasilisk';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thicket Basilisk";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thicket Basilisk"], ["Cyclops of One-Eyed Pass", "Staunch Shieldmate"]],
    scripts: createRegistry([THICKET_BASILISK_SCRIPT]),
...
describe("Thicket Basilisk", () => {
  test("Whenever this creature blocks or becomes blocked by a non-Wall creature: the vocabulary resolves \"Destroy target creature at end of combat.\"", () => {
