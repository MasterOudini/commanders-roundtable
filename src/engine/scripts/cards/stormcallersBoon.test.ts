// `Stormcaller's Boon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORMCALLERS_BOON_SCRIPT } from './stormcallersBoon';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stormcaller's Boon";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STORMCALLERS_BOON_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stormcaller's Boon", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORMCALLERS_BOON_SCRIPT]),
...
describe("Stormcaller's Boon", () => {
  test("Sacrifice this enchantment: its controller's creatures gain flying until end of turn", () => {
