// `Ridgescale Tusker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIDGESCALE_TUSKER_SCRIPT } from './ridgescaleTusker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ridgescale Tusker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ridgescale Tusker", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIDGESCALE_TUSKER_SCRIPT]),
  });
  holdEverywhere(g);
