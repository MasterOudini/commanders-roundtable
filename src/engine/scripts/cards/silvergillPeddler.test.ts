// `Silvergill Peddler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SILVERGILL_PEDDLER_SCRIPT } from './silvergillPeddler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Silvergill Peddler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Silvergill Peddler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SILVERGILL_PEDDLER_SCRIPT]),
  });
  holdEverywhere(g);
