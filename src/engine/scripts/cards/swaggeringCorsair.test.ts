// `Swaggering Corsair` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SWAGGERING_CORSAIR_SCRIPT } from './swaggeringCorsair';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Swaggering Corsair";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SWAGGERING_CORSAIR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Swaggering Corsair", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SWAGGERING_CORSAIR_SCRIPT]),
  });
  holdEverywhere(g);
