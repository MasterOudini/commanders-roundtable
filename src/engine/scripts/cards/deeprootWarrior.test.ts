// `Deeproot Warrior` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEEPROOT_WARRIOR_SCRIPT } from './deeprootWarrior';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deeproot Warrior";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DEEPROOT_WARRIOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deeproot Warrior"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEEPROOT_WARRIOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deeproot Warrior", () => {
  test("Whenever this creature becomes blocked: it gets +1/+1 until end of turn", () => {
