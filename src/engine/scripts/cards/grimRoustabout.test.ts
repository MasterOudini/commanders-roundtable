// `Grim Roustabout` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRIM_ROUSTABOUT_SCRIPT } from './grimRoustabout';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grim Roustabout";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grim Roustabout", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRIM_ROUSTABOUT_SCRIPT]),
