// `Bant Panorama` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bant Panorama", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BANT_PANORAMA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bant Panorama", () => {
  test("{1}, {T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Forest, Plains, or Island card, put it onto the battlefield tapped, then shuffle.\"", () => {
