// `Viashino Branchrider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VIASHINO_BRANCHRIDER_SCRIPT } from './viashinoBranchrider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Viashino Branchrider";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VIASHINO_BRANCHRIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Viashino Branchrider"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIASHINO_BRANCHRIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Viashino Branchrider", () => {
  test("If this creature was kicked, it enters with two +1/+1 counters on it.: cast, it enters with 2 +1/+1 counters", () => {
  ...
  test("{2}{R}: it gets +2/+0 until end of turn", () => {
