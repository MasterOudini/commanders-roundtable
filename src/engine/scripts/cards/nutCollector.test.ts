// `Nut Collector` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NUT_COLLECTOR_SCRIPT } from './nutCollector';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nut Collector";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NUT_COLLECTOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nut Collector", "Squirrelanoids"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NUT_COLLECTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Nut Collector", () => {
  test("At the beginning of your upkeep: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("Threshold — All Squirrels get +2/+2 as long as there are seven or more cards in your graveyard.: only the Squirrel reads it", () => {
