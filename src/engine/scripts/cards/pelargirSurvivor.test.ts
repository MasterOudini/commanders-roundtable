// `Pelargir Survivor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PELARGIR_SURVIVOR_SCRIPT } from './pelargirSurvivor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pelargir Survivor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pelargir Survivor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PELARGIR_SURVIVOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pelargir Survivor", () => {
  test("{5}{U}, {T}: the vocabulary resolves \"Target player mills three cards.\"", () => {
