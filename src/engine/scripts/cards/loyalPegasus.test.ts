// `Loyal Pegasus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOYAL_PEGASUS_SCRIPT } from './loyalPegasus';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Loyal Pegasus";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Loyal Pegasus", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOYAL_PEGASUS_SCRIPT]),
  });
  holdEverywhere(g);
