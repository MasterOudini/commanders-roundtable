// `Putrid Imp` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PUTRID_IMP_SCRIPT } from './putridImp';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Putrid Imp";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PUTRID_IMP_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Putrid Imp", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PUTRID_IMP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Putrid Imp", () => {
  test("Discard a card: it gains flying until end of turn", () => {
  ...
  test("Threshold — As long as there are seven or more cards in your graveyard, this creature gets +1/+1 and can't block.: with seven cards in the graveyard it reads the pump", () => {
    const { g, self } = armed(1);
    expect(pt(g, self)).toEqual([2, 2]);
  });

  test("Threshold — As long as there are seven or more cards in your graveyard, this creature gets +1/+1 and can't block.: the block is refused", () => {
