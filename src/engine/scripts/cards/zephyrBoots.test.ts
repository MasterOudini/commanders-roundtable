// `Zephyr Boots` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZEPHYR_BOOTS_SCRIPT } from './zephyrBoots';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zephyr Boots";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([ZEPHYR_BOOTS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zephyr Boots", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZEPHYR_BOOTS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zephyr Boots", () => {
  ...
  test("Whenever equipped creature deals combat damage to a player: a card is drawn, then one discarded", () => {
