// `Bog Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BOG_ELEMENTAL_SCRIPT } from './bogElemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bog Elemental";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bog Elemental", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOG_ELEMENTAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bog Elemental", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you sacrifice a land.\" - the price is declined", () => {
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you sacrifice a land.\" - the price is paid", () => {
    const { g, self, board0, priceL1_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
