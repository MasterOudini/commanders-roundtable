// `Tuktuk Rubblefort` - the grant reaches the permanent its scope names and not the other;
// it ends when the source leaves; replay equal (D300). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { TUKTUK_RUBBLEFORT_SCRIPT } from './tuktukRubblefort';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tuktuk Rubblefort";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([TUKTUK_RUBBLEFORT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Tuktuk Rubblefort", "Coral Eel"], ["Crimson Kobolds"]],
    scripts: createRegistry([TUKTUK_RUBBLEFORT_SCRIPT]),
  });
  holdEverywhere(g);
