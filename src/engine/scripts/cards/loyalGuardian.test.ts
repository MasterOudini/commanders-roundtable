// `Loyal Guardian` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOYAL_GUARDIAN_SCRIPT } from './loyalGuardian';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Loyal Guardian";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Loyal Guardian", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOYAL_GUARDIAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Loyal Guardian", () => {
  test("Lieutenant — At the beginning of combat on your turn: each of its controller's creatures gets 1 +1/+1 counter", () => {
