// `Feast of Sanity` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FEAST_OF_SANITY_SCRIPT } from './feastOfSanity';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Feast of Sanity";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Feast of Sanity"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FEAST_OF_SANITY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Feast of Sanity", () => {
  test("Whenever you discard a card: the vocabulary resolves \"This enchantment deals 1 damage to any target and you gain 1 life.\"", () => {
