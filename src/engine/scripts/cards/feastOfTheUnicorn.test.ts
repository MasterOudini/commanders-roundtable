// `Feast of the Unicorn` - cast on Grizzly Bears it attaches and the enchanted creature gets +4/+0;
...
import { FEAST_OF_THE_UNICORN_SCRIPT } from './feastOfTheUnicorn';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Feast of the Unicorn";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FEAST_OF_THE_UNICORN_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FEAST_OF_THE_UNICORN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Feast of the Unicorn", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +4/+0; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([6, 2]);
