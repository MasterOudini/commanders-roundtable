// `Welcoming Vampire` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Welcoming Vampire", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WELCOMING_VAMPIRE_SCRIPT]),
...
describe("Welcoming Vampire", () => {
  test("Whenever one or more other creatures you control with power 2 or less enter: a card is drawn", () => {
    const { g, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1);
  });

  test('the once-per-turn rider rides the def (D492)', () => {
    expect(WELCOMING_VAMPIRE_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("anotherCreatureEnters-1") && t.oncePerTurn === true), 'the def of line 1 carries oncePerTurn').toBeDefined();
  });
