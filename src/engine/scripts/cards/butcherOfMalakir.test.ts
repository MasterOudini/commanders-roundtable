// `Butcher of Malakir` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BUTCHER_OF_MALAKIR_SCRIPT } from './butcherOfMalakir';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Butcher of Malakir";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Butcher of Malakir", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BUTCHER_OF_MALAKIR_SCRIPT]),
...
describe("Butcher of Malakir", () => {
  test("Whenever this creature or another creature you control dies: the vocabulary resolves \"Each opponent sacrifices a creature of their choice.\" [dies]", () => {
    const { g } = armed(0);
  ...
  test("Whenever this creature or another creature you control dies: the vocabulary resolves \"Each opponent sacrifices a creature of their choice.\" [anotherCreatureDies]", () => {
    const { g } = armed(1);
    for (let qi = 0; qi < 4; qi++) {
      advanceUntil(g, (s) => s.priority.awaiting?.kind === 'chooseFromZone' || (s.stack.length === 0 && s.pendingTriggers.length === 0 && s.pendingAsks === null && s.priority.awaiting === null), 20_000);
      const qa = g.state.priority.awaiting;
