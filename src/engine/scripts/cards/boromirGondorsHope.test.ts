// `Boromir, Gondor's Hope` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Boromir, Gondor's Hope", "Sol Ring", "Sol Ring", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOROMIR_GONDORS_HOPE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 1) {
    const lookPicksL0_attacks = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Sol Ring" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL0_attacks.length, "Sol Ring is dealt for the look").toBeGreaterThan(0);
...
describe("Boromir, Gondor's Hope", () => {
  test("Whenever Boromir enters or attacks: the vocabulary resolves \"Look at the top six cards of your library. You may reveal a Human or artifact card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\" [etb]", () => {
    const { g } = armed(0);
  ...
  test("Whenever Boromir enters or attacks: the vocabulary resolves \"Look at the top six cards of your library. You may reveal a Human or artifact card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\" [attacks]", () => {
    const { g } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'chooseFromZone', 20_000);
    { const shown = (g.state.zones.library.p1 ?? []).filter((id) => g.state.cards[id]?.revealedTo.includes('p1'));
      const found = shown.find((id) => nameOf(g, id) === "Sol Ring");
