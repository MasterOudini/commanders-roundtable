// `Prophetic Flamespeaker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PROPHETIC_FLAMESPEAKER_SCRIPT } from './propheticFlamespeaker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prophetic Flamespeaker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prophetic Flamespeaker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PROPHETIC_FLAMESPEAKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Prophetic Flamespeaker", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Exile the top card of your library. You may play it this turn.\"", () => {
    const { g } = armed(0);
