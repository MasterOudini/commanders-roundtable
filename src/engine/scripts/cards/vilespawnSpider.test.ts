// `Vilespawn Spider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VILESPAWN_SPIDER_SCRIPT } from './vilespawnSpider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vilespawn Spider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vilespawn Spider", "Hill Giant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VILESPAWN_SPIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vilespawn Spider", () => {
  test("At the beginning of your upkeep: 1 card milled", () => {
    const { g, gy0 } = armed(0);
    expect((g.state.zones.graveyard.p1 ?? []).length).toBe(gy0 + 1);
  });

  test("{2}{G}{U}, {T}, Sacrifice this creature: the vocabulary resolves \"Create a 1/1 green Insect creature token for each creature card in your graveyard.\"", () => {
