// `Azorius Knight-Arbiter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AZORIUS_KNIGHT_ARBITER_SCRIPT } from './azoriusKnightArbiter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Azorius Knight-Arbiter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Azorius Knight-Arbiter", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AZORIUS_KNIGHT_ARBITER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Azorius Knight-Arbiter", () => {
