// `Castle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CASTLE_SCRIPT } from './castle';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CASTLE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Castle", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CASTLE_SCRIPT]),
  });
  holdEverywhere(g);
