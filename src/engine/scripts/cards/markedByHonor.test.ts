// `Marked by Honor` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2 and has vigilance;
...
import { MARKED_BY_HONOR_SCRIPT } from './markedByHonor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Marked by Honor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MARKED_BY_HONOR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([MARKED_BY_HONOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Marked by Honor", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+2 and has vigilance; Cyclops of One-Eyed Pass is untouched", () => {
