// `Vashta Nerada` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VASHTA_NERADA_SCRIPT } from './vashtaNerada';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vashta Nerada";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vashta Nerada", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VASHTA_NERADA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vashta Nerada", () => {
  test("Morbid — At the beginning of each end step: it gets 1 +1/+1 counter", () => {
