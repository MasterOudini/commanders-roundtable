// `Pyric Salamander` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PYRIC_SALAMANDER_SCRIPT } from './pyricSalamander';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pyric Salamander";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pyric Salamander"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PYRIC_SALAMANDER_SCRIPT]),
...
describe("Pyric Salamander", () => {
  test("{R}: the vocabulary resolves \"~ gets +1/+0 until end of turn. Sacrifice this creature at the beginning of the next end step.\"", () => {
