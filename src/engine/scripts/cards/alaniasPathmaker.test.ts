// `Alania's Pathmaker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ALANIAS_PATHMAKER_SCRIPT } from './alaniasPathmaker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Alania's Pathmaker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Alania's Pathmaker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ALANIAS_PATHMAKER_SCRIPT]),
  });
  holdEverywhere(g);
