// `Commanding Presence` - the granted TRIGGER proven end to end (D368): the recipient HAS the
...
import { COMMANDING_PRESENCE_SCRIPT } from './commandingPresence';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Commanding Presence";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Commanding Presence","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COMMANDING_PRESENCE_SCRIPT]),
  });
  holdEverywhere(g);
