// `The Fire Crystal` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THE_FIRE_CRYSTAL_SCRIPT } from './theFireCrystal';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "The Fire Crystal";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; yes: InstanceId; tok0: readonly InstanceId[]; vtA0_0: InstanceId };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([THE_FIRE_CRYSTAL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["The Fire Crystal", "Coral Eel", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THE_FIRE_CRYSTAL_SCRIPT]),
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, yes, tok0, vtA0_0 };
}

describe("The Fire Crystal", () => {
  test("Creatures you control have haste.: its controller's creatures read it", () => {
  ...
  test("{4}{R}{R}, {T}: the vocabulary resolves \"Create a token that's a copy of target creature you control. Sacrifice it at the beginning of the next end step.\"", () => {
