// `Stoic Champion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STOIC_CHAMPION_SCRIPT } from './stoicChampion';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stoic Champion";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([STOIC_CHAMPION_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stoic Champion"], ["Cyclops of One-Eyed Pass", "Lonely Sandbar"]],
    scripts: createRegistry([STOIC_CHAMPION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stoic Champion", () => {
  test("Whenever a player cycles a card: it gets +2/+2 until end of turn", () => {
