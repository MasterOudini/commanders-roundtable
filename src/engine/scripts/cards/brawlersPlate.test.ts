// `Brawler's Plate` - equipped to Grizzly Bears the creature gets +2/+2 and has trample; the Cyclops is untouched;
...
import { BRAWLERS_PLATE_SCRIPT } from './brawlersPlate';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brawler's Plate";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BRAWLERS_PLATE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([BRAWLERS_PLATE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Brawler's Plate", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +2/+2 and has trample; the Cyclops is untouched", () => {
