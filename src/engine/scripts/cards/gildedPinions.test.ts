// `Gilded Pinions` - equipped to Grizzly Bears the creature (on entering: 1 token); has flying; the Cyclops is untouched;
...
import { GILDED_PINIONS_SCRIPT } from './gildedPinions';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gilded Pinions";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([GILDED_PINIONS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GILDED_PINIONS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gilded Pinions", () => {
  test("equipped to Grizzly Bears: attached, and the creature (on entering: 1 token); has flying; the Cyclops is untouched", () => {
  ...
  test("entering: (on entering: 1 token)", () => {
    const g = startedGame({ players: 2, decks: [[CARD, "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]], scripts: createRegistry([GILDED_PINIONS_SCRIPT]) });
    holdEverywhere(g);
