// `Goldwarden's Helm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOLDWARDENS_HELM_SCRIPT } from './goldwardensHelm';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goldwarden's Helm";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOLDWARDENS_HELM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goldwarden's Helm", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOLDWARDENS_HELM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goldwarden's Helm", () => {
  test("For Mirrodin! (When this Equipment enters: a token is made and it attaches to it", () => {
    ...
    if (germ !== null) expect(pt(g, germ)).toEqual([2, 3]);
  });

  test("Equipped creature gets +0/+1.: the creature it is attached to reads it", () => {
