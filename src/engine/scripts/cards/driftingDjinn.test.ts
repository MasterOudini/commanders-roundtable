// `Drifting Djinn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRIFTING_DJINN_SCRIPT } from './driftingDjinn';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drifting Djinn";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drifting Djinn", "Island", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRIFTING_DJINN_SCRIPT]),
  });
  holdEverywhere(g);
