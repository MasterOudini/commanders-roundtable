// `Loyal Subordinate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOYAL_SUBORDINATE_SCRIPT } from './loyalSubordinate';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Loyal Subordinate";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Loyal Subordinate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOYAL_SUBORDINATE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Loyal Subordinate", () => {
  test("Lieutenant — At the beginning of combat on your turn: each opponent loses 3 life", () => {
