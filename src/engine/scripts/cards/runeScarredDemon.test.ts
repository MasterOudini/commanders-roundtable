// `Rune-Scarred Demon` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rune-Scarred Demon", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUNE_SCARRED_DEMON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rune-Scarred Demon", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a card, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
