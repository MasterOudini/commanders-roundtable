// `Deathmark Prelate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEATHMARK_PRELATE_SCRIPT } from './deathmarkPrelate';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deathmark Prelate";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deathmark Prelate", "Walking Corpse"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEATHMARK_PRELATE_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = put(g, 'p1', "Walking Corpse");
  settle(g);
...
describe("Deathmark Prelate", () => {
  test("{2}{B}, {T}, Sacrifice a Zombie: the vocabulary resolves \"Destroy target non-Zombie creature. It can't be regenerated.\"", () => {
