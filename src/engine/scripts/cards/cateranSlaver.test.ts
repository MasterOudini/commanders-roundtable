// `Cateran Slaver` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cateran Slaver", "Rampart Crawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CATERAN_SLAVER_SCRIPT]),
...
describe("Cateran Slaver", () => {
  test("{5}, {T}: the vocabulary resolves \"Search your library for a Mercenary permanent card with mana value 5 or less, put it onto the battlefield, then shuffle.\"", () => {
