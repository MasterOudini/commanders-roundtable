// `Malach of the Dawn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MALACH_OF_THE_DAWN_SCRIPT } from './malachOfTheDawn';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Malach of the Dawn";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Malach of the Dawn", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MALACH_OF_THE_DAWN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Malach of the Dawn", () => {
  test("{W}{W}{W}: it regenerates from the Bolt", () => {
