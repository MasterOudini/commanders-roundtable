// `Demon's Jester` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEMONS_JESTER_SCRIPT } from './demonsJester';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Demon's Jester";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DEMONS_JESTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Demon's Jester"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEMONS_JESTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Demon's Jester", () => {
  test("`Demon's Jester` - as long as you have no cards in hand: absent while it is unmet, read once it holds", () => {
