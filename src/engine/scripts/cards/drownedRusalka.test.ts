// `Drowned Rusalka` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DROWNED_RUSALKA_SCRIPT } from './drownedRusalka';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drowned Rusalka";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drowned Rusalka", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DROWNED_RUSALKA_SCRIPT]),
...
describe("Drowned Rusalka", () => {
  test("{U}, Sacrifice a creature: the vocabulary resolves \"Discard a card, then draw a card.\"", () => {
    const { g, hand0, fodder0 } = armed(0);
    let qdA0 = 0;
