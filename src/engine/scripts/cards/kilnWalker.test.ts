// `Kiln Walker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KILN_WALKER_SCRIPT } from './kilnWalker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kiln Walker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KILN_WALKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kiln Walker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KILN_WALKER_SCRIPT]),
  });
  holdEverywhere(g);
