// `Notorious Assassin` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { NOTORIOUS_ASSASSIN_SCRIPT } from './notoriousAssassin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Notorious Assassin";

function settle(g: Game): void {
  advanceUntil(g, (s) => s.stack.length === 0 && s.pendingTriggers.length === 0, 20_000);
}

function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number; handCard: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Notorious Assassin", "Grizzly Bears"], ["Grizzly Bears", "Vampire Nighthawk"]],
    scripts: createRegistry([NOTORIOUS_ASSASSIN_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, target, wrong, life0, handCard };
}

describe("Notorious Assassin", () => {
  test("Grizzly Bears is a legal target and it is destroyed", () => {
    const { g, self, target, handCard } = armed();
