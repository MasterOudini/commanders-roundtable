// `Psychic Membrane` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PSYCHIC_MEMBRANE_SCRIPT } from './psychicMembrane';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Psychic Membrane";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Psychic Membrane"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PSYCHIC_MEMBRANE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Psychic Membrane", () => {
  test("Whenever this creature blocks: a card is drawn", () => {
