// `Ruinous Minotaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUINOUS_MINOTAUR_SCRIPT } from './ruinousMinotaur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ruinous Minotaur";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; queueFodderL0_dealsDamageOpponent: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ruinous Minotaur", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUINOUS_MINOTAUR_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const queueFodderL0_dealsDamageOpponent = put(g, 'p1', "Forest");
  settle(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, queueFodderL0_dealsDamageOpponent };
}

describe("Ruinous Minotaur", () => {
  test("Whenever this creature deals damage to an opponent: the vocabulary resolves \"Sacrifice a land.\"", () => {
    const { g, board0, queueFodderL0_dealsDamageOpponent } = armed(0);
    for (let qi = 0; qi < 4; qi++) {
      ...
      if (qa.player === 'p1') { must(g.submit({ t: 'AnswerChooseFromZone', player: 'p1', cards: [queueFodderL0_dealsDamageOpponent] })); continue; }
      let qDone = false;
    ...
    expect(g.state.cards[queueFodderL0_dealsDamageOpponent]?.zone.kind).toBe('graveyard');
