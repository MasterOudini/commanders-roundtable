// `Hobblefiend` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOBBLEFIEND_SCRIPT } from './hobblefiend';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hobblefiend";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hobblefiend", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOBBLEFIEND_SCRIPT]),
  });
  holdEverywhere(g);
