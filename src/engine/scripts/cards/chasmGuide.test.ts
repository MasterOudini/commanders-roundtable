// `Chasm Guide` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHASM_GUIDE_SCRIPT } from './chasmGuide';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chasm Guide";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHASM_GUIDE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chasm Guide", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHASM_GUIDE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chasm Guide", () => {
  test("Rally — Whenever this creature or another Ally you control enters: its controller's creatures gain haste until end of turn", () => {
