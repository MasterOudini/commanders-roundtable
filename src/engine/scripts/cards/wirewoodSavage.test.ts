// `Wirewood Savage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WIREWOOD_SAVAGE_SCRIPT } from './wirewoodSavage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wirewood Savage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wirewood Savage", "Grizzly Bears", "Jibbirik Omnivore"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WIREWOOD_SAVAGE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    bears = put(g, 'p1', "Jibbirik Omnivore");
...
describe("Wirewood Savage", () => {
  test("Whenever a Beast enters: a card is drawn", () => {
