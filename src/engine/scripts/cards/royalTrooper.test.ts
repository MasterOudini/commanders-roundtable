// `Royal Trooper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROYAL_TROOPER_SCRIPT } from './royalTrooper';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Royal Trooper";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ROYAL_TROOPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Royal Trooper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROYAL_TROOPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Royal Trooper", () => {
  test("Whenever this creature blocks: it gets +2/+2 until end of turn", () => {
