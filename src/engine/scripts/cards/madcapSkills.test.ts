// `Madcap Skills` - cast on Grizzly Bears it attaches and the enchanted creature gets +3/+0 and has menace;
...
import { MADCAP_SKILLS_SCRIPT } from './madcapSkills';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Madcap Skills";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MADCAP_SKILLS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([MADCAP_SKILLS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Madcap Skills", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +3/+0 and has menace; Cyclops of One-Eyed Pass is untouched", () => {
