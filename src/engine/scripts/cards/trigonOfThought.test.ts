// `Trigon of Thought` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRIGON_OF_THOUGHT_SCRIPT } from './trigonOfThought';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trigon of Thought";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TRIGON_OF_THOUGHT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trigon of Thought"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRIGON_OF_THOUGHT_SCRIPT]),
...
describe("Trigon of Thought", () => {
  ...
  test("{U}{U}, {T}: it gets 1 charge counter", () => {
  ...
  test("{2}, {T}, Remove a charge counter from this artifact: a card is drawn", () => {
