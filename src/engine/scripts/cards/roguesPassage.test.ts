// `Rogue's Passage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROGUES_PASSAGE_SCRIPT } from './roguesPassage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rogue's Passage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rogue's Passage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROGUES_PASSAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rogue's Passage", () => {
  test("{4}, {T}: the vocabulary resolves \"Target creature can't be blocked this turn.\"", () => {
