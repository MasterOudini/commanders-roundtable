// `Surrakar Banisher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SURRAKAR_BANISHER_SCRIPT } from './surrakarBanisher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Surrakar Banisher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Surrakar Banisher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SURRAKAR_BANISHER_SCRIPT]),
  });
  holdEverywhere(g);
