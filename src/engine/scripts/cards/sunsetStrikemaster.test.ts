// `Sunset Strikemaster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUNSET_STRIKEMASTER_SCRIPT } from './sunsetStrikemaster';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunset Strikemaster";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sunset Strikemaster"], ["Cyclops of One-Eyed Pass", "Air Elemental"]],
    scripts: createRegistry([SUNSET_STRIKEMASTER_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtA1_0 = put(g, 'p2', "Air Elemental");
  settle(g);
...
describe("Sunset Strikemaster", () => {
  test("{2}{R}, {T}, Sacrifice this creature: the vocabulary resolves \"~ deals 6 damage to target creature with flying.\"", () => {
