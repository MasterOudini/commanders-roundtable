// `Experimental Synthesizer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EXPERIMENTAL_SYNTHESIZER_SCRIPT } from './experimentalSynthesizer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Experimental Synthesizer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Experimental Synthesizer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EXPERIMENTAL_SYNTHESIZER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Experimental Synthesizer", () => {
  test("When this artifact enters or leaves the battlefield: the vocabulary resolves \"Exile the top card of your library. Until end of turn, you may play that card.\" [etb]", () => {
    const { g } = armed(0);
  ...
  test("When this artifact enters or leaves the battlefield: the vocabulary resolves \"Exile the top card of your library. Until end of turn, you may play that card.\" [leavesBattlefield]", () => {
    const { g } = armed(1);
    { const granted = g.log.filter((ev) => ev.body.t === 'PlayPermissionGranted');
      expect(granted.length, 'a permission per exiled card').toBeGreaterThanOrEqual(1);
      for (const ev of granted) if (ev.body.t === 'PlayPermissionGranted') expect(g.state.cards[ev.body.permission.card]?.zone.kind, 'the card is in exile').toBe('exile'); }
  ...
  test("{2}{R}, Sacrifice this artifact: 1 token made", () => {
