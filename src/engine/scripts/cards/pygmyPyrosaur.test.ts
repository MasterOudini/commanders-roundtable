// `Pygmy Pyrosaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PYGMY_PYROSAUR_SCRIPT } from './pygmyPyrosaur';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pygmy Pyrosaur";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PYGMY_PYROSAUR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pygmy Pyrosaur", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PYGMY_PYROSAUR_SCRIPT]),
  });
  holdEverywhere(g);
