// `Beacon Hawk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BEACON_HAWK_SCRIPT } from './beaconHawk';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Beacon Hawk";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BEACON_HAWK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Beacon Hawk"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BEACON_HAWK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Beacon Hawk", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Untap target creature.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.tapped).toBe(false);
  });

  test("{W}: it gets +0/+1 until end of turn", () => {
