// `Bloodied Ghost` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODIED_GHOST_SCRIPT } from './bloodiedGhost';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodied Ghost";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLOODIED_GHOST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodied Ghost"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODIED_GHOST_SCRIPT]),
