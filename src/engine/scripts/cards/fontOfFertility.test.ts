// `Font of Fertility` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Font of Fertility", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FONT_OF_FERTILITY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Font of Fertility", () => {
  test("{1}{G}, Sacrifice this enchantment: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
