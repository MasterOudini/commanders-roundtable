// `Hedron Rover` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HEDRON_ROVER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hedron Rover"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HEDRON_ROVER_SCRIPT]),
  });
  holdEverywhere(g);
