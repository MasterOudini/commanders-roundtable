// `Soliton` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOLITON_SCRIPT } from './soliton';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soliton"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOLITON_SCRIPT]),
  });
  holdEverywhere(g);
