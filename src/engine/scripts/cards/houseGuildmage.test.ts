// `House Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOUSE_GUILDMAGE_SCRIPT } from './houseGuildmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "House Guildmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["House Guildmage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOUSE_GUILDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("House Guildmage", () => {
  test("{1}{U}, {T}: the vocabulary resolves \"Target creature doesn't untap during its controller's next untap step.\"", () => {
    const { g, self, no } = armed(0);
    expect(g.state.cards[no]?.skipsUntap, 'frozen').toBe(true);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{2}{B}, {T}: surveil 2 asks, and the cards stay on top", () => {
