// `Geistwave` - a scripted spell: the target clause is the parser's and the validator's
// (D294's adjectives), the rider is this script's. Generated from one table row (D295).

import { GEISTWAVE } from '../../../data/fixtures/engineCards';
...
const TEXT = printed(GEISTWAVE, "Return target nonland permanent to its owner's hand. If you controlled that permanent, draw a card.");

export const GEISTWAVE_SCRIPT: CardScript = {
  oracleId: GEISTWAVE.oracleId,
  name: GEISTWAVE.name,
  spell: {
    text: TEXT,
