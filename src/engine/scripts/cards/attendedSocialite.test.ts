// `Attended Socialite` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ATTENDED_SOCIALITE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Attended Socialite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ATTENDED_SOCIALITE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Attended Socialite", () => {
  test("Alliance — Whenever another creature you control enters: it gets +1/+1 until end of turn", () => {
