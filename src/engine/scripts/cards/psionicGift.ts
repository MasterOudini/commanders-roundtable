// `Psionic Gift` - the permanent it is attached to HAS a quoted
...
import { PSIONIC_GIFT } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(PSIONIC_GIFT, "Enchant creature\nEnchanted creature has \"{T}: This creature deals 1 damage to any target.\"");
const LINES = PRINTED.split('\n');

const GRANT = grantedActivated("{T}: This creature deals 1 damage to any target.", `${PSIONIC_GIFT.oracleId}#g1`, PSIONIC_GIFT.name);
const VOCAB = vocabularyEffects("This creature deals 1 damage to any target.", PSIONIC_GIFT.name);
const VOCAB_T = vocabularyTargets("This creature deals 1 damage to any target.");

export const PSIONIC_GIFT_SCRIPT: CardScript = {
  oracleId: PSIONIC_GIFT.oracleId,
  name: PSIONIC_GIFT.name,
  statics: [
    {
      abilityId: 'grant-1',
