// `Lightning-Rig Crew` - an activation damageOpponents, a castSpell trigger untapSelf
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { LIGHTNING_RIG_CREW } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(LIGHTNING_RIG_CREW, "{T}: This creature deals 1 damage to each opponent.\nWhenever you cast a Pirate spell, untap this creature.");
const LINES = PRINTED.split('\n');

export const LIGHTNING_RIG_CREW_SCRIPT: CardScript = {
  oracleId: LIGHTNING_RIG_CREW.oracleId,
  name: LIGHTNING_RIG_CREW.name,
  activated: [
    {
      ref: `${LIGHTNING_RIG_CREW.oracleId}#a0`,
  ...
  triggers: [
    {
      ...
      matches: (ctx, self, ev) =>
        ...
        ctx.derive(ev.obj.card).typeLine.subtypes.includes('Pirate'),
      label: () => "Lightning-Rig Crew - untapSelf",
