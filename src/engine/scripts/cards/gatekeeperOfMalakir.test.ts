// `Gatekeeper of Malakir` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GATEKEEPER_OF_MALAKIR_SCRIPT } from './gatekeeperOfMalakir';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gatekeeper of Malakir";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gatekeeper of Malakir"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GATEKEEPER_OF_MALAKIR_SCRIPT]),
...
describe("Gatekeeper of Malakir", () => {
  test("When this creature enters: the vocabulary resolves \"Target player sacrifices a creature of their choice.\"", () => {
    const { g } = armed(0);
