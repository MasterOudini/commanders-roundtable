// `Ripclaw Wrangler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIPCLAW_WRANGLER_SCRIPT } from './ripclawWrangler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ripclaw Wrangler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ripclaw Wrangler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIPCLAW_WRANGLER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ripclaw Wrangler", () => {
  test("When this Vehicle enters: the vocabulary resolves \"Each opponent discards a card.\"", () => {
    const { g } = armed(0);
    let qdL0 = 0;
