// `Kraken's Eye` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KRAKENS_EYE_SCRIPT } from './krakensEye';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kraken's Eye";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kraken's Eye", "Grizzly Bears", "Aegis Turtle"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KRAKENS_EYE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kraken's Eye", () => {
  test("Whenever a player casts a blue spell: 1 life is gained", () => {
