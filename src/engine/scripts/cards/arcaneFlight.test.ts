// `Arcane Flight` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+1 and has flying;
...
import { ARCANE_FLIGHT_SCRIPT } from './arcaneFlight';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Arcane Flight";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ARCANE_FLIGHT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ARCANE_FLIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Arcane Flight", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+1 and has flying; Cyclops of One-Eyed Pass is untouched", () => {
