// `Ninja` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NINJA_TOKEN14EB629E_SCRIPT } from './ninjaToken14eb629e';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([NINJA_TOKEN14EB629E_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: NINJA_14EB629E_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === NINJA_14EB629E_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
