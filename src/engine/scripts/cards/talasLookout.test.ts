// `Talas Lookout` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TALAS_LOOKOUT_SCRIPT } from './talasLookout';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Talas Lookout";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Talas Lookout"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TALAS_LOOKOUT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Talas Lookout", () => {
  test("When this creature dies: the vocabulary resolves \"Look at the top two cards of your library. Put one of them into your hand and the other into your graveyard.\"", () => {
    const { g } = armed(0);
