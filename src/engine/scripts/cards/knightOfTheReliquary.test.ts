// `Knight of the Reliquary` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KNIGHT_OF_THE_RELIQUARY_SCRIPT } from './knightOfTheReliquary';
import { advanceUntil, deps, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Knight of the Reliquary";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; cda0: number; cdaT0: number; fodder0: InstanceId[] };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KNIGHT_OF_THE_RELIQUARY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Knight of the Reliquary", "Forest", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KNIGHT_OF_THE_RELIQUARY_SCRIPT]),
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, cda0, cdaT0, fodder0 };
}

describe("Knight of the Reliquary", () => {
  test("This creature gets +1/+1 for each land card in your graveyard.: it gets +1/+1 for each land card in your graveyard", () => {
    const { g, self, cda0, cdaT0 } = armed(0);
    expect(pt(g, self)).toEqual([cda0 + 1, cdaT0 + 1]);
  });

  test("{T}, Sacrifice a Forest or Plains: the vocabulary resolves \"Search your library for a land card, put it onto the battlefield, then shuffle.\"", () => {
