// `Future Flight` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: draw 2); gets +2/+0 and has flying;
...
import { FUTURE_FLIGHT_SCRIPT } from './futureFlight';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Future Flight";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FUTURE_FLIGHT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FUTURE_FLIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Future Flight", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: draw 2); gets +2/+0 and has flying; Cyclops of One-Eyed Pass is untouched", () => {
