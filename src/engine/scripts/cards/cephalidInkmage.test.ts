// `Cephalid Inkmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CEPHALID_INKMAGE_SCRIPT } from './cephalidInkmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cephalid Inkmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cephalid Inkmage", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CEPHALID_INKMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cephalid Inkmage", () => {
  test("When this creature enters: surveil 3 asks, and the cards stay on top", () => {
    const { g, lib0 } = armed(0);
    expect(g.state.priority.awaiting).toBeNull();
    expect((g.state.zones.library.p1 ?? []).length).toBe(lib0);
  });

  test("Threshold — This creature can't be blocked as long as there are seven or more cards in your graveyard.: the block is refused", () => {
