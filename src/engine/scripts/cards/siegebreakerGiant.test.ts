// `Siegebreaker Giant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SIEGEBREAKER_GIANT_SCRIPT } from './siegebreakerGiant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Siegebreaker Giant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Siegebreaker Giant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIEGEBREAKER_GIANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Siegebreaker Giant", () => {
  test("{3}{R}: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
    const { g, no } = armed(0);
