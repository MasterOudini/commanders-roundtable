// `Dwarven Weaponsmith` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DWARVEN_WEAPONSMITH_SCRIPT } from './dwarvenWeaponsmith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dwarven Weaponsmith";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dwarven Weaponsmith", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DWARVEN_WEAPONSMITH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dwarven Weaponsmith", () => {
  test("{T}, Sacrifice an artifact: the declared creature gets 1 +1/+1 counter - refused unless during your upkeep", () => {
    const { g, self, no, condRefused, fodder0 } = armed(0);
