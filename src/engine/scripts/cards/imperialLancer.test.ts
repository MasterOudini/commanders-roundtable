// `Imperial Lancer` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([IMPERIAL_LANCER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Imperial Lancer", "Snubhorn Sentry"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IMPERIAL_LANCER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Imperial Lancer", () => {
  test("`Imperial Lancer` - as long as you control a Dinosaur: absent while it is unmet, read once it holds", () => {
