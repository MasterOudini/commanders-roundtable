// `Saber Ants` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SABER_ANTS_SCRIPT } from './saberAnts';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Saber Ants";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Saber Ants", "Lightning Bolt", "Hill Giant"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([SABER_ANTS_SCRIPT]),
