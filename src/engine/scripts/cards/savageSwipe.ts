// `Savage Swipe` — "Target creature you control gets +2/+2 until end of
// turn if its power is 2. Then it fights target creature you don't
// control." The EXACT-power condition read at resolution: a power-2
// biter pumps and fights at 4, anything else fights as printed. D243.

import { SAVAGE_SWIPE } from '../../../data/fixtures/engineCards';
...
export const SAVAGE_SWIPE_SCRIPT: CardScript = {
  oracleId: SAVAGE_SWIPE.oracleId,
  name: SAVAGE_SWIPE.name,
  spell: {
    text: TEXT,
    resolve: (ctx, _self, obj): readonly EventBody[] => {
      ...
      const pumped = (ctx.derive(a.id).power ?? 0) === 2;
      if (pumped) {
        events.push({ t: 'PtModifiedUntilEndOfTurn', card: a.id, power: 2, toughness: 2 });
      }
      ...
      const damages = [];
      const aPower = (da.power ?? 0) + (pumped ? 2 : 0);
      if (aPower > 0) {
        damages.push({
          source: a.id,
