// `Skeletal Changeling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKELETAL_CHANGELING_SCRIPT } from './skeletalChangeling';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skeletal Changeling";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skeletal Changeling", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKELETAL_CHANGELING_SCRIPT]),
  });
  holdEverywhere(g);
