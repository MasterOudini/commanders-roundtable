// `Vampire Warlord` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VAMPIRE_WARLORD_SCRIPT } from './vampireWarlord';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vampire Warlord";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vampire Warlord", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VAMPIRE_WARLORD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vampire Warlord", () => {
  test("Sacrifice another creature: it regenerates from the Bolt", () => {
