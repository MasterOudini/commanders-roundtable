// `Thermo-Alchemist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THERMO_ALCHEMIST_SCRIPT } from './thermoAlchemist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thermo-Alchemist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thermo-Alchemist", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THERMO_ALCHEMIST_SCRIPT]),
  });
  holdEverywhere(g);
