// `Tyrranax Rex` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TYRRANAX_REX_SCRIPT } from './tyrranaxRex';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tyrranax Rex";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tyrranax Rex"], ["Cyclops of One-Eyed Pass", "Counterspell"]],
    scripts: createRegistry([TYRRANAX_REX_SCRIPT]),
  });
  holdEverywhere(g);
