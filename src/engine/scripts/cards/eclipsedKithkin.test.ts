// `Eclipsed Kithkin` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Eclipsed Kithkin", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ECLIPSED_KITHKIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Eclipsed Kithkin", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top four cards of your library. You may reveal a Kithkin, Forest, or Plains card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
