// `Brazen Upstart` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brazen Upstart", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRAZEN_UPSTART_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const lookPicksL1_dies = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Grizzly Bears" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL1_dies.length, "Grizzly Bears is dealt for the look").toBeGreaterThan(0);
...
describe("Brazen Upstart", () => {
  test("When this creature dies: the vocabulary resolves \"Look at the top five cards of your library. You may reveal a creature card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
