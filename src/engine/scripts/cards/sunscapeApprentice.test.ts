// `Sunscape Apprentice` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUNSCAPE_APPRENTICE_SCRIPT } from './sunscapeApprentice';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunscape Apprentice";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUNSCAPE_APPRENTICE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sunscape Apprentice", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUNSCAPE_APPRENTICE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sunscape Apprentice", () => {
  test("{G}, {T}: the declared creature is pumped until end of turn", () => {
  ...
  test("{U}, {T}: the vocabulary resolves \"Put target creature you control on top of its owner's library.\"", () => {
    ...
    expect(g.state.cards[vtA1_0]?.zone.kind).toBe('library');
    expect(g.state.cards[self]?.tapped).toBe(true);
  });
