// `Cinderbones` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CINDERBONES_SCRIPT } from './cinderbones';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cinderbones";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cinderbones", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CINDERBONES_SCRIPT]),
  });
  holdEverywhere(g);
