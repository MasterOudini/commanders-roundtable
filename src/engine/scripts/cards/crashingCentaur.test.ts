// `Crashing Centaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRASHING_CENTAUR_SCRIPT } from './crashingCentaur';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crashing Centaur";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CRASHING_CENTAUR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crashing Centaur", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRASHING_CENTAUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Crashing Centaur", () => {
  test("{G}, Discard a card: it gains trample until end of turn", () => {
  ...
  test("Threshold — As long as there are seven or more cards in your graveyard, this creature gets +2/+2 and has shroud. (It can't be the target of spells or abilities.): with seven cards in the graveyard it reads the pump", () => {
    const { g, self } = armed(1);
    expect(pt(g, self)).toEqual([5, 6]);
    expect(kw(g, self).has("shroud")).toBe(true);
  });
