// `Kindlespark Duo` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KINDLESPARK_DUO_SCRIPT } from './kindlesparkDuo';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kindlespark Duo";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kindlespark Duo", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KINDLESPARK_DUO_SCRIPT]),
  });
  holdEverywhere(g);
