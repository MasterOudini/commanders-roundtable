// `Containment Protocol` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONTAINMENT_PROTOCOL_SCRIPT } from './containmentProtocol';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Containment Protocol";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Containment Protocol", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONTAINMENT_PROTOCOL_SCRIPT]),
  });
  holdEverywhere(g);
