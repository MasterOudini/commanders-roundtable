// `Verdant Field` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { VERDANT_FIELD_SCRIPT } from './verdantField';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Verdant Field";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Verdant Field","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VERDANT_FIELD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Verdant Field", () => {
  test("grants {T}: Target creature gets +1/+1 until end of turn. to Forest, and to nothing outside the scope", () => {
