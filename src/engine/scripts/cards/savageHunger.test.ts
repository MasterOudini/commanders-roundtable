// `Savage Hunger` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+0 and has trample;
...
import { SAVAGE_HUNGER_SCRIPT } from './savageHunger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Savage Hunger";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SAVAGE_HUNGER_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SAVAGE_HUNGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Savage Hunger", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+0 and has trample; Cyclops of One-Eyed Pass is untouched", () => {
