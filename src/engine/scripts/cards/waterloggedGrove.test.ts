// `Waterlogged Grove` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WATERLOGGED_GROVE_SCRIPT } from './waterloggedGrove';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Waterlogged Grove";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Waterlogged Grove"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WATERLOGGED_GROVE_SCRIPT]),
  });
  holdEverywhere(g);
