// `Spinewoods Paladin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPINEWOODS_PALADIN_SCRIPT } from './spinewoodsPaladin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spinewoods Paladin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spinewoods Paladin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPINEWOODS_PALADIN_SCRIPT]),
