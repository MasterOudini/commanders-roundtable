// `Yasmin Khan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { YASMIN_KHAN_SCRIPT } from './yasminKhan';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Yasmin Khan";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Yasmin Khan"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YASMIN_KHAN_SCRIPT]),
...
describe("Yasmin Khan", () => {
  test("{T}: the vocabulary resolves \"Exile the top card of your library. Until your next end step, you may play it.\"", () => {
