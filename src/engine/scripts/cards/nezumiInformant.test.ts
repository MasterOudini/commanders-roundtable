// `Nezumi Informant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NEZUMI_INFORMANT_SCRIPT } from './nezumiInformant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nezumi Informant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nezumi Informant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NEZUMI_INFORMANT_SCRIPT]),
  });
  holdEverywhere(g);
