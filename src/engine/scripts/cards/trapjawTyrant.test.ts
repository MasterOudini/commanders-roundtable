// `Trapjaw Tyrant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRAPJAW_TYRANT_SCRIPT } from './trapjawTyrant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trapjaw Tyrant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trapjaw Tyrant", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([TRAPJAW_TYRANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Trapjaw Tyrant", () => {
  test("Enrage — Whenever this creature is dealt damage: the vocabulary resolves \"Exile target creature an opponent controls until this creature leaves the battlefield.\" [isDealtCombatDamage]", () => {
  ...
  test("Enrage — Whenever this creature is dealt damage: the vocabulary resolves \"Exile target creature an opponent controls until this creature leaves the battlefield.\" [isDealtNoncombatDamage]", () => {
