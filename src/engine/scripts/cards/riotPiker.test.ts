// `Riot Piker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIOT_PIKER_SCRIPT } from './riotPiker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Riot Piker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Riot Piker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIOT_PIKER_SCRIPT]),
  });
  holdEverywhere(g);
