// `Primaris Chaplain` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PRIMARIS_CHAPLAIN_SCRIPT } from './primarisChaplain';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Primaris Chaplain";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PRIMARIS_CHAPLAIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Primaris Chaplain", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRIMARIS_CHAPLAIN_SCRIPT]),
...
describe("Primaris Chaplain", () => {
  ...
  test("Rosarius — Whenever this creature attacks: it gains indestructible until end of turn", () => {
