// `Scavenging Scarab` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCAVENGING_SCARAB_SCRIPT } from './scavengingScarab';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scavenging Scarab";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scavenging Scarab", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCAVENGING_SCARAB_SCRIPT]),
  });
  holdEverywhere(g);
