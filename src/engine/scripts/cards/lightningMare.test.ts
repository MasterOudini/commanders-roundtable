// `Lightning Mare` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIGHTNING_MARE_SCRIPT } from './lightningMare';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning Mare";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LIGHTNING_MARE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lightning Mare", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Counterspell", "Aegis Turtle"]],
    scripts: createRegistry([LIGHTNING_MARE_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const gift2 = put(g, 'p2', "Aegis Turtle");
...
describe("Lightning Mare", () => {
  ...
  test("This creature can't be blocked by blue creatures.: the block by blue creatures is refused", () => {
