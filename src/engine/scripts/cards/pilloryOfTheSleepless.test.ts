// `Pillory of the Sleepless` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PILLORY_OF_THE_SLEEPLESS_SCRIPT } from './pilloryOfTheSleepless';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pillory of the Sleepless";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pillory of the Sleepless", "Grizzly Bears", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PILLORY_OF_THE_SLEEPLESS_SCRIPT]),
...
describe("Pillory of the Sleepless", () => {
  ...
  test("Enchanted creature has \"At the beginning of your upkeep, you lose 1 life.\": the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
  });

  test("Enchanted creature has \"At the beginning of your upkeep: 1 life is lost [upkeep]", () => {
