// `Silkwing Scout` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Silkwing Scout", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SILKWING_SCOUT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Silkwing Scout", () => {
  test("{G}, Sacrifice this creature: the vocabulary resolves \"Search your library for a basic land card, put that card onto the battlefield tapped, then shuffle.\"", () => {
