// `Soul of Theros` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOUL_OF_THEROS_SCRIPT } from './soulOfTheros';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Soul of Theros";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SOUL_OF_THEROS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soul of Theros", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOUL_OF_THEROS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Soul of Theros", () => {
  test("{4}{W}{W}: its controller's creatures get +2/+2 and gain firstStrike and lifelink until end of turn", () => {
  ...
  test("{4}{W}{W}, Exile this card from your graveyard: its controller's creatures get +2/+2 and gain firstStrike and lifelink until end of turn", () => {
