// `Festering Gulch` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FESTERING_GULCH_SCRIPT } from './festeringGulch';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Festering Gulch";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Festering Gulch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FESTERING_GULCH_SCRIPT]),
  });
  holdEverywhere(g);
