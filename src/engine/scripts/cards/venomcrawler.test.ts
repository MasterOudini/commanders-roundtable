// `Venomcrawler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VENOMCRAWLER_SCRIPT } from './venomcrawler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Venomcrawler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Venomcrawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VENOMCRAWLER_SCRIPT]),
...
describe("Venomcrawler", () => {
  test("Devourer of Souls — Whenever another creature dies: it gets 1 +1/+1 counter", () => {
