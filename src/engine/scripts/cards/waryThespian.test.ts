// `Wary Thespian` — the enters-or-dies surveil, both arms. An EXACT-TEXT TWIN of
// `Wary Watchdog`; both are generated from one base, and both are tested, because a
// generator that is only proven on one member proves nothing about the other.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { WARY_THESPIAN_SCRIPT } from './waryThespian';
import { advanceUntil, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = 'Wary Thespian';
...
function entered(): { g: Game; card: InstanceId; revealed: InstanceId[] } {
  const g = startedGame({
    players: 2,
    decks: [[CARD], []],
    scripts: createRegistry([WARY_THESPIAN_SCRIPT]),
  });
