// `Prison Realm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PRISON_REALM_SCRIPT } from './prisonRealm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prison Realm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prison Realm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRISON_REALM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Prison Realm", () => {
  test("When this enchantment enters: the vocabulary resolves \"Exile target creature or planeswalker an opponent controls until this enchantment leaves the battlefield.\"", () => {
  ...
  test("When this enchantment enters: scry 1 asks, and the cards stay on top", () => {
