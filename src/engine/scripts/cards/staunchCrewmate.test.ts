// `Staunch Crewmate` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Staunch Crewmate", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STAUNCH_CREWMATE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Staunch Crewmate", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top four cards of your library. You may reveal an artifact or Pirate card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
