// `Charging Slateback` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHARGING_SLATEBACK_SCRIPT } from './chargingSlateback';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Charging Slateback";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Charging Slateback", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHARGING_SLATEBACK_SCRIPT]),
  });
  holdEverywhere(g);
