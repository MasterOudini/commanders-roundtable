// `Jeering Homunculus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JEERING_HOMUNCULUS_SCRIPT } from './jeeringHomunculus';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jeering Homunculus";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jeering Homunculus"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JEERING_HOMUNCULUS_SCRIPT]),
...
describe("Jeering Homunculus", () => {
  test("When this creature enters: the vocabulary resolves \"Goad target creature.\"", () => {
    const { g, no } = armed(0);
