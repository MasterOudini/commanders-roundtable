// `Auriok Champion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AURIOK_CHAMPION_SCRIPT } from './auriokChampion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Auriok Champion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Auriok Champion", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AURIOK_CHAMPION_SCRIPT]),
  });
  holdEverywhere(g);
