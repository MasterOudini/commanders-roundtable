// `Implement of Combustion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IMPLEMENT_OF_COMBUSTION_SCRIPT } from './implementOfCombustion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Implement of Combustion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Implement of Combustion", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IMPLEMENT_OF_COMBUSTION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Implement of Combustion", () => {
  test("{R}, Sacrifice this artifact: it deals 1 damage to the declared player", () => {
  ...
  test("When this artifact is put into a graveyard from the battlefield: a card is drawn", () => {
