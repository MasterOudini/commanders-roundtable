// `Ire of Kaminari` — damage to any target equal to the Arcane cards in
// my graveyard (oracle-face subtypes). D220.
...
export const IRE_OF_KAMINARI_SCRIPT: CardScript = {
  oracleId: IRE_OF_KAMINARI.oracleId,
  name: IRE_OF_KAMINARI.name,
  spell: {
    text: TEXT,
    resolve: (ctx, self, obj): readonly EventBody[] => {
      ...
      for (const id of ctx.state.zones.graveyard[obj.controller] ?? []) {
        ...
        if (!oc) continue;
        if (faceOf(oc, card.faceIndex ?? 0).typeLine.subtypes.includes('Arcane')) x++;
      }
      if (x === 0) return [];
      return [
        {
