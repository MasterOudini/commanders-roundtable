// `Caged Zombie` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAGED_ZOMBIE_SCRIPT } from './cagedZombie';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Caged Zombie";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Caged Zombie", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAGED_ZOMBIE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Caged Zombie", () => {
  test("{1}{B}, {T}: each opponent loses 2 life - refused unless if a creature died this turn", () => {
