// `Grasping Giant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRASPING_GIANT_SCRIPT } from './graspingGiant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grasping Giant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grasping Giant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRASPING_GIANT_SCRIPT]),
...
describe("Grasping Giant", () => {
  test("Whenever this creature becomes blocked by a creature: the vocabulary resolves \"Exile target creature until this creature leaves the battlefield.\"", () => {
