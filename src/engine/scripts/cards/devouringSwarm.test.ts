// `Devouring Swarm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEVOURING_SWARM_SCRIPT } from './devouringSwarm';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Devouring Swarm";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DEVOURING_SWARM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Devouring Swarm", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEVOURING_SWARM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Devouring Swarm", () => {
  test("Sacrifice a creature: it gets +1/+1 until end of turn", () => {
