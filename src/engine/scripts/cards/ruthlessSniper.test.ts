// `Ruthless Sniper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUTHLESS_SNIPER_SCRIPT } from './ruthlessSniper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ruthless Sniper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ruthless Sniper", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUTHLESS_SNIPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ruthless Sniper", () => {
  test("Whenever you cycle or discard a card: the vocabulary resolves \"You may pay {1}. If you do, put a -1/-1 counter on target creature.\" - the price is declined", () => {
  ...
  test("Whenever you cycle or discard a card: the vocabulary resolves \"You may pay {1}. If you do, put a -1/-1 counter on target creature.\" - the price is paid", () => {
    const { g, no, payL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
