// `Unstoppable Ogre` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNSTOPPABLE_OGRE_SCRIPT } from './unstoppableOgre';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unstoppable Ogre";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Unstoppable Ogre"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNSTOPPABLE_OGRE_SCRIPT]),
  });
  holdEverywhere(g);
