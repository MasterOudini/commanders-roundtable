// `Curious Altisaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CURIOUS_ALTISAUR_SCRIPT } from './curiousAltisaur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Curious Altisaur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Curious Altisaur", "Raptor Companion", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CURIOUS_ALTISAUR_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const stillFix0 = put(g, 'p1', "Raptor Companion");
  settle(g);
...
describe("Curious Altisaur", () => {
  test("Whenever a Dinosaur you control deals combat damage to a player: a card is drawn", () => {
