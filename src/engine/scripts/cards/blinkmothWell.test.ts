// `Blinkmoth Well` - the named target is accepted and it is tapped; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { BLINKMOTH_WELL_SCRIPT } from './blinkmothWell';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blinkmoth Well";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Blinkmoth Well"], ["Sol Ring", "Grizzly Bears"]],
    scripts: createRegistry([BLINKMOTH_WELL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Blinkmoth Well", () => {
  test("Sol Ring is a legal target and it is tapped", () => {
