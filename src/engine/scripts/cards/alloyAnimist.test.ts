// `Alloy Animist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ALLOY_ANIMIST_SCRIPT } from './alloyAnimist';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Alloy Animist";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ALLOY_ANIMIST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Alloy Animist", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ALLOY_ANIMIST_SCRIPT]),
  });
  holdEverywhere(g);
