// `Clockwork Fox` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLOCKWORK_FOX_SCRIPT } from './clockworkFox';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Clockwork Fox";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Clockwork Fox"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLOCKWORK_FOX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Clockwork Fox", () => {
  test("When this creature leaves the battlefield: the vocabulary resolves \"You draw two cards and each opponent draws a card.\"", () => {
