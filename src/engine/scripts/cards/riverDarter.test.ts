// `River Darter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIVER_DARTER_SCRIPT } from './riverDarter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "River Darter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["River Darter", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Raptor Companion"]],
    scripts: createRegistry([RIVER_DARTER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const gift2 = put(g, 'p2', "Raptor Companion");
  settle(g);
...
describe("River Darter", () => {
  test("This creature can't be blocked by Dinosaurs.: the block by Dinosaurs is refused", () => {
