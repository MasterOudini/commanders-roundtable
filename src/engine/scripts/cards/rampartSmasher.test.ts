// `Rampart Smasher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAMPART_SMASHER_SCRIPT } from './rampartSmasher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rampart Smasher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rampart Smasher", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Knight Errant"]],
    scripts: createRegistry([RAMPART_SMASHER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const gift2 = put(g, 'p2', "Knight Errant");
  settle(g);
...
describe("Rampart Smasher", () => {
  test("This creature can't be blocked by Knights or Walls.: the block by Knights or Walls is refused", () => {
