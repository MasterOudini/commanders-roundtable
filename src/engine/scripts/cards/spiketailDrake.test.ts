// `Spiketail Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIKETAIL_DRAKE_SCRIPT } from './spiketailDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spiketail Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spiketail Drake"], ["Cyclops of One-Eyed Pass", "Divination", "Forest", "Forest", "Forest"]],
    scripts: createRegistry([SPIKETAIL_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spiketail Drake", () => {
  test("Sacrifice this creature: the vocabulary resolves \"Counter target spell unless its controller pays {3}.\" - the price is declined", () => {
  ...
  test("Sacrifice this creature: the vocabulary resolves \"Counter target spell unless its controller pays {3}.\" - the price is paid", () => {
    const { g, self, payA0_p2_0, payA0_p2_1, payA0_p2_2 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
