// `Petrahydrox` - a becomesTargeted trigger bounceSelf
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { PETRAHYDROX } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(PETRAHYDROX, "({U/R} can be paid with either {U} or {R}.)\nWhen this creature becomes the target of a spell or ability, return this creature to its owner's hand.");
const LINES = PRINTED.split('\n');

export const PETRAHYDROX_SCRIPT: CardScript = {
  oracleId: PETRAHYDROX.oracleId,
  name: PETRAHYDROX.name,
  triggers: [
    {
