// `Keymaster Rogue` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KEYMASTER_ROGUE_SCRIPT } from './keymasterRogue';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Keymaster Rogue";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; refused: boolean; bearsB: InstanceId; queueFodderL1_etb: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Keymaster Rogue", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KEYMASTER_ROGUE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, refused, bearsB, queueFodderL1_etb };
}

describe("Keymaster Rogue", () => {
  ...
  test("When this creature enters: the vocabulary resolves \"Return a creature you control to its owner's hand.\"", () => {
    const { g, board0, queueFodderL1_etb } = armed(1);
