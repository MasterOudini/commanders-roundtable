// `Frantic Strength` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2 and has trample;
...
import { FRANTIC_STRENGTH_SCRIPT } from './franticStrength';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frantic Strength";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FRANTIC_STRENGTH_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FRANTIC_STRENGTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Frantic Strength", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+2 and has trample; Cyclops of One-Eyed Pass is untouched", () => {
