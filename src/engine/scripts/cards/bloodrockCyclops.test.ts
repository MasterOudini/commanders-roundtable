// `Bloodrock Cyclops` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODROCK_CYCLOPS_SCRIPT } from './bloodrockCyclops';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodrock Cyclops";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodrock Cyclops", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODROCK_CYCLOPS_SCRIPT]),
  });
  holdEverywhere(g);
