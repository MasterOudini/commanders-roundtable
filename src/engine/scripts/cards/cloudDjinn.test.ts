// `Cloud Djinn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLOUD_DJINN_SCRIPT } from './cloudDjinn';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cloud Djinn";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cloud Djinn", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLOUD_DJINN_SCRIPT]),
  });
  holdEverywhere(g);
