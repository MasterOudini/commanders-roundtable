// `Lu Xun, Scholar General` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LU_XUN_SCHOLAR_GENERAL_SCRIPT } from './luXunScholarGeneral';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lu Xun, Scholar General";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lu Xun, Scholar General"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LU_XUN_SCHOLAR_GENERAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lu Xun, Scholar General", () => {
  test("Whenever Lu Xun deals damage to an opponent: a card is drawn", () => {
