// `Lobber Crew` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOBBER_CREW_SCRIPT } from './lobberCrew';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lobber Crew";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lobber Crew", "Grizzly Bears", "Grizzled Leotau"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOBBER_CREW_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lobber Crew", () => {
  ...
  test("Whenever you cast a multicolored spell: it untaps", () => {
