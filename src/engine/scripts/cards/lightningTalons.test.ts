// `Lightning Talons` - cast on Grizzly Bears it attaches and the enchanted creature gets +3/+0 and has firstStrike;
...
import { LIGHTNING_TALONS_SCRIPT } from './lightningTalons';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning Talons";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LIGHTNING_TALONS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([LIGHTNING_TALONS_SCRIPT]),
  });
  holdEverywhere(g);
