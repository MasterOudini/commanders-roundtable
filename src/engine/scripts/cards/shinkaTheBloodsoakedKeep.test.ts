// `Shinka, the Bloodsoaked Keep` - the named target is accepted and the pump lands; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SHINKA_THE_BLOODSOAKED_KEEP_SCRIPT } from './shinkaTheBloodsoakedKeep';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shinka, the Bloodsoaked Keep";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Shinka, the Bloodsoaked Keep", "Lady Caleria", "Grizzly Bears"], ["Grizzly Bears"]],
    scripts: createRegistry([SHINKA_THE_BLOODSOAKED_KEEP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shinka, the Bloodsoaked Keep", () => {
  test("Lady Caleria is a legal target and the pump lands", () => {
    ...
    settle(g);
    const d = deps(createRegistry([SHINKA_THE_BLOODSOAKED_KEEP_SCRIPT]));
