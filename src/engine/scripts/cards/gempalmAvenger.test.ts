// `Gempalm Avenger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GEMPALM_AVENGER_SCRIPT } from './gempalmAvenger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gempalm Avenger";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([GEMPALM_AVENGER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gempalm Avenger", "Staunch Shieldmate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GEMPALM_AVENGER_SCRIPT]),
...
describe("Gempalm Avenger", () => {
  test("When you cycle this card: Soldier creatures get +1/+1 and gain firstStrike until end of turn", () => {
