// `Bat Whisperer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BAT_WHISPERER_SCRIPT } from './batWhisperer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bat Whisperer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bat Whisperer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BAT_WHISPERER_SCRIPT]),
  });
  holdEverywhere(g);
