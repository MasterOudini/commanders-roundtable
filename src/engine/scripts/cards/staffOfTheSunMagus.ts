// Staff of the Sun Magus - the white/Plains member of the five-Staff
// family (see staffOfTheDeathMagus.ts). D252.

import { STAFF_OF_THE_SUN_MAGUS } from '../../../data/fixtures/engineCards';
...
function landQualifies(ctx: ScriptCtx, self: InstanceId, entrant: InstanceId): boolean {
  ...
  return ctx.derive(entrant).typeLine.subtypes.includes('Plains');
}
...
export const STAFF_OF_THE_SUN_MAGUS_SCRIPT: CardScript = {
  oracleId: STAFF_OF_THE_SUN_MAGUS.oracleId,
  name: STAFF_OF_THE_SUN_MAGUS.name,
  triggers: [
    {
      abilityId: 'cast',
      text: TEXT,
      ...
      matches: (ctx, self, ev) => {
        ...
        if (!oc) return false;
        return faceOf(oc, ev.obj.faceIndex).colors.includes('W');
      },
      label: () => 'Staff of the Sun Magus — you gain 1 life',
