// `Condemn` — my attacking Bears goes to the bottom of my library and I (its
// controller) gain its toughness; a creature at home is refused (D291).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { CONDEMN_SCRIPT } from './condemn';
...
function aimed(): { g: Game; att: InstanceId; home: InstanceId; life: number } {
  const g = startedGame({ players: 2, decks: [[SPELL, BEARS, BEARS, 'Island', 'Island'], []], scripts: createRegistry([CONDEMN_SCRIPT]) });
...
describe('Condemn', () => {
  test('the attacker goes to the bottom of its owner’s library and its controller gains its toughness', () => {
    const { g, att, life } = aimed();
    must(g.submit({ t: 'ChooseTargets', player: 'p1', targets: [{ kind: 'card', id: att }] }));
    settle(g);
    expect(g.state.cards[att]?.zone).toEqual({ kind: 'library', player: 'p1' });
    expect((g.state.zones.library.p1 ?? [])[0]).toBe(att);
