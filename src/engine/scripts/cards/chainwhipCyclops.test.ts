// `Chainwhip Cyclops` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHAINWHIP_CYCLOPS_SCRIPT } from './chainwhipCyclops';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chainwhip Cyclops";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chainwhip Cyclops"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHAINWHIP_CYCLOPS_SCRIPT]),
  });
  holdEverywhere(g);
