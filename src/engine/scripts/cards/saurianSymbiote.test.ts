// `Saurian Symbiote` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SAURIAN_SYMBIOTE_SCRIPT } from './saurianSymbiote';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Saurian Symbiote";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Saurian Symbiote"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SAURIAN_SYMBIOTE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Saurian Symbiote", () => {
  ...
  test("When this creature enters: choosing \"Create a 1/1 green Saproling creature token.\", token", () => {
