// `Golem's Heart` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOLEMS_HEART_SCRIPT } from './golemsHeart';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Golem's Heart";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Golem's Heart", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOLEMS_HEART_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Golem's Heart", () => {
  test("Whenever a player casts an artifact spell: 1 life is gained", () => {
