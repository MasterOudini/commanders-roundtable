// `Sunset Pyramid` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUNSET_PYRAMID_SCRIPT } from './sunsetPyramid';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunset Pyramid";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUNSET_PYRAMID_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sunset Pyramid"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUNSET_PYRAMID_SCRIPT]),
  ...
  if (which === 1) {
    ...
    settle(g);
    cnt0 = g.state.cards[self]?.counters["brick"] ?? 0;
    pt0 = pt(g, self);
    must(g.submit({ t: 'ManualAddMana', player: 'p1', target: 'p1', symbol: 'C', amount: 2 }));
...
describe("Sunset Pyramid", () => {
  test("This artifact enters with three brick counters on it.: cast, it enters with 3 brick counters", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.zone.kind).toBe('battlefield');
    expect(g.state.cards[self]?.counters["brick"] ?? 0).toBe(3);
  });

  test("{2}, {T}, Remove a brick counter from this artifact: a card is drawn", () => {
    const { g, self, hand0, cnt0 } = armed(1);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1);
    expect(g.state.cards[self]?.counters["brick"] ?? 0).toBe(cnt0 - 1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{2}, {T}: scry 1 asks, and the cards stay on top", () => {
    const { g, self, lib0 } = armed(2);
