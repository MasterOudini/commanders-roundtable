// `Shard of Broken Glass` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHARD_OF_BROKEN_GLASS_SCRIPT } from './shardOfBrokenGlass';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shard of Broken Glass";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHARD_OF_BROKEN_GLASS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shard of Broken Glass", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHARD_OF_BROKEN_GLASS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shard of Broken Glass", () => {
  ...
  test("Whenever equipped creature attacks: 2 cards milled", () => {
    const { g, gy0 } = armed(1);
