// `Kitchen` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KITCHEN_SCRIPT } from './kitchen';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kitchen"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KITCHEN_SCRIPT]),
  });
  holdEverywhere(g);
