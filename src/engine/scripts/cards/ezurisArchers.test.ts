// `Ezuri's Archers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EZURIS_ARCHERS_SCRIPT } from './ezurisArchers';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ezuri's Archers";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([EZURIS_ARCHERS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ezuri's Archers"], ["Cyclops of One-Eyed Pass", "Aven Skirmisher"]],
    scripts: createRegistry([EZURIS_ARCHERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ezuri's Archers", () => {
  test("Whenever this creature blocks a creature with flying: it gets +3/+0 until end of turn", () => {
