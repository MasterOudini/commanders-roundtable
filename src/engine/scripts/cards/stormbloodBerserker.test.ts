// `Stormblood Berserker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORMBLOOD_BERSERKER_SCRIPT } from './stormbloodBerserker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stormblood Berserker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STORMBLOOD_BERSERKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stormblood Berserker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORMBLOOD_BERSERKER_SCRIPT]),
  });
  holdEverywhere(g);
