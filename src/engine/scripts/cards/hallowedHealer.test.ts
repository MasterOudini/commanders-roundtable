// `Hallowed Healer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HALLOWED_HEALER_SCRIPT } from './hallowedHealer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hallowed Healer";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; condRefused: boolean; sparkSpray: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hallowed Healer", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Spark Spray"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HALLOWED_HEALER_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, condRefused, sparkSpray };
}

describe("Hallowed Healer", () => {
  test("{T}: the vocabulary resolves \"Prevent the next 2 damage that would be dealt to any target this turn.\"", () => {
  ...
  test("Threshold — {T}: the vocabulary resolves \"Prevent the next 4 damage that would be dealt to any target this turn.\" - refused unless if there are seven or more cards in your graveyard", () => {
