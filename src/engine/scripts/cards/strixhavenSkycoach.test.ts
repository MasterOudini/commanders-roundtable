// `Strixhaven Skycoach` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Strixhaven Skycoach", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STRIXHAVEN_SKYCOACH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Strixhaven Skycoach", () => {
  test("When this Vehicle enters: the vocabulary resolves \"Search your library for a basic land card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
