// `Rushwood Herbalist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUSHWOOD_HERBALIST_SCRIPT } from './rushwoodHerbalist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rushwood Herbalist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rushwood Herbalist", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUSHWOOD_HERBALIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rushwood Herbalist", () => {
  test("{G}, {T}, Discard a card: the declared creature regenerates from the Bolt", () => {
    const { g, self, no, disc0 } = armed(0);
