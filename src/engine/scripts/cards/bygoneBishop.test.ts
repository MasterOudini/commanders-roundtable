// `Bygone Bishop` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BYGONE_BISHOP_SCRIPT } from './bygoneBishop';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bygone Bishop";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bygone Bishop", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BYGONE_BISHOP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bygone Bishop", () => {
  test("Whenever you cast a creature spell with mana value 3 or less: 1 token made", () => {
