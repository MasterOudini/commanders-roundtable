// `Owl Familiar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OWL_FAMILIAR_SCRIPT } from './owlFamiliar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Owl Familiar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Owl Familiar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OWL_FAMILIAR_SCRIPT]),
  });
  holdEverywhere(g);
