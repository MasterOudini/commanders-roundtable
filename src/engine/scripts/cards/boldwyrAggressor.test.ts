// `Boldwyr Aggressor` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BOLDWYR_AGGRESSOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Boldwyr Aggressor", "Hill Giant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOLDWYR_AGGRESSOR_SCRIPT]),
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Hill Giant");
...
describe("Boldwyr Aggressor", () => {
  test("Other Giants you control have double strike.: only the Giant reads it", () => {
