// `Rubblebelt Maverick` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUBBLEBELT_MAVERICK_SCRIPT } from './rubblebeltMaverick';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rubblebelt Maverick";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rubblebelt Maverick"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUBBLEBELT_MAVERICK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rubblebelt Maverick", () => {
  test("When this creature enters: surveil 2 asks, and the cards stay on top", () => {
    const { g, lib0 } = armed(0);
    expect(g.state.priority.awaiting).toBeNull();
    expect((g.state.zones.library.p1 ?? []).length).toBe(lib0);
  });

  test("{G}, Exile this card from your graveyard: the declared creature gets 1 +1/+1 counter", () => {
