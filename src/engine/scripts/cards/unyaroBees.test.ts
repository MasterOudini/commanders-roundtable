// `Unyaro Bees` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNYARO_BEES_SCRIPT } from './unyaroBees';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unyaro Bees";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([UNYARO_BEES_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Unyaro Bees"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNYARO_BEES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Unyaro Bees", () => {
  test("{G}: it gets +1/+1 until end of turn", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([1, 2]);
  });

  test("{3}{G}, Sacrifice this creature: it deals 2 damage to the declared creature", () => {
