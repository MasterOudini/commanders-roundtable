// `Rhox Maulers` - a renownDamage trigger renown
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { RHOX_MAULERS } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(RHOX_MAULERS, "Trample (This creature can deal excess combat damage to the player or planeswalker it's attacking.)\nRenown 2 (When this creature deals combat damage to a player, if it isn't renowned, put two +1/+1 counters on it and it becomes renowned.)");
const LINES = PRINTED.split('\n');

export const RHOX_MAULERS_SCRIPT: CardScript = {
  oracleId: RHOX_MAULERS.oracleId,
  name: RHOX_MAULERS.name,
  triggers: [
    {
