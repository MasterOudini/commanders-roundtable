// `Seismic Monstrosaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEISMIC_MONSTROSAUR_SCRIPT } from './seismicMonstrosaur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Seismic Monstrosaur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seismic Monstrosaur", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEISMIC_MONSTROSAUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Seismic Monstrosaur", () => {
  test("{2}{R}, Sacrifice a land: a card is drawn", () => {
