// `Bonded Fetch` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BONDED_FETCH_SCRIPT } from './bondedFetch';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bonded Fetch";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bonded Fetch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BONDED_FETCH_SCRIPT]),
  });
  holdEverywhere(g);
