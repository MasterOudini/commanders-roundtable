// `Aether Figment` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AETHER_FIGMENT_SCRIPT } from './aetherFigment';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aether Figment";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([AETHER_FIGMENT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aether Figment", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AETHER_FIGMENT_SCRIPT]),
  });
  holdEverywhere(g);
