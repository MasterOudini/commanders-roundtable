// `Esper Panorama` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Esper Panorama", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ESPER_PANORAMA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Esper Panorama", () => {
  test("{1}, {T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Plains, Island, or Swamp card, put it onto the battlefield tapped, then shuffle.\"", () => {
