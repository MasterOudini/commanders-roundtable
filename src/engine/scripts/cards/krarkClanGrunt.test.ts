// `Krark-Clan Grunt` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KRARK_CLAN_GRUNT_SCRIPT } from './krarkClanGrunt';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Krark-Clan Grunt";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KRARK_CLAN_GRUNT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Krark-Clan Grunt", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KRARK_CLAN_GRUNT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Krark-Clan Grunt", () => {
  test("Sacrifice an artifact: it gets +1/+0 and gains firstStrike until end of turn", () => {
