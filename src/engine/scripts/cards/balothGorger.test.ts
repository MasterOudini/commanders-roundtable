// `Baloth Gorger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BALOTH_GORGER_SCRIPT } from './balothGorger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Baloth Gorger";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BALOTH_GORGER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Baloth Gorger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BALOTH_GORGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Baloth Gorger", () => {
  test("If this creature was kicked, it enters with three +1/+1 counters on it.: cast, it enters with 3 +1/+1 counters", () => {
