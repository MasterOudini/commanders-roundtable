// `Midnight Covenant` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { MIDNIGHT_COVENANT_SCRIPT } from './midnightCovenant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Midnight Covenant";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Midnight Covenant","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIDNIGHT_COVENANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Midnight Covenant", () => {
  test("grants {B}: This creature gets +1/+1 until end of turn. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and the body hits the RECIPIENT', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{B}");
