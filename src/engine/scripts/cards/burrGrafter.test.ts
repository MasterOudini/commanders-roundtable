// `Burr Grafter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BURR_GRAFTER_SCRIPT } from './burrGrafter';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Burr Grafter";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BURR_GRAFTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Burr Grafter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BURR_GRAFTER_SCRIPT]),
  });
  holdEverywhere(g);
