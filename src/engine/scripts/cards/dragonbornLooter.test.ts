// `Dragonborn Looter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRAGONBORN_LOOTER_SCRIPT } from './dragonbornLooter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dragonborn Looter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dragonborn Looter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRAGONBORN_LOOTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dragonborn Looter", () => {
  test("{1}, {T}: a card is drawn, then one discarded", () => {
