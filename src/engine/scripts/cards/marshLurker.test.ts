// `Marsh Lurker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARSH_LURKER_SCRIPT } from './marshLurker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Marsh Lurker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MARSH_LURKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Marsh Lurker", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARSH_LURKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Marsh Lurker", () => {
  test("Sacrifice a Swamp: it gains fear until end of turn", () => {
