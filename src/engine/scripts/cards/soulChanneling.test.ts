// `Soul Channeling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOUL_CHANNELING_SCRIPT } from './soulChanneling';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Soul Channeling";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soul Channeling", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOUL_CHANNELING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Soul Channeling", () => {
  test("Pay 2 life: the enchanted creature regenerates from the Bolt", () => {
    const { g, life0, bearsB } = armed(0);
