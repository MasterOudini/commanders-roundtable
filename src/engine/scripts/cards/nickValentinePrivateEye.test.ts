// `Nick Valentine, Private Eye` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NICK_VALENTINE_PRIVATE_EYE_SCRIPT } from './nickValentinePrivateEye';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nick Valentine, Private Eye";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nick Valentine, Private Eye", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NICK_VALENTINE_PRIVATE_EYE_SCRIPT]),
...
describe("Nick Valentine, Private Eye", () => {
  test("Nick Valentine can't be blocked except by artifact creatures.: the block by a creature outside the exception (artifact creatures) is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("Whenever Nick Valentine or another artifact creature you control dies: 1 token made [dies]", () => {
    const { g, board0 } = armed(1);
    expect(onBoard(g)).toBe(board0 + 1 - 1);
  });

  test("Whenever Nick Valentine or another artifact creature you control dies: 1 token made [anotherCreatureDies]", () => {
