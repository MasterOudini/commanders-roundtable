// `Earth Servant` - the pump is COUNTED: zero of them is no bonus, and each one moves the
// derived power and toughness by the printed delta. Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { EARTH_SERVANT_SCRIPT } from './earthServant';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Earth Servant";
const COUNTED = "Mountain";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([EARTH_SERVANT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; subject: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([EARTH_SERVANT_SCRIPT]),
  });
  holdEverywhere(g);
...
function plus(a: [number | null, number | null], n: number): [number | null, number | null] {
  return [a[0] === null ? null : a[0] + 0 * n, a[1] === null ? null : a[1] + 1 * n];
}

describe("Earth Servant", () => {
  test("each Mountain counted moves the P/T by [0, 1], and one leaving takes it back", () => {
