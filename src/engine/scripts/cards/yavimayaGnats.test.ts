// `Yavimaya Gnats` - every printed ability proven in its own game: the cost's mark, the pump
...
import { YAVIMAYA_GNATS_SCRIPT } from './yavimayaGnats';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Yavimaya Gnats";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Yavimaya Gnats", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YAVIMAYA_GNATS_SCRIPT]),
  });
  holdEverywhere(g);
