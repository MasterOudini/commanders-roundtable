// `Steelbane Hydra` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STEELBANE_HYDRA_SCRIPT } from './steelbaneHydra';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Steelbane Hydra";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STEELBANE_HYDRA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Steelbane Hydra"], ["Cyclops of One-Eyed Pass", "Sol Ring"]],
    scripts: createRegistry([STEELBANE_HYDRA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Steelbane Hydra", () => {
  ...
  test("{2}{G}, Remove a +1/+1 counter from this creature: the vocabulary resolves \"Destroy target artifact or enchantment.\"", () => {
