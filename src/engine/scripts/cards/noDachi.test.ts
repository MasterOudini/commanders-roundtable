// `No-Dachi` - equipped to Grizzly Bears the creature gets +2/+0 and has firstStrike; the Cyclops is untouched;
...
import { NO_DACHI_SCRIPT } from './noDachi';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NO_DACHI_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([NO_DACHI_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("No-Dachi", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +2/+0 and has firstStrike; the Cyclops is untouched", () => {
