// `Will-o'-the-Wisp` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WILL_OTHE_WISP_SCRIPT } from './willOTheWisp';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Will-o'-the-Wisp";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Will-o'-the-Wisp", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WILL_OTHE_WISP_SCRIPT]),
  });
  holdEverywhere(g);
