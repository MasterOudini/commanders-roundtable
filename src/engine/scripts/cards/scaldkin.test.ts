// `Scaldkin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCALDKIN_SCRIPT } from './scaldkin';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scaldkin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCALDKIN_SCRIPT]),
  });
  holdEverywhere(g);
