// `Pus Kami` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PUS_KAMI_SCRIPT } from './pusKami';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pus Kami"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PUS_KAMI_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pus Kami", () => {
  test("{B}, Sacrifice this creature: the vocabulary resolves \"Destroy target nonblack creature.\"", () => {
