// `Countless Gears Renegade` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COUNTLESS_GEARS_RENEGADE_SCRIPT } from './countlessGearsRenegade';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Countless Gears Renegade";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Countless Gears Renegade", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COUNTLESS_GEARS_RENEGADE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Countless Gears Renegade", () => {
  test("Revolt — When this creature enters: 1 token made", () => {
