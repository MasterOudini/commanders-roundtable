// `Roaming Ghostlight` - every declared pick is accepted and it returns to hand; a permanent the clause
// excludes is refused (D299). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ROAMING_GHOSTLIGHT_SCRIPT } from './roamingGhostlight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Roaming Ghostlight";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Roaming Ghostlight"], ["Grizzly Bears", "Bile Urchin"]],
    scripts: createRegistry([ROAMING_GHOSTLIGHT_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD, 'graveyard');
  const target = put(g, 'p2', "Grizzly Bears");
  const wrong = put(g, 'p2', "Bile Urchin");
  settle(g);
...
describe("Roaming Ghostlight", () => {
  test("Grizzly Bears is accepted and it returns to hand", () => {
  ...
  test("Bile Urchin is refused (D299)", () => {
    const { g, wrong } = armed();
