// `Spinal Villain` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SPINAL_VILLAIN_SCRIPT } from './spinalVillain';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spinal Villain";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Spinal Villain"], ["Merfolk of the Pearl Trident", "Grizzly Bears"]],
    scripts: createRegistry([SPINAL_VILLAIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spinal Villain", () => {
  test("Merfolk of the Pearl Trident is a legal target and it is destroyed", () => {
