// `Tenured Concocter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TENURED_CONCOCTER_SCRIPT } from './tenuredConcocter';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tenured Concocter";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TENURED_CONCOCTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tenured Concocter"], ["Cyclops of One-Eyed Pass", "Giant Growth"]],
    scripts: createRegistry([TENURED_CONCOCTER_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 1) {
    condOff = pt(g, self)[0] === 4 && pt(g, self)[1] === 5;
...
describe("Tenured Concocter", () => {
  test("Whenever this creature becomes the target of a spell or ability an opponent controls: a card is drawn", () => {
    const { g, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1);
  });

  test("`Tenured Concocter` - as long as you gained life this turn: absent while it is unmet, read once it holds", () => {
