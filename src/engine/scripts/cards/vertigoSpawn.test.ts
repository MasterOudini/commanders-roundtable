// `Vertigo Spawn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VERTIGO_SPAWN_SCRIPT } from './vertigoSpawn';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vertigo Spawn";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vertigo Spawn"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VERTIGO_SPAWN_SCRIPT]),
...
describe("Vertigo Spawn", () => {
  test("Whenever this creature blocks a creature: the vocabulary resolves \"Tap target creature. That creature doesn't untap during its controller's next untap step.\"", () => {
    const { g, no } = armed(0);
