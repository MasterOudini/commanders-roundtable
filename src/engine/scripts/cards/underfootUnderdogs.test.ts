// `Underfoot Underdogs` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNDERFOOT_UNDERDOGS_SCRIPT } from './underfootUnderdogs';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Underfoot Underdogs";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Underfoot Underdogs", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNDERFOOT_UNDERDOGS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Underfoot Underdogs", () => {
  ...
  test("{1}, {T}: the vocabulary resolves \"Target creature you control with power 2 or less can't be blocked this turn.\"", () => {
