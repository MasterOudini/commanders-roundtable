// `Johtull Wurm` - a VARIABLE PUMP (D387): the delta is COUNTED at each derive
...
import { JOHTULL_WURM } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(JOHTULL_WURM, "Whenever this creature becomes blocked, it gets -2/-1 until end of turn for each creature blocking it beyond the first.");
...
export const JOHTULL_WURM_SCRIPT: CardScript = {
  oracleId: JOHTULL_WURM.oracleId,
  name: JOHTULL_WURM.name,
  triggers: [
    {
      abilityId: 'pump-0',
      ...
      label: () => "Johtull Wurm - blocked, it pumped until end of turn",
      resolve: (ctx, self): readonly EventBody[] => {
        ...
        if (n === 0) return [];
        return [{ t: 'PtModifiedUntilEndOfTurn', card: who, power: -2 * n, toughness: -1 * n }];
      },
    },
  ],
};
