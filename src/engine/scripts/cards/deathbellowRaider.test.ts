// `Deathbellow Raider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEATHBELLOW_RAIDER_SCRIPT } from './deathbellowRaider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deathbellow Raider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deathbellow Raider", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEATHBELLOW_RAIDER_SCRIPT]),
  });
  holdEverywhere(g);
