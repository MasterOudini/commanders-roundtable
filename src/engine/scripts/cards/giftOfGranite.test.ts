// `Gift of Granite` - cast on Grizzly Bears it attaches and the enchanted creature gets +0/+2;
...
import { GIFT_OF_GRANITE_SCRIPT } from './giftOfGranite';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gift of Granite";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GIFT_OF_GRANITE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GIFT_OF_GRANITE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gift of Granite", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +0/+2; Cyclops of One-Eyed Pass is untouched", () => {
