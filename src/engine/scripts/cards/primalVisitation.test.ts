// `Primal Visitation` - cast on Grizzly Bears it attaches and the enchanted creature gets +3/+3 and has haste;
...
import { PRIMAL_VISITATION_SCRIPT } from './primalVisitation';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Primal Visitation";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PRIMAL_VISITATION_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([PRIMAL_VISITATION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Primal Visitation", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +3/+3 and has haste; Cyclops of One-Eyed Pass is untouched", () => {
