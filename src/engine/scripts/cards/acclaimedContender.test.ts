// `Acclaimed Contender` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Acclaimed Contender", "Accorder's Shield", "Accorder's Shield", "Agent of Stromgald"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ACCLAIMED_CONTENDER_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const lookPicksL0_etb = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Accorder's Shield" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL0_etb.length, "Accorder's Shield is dealt for the look").toBeGreaterThan(0);
...
describe("Acclaimed Contender", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top five cards of your library. You may reveal a Knight, Aura, Equipment, or legendary artifact card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
