// `Amphibious Kavu` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AMPHIBIOUS_KAVU_SCRIPT } from './amphibiousKavu';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Amphibious Kavu";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([AMPHIBIOUS_KAVU_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Amphibious Kavu"], ["Cyclops of One-Eyed Pass", "Coral Eel"]],
    scripts: createRegistry([AMPHIBIOUS_KAVU_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const gift2 = put(g, 'p2', "Coral Eel");
  settle(g);
...
describe("Amphibious Kavu", () => {
  test("Whenever this creature blocks or becomes blocked by one or more blue and/or black creatures: it gets +3/+3 until end of turn", () => {
