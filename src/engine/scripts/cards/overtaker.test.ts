// `Overtaker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OVERTAKER_SCRIPT } from './overtaker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Overtaker";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; energy0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; disc0: InstanceId[] };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([OVERTAKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Overtaker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OVERTAKER_SCRIPT]),
  ...
  return { g, self, no, life0, energy0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, disc0 };
}

describe("Overtaker", () => {
  test("{3}{U}, {T}, Discard a card: the vocabulary resolves \"Untap target creature and gain control of it until end of turn. That creature gains haste until end of turn.\"", () => {
    const { g, self, no, board0, disc0 } = armed(0);
