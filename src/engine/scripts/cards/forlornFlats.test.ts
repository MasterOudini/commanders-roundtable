// `Forlorn Flats` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FORLORN_FLATS_SCRIPT } from './forlornFlats';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Forlorn Flats";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Forlorn Flats"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FORLORN_FLATS_SCRIPT]),
  });
  holdEverywhere(g);
