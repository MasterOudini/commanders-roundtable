// `Trudge Garden` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRUDGE_GARDEN_SCRIPT } from './trudgeGarden';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trudge Garden";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trudge Garden", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRUDGE_GARDEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Trudge Garden", () => {
  test("Whenever you gain life: the vocabulary resolves \"You may pay {2}. If you do, create a 4/4 green Fungus Beast creature token with trample.\" - the price is declined", () => {
  ...
  test("Whenever you gain life: the vocabulary resolves \"You may pay {2}. If you do, create a 4/4 green Fungus Beast creature token with trample.\" - the price is paid", () => {
