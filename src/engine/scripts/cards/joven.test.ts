// `Joven` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { JOVEN_SCRIPT } from './joven';
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Joven"], ["Sol Ring", "Grizzly Bears"]],
    scripts: createRegistry([JOVEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Joven", () => {
  test("Sol Ring is a legal target and it is destroyed", () => {
