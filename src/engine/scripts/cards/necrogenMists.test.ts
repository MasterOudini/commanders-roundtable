// `Necrogen Mists` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NECROGEN_MISTS_SCRIPT } from './necrogenMists';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Necrogen Mists";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Necrogen Mists"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NECROGEN_MISTS_SCRIPT]),
  });
  holdEverywhere(g);
