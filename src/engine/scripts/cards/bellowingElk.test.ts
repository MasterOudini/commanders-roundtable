// `Bellowing Elk` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BELLOWING_ELK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bellowing Elk", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BELLOWING_ELK_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  let condOff = false;
  if (which === 0) {
    condOff = !kw(g, self).has("trample") && !kw(g, self).has("indestructible");
...
describe("Bellowing Elk", () => {
  test("`Bellowing Elk` - as long as you had another creature enter the battlefield under your control this turn: absent while it is unmet, read once it holds", () => {
