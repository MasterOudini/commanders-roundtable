// `Kavu Primarch` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KAVU_PRIMARCH_SCRIPT } from './kavuPrimarch';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kavu Primarch";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KAVU_PRIMARCH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kavu Primarch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KAVU_PRIMARCH_SCRIPT]),
  });
  holdEverywhere(g);
