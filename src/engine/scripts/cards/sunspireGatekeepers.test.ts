// `Sunspire Gatekeepers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUNSPIRE_GATEKEEPERS_SCRIPT } from './sunspireGatekeepers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunspire Gatekeepers";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sunspire Gatekeepers", "Azorius Guildgate", "Azorius Guildgate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUNSPIRE_GATEKEEPERS_SCRIPT]),
  });
  holdEverywhere(g);
