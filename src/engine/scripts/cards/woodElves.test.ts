// `Wood Elves` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wood Elves", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WOOD_ELVES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wood Elves", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a Forest card, put that card onto the battlefield, then shuffle.\"", () => {
