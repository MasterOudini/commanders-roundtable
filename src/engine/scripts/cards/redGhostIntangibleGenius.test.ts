// `Red Ghost, Intangible Genius` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RED_GHOST_INTANGIBLE_GENIUS_SCRIPT } from './redGhostIntangibleGenius';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Red Ghost, Intangible Genius";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Red Ghost, Intangible Genius", "Divination", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RED_GHOST_INTANGIBLE_GENIUS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Red Ghost, Intangible Genius", () => {
  test("Red Ghost can't be blocked.: the block is refused", () => {
