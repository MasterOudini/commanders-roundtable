// `Knowledge Seeker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KNOWLEDGE_SEEKER_SCRIPT } from './knowledgeSeeker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Knowledge Seeker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Knowledge Seeker", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KNOWLEDGE_SEEKER_SCRIPT]),
  });
  holdEverywhere(g);
