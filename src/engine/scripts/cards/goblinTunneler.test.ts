// `Goblin Tunneler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_TUNNELER_SCRIPT } from './goblinTunneler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Tunneler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Tunneler"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([GOBLIN_TUNNELER_SCRIPT]),
  });
  holdEverywhere(g);
