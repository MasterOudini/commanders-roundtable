// `Daggerfang Duo` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAGGERFANG_DUO_SCRIPT } from './daggerfangDuo';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Daggerfang Duo";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Daggerfang Duo"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAGGERFANG_DUO_SCRIPT]),
  });
  holdEverywhere(g);
