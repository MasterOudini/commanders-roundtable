// `Revered Dead` - every printed ability proven in its own game: the cost's mark, the pump
...
import { REVERED_DEAD_SCRIPT } from './reveredDead';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Revered Dead";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Revered Dead", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([REVERED_DEAD_SCRIPT]),
  });
  holdEverywhere(g);
