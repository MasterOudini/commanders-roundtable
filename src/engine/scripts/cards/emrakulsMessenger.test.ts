// `Emrakul's Messenger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EMRAKULS_MESSENGER_SCRIPT } from './emrakulsMessenger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Emrakul's Messenger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Emrakul's Messenger", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EMRAKULS_MESSENGER_SCRIPT]),
...
describe("Emrakul's Messenger", () => {
  test("Whenever you draw your second card each turn: the vocabulary resolves \"Create a 0/1 colorless Eldrazi Spawn creature token with \\\"Sacrifice this token: Add {C}.\\\"\"", () => {
