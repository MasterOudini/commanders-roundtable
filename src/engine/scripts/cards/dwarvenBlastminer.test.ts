// `Dwarven Blastminer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DWARVEN_BLASTMINER_SCRIPT } from './dwarvenBlastminer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dwarven Blastminer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dwarven Blastminer"], ["Cyclops of One-Eyed Pass", "Tundra"]],
    scripts: createRegistry([DWARVEN_BLASTMINER_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtA0_0 = put(g, 'p2', "Tundra");
  settle(g);
...
describe("Dwarven Blastminer", () => {
  test("{2}{R}, {T}: the vocabulary resolves \"Destroy target nonbasic land.\"", () => {
