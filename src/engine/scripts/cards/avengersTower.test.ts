// `Avengers Tower` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Avengers Tower", "Big Bertha", "Big Bertha"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AVENGERS_TOWER_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    ...
    settle(g);
    const lookPicksA2 = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Big Bertha" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksA2.length, "Big Bertha is dealt for the look").toBeGreaterThan(0);
    for (const id of lookPicksA2) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: id, to: { kind: 'library', player: 'p1' }, placement: 'top' }));
...
describe("Avengers Tower", () => {
  test("{4}, {T}: the vocabulary resolves \"Look at the top three cards of your library. You may reveal a Hero card from among them and put it into your hand. Put the rest on the bottom of your library in any order.\"", () => {
