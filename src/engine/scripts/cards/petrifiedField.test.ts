// `Petrified Field` - the named target is accepted and it returns to hand; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { PETRIFIED_FIELD_SCRIPT } from './petrifiedField';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Petrified Field";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Petrified Field", "Forest", "Grizzly Bears"], ["Grizzly Bears"]],
    scripts: createRegistry([PETRIFIED_FIELD_SCRIPT]),
  });
  holdEverywhere(g);
