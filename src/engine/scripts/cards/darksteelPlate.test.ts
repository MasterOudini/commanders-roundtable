// `Darksteel Plate` - equipped to Grizzly Bears the creature has indestructible; the Cyclops is untouched;
...
import { DARKSTEEL_PLATE_SCRIPT } from './darksteelPlate';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Darksteel Plate";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([DARKSTEEL_PLATE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([DARKSTEEL_PLATE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Darksteel Plate", () => {
  test("equipped to Grizzly Bears: attached, and the creature has indestructible; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(kw(g, host).has("indestructible")).toBe(true);
    expect(kw(g, other).has("indestructible")).toBe(false);
  });
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("indestructible")).toBe(false);
  });
