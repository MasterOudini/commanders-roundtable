// `Horror of the Broken Lands` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HORROR_OF_THE_BROKEN_LANDS_SCRIPT } from './horrorOfTheBrokenLands';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Horror of the Broken Lands";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HORROR_OF_THE_BROKEN_LANDS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Horror of the Broken Lands"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HORROR_OF_THE_BROKEN_LANDS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Horror of the Broken Lands", () => {
  test("Whenever you cycle or discard another card: it gets +2/+1 until end of turn", () => {
