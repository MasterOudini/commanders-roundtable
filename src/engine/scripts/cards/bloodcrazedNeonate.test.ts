// `Bloodcrazed Neonate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODCRAZED_NEONATE_SCRIPT } from './bloodcrazedNeonate';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodcrazed Neonate";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodcrazed Neonate", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODCRAZED_NEONATE_SCRIPT]),
  });
  holdEverywhere(g);
