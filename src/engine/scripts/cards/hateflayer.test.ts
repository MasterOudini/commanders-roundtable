// `Hateflayer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HATEFLAYER_SCRIPT } from './hateflayer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hateflayer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hateflayer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HATEFLAYER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hateflayer", () => {
  test("{2}{R}, {Q}: the vocabulary resolves \"~ deals damage equal to its power to any target.\"", () => {
