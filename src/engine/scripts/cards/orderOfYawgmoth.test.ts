// `Order of Yawgmoth` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORDER_OF_YAWGMOTH_SCRIPT } from './orderOfYawgmoth';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Order of Yawgmoth";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Order of Yawgmoth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORDER_OF_YAWGMOTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Order of Yawgmoth", () => {
  test("Whenever this creature deals damage to a player: the vocabulary resolves \"Target player discards a card.\"", () => {
    const { g } = armed(0);
