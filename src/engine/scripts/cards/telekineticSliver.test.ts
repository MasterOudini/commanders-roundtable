// `Telekinetic Sliver` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { TELEKINETIC_SLIVER_SCRIPT } from './telekineticSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Telekinetic Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Telekinetic Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TELEKINETIC_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Telekinetic Sliver", () => {
  test("grants {T}: Tap target permanent. to Metallic Sliver, and to nothing outside the scope", () => {
