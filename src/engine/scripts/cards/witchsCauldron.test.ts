// `Witch's Cauldron` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WITCHS_CAULDRON_SCRIPT } from './witchsCauldron';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Witch's Cauldron";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Witch's Cauldron", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WITCHS_CAULDRON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Witch's Cauldron", () => {
  test("{1}{B}, {T}, Sacrifice a creature: the vocabulary resolves \"You gain 1 life and draw a card.\"", () => {
    const { g, self, life0, hand0, fodder0 } = armed(0);
