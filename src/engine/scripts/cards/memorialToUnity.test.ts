// `Memorial to Unity` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Memorial to Unity", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MEMORIAL_TO_UNITY_SCRIPT]),
  ...
  if (which === 0) {
    ...
    settle(g);
    const lookPicksA1 = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Grizzly Bears" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksA1.length, "Grizzly Bears is dealt for the look").toBeGreaterThan(0);
    for (const id of lookPicksA1) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: id, to: { kind: 'library', player: 'p1' }, placement: 'top' }));
...
describe("Memorial to Unity", () => {
  test("{2}{G}, {T}, Sacrifice this land: the vocabulary resolves \"Look at the top five cards of your library. You may reveal a creature card from among them and put it into your hand. Then put the rest on the bottom of your library in a random order.\"", () => {
