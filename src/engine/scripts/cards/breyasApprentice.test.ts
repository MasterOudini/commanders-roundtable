// `Breya's Apprentice` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BREYAS_APPRENTICE_SCRIPT } from './breyasApprentice';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Breya's Apprentice";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BREYAS_APPRENTICE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Breya's Apprentice", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BREYAS_APPRENTICE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Breya's Apprentice", () => {
  test("When this creature enters: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("{T}, Sacrifice an artifact: choosing \"Exile the top card of your library. Until the end of your next turn, you may play that card.\", the vocabulary resolves \"Exile the top card of your library. Until the end of your next turn, you may play that card.\"", () => {
  ...
  test("{T}, Sacrifice an artifact: choosing \"Target creature gets +2/+0 until end of turn.\", pumpTarget", () => {
    const { g, self, no, fodder0 } = armed(2);
