// `Storm God's Oracle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORM_GODS_ORACLE_SCRIPT } from './stormGodsOracle';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Storm God's Oracle";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STORM_GODS_ORACLE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Storm God's Oracle"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORM_GODS_ORACLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Storm God's Oracle", () => {
  test("{1}: it gets +1/-1 until end of turn", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([2, 2]);
  });

  test("When this creature dies: it deals 3 damage to the declared creature", () => {
    const { g, no } = armed(1);
