// `Golgari Findbroker` - the named target is accepted and it returns to hand; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { GOLGARI_FINDBROKER_SCRIPT } from './golgariFindbroker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Golgari Findbroker";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Golgari Findbroker", "Grizzly Bears", "Lightning Bolt"], ["Grizzly Bears"]],
    scripts: createRegistry([GOLGARI_FINDBROKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Golgari Findbroker", () => {
  ...
  test("Lightning Bolt is refused (D294)", () => {
    const { g, wrong } = armed();
