// `Stronghold Zeppelin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STRONGHOLD_ZEPPELIN_SCRIPT } from './strongholdZeppelin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stronghold Zeppelin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stronghold Zeppelin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STRONGHOLD_ZEPPELIN_SCRIPT]),
  });
  holdEverywhere(g);
