// `Soldevi Simulacrum` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOLDEVI_SIMULACRUM_SCRIPT } from './soldeviSimulacrum';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SOLDEVI_SIMULACRUM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soldevi Simulacrum", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOLDEVI_SIMULACRUM_SCRIPT]),
  });
  holdEverywhere(g);
