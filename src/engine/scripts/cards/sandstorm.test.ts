// `Sandstorm` — 1 to each attacker mid-combat: the attacking 1/1 dies,
// the bystander at home is untouched.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SANDSTORM_SCRIPT } from './sandstorm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

function settle(g: Game): void {
  advanceUntil(g, (s) => s.stack.length === 0 && s.pendingTriggers.length === 0, 20_000);
}

function stormed(): { g: Game; attacker: InstanceId; home: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [
      ['Sandstorm'],
      ['Aysen Bureaucrats', 'Grizzly Bears'],
    ],
    scripts: createRegistry([SANDSTORM_SCRIPT]),
  });
  const attacker = put(g, 'p2', 'Aysen Bureaucrats');
  const home = put(g, 'p2', 'Grizzly Bears');
  settle(g);
  holdEverywhere(g);
  advanceUntil(
    g,
    (s) => s.turn.activePlayer === 'p2' && s.priority.awaiting?.kind === 'declareAttackers',
    60_000,
  );
  must(
    g.submit({
      t: 'DeclareAttackers',
      player: 'p2',
      attackers: [{ card: attacker, defender: { kind: 'player', id: 'p1' } }],
    }),
  );
  advanceUntil(
    g,
    (s) => s.priority.player === 'p1' && (s.combat?.attackers.length ?? 0) > 0,
    20_000,
  );
  const spell = put(g, 'p1', 'Sandstorm', 'hand');
  must(g.submit({ t: 'ManualAddMana', player: 'p1', target: 'p1', symbol: 'G', amount: 1 }));
  must(g.submit({ t: 'CastSpell', player: 'p1', card: spell }));
  settle(g);
  return { g, attacker, home };
}

describe('Sandstorm', () => {
  test('the attacking 1/1 dies; the creature at home is untouched', () => {
    const { g, attacker, home } = stormed();
    expect(g.state.cards[attacker]?.zone.kind).toBe('graveyard');
    expect(g.state.cards[home]?.zone.kind).toBe('battlefield');
  });

  test('replays to the same hash', () => {
    const { g } = stormed();
    advanceUntil(g, (s) => s.turn.turnNumber >= 3, 60_000);
    expect(stateHash(replay(g.log, g.seed))).toBe(g.hash());
  });
});
