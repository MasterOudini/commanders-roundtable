// `Accorder's Shield` - equipped to Grizzly Bears the creature gets +0/+3 and has vigilance; the Cyclops is untouched;
...
import { ACCORDERS_SHIELD_SCRIPT } from './accordersShield';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Accorder's Shield";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ACCORDERS_SHIELD_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ACCORDERS_SHIELD_SCRIPT]),
  });
  holdEverywhere(g);
