// `Thunderwolf Cavalry` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THUNDERWOLF_CAVALRY_SCRIPT } from './thunderwolfCavalry';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thunderwolf Cavalry";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thunderwolf Cavalry", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THUNDERWOLF_CAVALRY_SCRIPT]),
...
describe("Thunderwolf Cavalry", () => {
  test("Crushing Teeth — Whenever this creature deals combat damage to a player: each of its controller's creatures gets 1 +1/+1 counter", () => {
