// `Facet Reader` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FACET_READER_SCRIPT } from './facetReader';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Facet Reader";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Facet Reader"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FACET_READER_SCRIPT]),
  });
  holdEverywhere(g);
