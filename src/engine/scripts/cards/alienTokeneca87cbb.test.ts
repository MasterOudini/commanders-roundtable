// `Alien` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ALIEN_TOKENECA87CBB_SCRIPT } from './alienTokeneca87cbb';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([ALIEN_TOKENECA87CBB_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: ALIEN_ECA87CBB_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === ALIEN_ECA87CBB_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
...
describe("Alien", () => {
  test("This token attacks each combat if able.: an empty declaration is refused, then it attacks", () => {
