// `Greenseeker` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Greenseeker", "Grizzly Bears", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GREENSEEKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Greenseeker", () => {
  test("{G}, {T}, Discard a card: the vocabulary resolves \"Search your library for a basic land card, reveal it, put it into your hand, then shuffle.\"", () => {
