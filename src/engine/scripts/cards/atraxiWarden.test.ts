// `Atraxi Warden` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ATRAXI_WARDEN_SCRIPT } from './atraxiWarden';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Atraxi Warden";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Atraxi Warden"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ATRAXI_WARDEN_SCRIPT]),
...
describe("Atraxi Warden", () => {
  test("When this creature enters: the vocabulary resolves \"Exile up to one target tapped creature.\"", () => {
    const { g, no } = armed(0);
