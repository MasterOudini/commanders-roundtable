// `Towashi Songshaper` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TOWASHI_SONGSHAPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Towashi Songshaper", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TOWASHI_SONGSHAPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Towashi Songshaper", () => {
  test("Whenever another artifact you control enters: it gets +1/+0 until end of turn", () => {
