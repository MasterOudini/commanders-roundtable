// `Akroan Sergeant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AKROAN_SERGEANT_SCRIPT } from './akroanSergeant';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Akroan Sergeant";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([AKROAN_SERGEANT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Akroan Sergeant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AKROAN_SERGEANT_SCRIPT]),
  });
  holdEverywhere(g);
