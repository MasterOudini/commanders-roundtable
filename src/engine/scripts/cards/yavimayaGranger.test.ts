// `Yavimaya Granger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { YAVIMAYA_GRANGER_SCRIPT } from './yavimayaGranger';
import { advanceUntil as walkUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';
import type { GameState } from '../../types/state';

const CARD = "Yavimaya Granger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Yavimaya Granger", "Forest", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YAVIMAYA_GRANGER_SCRIPT]),
...
describe("Yavimaya Granger", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a basic land card, put that card onto the battlefield tapped, then shuffle.\"", () => {
