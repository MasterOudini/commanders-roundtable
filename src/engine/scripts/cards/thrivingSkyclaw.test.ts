// `Thriving Skyclaw` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THRIVING_SKYCLAW_SCRIPT } from './thrivingSkyclaw';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thriving Skyclaw";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thriving Skyclaw"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THRIVING_SKYCLAW_SCRIPT]),
...
describe("Thriving Skyclaw", () => {
  test("When this creature enters: the vocabulary resolves \"You get {E}{E}{E}.\"", () => {
    const { g, energy0 } = armed(0);
    expect(g.state.players.p1?.energy).toBe(energy0 + 3);
  });

  test("Whenever this creature attacks: the vocabulary resolves \"You may pay {E}{E}{E}. If you do, put a +1/+1 counter on ~.\" - the price is declined", () => {
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"You may pay {E}{E}{E}. If you do, put a +1/+1 counter on ~.\" - the price is paid", () => {
