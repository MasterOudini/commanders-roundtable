// `War-Name Aspirant` - a static entersWithCounters, a static cantBeBlockedByPower
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { WAR_NAME_ASPIRANT } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(WAR_NAME_ASPIRANT, "Raid — This creature enters with a +1/+1 counter on it if you attacked this turn.\nThis creature can't be blocked by creatures with power 1 or less.");
...
export const WAR_NAME_ASPIRANT_SCRIPT: CardScript = {
  oracleId: WAR_NAME_ASPIRANT.oracleId,
  name: WAR_NAME_ASPIRANT.name,
  combat: [
    {
      abilityId: 'cantBeBlockedByPower-1',
      text: LINES[1] as string,
      activeZones: ['battlefield'],
      canBlock: (ctx, self, blocker, attacker) => attacker !== self || (ctx.derive(blocker).power ?? 0) > 1,
    },
  ],
  replacements: [
    {
