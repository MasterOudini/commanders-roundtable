// `Courageous Outrider` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Courageous Outrider", "Eager Cadet", "Eager Cadet"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COURAGEOUS_OUTRIDER_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const lookPicksL0_etb = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Eager Cadet" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL0_etb.length, "Eager Cadet is dealt for the look").toBeGreaterThan(0);
...
describe("Courageous Outrider", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top four cards of your library. You may reveal a Human card from among them and put it into your hand. Put the rest on the bottom of your library in any order.\"", () => {
    const { g } = armed(0);
    ...
    { const shown = (g.state.zones.library.p1 ?? []).filter((id) => g.state.cards[id]?.revealedTo.includes('p1'));
      const found = shown.find((id) => nameOf(g, id) === "Eager Cadet");
      expect(found, "Eager Cadet is not among the revealed cards").toBeDefined();
