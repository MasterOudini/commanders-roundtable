// `Horizon Spellbomb` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HORIZON_SPELLBOMB_SCRIPT } from './horizonSpellbomb';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Horizon Spellbomb";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; bearsB: InstanceId; payL1_p1_0: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Horizon Spellbomb", "Grizzly Bears", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HORIZON_SPELLBOMB_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const payL1_p1_0 = put(g, 'p1', "Forest");
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, bearsB, payL1_p1_0 };
}

describe("Horizon Spellbomb", () => {
  test("{2}, {T}, Sacrifice this artifact: the vocabulary resolves \"Search your library for a basic land card, reveal it, put it into your hand, then shuffle.\"", () => {
  ...
  test("When this artifact is put into a graveyard from the battlefield: the vocabulary resolves \"You may pay {G}. If you do, draw a card.\" - the price is declined", () => {
    const { g, payL1_p1_0 } = armed(1);
  ...
  test("When this artifact is put into a graveyard from the battlefield: the vocabulary resolves \"You may pay {G}. If you do, draw a card.\" - the price is paid", () => {
    const { g, payL1_p1_0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
