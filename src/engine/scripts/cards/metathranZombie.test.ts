// `Metathran Zombie` - every printed ability proven in its own game: the cost's mark, the pump
...
import { METATHRAN_ZOMBIE_SCRIPT } from './metathranZombie';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Metathran Zombie";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Metathran Zombie", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([METATHRAN_ZOMBIE_SCRIPT]),
  });
  holdEverywhere(g);
