// `Snorting Gahr` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SNORTING_GAHR_SCRIPT } from './snortingGahr';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Snorting Gahr";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SNORTING_GAHR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Snorting Gahr"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SNORTING_GAHR_SCRIPT]),
  });
  holdEverywhere(g);
