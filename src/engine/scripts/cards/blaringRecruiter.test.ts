// `Blaring Recruiter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLARING_RECRUITER_SCRIPT } from './blaringRecruiter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blaring Recruiter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blaring Recruiter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLARING_RECRUITER_SCRIPT]),
