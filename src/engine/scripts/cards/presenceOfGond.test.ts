// `Presence of Gond` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { PRESENCE_OF_GOND_SCRIPT } from './presenceOfGond';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Presence of Gond";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Presence of Gond","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRESENCE_OF_GOND_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Presence of Gond", () => {
  test("grants {T}: Create a 1/1 green Elf Warrior creature token. to Grizzly Bears, and to nothing outside the scope", () => {
