// `Craven Knight` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRAVEN_KNIGHT_SCRIPT } from './cravenKnight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Craven Knight";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Craven Knight", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRAVEN_KNIGHT_SCRIPT]),
  });
  holdEverywhere(g);
