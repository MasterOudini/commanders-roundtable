// `Rabid Elephant` - the pump is COUNTED: zero of them is no bonus, and each one moves the
// derived power and toughness by the printed delta. Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { RABID_ELEPHANT_SCRIPT } from './rabidElephant';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rabid Elephant";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RABID_ELEPHANT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; subject: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([RABID_ELEPHANT_SCRIPT]),
  });
  holdEverywhere(g);
...
function blockedBy(k: number): { g: Game; subject: InstanceId } {
  ...
  test("blocked by one creature it is pumped by [2, 2] x 1; by two, x 2", () => {
    const one = blockedBy(1);
    expect(pt(one.g, one.subject)).toEqual(plus([3, 4], 1));
    const two = blockedBy(2);
    expect(pt(two.g, two.subject)).toEqual(plus([3, 4], 2));
