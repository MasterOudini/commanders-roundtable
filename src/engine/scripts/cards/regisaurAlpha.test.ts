// `Regisaur Alpha` - every printed ability proven in its own game: the cost's mark, the pump
...
import { REGISAUR_ALPHA_SCRIPT } from './regisaurAlpha';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Regisaur Alpha";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([REGISAUR_ALPHA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Regisaur Alpha", "Raptor Companion"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([REGISAUR_ALPHA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Regisaur Alpha", () => {
  test("Other Dinosaurs you control have haste.: only the Dinosaur reads it", () => {
