// `Magnetic Theft` - a scripted spell: the target clause is the parser's and the validator's
// (D294's adjectives), the rider is this script's. Generated from one table row (D295).

import { MAGNETIC_THEFT } from '../../../data/fixtures/engineCards';
...
const TEXT = printed(MAGNETIC_THEFT, "Attach target Equipment to target creature. (Control of the Equipment doesn't change.)");

export const MAGNETIC_THEFT_SCRIPT: CardScript = {
  oracleId: MAGNETIC_THEFT.oracleId,
  name: MAGNETIC_THEFT.name,
  spell: {
    text: TEXT,
