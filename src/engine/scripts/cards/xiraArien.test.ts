// `Xira Arien` - every printed ability proven in its own game: the cost's mark, the pump
...
import { XIRA_ARIEN_SCRIPT } from './xiraArien';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Xira Arien";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Xira Arien"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([XIRA_ARIEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Xira Arien", () => {
  test("{B}{R}{G}, {T}: the vocabulary resolves \"Target player draws a card.\"", () => {
