// `Lashknife` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LASHKNIFE_SCRIPT } from './lashknife';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lashknife";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([LASHKNIFE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lashknife", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LASHKNIFE_SCRIPT]),
  });
  holdEverywhere(g);
