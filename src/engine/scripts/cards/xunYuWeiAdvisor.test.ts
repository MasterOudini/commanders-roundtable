// `Xun Yu, Wei Advisor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { XUN_YU_WEI_ADVISOR_SCRIPT } from './xunYuWeiAdvisor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Xun Yu, Wei Advisor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([XUN_YU_WEI_ADVISOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Xun Yu, Wei Advisor", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([XUN_YU_WEI_ADVISOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Xun Yu, Wei Advisor", () => {
  test("{T}: the vocabulary resolves \"Target creature you control gets +2/+0 until end of turn.\" - refused unless during your turn, before attackers are declared", () => {
    const { g, self, condRefused, vtA0_0 } = armed(0);
