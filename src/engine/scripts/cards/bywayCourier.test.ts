// `Byway Courier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BYWAY_COURIER_SCRIPT } from './bywayCourier';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Byway Courier";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Byway Courier"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BYWAY_COURIER_SCRIPT]),
  });
  holdEverywhere(g);
