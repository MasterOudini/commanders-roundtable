// `Ivy Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IVY_ELEMENTAL_SCRIPT } from './ivyElemental';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ivy Elemental";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([IVY_ELEMENTAL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ivy Elemental"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IVY_ELEMENTAL_SCRIPT]),
  });
  holdEverywhere(g);
