// `Zephyr Scribe` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZEPHYR_SCRIBE_SCRIPT } from './zephyrScribe';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zephyr Scribe";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zephyr Scribe", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZEPHYR_SCRIBE_SCRIPT]),
  });
  holdEverywhere(g);
