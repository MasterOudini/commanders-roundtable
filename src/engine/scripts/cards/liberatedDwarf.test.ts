// `Liberated Dwarf` - the named target is accepted and the pump lands; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { LIBERATED_DWARF_SCRIPT } from './liberatedDwarf';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Liberated Dwarf";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Liberated Dwarf"], ["Grizzly Bears", "Vampire Nighthawk"]],
    scripts: createRegistry([LIBERATED_DWARF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Liberated Dwarf", () => {
  test("Grizzly Bears is a legal target and the pump lands", () => {
    ...
    settle(g);
    const d = deps(createRegistry([LIBERATED_DWARF_SCRIPT]));
    const got = derive(g.state, d.oracle, d.scripts, target);
    expect([got.power, got.toughness]).toEqual([3, 2]);
