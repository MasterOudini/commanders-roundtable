// `Dapper Shieldmate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAPPER_SHIELDMATE_SCRIPT } from './dapperShieldmate';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dapper Shieldmate";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DAPPER_SHIELDMATE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dapper Shieldmate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAPPER_SHIELDMATE_SCRIPT]),
...
describe("Dapper Shieldmate", () => {
  ...
  test("`Dapper Shieldmate` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
