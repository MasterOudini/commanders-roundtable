// `Jinxed Idol` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JINXED_IDOL_SCRIPT } from './jinxedIdol';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jinxed Idol";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jinxed Idol", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JINXED_IDOL_SCRIPT]),
...
describe("Jinxed Idol", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"This artifact deals 2 damage to you.\"", () => {
    const { g, life0 } = armed(0);
    expect(g.state.players.p1?.life).toBe(life0 + -2);
  });

  test("Sacrifice a creature: the vocabulary resolves \"Target opponent gains control of this artifact.\"", () => {
