// `Necroduality` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Necroduality", "Grizzly Bears", "Gutter Skulk"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NECRODUALITY_SCRIPT]),
...
describe("Necroduality", () => {
  test("Whenever a nontoken Zombie you control enters: the vocabulary resolves \"Create a token that's a copy of target creature.\"", () => {
