// `Frontline Rebel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FRONTLINE_REBEL_SCRIPT } from './frontlineRebel';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frontline Rebel";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frontline Rebel", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FRONTLINE_REBEL_SCRIPT]),
  });
  holdEverywhere(g);
