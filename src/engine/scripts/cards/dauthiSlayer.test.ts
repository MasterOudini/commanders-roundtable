// `Dauthi Slayer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAUTHI_SLAYER_SCRIPT } from './dauthiSlayer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dauthi Slayer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dauthi Slayer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAUTHI_SLAYER_SCRIPT]),
  });
  holdEverywhere(g);
