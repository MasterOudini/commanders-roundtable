// `Daggersail Aeronaut` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DAGGERSAIL_AERONAUT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Daggersail Aeronaut"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAGGERSAIL_AERONAUT_SCRIPT]),
...
describe("Daggersail Aeronaut", () => {
  test("`Daggersail Aeronaut` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
