// `Vaporkin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VAPORKIN_SCRIPT } from './vaporkin';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vaporkin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VAPORKIN_SCRIPT]),
  });
  holdEverywhere(g);
