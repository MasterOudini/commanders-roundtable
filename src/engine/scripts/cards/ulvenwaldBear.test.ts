// `Ulvenwald Bear` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ULVENWALD_BEAR_SCRIPT } from './ulvenwaldBear';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ulvenwald Bear";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ulvenwald Bear", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ULVENWALD_BEAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ulvenwald Bear", () => {
  test("Morbid — When this creature enters: the vocabulary resolves \"Put two +1/+1 counters on target creature.\"", () => {
    const { g, no } = armed(0);
