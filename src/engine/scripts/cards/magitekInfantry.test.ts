// `Magitek Infantry` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MAGITEK_INFANTRY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Magitek Infantry", "Magitek Infantry", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAGITEK_INFANTRY_SCRIPT]),
...
describe("Magitek Infantry", () => {
  test("`Magitek Infantry` - as long as you control another artifact: absent while it is unmet, read once it holds", () => {
  ...
  test("{2}{W}: the vocabulary resolves \"Search your library for a card named ~, put it onto the battlefield tapped, then shuffle.\"", () => {
    const { g, board0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Magitek Infantry" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Magitek Infantry");
      expect(found, "Magitek Infantry is not in the library for the search to find").toBeDefined();
