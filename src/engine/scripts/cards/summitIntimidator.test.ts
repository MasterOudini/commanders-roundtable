// `Summit Intimidator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUMMIT_INTIMIDATOR_SCRIPT } from './summitIntimidator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Summit Intimidator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Summit Intimidator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUMMIT_INTIMIDATOR_SCRIPT]),
  });
  holdEverywhere(g);
