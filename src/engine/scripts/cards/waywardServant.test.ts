// `Wayward Servant` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wayward Servant", "Grizzly Bears", "Gutter Skulk"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WAYWARD_SERVANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wayward Servant", () => {
  test("Whenever another Zombie you control enters: each opponent loses 1 life and 1 is gained", () => {
