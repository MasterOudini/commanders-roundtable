// `Roiling Dragonstorm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROILING_DRAGONSTORM_SCRIPT } from './roilingDragonstorm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Roiling Dragonstorm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Roiling Dragonstorm", "Grizzly Bears", "Dragonborn Looter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROILING_DRAGONSTORM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Roiling Dragonstorm", () => {
  test("When this enchantment enters: the vocabulary resolves \"Draw two cards, then discard a card.\"", () => {
