// `Rakalite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAKALITE_SCRIPT } from './rakalite';
...
type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; sparkSpray: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rakalite", "Spark Spray"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAKALITE_SCRIPT]),
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, sparkSpray };
}

describe("Rakalite", () => {
  test("{2}: the vocabulary resolves \"Prevent the next 1 damage that would be dealt to any target this turn. Return this artifact to its owner's hand at the beginning of the next end step.\"", () => {
