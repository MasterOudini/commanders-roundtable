// `Yavimaya Hollow` - every printed ability proven in its own game: the cost's mark, the pump
...
import { YAVIMAYA_HOLLOW_SCRIPT } from './yavimayaHollow';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Yavimaya Hollow";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Yavimaya Hollow", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YAVIMAYA_HOLLOW_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Yavimaya Hollow", () => {
  test("{G}, {T}: the declared creature regenerates from the Bolt", () => {
