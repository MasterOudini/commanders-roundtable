// `Aspect of Mongoose` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASPECT_OF_MONGOOSE_SCRIPT } from './aspectOfMongoose';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aspect of Mongoose";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([ASPECT_OF_MONGOOSE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aspect of Mongoose", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASPECT_OF_MONGOOSE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aspect of Mongoose", () => {
  test("Enchanted creature has shroud. (It can't be the target of spells or abilities.): the creature it is attached to reads it", () => {
