// `Shady Informant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHADY_INFORMANT_SCRIPT } from './shadyInformant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shady Informant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shady Informant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHADY_INFORMANT_SCRIPT]),
