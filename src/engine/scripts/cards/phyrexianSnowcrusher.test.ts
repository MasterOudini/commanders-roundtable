// `Phyrexian Snowcrusher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHYREXIAN_SNOWCRUSHER_SCRIPT } from './phyrexianSnowcrusher';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phyrexian Snowcrusher";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PHYREXIAN_SNOWCRUSHER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phyrexian Snowcrusher", "Grizzly Bears", "Snow-Covered Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHYREXIAN_SNOWCRUSHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Phyrexian Snowcrusher", () => {
  ...
  test("{1}{S}: it gets +1/+0 until end of turn", () => {
    const { g, self } = armed(1);
    expect(pt(g, self)).toEqual([7, 5]);
  });
