// `Staggering Insight` - the granted TRIGGER proven end to end (D368): the recipient HAS the
...
import { STAGGERING_INSIGHT_SCRIPT } from './staggeringInsight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Staggering Insight";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Staggering Insight","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STAGGERING_INSIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Staggering Insight", () => {
  ...
  test('it FIRES off the recipient and the payload runs', () => {
    ...
    settle(g);
    expect((g.state.zones.hand.p1 ?? []).length).toBeGreaterThan(hand0);
