// `Living Wall` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIVING_WALL_SCRIPT } from './livingWall';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Living Wall";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Living Wall", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIVING_WALL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Living Wall", () => {
  test("{1}: it regenerates from the Bolt", () => {
