// `Shu Defender` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHU_DEFENDER_SCRIPT } from './shuDefender';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shu Defender";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHU_DEFENDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shu Defender"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHU_DEFENDER_SCRIPT]),
  });
  holdEverywhere(g);
