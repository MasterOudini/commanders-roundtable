// `Magmatic Force` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAGMATIC_FORCE_SCRIPT } from './magmaticForce';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Magmatic Force";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Magmatic Force"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAGMATIC_FORCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Magmatic Force", () => {
  test("At the beginning of each upkeep: it deals 3 damage to the declared creature", () => {
    const { g, no } = armed(0);
