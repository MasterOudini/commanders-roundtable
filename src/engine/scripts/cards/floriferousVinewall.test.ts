// `Floriferous Vinewall` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Floriferous Vinewall", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLORIFEROUS_VINEWALL_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const lookPicksL1_etb = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Forest" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL1_etb.length, "Forest is dealt for the look").toBeGreaterThan(0);
...
describe("Floriferous Vinewall", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top six cards of your library. You may reveal a land card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
