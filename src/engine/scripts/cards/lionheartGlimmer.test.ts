// `Lionheart Glimmer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIONHEART_GLIMMER_SCRIPT } from './lionheartGlimmer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lionheart Glimmer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LIONHEART_GLIMMER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lionheart Glimmer", "Coral Eel", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIONHEART_GLIMMER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lionheart Glimmer", () => {
  test("Whenever you attack: its controller's creatures get +1/+1 until end of turn", () => {
