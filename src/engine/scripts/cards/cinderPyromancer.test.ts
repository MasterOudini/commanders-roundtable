// `Cinder Pyromancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CINDER_PYROMANCER_SCRIPT } from './cinderPyromancer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cinder Pyromancer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cinder Pyromancer", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CINDER_PYROMANCER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cinder Pyromancer", () => {
  test("{T}: it deals 1 damage to the declared player", () => {
    const { g, self, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 - 1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("Whenever you cast a red spell: it untaps", () => {
