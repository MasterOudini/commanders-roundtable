// `Mindwarper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MINDWARPER_SCRIPT } from './mindwarper';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mindwarper";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MINDWARPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mindwarper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MINDWARPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mindwarper", () => {
  ...
  test("{2}{B}, Remove a +1/+1 counter from this creature: the vocabulary resolves \"Target player discards a card.\"", () => {
