// `Quagmire Lamprey` - every printed ability proven in its own game: the cost's mark, the pump
...
import { QUAGMIRE_LAMPREY_SCRIPT } from './quagmireLamprey';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Quagmire Lamprey";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Quagmire Lamprey"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([QUAGMIRE_LAMPREY_SCRIPT]),
...
describe("Quagmire Lamprey", () => {
  test("Whenever this creature becomes blocked by a creature: the vocabulary resolves \"Put a -1/-1 counter on target creature.\"", () => {
    const { g, no } = armed(0);
