// `Soul Bleed` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOUL_BLEED_SCRIPT } from './soulBleed';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Soul Bleed";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soul Bleed", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOUL_BLEED_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Soul Bleed", () => {
  test("At the beginning of the upkeep of enchanted creature's controller: the vocabulary resolves \"Target player loses 1 life.\"", () => {
