// `Kolaghan Aspirant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KOLAGHAN_ASPIRANT_SCRIPT } from './kolaghanAspirant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kolaghan Aspirant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kolaghan Aspirant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KOLAGHAN_ASPIRANT_SCRIPT]),
