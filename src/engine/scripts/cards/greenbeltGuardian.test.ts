// `Greenbelt Guardian` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GREENBELT_GUARDIAN_SCRIPT } from './greenbeltGuardian';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Greenbelt Guardian";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GREENBELT_GUARDIAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Greenbelt Guardian", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GREENBELT_GUARDIAN_SCRIPT]),
...
describe("Greenbelt Guardian", () => {
  test("{G}: the declared creature is pumped until end of turn", () => {
    const { g, no } = armed(0);
    expect(pt(g, no)).toEqual([5, 2]);
    expect(kw(g, no).has("trample")).toBe(true);
  });

  test("Exhaust — {3}{G}: it gets 3 +1/+1 counters", () => {
