// `Wall of Brambles` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WALL_OF_BRAMBLES_SCRIPT } from './wallOfBrambles';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wall of Brambles";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wall of Brambles", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WALL_OF_BRAMBLES_SCRIPT]),
  });
  holdEverywhere(g);
