// `Tectonic Fiend` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tectonic Fiend", "Grizzly Bears", "Forest", "Forest", "Forest", "Forest", "Mountain", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TECTONIC_FIEND_SCRIPT]),
  });
  holdEverywhere(g);
