// `Prowcatcher Specialist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PROWCATCHER_SPECIALIST_SCRIPT } from './prowcatcherSpecialist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prowcatcher Specialist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prowcatcher Specialist", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PROWCATCHER_SPECIALIST_SCRIPT]),
...
describe("Prowcatcher Specialist", () => {
  test("Exhaust — {3}{R}: it gets 2 +1/+1 counters", () => {
