// `Tin Street Market` - the permanent it is attached to HAS a quoted
...
import { TIN_STREET_MARKET } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(TIN_STREET_MARKET, "Enchant land\nEnchanted land has \"{T}, Discard a card: Draw a card.\"");
const LINES = PRINTED.split('\n');

const GRANT = grantedActivated("{T}, Discard a card: Draw a card.", `${TIN_STREET_MARKET.oracleId}#g1`, TIN_STREET_MARKET.name);
const VOCAB = vocabularyEffects("Draw a card.", TIN_STREET_MARKET.name);
const VOCAB_T = vocabularyTargets("Draw a card.");

export const TIN_STREET_MARKET_SCRIPT: CardScript = {
  oracleId: TIN_STREET_MARKET.oracleId,
  name: TIN_STREET_MARKET.name,
  statics: [
    {
      abilityId: 'grant-1',
