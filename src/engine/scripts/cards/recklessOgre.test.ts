// `Reckless Ogre` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RECKLESS_OGRE_SCRIPT } from './recklessOgre';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Reckless Ogre";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RECKLESS_OGRE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Reckless Ogre"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RECKLESS_OGRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Reckless Ogre", () => {
  test("Whenever this creature attacks alone: it gets +3/+0 until end of turn", () => {
