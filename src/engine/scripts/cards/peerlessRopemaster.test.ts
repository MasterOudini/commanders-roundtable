// `Peerless Ropemaster` - every declared pick is accepted and it returns to hand; a permanent the clause
// excludes is refused (D299). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { PEERLESS_ROPEMASTER_SCRIPT } from './peerlessRopemaster';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Peerless Ropemaster";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Peerless Ropemaster"], ["Grizzly Bears", "Vampire Nighthawk"]],
    scripts: createRegistry([PEERLESS_ROPEMASTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Peerless Ropemaster", () => {
  test("Grizzly Bears (tapped) is accepted and it returns to hand", () => {
