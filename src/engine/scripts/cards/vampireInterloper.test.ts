// `Vampire Interloper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VAMPIRE_INTERLOPER_SCRIPT } from './vampireInterloper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vampire Interloper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vampire Interloper", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VAMPIRE_INTERLOPER_SCRIPT]),
  });
  holdEverywhere(g);
