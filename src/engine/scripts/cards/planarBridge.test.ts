// `Planar Bridge` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Planar Bridge", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PLANAR_BRIDGE_SCRIPT]),
...
describe("Planar Bridge", () => {
  test("{8}, {T}: the vocabulary resolves \"Search your library for a permanent card, put it onto the battlefield, then shuffle.\"", () => {
