// `Viridian Longbow` - the permanent it is attached to HAS a quoted
...
import { VIRIDIAN_LONGBOW } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(VIRIDIAN_LONGBOW, "Equipped creature has \"{T}: This creature deals 1 damage to any target.\"\nEquip {3} ({3}: Attach to target creature you control. Equip only as a sorcery.)");
const LINES = PRINTED.split('\n');

const GRANT = grantedActivated("{T}: This creature deals 1 damage to any target.", `${VIRIDIAN_LONGBOW.oracleId}#g0`, VIRIDIAN_LONGBOW.name);
const VOCAB = vocabularyEffects("This creature deals 1 damage to any target.", VIRIDIAN_LONGBOW.name);
const VOCAB_T = vocabularyTargets("This creature deals 1 damage to any target.");

export const VIRIDIAN_LONGBOW_SCRIPT: CardScript = {
  oracleId: VIRIDIAN_LONGBOW.oracleId,
  name: VIRIDIAN_LONGBOW.name,
  statics: [
    {
      abilityId: 'grant-0',
