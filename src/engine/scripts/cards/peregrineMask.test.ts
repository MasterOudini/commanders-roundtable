// `Peregrine Mask` - equipped to Grizzly Bears the creature has defender, flying, firstStrike; the Cyclops is untouched;
...
import { PEREGRINE_MASK_SCRIPT } from './peregrineMask';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Peregrine Mask";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([PEREGRINE_MASK_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([PEREGRINE_MASK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Peregrine Mask", () => {
  test("equipped to Grizzly Bears: attached, and the creature has defender, flying, firstStrike; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(kw(g, host).has("defender")).toBe(true);
    expect(kw(g, host).has("flying")).toBe(true);
    expect(kw(g, host).has("firstStrike")).toBe(true);
    expect(kw(g, other).has("defender")).toBe(false);
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("defender")).toBe(false);
