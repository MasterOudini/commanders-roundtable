// `Grim Poppet` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRIM_POPPET_SCRIPT } from './grimPoppet';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grim Poppet";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GRIM_POPPET_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grim Poppet"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRIM_POPPET_SCRIPT]),
...
describe("Grim Poppet", () => {
  ...
  test("Remove a -1/-1 counter from this creature: the vocabulary resolves \"Put a -1/-1 counter on another target creature.\"", () => {
