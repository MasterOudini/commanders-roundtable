// `Gleaming Geardrake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GLEAMING_GEARDRAKE_SCRIPT } from './gleamingGeardrake';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gleaming Geardrake";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([GLEAMING_GEARDRAKE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gleaming Geardrake", "Lotus Petal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLEAMING_GEARDRAKE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gleaming Geardrake", () => {
  ...
  test("Whenever you sacrifice an artifact: it gets 1 +1/+1 counter", () => {
