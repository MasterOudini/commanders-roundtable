// `Thorn Mammoth` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THORN_MAMMOTH_SCRIPT } from './thornMammoth';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thorn Mammoth";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thorn Mammoth", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THORN_MAMMOTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thorn Mammoth", () => {
  test("Whenever this creature or another creature you control enters: the vocabulary resolves \"~ fights up to one target creature you don't control.\"", () => {
    const { g, self, no } = armed(0);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
    expect(g.state.cards[self]?.damage).toBe(5);
  });
