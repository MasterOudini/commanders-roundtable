// `Hammerhand` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HAMMERHAND_SCRIPT } from './hammerhand';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hammerhand";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HAMMERHAND_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hammerhand", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HAMMERHAND_SCRIPT]),
  });
  holdEverywhere(g);
