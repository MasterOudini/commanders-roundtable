// `White Orchid Phantom` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["White Orchid Phantom"], ["Cyclops of One-Eyed Pass", "Tundra", "Forest"]],
    scripts: createRegistry([WHITE_ORCHID_PHANTOM_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtL1_etb_0 = put(g, 'p2', "Tundra");
  settle(g);
...
describe("White Orchid Phantom", () => {
  test("When this creature enters: the vocabulary resolves \"Destroy up to one target nonbasic land. Its controller may search their library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
