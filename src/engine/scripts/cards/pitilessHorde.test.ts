// `Pitiless Horde` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PITILESS_HORDE_SCRIPT } from './pitilessHorde';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pitiless Horde";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pitiless Horde"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PITILESS_HORDE_SCRIPT]),
