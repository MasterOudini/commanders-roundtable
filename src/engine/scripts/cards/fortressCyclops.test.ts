// `Fortress Cyclops` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FORTRESS_CYCLOPS_SCRIPT } from './fortressCyclops';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fortress Cyclops";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FORTRESS_CYCLOPS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fortress Cyclops"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FORTRESS_CYCLOPS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fortress Cyclops", () => {
  test("Whenever this creature attacks: it gets +3/+0 until end of turn", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([6, 3]);
  });

  test("Whenever this creature blocks: it gets +0/+3 until end of turn", () => {
    const { g, self } = armed(1);
    expect(pt(g, self)).toEqual([3, 6]);
  });
