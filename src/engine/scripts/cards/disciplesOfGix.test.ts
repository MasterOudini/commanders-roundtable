// `Disciples of Gix` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Disciples of Gix", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DISCIPLES_OF_GIX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Disciples of Gix", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for up to three artifact cards, put them into your graveyard, then shuffle.\"", () => {
    const { g } = armed(0);
