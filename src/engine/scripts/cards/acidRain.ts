// `Acid Rain` — "Destroy all Forests." The first wipe by SUBTYPE: derived,
...
const TEXT = printed(ACID_RAIN, 'Destroy all Forests.');

export const ACID_RAIN_SCRIPT: CardScript = {
  oracleId: ACID_RAIN.oracleId,
  name: ACID_RAIN.name,
  spell: {
    text: TEXT,
    resolve: (ctx, _self, _obj): readonly EventBody[] => {
      ...
      for (const id of ctx.state.zones.battlefield) {
        ...
        if (!d.typeLine.subtypes.includes('Forest')) continue;
