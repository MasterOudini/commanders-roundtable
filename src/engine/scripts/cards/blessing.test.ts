// `Blessing` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLESSING_SCRIPT } from './blessing';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLESSING_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blessing", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLESSING_SCRIPT]),
  });
  holdEverywhere(g);
