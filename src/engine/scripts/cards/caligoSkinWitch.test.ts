// `Caligo Skin-Witch` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CALIGO_SKIN_WITCH_SCRIPT } from './caligoSkinWitch';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Caligo Skin-Witch";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Caligo Skin-Witch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CALIGO_SKIN_WITCH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Caligo Skin-Witch", () => {
  test("When this creature enters: the vocabulary resolves \"Each opponent discards two cards.\"", () => {
    const { g } = armed(0);
    let qdL1 = 0;
