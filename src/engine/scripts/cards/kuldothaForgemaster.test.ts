// `Kuldotha Forgemaster` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kuldotha Forgemaster", "Sol Ring", "Sol Ring", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KULDOTHA_FORGEMASTER_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = [put(g, 'p1', "Sol Ring"), put(g, 'p1', "Sol Ring"), put(g, 'p1', "Sol Ring")];
  settle(g);
...
describe("Kuldotha Forgemaster", () => {
  test("{T}, Sacrifice three artifacts: the vocabulary resolves \"Search your library for an artifact card, put it onto the battlefield, then shuffle.\"", () => {
    const { g, self, board0, fodder0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Sol Ring" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
