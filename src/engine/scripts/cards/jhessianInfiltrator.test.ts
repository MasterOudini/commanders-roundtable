// `Jhessian Infiltrator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JHESSIAN_INFILTRATOR_SCRIPT } from './jhessianInfiltrator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jhessian Infiltrator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jhessian Infiltrator", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JHESSIAN_INFILTRATOR_SCRIPT]),
  });
  holdEverywhere(g);
