// `Bloodbriar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODBRIAR_SCRIPT } from './bloodbriar';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodbriar";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([BLOODBRIAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodbriar", "Lotus Petal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODBRIAR_SCRIPT]),
  });
  holdEverywhere(g);
