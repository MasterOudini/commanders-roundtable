// `Drake Haven` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRAKE_HAVEN_SCRIPT } from './drakeHaven';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drake Haven";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drake Haven", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRAKE_HAVEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Drake Haven", () => {
  test("Whenever you cycle or discard a card: the vocabulary resolves \"You may pay {1}. If you do, create a 2/2 blue Drake creature token with flying.\" - the price is declined", () => {
  ...
  test("Whenever you cycle or discard a card: the vocabulary resolves \"You may pay {1}. If you do, create a 2/2 blue Drake creature token with flying.\" - the price is paid", () => {
