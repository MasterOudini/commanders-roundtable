// `Malakir Soothsayer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MALAKIR_SOOTHSAYER_SCRIPT } from './malakirSoothsayer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Malakir Soothsayer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Malakir Soothsayer", "Expedition Envoy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MALAKIR_SOOTHSAYER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Malakir Soothsayer", () => {
  test("Cohort — {T}, Tap an untapped Ally you control: a card is drawn and 1 life lost", () => {
    const { g, self, life0, hand0, tapper0 } = armed(0);
