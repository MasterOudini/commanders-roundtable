// `Agent Phil Coulson` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AGENT_PHIL_COULSON_SCRIPT } from './agentPhilCoulson';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Agent Phil Coulson";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Agent Phil Coulson", "Black Widow, Natasha Romanoff"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AGENT_PHIL_COULSON_SCRIPT]),
...
describe("Agent Phil Coulson", () => {
  test("{T}: each of its controller's creatures gets 1 +1/+1 counter", () => {
