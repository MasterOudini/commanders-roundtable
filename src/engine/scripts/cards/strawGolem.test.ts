// `Straw Golem` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STRAW_GOLEM_SCRIPT } from './strawGolem';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Straw Golem";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Straw Golem"], ["Cyclops of One-Eyed Pass", "Crimson Kobolds", "Grizzly Bears"]],
    scripts: createRegistry([STRAW_GOLEM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Straw Golem", () => {
  test("When an opponent casts a creature spell: the vocabulary resolves \"Sacrifice this creature.\"", () => {
