// `Body Dropper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BODY_DROPPER_SCRIPT } from './bodyDropper';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Body Dropper";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([BODY_DROPPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Body Dropper", "Grizzly Bears", "Blood Pet"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BODY_DROPPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Body Dropper", () => {
  test("Whenever you sacrifice another creature: it gets 1 +1/+1 counter", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(1);
  });

  test("{B}{R}, Sacrifice another creature: it gains menace until end of turn", () => {
