// `Geothermal Kami` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GEOTHERMAL_KAMI_SCRIPT } from './geothermalKami';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Geothermal Kami";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Geothermal Kami", "Hissing Miasma"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GEOTHERMAL_KAMI_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const priceL0_p1_0 = put(g, 'p1', "Hissing Miasma");
  settle(g);
...
describe("Geothermal Kami", () => {
  test("When this creature enters: the vocabulary resolves \"You may return an enchantment you control to its owner's hand. If you do, you gain 3 life.\" - the price is declined", () => {
  ...
  test("When this creature enters: the vocabulary resolves \"You may return an enchantment you control to its owner's hand. If you do, you gain 3 life.\" - the price is paid", () => {
    const { g, life0, board0, priceL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
