// `Smolder Initiate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SMOLDER_INITIATE_SCRIPT } from './smolderInitiate';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Smolder Initiate";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Smolder Initiate", "Grizzly Bears", "Muck Rats", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SMOLDER_INITIATE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Smolder Initiate", () => {
  test("Whenever a player casts a black spell: the vocabulary resolves \"You may pay {1}. If you do, target player loses 1 life.\" - the price is declined", () => {
  ...
  test("Whenever a player casts a black spell: the vocabulary resolves \"You may pay {1}. If you do, target player loses 1 life.\" - the price is paid", () => {
    const { g, p2life0, payL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
