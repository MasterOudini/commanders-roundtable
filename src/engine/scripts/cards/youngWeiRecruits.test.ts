// `Young Wei Recruits` - every printed ability proven in its own game: the cost's mark, the pump
...
import { YOUNG_WEI_RECRUITS_SCRIPT } from './youngWeiRecruits';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Young Wei Recruits";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Young Wei Recruits", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YOUNG_WEI_RECRUITS_SCRIPT]),
  });
  holdEverywhere(g);
