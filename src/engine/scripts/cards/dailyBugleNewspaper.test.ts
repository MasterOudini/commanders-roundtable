// `Daily Bugle Newspaper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAILY_BUGLE_NEWSPAPER_SCRIPT } from './dailyBugleNewspaper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Daily Bugle Newspaper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Daily Bugle Newspaper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAILY_BUGLE_NEWSPAPER_SCRIPT]),
...
describe("Daily Bugle Newspaper", () => {
  test("{2}, {T}: the vocabulary resolves \"Draw a card, then discard a card. Create a Treasure token.\"", () => {
    ...
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1 - qdA0);
