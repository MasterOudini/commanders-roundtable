// `Nettle Drone` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NETTLE_DRONE_SCRIPT } from './nettleDrone';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nettle Drone";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nettle Drone", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NETTLE_DRONE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Nettle Drone", () => {
  ...
  test("Whenever you cast a colorless spell: it untaps", () => {
