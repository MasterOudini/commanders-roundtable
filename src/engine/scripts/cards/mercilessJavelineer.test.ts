// `Merciless Javelineer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERCILESS_JAVELINEER_SCRIPT } from './mercilessJavelineer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Merciless Javelineer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merciless Javelineer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERCILESS_JAVELINEER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Merciless Javelineer", () => {
  test("{2}, Discard a card: the vocabulary resolves \"Put a -1/-1 counter on target creature. That creature can't block this turn.\"", () => {
