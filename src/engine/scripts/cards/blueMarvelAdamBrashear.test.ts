// `Blue Marvel, Adam Brashear` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLUE_MARVEL_ADAM_BRASHEAR_SCRIPT } from './blueMarvelAdamBrashear';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blue Marvel, Adam Brashear";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blue Marvel, Adam Brashear", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLUE_MARVEL_ADAM_BRASHEAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Blue Marvel, Adam Brashear", () => {
