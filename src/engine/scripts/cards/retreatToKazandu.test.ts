// `Retreat to Kazandu` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RETREAT_TO_KAZANDU_SCRIPT } from './retreatToKazandu';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Retreat to Kazandu";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Retreat to Kazandu"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RETREAT_TO_KAZANDU_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Retreat to Kazandu", () => {
  test("Landfall — Whenever a land you control enters: choosing \"Put a +1/+1 counter on target creature.\", counterOnTarget", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.counters["+1/+1"] ?? 0).toBe(1);
  });

  test("Landfall — Whenever a land you control enters: choosing \"You gain 2 life.\", gainLife", () => {
