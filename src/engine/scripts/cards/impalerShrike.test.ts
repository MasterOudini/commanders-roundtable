// `Impaler Shrike` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IMPALER_SHRIKE_SCRIPT } from './impalerShrike';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Impaler Shrike";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Impaler Shrike"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IMPALER_SHRIKE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Impaler Shrike", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"You may sacrifice it. If you do, draw three cards.\" - the price is declined", () => {
  ...
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"You may sacrifice it. If you do, draw three cards.\" - the price is paid", () => {
