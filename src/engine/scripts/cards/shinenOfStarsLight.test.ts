// `Shinen of Stars' Light` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHINEN_OF_STARS_LIGHT_SCRIPT } from './shinenOfStarsLight';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shinen of Stars' Light";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHINEN_OF_STARS_LIGHT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shinen of Stars' Light"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHINEN_OF_STARS_LIGHT_SCRIPT]),
...
describe("Shinen of Stars' Light", () => {
  test("Channel — {1}{W}, Discard this card: the declared creature is pumped until end of turn", () => {
