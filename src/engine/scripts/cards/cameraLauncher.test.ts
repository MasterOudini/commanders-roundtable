// `Camera Launcher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAMERA_LAUNCHER_SCRIPT } from './cameraLauncher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Camera Launcher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Camera Launcher", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAMERA_LAUNCHER_SCRIPT]),
...
describe("Camera Launcher", () => {
  test("Exhaust — {3}: the vocabulary resolves \"Put a +1/+1 counter on this creature. Create a 1/1 colorless Thopter artifact creature token with flying.\"", () => {
