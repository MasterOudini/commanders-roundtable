// `Loxodon Eavesdropper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOXODON_EAVESDROPPER_SCRIPT } from './loxodonEavesdropper';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Loxodon Eavesdropper";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LOXODON_EAVESDROPPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Loxodon Eavesdropper", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOXODON_EAVESDROPPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Loxodon Eavesdropper", () => {
  ...
  test("Whenever you draw your second card each turn: it gets +1/+1 and gains vigilance until end of turn", () => {
