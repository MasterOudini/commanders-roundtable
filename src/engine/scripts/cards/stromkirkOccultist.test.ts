// `Stromkirk Occultist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STROMKIRK_OCCULTIST_SCRIPT } from './stromkirkOccultist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stromkirk Occultist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stromkirk Occultist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STROMKIRK_OCCULTIST_SCRIPT]),
...
describe("Stromkirk Occultist", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Exile the top card of your library. Until end of turn, you may play that card.\"", () => {
    const { g } = armed(0);
