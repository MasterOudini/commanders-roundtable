// `Reflexes` - cast on Grizzly Bears it attaches and the enchanted creature has firstStrike;
...
import { REFLEXES_SCRIPT } from './reflexes';
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([REFLEXES_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([REFLEXES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Reflexes", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has firstStrike; Cyclops of One-Eyed Pass is untouched", () => {
