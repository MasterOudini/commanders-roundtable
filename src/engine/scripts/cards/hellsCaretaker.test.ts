// `Hell's Caretaker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HELLS_CARETAKER_SCRIPT } from './hellsCaretaker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hell's Caretaker";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; condRefused: boolean; vtA0_0: InstanceId; fodder0: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hell's Caretaker", "Grizzly Bears", "Walking Corpse"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HELLS_CARETAKER_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, condRefused, vtA0_0, fodder0 };
}

describe("Hell's Caretaker", () => {
  test("{T}, Sacrifice a creature: the vocabulary resolves \"Return target creature card from your graveyard to the battlefield.\" - refused unless during your upkeep", () => {
    const { g, self, board0, condRefused, vtA0_0, fodder0 } = armed(0);
