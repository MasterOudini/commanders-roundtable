// `Gloomwidow` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GLOOMWIDOW_SCRIPT } from './gloomwidow';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gloomwidow";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gloomwidow", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLOOMWIDOW_SCRIPT]),
  });
  holdEverywhere(g);
