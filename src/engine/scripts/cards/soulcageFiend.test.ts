// `Soulcage Fiend` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOULCAGE_FIEND_SCRIPT } from './soulcageFiend';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Soulcage Fiend";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soulcage Fiend"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOULCAGE_FIEND_SCRIPT]),
  });
  holdEverywhere(g);
