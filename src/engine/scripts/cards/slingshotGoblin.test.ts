// `Slingshot Goblin` - the named target is accepted and the damage is lethal; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SLINGSHOT_GOBLIN_SCRIPT } from './slingshotGoblin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slingshot Goblin";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Slingshot Goblin"], ["Merfolk of the Pearl Trident", "Grizzly Bears"]],
    scripts: createRegistry([SLINGSHOT_GOBLIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Slingshot Goblin", () => {
  test("Merfolk of the Pearl Trident is a legal target and the damage is lethal", () => {
