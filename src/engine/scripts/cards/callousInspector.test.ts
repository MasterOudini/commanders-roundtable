// `Callous Inspector` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CALLOUS_INSPECTOR_SCRIPT } from './callousInspector';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Callous Inspector";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Callous Inspector"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CALLOUS_INSPECTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Callous Inspector", () => {
  test("When this creature dies: the vocabulary resolves \"~ deals 1 damage to you. Create a Clue token.\"", () => {
