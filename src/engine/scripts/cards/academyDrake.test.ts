// `Academy Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ACADEMY_DRAKE_SCRIPT } from './academyDrake';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Academy Drake";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ACADEMY_DRAKE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Academy Drake"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ACADEMY_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
