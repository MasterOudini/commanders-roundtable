// `Somnomancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOMNOMANCER_SCRIPT } from './somnomancer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Somnomancer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Somnomancer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOMNOMANCER_SCRIPT]),
  });
  holdEverywhere(g);
