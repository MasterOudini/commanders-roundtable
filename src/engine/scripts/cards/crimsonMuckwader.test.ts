// `Crimson Muckwader` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRIMSON_MUCKWADER_SCRIPT } from './crimsonMuckwader';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crimson Muckwader";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CRIMSON_MUCKWADER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crimson Muckwader", "Lightning Bolt", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRIMSON_MUCKWADER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Crimson Muckwader", () => {
  test("`Crimson Muckwader` - as long as you control a Swamp: absent while it is unmet, read once it holds", () => {
