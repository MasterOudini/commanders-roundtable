// `Windcaller Aven` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WINDCALLER_AVEN_SCRIPT } from './windcallerAven';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Windcaller Aven";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([WINDCALLER_AVEN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Windcaller Aven"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WINDCALLER_AVEN_SCRIPT]),
...
describe("Windcaller Aven", () => {
  test("When you cycle this card: the vocabulary resolves \"Target creature gains flying until end of turn.\"", () => {
    const { g, no } = armed(0);
