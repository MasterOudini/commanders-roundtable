// `Prayer of Binding` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PRAYER_OF_BINDING_SCRIPT } from './prayerOfBinding';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prayer of Binding";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prayer of Binding"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRAYER_OF_BINDING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Prayer of Binding", () => {
  test("When this enchantment enters: the vocabulary resolves \"Exile up to one target nonland permanent an opponent controls until this enchantment leaves the battlefield. You gain 2 life.\"", () => {
    const { g, self, no, life0 } = armed(0);
