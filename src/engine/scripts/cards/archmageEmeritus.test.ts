// `Archmage Emeritus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ARCHMAGE_EMERITUS_SCRIPT } from './archmageEmeritus';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Archmage Emeritus";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Archmage Emeritus", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARCHMAGE_EMERITUS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Archmage Emeritus", () => {
  test("Magecraft — Whenever you cast or copy an instant or sorcery spell: a card is drawn", () => {
