// `Citizen's Arrest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CITIZENS_ARREST_SCRIPT } from './citizensArrest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Citizen's Arrest";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Citizen's Arrest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CITIZENS_ARREST_SCRIPT]),
  });
  holdEverywhere(g);
