// `Storm Fleet Aerialist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORM_FLEET_AERIALIST_SCRIPT } from './stormFleetAerialist';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Storm Fleet Aerialist";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STORM_FLEET_AERIALIST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Storm Fleet Aerialist", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORM_FLEET_AERIALIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Storm Fleet Aerialist", () => {
