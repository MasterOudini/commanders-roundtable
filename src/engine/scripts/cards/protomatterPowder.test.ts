// `Protomatter Powder` - the named target is accepted and it returns to the battlefield; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { PROTOMATTER_POWDER_SCRIPT } from './protomatterPowder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Protomatter Powder";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Protomatter Powder", "Sol Ring", "Grizzly Bears"], ["Grizzly Bears"]],
    scripts: createRegistry([PROTOMATTER_POWDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Protomatter Powder", () => {
  test("Sol Ring is a legal target and it returns to the battlefield", () => {
