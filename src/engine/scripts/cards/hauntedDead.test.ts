// `Haunted Dead` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HAUNTED_DEAD_SCRIPT } from './hauntedDead';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Haunted Dead";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Haunted Dead", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HAUNTED_DEAD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Haunted Dead", () => {
  test("When this creature enters: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("{1}{B}, Discard two cards: it returns from the graveyard to the battlefield tapped", () => {
