// `Pharika's Disciple` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHARIKAS_DISCIPLE_SCRIPT } from './pharikasDisciple';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pharika's Disciple";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PHARIKAS_DISCIPLE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pharika's Disciple"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHARIKAS_DISCIPLE_SCRIPT]),
  });
  holdEverywhere(g);
