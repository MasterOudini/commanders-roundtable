// `Collector's Case` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COLLECTORS_CASE_SCRIPT } from './collectorsCase';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Collector's Case";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Collector's Case", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COLLECTORS_CASE_SCRIPT]),
...
describe("Collector's Case", () => {
  test("When this artifact enters: the vocabulary resolves \"Tap up to one target creature and put two stun counters on it.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.tapped).toBe(true);
    expect(g.state.cards[no]?.counters["stun"] ?? 0).toBe(2);
  });

  test("{3}{U}, {T}: the declared creature is tapped", () => {
