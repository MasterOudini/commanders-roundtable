// `Flying Octobot` - a anotherCreatureEnters trigger selfCounter
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { FLYING_OCTOBOT } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(FLYING_OCTOBOT, "Flying\nWhenever another Villain you control enters, put a +1/+1 counter on this creature. This ability triggers only once each turn.");
const LINES = PRINTED.split('\n');

export const FLYING_OCTOBOT_SCRIPT: CardScript = {
  oracleId: FLYING_OCTOBOT.oracleId,
  name: FLYING_OCTOBOT.name,
  triggers: [
    {
      ...
      label: () => "Flying Octobot - a counter on it",
