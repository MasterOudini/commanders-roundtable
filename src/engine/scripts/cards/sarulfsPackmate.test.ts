// `Sarulf's Packmate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SARULFS_PACKMATE_SCRIPT } from './sarulfsPackmate';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sarulf's Packmate";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sarulf's Packmate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SARULFS_PACKMATE_SCRIPT]),
