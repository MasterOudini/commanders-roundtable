// `Bumbleflower's Sharepot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BUMBLEFLOWERS_SHAREPOT_SCRIPT } from './bumbleflowersSharepot';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bumbleflower's Sharepot";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bumbleflower's Sharepot"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BUMBLEFLOWERS_SHAREPOT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bumbleflower's Sharepot", () => {
  test("When this artifact enters: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("{5}, {T}, Sacrifice this artifact: the vocabulary resolves \"Destroy target nonland permanent.\"", () => {
