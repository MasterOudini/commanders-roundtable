// `Untamed Kavu` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNTAMED_KAVU_SCRIPT } from './untamedKavu';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Untamed Kavu";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([UNTAMED_KAVU_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Untamed Kavu"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNTAMED_KAVU_SCRIPT]),
  });
  holdEverywhere(g);
