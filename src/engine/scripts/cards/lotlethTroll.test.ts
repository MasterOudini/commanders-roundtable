// `Lotleth Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOTLETH_TROLL_SCRIPT } from './lotlethTroll';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lotleth Troll";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lotleth Troll", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOTLETH_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lotleth Troll", () => {
  test("Discard a creature card: it gets 1 +1/+1 counter", () => {
