// `Lightning Spear` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIGHTNING_SPEAR_SCRIPT } from './lightningSpear';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning Spear";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LIGHTNING_SPEAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lightning Spear", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIGHTNING_SPEAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lightning Spear", () => {
  test("Equipped creature gets +1/+0 and has trample.: the creature it is attached to reads it", () => {
  ...
  test("{2}{R}, Sacrifice this Equipment: it deals 3 damage to the declared creature", () => {
