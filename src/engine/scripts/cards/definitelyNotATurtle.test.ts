// `Definitely Not a Turtle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEFINITELY_NOT_ATURTLE_SCRIPT } from './definitelyNotATurtle';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Definitely Not a Turtle";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Definitely Not a Turtle", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEFINITELY_NOT_ATURTLE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const lookPicksL0_dies = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Forest" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL0_dies.length, "Forest is dealt for the look").toBeGreaterThan(0);
    for (const id of lookPicksL0_dies) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: id, to: { kind: 'library', player: 'p1' }, placement: 'top' }));
...
describe("Definitely Not a Turtle", () => {
  test("When this creature dies: the vocabulary resolves \"Look at the top six cards of your library. You may reveal a land or legendary Turtle card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
