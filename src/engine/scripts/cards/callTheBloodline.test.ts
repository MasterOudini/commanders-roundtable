// `Call the Bloodline` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CALL_THE_BLOODLINE_SCRIPT } from './callTheBloodline';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Call the Bloodline";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Call the Bloodline", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CALL_THE_BLOODLINE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Call the Bloodline", () => {
  test("{1}, Discard a card: 1 token made", () => {
    const { g, board0, refused, disc0 } = armed(0);
