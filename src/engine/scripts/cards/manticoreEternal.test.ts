// `Manticore Eternal` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MANTICORE_ETERNAL_SCRIPT } from './manticoreEternal';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Manticore Eternal";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Manticore Eternal", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MANTICORE_ETERNAL_SCRIPT]),
  });
  holdEverywhere(g);
