// `Shifting Wall` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHIFTING_WALL_SCRIPT } from './shiftingWall';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shifting Wall";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHIFTING_WALL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shifting Wall"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHIFTING_WALL_SCRIPT]),
  });
  holdEverywhere(g);
