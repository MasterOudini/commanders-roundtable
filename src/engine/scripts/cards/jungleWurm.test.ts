// `Jungle Wurm` - the pump is COUNTED: zero of them is no bonus, and each one moves the
// derived power and toughness by the printed delta. Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { JUNGLE_WURM_SCRIPT } from './jungleWurm';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jungle Wurm";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([JUNGLE_WURM_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; subject: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([JUNGLE_WURM_SCRIPT]),
  });
  holdEverywhere(g);
...
function blockedBy(k: number): { g: Game; subject: InstanceId } {
  ...
  test("blocked by one creature it is pumped by [-1, -1] x 0; by two, x 1", () => {
    const one = blockedBy(1);
    expect(pt(one.g, one.subject)).toEqual(plus([5, 5], 0));
    const two = blockedBy(2);
    expect(pt(two.g, two.subject)).toEqual(plus([5, 5], 1));
