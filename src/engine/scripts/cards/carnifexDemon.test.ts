// `Carnifex Demon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CARNIFEX_DEMON_SCRIPT } from './carnifexDemon';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Carnifex Demon";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CARNIFEX_DEMON_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Carnifex Demon"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CARNIFEX_DEMON_SCRIPT]),
...
describe("Carnifex Demon", () => {
  ...
  test("{B}, Remove a -1/-1 counter from this creature: the vocabulary resolves \"Put a -1/-1 counter on each other creature.\"", () => {
