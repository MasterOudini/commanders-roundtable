// `Hollowhenge Scavenger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOLLOWHENGE_SCAVENGER_SCRIPT } from './hollowhengeScavenger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hollowhenge Scavenger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hollowhenge Scavenger", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOLLOWHENGE_SCAVENGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hollowhenge Scavenger", () => {
  test("Morbid — When this creature enters: 5 life is gained", () => {
