// `Pit Raptor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PIT_RAPTOR_SCRIPT } from './pitRaptor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pit Raptor";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; payL1_p1_0: InstanceId; payL1_p1_1: InstanceId; payL1_p1_2: InstanceId; payL1_p1_3: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pit Raptor", "Swamp", "Swamp", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PIT_RAPTOR_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const payL1_p1_0 = put(g, 'p1', "Swamp");
  const payL1_p1_1 = put(g, 'p1', "Swamp");
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, payL1_p1_0, payL1_p1_1, payL1_p1_2, payL1_p1_3 };
}

describe("Pit Raptor", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you pay {2}{B}{B}.\" - the price is declined", () => {
    const { g, self, board0, payL1_p1_0, payL1_p1_1, payL1_p1_2, payL1_p1_3 } = armed(0);
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you pay {2}{B}{B}.\" - the price is paid", () => {
    const { g, self, payL1_p1_0, payL1_p1_1, payL1_p1_2, payL1_p1_3 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
