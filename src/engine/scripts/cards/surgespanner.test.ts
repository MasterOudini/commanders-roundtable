// `Surgespanner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SURGESPANNER_SCRIPT } from './surgespanner';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Surgespanner";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Surgespanner", "Island", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SURGESPANNER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Surgespanner", () => {
  test("Whenever this creature becomes tapped: the vocabulary resolves \"You may pay {1}{U}. If you do, return target permanent to its owner's hand.\" - the price is declined", () => {
  ...
  test("Whenever this creature becomes tapped: the vocabulary resolves \"You may pay {1}{U}. If you do, return target permanent to its owner's hand.\" - the price is paid", () => {
    const { g, no, payL0_p1_0, payL0_p1_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
