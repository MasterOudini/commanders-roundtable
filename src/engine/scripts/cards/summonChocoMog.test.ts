// `Summon: Choco/Mog` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUMMON_CHOCO_MOG_SCRIPT } from './summonChocoMog';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Summon: Choco/Mog";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUMMON_CHOCO_MOG_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Summon: Choco/Mog", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUMMON_CHOCO_MOG_SCRIPT]),
...
describe("Summon: Choco/Mog", () => {
  test("I: its controller's creatures get +1/+0 until end of turn", () => {
