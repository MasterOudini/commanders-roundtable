// `Mother Bear` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOTHER_BEAR_SCRIPT } from './motherBear';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mother Bear";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mother Bear"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOTHER_BEAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mother Bear", () => {
  test("{3}{G}{G}, Exile this card from your graveyard: 2 tokens made", () => {
