// `Extruder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EXTRUDER_SCRIPT } from './extruder';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Extruder", "Sol Ring", "Forest", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EXTRUDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Extruder", () => {
  test("Sacrifice an artifact: the declared creature gets 1 +1/+1 counter", () => {
