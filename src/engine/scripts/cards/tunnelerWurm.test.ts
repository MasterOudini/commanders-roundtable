// `Tunneler Wurm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TUNNELER_WURM_SCRIPT } from './tunnelerWurm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tunneler Wurm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tunneler Wurm", "Grizzly Bears", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TUNNELER_WURM_SCRIPT]),
  });
  holdEverywhere(g);
