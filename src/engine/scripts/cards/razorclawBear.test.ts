// `Razorclaw Bear` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAZORCLAW_BEAR_SCRIPT } from './razorclawBear';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Razorclaw Bear";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RAZORCLAW_BEAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Razorclaw Bear"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAZORCLAW_BEAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Razorclaw Bear", () => {
  test("Whenever this creature becomes blocked: it gets +2/+2 until end of turn", () => {
