// `Clarion Spirit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLARION_SPIRIT_SCRIPT } from './clarionSpirit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Clarion Spirit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Clarion Spirit", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLARION_SPIRIT_SCRIPT]),
  });
  holdEverywhere(g);
