// `Aerial Caravan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AERIAL_CARAVAN_SCRIPT } from './aerialCaravan';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aerial Caravan";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aerial Caravan"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AERIAL_CARAVAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aerial Caravan", () => {
  test("{1}{U}{U}: the vocabulary resolves \"Exile the top card of your library. Until end of turn, you may play that card.\"", () => {
    const { g } = armed(0);
