// `Attuned Hunter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ATTUNED_HUNTER_SCRIPT } from './attunedHunter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Attuned Hunter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Attuned Hunter", "Badlands"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ATTUNED_HUNTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Attuned Hunter", () => {
  test("Whenever one or more cards leave your graveyard during your turn: it gets 1 +1/+1 counter", () => {
