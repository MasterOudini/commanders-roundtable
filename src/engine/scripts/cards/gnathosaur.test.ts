// `Gnathosaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GNATHOSAUR_SCRIPT } from './gnathosaur';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gnathosaur";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GNATHOSAUR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gnathosaur", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GNATHOSAUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gnathosaur", () => {
  test("Sacrifice an artifact: it gains trample until end of turn", () => {
