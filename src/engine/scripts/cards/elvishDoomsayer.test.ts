// `Elvish Doomsayer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ELVISH_DOOMSAYER_SCRIPT } from './elvishDoomsayer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Elvish Doomsayer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elvish Doomsayer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELVISH_DOOMSAYER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Elvish Doomsayer", () => {
  test("When this creature dies: the vocabulary resolves \"Each opponent discards a card.\"", () => {
    const { g } = armed(0);
    let qdL0 = 0;
