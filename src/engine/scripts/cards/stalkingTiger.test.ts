// `Stalking Tiger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STALKING_TIGER_SCRIPT } from './stalkingTiger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stalking Tiger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stalking Tiger", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([STALKING_TIGER_SCRIPT]),
  });
  holdEverywhere(g);
