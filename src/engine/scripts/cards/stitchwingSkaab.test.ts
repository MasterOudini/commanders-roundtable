// `Stitchwing Skaab` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STITCHWING_SKAAB_SCRIPT } from './stitchwingSkaab';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stitchwing Skaab";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stitchwing Skaab", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STITCHWING_SKAAB_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stitchwing Skaab", () => {
  test("{1}{U}, Discard two cards: it returns from the graveyard to the battlefield tapped", () => {
