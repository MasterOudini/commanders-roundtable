// `Polluted Delta` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Polluted Delta", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([POLLUTED_DELTA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Polluted Delta", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for an Island or Swamp card, put it onto the battlefield, then shuffle.\"", () => {
