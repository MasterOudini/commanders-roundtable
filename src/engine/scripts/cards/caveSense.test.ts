// `Cave Sense` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAVE_SENSE_SCRIPT } from './caveSense';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cave Sense";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CAVE_SENSE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cave Sense", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAVE_SENSE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cave Sense", () => {
  test("Enchanted creature gets +1/+1 and has mountainwalk. (It can't be blocked as long as defending player controls a Mountain.): the creature it is attached to reads it", () => {
