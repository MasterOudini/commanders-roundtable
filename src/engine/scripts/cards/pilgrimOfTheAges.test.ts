// `Pilgrim of the Ages` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pilgrim of the Ages", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PILGRIM_OF_THE_AGES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pilgrim of the Ages", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a basic Plains card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Plains" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
  ...
  test("{6}: it returns from the graveyard to hand", () => {
