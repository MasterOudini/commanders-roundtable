// `Bloodhaze Wolverine` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODHAZE_WOLVERINE_SCRIPT } from './bloodhazeWolverine';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodhaze Wolverine";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLOODHAZE_WOLVERINE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodhaze Wolverine", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODHAZE_WOLVERINE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bloodhaze Wolverine", () => {
  test("Whenever you draw your second card each turn: it gets +1/+1 and gains firstStrike until end of turn", () => {
