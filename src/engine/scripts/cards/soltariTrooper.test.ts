// `Soltari Trooper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOLTARI_TROOPER_SCRIPT } from './soltariTrooper';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Soltari Trooper";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SOLTARI_TROOPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soltari Trooper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOLTARI_TROOPER_SCRIPT]),
  });
  holdEverywhere(g);
