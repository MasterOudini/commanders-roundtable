// `Ulvenwald Tracker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ULVENWALD_TRACKER_SCRIPT } from './ulvenwaldTracker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ulvenwald Tracker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ulvenwald Tracker", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ULVENWALD_TRACKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ulvenwald Tracker", () => {
  test("{1}{G}, {T}: the vocabulary resolves \"Target creature you control fights another target creature.\"", () => {
