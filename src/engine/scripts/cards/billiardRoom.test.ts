// `Billiard Room` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BILLIARD_ROOM_SCRIPT } from './billiardRoom';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Billiard Room";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Billiard Room"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BILLIARD_ROOM_SCRIPT]),
  });
  holdEverywhere(g);
