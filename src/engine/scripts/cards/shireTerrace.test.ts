// `Shire Terrace` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shire Terrace", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHIRE_TERRACE_SCRIPT]),
  });
  holdEverywhere(g);
