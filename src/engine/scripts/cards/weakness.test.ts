// `Weakness` - cast on Grizzly Bears it attaches and the enchanted creature gets -2/-1;
...
import { WEAKNESS_SCRIPT } from './weakness';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WEAKNESS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([WEAKNESS_SCRIPT]),
  });
  holdEverywhere(g);
