// `Ashen Monstrosity` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASHEN_MONSTROSITY_SCRIPT } from './ashenMonstrosity';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ashen Monstrosity";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ashen Monstrosity", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASHEN_MONSTROSITY_SCRIPT]),
  });
  holdEverywhere(g);
