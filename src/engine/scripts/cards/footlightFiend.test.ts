// `Footlight Fiend` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FOOTLIGHT_FIEND_SCRIPT } from './footlightFiend';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Footlight Fiend";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Footlight Fiend"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOOTLIGHT_FIEND_SCRIPT]),
  });
  holdEverywhere(g);
