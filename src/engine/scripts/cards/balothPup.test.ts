// `Baloth Pup` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BALOTH_PUP_SCRIPT } from './balothPup';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Baloth Pup";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BALOTH_PUP_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Baloth Pup"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BALOTH_PUP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Baloth Pup", () => {
  test("`Baloth Pup` - as long as it has a +1/+1 counter on it: absent while it is unmet, read once it holds", () => {
