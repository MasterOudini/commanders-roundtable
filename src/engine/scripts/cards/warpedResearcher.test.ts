// `Warped Researcher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WARPED_RESEARCHER_SCRIPT } from './warpedResearcher';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Warped Researcher";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([WARPED_RESEARCHER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Warped Researcher"], ["Cyclops of One-Eyed Pass", "Lonely Sandbar"]],
    scripts: createRegistry([WARPED_RESEARCHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Warped Researcher", () => {
  test("Whenever a player cycles a card: it gains flying and shroud until end of turn", () => {
