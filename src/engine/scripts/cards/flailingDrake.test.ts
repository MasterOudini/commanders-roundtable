// `Flailing Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLAILING_DRAKE_SCRIPT } from './flailingDrake';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flailing Drake";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FLAILING_DRAKE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flailing Drake"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLAILING_DRAKE_SCRIPT]),
...
describe("Flailing Drake", () => {
  test("Whenever this creature blocks or becomes blocked by a creature: the vocabulary resolves \"Target creature gets +1/+1 until end of turn.\"", () => {
    const { g, no } = armed(0);
