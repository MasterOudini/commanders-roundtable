// `Sword of the Chosen` - the named target is accepted and the pump lands; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SWORD_OF_THE_CHOSEN_SCRIPT } from './swordOfTheChosen';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sword of the Chosen";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Sword of the Chosen", "Lady Caleria", "Grizzly Bears"], ["Grizzly Bears"]],
    scripts: createRegistry([SWORD_OF_THE_CHOSEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sword of the Chosen", () => {
  test("Lady Caleria is a legal target and the pump lands", () => {
    ...
    settle(g);
    const d = deps(createRegistry([SWORD_OF_THE_CHOSEN_SCRIPT]));
    const got = derive(g.state, d.oracle, d.scripts, target);
    expect([got.power, got.toughness]).toEqual([5, 8]);
