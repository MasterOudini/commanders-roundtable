// `Captivating Crew` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAPTIVATING_CREW_SCRIPT } from './captivatingCrew';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Captivating Crew";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CAPTIVATING_CREW_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Captivating Crew"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAPTIVATING_CREW_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Captivating Crew", () => {
  test("{3}{R}: the vocabulary resolves \"Gain control of target creature an opponent controls until end of turn. Untap that creature. It gains haste until end of turn.\"", () => {
