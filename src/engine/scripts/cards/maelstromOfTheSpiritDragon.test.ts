// `Maelstrom of the Spirit Dragon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAELSTROM_OF_THE_SPIRIT_DRAGON_SCRIPT } from './maelstromOfTheSpiritDragon';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Maelstrom of the Spirit Dragon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Maelstrom of the Spirit Dragon", "Shivan Devastator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAELSTROM_OF_THE_SPIRIT_DRAGON_SCRIPT]),
...
describe("Maelstrom of the Spirit Dragon", () => {
  test("{4}, {T}, Sacrifice this land: the vocabulary resolves \"Search your library for a Dragon card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g, self } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Shivan Devastator" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
