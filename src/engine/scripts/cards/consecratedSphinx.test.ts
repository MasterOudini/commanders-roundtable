// `Consecrated Sphinx` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONSECRATED_SPHINX_SCRIPT } from './consecratedSphinx';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Consecrated Sphinx";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Consecrated Sphinx"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONSECRATED_SPHINX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Consecrated Sphinx", () => {
  test("Whenever an opponent draws a card: 2 cards are drawn", () => {
