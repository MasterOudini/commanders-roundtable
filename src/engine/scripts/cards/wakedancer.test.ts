// `Wakedancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WAKEDANCER_SCRIPT } from './wakedancer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wakedancer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wakedancer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WAKEDANCER_SCRIPT]),
  });
  holdEverywhere(g);
