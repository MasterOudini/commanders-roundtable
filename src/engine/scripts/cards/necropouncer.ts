// `Necropouncer` - a etb trigger germ, a static attachedStatic
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { NECROPOUNCER, PHYREXIAN_GERM_TOKEN } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(NECROPOUNCER, "Living weapon (When this Equipment enters, create a 0/0 black Phyrexian Germ creature token, then attach this to it.)\nEquipped creature gets +3/+1 and has haste.\nEquip {2}");
const LINES = PRINTED.split('\n');

export const NECROPOUNCER_SCRIPT: CardScript = {
  oracleId: NECROPOUNCER.oracleId,
  name: NECROPOUNCER.name,
  triggers: [
    {
      abilityId: 'etb-0',
