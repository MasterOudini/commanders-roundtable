// `Citanul Woodreaders` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CITANUL_WOODREADERS_SCRIPT } from './citanulWoodreaders';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Citanul Woodreaders";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Citanul Woodreaders"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CITANUL_WOODREADERS_SCRIPT]),
  });
  holdEverywhere(g);
