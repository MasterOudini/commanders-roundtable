// `Ondu Spiritdancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ONDU_SPIRITDANCER_SCRIPT } from './onduSpiritdancer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ondu Spiritdancer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ondu Spiritdancer", "Grizzly Bears", "Nyxborn Courser"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ONDU_SPIRITDANCER_SCRIPT]),
...
describe("Ondu Spiritdancer", () => {
  test("Whenever an enchantment you control enters: the vocabulary resolves \"Create a token that's a copy of target creature.\"", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test('the once-per-turn rider rides the def (D492)', () => {
    expect(ONDU_SPIRITDANCER_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("creatureEnters-0") && t.oncePerTurn === true), 'the def of line 0 carries oncePerTurn').toBeDefined();
  });
