// `Steelfin Whale` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Steelfin Whale", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STEELFIN_WHALE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Steelfin Whale", () => {
  test("Whenever an artifact you control enters: it untaps", () => {
