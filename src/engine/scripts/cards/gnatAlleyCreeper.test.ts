// `Gnat Alley Creeper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GNAT_ALLEY_CREEPER_SCRIPT } from './gnatAlleyCreeper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gnat Alley Creeper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gnat Alley Creeper", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Ornithopter"]],
    scripts: createRegistry([GNAT_ALLEY_CREEPER_SCRIPT]),
