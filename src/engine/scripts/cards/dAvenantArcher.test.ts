// `D'Avenant Archer` — damage on my ATTACKING creature: the right target is
// accepted, a creature that stayed home is refused (D291). Generated from one table row (D292).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { D_AVENANT_ARCHER_SCRIPT } from './dAvenantArcher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "D'Avenant Archer";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([D_AVENANT_ARCHER_SCRIPT]),
  });
