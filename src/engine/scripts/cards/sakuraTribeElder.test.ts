// `Sakura-Tribe Elder` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sakura-Tribe Elder", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SAKURA_TRIBE_ELDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sakura-Tribe Elder", () => {
  test("Sacrifice this creature: the vocabulary resolves \"Search your library for a basic land card, put that card onto the battlefield tapped, then shuffle.\"", () => {
