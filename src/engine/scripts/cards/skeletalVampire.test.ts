// `Skeletal Vampire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKELETAL_VAMPIRE_SCRIPT } from './skeletalVampire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skeletal Vampire";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skeletal Vampire", "Blight Keeper", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKELETAL_VAMPIRE_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = put(g, 'p1', "Blight Keeper");
  settle(g);
