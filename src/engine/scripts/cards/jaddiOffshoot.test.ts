// `Jaddi Offshoot` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jaddi Offshoot"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JADDI_OFFSHOOT_SCRIPT]),
  });
  holdEverywhere(g);
