// `Magefire Wings` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+0 and has flying;
...
import { MAGEFIRE_WINGS_SCRIPT } from './magefireWings';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Magefire Wings";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MAGEFIRE_WINGS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([MAGEFIRE_WINGS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Magefire Wings", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+0 and has flying; Cyclops of One-Eyed Pass is untouched", () => {
