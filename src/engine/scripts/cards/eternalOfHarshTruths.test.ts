// `Eternal of Harsh Truths` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ETERNAL_OF_HARSH_TRUTHS_SCRIPT } from './eternalOfHarshTruths';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Eternal of Harsh Truths";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Eternal of Harsh Truths"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ETERNAL_OF_HARSH_TRUTHS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Eternal of Harsh Truths", () => {
  test("Whenever this creature attacks and isn't blocked: a card is drawn", () => {
