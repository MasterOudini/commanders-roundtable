// `Scab-Clan Mauler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCAB_CLAN_MAULER_SCRIPT } from './scabClanMauler';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scab-Clan Mauler";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SCAB_CLAN_MAULER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scab-Clan Mauler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCAB_CLAN_MAULER_SCRIPT]),
  });
  holdEverywhere(g);
