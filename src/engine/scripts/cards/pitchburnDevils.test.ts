// `Pitchburn Devils` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PITCHBURN_DEVILS_SCRIPT } from './pitchburnDevils';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pitchburn Devils";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pitchburn Devils"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PITCHBURN_DEVILS_SCRIPT]),
  });
  holdEverywhere(g);
