// `Order of the Sacred Torch` - counters a held spell of the named colour and refuses one of another:
// the adjective is the parser's and the validator's (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ORDER_OF_THE_SACRED_TORCH_SCRIPT } from './orderOfTheSacredTorch';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Order of the Sacred Torch";
...
function armed(which: 'right' | 'wrong'): { g: Game; self: InstanceId; spell: InstanceId; stackId: string; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Order of the Sacred Torch"], ["Vampire Nighthawk", "Grizzly Bears"]],
    scripts: createRegistry([ORDER_OF_THE_SACRED_TORCH_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const spell = put(g, 'p2', which === 'right' ? "Vampire Nighthawk" : "Grizzly Bears", 'hand');
  settle(g);
...
describe("Order of the Sacred Torch", () => {
  test("the held Vampire Nighthawk is a legal target and is countered", () => {
