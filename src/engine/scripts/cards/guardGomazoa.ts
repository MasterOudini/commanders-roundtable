// `Guard Gomazoa` - a CONTINUOUS prevention effect (CR 615, D385): a `PreventionDef` the
...
import { GUARD_GOMAZOA } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(GUARD_GOMAZOA, "Defender, flying\nPrevent all combat damage that would be dealt to this creature.");
const LINES = PRINTED.split(String.fromCharCode(10));

export const GUARD_GOMAZOA_SCRIPT: CardScript = {
  oracleId: GUARD_GOMAZOA.oracleId,
  name: GUARD_GOMAZOA.name,
  prevention: [
    {
      abilityId: 'prevent-1',
