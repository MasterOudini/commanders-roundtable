// `Supply Caravan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUPPLY_CARAVAN_SCRIPT } from './supplyCaravan';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Supply Caravan";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Supply Caravan", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUPPLY_CARAVAN_SCRIPT]),
  });
  holdEverywhere(g);
