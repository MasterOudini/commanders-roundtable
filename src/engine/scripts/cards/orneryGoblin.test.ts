// `Ornery Goblin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORNERY_GOBLIN_SCRIPT } from './orneryGoblin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ornery Goblin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ornery Goblin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORNERY_GOBLIN_SCRIPT]),
