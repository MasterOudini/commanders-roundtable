// `Explosive Apparatus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EXPLOSIVE_APPARATUS_SCRIPT } from './explosiveApparatus';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Explosive Apparatus";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Explosive Apparatus"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EXPLOSIVE_APPARATUS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Explosive Apparatus", () => {
  test("{3}, {T}, Sacrifice this artifact: it deals 2 damage to the declared creature", () => {
