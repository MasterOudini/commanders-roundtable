// `Mogg Sentry` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOGG_SENTRY_SCRIPT } from './moggSentry';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mogg Sentry";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MOGG_SENTRY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mogg Sentry"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([MOGG_SENTRY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mogg Sentry", () => {
  test("Whenever an opponent casts a spell: it gets +2/+2 until end of turn", () => {
