// `Quicksilver Dagger` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { QUICKSILVER_DAGGER_SCRIPT } from './quicksilverDagger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Quicksilver Dagger";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Quicksilver Dagger","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([QUICKSILVER_DAGGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Quicksilver Dagger", () => {
  test("grants {T}: This creature deals 1 damage to target player or planeswalker. You draw a card. to Grizzly Bears, and to nothing outside the scope", () => {
