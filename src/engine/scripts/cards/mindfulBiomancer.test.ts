// `Mindful Biomancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MINDFUL_BIOMANCER_SCRIPT } from './mindfulBiomancer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mindful Biomancer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MINDFUL_BIOMANCER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mindful Biomancer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MINDFUL_BIOMANCER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mindful Biomancer", () => {
  test("When this creature enters: 1 life is gained", () => {
