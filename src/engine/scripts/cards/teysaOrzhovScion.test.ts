// `Teysa, Orzhov Scion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEYSA_ORZHOV_SCION_SCRIPT } from './teysaOrzhovScion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Teysa, Orzhov Scion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Teysa, Orzhov Scion", "Devoted Hero", "Grizzly Bears", "Muck Rats", "Devoted Hero", "Devoted Hero"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEYSA_ORZHOV_SCION_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = [put(g, 'p1', "Devoted Hero"), put(g, 'p1', "Devoted Hero"), put(g, 'p1', "Devoted Hero")];
  settle(g);
  ...
  if (which === 1) {
    const bears = put(g, 'p1', "Muck Rats");
...
describe("Teysa, Orzhov Scion", () => {
  test("Sacrifice three white creatures: the vocabulary resolves \"Exile target creature.\"", () => {
  ...
  test("Whenever another black creature you control dies: 1 token made", () => {
