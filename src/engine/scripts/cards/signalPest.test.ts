// `Signal Pest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SIGNAL_PEST_SCRIPT } from './signalPest';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Signal Pest";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SIGNAL_PEST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Signal Pest", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIGNAL_PEST_SCRIPT]),
...
describe("Signal Pest", () => {
  test("Battle cry (Whenever this creature attacks: it attacks beside the Bears, which gets +1/+0 until end of turn", () => {
    const { g, self, bearsB } = armed(0);
    expect(pt(g, bearsB)).toEqual([3, 2]);
    expect(pt(g, self)).toEqual([0, 1]);
  });

  test("This creature can't be blocked except by creatures with flying or reach.: the block by a creature outside the exception (creatures with flying or reach) is refused", () => {
