// `Aether Poisoner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AETHER_POISONER_SCRIPT } from './aetherPoisoner';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aether Poisoner";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aether Poisoner"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AETHER_POISONER_SCRIPT]),
