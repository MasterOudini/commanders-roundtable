// `Wayfarer's Bauble` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wayfarer's Bauble", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WAYFARERS_BAUBLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wayfarer's Bauble", () => {
  test("{2}, {T}, Sacrifice this artifact: the vocabulary resolves \"Search your library for a basic land card, put that card onto the battlefield tapped, then shuffle.\"", () => {
