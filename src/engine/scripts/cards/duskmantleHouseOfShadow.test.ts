// `Duskmantle, House of Shadow` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DUSKMANTLE_HOUSE_OF_SHADOW_SCRIPT } from './duskmantleHouseOfShadow';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Duskmantle, House of Shadow";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Duskmantle, House of Shadow"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUSKMANTLE_HOUSE_OF_SHADOW_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Duskmantle, House of Shadow", () => {
  test("{U}{B}, {T}: the vocabulary resolves \"Target player mills a card.\"", () => {
