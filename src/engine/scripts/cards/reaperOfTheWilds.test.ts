// `Reaper of the Wilds` - every printed ability proven in its own game: the cost's mark, the pump
...
import { REAPER_OF_THE_WILDS_SCRIPT } from './reaperOfTheWilds';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Reaper of the Wilds";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([REAPER_OF_THE_WILDS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Reaper of the Wilds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([REAPER_OF_THE_WILDS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Reaper of the Wilds", () => {
  test("Whenever another creature dies: scry 1 asks, and the cards stay on top", () => {
  ...
  test("{1}{G}: it gains hexproof until end of turn", () => {
