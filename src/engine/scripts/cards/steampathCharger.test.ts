// `Steampath Charger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STEAMPATH_CHARGER_SCRIPT } from './steampathCharger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Steampath Charger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Steampath Charger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STEAMPATH_CHARGER_SCRIPT]),
...
describe("Steampath Charger", () => {
  test("When this creature dies: it deals 1 damage to the declared player", () => {
