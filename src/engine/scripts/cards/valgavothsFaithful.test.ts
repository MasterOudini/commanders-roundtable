// `Valgavoth's Faithful` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VALGAVOTHS_FAITHFUL_SCRIPT } from './valgavothsFaithful';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Valgavoth's Faithful";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Valgavoth's Faithful", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VALGAVOTHS_FAITHFUL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Valgavoth's Faithful", () => {
  test("{3}{B}, Sacrifice this creature: the vocabulary resolves \"Return target creature card from your graveyard to the battlefield.\"", () => {
