// `Saruli Gatekeepers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SARULI_GATEKEEPERS_SCRIPT } from './saruliGatekeepers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Saruli Gatekeepers";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Saruli Gatekeepers", "Azorius Guildgate", "Azorius Guildgate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SARULI_GATEKEEPERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Saruli Gatekeepers", () => {
  test("When this creature enters: 7 life is gained", () => {
