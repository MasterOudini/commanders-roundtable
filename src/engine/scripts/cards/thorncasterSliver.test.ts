// `Thorncaster Sliver` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { THORNCASTER_SLIVER_SCRIPT } from './thorncasterSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thorncaster Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Thorncaster Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THORNCASTER_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thorncaster Sliver", () => {
  test("grants Whenever this creature attacks, it deals 1 damage to any target. to Metallic Sliver, and to nothing outside the scope", () => {
  ...
  test('it FIRES off the recipient and the body hits the RECIPIENT', () => {
    ...
    settle(g);
    expect(g.log.some((e) => e.body.t === 'DamageDealt' && e.body.damages.some((d) => d.source === host))).toBe(true);
  });
