// `Plaguemaw Beast` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PLAGUEMAW_BEAST_SCRIPT } from './plaguemawBeast';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Plaguemaw Beast";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Plaguemaw Beast", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PLAGUEMAW_BEAST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Plaguemaw Beast", () => {
  test("{T}, Sacrifice a creature: the vocabulary resolves \"Proliferate.\"", () => {
