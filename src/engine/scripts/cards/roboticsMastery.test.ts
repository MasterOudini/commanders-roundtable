// `Robotics Mastery` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: 2 tokens); gets +2/+2;
...
import { ROBOTICS_MASTERY_SCRIPT } from './roboticsMastery';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Robotics Mastery";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ROBOTICS_MASTERY_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ROBOTICS_MASTERY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Robotics Mastery", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: 2 tokens); gets +2/+2; Cyclops of One-Eyed Pass is untouched", () => {
