// `Changeling Wayfinder` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Changeling Wayfinder", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHANGELING_WAYFINDER_SCRIPT]),
  });
  holdEverywhere(g);
