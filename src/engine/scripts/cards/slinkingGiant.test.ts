// `Slinking Giant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLINKING_GIANT_SCRIPT } from './slinkingGiant';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slinking Giant";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SLINKING_GIANT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slinking Giant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLINKING_GIANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Slinking Giant", () => {
  test("Whenever this creature blocks or becomes blocked: it gets -3/+0 until end of turn", () => {
