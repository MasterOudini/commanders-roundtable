// `Smelt-Ward Gatekeepers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SMELT_WARD_GATEKEEPERS_SCRIPT } from './smeltWardGatekeepers';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Smelt-Ward Gatekeepers";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SMELT_WARD_GATEKEEPERS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Smelt-Ward Gatekeepers", "Azorius Guildgate", "Azorius Guildgate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SMELT_WARD_GATEKEEPERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Smelt-Ward Gatekeepers", () => {
  test("When this creature enters: the vocabulary resolves \"Gain control of target creature an opponent controls until end of turn. Untap that creature. It gains haste until end of turn.\"", () => {
