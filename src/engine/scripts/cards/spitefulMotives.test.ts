// `Spiteful Motives` - cast on Grizzly Bears it attaches and the enchanted creature gets +3/+0 and has firstStrike;
...
import { SPITEFUL_MOTIVES_SCRIPT } from './spitefulMotives';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spiteful Motives";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPITEFUL_MOTIVES_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SPITEFUL_MOTIVES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spiteful Motives", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +3/+0 and has firstStrike; Cyclops of One-Eyed Pass is untouched", () => {
