// `Coffin Puppets` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COFFIN_PUPPETS_SCRIPT } from './coffinPuppets';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Coffin Puppets";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Coffin Puppets", "Forest", "Forest", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COFFIN_PUPPETS_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = [put(g, 'p1', "Forest"), put(g, 'p1', "Forest")];
  settle(g);
  const self = put(g, 'p1', CARD, [0].includes(which) ? 'graveyard' : 'battlefield');
  settle(g);
  const cond0_1_0 = put(g, 'p1', "Swamp");
  let condRefused = false;
  if (which === 0) {
    // D353 - the opponent's second turn: not your turn, not your upkeep - refused for the condition alone.
    advanceUntil(g, (s) => s.turn.turnNumber === 2 && s.turn.step === 'upkeep' && s.priority.player === 'p1' && s.priority.awaiting === null, 20_000);
    { const early = g.submit({ t: 'ActivateAbility', player: 'p1', card: self, abilityIndex: 0, sacrifice: fodder0.slice(0, 2) }); condRefused = !early.ok && early.reason === 'timingRestriction'; }
  }
  ...
  if (which === 0) {
    // D353 - the condition broken: every fixture exiled (the card itself may count), the activation refused, the fixtures back.
...
describe("Coffin Puppets", () => {
  test("Sacrifice two lands: it returns from the graveyard to the battlefield - refused unless during your upkeep and only if you control a Swamp", () => {
    const { g, self, condRefused, fodder0 } = armed(0);
