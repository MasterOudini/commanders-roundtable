// `Oculus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OCULUS_SCRIPT } from './oculus';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Oculus"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OCULUS_SCRIPT]),
  });
  holdEverywhere(g);
