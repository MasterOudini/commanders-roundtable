// `Imperious Perfect` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IMPERIOUS_PERFECT_SCRIPT } from './imperiousPerfect';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Imperious Perfect";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([IMPERIOUS_PERFECT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Imperious Perfect", "Willow Elf"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IMPERIOUS_PERFECT_SCRIPT]),
  });
  holdEverywhere(g);
