// `Jeskai Monument` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jeskai Monument", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JESKAI_MONUMENT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Jeskai Monument", () => {
  test("When this artifact enters: the vocabulary resolves \"Search your library for a basic Island, Mountain, or Plains card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
  ...
  test("{1}{U}{R}{W}, {T}, Sacrifice this artifact: 2 tokens made", () => {
