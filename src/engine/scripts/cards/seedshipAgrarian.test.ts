// `Seedship Agrarian` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEEDSHIP_AGRARIAN_SCRIPT } from './seedshipAgrarian';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Seedship Agrarian";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seedship Agrarian"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEEDSHIP_AGRARIAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Seedship Agrarian", () => {
  test("Whenever this creature becomes tapped: the vocabulary resolves \"Create a Lander token.\"", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("Landfall — Whenever a land you control enters: it gets 1 +1/+1 counter", () => {
