// `Brilliant Halo` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRILLIANT_HALO_SCRIPT } from './brilliantHalo';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brilliant Halo";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BRILLIANT_HALO_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brilliant Halo", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRILLIANT_HALO_SCRIPT]),
  });
  holdEverywhere(g);
