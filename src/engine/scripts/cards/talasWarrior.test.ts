// `Talas Warrior` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TALAS_WARRIOR_SCRIPT } from './talasWarrior';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Talas Warrior";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Talas Warrior", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TALAS_WARRIOR_SCRIPT]),
  });
  holdEverywhere(g);
