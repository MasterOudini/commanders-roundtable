// `Scrapwork Cohort` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCRAPWORK_COHORT_SCRIPT } from './scrapworkCohort';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scrapwork Cohort";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scrapwork Cohort"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCRAPWORK_COHORT_SCRIPT]),
