// `Charging Bandits` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHARGING_BANDITS_SCRIPT } from './chargingBandits';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Charging Bandits";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHARGING_BANDITS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Charging Bandits"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHARGING_BANDITS_SCRIPT]),
  });
  holdEverywhere(g);
