// `Cudgel Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CUDGEL_TROLL_SCRIPT } from './cudgelTroll';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cudgel Troll";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cudgel Troll", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CUDGEL_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
