// `Nimana Sell-Sword` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIMANA_SELL_SWORD_SCRIPT } from './nimanaSellSword';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nimana Sell-Sword";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nimana Sell-Sword"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIMANA_SELL_SWORD_SCRIPT]),
  });
  holdEverywhere(g);
