// `Veteran of the Depths` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VETERAN_OF_THE_DEPTHS_SCRIPT } from './veteranOfTheDepths';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Veteran of the Depths";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Veteran of the Depths"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VETERAN_OF_THE_DEPTHS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Veteran of the Depths", () => {
  test("Whenever this creature becomes tapped: it gets 1 +1/+1 counter", () => {
