// `Bladed Pinions` - equipped to Grizzly Bears the creature has flying, firstStrike; the Cyclops is untouched;
...
import { BLADED_PINIONS_SCRIPT } from './bladedPinions';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bladed Pinions";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([BLADED_PINIONS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([BLADED_PINIONS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bladed Pinions", () => {
  test("equipped to Grizzly Bears: attached, and the creature has flying, firstStrike; the Cyclops is untouched", () => {
