// `Serpent Assassin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SERPENT_ASSASSIN_SCRIPT } from './serpentAssassin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Serpent Assassin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Serpent Assassin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SERPENT_ASSASSIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Serpent Assassin", () => {
  test("When this creature enters: the vocabulary resolves \"Destroy target nonblack creature.\"", () => {
    const { g, no } = armed(0);
