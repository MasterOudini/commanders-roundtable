// `Skyscythe Engulfer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKYSCYTHE_ENGULFER_SCRIPT } from './skyscytheEngulfer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skyscythe Engulfer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skyscythe Engulfer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Ornithopter"]],
    scripts: createRegistry([SKYSCYTHE_ENGULFER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const gift2 = put(g, 'p2', "Ornithopter");
  settle(g);
...
describe("Skyscythe Engulfer", () => {
  test("This creature can't be blocked by creatures with flying.: the block by creatures with flying is refused", () => {
