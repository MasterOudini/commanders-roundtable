// `Atalan Jackal` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Atalan Jackal", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ATALAN_JACKAL_SCRIPT]),
...
describe("Atalan Jackal", () => {
  test("Skilled Outrider — Whenever this creature deals combat damage to a player: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
