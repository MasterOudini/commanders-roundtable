// `Most Wanted` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOST_WANTED_SCRIPT } from './mostWanted';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Most Wanted";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MOST_WANTED_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Most Wanted", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOST_WANTED_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Most Wanted", () => {
  test("Enchanted creature gets +2/+1.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([4, 3]);
  });

  test("When enchanted creature dies: 2 tokens made", () => {
