// `Cat Burglar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAT_BURGLAR_SCRIPT } from './catBurglar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cat Burglar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cat Burglar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAT_BURGLAR_SCRIPT]),
  });
  holdEverywhere(g);
