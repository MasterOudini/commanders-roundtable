// `Myrsmith` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MYRSMITH_SCRIPT } from './myrsmith';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Myrsmith", "Sol Ring", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MYRSMITH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Myrsmith", () => {
  test("Whenever you cast an artifact spell: the vocabulary resolves \"You may pay {1}. If you do, create a 1/1 colorless Myr artifact creature token.\" - the price is declined", () => {
  ...
  test("Whenever you cast an artifact spell: the vocabulary resolves \"You may pay {1}. If you do, create a 1/1 colorless Myr artifact creature token.\" - the price is paid", () => {
