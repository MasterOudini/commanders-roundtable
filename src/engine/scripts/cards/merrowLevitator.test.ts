// `Merrow Levitator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERROW_LEVITATOR_SCRIPT } from './merrowLevitator';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Merrow Levitator";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MERROW_LEVITATOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merrow Levitator", "Grizzly Bears", "Aegis Turtle"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERROW_LEVITATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Merrow Levitator", () => {
  ...
  test("Whenever you cast a blue spell: it untaps", () => {
