// `Makindi Patrol` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAKINDI_PATROL_SCRIPT } from './makindiPatrol';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Makindi Patrol";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MAKINDI_PATROL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Makindi Patrol", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAKINDI_PATROL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Makindi Patrol", () => {
  test("Rally — Whenever this creature or another Ally you control enters: its controller's creatures gain vigilance until end of turn", () => {
