// `Nimbus Swimmer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIMBUS_SWIMMER_SCRIPT } from './nimbusSwimmer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nimbus Swimmer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NIMBUS_SWIMMER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nimbus Swimmer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIMBUS_SWIMMER_SCRIPT]),
  });
  holdEverywhere(g);
