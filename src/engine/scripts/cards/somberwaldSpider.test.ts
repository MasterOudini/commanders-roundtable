// `Somberwald Spider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOMBERWALD_SPIDER_SCRIPT } from './somberwaldSpider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Somberwald Spider";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SOMBERWALD_SPIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Somberwald Spider", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOMBERWALD_SPIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Somberwald Spider", () => {
  test("Morbid — This creature enters with two +1/+1 counters on it if a creature died this turn.: cast, it enters with 2 +1/+1 counters", () => {
