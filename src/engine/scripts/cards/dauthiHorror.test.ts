// `Dauthi Horror` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAUTHI_HORROR_SCRIPT } from './dauthiHorror';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dauthi Horror";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dauthi Horror", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Devoted Hero"]],
    scripts: createRegistry([DAUTHI_HORROR_SCRIPT]),
