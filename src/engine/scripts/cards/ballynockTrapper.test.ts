// `Ballynock Trapper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BALLYNOCK_TRAPPER_SCRIPT } from './ballynockTrapper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ballynock Trapper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ballynock Trapper", "Coral Eel", "Grizzly Bears", "Devoted Hero"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BALLYNOCK_TRAPPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ballynock Trapper", () => {
  test("{T}: the declared creature is tapped", () => {
    const { g, self, no } = armed(0);
    expect(g.state.cards[no]?.tapped).toBe(true);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("Whenever you cast a white spell: it untaps", () => {
