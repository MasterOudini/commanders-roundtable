// `Research Assistant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RESEARCH_ASSISTANT_SCRIPT } from './researchAssistant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Research Assistant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Research Assistant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RESEARCH_ASSISTANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Research Assistant", () => {
  test("{3}{U}, {T}: a card is drawn, then one discarded", () => {
