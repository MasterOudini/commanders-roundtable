// `Esperzoa` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ESPERZOA_SCRIPT } from './esperzoa';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Esperzoa", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ESPERZOA_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const queueFodderL1_upkeep = put(g, 'p1', "Sol Ring");
  settle(g);
...
describe("Esperzoa", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Return an artifact you control to its owner's hand.\"", () => {
