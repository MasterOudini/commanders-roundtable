// `Darkthicket Wolf` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DARKTHICKET_WOLF_SCRIPT } from './darkthicketWolf';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Darkthicket Wolf";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DARKTHICKET_WOLF_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Darkthicket Wolf", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DARKTHICKET_WOLF_SCRIPT]),
  });
  holdEverywhere(g);
