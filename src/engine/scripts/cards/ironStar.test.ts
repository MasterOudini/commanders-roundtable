// `Iron Star` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IRON_STAR_SCRIPT } from './ironStar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Iron Star";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Iron Star", "Grizzly Bears", "Crimson Kobolds", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IRON_STAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Iron Star", () => {
  test("Whenever a player casts a red spell: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is declined", () => {
  ...
  test("Whenever a player casts a red spell: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is paid", () => {
