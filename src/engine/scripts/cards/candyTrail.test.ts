// `Candy Trail` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CANDY_TRAIL_SCRIPT } from './candyTrail';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Candy Trail";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Candy Trail"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CANDY_TRAIL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Candy Trail", () => {
  ...
  test("{2}, {T}, Sacrifice this artifact: the vocabulary resolves \"You gain 3 life and draw a card.\"", () => {
