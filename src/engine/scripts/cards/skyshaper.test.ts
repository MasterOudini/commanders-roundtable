// `Skyshaper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKYSHAPER_SCRIPT } from './skyshaper';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skyshaper";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SKYSHAPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skyshaper", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKYSHAPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skyshaper", () => {
  test("Sacrifice this artifact: its controller's creatures gain flying until end of turn", () => {
