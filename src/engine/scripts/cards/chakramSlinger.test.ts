// `Chakram Slinger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHAKRAM_SLINGER_SCRIPT } from './chakramSlinger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chakram Slinger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chakram Slinger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHAKRAM_SLINGER_SCRIPT]),
...
describe("Chakram Slinger", () => {
  test("{R}, {T}: it deals 2 damage to the declared player", () => {
