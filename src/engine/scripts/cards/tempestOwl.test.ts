// `Tempest Owl` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEMPEST_OWL_SCRIPT } from './tempestOwl';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tempest Owl";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tempest Owl"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEMPEST_OWL_SCRIPT]),
...
describe("Tempest Owl", () => {
  test("When this creature enters: the vocabulary resolves \"Tap up to three target permanents.\"", () => {
    const { g, no } = armed(0);
