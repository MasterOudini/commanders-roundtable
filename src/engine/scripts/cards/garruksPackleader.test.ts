// `Garruk's Packleader` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GARRUKS_PACKLEADER_SCRIPT } from './garruksPackleader';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Garruk's Packleader";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Garruk's Packleader", "Grizzly Bears", "Blade of the Sixth Pride"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GARRUKS_PACKLEADER_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    bears = put(g, 'p1', "Blade of the Sixth Pride");
...
describe("Garruk's Packleader", () => {
  test("Whenever another creature you control with power 3 or greater enters: a card is drawn", () => {
