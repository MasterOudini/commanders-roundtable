// `Twisted Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Twisted Landscape", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TWISTED_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Twisted Landscape", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Swamp, Mountain, or Forest card, put it onto the battlefield tapped, then shuffle.\"", () => {
