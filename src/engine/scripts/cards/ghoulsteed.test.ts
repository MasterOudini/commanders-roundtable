// `Ghoulsteed` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHOULSTEED_SCRIPT } from './ghoulsteed';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghoulsteed";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghoulsteed", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHOULSTEED_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ghoulsteed", () => {
  test("{2}{B}, Discard two cards: it returns from the graveyard to the battlefield tapped", () => {
