// `Niv-Mizzet, Dracogenius` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIV_MIZZET_DRACOGENIUS_SCRIPT } from './nivMizzetDracogenius';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Niv-Mizzet, Dracogenius";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Niv-Mizzet, Dracogenius"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIV_MIZZET_DRACOGENIUS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Niv-Mizzet, Dracogenius", () => {
  test("Whenever Niv-Mizzet deals damage to a player: a card is drawn", () => {
    const { g, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1);
  });

  test("{U}{R}: it deals 1 damage to the declared creature", () => {
    const { g, no } = armed(1);
