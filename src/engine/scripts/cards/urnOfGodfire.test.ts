// `Urn of Godfire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { URN_OF_GODFIRE_SCRIPT } from './urnOfGodfire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Urn of Godfire";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Urn of Godfire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([URN_OF_GODFIRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Urn of Godfire", () => {
  test("{6}, {T}, Sacrifice this artifact: the vocabulary resolves \"Destroy target creature or enchantment.\"", () => {
