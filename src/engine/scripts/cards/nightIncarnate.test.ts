// `Night Incarnate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIGHT_INCARNATE_SCRIPT } from './nightIncarnate';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Night Incarnate";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NIGHT_INCARNATE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Night Incarnate", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIGHT_INCARNATE_SCRIPT]),
...
describe("Night Incarnate", () => {
  test("When this creature leaves the battlefield: all creatures get -3/-3 until end of turn", () => {
    const { g, no, yes } = armed(0);
    expect(pt(g, yes)).toEqual([-1, -2]);
