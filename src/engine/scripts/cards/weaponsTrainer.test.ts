// `Weapons Trainer` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WEAPONS_TRAINER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Weapons Trainer", "Coral Eel", "Swiftfoot Boots"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WEAPONS_TRAINER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Weapons Trainer", () => {
  test("`Weapons Trainer` - as long as you control an Equipment: absent while it is unmet, read once it holds", () => {
