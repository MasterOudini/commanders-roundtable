// `Mirozel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MIROZEL_SCRIPT } from './mirozel';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mirozel", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIROZEL_SCRIPT]),
  });
  holdEverywhere(g);
