// `Oashra Cultivator` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Oashra Cultivator", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OASHRA_CULTIVATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Oashra Cultivator", () => {
  test("{2}{G}, {T}, Sacrifice this creature: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
