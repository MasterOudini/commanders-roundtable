// `Scrapyard Recombiner` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scrapyard Recombiner", "Sol Ring", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCRAPYARD_RECOMBINER_SCRIPT]),
...
describe("Scrapyard Recombiner", () => {
  test("{T}, Sacrifice an artifact: the vocabulary resolves \"Search your library for a Construct card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g, self, fodder0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Memnite" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Memnite");
      expect(found, "Memnite is not in the library for the search to find").toBeDefined();
