// `Kaervek, the Spiteful` - the anthem reaches the permanent its scope names and not the other;
// it ends when the source leaves; replay equal (D300). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { KAERVEK_THE_SPITEFUL_SCRIPT } from './kaervekTheSpiteful';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kaervek, the Spiteful";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KAERVEK_THE_SPITEFUL_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Kaervek, the Spiteful"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KAERVEK_THE_SPITEFUL_SCRIPT]),
  });
  holdEverywhere(g);
