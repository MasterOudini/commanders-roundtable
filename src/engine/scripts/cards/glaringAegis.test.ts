// `Glaring Aegis` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GLARING_AEGIS_SCRIPT } from './glaringAegis';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Glaring Aegis";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GLARING_AEGIS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glaring Aegis", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLARING_AEGIS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Glaring Aegis", () => {
  test("When this Aura enters: the vocabulary resolves \"Tap target creature an opponent controls.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.tapped).toBe(true);
  });

  test("Enchanted creature gets +1/+3.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([3, 5]);
  });
