// `Urza's Chalice` - every printed ability proven in its own game: the cost's mark, the pump
...
import { URZAS_CHALICE_SCRIPT } from './urzasChalice';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Urza's Chalice";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Urza's Chalice", "Grizzly Bears", "Memnite", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([URZAS_CHALICE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Urza's Chalice", () => {
  test("Whenever a player casts an artifact spell: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is declined", () => {
  ...
  test("Whenever a player casts an artifact spell: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is paid", () => {
