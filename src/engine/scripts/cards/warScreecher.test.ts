// `War Screecher` - the printed cost buys the pump until end of turn; it ends at cleanup;
// replay equal (D301). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { WAR_SCREECHER_SCRIPT } from './warScreecher';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "War Screecher";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WAR_SCREECHER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["War Screecher", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WAR_SCREECHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("War Screecher", () => {
  test("{5}{W}, {T}: its controller's creatures get +1/+1 until end of turn", () => {
