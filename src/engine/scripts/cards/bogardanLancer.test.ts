// `Bogardan Lancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BOGARDAN_LANCER_SCRIPT } from './bogardanLancer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bogardan Lancer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BOGARDAN_LANCER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bogardan Lancer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOGARDAN_LANCER_SCRIPT]),
  });
  holdEverywhere(g);
