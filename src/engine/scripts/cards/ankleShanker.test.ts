// `Ankle Shanker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ANKLE_SHANKER_SCRIPT } from './ankleShanker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ankle Shanker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ANKLE_SHANKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ankle Shanker", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ANKLE_SHANKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ankle Shanker", () => {
  test("Whenever this creature attacks: its controller's creatures gain firstStrike and deathtouch until end of turn", () => {
