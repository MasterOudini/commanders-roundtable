// `Nissa's Defeat` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { NISSAS_DEFEAT_SCRIPT } from './nissasDefeat';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nissa's Defeat";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; hosts: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Nissa's Defeat"], ["Forest", "Island"]],
    scripts: createRegistry([NISSAS_DEFEAT_SCRIPT]),
  });
  holdEverywhere(g);
  const targets: InstanceId[] = [];
  const hosts: InstanceId[] = [];
  targets.push(put(g, 'p2', "Forest"));
  const wrong = put(g, 'p2', "Island");
...
describe("Nissa's Defeat", () => {
  test('the named target is accepted and the effect lands', () => {
    ...
    expect((g.state.zones.hand.p1 ?? []).length - handBefore.p1).toBe(0);
  });

  test("Island is refused where the clause excludes it (D294)", () => {
