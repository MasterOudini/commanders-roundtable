// `Squirrel Nest` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { SQUIRREL_NEST_SCRIPT } from './squirrelNest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Squirrel Nest";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Squirrel Nest","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SQUIRREL_NEST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Squirrel Nest", () => {
  test("grants {T}: Create a 1/1 green Squirrel creature token. to Forest, and to nothing outside the scope", () => {
