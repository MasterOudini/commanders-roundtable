// `Phase Dolphin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHASE_DOLPHIN_SCRIPT } from './phaseDolphin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phase Dolphin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phase Dolphin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHASE_DOLPHIN_SCRIPT]),
