// `Good-Fortune Unicorn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOOD_FORTUNE_UNICORN_SCRIPT } from './goodFortuneUnicorn';
import { advanceUntil, holdEverywhere, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Good-Fortune Unicorn";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Good-Fortune Unicorn", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOOD_FORTUNE_UNICORN_SCRIPT]),
...
describe("Good-Fortune Unicorn", () => {
  test("Whenever another creature you control enters: the vocabulary resolves \"Put a +1/+1 counter on target creature.\"", () => {
