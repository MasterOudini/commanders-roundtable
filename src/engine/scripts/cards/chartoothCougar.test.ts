// `Chartooth Cougar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHARTOOTH_COUGAR_SCRIPT } from './chartoothCougar';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chartooth Cougar";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHARTOOTH_COUGAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chartooth Cougar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHARTOOTH_COUGAR_SCRIPT]),
  });
  holdEverywhere(g);
