// `Social Climber` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Social Climber", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOCIAL_CLIMBER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Social Climber", () => {
  test("Alliance — Whenever another creature you control enters: 1 life is gained", () => {
