// `Bad River` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bad River", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BAD_RIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bad River", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for an Island or Swamp card, put it onto the battlefield, then shuffle.\"", () => {
