// `Arid Mesa` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Arid Mesa", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARID_MESA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Arid Mesa", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for a Mountain or Plains card, put it onto the battlefield, then shuffle.\"", () => {
