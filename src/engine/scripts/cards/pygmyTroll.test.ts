// `Pygmy Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PYGMY_TROLL_SCRIPT } from './pygmyTroll';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pygmy Troll";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PYGMY_TROLL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pygmy Troll", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Memnite"]],
    scripts: createRegistry([PYGMY_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pygmy Troll", () => {
  ...
  test("{G}: it regenerates from the Bolt", () => {
