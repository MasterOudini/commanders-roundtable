// `Goblin Turncoat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_TURNCOAT_SCRIPT } from './goblinTurncoat';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Turncoat";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Turncoat", "Raging Goblin", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_TURNCOAT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Turncoat", () => {
  test("Sacrifice a Goblin: it regenerates from the Bolt", () => {
