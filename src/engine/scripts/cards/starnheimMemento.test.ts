// `Starnheim Memento` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STARNHEIM_MEMENTO_SCRIPT } from './starnheimMemento';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Starnheim Memento";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STARNHEIM_MEMENTO_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Starnheim Memento"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STARNHEIM_MEMENTO_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Starnheim Memento", () => {
  test("{1}{W}, {T}: the declared creature is pumped until end of turn", () => {
