// `Hot Springs` - the carrier proven end to end (D367/D368) with a D384 payload: the recipient
...
import { HOT_SPRINGS_SCRIPT } from './hotSprings';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hot Springs";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Hot Springs","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOT_SPRINGS_SCRIPT]),
  });
  holdEverywhere(g);
