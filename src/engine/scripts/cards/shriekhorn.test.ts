// `Shriekhorn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHRIEKHORN_SCRIPT } from './shriekhorn';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shriekhorn";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHRIEKHORN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shriekhorn"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHRIEKHORN_SCRIPT]),
...
describe("Shriekhorn", () => {
  ...
  test("{T}, Remove a charge counter from this artifact: the vocabulary resolves \"Target player mills two cards.\"", () => {
    const { g, self, p2gy0, cnt0 } = armed(1);
