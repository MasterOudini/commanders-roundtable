// `Ultimo, Civilization's End` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ULTIMO_CIVILIZATIONS_END_SCRIPT } from './ultimoCivilizationsEnd';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ultimo, Civilization's End";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ultimo, Civilization's End"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ULTIMO_CIVILIZATIONS_END_SCRIPT]),
...
describe("Ultimo, Civilization's End", () => {
  test("When Ultimo enters: the vocabulary resolves \"Each opponent sacrifices a creature of their choice.\"", () => {
    const { g } = armed(0);
  ...
  test("{2}{B}, Discard this card: the vocabulary resolves \"Each opponent sacrifices a creature of their choice.\"", () => {
