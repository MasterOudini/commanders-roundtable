// `Owlbear` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OWLBEAR_SCRIPT } from './owlbear';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Owlbear"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OWLBEAR_SCRIPT]),
...
describe("Owlbear", () => {
  test("Keen Senses — When this creature enters: a card is drawn", () => {
