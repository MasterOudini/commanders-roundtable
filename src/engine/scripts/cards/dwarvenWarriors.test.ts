// `Dwarven Warriors` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DWARVEN_WARRIORS_SCRIPT } from './dwarvenWarriors';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dwarven Warriors";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dwarven Warriors"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([DWARVEN_WARRIORS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dwarven Warriors", () => {
  test("{T}: the vocabulary resolves \"Target creature with power 2 or less can't be blocked this turn.\"", () => {
