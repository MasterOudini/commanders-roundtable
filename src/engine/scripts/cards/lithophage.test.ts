// `Lithophage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LITHOPHAGE_SCRIPT } from './lithophage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lithophage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lithophage", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LITHOPHAGE_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const priceL0_p1_0 = put(g, 'p1', "Mountain");
  settle(g);
...
describe("Lithophage", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you sacrifice a Mountain.\" - the price is declined", () => {
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you sacrifice a Mountain.\" - the price is paid", () => {
    const { g, self, board0, priceL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
