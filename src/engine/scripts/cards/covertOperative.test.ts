// `Covert Operative` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COVERT_OPERATIVE_SCRIPT } from './covertOperative';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Covert Operative";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Covert Operative", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COVERT_OPERATIVE_SCRIPT]),
  });
  holdEverywhere(g);
