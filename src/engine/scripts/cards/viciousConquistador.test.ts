// `Vicious Conquistador` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VICIOUS_CONQUISTADOR_SCRIPT } from './viciousConquistador';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vicious Conquistador";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vicious Conquistador"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VICIOUS_CONQUISTADOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vicious Conquistador", () => {
  test("Whenever this creature attacks: each opponent loses 1 life", () => {
