// `Screaming Phantom` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCREAMING_PHANTOM_SCRIPT } from './screamingPhantom';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Screaming Phantom";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Screaming Phantom"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCREAMING_PHANTOM_SCRIPT]),
  });
  holdEverywhere(g);
