// `April, Reporter of the Weird` - every printed ability proven in its own game: the cost's mark, the pump
...
import { APRIL_REPORTER_OF_THE_WEIRD_SCRIPT } from './aprilReporterOfTheWeird';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "April, Reporter of the Weird";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["April, Reporter of the Weird", "Hill Giant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([APRIL_REPORTER_OF_THE_WEIRD_SCRIPT]),
...
describe("April, Reporter of the Weird", () => {
  test("Whenever April deals combat damage to a player: the vocabulary resolves \"Draw that many cards, then discard a card.\"", () => {
    ...
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 2 - qdL0);
  });
