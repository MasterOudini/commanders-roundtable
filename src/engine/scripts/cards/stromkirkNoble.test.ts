// `Stromkirk Noble` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STROMKIRK_NOBLE_SCRIPT } from './stromkirkNoble';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stromkirk Noble";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stromkirk Noble", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Eager Cadet"]],
    scripts: createRegistry([STROMKIRK_NOBLE_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const gift2 = put(g, 'p2', "Eager Cadet");
  settle(g);
...
describe("Stromkirk Noble", () => {
  test("This creature can't be blocked by Humans.: the block by Humans is refused", () => {
