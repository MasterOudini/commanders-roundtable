// `Prickly Marmoset` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PRICKLY_MARMOSET_SCRIPT } from './pricklyMarmoset';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prickly Marmoset";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([PRICKLY_MARMOSET_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prickly Marmoset", "Lonely Sandbar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRICKLY_MARMOSET_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Prickly Marmoset", () => {
  test("Whenever you cycle a card: it gets +2/+0 until end of turn", () => {
