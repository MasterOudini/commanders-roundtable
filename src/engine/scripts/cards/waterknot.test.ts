// `Waterknot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WATERKNOT_SCRIPT } from './waterknot';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Waterknot";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Waterknot", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WATERKNOT_SCRIPT]),
  });
  holdEverywhere(g);
