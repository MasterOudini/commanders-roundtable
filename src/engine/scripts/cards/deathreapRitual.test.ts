// `Deathreap Ritual` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEATHREAP_RITUAL_SCRIPT } from './deathreapRitual';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deathreap Ritual";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deathreap Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEATHREAP_RITUAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deathreap Ritual", () => {
  test("Morbid — At the beginning of each end step: a card is drawn", () => {
