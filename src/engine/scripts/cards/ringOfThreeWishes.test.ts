// `Ring of Three Wishes` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RING_OF_THREE_WISHES_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ring of Three Wishes", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RING_OF_THREE_WISHES_SCRIPT]),
  ...
  if (which === 1) {
    ...
    settle(g);
    cnt0 = g.state.cards[self]?.counters["wish"] ?? 0;
    pt0 = pt(g, self);
    must(g.submit({ t: 'ManualAddMana', player: 'p1', target: 'p1', symbol: 'C', amount: 5 }));
...
describe("Ring of Three Wishes", () => {
  test("This artifact enters with three wish counters on it.: cast, it enters with 3 wish counters", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.zone.kind).toBe('battlefield');
    expect(g.state.cards[self]?.counters["wish"] ?? 0).toBe(3);
  });

  test("{5}, {T}, Remove a wish counter from this artifact: the vocabulary resolves \"Search your library for a card, put that card into your hand, then shuffle.\"", () => {
    ...
    expect(g.state.cards[self]?.counters["wish"] ?? 0).toBe(cnt0 - 1);
