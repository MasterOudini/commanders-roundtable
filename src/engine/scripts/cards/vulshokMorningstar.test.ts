// `Vulshok Morningstar` - equipped to Grizzly Bears the creature gets +2/+2; the Cyclops is untouched;
...
import { VULSHOK_MORNINGSTAR_SCRIPT } from './vulshokMorningstar';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vulshok Morningstar";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VULSHOK_MORNINGSTAR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([VULSHOK_MORNINGSTAR_SCRIPT]),
  });
  holdEverywhere(g);
