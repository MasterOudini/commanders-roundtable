// `Mysterio's Phantasm` - a attacks trigger mill
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { MYSTERIO_S_PHANTASM } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(MYSTERIO_S_PHANTASM, "Flying, vigilance\nWhenever this creature attacks, mill a card. (Put the top card of your library into your graveyard.)");
const LINES = PRINTED.split('\n');

export const MYSTERIOS_PHANTASM_SCRIPT: CardScript = {
  oracleId: MYSTERIO_S_PHANTASM.oracleId,
  name: MYSTERIO_S_PHANTASM.name,
  triggers: [
    {
      abilityId: 'attacks-1',
      ...
      label: () => "Mysterio's Phantasm - mill",
