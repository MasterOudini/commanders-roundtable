// `Savage Gorger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SAVAGE_GORGER_SCRIPT } from './savageGorger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Savage Gorger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Savage Gorger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SAVAGE_GORGER_SCRIPT]),
  });
  holdEverywhere(g);
