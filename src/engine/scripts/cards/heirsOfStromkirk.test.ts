// `Heirs of Stromkirk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HEIRS_OF_STROMKIRK_SCRIPT } from './heirsOfStromkirk';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Heirs of Stromkirk";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Heirs of Stromkirk"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HEIRS_OF_STROMKIRK_SCRIPT]),
  });
  holdEverywhere(g);
