// `Temur Monument` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Temur Monument", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEMUR_MONUMENT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Temur Monument", () => {
  test("When this artifact enters: the vocabulary resolves \"Search your library for a basic Forest, Island, or Mountain card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
  ...
  test("{3}{G}{U}{R}, {T}, Sacrifice this artifact: 1 token made", () => {
