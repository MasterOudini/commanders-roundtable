// `Captivating Cave` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAPTIVATING_CAVE_SCRIPT } from './captivatingCave';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Captivating Cave";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Captivating Cave"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAPTIVATING_CAVE_SCRIPT]),
  });
  holdEverywhere(g);
