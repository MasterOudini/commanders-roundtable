// `Iron-Barb Hellion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IRON_BARB_HELLION_SCRIPT } from './ironBarbHellion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Iron-Barb Hellion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Iron-Barb Hellion", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IRON_BARB_HELLION_SCRIPT]),
  });
  holdEverywhere(g);
