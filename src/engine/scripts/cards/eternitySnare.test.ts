// `Eternity Snare` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ETERNITY_SNARE_SCRIPT } from './eternitySnare';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Eternity Snare";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Eternity Snare", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ETERNITY_SNARE_SCRIPT]),
  });
  holdEverywhere(g);
