// `Thopter Mechanic` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THOPTER_MECHANIC_SCRIPT } from './thopterMechanic';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thopter Mechanic";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thopter Mechanic", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THOPTER_MECHANIC_SCRIPT]),
  });
  holdEverywhere(g);
