// `Mist-Syndicate Naga` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MIST_SYNDICATE_NAGA_SCRIPT } from './mistSyndicateNaga';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mist-Syndicate Naga";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mist-Syndicate Naga"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIST_SYNDICATE_NAGA_SCRIPT]),
