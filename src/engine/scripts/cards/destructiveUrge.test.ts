// `Destructive Urge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DESTRUCTIVE_URGE_SCRIPT } from './destructiveUrge';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Destructive Urge";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Destructive Urge", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DESTRUCTIVE_URGE_SCRIPT]),
...
describe("Destructive Urge", () => {
  test("Whenever enchanted creature deals combat damage to a player: the vocabulary resolves \"Target player sacrifices a land of target player's choice.\"", () => {
    const { g } = armed(0);
