// `Celebrity Fencer` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Celebrity Fencer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CELEBRITY_FENCER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Celebrity Fencer", () => {
  test("Alliance — Whenever another creature you control enters: it gets 1 +1/+1 counter", () => {
