// `Juggernaut` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JUGGERNAUT_SCRIPT } from './juggernaut';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Juggernaut";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Juggernaut", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Steel Wall"]],
    scripts: createRegistry([JUGGERNAUT_SCRIPT]),
