// `Gixian Infiltrator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GIXIAN_INFILTRATOR_SCRIPT } from './gixianInfiltrator';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gixian Infiltrator";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([GIXIAN_INFILTRATOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gixian Infiltrator", "Lotus Petal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GIXIAN_INFILTRATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gixian Infiltrator", () => {
  test("Whenever you sacrifice another permanent: it gets 1 +1/+1 counter", () => {
