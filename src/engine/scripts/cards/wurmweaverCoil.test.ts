// `Wurmweaver Coil` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WURMWEAVER_COIL_SCRIPT } from './wurmweaverCoil';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wurmweaver Coil";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WURMWEAVER_COIL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wurmweaver Coil", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WURMWEAVER_COIL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wurmweaver Coil", () => {
  test("Enchanted creature gets +6/+6.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([8, 8]);
  });

  test("{G}{G}{G}, Sacrifice this Aura: 1 token made", () => {
