// `Morphling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MORPHLING_SCRIPT } from './morphling';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Morphling";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MORPHLING_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Morphling"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MORPHLING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Morphling", () => {
  ...
  test("{U}: it gains flying until end of turn", () => {
