// `Giant Harbinger` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Giant Harbinger", "Burdened Stoneback"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GIANT_HARBINGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Giant Harbinger", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a Giant card, reveal it, then shuffle and put that card on top.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Burdened Stoneback" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Burdened Stoneback");
      expect(found, "Burdened Stoneback is not in the library for the search to find").toBeDefined();
