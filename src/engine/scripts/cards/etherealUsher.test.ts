// `Ethereal Usher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ETHEREAL_USHER_SCRIPT } from './etherealUsher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ethereal Usher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ethereal Usher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ETHEREAL_USHER_SCRIPT]),
...
describe("Ethereal Usher", () => {
  test("{U}, {T}: the vocabulary resolves \"Target creature can't be blocked this turn.\"", () => {
