// `Hedge Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HEDGE_TROLL_SCRIPT } from './hedgeTroll';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hedge Troll";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HEDGE_TROLL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hedge Troll", "Lightning Bolt", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HEDGE_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hedge Troll", () => {
  test("`Hedge Troll` - as long as you control a Plains: absent while it is unmet, read once it holds", () => {
  ...
  test("{W}: it regenerates from the Bolt", () => {
