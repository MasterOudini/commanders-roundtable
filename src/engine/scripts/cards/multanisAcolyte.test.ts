// `Multani's Acolyte` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Multani's Acolyte", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MULTANIS_ACOLYTE_SCRIPT]),
  });
  holdEverywhere(g);
