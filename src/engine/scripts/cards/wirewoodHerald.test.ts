// `Wirewood Herald` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wirewood Herald", "Devoted Hero"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WIREWOOD_HERALD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wirewood Herald", () => {
  test("When this creature dies: the vocabulary resolves \"Search your library for an Elf card, reveal that card, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
