// `Mechan Assembler` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mechan Assembler", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MECHAN_ASSEMBLER_SCRIPT]),
...
describe("Mechan Assembler", () => {
  test("Whenever another artifact you control enters: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test('the once-per-turn rider rides the def (D492)', () => {
    expect(MECHAN_ASSEMBLER_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("anotherCreatureEnters-0") && t.oncePerTurn === true), 'the def of line 0 carries oncePerTurn').toBeDefined();
  });
