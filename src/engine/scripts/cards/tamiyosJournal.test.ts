// `Tamiyo's Journal` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TAMIYOS_JOURNAL_SCRIPT } from './tamiyosJournal';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tamiyo's Journal", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TAMIYOS_JOURNAL_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: TOKEN_TABLE["Clue|/||Artifact|"]?.printingId ?? '', count: 3 }));
...
describe("Tamiyo's Journal", () => {
  test("At the beginning of your upkeep: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("{T}, Sacrifice three Clues: the vocabulary resolves \"Search your library for a card, put that card into your hand, then shuffle.\"", () => {
