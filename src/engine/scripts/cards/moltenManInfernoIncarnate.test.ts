// `Molten Man, Inferno Incarnate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOLTEN_MAN_INFERNO_INCARNATE_SCRIPT } from './moltenManInfernoIncarnate';
import { advanceUntil, deps, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Molten Man, Inferno Incarnate";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; cda0: number; cdaT0: number; queueFodderL2_leavesBattlefield: InstanceId };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MOLTEN_MAN_INFERNO_INCARNATE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Molten Man, Inferno Incarnate", "Mountain", "Mountain", "Mountain", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOLTEN_MAN_INFERNO_INCARNATE_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const queueFodderL2_leavesBattlefield = put(g, 'p1', "Forest");
  put(g, 'p1', "Mountain");
  settle(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, cda0, cdaT0, queueFodderL2_leavesBattlefield };
}

describe("Molten Man, Inferno Incarnate", () => {
  test("When Molten Man enters: the vocabulary resolves \"Search your library for a basic Mountain card, put it onto the battlefield tapped, then shuffle.\"", () => {
  ...
  test("Molten Man gets +1/+1 for each Mountain you control.: it gets +1/+1 for each Mountain you control", () => {
    const { g, self, cda0, cdaT0 } = armed(1);
    expect(pt(g, self)).toEqual([cda0 + 1, cdaT0 + 1]);
  });

  test("When Molten Man leaves the battlefield: the vocabulary resolves \"Sacrifice a land.\"", () => {
