// `Daru Encampment` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DARU_ENCAMPMENT_SCRIPT } from './daruEncampment';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Daru Encampment";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DARU_ENCAMPMENT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Daru Encampment"], ["Cyclops of One-Eyed Pass", "Devoted Hero"]],
    scripts: createRegistry([DARU_ENCAMPMENT_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtA1_0 = put(g, 'p2', "Devoted Hero");
  settle(g);
...
describe("Daru Encampment", () => {
  test("{W}, {T}: the vocabulary resolves \"Target Soldier creature gets +1/+1 until end of turn.\"", () => {
    const { g, self, vtA1_0 } = armed(0);
    expect(pt(g, vtA1_0)).toEqual([2, 3]);
