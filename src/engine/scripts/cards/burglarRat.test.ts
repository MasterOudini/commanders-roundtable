// `Burglar Rat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BURGLAR_RAT_SCRIPT } from './burglarRat';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Burglar Rat";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Burglar Rat"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BURGLAR_RAT_SCRIPT]),
  });
  holdEverywhere(g);
