// `Freyalise's Charm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FREYALISES_CHARM_SCRIPT } from './freyalisesCharm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Freyalise's Charm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Freyalise's Charm", "Forest", "Forest"], ["Cyclops of One-Eyed Pass", "Muck Rats", "Grizzly Bears"]],
    scripts: createRegistry([FREYALISES_CHARM_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const payL0_p1_0 = put(g, 'p1', "Forest");
  const payL0_p1_1 = put(g, 'p1', "Forest");
  const bearsP2 = put(g, 'p2', "Muck Rats", 'hand');
  settle(g);
...
describe("Freyalise's Charm", () => {
  test("Whenever an opponent casts a black spell: the vocabulary resolves \"You may pay {G}{G}. If you do, you draw a card.\" - the price is declined", () => {
  ...
  test("Whenever an opponent casts a black spell: the vocabulary resolves \"You may pay {G}{G}. If you do, you draw a card.\" - the price is paid", () => {
    const { g, payL0_p1_0, payL0_p1_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
  ...
  test("{G}{G}: it returns to its owner's hand", () => {
