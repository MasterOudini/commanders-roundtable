// `Daring Saboteur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DARING_SABOTEUR_SCRIPT } from './daringSaboteur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Daring Saboteur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Daring Saboteur"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DARING_SABOTEUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Daring Saboteur", () => {
  test("{2}{U}: the vocabulary resolves \"~ can't be blocked this turn.\"", () => {
    const {  } = armed(0);
  });

  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(1);
