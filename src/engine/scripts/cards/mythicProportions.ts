// `Mythic Proportions` - an Aura (Enchant creature): the enchanted creature gets +8/+8 and has trample.
...
import { MYTHIC_PROPORTIONS } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(MYTHIC_PROPORTIONS, "Enchant creature\nEnchanted creature gets +8/+8 and has trample.");
const LINES = PRINTED.split('\n');

export const MYTHIC_PROPORTIONS_SCRIPT: CardScript = {
  oracleId: MYTHIC_PROPORTIONS.oracleId,
  name: MYTHIC_PROPORTIONS.name,
  statics: [
    {
      ...
      modify: (chars) => {
        if (chars.power !== null) chars.power += 8;
