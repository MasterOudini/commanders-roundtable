// `Skystinger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKYSTINGER_SCRIPT } from './skystinger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skystinger";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SKYSTINGER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skystinger"], ["Cyclops of One-Eyed Pass", "Aven Skirmisher"]],
    scripts: createRegistry([SKYSTINGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skystinger", () => {
  test("Whenever this creature blocks a creature with flying: it gets +5/+0 until end of turn", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([8, 3]);
  });
