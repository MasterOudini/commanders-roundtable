// `Crashing Drawbridge` - the printed cost buys the pump until end of turn; it ends at cleanup;
// replay equal (D301). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { CRASHING_DRAWBRIDGE_SCRIPT } from './crashingDrawbridge';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crashing Drawbridge";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CRASHING_DRAWBRIDGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crashing Drawbridge", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRASHING_DRAWBRIDGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Crashing Drawbridge", () => {
  test("{T}: its controller's creatures gain haste until end of turn", () => {
