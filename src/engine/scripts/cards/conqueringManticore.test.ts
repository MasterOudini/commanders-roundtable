// `Conquering Manticore` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONQUERING_MANTICORE_SCRIPT } from './conqueringManticore';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Conquering Manticore";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CONQUERING_MANTICORE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Conquering Manticore"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONQUERING_MANTICORE_SCRIPT]),
  });
  holdEverywhere(g);
