// `Frostweb Spider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FROSTWEB_SPIDER_SCRIPT } from './frostwebSpider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frostweb Spider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frostweb Spider"], ["Cyclops of One-Eyed Pass", "Aven Skirmisher"]],
    scripts: createRegistry([FROSTWEB_SPIDER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const gift2 = put(g, 'p2', "Aven Skirmisher");
  settle(g);
...
describe("Frostweb Spider", () => {
  test("Whenever this creature blocks a creature with flying: the vocabulary resolves \"Put a +1/+1 counter on ~ at end of combat.\"", () => {
