// `Spitting Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPITTING_DRAKE_SCRIPT } from './spittingDrake';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spitting Drake";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPITTING_DRAKE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spitting Drake", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPITTING_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
