// `Restless Dead` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RESTLESS_DEAD_SCRIPT } from './restlessDead';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Restless Dead";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Restless Dead", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RESTLESS_DEAD_SCRIPT]),
  });
  holdEverywhere(g);
