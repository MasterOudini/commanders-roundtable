// `Deathcurse Ogre` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEATHCURSE_OGRE_SCRIPT } from './deathcurseOgre';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deathcurse Ogre";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deathcurse Ogre"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEATHCURSE_OGRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deathcurse Ogre", () => {
  test("When this creature dies: the vocabulary resolves \"Each player loses 3 life.\"", () => {
