// `Demon's Horn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEMONS_HORN_SCRIPT } from './demonsHorn';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Demon's Horn";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Demon's Horn", "Grizzly Bears", "Muck Rats"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEMONS_HORN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Demon's Horn", () => {
  test("Whenever a player casts a black spell: 1 life is gained", () => {
