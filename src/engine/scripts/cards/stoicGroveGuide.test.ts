// `Stoic Grove-Guide` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STOIC_GROVE_GUIDE_SCRIPT } from './stoicGroveGuide';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stoic Grove-Guide";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stoic Grove-Guide"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STOIC_GROVE_GUIDE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stoic Grove-Guide", () => {
  test("{1}{B/G}, Exile this card from your graveyard: 1 token made", () => {
