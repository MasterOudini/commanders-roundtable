// `Ascending Aven` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASCENDING_AVEN_SCRIPT } from './ascendingAven';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ascending Aven";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ascending Aven", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASCENDING_AVEN_SCRIPT]),
  });
  holdEverywhere(g);
