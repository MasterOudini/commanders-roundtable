// `Vampiric Rites` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VAMPIRIC_RITES_SCRIPT } from './vampiricRites';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vampiric Rites";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vampiric Rites", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VAMPIRIC_RITES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vampiric Rites", () => {
  test("{1}{B}, Sacrifice a creature: the vocabulary resolves \"You gain 1 life and draw a card.\"", () => {
