// `Spider-Man, Web-Spinner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIDER_MAN_WEB_SPINNER_SCRIPT } from './spiderManWebSpinner';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spider-Man, Web-Spinner";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spider-Man, Web-Spinner"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPIDER_MAN_WEB_SPINNER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spider-Man, Web-Spinner", () => {
  test("Whenever Spider-Man attacks: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
    const { g, no } = armed(0);
