// `Loxodon Warhammer` - equipped to Grizzly Bears the creature gets +3/+0 and has trample, lifelink; the Cyclops is untouched;
...
import { LOXODON_WARHAMMER_SCRIPT } from './loxodonWarhammer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Loxodon Warhammer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LOXODON_WARHAMMER_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([LOXODON_WARHAMMER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Loxodon Warhammer", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +3/+0 and has trample, lifelink; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([5, 2]);
    expect(kw(g, host).has("trample")).toBe(true);
    expect(kw(g, host).has("lifelink")).toBe(true);
    expect(pt(g, other)).toEqual([5, 2]);
    expect(kw(g, other).has("trample")).toBe(false);
    expect(kw(g, other).has("lifelink")).toBe(false);
  });
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("lifelink")).toBe(false);
  });
