// `Sheoldred, the Apocalypse` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHEOLDRED_THE_APOCALYPSE_SCRIPT } from './sheoldredTheApocalypse';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sheoldred, the Apocalypse";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sheoldred, the Apocalypse"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHEOLDRED_THE_APOCALYPSE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sheoldred, the Apocalypse", () => {
  test("Whenever you draw a card: 2 life is gained", () => {
    const { g, life0 } = armed(0);
    expect(g.state.players.p1?.life).toBe(life0 + 2);
  });

  test("Whenever an opponent draws a card: the vocabulary resolves \"Target player loses 2 life.\"", () => {
