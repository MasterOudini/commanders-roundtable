// `Kitesail Corsair` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KITESAIL_CORSAIR_SCRIPT } from './kitesailCorsair';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kitesail Corsair";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KITESAIL_CORSAIR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kitesail Corsair"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KITESAIL_CORSAIR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kitesail Corsair", () => {
  test("`Kitesail Corsair` - as long as it's attacking: absent while it is unmet, read once it holds", () => {
