// `Trestle Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRESTLE_TROLL_SCRIPT } from './trestleTroll';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trestle Troll";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trestle Troll", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRESTLE_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Trestle Troll", () => {
  test("{1}{B}{G}: it regenerates from the Bolt", () => {
