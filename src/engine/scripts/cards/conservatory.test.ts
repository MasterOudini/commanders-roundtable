// `Conservatory` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONSERVATORY_SCRIPT } from './conservatory';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Conservatory";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Conservatory"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONSERVATORY_SCRIPT]),
  });
  holdEverywhere(g);
