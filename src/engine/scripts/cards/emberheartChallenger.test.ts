// `Emberheart Challenger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EMBERHEART_CHALLENGER_SCRIPT } from './emberheartChallenger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Emberheart Challenger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Emberheart Challenger", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EMBERHEART_CHALLENGER_SCRIPT]),
...
describe("Emberheart Challenger", () => {
  test("Valiant — Whenever this creature becomes the target of a spell or ability you control for the first time each turn: the vocabulary resolves \"Exile the top card of your library. Until end of turn, you may play that card.\"", () => {
    const { g } = armed(0);
  ...
  test('the once-per-turn rider rides the def (D492)', () => {
    expect(EMBERHEART_CHALLENGER_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("becomesTargetedByYou-2") && t.oncePerTurn === true), 'the def of line 2 carries oncePerTurn').toBeDefined();
  });
