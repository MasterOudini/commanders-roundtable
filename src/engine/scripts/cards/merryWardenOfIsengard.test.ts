// `Merry, Warden of Isengard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERRY_WARDEN_OF_ISENGARD_SCRIPT } from './merryWardenOfIsengard';
import { advanceUntil, holdEverywhere, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Merry, Warden of Isengard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merry, Warden of Isengard", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERRY_WARDEN_OF_ISENGARD_SCRIPT]),
...
describe("Merry, Warden of Isengard", () => {
  test("Whenever one or more artifacts you control enter: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test('the once-per-turn rider rides the def (D492)', () => {
    expect(MERRY_WARDEN_OF_ISENGARD_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("artifactEnters-1") && t.oncePerTurn === true), 'the def of line 1 carries oncePerTurn').toBeDefined();
  });
