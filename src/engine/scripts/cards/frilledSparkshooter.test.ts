// `Frilled Sparkshooter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FRILLED_SPARKSHOOTER_SCRIPT } from './frilledSparkshooter';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frilled Sparkshooter";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FRILLED_SPARKSHOOTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frilled Sparkshooter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FRILLED_SPARKSHOOTER_SCRIPT]),
  });
  holdEverywhere(g);
