// `Orcish Oriflamme` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORCISH_ORIFLAMME_SCRIPT } from './orcishOriflamme';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Orcish Oriflamme";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ORCISH_ORIFLAMME_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Orcish Oriflamme", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORCISH_ORIFLAMME_SCRIPT]),
  });
  holdEverywhere(g);
