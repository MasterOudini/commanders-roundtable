// `Omnath, Locus of Rage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OMNATH_LOCUS_OF_RAGE_SCRIPT } from './omnathLocusOfRage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Omnath, Locus of Rage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Omnath, Locus of Rage", "Grizzly Bears", "Terrain Elemental"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OMNATH_LOCUS_OF_RAGE_SCRIPT]),
  ...
  if (which === 2) {
    const bears = put(g, 'p1', "Terrain Elemental");
    hand0 = (g.state.zones.hand.p1 ?? []).length;
    settle(g);
...
describe("Omnath, Locus of Rage", () => {
  test("Landfall — Whenever a land you control enters: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("Whenever Omnath or another Elemental you control dies: it deals 3 damage to the declared creature [dies]", () => {
    const { g, no } = armed(1);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
  });

  test("Whenever Omnath or another Elemental you control dies: it deals 3 damage to the declared creature [anotherCreatureDies]", () => {
    const { g, no } = armed(2);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
  });
