// `Webweaver Changeling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WEBWEAVER_CHANGELING_SCRIPT } from './webweaverChangeling';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Webweaver Changeling";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Webweaver Changeling", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WEBWEAVER_CHANGELING_SCRIPT]),
  });
  holdEverywhere(g);
