// `Sprinting Warbrute` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPRINTING_WARBRUTE_SCRIPT } from './sprintingWarbrute';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sprinting Warbrute";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sprinting Warbrute", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPRINTING_WARBRUTE_SCRIPT]),
