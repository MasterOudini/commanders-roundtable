// `Ghirapur Aether Grid` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHIRAPUR_AETHER_GRID_SCRIPT } from './ghirapurAetherGrid';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghirapur Aether Grid";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghirapur Aether Grid", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHIRAPUR_AETHER_GRID_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ghirapur Aether Grid", () => {
  test("Tap two untapped artifacts you control: it deals 1 damage to the declared creature", () => {
