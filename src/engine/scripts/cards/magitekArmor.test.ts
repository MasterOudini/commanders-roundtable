// `Magitek Armor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAGITEK_ARMOR_SCRIPT } from './magitekArmor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Magitek Armor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Magitek Armor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAGITEK_ARMOR_SCRIPT]),
  });
  holdEverywhere(g);
