// `Lightning-Rig Crew` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIGHTNING_RIG_CREW_SCRIPT } from './lightningRigCrew';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning-Rig Crew";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lightning-Rig Crew", "Grizzly Bears", "Swab Goblin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIGHTNING_RIG_CREW_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  const bearsSpell = put(g, 'p1', "Swab Goblin", 'hand');
...
describe("Lightning-Rig Crew", () => {
  ...
  test("Whenever you cast a Pirate spell: it untaps", () => {
