// `Super Villain Lockup` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUPER_VILLAIN_LOCKUP_SCRIPT } from './superVillainLockup';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Super Villain Lockup";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Super Villain Lockup"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUPER_VILLAIN_LOCKUP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Super Villain Lockup", () => {
  test("When this enchantment enters: the vocabulary resolves \"Exile target tapped creature an opponent controls until this enchantment leaves the battlefield.\"", () => {
