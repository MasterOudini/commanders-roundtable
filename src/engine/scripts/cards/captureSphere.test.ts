// `Capture Sphere` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAPTURE_SPHERE_SCRIPT } from './captureSphere';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Capture Sphere";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Capture Sphere", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAPTURE_SPHERE_SCRIPT]),
  });
  holdEverywhere(g);
