// `Rohirrim Lancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROHIRRIM_LANCER_SCRIPT } from './rohirrimLancer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rohirrim Lancer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rohirrim Lancer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROHIRRIM_LANCER_SCRIPT]),
