// `Odious Trow` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ODIOUS_TROW_SCRIPT } from './odiousTrow';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Odious Trow";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Odious Trow", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ODIOUS_TROW_SCRIPT]),
  });
  holdEverywhere(g);
