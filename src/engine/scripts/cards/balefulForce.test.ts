// `Baleful Force` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Baleful Force"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BALEFUL_FORCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Baleful Force", () => {
  test("At the beginning of each upkeep: a card is drawn and 1 life lost", () => {
