// `Stonehands` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STONEHANDS_SCRIPT } from './stonehands';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stonehands";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STONEHANDS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stonehands", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STONEHANDS_SCRIPT]),
  });
  holdEverywhere(g);
