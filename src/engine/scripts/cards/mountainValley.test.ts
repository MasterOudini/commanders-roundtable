// `Mountain Valley` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mountain Valley", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOUNTAIN_VALLEY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mountain Valley", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a Mountain or Forest card, put it onto the battlefield, then shuffle.\"", () => {
