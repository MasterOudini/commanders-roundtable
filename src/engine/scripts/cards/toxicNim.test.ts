// `Toxic Nim` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TOXIC_NIM_SCRIPT } from './toxicNim';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Toxic Nim";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Toxic Nim", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TOXIC_NIM_SCRIPT]),
  });
  holdEverywhere(g);
