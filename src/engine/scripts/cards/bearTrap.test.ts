// `Bear Trap` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BEAR_TRAP_SCRIPT } from './bearTrap';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bear Trap";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bear Trap"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BEAR_TRAP_SCRIPT]),
  });
  holdEverywhere(g);
