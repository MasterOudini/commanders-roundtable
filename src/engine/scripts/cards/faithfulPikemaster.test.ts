// `Faithful Pikemaster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FAITHFUL_PIKEMASTER_SCRIPT } from './faithfulPikemaster';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Faithful Pikemaster";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FAITHFUL_PIKEMASTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Faithful Pikemaster"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FAITHFUL_PIKEMASTER_SCRIPT]),
...
describe("Faithful Pikemaster", () => {
  test("When this creature enters: scry 2 asks, and the cards stay on top", () => {
    const { g, lib0 } = armed(0);
    expect(g.state.priority.awaiting).toBeNull();
    expect((g.state.zones.library.p1 ?? []).length).toBe(lib0);
  });

  test("`Faithful Pikemaster` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
