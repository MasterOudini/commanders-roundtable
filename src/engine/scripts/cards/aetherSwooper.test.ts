// `Aether Swooper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AETHER_SWOOPER_SCRIPT } from './aetherSwooper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aether Swooper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aether Swooper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AETHER_SWOOPER_SCRIPT]),
