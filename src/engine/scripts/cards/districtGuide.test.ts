// `District Guide` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["District Guide", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DISTRICT_GUIDE_SCRIPT]),
...
describe("District Guide", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a basic land card or Gate card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
