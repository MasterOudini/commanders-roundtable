// `Contagion Clasp` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONTAGION_CLASP_SCRIPT } from './contagionClasp';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Contagion Clasp";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; prolifFodderA0: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Contagion Clasp", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONTAGION_CLASP_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, prolifFodderA0 };
}

describe("Contagion Clasp", () => {
  test("When this artifact enters: the vocabulary resolves \"Put a -1/-1 counter on target creature.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.counters["-1/-1"] ?? 0).toBe(1);
  });

  test("{4}, {T}: the vocabulary resolves \"Proliferate.\"", () => {
    const { g, self, prolifFodderA0 } = armed(1);
