// `Biogenic Ooze` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BIOGENIC_OOZE_SCRIPT } from './biogenicOoze';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Biogenic Ooze";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Biogenic Ooze", "Terrian, World Tyrant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BIOGENIC_OOZE_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Terrian, World Tyrant");
