// `Slith Strider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLITH_STRIDER_SCRIPT } from './slithStrider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slith Strider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slith Strider"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLITH_STRIDER_SCRIPT]),
  });
  holdEverywhere(g);
