// `Chime of Night` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHIME_OF_NIGHT_SCRIPT } from './chimeOfNight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chime of Night";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chime of Night", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHIME_OF_NIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chime of Night", () => {
  test("When this Aura is put into a graveyard from the battlefield: the vocabulary resolves \"Destroy target nonblack creature.\"", () => {
    const { g, no } = armed(0);
