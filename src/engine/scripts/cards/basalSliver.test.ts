// `Basal Sliver` - the granted MANA ability proven end to end (D372): the recipient HAS the
...
import { BASAL_SLIVER_SCRIPT } from './basalSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Basal Sliver";
const QUOTED = "Sacrifice this permanent: Add {B}{B}.";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Basal Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BASAL_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Basal Sliver", () => {
  test("grants Sacrifice this permanent: Add {B}{B}. to Metallic Sliver, and to nothing outside the scope", () => {
    const { g, self, host, no } = board();
    const prod = granted(g, host);
    expect(prod).toBeDefined();
    expect(prod?.requiresTap).toBe(false);
  ...
  test('the recipient is offered it, and tapping it charges the RECIPIENT and adds the mana', () => {
    ...
    expect(poolTotal(g)).toBe(before + 2);
  });
