// `Zimone, Quandrix Prodigy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZIMONE_QUANDRIX_PRODIGY_SCRIPT } from './zimoneQuandrixProdigy';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zimone, Quandrix Prodigy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zimone, Quandrix Prodigy", "Forest", "Forest", "Forest", "Forest", "Forest", "Forest", "Forest", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZIMONE_QUANDRIX_PRODIGY_SCRIPT]),
  ...
  if (which === 1) {
    // the payload's gate: you control 8 or more Land
    put(g, 'p1', "Forest");
    put(g, 'p1', "Forest");
    put(g, 'p1', "Forest");
    put(g, 'p1', "Forest");
...
describe("Zimone, Quandrix Prodigy", () => {
  test("{1}, {T}: the vocabulary resolves \"You may put a land card from your hand onto the battlefield tapped.\"", () => {
  ...
  test("{4}, {T}: the vocabulary resolves \"Draw a card. If you control eight or more lands, draw two cards instead.\"", () => {
