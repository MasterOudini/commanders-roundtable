// `Stampeding Scurryfoot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STAMPEDING_SCURRYFOOT_SCRIPT } from './stampedingScurryfoot';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stampeding Scurryfoot";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stampeding Scurryfoot", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STAMPEDING_SCURRYFOOT_SCRIPT]),
...
describe("Stampeding Scurryfoot", () => {
  test("Exhaust — {3}{G}: the vocabulary resolves \"Put a +1/+1 counter on this creature. Create a 3/3 green Elephant creature token.\"", () => {
    const { g, board0, refused } = armed(0);
