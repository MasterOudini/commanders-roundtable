// `Telekinetic Sliver` - every permanent in its scope HAS a quoted
...
import { TELEKINETIC_SLIVER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(TELEKINETIC_SLIVER, "All Slivers have \"{T}: Tap target permanent.\"");

const GRANT = grantedActivated("{T}: Tap target permanent.", `${TELEKINETIC_SLIVER.oracleId}#g0`, TELEKINETIC_SLIVER.name);
const VOCAB = vocabularyEffects("Tap target permanent.", TELEKINETIC_SLIVER.name);
const VOCAB_T = vocabularyTargets("Tap target permanent.");

export const TELEKINETIC_SLIVER_SCRIPT: CardScript = {
  oracleId: TELEKINETIC_SLIVER.oracleId,
  name: TELEKINETIC_SLIVER.name,
  statics: [
    {
      abilityId: 'grant-0',
      text: PRINTED,
