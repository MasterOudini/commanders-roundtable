// `Chainflinger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHAINFLINGER_SCRIPT } from './chainflinger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chainflinger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chainflinger", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHAINFLINGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chainflinger", () => {
  test("{1}{R}, {T}: it deals 1 damage to the declared creature", () => {
    const { g, self, no } = armed(0);
    expect(g.state.cards[no]?.damage).toBe(1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("Threshold — {2}{R}, {T}: it deals 2 damage to the declared creature - refused unless if there are seven or more cards in your graveyard", () => {
