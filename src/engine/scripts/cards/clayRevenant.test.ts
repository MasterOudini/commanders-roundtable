// `Clay Revenant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLAY_REVENANT_SCRIPT } from './clayRevenant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Clay Revenant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Clay Revenant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLAY_REVENANT_SCRIPT]),
  });
  holdEverywhere(g);
