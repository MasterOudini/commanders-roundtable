// `Shivan Harvest` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SHIVAN_HARVEST_SCRIPT } from './shivanHarvest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shivan Harvest";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number; fodder: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Shivan Harvest", "Grizzly Bears"], ["Quicksand", "Forest"]],
    scripts: createRegistry([SHIVAN_HARVEST_SCRIPT]),
  });
  holdEverywhere(g);
