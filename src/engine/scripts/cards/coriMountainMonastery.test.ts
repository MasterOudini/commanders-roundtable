// `Cori Mountain Monastery` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CORI_MOUNTAIN_MONASTERY_SCRIPT } from './coriMountainMonastery';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cori Mountain Monastery";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cori Mountain Monastery"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CORI_MOUNTAIN_MONASTERY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cori Mountain Monastery", () => {
  test("{3}{R}, {T}: the vocabulary resolves \"Exile the top card of your library. Until the end of your next turn, you may play that card.\"", () => {
