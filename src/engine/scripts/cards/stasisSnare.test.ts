// `Stasis Snare` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STASIS_SNARE_SCRIPT } from './stasisSnare';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stasis Snare";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stasis Snare"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STASIS_SNARE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stasis Snare", () => {
  test("When this enchantment enters: the vocabulary resolves \"Exile target creature an opponent controls until this enchantment leaves the battlefield.\"", () => {
