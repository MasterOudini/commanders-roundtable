// `Seething Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seething Landscape", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEETHING_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Seething Landscape", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Island, Swamp, or Mountain card, put it onto the battlefield tapped, then shuffle.\"", () => {
    const { g, self, board0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Island" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Island");
      expect(found, "Island is not in the library for the search to find").toBeDefined();
