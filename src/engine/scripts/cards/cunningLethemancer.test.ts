// `Cunning Lethemancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CUNNING_LETHEMANCER_SCRIPT } from './cunningLethemancer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cunning Lethemancer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cunning Lethemancer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CUNNING_LETHEMANCER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cunning Lethemancer", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Each player discards a card.\"", () => {
