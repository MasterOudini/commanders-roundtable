// `Stratus Walk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STRATUS_WALK_SCRIPT } from './stratusWalk';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stratus Walk";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([STRATUS_WALK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stratus Walk", "Grizzly Bears", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STRATUS_WALK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stratus Walk", () => {
  test("When this Aura enters: a card is drawn", () => {
    const { g, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1 - 1);
  });

  test("Enchanted creature has flying. (It can't be blocked except by creatures with flying or reach.): the creature it is attached to reads it", () => {
  ...
  test("Enchanted creature can block only creatures with flying.: the creature it is attached to is held back", () => {
    const { g, self, refused, bearsB } = armed(2);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
