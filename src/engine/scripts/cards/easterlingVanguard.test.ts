// `Easterling Vanguard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EASTERLING_VANGUARD_SCRIPT } from './easterlingVanguard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Easterling Vanguard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Easterling Vanguard"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EASTERLING_VANGUARD_SCRIPT]),
...
describe("Easterling Vanguard", () => {
  test("When this creature dies: the vocabulary resolves \"Amass Orcs 1.\"", () => {
    ...
    { const am = g.log.filter((e) => e.body.t === 'Amassed' && e.body.player === 'p1');
      ...
      expect(g.state.cards[army]?.counters['+1/+1'] ?? 0, 'amassed: the Army carries the counters').toBe(1);
    }
