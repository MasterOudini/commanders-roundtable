// `Behemoth Sledge` - equipped to Grizzly Bears the creature gets +2/+2 and has trample, lifelink; the Cyclops is untouched;
...
import { BEHEMOTH_SLEDGE_SCRIPT } from './behemothSledge';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Behemoth Sledge";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BEHEMOTH_SLEDGE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([BEHEMOTH_SLEDGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Behemoth Sledge", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +2/+2 and has trample, lifelink; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([4, 4]);
