// `Snapping Creeper` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SNAPPING_CREEPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Snapping Creeper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SNAPPING_CREEPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Snapping Creeper", () => {
  test("Landfall — Whenever a land you control enters: it gains vigilance until end of turn", () => {
