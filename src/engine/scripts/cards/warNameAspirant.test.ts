// `War-Name Aspirant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WAR_NAME_ASPIRANT_SCRIPT } from './warNameAspirant';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "War-Name Aspirant";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WAR_NAME_ASPIRANT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["War-Name Aspirant", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([WAR_NAME_ASPIRANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("War-Name Aspirant", () => {
  test("Raid — This creature enters with a +1/+1 counter on it if you attacked this turn.: cast, it enters with 1 +1/+1 counter", () => {
  ...
  test("This creature can't be blocked by creatures with power 1 or less.: the block is refused", () => {
