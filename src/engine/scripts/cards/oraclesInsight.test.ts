// `Oracle's Insight` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { ORACLES_INSIGHT_SCRIPT } from './oraclesInsight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Oracle's Insight";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Oracle's Insight","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORACLES_INSIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Oracle's Insight", () => {
  test("grants {T}: Scry 1, then draw a card. to Grizzly Bears, and to nothing outside the scope", () => {
