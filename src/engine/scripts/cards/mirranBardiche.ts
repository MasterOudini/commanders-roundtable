// `Mirran Bardiche` - a etb trigger germ, a static attachedStatic
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { MIRRAN_BARDICHE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(MIRRAN_BARDICHE, "For Mirrodin! (When this Equipment enters, create a 2/2 red Rebel creature token, then attach this to it.)\nEquipped creature gets +2/+1 and has vigilance.\nEquip {3}{W} ({3}{W}: Attach to target creature you control. Equip only as a sorcery.)");
const LINES = PRINTED.split('\n');
const TOKEN_L0 = tokenRef("Rebel|2/2|R|Creature|");

export const MIRRAN_BARDICHE_SCRIPT: CardScript = {
  oracleId: MIRRAN_BARDICHE.oracleId,
  name: MIRRAN_BARDICHE.name,
  triggers: [
    {
      abilityId: 'etb-0',
