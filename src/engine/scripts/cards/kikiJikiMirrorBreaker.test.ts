// `Kiki-Jiki, Mirror Breaker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KIKI_JIKI_MIRROR_BREAKER_SCRIPT } from './kikiJikiMirrorBreaker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kiki-Jiki, Mirror Breaker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kiki-Jiki, Mirror Breaker", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KIKI_JIKI_MIRROR_BREAKER_SCRIPT]),
...
describe("Kiki-Jiki, Mirror Breaker", () => {
  test("{T}: the vocabulary resolves \"Create a token that's a copy of target nonlegendary creature you control, except it has haste. Sacrifice it at the beginning of the next end step.\"", () => {
