// `Thermal Navigator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THERMAL_NAVIGATOR_SCRIPT } from './thermalNavigator';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thermal Navigator";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([THERMAL_NAVIGATOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thermal Navigator", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THERMAL_NAVIGATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thermal Navigator", () => {
  test("Sacrifice an artifact: it gains flying until end of turn", () => {
