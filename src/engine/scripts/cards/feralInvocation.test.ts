// `Feral Invocation` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2;
...
import { FERAL_INVOCATION_SCRIPT } from './feralInvocation';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Feral Invocation";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FERAL_INVOCATION_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FERAL_INVOCATION_SCRIPT]),
  });
  holdEverywhere(g);
