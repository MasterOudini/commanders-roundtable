// `Pouncing Wurm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { POUNCING_WURM_SCRIPT } from './pouncingWurm';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pouncing Wurm";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([POUNCING_WURM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pouncing Wurm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([POUNCING_WURM_SCRIPT]),
  ...
  if (which === 0) {
    ...
    if (kw(g, self).has("haste")) throw new Error('the kicked keyword held on an unkicked cast');
...
describe("Pouncing Wurm", () => {
  test("If this creature was kicked, it enters with three +1/+1 counters on it and with haste.: cast, it enters with 3 +1/+1 counters", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.zone.kind).toBe('battlefield');
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(3);
    expect(kw(g, self).has("haste"), 'the kick granted haste').toBe(true);
