// `Lore Weaver` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LORE_WEAVER_SCRIPT } from './loreWeaver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lore Weaver";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lore Weaver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LORE_WEAVER_SCRIPT]),
...
describe("Lore Weaver", () => {
  test("{5}{U}{U}: the vocabulary resolves \"Target player draws two cards.\"", () => {
