// `Codecracker Hound` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CODECRACKER_HOUND_SCRIPT } from './codecrackerHound';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Codecracker Hound";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Codecracker Hound"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CODECRACKER_HOUND_SCRIPT]),
...
describe("Codecracker Hound", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top two cards of your library. Put one into your hand and the other into your graveyard.\"", () => {
    const { g } = armed(0);
