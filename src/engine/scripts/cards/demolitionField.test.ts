// `Demolition Field` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Demolition Field", "Forest"], ["Cyclops of One-Eyed Pass", "Tundra", "Forest"]],
    scripts: createRegistry([DEMOLITION_FIELD_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtA1_0 = put(g, 'p2', "Tundra");
  settle(g);
