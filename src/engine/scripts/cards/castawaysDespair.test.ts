// `Castaway's Despair` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CASTAWAYS_DESPAIR_SCRIPT } from './castawaysDespair';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Castaway's Despair";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Castaway's Despair", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CASTAWAYS_DESPAIR_SCRIPT]),
  });
  holdEverywhere(g);
