// `Sharding Sphinx` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHARDING_SPHINX_SCRIPT } from './shardingSphinx';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sharding Sphinx";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sharding Sphinx", "Memnite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHARDING_SPHINX_SCRIPT]),
...
describe("Sharding Sphinx", () => {
  test("Whenever an artifact creature you control deals combat damage to a player: 1 token made", () => {
