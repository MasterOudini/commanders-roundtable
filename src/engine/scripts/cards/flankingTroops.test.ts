// `Flanking Troops` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLANKING_TROOPS_SCRIPT } from './flankingTroops';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flanking Troops";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flanking Troops"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLANKING_TROOPS_SCRIPT]),
  });
  holdEverywhere(g);
