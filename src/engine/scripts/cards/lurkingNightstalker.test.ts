// `Lurking Nightstalker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LURKING_NIGHTSTALKER_SCRIPT } from './lurkingNightstalker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lurking Nightstalker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LURKING_NIGHTSTALKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lurking Nightstalker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LURKING_NIGHTSTALKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lurking Nightstalker", () => {
  test("Whenever this creature attacks: it gets +2/+0 until end of turn", () => {
