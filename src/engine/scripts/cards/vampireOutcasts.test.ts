// `Vampire Outcasts` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VAMPIRE_OUTCASTS_SCRIPT } from './vampireOutcasts';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vampire Outcasts";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VAMPIRE_OUTCASTS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vampire Outcasts", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VAMPIRE_OUTCASTS_SCRIPT]),
  });
  holdEverywhere(g);
