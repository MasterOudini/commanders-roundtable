// `Daru Cavalier` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Daru Cavalier", "Daru Cavalier"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DARU_CAVALIER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Daru Cavalier", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a card named ~, reveal it, put it into your hand, then shuffle.\"", () => {
    ...
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Daru Cavalier" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Daru Cavalier");
      expect(found, "Daru Cavalier is not in the library for the search to find").toBeDefined();
