// `Hulking Goblin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HULKING_GOBLIN_SCRIPT } from './hulkingGoblin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hulking Goblin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hulking Goblin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HULKING_GOBLIN_SCRIPT]),
  });
  holdEverywhere(g);
