// `Ashes to Ashes` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ASHES_TO_ASHES_SCRIPT } from './ashesToAshes';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ashes to Ashes";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Ashes to Ashes"], ["Grizzly Bears", "Vampire Nighthawk", "Grapeshot Catapult"]],
    scripts: createRegistry([ASHES_TO_ASHES_SCRIPT]),
  });
  holdEverywhere(g);
  const targets: InstanceId[] = [];
  targets.push(put(g, 'p2', "Grizzly Bears"));
  targets.push(put(g, 'p2', "Vampire Nighthawk"));
  const wrong = put(g, 'p2', "Grapeshot Catapult");
...
describe("Ashes to Ashes", () => {
  test('the named targets are accepted and the effect lands', () => {
    ...
    expect(g.state.cards[targets[1] as InstanceId]?.zone.kind).toBe('exile');
    expect(g.state.players.p1?.life).toBe(life0.p1 + (-5));
  });

  test("Grapeshot Catapult is refused where the clause excludes it (D294)", () => {
