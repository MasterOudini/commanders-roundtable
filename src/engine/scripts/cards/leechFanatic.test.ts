// `Leech Fanatic` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LEECH_FANATIC_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Leech Fanatic"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LEECH_FANATIC_SCRIPT]),
...
describe("Leech Fanatic", () => {
  test("`Leech Fanatic` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
