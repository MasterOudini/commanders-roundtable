// `Commander's Authority` - the granted TRIGGER proven end to end (D368): the recipient HAS the
...
import { COMMANDERS_AUTHORITY_SCRIPT } from './commandersAuthority';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Commander's Authority";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Commander's Authority","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COMMANDERS_AUTHORITY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Commander's Authority", () => {
