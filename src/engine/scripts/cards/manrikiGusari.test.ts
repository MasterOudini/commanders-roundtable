// `Manriki-Gusari` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { MANRIKI_GUSARI_SCRIPT } from './manrikiGusari';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Manriki-Gusari";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Manriki-Gusari","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MANRIKI_GUSARI_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Manriki-Gusari", () => {
  test("grants {T}: Destroy target Equipment. to Grizzly Bears, and to nothing outside the scope", () => {
