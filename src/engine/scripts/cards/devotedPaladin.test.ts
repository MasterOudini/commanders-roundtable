// `Devoted Paladin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEVOTED_PALADIN_SCRIPT } from './devotedPaladin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Devoted Paladin";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DEVOTED_PALADIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Devoted Paladin", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEVOTED_PALADIN_SCRIPT]),
...
describe("Devoted Paladin", () => {
  test("Beacon of Hope — When this creature enters: its controller's creatures get +1/+1 and gain vigilance until end of turn", () => {
