// `Tramway Station` — enters tapped, and the sac-draw eats the land for a card.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { TRAMWAY_STATION_SCRIPT } from './tramwayStation';
import { advanceUntil, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const SPHERE = 'Tramway Station';
...
function game(): { g: Game; sphere: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [[SPHERE], []],
    scripts: createRegistry([TRAMWAY_STATION_SCRIPT]),
  });
...
/** Straightens the land (it entered tapped) and funds the coloured cost. */
