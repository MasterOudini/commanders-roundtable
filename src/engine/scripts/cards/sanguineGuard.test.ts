// `Sanguine Guard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SANGUINE_GUARD_SCRIPT } from './sanguineGuard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sanguine Guard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sanguine Guard", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SANGUINE_GUARD_SCRIPT]),
  });
  holdEverywhere(g);
