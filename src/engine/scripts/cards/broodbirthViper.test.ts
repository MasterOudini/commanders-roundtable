// `Broodbirth Viper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BROODBIRTH_VIPER_SCRIPT } from './broodbirthViper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Broodbirth Viper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Broodbirth Viper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BROODBIRTH_VIPER_SCRIPT]),
