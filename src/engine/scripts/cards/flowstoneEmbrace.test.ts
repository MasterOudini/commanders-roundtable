// `Flowstone Embrace` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLOWSTONE_EMBRACE_SCRIPT } from './flowstoneEmbrace';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flowstone Embrace";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flowstone Embrace", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLOWSTONE_EMBRACE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Flowstone Embrace", () => {
  test("{T}: the enchanted creature is pumped until end of turn", () => {
