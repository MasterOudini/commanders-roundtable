// `Blurred Mongoose` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLURRED_MONGOOSE_SCRIPT } from './blurredMongoose';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blurred Mongoose";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blurred Mongoose"], ["Cyclops of One-Eyed Pass", "Counterspell"]],
    scripts: createRegistry([BLURRED_MONGOOSE_SCRIPT]),
  });
  holdEverywhere(g);
