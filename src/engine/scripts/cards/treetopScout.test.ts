// `Treetop Scout` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TREETOP_SCOUT_SCRIPT } from './treetopScout';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Treetop Scout";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Treetop Scout", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TREETOP_SCOUT_SCRIPT]),
