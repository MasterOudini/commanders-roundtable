// `Bristling Backwoods` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRISTLING_BACKWOODS_SCRIPT } from './bristlingBackwoods';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bristling Backwoods";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bristling Backwoods"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRISTLING_BACKWOODS_SCRIPT]),
  });
  holdEverywhere(g);
