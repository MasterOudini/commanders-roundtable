// `Akul the Unrepentant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AKUL_THE_UNREPENTANT_SCRIPT } from './akulTheUnrepentant';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Akul the Unrepentant";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; refused: boolean; fodder0: InstanceId[] };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Akul the Unrepentant", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AKUL_THE_UNREPENTANT_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = [put(g, 'p1', "Grizzly Bears"), put(g, 'p1', "Grizzly Bears"), put(g, 'p1', "Grizzly Bears")];
  settle(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, refused, fodder0 };
}

describe("Akul the Unrepentant", () => {
  test("Sacrifice three other creatures: the vocabulary resolves \"You may put a creature card from your hand onto the battlefield.\"", () => {
    const { g, self, hand0, board0, fodder0 } = armed(0);
    ...
    { const again = g.submit({ t: 'ActivateAbility', player: 'p1', card: self, abilityIndex: 0, sacrifice: fodder0.slice(0, 3) }); expect(!again.ok && again.reason === 'timingRestriction', 'once each turn').toBe(true); }
