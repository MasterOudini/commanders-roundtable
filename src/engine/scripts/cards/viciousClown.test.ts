// `Vicious Clown` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VICIOUS_CLOWN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vicious Clown", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VICIOUS_CLOWN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vicious Clown", () => {
  test("Whenever another creature you control with power 2 or less enters: it gets +2/+0 until end of turn", () => {
