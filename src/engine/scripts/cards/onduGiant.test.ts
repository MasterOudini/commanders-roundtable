// `Ondu Giant` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ondu Giant", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ONDU_GIANT_SCRIPT]),
  });
  holdEverywhere(g);
