// `Starlight Snare` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STARLIGHT_SNARE_SCRIPT } from './starlightSnare';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Starlight Snare";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Starlight Snare", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STARLIGHT_SNARE_SCRIPT]),
  });
  holdEverywhere(g);
