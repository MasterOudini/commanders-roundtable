// `Urza's Blueprints` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Urza's Blueprints", "Forest", "Forest", "Forest", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([URZAS_BLUEPRINTS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Urza's Blueprints", () => {
  test("{T}: a card is drawn", () => {
