// `Shinen of Fear's Chill` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHINEN_OF_FEARS_CHILL_SCRIPT } from './shinenOfFearsChill';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shinen of Fear's Chill";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shinen of Fear's Chill", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHINEN_OF_FEARS_CHILL_SCRIPT]),
...
describe("Shinen of Fear's Chill", () => {
  test("This creature can't block.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("Channel — {1}{B}, Discard this card: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
