// `Ramunap Ruins` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAMUNAP_RUINS_SCRIPT } from './ramunapRuins';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ramunap Ruins";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ramunap Ruins", "Bucolic Ranch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAMUNAP_RUINS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ramunap Ruins", () => {
  test("{2}{R}{R}, {T}, Sacrifice a Desert: it deals 2 damage to each opponent", () => {
