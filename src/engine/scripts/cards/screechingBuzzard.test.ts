// `Screeching Buzzard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCREECHING_BUZZARD_SCRIPT } from './screechingBuzzard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Screeching Buzzard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Screeching Buzzard"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCREECHING_BUZZARD_SCRIPT]),
  });
  holdEverywhere(g);
