// `Stronghold Confessor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STRONGHOLD_CONFESSOR_SCRIPT } from './strongholdConfessor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stronghold Confessor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STRONGHOLD_CONFESSOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stronghold Confessor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STRONGHOLD_CONFESSOR_SCRIPT]),
  });
  holdEverywhere(g);
