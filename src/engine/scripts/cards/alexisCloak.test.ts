// `Alexi's Cloak` - cast on Grizzly Bears it attaches and the enchanted creature has shroud;
...
import { ALEXIS_CLOAK_SCRIPT } from './alexisCloak';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Alexi's Cloak";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([ALEXIS_CLOAK_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ALEXIS_CLOAK_SCRIPT]),
  });
  holdEverywhere(g);
