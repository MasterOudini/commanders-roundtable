// `Drannith Stinger` - a youCycle trigger damageOpponents
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { DRANNITH_STINGER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(DRANNITH_STINGER, "Whenever you cycle another card, this creature deals 1 damage to each opponent.\nCycling {1} ({1}, Discard this card: Draw a card.)");
const LINES = PRINTED.split('\n');

export const DRANNITH_STINGER_SCRIPT: CardScript = {
  oracleId: DRANNITH_STINGER.oracleId,
  name: DRANNITH_STINGER.name,
  triggers: [
    {
      ...
      label: () => "Drannith Stinger - damageOpponents",
