// `Henchbots` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HENCHBOTS_SCRIPT } from './henchbots';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Henchbots";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Henchbots"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HENCHBOTS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Henchbots", () => {
  test("When this creature enters: the vocabulary resolves \"Exile target tapped creature an opponent controls until this creature leaves the battlefield.\"", () => {
