// `Windswept Heath` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Windswept Heath", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WINDSWEPT_HEATH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Windswept Heath", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for a Forest or Plains card, put it onto the battlefield, then shuffle.\"", () => {
