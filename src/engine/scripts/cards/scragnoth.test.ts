// `Scragnoth` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCRAGNOTH_SCRIPT } from './scragnoth';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scragnoth";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scragnoth"], ["Cyclops of One-Eyed Pass", "Counterspell"]],
    scripts: createRegistry([SCRAGNOTH_SCRIPT]),
  });
  holdEverywhere(g);
