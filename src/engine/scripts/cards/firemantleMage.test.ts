// `Firemantle Mage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIREMANTLE_MAGE_SCRIPT } from './firemantleMage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Firemantle Mage";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FIREMANTLE_MAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Firemantle Mage", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIREMANTLE_MAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Firemantle Mage", () => {
  test("Rally — Whenever this creature or another Ally you control enters: its controller's creatures gain menace until end of turn", () => {
