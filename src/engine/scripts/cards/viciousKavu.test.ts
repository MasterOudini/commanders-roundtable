// `Vicious Kavu` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VICIOUS_KAVU_SCRIPT } from './viciousKavu';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vicious Kavu";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VICIOUS_KAVU_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vicious Kavu"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VICIOUS_KAVU_SCRIPT]),
  });
  holdEverywhere(g);
