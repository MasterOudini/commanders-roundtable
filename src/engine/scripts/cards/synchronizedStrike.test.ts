// `Synchronized Strike` — my two attacking bears untap and each get +2/+2
// mid-combat; zero targets resolves.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry, SHIPPED_REGISTRY } from '../registry';
import { SYNCHRONIZED_STRIKE_SCRIPT } from './synchronizedStrike';
...
const SPELL = 'Synchronized Strike';
...
function pt(g: Game, id: InstanceId): { power: number | null; toughness: number | null } {
  const d = deps(createRegistry([SYNCHRONIZED_STRIKE_SCRIPT]));
  const got = derive(g.state, d.oracle, d.scripts, id);
  return { power: got.power, toughness: got.toughness };
}

/** Turn 3: both bears attack and are tapped; p1 holds priority in the declare-attackers step, spell cast. */
function midCombat(): { g: Game; spell: InstanceId; a: InstanceId; b: InstanceId; logAt: number } {
  const g = startedGame({
    players: 2,
    decks: [[SPELL, BEARS, BEARS], []],
    scripts: createRegistry([SYNCHRONIZED_STRIKE_SCRIPT]),
  });
...
describe('Synchronized Strike (up to two targets)', () => {
  test('two attackers: both untap and get +2/+2; the attack lands for 8', () => {
    ...
    expect(pt(g, a)).toEqual({ power: 4, toughness: 4 });
  ...
  test('the suppression predicate holds (D187)', () => {
    const text = SYNCHRONIZED_STRIKE.faces[0]?.oracleText ?? '';
    expect(parseEffects(text, SYNCHRONIZED_STRIKE.name, true).mode).not.toBe('auto');
    expect(SHIPPED_REGISTRY.spell(SYNCHRONIZED_STRIKE.oracleId)).toBeDefined();
  });
