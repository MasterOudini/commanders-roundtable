// `Static Prison` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STATIC_PRISON_SCRIPT } from './staticPrison';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Static Prison";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Static Prison"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STATIC_PRISON_SCRIPT]),
  ...
  if (which === 1) {
    advanceUntil(g, (s) => s.turn.turnNumber === 5 && s.turn.step === 'precombatMain', 40_000);
...
describe("Static Prison", () => {
  test("When this enchantment enters: the vocabulary resolves \"Exile target nonland permanent an opponent controls until this enchantment leaves the battlefield. You get {E}{E}.\"", () => {
    const { g, self, no, energy0 } = armed(0);
  ...
  test("At the beginning of your first main phase: the vocabulary resolves \"Sacrifice this enchantment unless you pay {E}.\" - the price is declined", () => {
  ...
  test("At the beginning of your first main phase: the vocabulary resolves \"Sacrifice this enchantment unless you pay {E}.\" - the price is paid", () => {
    const { g, self, energy0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
