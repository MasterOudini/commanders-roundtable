// `Seal of Fire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEAL_OF_FIRE_SCRIPT } from './sealOfFire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Seal of Fire";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seal of Fire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEAL_OF_FIRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Seal of Fire", () => {
  test("Sacrifice this enchantment: it deals 2 damage to the declared creature", () => {
