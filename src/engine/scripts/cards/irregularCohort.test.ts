// `Irregular Cohort` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IRREGULAR_COHORT_SCRIPT } from './irregularCohort';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Irregular Cohort";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Irregular Cohort"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IRREGULAR_COHORT_SCRIPT]),
  });
  holdEverywhere(g);
