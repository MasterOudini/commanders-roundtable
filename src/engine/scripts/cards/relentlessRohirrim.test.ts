// `Relentless Rohirrim` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RELENTLESS_ROHIRRIM_SCRIPT } from './relentlessRohirrim';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Relentless Rohirrim";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Relentless Rohirrim"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RELENTLESS_ROHIRRIM_SCRIPT]),
