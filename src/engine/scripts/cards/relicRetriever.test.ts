// `Relic Retriever` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RELIC_RETRIEVER_SCRIPT } from './relicRetriever';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Relic Retriever";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Relic Retriever"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RELIC_RETRIEVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Relic Retriever", () => {
  test("At the beginning of each end step: 1 token made", () => {
