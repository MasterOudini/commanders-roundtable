// `Unflinching Courage` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2 and has trample, lifelink;
...
import { UNFLINCHING_COURAGE_SCRIPT } from './unflinchingCourage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unflinching Courage";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([UNFLINCHING_COURAGE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([UNFLINCHING_COURAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Unflinching Courage", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+2 and has trample, lifelink; Cyclops of One-Eyed Pass is untouched", () => {
