// `The Grey Havens` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THE_GREY_HAVENS_SCRIPT } from './theGreyHavens';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "The Grey Havens";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["The Grey Havens"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THE_GREY_HAVENS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("The Grey Havens", () => {
  test("When The Grey Havens enters: scry 1 asks, and the cards stay on top", () => {
