// `Rogue Elephant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROGUE_ELEPHANT_SCRIPT } from './rogueElephant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rogue Elephant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rogue Elephant", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROGUE_ELEPHANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rogue Elephant", () => {
  test("When this creature enters: the vocabulary resolves \"Sacrifice it unless you sacrifice a Forest.\" - the price is declined", () => {
  ...
  test("When this creature enters: the vocabulary resolves \"Sacrifice it unless you sacrifice a Forest.\" - the price is paid", () => {
    const { g, self, board0, priceL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
