// `Tiger Claws` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+1 and has trample;
...
import { TIGER_CLAWS_SCRIPT } from './tigerClaws';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tiger Claws";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TIGER_CLAWS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([TIGER_CLAWS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Tiger Claws", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+1 and has trample; Cyclops of One-Eyed Pass is untouched", () => {
