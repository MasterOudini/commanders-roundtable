// `Trollhide` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { TROLLHIDE_SCRIPT } from './trollhide';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trollhide";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Trollhide","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TROLLHIDE_SCRIPT]),
  });
  holdEverywhere(g);
