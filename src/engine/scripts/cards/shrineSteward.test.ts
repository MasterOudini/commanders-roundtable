// `Shrine Steward` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shrine Steward", "Aboshan's Desire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHRINE_STEWARD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shrine Steward", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for an Aura or Shrine card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
