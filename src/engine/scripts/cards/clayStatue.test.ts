// `Clay Statue` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLAY_STATUE_SCRIPT } from './clayStatue';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Clay Statue";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Clay Statue", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLAY_STATUE_SCRIPT]),
  });
  holdEverywhere(g);
