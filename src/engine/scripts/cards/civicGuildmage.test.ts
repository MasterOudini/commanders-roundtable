// `Civic Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CIVIC_GUILDMAGE_SCRIPT } from './civicGuildmage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Civic Guildmage";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CIVIC_GUILDMAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Civic Guildmage", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CIVIC_GUILDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
