// `Drownyard Temple` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DROWNYARD_TEMPLE_SCRIPT } from './drownyardTemple';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drownyard Temple";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drownyard Temple"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DROWNYARD_TEMPLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Drownyard Temple", () => {
  test("{3}: it returns from the graveyard to the battlefield tapped", () => {
