// `Gobbling Ooze` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBBLING_OOZE_SCRIPT } from './gobblingOoze';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gobbling Ooze";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gobbling Ooze", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBBLING_OOZE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gobbling Ooze", () => {
  test("{G}, Sacrifice another creature: it gets 1 +1/+1 counter", () => {
