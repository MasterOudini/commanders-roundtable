// `Epiphany Storm` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { EPIPHANY_STORM_SCRIPT } from './epiphanyStorm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Epiphany Storm";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Epiphany Storm","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EPIPHANY_STORM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Epiphany Storm", () => {
  test("grants {R}, {T}, Discard a card: Draw a card. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and runs the payload', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{R}, {T}, Discard a card");
