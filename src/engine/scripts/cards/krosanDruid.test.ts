// `Krosan Druid` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KROSAN_DRUID_SCRIPT } from './krosanDruid';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Krosan Druid";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Krosan Druid"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KROSAN_DRUID_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Krosan Druid", () => {
  test("When this creature enters: 10 life is gained", () => {
