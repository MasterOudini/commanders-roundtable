// `Resplendent Mentor` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { RESPLENDENT_MENTOR_SCRIPT } from './resplendentMentor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Resplendent Mentor";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Resplendent Mentor","Silvercoat Lion"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RESPLENDENT_MENTOR_SCRIPT]),
  });
  holdEverywhere(g);
  const host = put(g, 'p1', "Silvercoat Lion");
...
describe("Resplendent Mentor", () => {
  test("grants {T}: You gain 1 life. to Silvercoat Lion, and to nothing outside the scope", () => {
