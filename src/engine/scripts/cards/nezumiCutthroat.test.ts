// `Nezumi Cutthroat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NEZUMI_CUTTHROAT_SCRIPT } from './nezumiCutthroat';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nezumi Cutthroat";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nezumi Cutthroat", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NEZUMI_CUTTHROAT_SCRIPT]),
  });
  holdEverywhere(g);
