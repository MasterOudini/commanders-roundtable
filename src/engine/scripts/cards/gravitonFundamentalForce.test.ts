// `Graviton, Fundamental Force` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRAVITON_FUNDAMENTAL_FORCE_SCRIPT } from './gravitonFundamentalForce';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Graviton, Fundamental Force";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GRAVITON_FUNDAMENTAL_FORCE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Graviton, Fundamental Force", "Coral Eel", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRAVITON_FUNDAMENTAL_FORCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Graviton, Fundamental Force", () => {
  test("Whenever you draw your second card each turn: choosing \"Target creature gains flying until end of turn.\", pumpTarget", () => {
    const { g, no } = armed(0);
    expect(pt(g, no)).toEqual([5, 2]);
    expect(kw(g, no).has("flying")).toBe(true);
  });

  test("Whenever you draw your second card each turn: choosing \"Tap target creature.\", tapTarget", () => {
    const { g, no } = armed(1);
