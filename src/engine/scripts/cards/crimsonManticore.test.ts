// `Crimson Manticore` — damage on my ATTACKING creature: the right target is
// accepted, a creature that stayed home is refused (D291). Generated from one table row (D292).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { CRIMSON_MANTICORE_SCRIPT } from './crimsonManticore';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crimson Manticore";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([CRIMSON_MANTICORE_SCRIPT]),
  });
...
describe("Crimson Manticore", () => {
  test('the attacking creature is a legal target and the effect lands', () => {
    ...
    settle(g);
    expect(g.state.cards[target]?.damage).toBe(1);
