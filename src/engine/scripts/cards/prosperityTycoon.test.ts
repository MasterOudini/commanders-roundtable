// `Prosperity Tycoon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PROSPERITY_TYCOON_SCRIPT } from './prosperityTycoon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';
import { TOKEN_TABLE } from '../../../data/tokenTable';

const CARD = "Prosperity Tycoon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prosperity Tycoon"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PROSPERITY_TYCOON_SCRIPT]),
...
describe("Prosperity Tycoon", () => {
  test("When this creature enters: the vocabulary resolves \"Create a 1/1 red Mercenary creature token with \\\"{T}: Target creature you control gets +1/+0 until end of turn. Activate only as a sorcery.\\\"\"", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("{2}, Sacrifice a token: the vocabulary resolves \"~ gains indestructible until end of turn. Tap it.\"", () => {
