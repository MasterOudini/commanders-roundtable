// `Skywing Aven` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKYWING_AVEN_SCRIPT } from './skywingAven';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skywing Aven";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skywing Aven", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKYWING_AVEN_SCRIPT]),
  });
  holdEverywhere(g);
