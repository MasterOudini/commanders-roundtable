// `Thopter Fabricator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THOPTER_FABRICATOR_SCRIPT } from './thopterFabricator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thopter Fabricator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thopter Fabricator", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THOPTER_FABRICATOR_SCRIPT]),
  });
  holdEverywhere(g);
