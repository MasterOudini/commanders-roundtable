// `Kazandu Blademaster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KAZANDU_BLADEMASTER_SCRIPT } from './kazanduBlademaster';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kazandu Blademaster";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kazandu Blademaster"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KAZANDU_BLADEMASTER_SCRIPT]),
  });
  holdEverywhere(g);
