// `Reaper from the Abyss` - every printed ability proven in its own game: the cost's mark, the pump
...
import { REAPER_FROM_THE_ABYSS_SCRIPT } from './reaperFromTheAbyss';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Reaper from the Abyss";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Reaper from the Abyss", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([REAPER_FROM_THE_ABYSS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Reaper from the Abyss", () => {
  test("Morbid — At the beginning of each end step: the vocabulary resolves \"Destroy target non-Demon creature.\"", () => {
    const { g, no } = armed(0);
