// `Rotting Rats` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROTTING_RATS_SCRIPT } from './rottingRats';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rotting Rats";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rotting Rats"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROTTING_RATS_SCRIPT]),
