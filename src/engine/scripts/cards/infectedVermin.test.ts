// `Infected Vermin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INFECTED_VERMIN_SCRIPT } from './infectedVermin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Infected Vermin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Infected Vermin", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INFECTED_VERMIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Infected Vermin", () => {
  test("{2}{B}: the vocabulary resolves \"~ deals 1 damage to each creature and each player.\"", () => {
    const { g, self, no, life0, p2life0 } = armed(0);
  ...
  test("Threshold — {3}{B}: the vocabulary resolves \"~ deals 3 damage to each creature and each player.\" - refused unless if there are seven or more cards in your graveyard", () => {
    const { g, self, no, life0, p2life0, condRefused } = armed(1);
