// `Asphodel Wanderer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASPHODEL_WANDERER_SCRIPT } from './asphodelWanderer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Asphodel Wanderer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Asphodel Wanderer", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASPHODEL_WANDERER_SCRIPT]),
  });
  holdEverywhere(g);
