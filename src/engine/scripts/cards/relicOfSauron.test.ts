// `Relic of Sauron` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RELIC_OF_SAURON_SCRIPT } from './relicOfSauron';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Relic of Sauron";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Relic of Sauron"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RELIC_OF_SAURON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Relic of Sauron", () => {
  test("{3}, {T}: the vocabulary resolves \"Draw two cards, then discard a card.\"", () => {
    ...
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 2 - qdA1);
