// `Guard Gomazoa` - the continuous prevention effect proven both ways (D385): the damage the line
// covers never arrives and nothing is spent; damage outside the line lands. Generated from one row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { GUARD_GOMAZOA_SCRIPT } from './guardGomazoa';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Guard Gomazoa";
...
function armed(): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Guard Gomazoa", "Grizzly Bears", "Spark Spray"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GUARD_GOMAZOA_SCRIPT]),
  });
  holdEverywhere(g);
