// `Markov Warlord` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARKOV_WARLORD_SCRIPT } from './markovWarlord';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Markov Warlord";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Markov Warlord"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARKOV_WARLORD_SCRIPT]),
