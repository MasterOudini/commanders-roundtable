// `Greataxe` - equipped to Grizzly Bears the creature gets +4/+0; the Cyclops is untouched;
...
import { GREATAXE_SCRIPT } from './greataxe';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GREATAXE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GREATAXE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Greataxe", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +4/+0; the Cyclops is untouched", () => {
