// `Gudul Lurker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GUDUL_LURKER_SCRIPT } from './gudulLurker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gudul Lurker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gudul Lurker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GUDUL_LURKER_SCRIPT]),
  });
  holdEverywhere(g);
