// `Sarkhan Vol` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SARKHAN_VOL_SCRIPT } from './sarkhanVol';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sarkhan Vol";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SARKHAN_VOL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sarkhan Vol", "Coral Eel", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SARKHAN_VOL_SCRIPT]),
  ...
  if (which === 2) {
    must(g.submit({ t: 'ManualSetCounter', player: 'p1', card: self, kind: 'loyalty', delta: 3 }));
...
describe("Sarkhan Vol", () => {
  test("+1: its controller's creatures get +1/+1 and gain haste until end of turn", () => {
  ...
  test("−2: the vocabulary resolves \"Gain control of target creature until end of turn. Untap that creature. It gains haste until end of turn.\"", () => {
    const { g, self, no, board0, refused } = armed(1);
  ...
  test("−6: the vocabulary resolves \"Create five 4/4 red Dragon creature tokens with flying.\"", () => {
    const { g, self, board0, refused } = armed(2);
