// `Patron Wizard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PATRON_WIZARD_SCRIPT } from './patronWizard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Patron Wizard";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; vtA0_0: InstanceId; payA0_p2_0: InstanceId; tapper0: InstanceId[] };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Patron Wizard", "Fugitive Wizard"], ["Cyclops of One-Eyed Pass", "Divination", "Forest"]],
    scripts: createRegistry([PATRON_WIZARD_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtA0_0 = put(g, 'p2', "Divination", 'hand');
  const payA0_p2_0 = put(g, 'p2', "Forest");
  const tapper0 = [put(g, 'p1', "Fugitive Wizard")];
  settle(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, vtA0_0, payA0_p2_0, tapper0 };
}

describe("Patron Wizard", () => {
  test("Tap an untapped Wizard you control: the vocabulary resolves \"Counter target spell unless its controller pays {1}.\" - the price is declined", () => {
    const { g, payA0_p2_0, tapper0 } = armed(0);
  ...
  test("Tap an untapped Wizard you control: the vocabulary resolves \"Counter target spell unless its controller pays {1}.\" - the price is paid", () => {
    const { g, payA0_p2_0, tapper0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
