// `Riot Gear` - equipped to Grizzly Bears the creature gets +1/+2; the Cyclops is untouched;
...
import { RIOT_GEAR_SCRIPT } from './riotGear';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Riot Gear";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RIOT_GEAR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([RIOT_GEAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Riot Gear", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+2; the Cyclops is untouched", () => {
