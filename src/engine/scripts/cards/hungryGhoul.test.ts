// `Hungry Ghoul` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HUNGRY_GHOUL_SCRIPT } from './hungryGhoul';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hungry Ghoul";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hungry Ghoul", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HUNGRY_GHOUL_SCRIPT]),
  });
  holdEverywhere(g);
