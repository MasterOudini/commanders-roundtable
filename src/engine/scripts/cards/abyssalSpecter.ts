// `Abyssal Specter` - a dealsDamagePlayer trigger vocab
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { ABYSSAL_SPECTER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(ABYSSAL_SPECTER, "Flying\nWhenever this creature deals damage to a player, that player discards a card.");
const LINES = PRINTED.split('\n');

const VOCAB_L1 = vocabularyEffects("Target player discards a card.", ABYSSAL_SPECTER.name);
const VOCAB_T_L1 = vocabularyTargets("Target player discards a card.");

export const ABYSSAL_SPECTER_SCRIPT: CardScript = {
  oracleId: ABYSSAL_SPECTER.oracleId,
  name: ABYSSAL_SPECTER.name,
  triggers: [
    {
      ...
      label: () => "Abyssal Specter - Target player discards a card.",
