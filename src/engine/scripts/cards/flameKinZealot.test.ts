// `Flame-Kin Zealot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLAME_KIN_ZEALOT_SCRIPT } from './flameKinZealot';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flame-Kin Zealot";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FLAME_KIN_ZEALOT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flame-Kin Zealot", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLAME_KIN_ZEALOT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Flame-Kin Zealot", () => {
  test("When this creature enters: its controller's creatures get +1/+1 and gain haste until end of turn", () => {
