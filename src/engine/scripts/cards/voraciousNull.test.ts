// `Voracious Null` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VORACIOUS_NULL_SCRIPT } from './voraciousNull';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Voracious Null";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Voracious Null", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VORACIOUS_NULL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Voracious Null", () => {
  test("{1}{B}, Sacrifice another creature: it gets 2 +1/+1 counters", () => {
