// `Simic Locket` — all-green pays the hybrids for the draw-two.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SIMIC_LOCKET_SCRIPT } from './simicLocket';
...
function locketed(): { g: Game; mid: number } {
  const g = startedGame({
    players: 2,
    decks: [['Simic Locket'], []],
    scripts: createRegistry([SIMIC_LOCKET_SCRIPT]),
  });
  const locket = put(g, 'p1', 'Simic Locket');
  settle(g);
  holdEverywhere(g);
...
describe('Simic Locket', () => {
  test('the Locket dies and two draws arrive', () => {
