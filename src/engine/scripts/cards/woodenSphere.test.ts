// `Wooden Sphere` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WOODEN_SPHERE_SCRIPT } from './woodenSphere';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wooden Sphere";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wooden Sphere", "Grizzly Bears", "Norwood Ranger", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WOODEN_SPHERE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wooden Sphere", () => {
  test("Whenever a player casts a green spell: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is declined", () => {
  ...
  test("Whenever a player casts a green spell: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is paid", () => {
