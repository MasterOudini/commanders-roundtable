// `Cosmogrand Zenith` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COSMOGRAND_ZENITH_SCRIPT } from './cosmograndZenith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cosmogrand Zenith";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cosmogrand Zenith", "Coral Eel", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COSMOGRAND_ZENITH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cosmogrand Zenith", () => {
  test("Whenever you cast your second spell each turn: choosing \"Create two 1/1 white Human Soldier creature tokens.\", token", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 2 + 1);
  });

  test("Whenever you cast your second spell each turn: choosing \"Put a +1/+1 counter on each creature you control.\", massCounter", () => {
