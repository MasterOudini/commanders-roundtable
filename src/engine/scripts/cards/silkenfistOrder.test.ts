// `Silkenfist Order` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SILKENFIST_ORDER_SCRIPT } from './silkenfistOrder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Silkenfist Order";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Silkenfist Order"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SILKENFIST_ORDER_SCRIPT]),
  });
  holdEverywhere(g);
