// `Welkin Hawk` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Welkin Hawk", "Welkin Hawk"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WELKIN_HAWK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Welkin Hawk", () => {
  test("When this creature dies: the vocabulary resolves \"Search your library for a card named ~, reveal that card, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Welkin Hawk" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Welkin Hawk");
      expect(found, "Welkin Hawk is not in the library for the search to find").toBeDefined();
