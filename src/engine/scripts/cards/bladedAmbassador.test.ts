// `Bladed Ambassador` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLADED_AMBASSADOR_SCRIPT } from './bladedAmbassador';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bladed Ambassador";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLADED_AMBASSADOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bladed Ambassador"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLADED_AMBASSADOR_SCRIPT]),
...
describe("Bladed Ambassador", () => {
  ...
  test("{1}, Remove an oil counter from this creature: it gains indestructible until end of turn", () => {
    const { g, self, cnt0, pt0 } = armed(1);
    expect(pt(g, self)).toEqual([(pt0[0] ?? 0) + 0 - 0, (pt0[1] ?? 0) + 0 - 0]);
