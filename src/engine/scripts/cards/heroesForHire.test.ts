// `Heroes for Hire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HEROES_FOR_HIRE_SCRIPT } from './heroesForHire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';
import { TOKEN_TABLE } from '../../../data/tokenTable';

const CARD = "Heroes for Hire";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Heroes for Hire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HEROES_FOR_HIRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Heroes for Hire", () => {
  test("When this enchantment enters: 3 tokens made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 3 + 1);
  });

  test("Sacrifice a Treasure: the vocabulary resolves \"Exile the top card of your library. You may play it this turn.\"", () => {
