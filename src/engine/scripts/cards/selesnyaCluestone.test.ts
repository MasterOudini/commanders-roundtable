// `Selesnya Cluestone` — the self-sac draw at #a1.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SELESNYA_CLUESTONE_SCRIPT } from './selesnyaCluestone';
...
function clued(): { g: Game; mid: number } {
  const g = startedGame({
    players: 2,
    decks: [['Selesnya Cluestone'], []],
    scripts: createRegistry([SELESNYA_CLUESTONE_SCRIPT]),
  });
  const stone = put(g, 'p1', 'Selesnya Cluestone');
  settle(g);
  holdEverywhere(g);
