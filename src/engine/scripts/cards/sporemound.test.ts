// `Sporemound` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sporemound"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPOREMOUND_SCRIPT]),
  });
  holdEverywhere(g);
