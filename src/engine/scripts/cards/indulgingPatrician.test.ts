// `Indulging Patrician` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INDULGING_PATRICIAN_SCRIPT } from './indulgingPatrician';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Indulging Patrician";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Indulging Patrician"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INDULGING_PATRICIAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Indulging Patrician", () => {
  test("At the beginning of your end step: each opponent loses 3 life", () => {
