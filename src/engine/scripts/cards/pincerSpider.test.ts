// `Pincer Spider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PINCER_SPIDER_SCRIPT } from './pincerSpider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pincer Spider";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PINCER_SPIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pincer Spider"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PINCER_SPIDER_SCRIPT]),
  });
  holdEverywhere(g);
