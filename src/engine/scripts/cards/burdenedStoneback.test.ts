// `Burdened Stoneback` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BURDENED_STONEBACK_SCRIPT } from './burdenedStoneback';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Burdened Stoneback";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BURDENED_STONEBACK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Burdened Stoneback"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BURDENED_STONEBACK_SCRIPT]),
...
describe("Burdened Stoneback", () => {
  ...
  test("{1}{W}, Remove a counter from this creature: the declared creature is pumped until end of turn", () => {
