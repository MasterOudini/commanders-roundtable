// `Kavu Titan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KAVU_TITAN_SCRIPT } from './kavuTitan';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kavu Titan";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KAVU_TITAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kavu Titan"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KAVU_TITAN_SCRIPT]),
  ...
  if (which === 0) {
    ...
    if (kw(g, self).has("trample")) throw new Error('the kicked keyword held on an unkicked cast');
...
describe("Kavu Titan", () => {
  test("If this creature was kicked, it enters with three +1/+1 counters on it and with trample.: cast, it enters with 3 +1/+1 counters", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.zone.kind).toBe('battlefield');
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(3);
    expect(kw(g, self).has("trample"), 'the kick granted trample').toBe(true);
