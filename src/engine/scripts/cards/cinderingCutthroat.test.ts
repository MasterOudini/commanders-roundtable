// `Cindering Cutthroat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CINDERING_CUTTHROAT_SCRIPT } from './cinderingCutthroat';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cindering Cutthroat";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CINDERING_CUTTHROAT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cindering Cutthroat"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CINDERING_CUTTHROAT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cindering Cutthroat", () => {
  test("This creature enters with a +1/+1 counter on it if an opponent lost life this turn.: cast, it enters with 1 +1/+1 counter", () => {
  ...
  test("{1}{B/R}: it gains menace until end of turn", () => {
