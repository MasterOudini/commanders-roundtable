// `Graypelt Hunter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRAYPELT_HUNTER_SCRIPT } from './graypeltHunter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Graypelt Hunter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Graypelt Hunter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRAYPELT_HUNTER_SCRIPT]),
  });
  holdEverywhere(g);
