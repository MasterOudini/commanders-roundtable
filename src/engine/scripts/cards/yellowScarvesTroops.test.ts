// `Yellow Scarves Troops` - every printed ability proven in its own game: the cost's mark, the pump
...
import { YELLOW_SCARVES_TROOPS_SCRIPT } from './yellowScarvesTroops';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Yellow Scarves Troops";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Yellow Scarves Troops", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YELLOW_SCARVES_TROOPS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Yellow Scarves Troops", () => {
