// `Twinblade Blessing` - cast on Grizzly Bears it attaches and the enchanted creature has doubleStrike;
...
import { TWINBLADE_BLESSING_SCRIPT } from './twinbladeBlessing';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Twinblade Blessing";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([TWINBLADE_BLESSING_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([TWINBLADE_BLESSING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Twinblade Blessing", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has doubleStrike; Cyclops of One-Eyed Pass is untouched", () => {
