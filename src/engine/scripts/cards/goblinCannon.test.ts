// `Goblin Cannon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_CANNON_SCRIPT } from './goblinCannon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Cannon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Cannon"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_CANNON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Cannon", () => {
  test("{2}: the vocabulary resolves \"~ deals 1 damage to any target. Sacrifice this artifact.\"", () => {
