// `Ainok Guide` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ainok Guide", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AINOK_GUIDE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ainok Guide", () => {
  ...
  test("When this creature enters: choosing \"Search your library for a basic land card, reveal it, then shuffle and put that card on top.\", the vocabulary resolves \"Search your library for a basic land card, reveal it, then shuffle and put that card on top.\"", () => {
    const { g } = armed(1);
