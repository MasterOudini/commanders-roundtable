// `Stonecoil Serpent` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STONECOIL_SERPENT_SCRIPT } from './stonecoilSerpent';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stonecoil Serpent";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STONECOIL_SERPENT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stonecoil Serpent"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STONECOIL_SERPENT_SCRIPT]),
  });
  holdEverywhere(g);
