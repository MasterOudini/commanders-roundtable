// `One With the Wind` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2 and has flying;
...
import { ONE_WITH_THE_WIND_SCRIPT } from './oneWithTheWind';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "One With the Wind";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ONE_WITH_THE_WIND_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ONE_WITH_THE_WIND_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("One With the Wind", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+2 and has flying; Cyclops of One-Eyed Pass is untouched", () => {
