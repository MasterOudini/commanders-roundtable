// `Blightspeaker` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blightspeaker", "Children of Korlis"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLIGHTSPEAKER_SCRIPT]),
...
describe("Blightspeaker", () => {
  test("{T}: the vocabulary resolves \"Target player loses 1 life.\"", () => {
    const { g, self, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 + -1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{4}, {T}: the vocabulary resolves \"Search your library for a Rebel permanent card with mana value 3 or less, put it onto the battlefield, then shuffle.\"", () => {
    const { g, self, board0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Children of Korlis" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Children of Korlis");
      expect(found, "Children of Korlis is not in the library for the search to find").toBeDefined();
