// `Rootwater Alligator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROOTWATER_ALLIGATOR_SCRIPT } from './rootwaterAlligator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rootwater Alligator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rootwater Alligator", "Forest", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROOTWATER_ALLIGATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rootwater Alligator", () => {
  test("Sacrifice a Forest: it regenerates from the Bolt", () => {
