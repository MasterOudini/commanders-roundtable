// `Roxxon Brutes` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROXXON_BRUTES_SCRIPT } from './roxxonBrutes';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Roxxon Brutes";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Roxxon Brutes", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROXXON_BRUTES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Roxxon Brutes", () => {
  test("Whenever you draw your second card each turn: the vocabulary resolves \"Put a +1/+1 counter on target creature.\"", () => {
    const { g, no } = armed(0);
