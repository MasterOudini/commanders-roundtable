// `Burlfist Oak` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BURLFIST_OAK_SCRIPT } from './burlfistOak';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Burlfist Oak";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BURLFIST_OAK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Burlfist Oak"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BURLFIST_OAK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Burlfist Oak", () => {
  test("Whenever you draw a card: it gets +2/+2 until end of turn", () => {
