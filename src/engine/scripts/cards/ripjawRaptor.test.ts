// `Ripjaw Raptor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIPJAW_RAPTOR_SCRIPT } from './ripjawRaptor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ripjaw Raptor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ripjaw Raptor", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([RIPJAW_RAPTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ripjaw Raptor", () => {
  test("Enrage — Whenever this creature is dealt damage: a card is drawn", () => {
