// `Ember Hauler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EMBER_HAULER_SCRIPT } from './emberHauler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ember Hauler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ember Hauler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EMBER_HAULER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ember Hauler", () => {
  test("{1}, Sacrifice this creature: it deals 2 damage to the declared creature", () => {
