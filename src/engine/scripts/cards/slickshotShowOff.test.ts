// `Slickshot Show-Off` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLICKSHOT_SHOW_OFF_SCRIPT } from './slickshotShowOff';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slickshot Show-Off";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SLICKSHOT_SHOW_OFF_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slickshot Show-Off", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLICKSHOT_SHOW_OFF_SCRIPT]),
...
describe("Slickshot Show-Off", () => {
  test("Whenever you cast a noncreature spell: it gets +2/+0 until end of turn", () => {
