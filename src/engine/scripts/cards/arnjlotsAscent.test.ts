// `Arnjlot's Ascent` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ARNJLOTS_ASCENT_SCRIPT } from './arnjlotsAscent';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ARNJLOTS_ASCENT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Arnjlot's Ascent", "Island", "Island", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARNJLOTS_ASCENT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Arnjlot's Ascent", () => {
  test("{1}: the declared creature is pumped until end of turn", () => {
    const { g, no } = armed(0);
