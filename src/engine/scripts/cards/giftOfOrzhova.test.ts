// `Gift of Orzhova` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+1 and has flying, lifelink;
...
import { GIFT_OF_ORZHOVA_SCRIPT } from './giftOfOrzhova';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gift of Orzhova";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GIFT_OF_ORZHOVA_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GIFT_OF_ORZHOVA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gift of Orzhova", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+1 and has flying, lifelink; Cyclops of One-Eyed Pass is untouched", () => {
