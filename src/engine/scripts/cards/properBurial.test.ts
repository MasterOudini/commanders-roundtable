// `Proper Burial` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PROPER_BURIAL_SCRIPT } from './properBurial';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Proper Burial";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Proper Burial", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PROPER_BURIAL_SCRIPT]),
...
describe("Proper Burial", () => {
  test("Whenever a creature you control dies: the vocabulary resolves \"You gain life equal to target creature's toughness.\"", () => {
