// `Twisted Abomination` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TWISTED_ABOMINATION_SCRIPT } from './twistedAbomination';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Twisted Abomination";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Twisted Abomination", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TWISTED_ABOMINATION_SCRIPT]),
  });
  holdEverywhere(g);
