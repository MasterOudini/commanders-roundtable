...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([RABID_RATS_SCRIPT]),
  });
...
describe("Rabid Rats", () => {
  test('the blocking creature is a legal target and the effect lands', () => {
    ...
    settle(g);
    const d = deps(createRegistry([RABID_RATS_SCRIPT]));
