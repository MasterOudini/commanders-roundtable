// `Bootleggers' Stash` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BOOTLEGGERS_STASH_SCRIPT } from './bootleggersStash';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BOOTLEGGERS_STASH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bootleggers' Stash", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOOTLEGGERS_STASH_SCRIPT]),
...
describe("Bootleggers' Stash", () => {
  test("Lands you control have \"{T}: only the land reads it", () => {
    const { g, no } = armed(0);
    expect(pt(g, no)).toEqual([5, 2]);
  });

  test("Lands you control have \"{T}: the vocabulary resolves \"Create a Treasure token.\"", () => {
