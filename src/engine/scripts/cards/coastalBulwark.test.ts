// `Coastal Bulwark` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COASTAL_BULWARK_SCRIPT } from './coastalBulwark';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Coastal Bulwark";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([COASTAL_BULWARK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Coastal Bulwark", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COASTAL_BULWARK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Coastal Bulwark", () => {
  test("`Coastal Bulwark` - as long as you control an Island: absent while it is unmet, read once it holds", () => {
