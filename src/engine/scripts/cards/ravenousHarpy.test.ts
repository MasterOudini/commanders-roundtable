// `Ravenous Harpy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAVENOUS_HARPY_SCRIPT } from './ravenousHarpy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ravenous Harpy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ravenous Harpy", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAVENOUS_HARPY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ravenous Harpy", () => {
  test("{1}, Sacrifice another creature: it gets 1 +1/+1 counter", () => {
