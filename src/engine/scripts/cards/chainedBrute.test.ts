// `Chained Brute` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHAINED_BRUTE_SCRIPT } from './chainedBrute';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chained Brute";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chained Brute", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHAINED_BRUTE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chained Brute", () => {
  ...
  test("{1}, Sacrifice another creature: it untaps - refused unless during your turn", () => {
