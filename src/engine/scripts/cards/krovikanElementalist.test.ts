// `Krovikan Elementalist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KROVIKAN_ELEMENTALIST_SCRIPT } from './krovikanElementalist';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Krovikan Elementalist";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KROVIKAN_ELEMENTALIST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Krovikan Elementalist", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KROVIKAN_ELEMENTALIST_SCRIPT]),
...
describe("Krovikan Elementalist", () => {
  test("{2}{R}: the declared creature is pumped until end of turn", () => {
    const { g, no } = armed(0);
    expect(pt(g, no)).toEqual([6, 2]);
  });

  test("{U}{U}: the vocabulary resolves \"Target creature you control gains flying until end of turn. Sacrifice it at the beginning of the next end step.\"", () => {
    const { g, vtA1_0 } = armed(1);
    expect(pt(g, vtA1_0)).toEqual([5, 2]);
    expect(kw(g, vtA1_0).has("flying")).toBe(true);
    ...
    { const boardD = onBoard(g);
      ...
      expect(g.state.cards[vtA1_0]?.zone.kind, "sacrificeObj at the step").toBe("graveyard");
    }
