// `Serra Inquisitors` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SERRA_INQUISITORS_SCRIPT } from './serraInquisitors';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Serra Inquisitors";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SERRA_INQUISITORS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Serra Inquisitors"], ["Cyclops of One-Eyed Pass", "Walking Corpse"]],
    scripts: createRegistry([SERRA_INQUISITORS_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const gift2 = put(g, 'p2', "Walking Corpse");
  settle(g);
...
describe("Serra Inquisitors", () => {
  test("Whenever this creature blocks or becomes blocked by one or more black creatures: it gets +2/+0 until end of turn", () => {
