// `Sorcerer of the Fang` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SORCERER_OF_THE_FANG_SCRIPT } from './sorcererOfTheFang';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sorcerer of the Fang";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sorcerer of the Fang"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SORCERER_OF_THE_FANG_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sorcerer of the Fang", () => {
  test("{5}{B}, {T}: the vocabulary resolves \"~ deals 2 damage to target opponent or planeswalker.\"", () => {
