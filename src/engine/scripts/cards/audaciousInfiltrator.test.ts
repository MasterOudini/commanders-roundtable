// `Audacious Infiltrator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AUDACIOUS_INFILTRATOR_SCRIPT } from './audaciousInfiltrator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Audacious Infiltrator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Audacious Infiltrator", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Memnite"]],
    scripts: createRegistry([AUDACIOUS_INFILTRATOR_SCRIPT]),
...
describe("Audacious Infiltrator", () => {
  test("This creature can't be blocked by artifact creatures.: the block by artifact creatures is refused", () => {
