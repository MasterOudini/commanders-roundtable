// `Irencrag Pyromancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IRENCRAG_PYROMANCER_SCRIPT } from './irencragPyromancer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Irencrag Pyromancer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Irencrag Pyromancer", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IRENCRAG_PYROMANCER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Irencrag Pyromancer", () => {
  test("Whenever you draw your second card each turn: it deals 3 damage to the declared creature", () => {
    const { g, no } = armed(0);
