// `Crystal Slipper` - equipped to Grizzly Bears the creature gets +1/+0 and has haste; the Cyclops is untouched;
...
import { CRYSTAL_SLIPPER_SCRIPT } from './crystalSlipper';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crystal Slipper";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CRYSTAL_SLIPPER_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([CRYSTAL_SLIPPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Crystal Slipper", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+0 and has haste; the Cyclops is untouched", () => {
