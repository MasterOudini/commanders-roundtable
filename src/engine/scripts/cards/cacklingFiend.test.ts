// `Cackling Fiend` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CACKLING_FIEND_SCRIPT } from './cacklingFiend';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cackling Fiend";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cackling Fiend"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CACKLING_FIEND_SCRIPT]),
  });
  holdEverywhere(g);
