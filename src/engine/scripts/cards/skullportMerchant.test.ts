// `Skullport Merchant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKULLPORT_MERCHANT_SCRIPT } from './skullportMerchant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skullport Merchant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skullport Merchant", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKULLPORT_MERCHANT_SCRIPT]),
  });
  holdEverywhere(g);
