// `Cryptolith Rite` - the granted MANA ability proven end to end (D372): the recipient HAS the
...
import { CRYPTOLITH_RITE_SCRIPT } from './cryptolithRite';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cryptolith Rite";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Cryptolith Rite","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRYPTOLITH_RITE_SCRIPT]),
  });
  holdEverywhere(g);
