// `Traveler's Amulet` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Traveler's Amulet", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRAVELERS_AMULET_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Traveler's Amulet", () => {
  test("{1}, Sacrifice this artifact: the vocabulary resolves \"Search your library for a basic land card, reveal it, put it into your hand, then shuffle.\"", () => {
