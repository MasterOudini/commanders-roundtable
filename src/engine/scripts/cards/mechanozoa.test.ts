// `Mechanozoa` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MECHANOZOA_SCRIPT } from './mechanozoa';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mechanozoa";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mechanozoa"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MECHANOZOA_SCRIPT]),
