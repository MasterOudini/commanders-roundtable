// `Erratic Visionary` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ERRATIC_VISIONARY_SCRIPT } from './erraticVisionary';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Erratic Visionary";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Erratic Visionary"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ERRATIC_VISIONARY_SCRIPT]),
  });
  holdEverywhere(g);
