// `Copperhorn Scout` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COPPERHORN_SCOUT_SCRIPT } from './copperhornScout';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Copperhorn Scout";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Copperhorn Scout", "Runeclaw Bear"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COPPERHORN_SCOUT_SCRIPT]),
...
describe("Copperhorn Scout", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Untap each other creature you control.\"", () => {
