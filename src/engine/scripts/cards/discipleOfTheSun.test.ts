// `Disciple of the Sun` - the named target is accepted and it returns to hand; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { DISCIPLE_OF_THE_SUN_SCRIPT } from './discipleOfTheSun';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Disciple of the Sun";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Disciple of the Sun", "Grizzly Bears", "Dread Reaper"], ["Grizzly Bears"]],
    scripts: createRegistry([DISCIPLE_OF_THE_SUN_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD, 'graveyard');
  const target = put(g, 'p1', "Grizzly Bears", 'graveyard');
  const wrong = put(g, 'p1', "Dread Reaper", 'graveyard');
  settle(g);
...
describe("Disciple of the Sun", () => {
  ...
  test("Dread Reaper is refused (D294)", () => {
    const { g, wrong } = armed();
