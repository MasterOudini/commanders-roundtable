// `Sylvok Battle-Chair` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SYLVOK_BATTLE_CHAIR_SCRIPT } from './sylvokBattleChair';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sylvok Battle-Chair";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SYLVOK_BATTLE_CHAIR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sylvok Battle-Chair", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SYLVOK_BATTLE_CHAIR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sylvok Battle-Chair", () => {
  test("For Mirrodin! (When this Equipment enters: a token is made and it attaches to it", () => {
    ...
    if (germ !== null) expect(pt(g, germ)).toEqual([6, 6]);
  });

  test("Equipped creature gets +4/+4 and has trample.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([6, 6]);
