// `Ahn-Crop Invader` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AHN_CROP_INVADER_SCRIPT } from './ahnCropInvader';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ahn-Crop Invader";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([AHN_CROP_INVADER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ahn-Crop Invader", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AHN_CROP_INVADER_SCRIPT]),
...
describe("Ahn-Crop Invader", () => {
  test("`Ahn-Crop Invader` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
  ...
  test("{1}, Sacrifice another creature: it gets +2/+0 until end of turn", () => {
