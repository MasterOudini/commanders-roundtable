// `Dwarven Soldier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DWARVEN_SOLDIER_SCRIPT } from './dwarvenSoldier';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dwarven Soldier";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DWARVEN_SOLDIER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dwarven Soldier"], ["Cyclops of One-Eyed Pass", "Orcish Captain"]],
    scripts: createRegistry([DWARVEN_SOLDIER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const gift2 = put(g, 'p2', "Orcish Captain");
  settle(g);
...
describe("Dwarven Soldier", () => {
  test("Whenever this creature blocks or becomes blocked by one or more Orcs: it gets +0/+2 until end of turn", () => {
