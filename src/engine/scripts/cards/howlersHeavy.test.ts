// `Howler's Heavy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOWLERS_HEAVY_SCRIPT } from './howlersHeavy';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Howler's Heavy";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([HOWLERS_HEAVY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Howler's Heavy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOWLERS_HEAVY_SCRIPT]),
...
describe("Howler's Heavy", () => {
  test("When you cycle this card: the vocabulary resolves \"Target creature or Vehicle an opponent controls gets -3/-0 until end of turn.\"", () => {
    const { g, no } = armed(0);
    expect(pt(g, no)).toEqual([2, 2]);
  });
