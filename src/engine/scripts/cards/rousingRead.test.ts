// `Rousing Read` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROUSING_READ_SCRIPT } from './rousingRead';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rousing Read";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ROUSING_READ_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rousing Read", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROUSING_READ_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rousing Read", () => {
  test("When this Aura enters: the vocabulary resolves \"Draw two cards, then discard a card.\"", () => {
    ...
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 2 - 1 - qdL1);
  });

  test("Enchanted creature gets +1/+1 and has flying.: the creature it is attached to reads it", () => {
