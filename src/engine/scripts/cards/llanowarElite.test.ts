// `Llanowar Elite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LLANOWAR_ELITE_SCRIPT } from './llanowarElite';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Llanowar Elite";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LLANOWAR_ELITE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Llanowar Elite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LLANOWAR_ELITE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Llanowar Elite", () => {
  test("If this creature was kicked, it enters with five +1/+1 counters on it.: cast, it enters with 5 +1/+1 counters", () => {
