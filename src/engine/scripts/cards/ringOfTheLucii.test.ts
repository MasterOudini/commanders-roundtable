// `Ring of the Lucii` - the named target is accepted and it is tapped; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { RING_OF_THE_LUCII_SCRIPT } from './ringOfTheLucii';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ring of the Lucii";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Ring of the Lucii"], ["Grizzly Bears", "Forest"]],
    scripts: createRegistry([RING_OF_THE_LUCII_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ring of the Lucii", () => {
  test("Grizzly Bears is a legal target and it is tapped", () => {
    const { g, self, target, life0 } = armed();
