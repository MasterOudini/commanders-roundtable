// `Zhuge Jin, Wu Strategist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZHUGE_JIN_WU_STRATEGIST_SCRIPT } from './zhugeJinWuStrategist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zhuge Jin, Wu Strategist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zhuge Jin, Wu Strategist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZHUGE_JIN_WU_STRATEGIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zhuge Jin, Wu Strategist", () => {
  test("{T}: the vocabulary resolves \"Target creature can't be blocked this turn.\" - refused unless during your turn, before attackers are declared", () => {
