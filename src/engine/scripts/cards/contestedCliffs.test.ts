// `Contested Cliffs` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONTESTED_CLIFFS_SCRIPT } from './contestedCliffs';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Contested Cliffs";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Contested Cliffs", "Jibbirik Omnivore"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONTESTED_CLIFFS_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtA1_0 = put(g, 'p1', "Jibbirik Omnivore");
  settle(g);
...
describe("Contested Cliffs", () => {
  test("{R}{G}, {T}: the vocabulary resolves \"Target Beast creature you control fights target creature an opponent controls.\"", () => {
    const { g, self, no, board0, vtA1_0 } = armed(0);
