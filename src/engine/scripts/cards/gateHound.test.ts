// `Gate Hound` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GATE_HOUND_SCRIPT } from './gateHound';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gate Hound";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GATE_HOUND_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gate Hound", "Coral Eel", "Pacifism"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GATE_HOUND_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gate Hound", () => {
  test("`Gate Hound` - as long as this creature is enchanted: absent while it is unmet, read once it holds", () => {
