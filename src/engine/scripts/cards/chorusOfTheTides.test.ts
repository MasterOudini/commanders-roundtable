// `Chorus of the Tides` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHORUS_OF_THE_TIDES_SCRIPT } from './chorusOfTheTides';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chorus of the Tides";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chorus of the Tides", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHORUS_OF_THE_TIDES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chorus of the Tides", () => {
  test("Heroic — Whenever you cast a spell that targets this creature: scry 1 asks, and the cards stay on top", () => {
