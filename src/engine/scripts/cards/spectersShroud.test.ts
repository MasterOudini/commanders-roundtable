// `Specter's Shroud` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPECTERS_SHROUD_SCRIPT } from './spectersShroud';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Specter's Shroud";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPECTERS_SHROUD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Specter's Shroud", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPECTERS_SHROUD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Specter's Shroud", () => {
  ...
  test("Whenever equipped creature deals combat damage to a player: the vocabulary resolves \"Target player discards a card.\"", () => {
    const { g } = armed(1);
