// `Bojuka Brigand` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BOJUKA_BRIGAND_SCRIPT } from './bojukaBrigand';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bojuka Brigand";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bojuka Brigand", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOJUKA_BRIGAND_SCRIPT]),
  });
  holdEverywhere(g);
