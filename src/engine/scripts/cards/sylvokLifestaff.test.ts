// `Sylvok Lifestaff` - equipped to Grizzly Bears the creature gets +1/+0; (on the equipped creature dying: gain 3 life); the Cyclops is untouched;
...
import { SYLVOK_LIFESTAFF_SCRIPT } from './sylvokLifestaff';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sylvok Lifestaff";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SYLVOK_LIFESTAFF_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SYLVOK_LIFESTAFF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sylvok Lifestaff", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+0; (on the equipped creature dying: gain 3 life); the Cyclops is untouched", () => {
  ...
  test("the equipped creature dying: (on the equipped creature dying: gain 3 life)", () => {
    const { g, self, host } = board();
    equip(g, self, host);
