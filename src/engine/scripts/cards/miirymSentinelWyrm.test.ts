// `Miirym, Sentinel Wyrm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MIIRYM_SENTINEL_WYRM_SCRIPT } from './miirymSentinelWyrm';
import { advanceUntil, holdEverywhere, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Miirym, Sentinel Wyrm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Miirym, Sentinel Wyrm", "Grizzly Bears", "Dragonborn Looter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIIRYM_SENTINEL_WYRM_SCRIPT]),
...
describe("Miirym, Sentinel Wyrm", () => {
  test("Whenever another nontoken Dragon you control enters: the vocabulary resolves \"Create a token that's a copy of target creature, except the token isn't legendary.\"", () => {
