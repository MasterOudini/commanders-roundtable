// `Horizon Canopy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HORIZON_CANOPY_SCRIPT } from './horizonCanopy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Horizon Canopy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Horizon Canopy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HORIZON_CANOPY_SCRIPT]),
  });
  holdEverywhere(g);
