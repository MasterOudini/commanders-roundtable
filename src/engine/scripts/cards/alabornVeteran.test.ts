// `Alaborn Veteran` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ALABORN_VETERAN_SCRIPT } from './alabornVeteran';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Alaborn Veteran";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ALABORN_VETERAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Alaborn Veteran"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ALABORN_VETERAN_SCRIPT]),
  });
  holdEverywhere(g);
