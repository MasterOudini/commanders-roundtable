// `Warped Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Warped Landscape", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WARPED_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Warped Landscape", () => {
  test("{2}, {T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
