// `Geier Reach Sanitarium` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GEIER_REACH_SANITARIUM_SCRIPT } from './geierReachSanitarium';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Geier Reach Sanitarium";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Geier Reach Sanitarium"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GEIER_REACH_SANITARIUM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Geier Reach Sanitarium", () => {
  test("{2}, {T}: the vocabulary resolves \"Each player draws a card, then discards a card.\"", () => {
    ...
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1 - qdA1);
