// `Aven Surveyor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AVEN_SURVEYOR_SCRIPT } from './avenSurveyor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aven Surveyor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aven Surveyor", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AVEN_SURVEYOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aven Surveyor", () => {
  ...
  test("When this creature enters: choosing \"Return target creature to its owner's hand.\", bounceTarget", () => {
    const { g, no } = armed(1);
