// `South Wind Avatar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOUTH_WIND_AVATAR_SCRIPT } from './southWindAvatar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "South Wind Avatar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["South Wind Avatar", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOUTH_WIND_AVATAR_SCRIPT]),
...
describe("South Wind Avatar", () => {
  test("Whenever another creature you control dies: the vocabulary resolves \"You gain life equal to target creature's toughness.\"", () => {
