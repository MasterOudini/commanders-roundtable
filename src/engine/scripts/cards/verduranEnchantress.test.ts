// `Verduran Enchantress` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VERDURAN_ENCHANTRESS_SCRIPT } from './verduranEnchantress';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Verduran Enchantress";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Verduran Enchantress", "Grizzly Bears", "Nyxborn Courser"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VERDURAN_ENCHANTRESS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Verduran Enchantress", () => {
  test("Whenever you cast an enchantment spell: a card is drawn", () => {
