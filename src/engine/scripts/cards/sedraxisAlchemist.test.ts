// `Sedraxis Alchemist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEDRAXIS_ALCHEMIST_SCRIPT } from './sedraxisAlchemist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sedraxis Alchemist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sedraxis Alchemist", "Aegis Turtle"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEDRAXIS_ALCHEMIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sedraxis Alchemist", () => {
  test("When this creature enters: the vocabulary resolves \"Return target nonland permanent to its owner's hand.\"", () => {
    const { g, no } = armed(0);
