// `Savage Knuckleblade` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SAVAGE_KNUCKLEBLADE_SCRIPT } from './savageKnuckleblade';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Savage Knuckleblade";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SAVAGE_KNUCKLEBLADE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Savage Knuckleblade", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SAVAGE_KNUCKLEBLADE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Savage Knuckleblade", () => {
  test("{2}{G}: it gets +2/+2 until end of turn", () => {
    const { g, self, refused } = armed(0);
    expect(pt(g, self)).toEqual([6, 6]);
    expect(refused).toBe(true);
  });

  test("{2}{U}: it returns to its owner's hand", () => {
