// `Scuttling Sliver` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { SCUTTLING_SLIVER_SCRIPT } from './scuttlingSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scuttling Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Scuttling Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCUTTLING_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Scuttling Sliver", () => {
  test("grants {2}: Untap this creature. to Metallic Sliver, and to nothing outside the scope", () => {
