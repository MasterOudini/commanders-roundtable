// `Ring of Gix` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ring of Gix", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RING_OF_GIX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ring of Gix", () => {
  test("{1}, {T}: the vocabulary resolves \"Tap target artifact, creature, or land.\"", () => {
