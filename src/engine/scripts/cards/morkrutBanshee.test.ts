// `Morkrut Banshee` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MORKRUT_BANSHEE_SCRIPT } from './morkrutBanshee';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Morkrut Banshee";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Morkrut Banshee", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MORKRUT_BANSHEE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Morkrut Banshee", () => {
  test("Morbid — When this creature enters: the vocabulary resolves \"Target creature gets -4/-4 until end of turn.\"", () => {
    const { g, no } = armed(0);
