// `Flameblade Adept` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLAMEBLADE_ADEPT_SCRIPT } from './flamebladeAdept';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flameblade Adept";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FLAMEBLADE_ADEPT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flameblade Adept"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLAMEBLADE_ADEPT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Flameblade Adept", () => {
  test("Whenever you cycle or discard a card: it gets +1/+0 until end of turn", () => {
