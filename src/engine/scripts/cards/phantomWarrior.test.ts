// `Phantom Warrior` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHANTOM_WARRIOR_SCRIPT } from './phantomWarrior';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phantom Warrior";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phantom Warrior", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHANTOM_WARRIOR_SCRIPT]),
  });
  holdEverywhere(g);
