// `Viridian Shaman` — the ETB artifact destroy, with an indestructible artifact
// proving the check.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { VIRIDIAN_SHAMAN_SCRIPT } from './viridianShaman';
import { advanceUntil, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = 'Viridian Shaman';
const RING = 'Sol Ring';
...
function landed(victimName: string): { g: Game; victim: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([VIRIDIAN_SHAMAN_SCRIPT]),
  });
