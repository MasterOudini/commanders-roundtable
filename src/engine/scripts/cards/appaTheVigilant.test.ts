// `Appa, the Vigilant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { APPA_THE_VIGILANT_SCRIPT } from './appaTheVigilant';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Appa, the Vigilant";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([APPA_THE_VIGILANT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Appa, the Vigilant", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([APPA_THE_VIGILANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Appa, the Vigilant", () => {
  test("Whenever Appa or another Ally you control enters: its controller's creatures get +1/+1 and gain flying and vigilance until end of turn", () => {
