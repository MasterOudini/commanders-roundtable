// `Blighted Gorge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLIGHTED_GORGE_SCRIPT } from './blightedGorge';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blighted Gorge";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blighted Gorge"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLIGHTED_GORGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Blighted Gorge", () => {
  test("{4}{R}, {T}, Sacrifice this land: it deals 2 damage to the declared creature", () => {
