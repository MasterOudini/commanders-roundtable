// `Citadel Castellan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CITADEL_CASTELLAN_SCRIPT } from './citadelCastellan';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Citadel Castellan";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CITADEL_CASTELLAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Citadel Castellan"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CITADEL_CASTELLAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Citadel Castellan", () => {
  test("Renown 2 (When this creature deals combat damage to a player: dealing combat damage to a player, it gets 2 +1/+1 counters and becomes renowned", () => {
