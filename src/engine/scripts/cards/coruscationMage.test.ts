// `Coruscation Mage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CORUSCATION_MAGE_SCRIPT } from './coruscationMage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Coruscation Mage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Coruscation Mage", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CORUSCATION_MAGE_SCRIPT]),
