// `Cruel Administrator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRUEL_ADMINISTRATOR_SCRIPT } from './cruelAdministrator';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cruel Administrator";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CRUEL_ADMINISTRATOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cruel Administrator", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRUEL_ADMINISTRATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cruel Administrator", () => {
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"Create a 2/2 red Soldier creature token with firebending 1.\"", () => {
