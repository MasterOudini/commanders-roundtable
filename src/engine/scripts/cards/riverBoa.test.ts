// `River Boa` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIVER_BOA_SCRIPT } from './riverBoa';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "River Boa";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["River Boa", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIVER_BOA_SCRIPT]),
  });
  holdEverywhere(g);
