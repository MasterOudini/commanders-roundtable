// `Conclave Tribunal` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONCLAVE_TRIBUNAL_SCRIPT } from './conclaveTribunal';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Conclave Tribunal";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Conclave Tribunal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONCLAVE_TRIBUNAL_SCRIPT]),
  });
  holdEverywhere(g);
