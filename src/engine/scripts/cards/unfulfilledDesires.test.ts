// `Unfulfilled Desires` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNFULFILLED_DESIRES_SCRIPT } from './unfulfilledDesires';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unfulfilled Desires";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Unfulfilled Desires"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNFULFILLED_DESIRES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Unfulfilled Desires", () => {
  test("{1}, Pay 1 life: a card is drawn, then one discarded", () => {
    const { g, life0, hand0, gy0 } = armed(0);
