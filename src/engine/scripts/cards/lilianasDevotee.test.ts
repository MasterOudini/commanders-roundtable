// `Liliana's Devotee` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LILIANAS_DEVOTEE_SCRIPT } from './lilianasDevotee';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Liliana's Devotee";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; yes: InstanceId; payL1_p1_0: InstanceId; payL1_p1_1: InstanceId };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LILIANAS_DEVOTEE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Liliana's Devotee", "Scathe Zombies", "Swamp", "Forest", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LILIANAS_DEVOTEE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, yes, payL1_p1_0, payL1_p1_1 };
}

describe("Liliana's Devotee", () => {
  test("Zombies you control get +1/+0.: only the Zombie reads it", () => {
    const { g, no, yes } = armed(0);
    expect(pt(g, yes)).toEqual([3, 2]);
    expect(pt(g, no)).toEqual([5, 2]);
  });

  test("At the beginning of your end step: the vocabulary resolves \"You may pay {1}{B}. If you do, create a 2/2 black Zombie creature token.\" - the price is declined", () => {
  ...
  test("At the beginning of your end step: the vocabulary resolves \"You may pay {1}{B}. If you do, create a 2/2 black Zombie creature token.\" - the price is paid", () => {
    const { g, board0, payL1_p1_0, payL1_p1_1 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
