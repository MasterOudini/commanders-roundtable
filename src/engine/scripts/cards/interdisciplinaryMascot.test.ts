// `Interdisciplinary Mascot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INTERDISCIPLINARY_MASCOT_SCRIPT } from './interdisciplinaryMascot';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Interdisciplinary Mascot";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Interdisciplinary Mascot"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INTERDISCIPLINARY_MASCOT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Interdisciplinary Mascot", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top four cards of your library. Put one of them into your hand and the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
