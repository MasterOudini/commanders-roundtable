// `Super-Skrull` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUPER_SKRULL_SCRIPT } from './superSkrull';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Super-Skrull";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUPER_SKRULL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Super-Skrull"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUPER_SKRULL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Super-Skrull", () => {
  test("{2}{W}: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("{3}{G}: it gets +4/+4 until end of turn", () => {
    const { g, self } = armed(1);
    expect(pt(g, self)).toEqual([8, 9]);
  });

  test("{4}{R}: it deals 4 damage to the declared creature", () => {
    const { g, no } = armed(2);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
  });

  test("{5}{U}: the vocabulary resolves \"Target player draws four cards.\"", () => {
    const { g, hand0, p2hand0 } = armed(3);
