// `Riptide Pilferer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIPTIDE_PILFERER_SCRIPT } from './riptidePilferer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Riptide Pilferer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Riptide Pilferer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIPTIDE_PILFERER_SCRIPT]),
  });
  holdEverywhere(g);
