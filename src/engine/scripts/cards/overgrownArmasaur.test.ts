// `Overgrown Armasaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OVERGROWN_ARMASAUR_SCRIPT } from './overgrownArmasaur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Overgrown Armasaur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Overgrown Armasaur", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([OVERGROWN_ARMASAUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Overgrown Armasaur", () => {
  test("Enrage — Whenever this creature is dealt damage: 1 token made", () => {
