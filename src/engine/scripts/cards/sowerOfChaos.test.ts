// `Sower of Chaos` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOWER_OF_CHAOS_SCRIPT } from './sowerOfChaos';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sower of Chaos";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sower of Chaos"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOWER_OF_CHAOS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sower of Chaos", () => {
  test("{2}{R}: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
    const { g, no } = armed(0);
