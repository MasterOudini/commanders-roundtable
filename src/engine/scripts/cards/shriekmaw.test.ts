// `Shriekmaw` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHRIEKMAW_SCRIPT } from './shriekmaw';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shriekmaw";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shriekmaw"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHRIEKMAW_SCRIPT]),
