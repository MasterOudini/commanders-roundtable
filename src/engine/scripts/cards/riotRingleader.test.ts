// `Riot Ringleader` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIOT_RINGLEADER_SCRIPT } from './riotRingleader';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Riot Ringleader";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RIOT_RINGLEADER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Riot Ringleader", "Jasmine Boreal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIOT_RINGLEADER_SCRIPT]),
...
describe("Riot Ringleader", () => {
  test("Whenever this creature attacks: Human creatures you control get +1/+0 until end of turn", () => {
