// `Web` - cast on Grizzly Bears it attaches and the enchanted creature gets +0/+2 and has reach;
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WEB_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([WEB_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Web", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +0/+2 and has reach; Cyclops of One-Eyed Pass is untouched", () => {
