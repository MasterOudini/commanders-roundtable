// `Merrow Wavebreakers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERROW_WAVEBREAKERS_SCRIPT } from './merrowWavebreakers';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Merrow Wavebreakers";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MERROW_WAVEBREAKERS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merrow Wavebreakers"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERROW_WAVEBREAKERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Merrow Wavebreakers", () => {
  test("{1}{U}, {Q}: it gains flying until end of turn", () => {
