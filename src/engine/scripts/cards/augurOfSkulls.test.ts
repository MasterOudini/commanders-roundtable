// `Augur of Skulls` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AUGUR_OF_SKULLS_SCRIPT } from './augurOfSkulls';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Augur of Skulls";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Augur of Skulls", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AUGUR_OF_SKULLS_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 1) {
    // D349 - the opponent's second turn: not your turn, not your upkeep - refused for the condition alone.
...
describe("Augur of Skulls", () => {
  ...
  test("Sacrifice this creature: the vocabulary resolves \"Target player discards two cards.\" - refused unless during your upkeep", () => {
