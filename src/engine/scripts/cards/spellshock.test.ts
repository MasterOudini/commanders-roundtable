// `Spellshock` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPELLSHOCK_SCRIPT } from './spellshock';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spellshock";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spellshock", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPELLSHOCK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spellshock", () => {
  test("Whenever a player casts a spell: the vocabulary resolves \"This enchantment deals 2 damage to target player.\"", () => {
