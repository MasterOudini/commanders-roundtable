// `Furious Strength` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2 and has menace;
...
import { FURIOUS_STRENGTH_SCRIPT } from './furiousStrength';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Furious Strength";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FURIOUS_STRENGTH_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FURIOUS_STRENGTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Furious Strength", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+2 and has menace; Cyclops of One-Eyed Pass is untouched", () => {
