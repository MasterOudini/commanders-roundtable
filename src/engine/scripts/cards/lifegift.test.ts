// `Lifegift` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIFEGIFT_SCRIPT } from './lifegift';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lifegift", "Grizzly Bears", "Badlands"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIFEGIFT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lifegift", () => {
  test("Whenever a land enters: 1 life is gained", () => {
