// `Illusory Demon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ILLUSORY_DEMON_SCRIPT } from './illusoryDemon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Illusory Demon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Illusory Demon", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ILLUSORY_DEMON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Illusory Demon", () => {
  test("When you cast a spell: it is sacrificed", () => {
