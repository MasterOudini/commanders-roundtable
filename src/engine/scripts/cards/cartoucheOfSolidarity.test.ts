// `Cartouche of Solidarity` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: 1 token); gets +1/+1 and has firstStrike;
// the other creature is untouched; the wrong host is refused; the host dying drops the Aura
// (CR 704.5m); the replay hash (D304). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { CARTOUCHE_OF_SOLIDARITY_SCRIPT } from './cartoucheOfSolidarity';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cartouche of Solidarity";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CARTOUCHE_OF_SOLIDARITY_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([CARTOUCHE_OF_SOLIDARITY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cartouche of Solidarity", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: 1 token); gets +1/+1 and has firstStrike; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    const board0 = onBoard(g);
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([3, 3]);
  ...
  test("Cyclops of One-Eyed Pass is no legal host (Enchant creature you control)", () => {
    const { g, self, other } = board();
    must(g.submit({ t: 'ManualAddMana', player: 'p1', target: 'p1', symbol: 'W', amount: 1 }));
    expect(g.submit({ t: 'CastSpell', player: 'p1', card: self, targets: [{ kind: 'card', id: other }] }).ok).toBe(false);
  });
