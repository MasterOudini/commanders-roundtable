// `Sadistic Obsession` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { SADISTIC_OBSESSION_SCRIPT } from './sadisticObsession';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sadistic Obsession";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Sadistic Obsession","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SADISTIC_OBSESSION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sadistic Obsession", () => {
  test("grants {B}, {T}: Put a -1/-1 counter on target creature. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and runs the payload', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{B}, {T}");
