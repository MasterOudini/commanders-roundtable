// `Salvage Drone` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SALVAGE_DRONE_SCRIPT } from './salvageDrone';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Salvage Drone";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Salvage Drone"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SALVAGE_DRONE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Salvage Drone", () => {
  test("When this creature dies: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(0);
