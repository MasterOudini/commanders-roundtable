// `Hellkite Overlord` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HELLKITE_OVERLORD_SCRIPT } from './hellkiteOverlord';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hellkite Overlord";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HELLKITE_OVERLORD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hellkite Overlord", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HELLKITE_OVERLORD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hellkite Overlord", () => {
  test("{R}: it gets +1/+0 until end of turn", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([9, 8]);
  });

  test("{B}{G}: it regenerates from the Bolt", () => {
