// `Argothian Treefolk` - the continuous prevention effect proven both ways (D385): the damage the line
// covers never arrives and nothing is spent; damage outside the line lands. Generated from one row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { ARGOTHIAN_TREEFOLK_SCRIPT } from './argothianTreefolk';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Argothian Treefolk";

type Armed = { g: Game; self: InstanceId; no: InstanceId; bears: InstanceId; life0: number; p2life0: number; sliver: InstanceId };
...
function armed(): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Argothian Treefolk", "Grizzly Bears", "Spark Spray"], ["Cyclops of One-Eyed Pass", "Metallic Sliver"]],
    scripts: createRegistry([ARGOTHIAN_TREEFOLK_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, bears, life0, p2life0, sliver };
}

describe("Argothian Treefolk", () => {
  test("combat damage from the artifact attacker is prevented, and nothing is spent", () => {
    const a = armed();
    advanceUntil(a.g, (s) => s.turn.turnNumber === 4 && s.priority.awaiting?.kind === 'declareAttackers', 40_000);
    must(a.g.submit({ t: 'DeclareAttackers', player: 'p2', attackers: [{ card: a.sliver, defender: { kind: 'player', id: 'p1' } }] }));
    advanceUntil(a.g, (s) => s.priority.awaiting?.kind === 'declareBlockers', 20_000);
    must(a.g.submit({ t: 'DeclareBlockers', player: 'p1', blocks: [{ blocker: a.self, attacker: a.sliver }] }));
