// `Ice Cream Kitty` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ICE_CREAM_KITTY_SCRIPT } from './iceCreamKitty';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ice Cream Kitty";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ice Cream Kitty", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ICE_CREAM_KITTY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ice Cream Kitty", () => {
  test("{2}, Sacrifice another creature or token: a card is drawn", () => {
