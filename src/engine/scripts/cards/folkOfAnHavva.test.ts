// `Folk of An-Havva` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FOLK_OF_AN_HAVVA_SCRIPT } from './folkOfAnHavva';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Folk of An-Havva";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FOLK_OF_AN_HAVVA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Folk of An-Havva"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOLK_OF_AN_HAVVA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Folk of An-Havva", () => {
  test("Whenever this creature blocks: it gets +2/+0 until end of turn", () => {
