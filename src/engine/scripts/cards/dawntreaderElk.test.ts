// `Dawntreader Elk` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dawntreader Elk", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAWNTREADER_ELK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dawntreader Elk", () => {
  test("{G}, Sacrifice this creature: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
