// `Bloodflow Connoisseur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODFLOW_CONNOISSEUR_SCRIPT } from './bloodflowConnoisseur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodflow Connoisseur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodflow Connoisseur", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODFLOW_CONNOISSEUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bloodflow Connoisseur", () => {
