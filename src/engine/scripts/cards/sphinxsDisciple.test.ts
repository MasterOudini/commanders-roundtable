// `Sphinx's Disciple` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPHINXS_DISCIPLE_SCRIPT } from './sphinxsDisciple';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sphinx's Disciple";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sphinx's Disciple"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPHINXS_DISCIPLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sphinx's Disciple", () => {
  test("Inspired — Whenever this creature becomes untapped: a card is drawn", () => {
