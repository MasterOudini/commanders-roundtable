// `Skithiryx, the Blight Dragon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKITHIRYX_THE_BLIGHT_DRAGON_SCRIPT } from './skithiryxTheBlightDragon';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skithiryx, the Blight Dragon";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SKITHIRYX_THE_BLIGHT_DRAGON_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skithiryx, the Blight Dragon", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKITHIRYX_THE_BLIGHT_DRAGON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skithiryx, the Blight Dragon", () => {
  test("{B}: it gains haste until end of turn", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([4, 4]);
    expect(kw(g, self).has("haste")).toBe(true);
  });

  test("{B}{B}: it regenerates from the Bolt", () => {
