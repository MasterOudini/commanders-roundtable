// `Burdened Aerialist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BURDENED_AERIALIST_SCRIPT } from './burdenedAerialist';
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([BURDENED_AERIALIST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Burdened Aerialist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BURDENED_AERIALIST_SCRIPT]),
  });
  holdEverywhere(g);
