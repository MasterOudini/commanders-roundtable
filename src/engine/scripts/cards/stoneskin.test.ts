// `Stoneskin` - cast on Grizzly Bears it attaches and the enchanted creature gets +0/+10;
...
import { STONESKIN_SCRIPT } from './stoneskin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stoneskin";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STONESKIN_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([STONESKIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stoneskin", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +0/+10; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([2, 12]);
