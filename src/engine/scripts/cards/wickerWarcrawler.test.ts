// `Wicker Warcrawler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WICKER_WARCRAWLER_SCRIPT } from './wickerWarcrawler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wicker Warcrawler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wicker Warcrawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WICKER_WARCRAWLER_SCRIPT]),
...
describe("Wicker Warcrawler", () => {
  test("Whenever this creature attacks or blocks: the vocabulary resolves \"Put a -1/-1 counter on ~ at end of combat.\" [attacks]", () => {
    ...
    expect(g.state.delayedTriggers.length, 'the payload is armed, not run').toBe(1);
    {
      ...
      settle(g);
      expect(g.state.cards[self]?.counters["-1/-1"] ?? 0, 'the counter came at the step').toBe(1);
    }
    expect(g.state.delayedTriggers.length, 'every entry fired').toBe(0);
  });

  test("Whenever this creature attacks or blocks: the vocabulary resolves \"Put a -1/-1 counter on ~ at end of combat.\" [blocks]", () => {
    ...
    { const boardD = onBoard(g);
      ...
      expect(g.state.cards[self]?.zone.kind, 'the counter came at the step and, with the combat damage, killed').toBe('graveyard');
    }
    expect(g.state.delayedTriggers.length, 'every entry fired').toBe(0);
  });
