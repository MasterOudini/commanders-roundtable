// `Mossdog` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOSSDOG_SCRIPT } from './mossdog';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mossdog"], ["Cyclops of One-Eyed Pass", "Giant Growth"]],
    scripts: createRegistry([MOSSDOG_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mossdog", () => {
  test("Whenever this creature becomes the target of a spell or ability an opponent controls: it gets 1 +1/+1 counter", () => {
