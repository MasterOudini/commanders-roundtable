// `Flicker` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { FLICKER_SCRIPT } from './flicker';
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Flicker"], ["Grizzly Bears", "Grizzly Bears"]],
    scripts: createRegistry([FLICKER_SCRIPT]),
  });
  holdEverywhere(g);
  const targets: InstanceId[] = [];
  targets.push(put(g, 'p2', "Grizzly Bears"));
  const wrong = put(g, 'p2', "Grizzly Bears", 'graveyard');
...
describe("Flicker", () => {
  ...
  test("Grizzly Bears (in the graveyard) is refused where the clause excludes it (D294)", () => {
