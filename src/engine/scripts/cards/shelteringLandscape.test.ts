// `Sheltering Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sheltering Landscape", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHELTERING_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sheltering Landscape", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Mountain, Forest, or Plains card, put it onto the battlefield tapped, then shuffle.\"", () => {
