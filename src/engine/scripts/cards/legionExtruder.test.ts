// `Legion Extruder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LEGION_EXTRUDER_SCRIPT } from './legionExtruder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Legion Extruder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Legion Extruder", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LEGION_EXTRUDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Legion Extruder", () => {
  test("When this artifact enters: it deals 2 damage to the declared creature", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
  });

  test("{2}, {T}, Sacrifice another artifact: 1 token made", () => {
