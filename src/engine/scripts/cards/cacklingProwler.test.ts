// `Cackling Prowler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CACKLING_PROWLER_SCRIPT } from './cacklingProwler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cackling Prowler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cackling Prowler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CACKLING_PROWLER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cackling Prowler", () => {
  test("Morbid — At the beginning of your end step: it gets 1 +1/+1 counter", () => {
