// `Goblin General` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_GENERAL_SCRIPT } from './goblinGeneral';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin General";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOBLIN_GENERAL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin General", "Mons's Goblin Raiders"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_GENERAL_SCRIPT]),
...
describe("Goblin General", () => {
  test("Whenever this creature attacks: Goblin creatures you control get +1/+1 until end of turn", () => {
