// `Bone Saw` - equipped to Grizzly Bears the creature gets +1/+0; the Cyclops is untouched;
...
import { BONE_SAW_SCRIPT } from './boneSaw';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BONE_SAW_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([BONE_SAW_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bone Saw", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+0; the Cyclops is untouched", () => {
