// `Ishai, Ojutai Dragonspeaker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ISHAI_OJUTAI_DRAGONSPEAKER_SCRIPT } from './ishaiOjutaiDragonspeaker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ishai, Ojutai Dragonspeaker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ishai, Ojutai Dragonspeaker"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([ISHAI_OJUTAI_DRAGONSPEAKER_SCRIPT]),
...
describe("Ishai, Ojutai Dragonspeaker", () => {
  test("Whenever an opponent casts a spell: it gets 1 +1/+1 counter", () => {
