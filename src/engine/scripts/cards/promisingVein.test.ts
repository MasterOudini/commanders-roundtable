// `Promising Vein` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Promising Vein", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PROMISING_VEIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Promising Vein", () => {
  test("{1}, {T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
