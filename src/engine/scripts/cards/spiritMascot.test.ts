// `Spirit Mascot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIRIT_MASCOT_SCRIPT } from './spiritMascot';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spirit Mascot";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spirit Mascot", "Badlands"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPIRIT_MASCOT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spirit Mascot", () => {
  test("Whenever one or more cards leave your graveyard: it gets 1 +1/+1 counter", () => {
