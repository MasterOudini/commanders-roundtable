// `Gaea's Courser` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GAEAS_COURSER_SCRIPT } from './gaeasCourser';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gaea's Courser";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gaea's Courser", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GAEAS_COURSER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gaea's Courser", () => {
  test("Whenever this creature attacks: a card is drawn", () => {
