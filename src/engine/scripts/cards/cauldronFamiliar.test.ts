// `Cauldron Familiar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAULDRON_FAMILIAR_SCRIPT } from './cauldronFamiliar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';
import { TOKEN_TABLE } from '../../../data/tokenTable';

const CARD = "Cauldron Familiar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cauldron Familiar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAULDRON_FAMILIAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cauldron Familiar", () => {
  test("When this creature enters: each opponent loses 1 life and 1 is gained", () => {
    const { g, life0, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 - 1);
    expect(g.state.players.p1?.life).toBe(life0 + 1);
  });

  test("Sacrifice a Food: it returns from the graveyard to the battlefield", () => {
