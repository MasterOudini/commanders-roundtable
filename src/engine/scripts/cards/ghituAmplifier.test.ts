// `Ghitu Amplifier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHITU_AMPLIFIER_SCRIPT } from './ghituAmplifier';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghitu Amplifier";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GHITU_AMPLIFIER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghitu Amplifier", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHITU_AMPLIFIER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ghitu Amplifier", () => {
  test("When this creature enters: the vocabulary resolves \"Return target creature an opponent controls to its owner's hand.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.zone.kind).toBe('hand');
  });

  test("Whenever you cast an instant or sorcery spell: it gets +2/+0 until end of turn", () => {
