// `Abomination of Gudul` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ABOMINATION_OF_GUDUL_SCRIPT } from './abominationOfGudul';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Abomination of Gudul";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Abomination of Gudul"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ABOMINATION_OF_GUDUL_SCRIPT]),
  });
  holdEverywhere(g);
