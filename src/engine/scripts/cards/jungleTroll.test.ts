// `Jungle Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JUNGLE_TROLL_SCRIPT } from './jungleTroll';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jungle Troll";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jungle Troll", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JUNGLE_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Jungle Troll", () => {
  test("{R}: it regenerates from the Bolt", () => {
