// `Rhox Meditant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RHOX_MEDITANT_SCRIPT } from './rhoxMeditant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rhox Meditant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rhox Meditant", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RHOX_MEDITANT_SCRIPT]),
  });
  holdEverywhere(g);
