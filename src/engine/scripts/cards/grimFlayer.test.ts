// `Grim Flayer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRIM_FLAYER_SCRIPT } from './grimFlayer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grim Flayer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GRIM_FLAYER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grim Flayer", "Grizzly Bears", "Forest", "Sol Ring", "Hissing Miasma"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRIM_FLAYER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Grim Flayer", () => {
  test("Whenever this creature deals combat damage to a player: surveil 3 asks, and the cards stay on top", () => {
    const { g, lib0 } = armed(0);
    expect(g.state.priority.awaiting).toBeNull();
    expect((g.state.zones.library.p1 ?? []).length).toBe(lib0);
  });

  test("`Grim Flayer` - as long as there are four or more card types among cards in your graveyard: absent while it is unmet, read once it holds", () => {
