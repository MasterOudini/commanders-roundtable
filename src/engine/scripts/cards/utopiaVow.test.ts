// `Utopia Vow` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UTOPIA_VOW_SCRIPT } from './utopiaVow';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Utopia Vow";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Utopia Vow", "Grizzly Bears", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UTOPIA_VOW_SCRIPT]),
...
describe("Utopia Vow", () => {
  ...
  test("Enchanted creature has \"{T}: the creature it is attached to reads it", () => {
    ...
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    { const d = deps(createRegistry([UTOPIA_VOW_SCRIPT])); const prod = derive(g.state, d.oracle, d.scripts, bearsB).producesMana.find((p) => p.text === "{T}: Add one mana of any color."); expect(prod, 'the host has the granted mana ability').toBeDefined(); const pool0 = poolTotal(g); must(g.submit({ t: 'TapForMana', player: 'p1', card: bearsB, abilityIndex: prod?.abilityIndex ?? -1, outputChoice: 0 })); expect(g.state.cards[bearsB]?.tapped).toBe(true); expect(poolTotal(g)).toBeGreaterThan(pool0); }
  });
