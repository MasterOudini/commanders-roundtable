// `Eternal Dragon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ETERNAL_DRAGON_SCRIPT } from './eternalDragon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Eternal Dragon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Eternal Dragon"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ETERNAL_DRAGON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Eternal Dragon", () => {
  test("{3}{W}{W}: it returns from the graveyard to hand - refused unless during your upkeep", () => {
