// `Medicine Bag` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MEDICINE_BAG_SCRIPT } from './medicineBag';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Medicine Bag";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Medicine Bag", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MEDICINE_BAG_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Medicine Bag", () => {
  test("{1}, {T}, Discard a card: the declared creature regenerates from the Bolt", () => {
