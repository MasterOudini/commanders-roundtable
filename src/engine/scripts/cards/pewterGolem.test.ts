// `Pewter Golem` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PEWTER_GOLEM_SCRIPT } from './pewterGolem';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pewter Golem";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pewter Golem", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PEWTER_GOLEM_SCRIPT]),
  });
  holdEverywhere(g);
