// `Drake Hatchling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRAKE_HATCHLING_SCRIPT } from './drakeHatchling';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drake Hatchling";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DRAKE_HATCHLING_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drake Hatchling", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRAKE_HATCHLING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Drake Hatchling", () => {
  test("{U}: it gets +1/+0 until end of turn", () => {
