// `Crafty Pathmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRAFTY_PATHMAGE_SCRIPT } from './craftyPathmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crafty Pathmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crafty Pathmage"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([CRAFTY_PATHMAGE_SCRIPT]),
  });
  holdEverywhere(g);
