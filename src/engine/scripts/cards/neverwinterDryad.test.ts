// `Neverwinter Dryad` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Neverwinter Dryad", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NEVERWINTER_DRYAD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Neverwinter Dryad", () => {
  test("{2}, Sacrifice this creature: the vocabulary resolves \"Search your library for a basic Forest card, put it onto the battlefield tapped, then shuffle.\"", () => {
