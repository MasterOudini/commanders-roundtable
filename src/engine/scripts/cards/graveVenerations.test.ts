// `Grave Venerations` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRAVE_VENERATIONS_SCRIPT } from './graveVenerations';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grave Venerations";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grave Venerations", "Grizzly Bears", "Walking Corpse"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRAVE_VENERATIONS_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtL1_endStep_0 = put(g, 'p1', "Walking Corpse", 'graveyard');
  settle(g);
...
describe("Grave Venerations", () => {
  ...
  test("At the beginning of your end step: the vocabulary resolves \"Return up to one target creature card from your graveyard to your hand.\"", () => {
    const { g, hand0, vtL1_endStep_0 } = armed(1);
    expect(g.state.cards[vtL1_endStep_0]?.zone.kind).toBe('hand');
