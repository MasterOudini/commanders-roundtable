// `Krark-Clan Shaman` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KRARK_CLAN_SHAMAN_SCRIPT } from './krarkClanShaman';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Krark-Clan Shaman";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Krark-Clan Shaman", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KRARK_CLAN_SHAMAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Krark-Clan Shaman", () => {
  test("Sacrifice an artifact: the vocabulary resolves \"~ deals 1 damage to each creature without flying.\"", () => {
