// `Lawbringer` - the named target is accepted and it is exiled; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { LAWBRINGER_SCRIPT } from './lawbringer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lawbringer";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Lawbringer"], ["Raging Goblin", "Grizzly Bears"]],
    scripts: createRegistry([LAWBRINGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lawbringer", () => {
  test("Raging Goblin is a legal target and it is exiled", () => {
