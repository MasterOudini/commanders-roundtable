// `Whipstitched Zombie` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WHIPSTITCHED_ZOMBIE_SCRIPT } from './whipstitchedZombie';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Whipstitched Zombie";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Whipstitched Zombie", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WHIPSTITCHED_ZOMBIE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Whipstitched Zombie", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you pay {B}.\" - the price is declined", () => {
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you pay {B}.\" - the price is paid", () => {
