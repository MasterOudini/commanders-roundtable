// `Bleak Coven Vampires` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLEAK_COVEN_VAMPIRES_SCRIPT } from './bleakCovenVampires';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bleak Coven Vampires";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bleak Coven Vampires", "Sol Ring", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLEAK_COVEN_VAMPIRES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bleak Coven Vampires", () => {
  test("Metalcraft — When this creature enters: the vocabulary resolves \"Target player loses 4 life and you gain 4 life.\"", () => {
