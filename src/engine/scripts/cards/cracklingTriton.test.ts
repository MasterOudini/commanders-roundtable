// `Crackling Triton` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRACKLING_TRITON_SCRIPT } from './cracklingTriton';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crackling Triton";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crackling Triton"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRACKLING_TRITON_SCRIPT]),
  });
  holdEverywhere(g);
