// `Mire Shade` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MIRE_SHADE_SCRIPT } from './mireShade';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mire Shade";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mire Shade", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIRE_SHADE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mire Shade", () => {
  test("{B}, Sacrifice a Swamp: it gets 1 +1/+1 counter", () => {
