// `Lion Heart` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LION_HEART_SCRIPT } from './lionHeart';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lion Heart";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LION_HEART_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lion Heart", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LION_HEART_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lion Heart", () => {
  test("When this Equipment enters: it deals 2 damage to the declared creature", () => {
    const { g, no } = armed(0);
