// `Hollow Dogs` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOLLOW_DOGS_SCRIPT } from './hollowDogs';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hollow Dogs";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HOLLOW_DOGS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hollow Dogs"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOLLOW_DOGS_SCRIPT]),
  });
  holdEverywhere(g);
