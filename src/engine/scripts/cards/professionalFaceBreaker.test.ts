// `Professional Face-Breaker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PROFESSIONAL_FACE_BREAKER_SCRIPT } from './professionalFaceBreaker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';
import { TOKEN_TABLE } from '../../../data/tokenTable';

const CARD = "Professional Face-Breaker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Professional Face-Breaker", "Memnite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PROFESSIONAL_FACE_BREAKER_SCRIPT]),
...
describe("Professional Face-Breaker", () => {
  ...
  test("Sacrifice a Treasure: the vocabulary resolves \"Exile the top card of your library. You may play that card this turn.\"", () => {
    const { g, fodder0 } = armed(1);
