// `Tocasia's Welcome` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tocasia's Welcome", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TOCASIAS_WELCOME_SCRIPT]),
...
describe("Tocasia's Welcome", () => {
  test("Whenever one or more creatures you control with mana value 3 or less enter: a card is drawn", () => {
    const { g, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1);
  });

  test('the once-per-turn rider rides the def (D492)', () => {
    expect(TOCASIAS_WELCOME_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("creatureEnters-0") && t.oncePerTurn === true), 'the def of line 0 carries oncePerTurn').toBeDefined();
  });
