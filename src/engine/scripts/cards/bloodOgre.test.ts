// `Blood Ogre` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOOD_OGRE_SCRIPT } from './bloodOgre';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blood Ogre";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLOOD_OGRE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blood Ogre", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOOD_OGRE_SCRIPT]),
  });
  holdEverywhere(g);
