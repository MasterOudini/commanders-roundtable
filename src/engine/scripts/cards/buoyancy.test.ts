// `Buoyancy` - cast on Grizzly Bears it attaches and the enchanted creature has flying;
...
import { BUOYANCY_SCRIPT } from './buoyancy';
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([BUOYANCY_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([BUOYANCY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Buoyancy", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has flying; Cyclops of One-Eyed Pass is untouched", () => {
