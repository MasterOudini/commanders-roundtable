// `Svyelunite Priest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SVYELUNITE_PRIEST_SCRIPT } from './svyelunitePriest';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Svyelunite Priest";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SVYELUNITE_PRIEST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Svyelunite Priest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SVYELUNITE_PRIEST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Svyelunite Priest", () => {
  test("{U}{U}, {T}: the declared creature is pumped until end of turn - refused unless during your upkeep", () => {
    const { g, self, no, condRefused } = armed(0);
    expect(pt(g, no)).toEqual([5, 2]);
    expect(kw(g, no).has("shroud")).toBe(true);
