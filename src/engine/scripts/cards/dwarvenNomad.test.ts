// `Dwarven Nomad` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DWARVEN_NOMAD_SCRIPT } from './dwarvenNomad';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dwarven Nomad";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dwarven Nomad"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([DWARVEN_NOMAD_SCRIPT]),
  });
  holdEverywhere(g);
