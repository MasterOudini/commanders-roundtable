// `Jeskai Elder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JESKAI_ELDER_SCRIPT } from './jeskaiElder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jeskai Elder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jeskai Elder"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JESKAI_ELDER_SCRIPT]),
  });
  holdEverywhere(g);
