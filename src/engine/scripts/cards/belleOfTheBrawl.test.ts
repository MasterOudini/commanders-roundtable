// `Belle of the Brawl` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BELLE_OF_THE_BRAWL_SCRIPT } from './belleOfTheBrawl';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Belle of the Brawl";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BELLE_OF_THE_BRAWL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Belle of the Brawl", "Kithkin Billyrider"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BELLE_OF_THE_BRAWL_SCRIPT]),
...
describe("Belle of the Brawl", () => {
  test("Whenever this creature attacks: other Knight creatures you control get +1/+0 until end of turn", () => {
