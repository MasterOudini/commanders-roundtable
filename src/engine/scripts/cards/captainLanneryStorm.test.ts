// `Captain Lannery Storm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAPTAIN_LANNERY_STORM_SCRIPT } from './captainLanneryStorm';
...
const CARD = "Captain Lannery Storm";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([CAPTAIN_LANNERY_STORM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Captain Lannery Storm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAPTAIN_LANNERY_STORM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Captain Lannery Storm", () => {
  test("Whenever Captain Lannery Storm attacks: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("Whenever you sacrifice a Treasure: it gets +1/+0 until end of turn", () => {
