// `Blaster Mage` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { BLASTER_MAGE_SCRIPT } from './blasterMage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blaster Mage";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number; handCard: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Blaster Mage", "Grizzly Bears"], ["Wall of Omens", "Grizzly Bears"]],
    scripts: createRegistry([BLASTER_MAGE_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const target = put(g, 'p2', "Wall of Omens");
...
describe("Blaster Mage", () => {
  test("Wall of Omens is a legal target and it is destroyed", () => {
