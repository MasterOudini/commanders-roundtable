// `Fallen Askari` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FALLEN_ASKARI_SCRIPT } from './fallenAskari';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fallen Askari";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fallen Askari", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FALLEN_ASKARI_SCRIPT]),
  });
  holdEverywhere(g);
