// `Altar of the Brood` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Altar of the Brood", "Grizzly Bears", "Badlands"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ALTAR_OF_THE_BROOD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Altar of the Brood", () => {
  test("Whenever another permanent you control enters: the vocabulary resolves \"Each opponent mills a card.\"", () => {
