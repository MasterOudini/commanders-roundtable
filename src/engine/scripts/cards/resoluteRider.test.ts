// `Resolute Rider` - the printed cost buys the pump until end of turn; it ends at cleanup;
// replay equal (D301). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { RESOLUTE_RIDER_SCRIPT } from './resoluteRider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Resolute Rider";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RESOLUTE_RIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Resolute Rider"], ["Grizzly Bears"]],
    scripts: createRegistry([RESOLUTE_RIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Resolute Rider", () => {
  test("{W/B}{W/B}: it gains lifelink until end of turn", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([4, 2]);
    expect(kw(g, self).has("lifelink")).toBe(true);
  });

  test("{W/B}{W/B}{W/B}: it gains indestructible until end of turn", () => {
