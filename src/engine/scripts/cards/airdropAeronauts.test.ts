// `Airdrop Aeronauts` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AIRDROP_AERONAUTS_SCRIPT } from './airdropAeronauts';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Airdrop Aeronauts";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Airdrop Aeronauts", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AIRDROP_AERONAUTS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Airdrop Aeronauts", () => {
  test("Revolt — When this creature enters: 5 life is gained", () => {
