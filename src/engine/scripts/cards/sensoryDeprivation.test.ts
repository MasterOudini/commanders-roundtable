// `Sensory Deprivation` - cast on Grizzly Bears it attaches and the enchanted creature gets -3/+0;
...
import { SENSORY_DEPRIVATION_SCRIPT } from './sensoryDeprivation';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sensory Deprivation";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SENSORY_DEPRIVATION_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SENSORY_DEPRIVATION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sensory Deprivation", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets -3/+0; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([-1, 2]);
