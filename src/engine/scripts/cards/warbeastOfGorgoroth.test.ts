// `Warbeast of Gorgoroth` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WARBEAST_OF_GORGOROTH_SCRIPT } from './warbeastOfGorgoroth';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Warbeast of Gorgoroth";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Warbeast of Gorgoroth", "Grizzly Bears", "Tyrox, Saurid Tyrant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WARBEAST_OF_GORGOROTH_SCRIPT]),
  ...
  if (which === 1) {
    const bears = put(g, 'p1', "Tyrox, Saurid Tyrant");
...
describe("Warbeast of Gorgoroth", () => {
  test("Whenever this creature or another creature you control with power 4 or greater dies: the vocabulary resolves \"Amass Orcs 2.\" [dies]", () => {
  ...
  test("Whenever this creature or another creature you control with power 4 or greater dies: the vocabulary resolves \"Amass Orcs 2.\" [anotherCreatureDies]", () => {
