// `Chosen by Heliod` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: draw 1); gets +0/+2;
...
import { CHOSEN_BY_HELIOD_SCRIPT } from './chosenByHeliod';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chosen by Heliod";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHOSEN_BY_HELIOD_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([CHOSEN_BY_HELIOD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chosen by Heliod", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: draw 1); gets +0/+2; Cyclops of One-Eyed Pass is untouched", () => {
