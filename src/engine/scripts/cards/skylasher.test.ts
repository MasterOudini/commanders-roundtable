// `Skylasher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKYLASHER_SCRIPT } from './skylasher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skylasher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skylasher"], ["Cyclops of One-Eyed Pass", "Counterspell"]],
    scripts: createRegistry([SKYLASHER_SCRIPT]),
  });
  holdEverywhere(g);
