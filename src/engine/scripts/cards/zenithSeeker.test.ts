// `Zenith Seeker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZENITH_SEEKER_SCRIPT } from './zenithSeeker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zenith Seeker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ZENITH_SEEKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zenith Seeker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZENITH_SEEKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zenith Seeker", () => {
  test("Whenever you cycle or discard a card: the vocabulary resolves \"Target creature gains flying until end of turn.\"", () => {
    const { g, no } = armed(0);
