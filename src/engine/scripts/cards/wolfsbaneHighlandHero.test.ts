// `Wolfsbane, Highland Hero` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WOLFSBANE_HIGHLAND_HERO_SCRIPT } from './wolfsbaneHighlandHero';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wolfsbane, Highland Hero";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WOLFSBANE_HIGHLAND_HERO_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wolfsbane, Highland Hero", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WOLFSBANE_HIGHLAND_HERO_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wolfsbane, Highland Hero", () => {
