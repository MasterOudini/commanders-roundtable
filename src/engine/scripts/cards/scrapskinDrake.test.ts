// `Scrapskin Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCRAPSKIN_DRAKE_SCRIPT } from './scrapskinDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scrapskin Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scrapskin Drake", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCRAPSKIN_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
