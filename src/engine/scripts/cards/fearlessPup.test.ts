// `Fearless Pup` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FEARLESS_PUP_SCRIPT } from './fearlessPup';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fearless Pup";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FEARLESS_PUP_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fearless Pup", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FEARLESS_PUP_SCRIPT]),
...
describe("Fearless Pup", () => {
  test("Boast — {2}{R}: it gets +2/+0 until end of turn - refused unless this creature attacked this turn", () => {
    const { g, self, refused, condRefused } = armed(0);
