// `Eyes Everywhere` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EYES_EVERYWHERE_SCRIPT } from './eyesEverywhere';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Eyes Everywhere";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Eyes Everywhere"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EYES_EVERYWHERE_SCRIPT]),
...
describe("Eyes Everywhere", () => {
  test("At the beginning of your upkeep: scry 1 asks, and the cards stay on top", () => {
    const { g, lib0 } = armed(0);
    expect(g.state.priority.awaiting).toBeNull();
    expect((g.state.zones.library.p1 ?? []).length).toBe(lib0);
  });

  test("{5}{U}: the vocabulary resolves \"Exchange control of this enchantment and target nonland permanent.\"", () => {
    const { g, self, no } = armed(1);
    expect(g.state.cards[self]?.controller, 'exchanged').toBe('p2');
