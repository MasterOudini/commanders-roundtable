// `Faerie Snoop` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FAERIE_SNOOP_SCRIPT } from './faerieSnoop';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Faerie Snoop";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Faerie Snoop"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FAERIE_SNOOP_SCRIPT]),
...
describe("Faerie Snoop", () => {
  test("When this creature is turned face up: the vocabulary resolves \"Look at the top two cards of your library. Put one into your hand and the other into your graveyard.\"", () => {
    const { g } = armed(0);
