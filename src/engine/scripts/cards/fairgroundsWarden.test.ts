// `Fairgrounds Warden` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FAIRGROUNDS_WARDEN_SCRIPT } from './fairgroundsWarden';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fairgrounds Warden";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fairgrounds Warden"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FAIRGROUNDS_WARDEN_SCRIPT]),
  });
  holdEverywhere(g);
