// `Sai, Master Thopterist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SAI_MASTER_THOPTERIST_SCRIPT } from './saiMasterThopterist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sai, Master Thopterist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sai, Master Thopterist", "Sol Ring", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SAI_MASTER_THOPTERIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sai, Master Thopterist", () => {
  test("Whenever you cast an artifact spell: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("{1}{U}, Sacrifice two artifacts: a card is drawn", () => {
