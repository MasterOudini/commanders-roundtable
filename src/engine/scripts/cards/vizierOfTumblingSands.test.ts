// `Vizier of Tumbling Sands` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VIZIER_OF_TUMBLING_SANDS_SCRIPT } from './vizierOfTumblingSands';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vizier of Tumbling Sands";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([VIZIER_OF_TUMBLING_SANDS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vizier of Tumbling Sands"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIZIER_OF_TUMBLING_SANDS_SCRIPT]),
...
describe("Vizier of Tumbling Sands", () => {
  test("{T}: the vocabulary resolves \"Untap another target permanent.\"", () => {
    const { g, self, no } = armed(0);
    expect(g.state.cards[no]?.tapped).toBe(false);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("When you cycle this card: the vocabulary resolves \"Untap target permanent.\"", () => {
    const { g, no } = armed(1);
    expect(g.state.cards[no]?.tapped).toBe(false);
  });
