// `Friendly Teddy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FRIENDLY_TEDDY_SCRIPT } from './friendlyTeddy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Friendly Teddy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Friendly Teddy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FRIENDLY_TEDDY_SCRIPT]),
  });
  holdEverywhere(g);
