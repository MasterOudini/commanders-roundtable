// `Vengeful Dead` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VENGEFUL_DEAD_SCRIPT } from './vengefulDead';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vengeful Dead";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vengeful Dead", "Grizzly Bears", "Gutter Skulk"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VENGEFUL_DEAD_SCRIPT]),
...
describe("Vengeful Dead", () => {
  test("Whenever this creature or another Zombie dies: each opponent loses 1 life [dies]", () => {
    const { g, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 - 1);
  });

  test("Whenever this creature or another Zombie dies: each opponent loses 1 life [anotherCreatureDies]", () => {
