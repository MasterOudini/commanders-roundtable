// `Predation Steward` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PREDATION_STEWARD_SCRIPT } from './predationSteward';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Predation Steward";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PREDATION_STEWARD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Predation Steward"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PREDATION_STEWARD_SCRIPT]),
...
describe("Predation Steward", () => {
  ...
  test("{2}{G}, {T}, Remove an oil counter from this creature: the declared creature is pumped until end of turn", () => {
