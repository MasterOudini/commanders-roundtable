// `Loam Lion` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LOAM_LION_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Loam Lion", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOAM_LION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Loam Lion", () => {
  test("`Loam Lion` - as long as you control a Forest: absent while it is unmet, read once it holds", () => {
