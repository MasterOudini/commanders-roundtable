// `Borrowed Time` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BORROWED_TIME_SCRIPT } from './borrowedTime';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Borrowed Time";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Borrowed Time"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BORROWED_TIME_SCRIPT]),
  });
  holdEverywhere(g);
