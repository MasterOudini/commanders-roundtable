// `Zulaport Chainmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZULAPORT_CHAINMAGE_SCRIPT } from './zulaportChainmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zulaport Chainmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zulaport Chainmage", "Expedition Envoy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZULAPORT_CHAINMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zulaport Chainmage", () => {
  test("Cohort — {T}, Tap an untapped Ally you control: the vocabulary resolves \"Target opponent loses 2 life.\"", () => {
    const { g, self, p2life0, tapper0 } = armed(0);
