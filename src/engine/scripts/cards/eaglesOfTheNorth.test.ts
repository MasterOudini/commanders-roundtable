// `Eagles of the North` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EAGLES_OF_THE_NORTH_SCRIPT } from './eaglesOfTheNorth';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Eagles of the North";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([EAGLES_OF_THE_NORTH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Eagles of the North", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EAGLES_OF_THE_NORTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Eagles of the North", () => {
  test("When this creature enters: its controller's creatures get +1/+0 and gain firstStrike until end of turn", () => {
