// `Greater Tanuki` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Greater Tanuki", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GREATER_TANUKI_SCRIPT]),
...
describe("Greater Tanuki", () => {
  test("Channel — {2}{G}, Discard this card: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
