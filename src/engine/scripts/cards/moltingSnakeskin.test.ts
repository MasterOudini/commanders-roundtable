// `Molting Snakeskin` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { MOLTING_SNAKESKIN_SCRIPT } from './moltingSnakeskin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Molting Snakeskin";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Molting Snakeskin","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOLTING_SNAKESKIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Molting Snakeskin", () => {
  test("grants {2}{B}: Regenerate this creature. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and the body hits the RECIPIENT', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{2}{B}");
