// `Ali Baba` - the named target is accepted and it is tapped; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ALI_BABA_SCRIPT } from './aliBaba';
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Ali Baba"], ["Wall of Omens", "Grizzly Bears"]],
    scripts: createRegistry([ALI_BABA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ali Baba", () => {
  test("Wall of Omens is a legal target and it is tapped", () => {
