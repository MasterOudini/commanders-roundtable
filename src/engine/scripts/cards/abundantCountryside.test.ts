// `Abundant Countryside` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ABUNDANT_COUNTRYSIDE_SCRIPT } from './abundantCountryside';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Abundant Countryside";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Abundant Countryside"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ABUNDANT_COUNTRYSIDE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Abundant Countryside", () => {
  test("{6}, {T}: 1 token made", () => {
