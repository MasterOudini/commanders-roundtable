// `Heliod's Pilgrim` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Heliod's Pilgrim", "Aboshan's Desire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HELIODS_PILGRIM_SCRIPT]),
  });
  holdEverywhere(g);
