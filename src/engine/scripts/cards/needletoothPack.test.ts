// `Needletooth Pack` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NEEDLETOOTH_PACK_SCRIPT } from './needletoothPack';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Needletooth Pack";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; vtL0_endStep_0: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Needletooth Pack", "Cyclops of One-Eyed Pass", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NEEDLETOOTH_PACK_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, vtL0_endStep_0 };
}

describe("Needletooth Pack", () => {
  test("Morbid — At the beginning of your end step: the vocabulary resolves \"Put two +1/+1 counters on target creature you control.\"", () => {
    const { g, vtL0_endStep_0 } = armed(0);
    expect(g.state.cards[vtL0_endStep_0]?.counters["+1/+1"] ?? 0).toBe(2);
  });
