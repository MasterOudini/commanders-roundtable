// `Griffin Aerie` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRIFFIN_AERIE_SCRIPT } from './griffinAerie';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Griffin Aerie";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Griffin Aerie"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRIFFIN_AERIE_SCRIPT]),
  });
  holdEverywhere(g);
