// `Banishing Light` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BANISHING_LIGHT_SCRIPT } from './banishingLight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Banishing Light";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Banishing Light"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BANISHING_LIGHT_SCRIPT]),
  });
  holdEverywhere(g);
