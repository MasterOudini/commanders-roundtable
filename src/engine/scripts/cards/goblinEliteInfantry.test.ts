// `Goblin Elite Infantry` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_ELITE_INFANTRY_SCRIPT } from './goblinEliteInfantry';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Elite Infantry";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOBLIN_ELITE_INFANTRY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Elite Infantry"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_ELITE_INFANTRY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Elite Infantry", () => {
  test("Whenever this creature blocks or becomes blocked: it gets -1/-1 until end of turn", () => {
