// `Deathknell Kami` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEATHKNELL_KAMI_SCRIPT } from './deathknellKami';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deathknell Kami";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deathknell Kami"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEATHKNELL_KAMI_SCRIPT]),
...
describe("Deathknell Kami", () => {
  test("{2}: the vocabulary resolves \"~ gets +1/+1 until end of turn. Sacrifice it at the beginning of the next end step.\"", () => {
    ...
    { const boardD = onBoard(g);
      ...
      expect(g.state.cards[self]?.zone.kind, "sacrificeObj at the step").toBe("graveyard");
    }
