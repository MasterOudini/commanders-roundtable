// `Putrid Pals` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PUTRID_PALS_SCRIPT } from './putridPals';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Putrid Pals";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PUTRID_PALS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Putrid Pals", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PUTRID_PALS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Putrid Pals", () => {
  test("Disappear — This creature enters with two +1/+1 counters on it if a permanent left the battlefield under your control this turn.: cast, it enters with 2 +1/+1 counters", () => {
