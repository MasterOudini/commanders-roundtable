// `Urborg Skeleton` - every printed ability proven in its own game: the cost's mark, the pump
...
import { URBORG_SKELETON_SCRIPT } from './urborgSkeleton';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Urborg Skeleton";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([URBORG_SKELETON_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Urborg Skeleton", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([URBORG_SKELETON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Urborg Skeleton", () => {
  ...
  test("If this creature was kicked, it enters with a +1/+1 counter on it.: cast, it enters with 1 +1/+1 counter", () => {
