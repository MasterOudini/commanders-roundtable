// `Cactarantula` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CACTARANTULA_SCRIPT } from './cactarantula';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cactarantula";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cactarantula"], ["Cyclops of One-Eyed Pass", "Giant Growth"]],
    scripts: createRegistry([CACTARANTULA_SCRIPT]),
  });
  holdEverywhere(g);
