// `Stormbind` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORMBIND_SCRIPT } from './stormbind';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stormbind";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stormbind"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORMBIND_SCRIPT]),
  });
  holdEverywhere(g);
