// `Facevaulter` - the printed cost buys the pump until end of turn; it ends at cleanup;
// replay equal (D301). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { FACEVAULTER_SCRIPT } from './facevaulter';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Facevaulter";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FACEVAULTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Facevaulter", "Raging Goblin"], ["Grizzly Bears"]],
    scripts: createRegistry([FACEVAULTER_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const fodder = put(g, 'p1', "Raging Goblin");
  settle(g);
...
describe("Facevaulter", () => {
  test("{B}, Sacrifice a Goblin: it gets +2/+2 until end of turn", () => {
