// `Titanium Man` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TITANIUM_MAN_SCRIPT } from './titaniumMan';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Titanium Man";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TITANIUM_MAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Titanium Man"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TITANIUM_MAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Titanium Man", () => {
  test("Whenever Titanium Man attacks: choosing \"Titanium Man gains flying until end of turn.\", self [attacks 1]", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([3, 4]);
    expect(kw(g, self).has("flying")).toBe(true);
  });

  test("Whenever Titanium Man attacks: choosing \"Titanium Man deals 1 damage to any target.\", damageTarget [attacks 2]", () => {
    const { g, no } = armed(1);
