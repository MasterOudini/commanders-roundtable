// `Suture Spirit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUTURE_SPIRIT_SCRIPT } from './sutureSpirit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Suture Spirit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Suture Spirit", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUTURE_SPIRIT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Suture Spirit", () => {
  test("{W/B}{W/B}{W/B}: the declared creature regenerates from the Bolt", () => {
    const { g, no } = armed(0);
