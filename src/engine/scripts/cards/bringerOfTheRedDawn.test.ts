// `Bringer of the Red Dawn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRINGER_OF_THE_RED_DAWN_SCRIPT } from './bringerOfTheRedDawn';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bringer of the Red Dawn";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BRINGER_OF_THE_RED_DAWN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bringer of the Red Dawn"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRINGER_OF_THE_RED_DAWN_SCRIPT]),
...
describe("Bringer of the Red Dawn", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Untap target creature and gain control of it until end of turn. That creature gains haste until end of turn.\"", () => {
