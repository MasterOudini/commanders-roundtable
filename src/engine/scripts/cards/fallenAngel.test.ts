// `Fallen Angel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FALLEN_ANGEL_SCRIPT } from './fallenAngel';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fallen Angel";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FALLEN_ANGEL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fallen Angel", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FALLEN_ANGEL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fallen Angel", () => {
  test("Sacrifice a creature: it gets +2/+1 until end of turn", () => {
