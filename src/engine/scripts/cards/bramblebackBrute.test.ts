// `Brambleback Brute` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRAMBLEBACK_BRUTE_SCRIPT } from './bramblebackBrute';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brambleback Brute";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BRAMBLEBACK_BRUTE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brambleback Brute"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRAMBLEBACK_BRUTE_SCRIPT]),
...
describe("Brambleback Brute", () => {
  ...
  test("{1}{R}, Remove a counter from this creature: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
