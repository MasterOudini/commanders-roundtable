// `Perimeter Patrol` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PERIMETER_PATROL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Perimeter Patrol", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PERIMETER_PATROL_SCRIPT]),
  });
  holdEverywhere(g);
