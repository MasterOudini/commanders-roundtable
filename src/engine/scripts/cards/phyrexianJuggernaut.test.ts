// `Phyrexian Juggernaut` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHYREXIAN_JUGGERNAUT_SCRIPT } from './phyrexianJuggernaut';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phyrexian Juggernaut";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phyrexian Juggernaut", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHYREXIAN_JUGGERNAUT_SCRIPT]),
  });
  holdEverywhere(g);
