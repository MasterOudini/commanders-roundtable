// `Divine Transformation` - cast on Grizzly Bears it attaches and the enchanted creature gets +3/+3;
...
import { DIVINE_TRANSFORMATION_SCRIPT } from './divineTransformation';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Divine Transformation";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DIVINE_TRANSFORMATION_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([DIVINE_TRANSFORMATION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Divine Transformation", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +3/+3; Cyclops of One-Eyed Pass is untouched", () => {
