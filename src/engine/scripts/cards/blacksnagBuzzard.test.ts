// `Blacksnag Buzzard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLACKSNAG_BUZZARD_SCRIPT } from './blacksnagBuzzard';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blacksnag Buzzard";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLACKSNAG_BUZZARD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blacksnag Buzzard", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLACKSNAG_BUZZARD_SCRIPT]),
