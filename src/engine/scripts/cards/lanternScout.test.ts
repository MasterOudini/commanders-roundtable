// `Lantern Scout` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LANTERN_SCOUT_SCRIPT } from './lanternScout';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lantern Scout";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LANTERN_SCOUT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lantern Scout", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LANTERN_SCOUT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lantern Scout", () => {
  test("Rally — Whenever this creature or another Ally you control enters: its controller's creatures gain lifelink until end of turn", () => {
