// `Blood Host` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOOD_HOST_SCRIPT } from './bloodHost';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blood Host";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blood Host", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOOD_HOST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Blood Host", () => {
  test("{1}{B}, Sacrifice another creature: the vocabulary resolves \"Put a +1/+1 counter on this creature and you gain 2 life.\"", () => {
