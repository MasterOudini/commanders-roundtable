// `Undercover Crocodelf` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNDERCOVER_CROCODELF_SCRIPT } from './undercoverCrocodelf';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Undercover Crocodelf";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Undercover Crocodelf"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNDERCOVER_CROCODELF_SCRIPT]),
