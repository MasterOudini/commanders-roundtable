// `Renegade Wheelsmith` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RENEGADE_WHEELSMITH_SCRIPT } from './renegadeWheelsmith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Renegade Wheelsmith";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Renegade Wheelsmith"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RENEGADE_WHEELSMITH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Renegade Wheelsmith", () => {
  test("Whenever this creature becomes tapped: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
    const { g, no } = armed(0);
