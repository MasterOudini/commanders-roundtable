// `Bad Moon` - the anthem reaches the permanent its scope names and not the other;
// it ends when the source leaves; replay equal (D300). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { BAD_MOON_SCRIPT } from './badMoon';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BAD_MOON_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Bad Moon"], ["Walking Corpse", "Coral Eel"]],
    scripts: createRegistry([BAD_MOON_SCRIPT]),
  });
  holdEverywhere(g);
  const yes = put(g, 'p2', "Walking Corpse");
...
describe("Bad Moon", () => {
  test("Walking Corpse is reached, Coral Eel is not", () => {
