// `Experimental Armor` - equipped to Grizzly Bears the creature gets +1/+1 and has flying, haste; the Cyclops is untouched;
...
import { EXPERIMENTAL_ARMOR_SCRIPT } from './experimentalArmor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Experimental Armor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([EXPERIMENTAL_ARMOR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([EXPERIMENTAL_ARMOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Experimental Armor", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+1 and has flying, haste; the Cyclops is untouched", () => {
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("flying")).toBe(false);
