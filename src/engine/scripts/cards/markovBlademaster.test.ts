// `Markov Blademaster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARKOV_BLADEMASTER_SCRIPT } from './markovBlademaster';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Markov Blademaster";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Markov Blademaster"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARKOV_BLADEMASTER_SCRIPT]),
  });
  holdEverywhere(g);
