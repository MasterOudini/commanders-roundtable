// `Temple Bell` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEMPLE_BELL_SCRIPT } from './templeBell';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Temple Bell";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Temple Bell"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEMPLE_BELL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Temple Bell", () => {
  test("{T}: the vocabulary resolves \"Each player draws a card.\"", () => {
