// `Minister of Inquiries` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MINISTER_OF_INQUIRIES_SCRIPT } from './ministerOfInquiries';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Minister of Inquiries";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Minister of Inquiries"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MINISTER_OF_INQUIRIES_SCRIPT]),
...
describe("Minister of Inquiries", () => {
  ...
  test("{T}, Pay {E}: the vocabulary resolves \"Target player mills three cards.\"", () => {
    const { g, self, energy0, p2gy0 } = armed(1);
    expect(g.state.players.p1?.energy).toBe(energy0 + 0 - 1);
