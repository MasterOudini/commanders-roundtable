// `Count on Luck` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COUNT_ON_LUCK_SCRIPT } from './countOnLuck';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Count on Luck";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Count on Luck"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COUNT_ON_LUCK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Count on Luck", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Exile the top card of your library. You may play that card this turn.\"", () => {
    const { g } = armed(0);
