// `Dictate of Erebos` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DICTATE_OF_EREBOS_SCRIPT } from './dictateOfErebos';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dictate of Erebos";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dictate of Erebos", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DICTATE_OF_EREBOS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dictate of Erebos", () => {
  test("Whenever a creature you control dies: the vocabulary resolves \"Each opponent sacrifices a creature of their choice.\"", () => {
    const { g } = armed(0);
