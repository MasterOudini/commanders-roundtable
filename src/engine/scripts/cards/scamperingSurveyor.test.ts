// `Scampering Surveyor` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scampering Surveyor", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCAMPERING_SURVEYOR_SCRIPT]),
...
describe("Scampering Surveyor", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a basic land card or Cave card, put it onto the battlefield tapped, then shuffle.\"", () => {
