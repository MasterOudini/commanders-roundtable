// `Knights of Dol Amroth` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KNIGHTS_OF_DOL_AMROTH_SCRIPT } from './knightsOfDolAmroth';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Knights of Dol Amroth";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Knights of Dol Amroth", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KNIGHTS_OF_DOL_AMROTH_SCRIPT]),
  });
  holdEverywhere(g);
