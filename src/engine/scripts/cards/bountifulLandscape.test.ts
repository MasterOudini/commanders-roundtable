// `Bountiful Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bountiful Landscape", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOUNTIFUL_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bountiful Landscape", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Forest, Island, or Mountain card, put it onto the battlefield tapped, then shuffle.\"", () => {
