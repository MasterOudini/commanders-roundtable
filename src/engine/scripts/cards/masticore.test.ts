// `Masticore` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MASTICORE_SCRIPT } from './masticore';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Masticore";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Masticore", "Infernal Grasp", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MASTICORE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Masticore", () => {
  ...
  test("{2}: it deals 1 damage to the declared creature", () => {
    const { g, no } = armed(1);
