// `Grasping Dunes` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRASPING_DUNES_SCRIPT } from './graspingDunes';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grasping Dunes";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grasping Dunes"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRASPING_DUNES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Grasping Dunes", () => {
  test("{1}, {T}, Sacrifice this land: the declared creature gets 1 -1/-1 counter", () => {
