// `Faerie Swarm` - a static cdaCount
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { FAERIE_SWARM } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(FAERIE_SWARM, "Flying\nFaerie Swarm's power and toughness are each equal to the number of blue permanents you control.");
const LINES = PRINTED.split('\n');

// "blue permanents you control", read off the printed faces (a count is not a characteristic, CR 604.3).
...
export const FAERIE_SWARM_SCRIPT: CardScript = {
  oracleId: FAERIE_SWARM.oracleId,
  name: FAERIE_SWARM.name,
  statics: [
    {
      abilityId: 'cda-1',
