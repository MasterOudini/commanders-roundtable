// `Hazard of the Dunes` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HAZARD_OF_THE_DUNES_SCRIPT } from './hazardOfTheDunes';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hazard of the Dunes";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hazard of the Dunes", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HAZARD_OF_THE_DUNES_SCRIPT]),
...
describe("Hazard of the Dunes", () => {
  test("Exhaust — {6}{G}: it gets 3 +1/+1 counters", () => {
