// `Ballroom` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BALLROOM_SCRIPT } from './ballroom';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ballroom"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BALLROOM_SCRIPT]),
  });
  holdEverywhere(g);
