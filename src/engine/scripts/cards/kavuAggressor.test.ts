// `Kavu Aggressor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KAVU_AGGRESSOR_SCRIPT } from './kavuAggressor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kavu Aggressor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KAVU_AGGRESSOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kavu Aggressor", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KAVU_AGGRESSOR_SCRIPT]),
  });
  holdEverywhere(g);
