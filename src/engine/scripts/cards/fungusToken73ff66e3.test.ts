// `Fungus` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([FUNGUS_TOKEN73FF66E3_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: FUNGUS_73FF66E3_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === FUNGUS_73FF66E3_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
