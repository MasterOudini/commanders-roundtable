// `Laurine, the Diversion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LAURINE_THE_DIVERSION_SCRIPT } from './laurineTheDiversion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Laurine, the Diversion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Laurine, the Diversion", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LAURINE_THE_DIVERSION_SCRIPT]),
...
describe("Laurine, the Diversion", () => {
  test("{2}, Sacrifice an artifact or creature: the vocabulary resolves \"Goad target creature.\"", () => {
