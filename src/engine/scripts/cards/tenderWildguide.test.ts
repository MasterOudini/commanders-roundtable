// `Tender Wildguide` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TENDER_WILDGUIDE_SCRIPT } from './tenderWildguide';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tender Wildguide";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tender Wildguide"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TENDER_WILDGUIDE_SCRIPT]),
