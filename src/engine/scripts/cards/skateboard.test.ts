// `Skateboard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKATEBOARD_SCRIPT } from './skateboard';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skateboard";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SKATEBOARD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skateboard", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKATEBOARD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skateboard", () => {
  test("When this Equipment enters: the vocabulary resolves \"Tap target permanent.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.tapped).toBe(true);
  });

  test("Equipped creature gets +1/+0 and has haste.: the creature it is attached to reads it", () => {
