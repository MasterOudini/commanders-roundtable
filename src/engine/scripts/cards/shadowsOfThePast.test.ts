// `Shadows of the Past` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHADOWS_OF_THE_PAST_SCRIPT } from './shadowsOfThePast';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shadows of the Past";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shadows of the Past", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHADOWS_OF_THE_PAST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shadows of the Past", () => {
  test("Whenever a creature dies: scry 1 asks, and the cards stay on top", () => {
    const { g, lib0 } = armed(0);
    expect(g.state.priority.awaiting).toBeNull();
    expect((g.state.zones.library.p1 ?? []).length).toBe(lib0);
  });

  test("{4}{B}: each opponent loses 2 life and 2 is gained - refused unless if there are four or more creature cards in your graveyard", () => {
    const { g, life0, p2life0, condRefused } = armed(1);
