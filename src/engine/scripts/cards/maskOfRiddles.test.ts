// `Mask of Riddles` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MASK_OF_RIDDLES_SCRIPT } from './maskOfRiddles';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mask of Riddles";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([MASK_OF_RIDDLES_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mask of Riddles", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MASK_OF_RIDDLES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mask of Riddles", () => {
  test("Equipped creature has fear. (It can't be blocked except by artifact creatures and/or black creatures.): the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(kw(g, bearsB).has("fear")).toBe(true);
  });

  test("Whenever equipped creature deals combat damage to a player: a card is drawn", () => {
