// `Tempest Harvester` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEMPEST_HARVESTER_SCRIPT } from './tempestHarvester';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tempest Harvester";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tempest Harvester"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEMPEST_HARVESTER_SCRIPT]),
...
describe("Tempest Harvester", () => {
  ...
  test("{T}, Pay {E}: a card is drawn, then one discarded", () => {
    const { g, self, energy0, hand0, gy0 } = armed(1);
