// `Prophetic Ravings` - the permanent it is attached to HAS a quoted
...
import { PROPHETIC_RAVINGS } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(PROPHETIC_RAVINGS, "Enchant creature\nEnchanted creature has haste and \"{T}, Discard a card: Draw a card.\"");
const LINES = PRINTED.split('\n');

const GRANT = grantedActivated("{T}, Discard a card: Draw a card.", `${PROPHETIC_RAVINGS.oracleId}#g1`, PROPHETIC_RAVINGS.name);
const VOCAB = vocabularyEffects("Draw a card.", PROPHETIC_RAVINGS.name);
const VOCAB_T = vocabularyTargets("Draw a card.");

export const PROPHETIC_RAVINGS_SCRIPT: CardScript = {
  oracleId: PROPHETIC_RAVINGS.oracleId,
  name: PROPHETIC_RAVINGS.name,
  statics: [
    {
      abilityId: 'grant-1',
