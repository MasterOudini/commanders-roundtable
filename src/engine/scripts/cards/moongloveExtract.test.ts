// `Moonglove Extract` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOONGLOVE_EXTRACT_SCRIPT } from './moongloveExtract';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Moonglove Extract";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Moonglove Extract"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOONGLOVE_EXTRACT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Moonglove Extract", () => {
  test("Sacrifice this artifact: it deals 2 damage to the declared creature", () => {
