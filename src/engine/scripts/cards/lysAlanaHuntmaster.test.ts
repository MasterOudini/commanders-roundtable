// `Lys Alana Huntmaster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LYS_ALANA_HUNTMASTER_SCRIPT } from './lysAlanaHuntmaster';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lys Alana Huntmaster";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lys Alana Huntmaster", "Grizzly Bears", "Devoted Hero"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LYS_ALANA_HUNTMASTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lys Alana Huntmaster", () => {
  test("Whenever you cast an Elf spell: 1 token made", () => {
