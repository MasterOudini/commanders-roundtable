// `Chilling Apparition` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHILLING_APPARITION_SCRIPT } from './chillingApparition';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chilling Apparition";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chilling Apparition", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHILLING_APPARITION_SCRIPT]),
  });
  holdEverywhere(g);
