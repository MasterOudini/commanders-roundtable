// `Bonded Horncrest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BONDED_HORNCREST_SCRIPT } from './bondedHorncrest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bonded Horncrest";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bonded Horncrest", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BONDED_HORNCREST_SCRIPT]),
  });
  holdEverywhere(g);
