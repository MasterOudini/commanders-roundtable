// `Samut, Voice of Dissent` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SAMUT_VOICE_OF_DISSENT_SCRIPT } from './samutVoiceOfDissent';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Samut, Voice of Dissent";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SAMUT_VOICE_OF_DISSENT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Samut, Voice of Dissent", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SAMUT_VOICE_OF_DISSENT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Samut, Voice of Dissent", () => {
  test("Other creatures you control have haste.: its controller's other creatures read it", () => {
  ...
  test("{W}, {T}: the vocabulary resolves \"Untap another target creature.\"", () => {
