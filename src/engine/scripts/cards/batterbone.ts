// `Batterbone` - a etb trigger germ, a static attachedStatic
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { BATTERBONE, PHYREXIAN_GERM_TOKEN } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(BATTERBONE, "Living weapon (When this Equipment enters, create a 0/0 black Phyrexian Germ creature token, then attach this to it.)\nEquipped creature gets +1/+1 and has vigilance and lifelink.\nEquip {5}");
const LINES = PRINTED.split('\n');

export const BATTERBONE_SCRIPT: CardScript = {
  oracleId: BATTERBONE.oracleId,
  name: BATTERBONE.name,
  triggers: [
    {
      abilityId: 'etb-0',
