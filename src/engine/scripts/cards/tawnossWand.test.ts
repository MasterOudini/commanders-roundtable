// `Tawnos's Wand` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TAWNOSS_WAND_SCRIPT } from './tawnossWand';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tawnos's Wand";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tawnos's Wand"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([TAWNOSS_WAND_SCRIPT]),
  });
  holdEverywhere(g);
