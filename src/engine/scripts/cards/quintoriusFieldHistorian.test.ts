// `Quintorius, Field Historian` - every printed ability proven in its own game: the cost's mark, the pump
...
import { QUINTORIUS_FIELD_HISTORIAN_SCRIPT } from './quintoriusFieldHistorian';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Quintorius, Field Historian";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([QUINTORIUS_FIELD_HISTORIAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Quintorius, Field Historian", "Wandering Ones", "Badlands"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([QUINTORIUS_FIELD_HISTORIAN_SCRIPT]),
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Wandering Ones");
...
describe("Quintorius, Field Historian", () => {
  test("Spirits you control get +1/+0.: only the Spirit reads it", () => {
    const { g, no, yes } = armed(0);
    expect(pt(g, yes)).toEqual([2, 1]);
    expect(pt(g, no)).toEqual([5, 2]);
  });

  test("Whenever one or more cards leave your graveyard: 1 token made", () => {
