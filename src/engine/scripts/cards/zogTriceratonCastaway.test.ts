// `Zog, Triceraton Castaway` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZOG_TRICERATON_CASTAWAY_SCRIPT } from './zogTriceratonCastaway';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zog, Triceraton Castaway";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zog, Triceraton Castaway"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZOG_TRICERATON_CASTAWAY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zog, Triceraton Castaway", () => {
  test("When Zog enters: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
    const { g, no } = armed(0);
