// `Living Totem` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIVING_TOTEM_SCRIPT } from './livingTotem';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Living Totem";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Living Totem"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIVING_TOTEM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Living Totem", () => {
  test("When this creature enters: the vocabulary resolves \"Put a +1/+1 counter on another target creature.\"", () => {
    const { g, no } = armed(0);
