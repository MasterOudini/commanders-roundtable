// `Kulrath Zealot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KULRATH_ZEALOT_SCRIPT } from './kulrathZealot';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kulrath Zealot";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kulrath Zealot"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KULRATH_ZEALOT_SCRIPT]),
  });
  holdEverywhere(g);
