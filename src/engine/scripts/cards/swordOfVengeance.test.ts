// `Sword of Vengeance` - equipped to Grizzly Bears the creature gets +2/+0 and has firstStrike, vigilance, trample, haste; the Cyclops is untouched;
...
import { SWORD_OF_VENGEANCE_SCRIPT } from './swordOfVengeance';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sword of Vengeance";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SWORD_OF_VENGEANCE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SWORD_OF_VENGEANCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sword of Vengeance", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +2/+0 and has firstStrike, vigilance, trample, haste; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([4, 2]);
    expect(kw(g, host).has("firstStrike")).toBe(true);
    expect(kw(g, host).has("vigilance")).toBe(true);
    expect(kw(g, host).has("trample")).toBe(true);
    expect(kw(g, host).has("haste")).toBe(true);
    ...
    expect(kw(g, other).has("haste")).toBe(false);
  });
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("trample")).toBe(false);
    expect(kw(g, host).has("haste")).toBe(false);
  });
