// `Robot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROBOT_TOKEN319AAAAE_SCRIPT } from './robotToken319aaaae';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([ROBOT_TOKEN319AAAAE_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: ROBOT_319AAAAE_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === ROBOT_319AAAAE_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
