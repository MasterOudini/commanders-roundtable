// `Voldaren Stinger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VOLDAREN_STINGER_SCRIPT } from './voldarenStinger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Voldaren Stinger";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VOLDAREN_STINGER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Voldaren Stinger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VOLDAREN_STINGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Voldaren Stinger", () => {
  test("`Voldaren Stinger` - as long as it's attacking: absent while it is unmet, read once it holds", () => {
