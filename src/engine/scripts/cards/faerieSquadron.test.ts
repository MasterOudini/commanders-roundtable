// `Faerie Squadron` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FAERIE_SQUADRON_SCRIPT } from './faerieSquadron';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Faerie Squadron";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FAERIE_SQUADRON_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Faerie Squadron"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FAERIE_SQUADRON_SCRIPT]),
  ...
  if (which === 0) {
    ...
    if (kw(g, self).has("flying")) throw new Error('the kicked keyword held on an unkicked cast');
...
describe("Faerie Squadron", () => {
  test("If this creature was kicked, it enters with two +1/+1 counters on it and with flying.: cast, it enters with 2 +1/+1 counters", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.zone.kind).toBe('battlefield');
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(2);
    expect(kw(g, self).has("flying"), 'the kick granted flying').toBe(true);
