// `Short Bow` - equipped to Grizzly Bears the creature gets +1/+1 and has vigilance, reach; the Cyclops is untouched;
...
import { SHORT_BOW_SCRIPT } from './shortBow';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Short Bow";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHORT_BOW_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SHORT_BOW_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Short Bow", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+1 and has vigilance, reach; the Cyclops is untouched", () => {
