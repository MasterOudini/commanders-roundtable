// `Mardu Shadowspear` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARDU_SHADOWSPEAR_SCRIPT } from './marduShadowspear';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mardu Shadowspear";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mardu Shadowspear"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARDU_SHADOWSPEAR_SCRIPT]),
