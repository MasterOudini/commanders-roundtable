// `Fire Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIRE_DRAKE_SCRIPT } from './fireDrake';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fire Drake";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FIRE_DRAKE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fire Drake", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIRE_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
