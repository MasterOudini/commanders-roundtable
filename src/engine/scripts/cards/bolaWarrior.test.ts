// `Bola Warrior` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BOLA_WARRIOR_SCRIPT } from './bolaWarrior';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bola Warrior";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bola Warrior", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOLA_WARRIOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bola Warrior", () => {
  test("{R}, {T}, Discard a card: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
