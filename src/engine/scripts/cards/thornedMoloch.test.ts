// `Thorned Moloch` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THORNED_MOLOCH_SCRIPT } from './thornedMoloch';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thorned Moloch";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([THORNED_MOLOCH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thorned Moloch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THORNED_MOLOCH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thorned Moloch", () => {
  test("`Thorned Moloch` - as long as it's attacking: absent while it is unmet, read once it holds", () => {
