// `Aggravated Assault` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AGGRAVATED_ASSAULT_SCRIPT } from './aggravatedAssault';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aggravated Assault";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aggravated Assault", "Runeclaw Bear"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AGGRAVATED_ASSAULT_SCRIPT]),
...
describe("Aggravated Assault", () => {
  test("{3}{R}{R}: the vocabulary resolves \"Untap all creatures you control. After this main phase, there is an additional combat phase followed by an additional main phase.\"", () => {
    const { g, ownBear } = armed(0);
    expect(g.state.cards[ownBear]?.tapped, 'the witness on your side untapped').toBe(false);
    expect(g.log.some((e) => e.body.t === 'ExtraPhasesAdded' && JSON.stringify(e.body.phases) === "[\"combat\",\"main\"]" && e.body.after === 'main'), "the additional phases queued: [\"combat\",\"main\"]").toBe(true);
    /*EXTRA_COMBAT_WALK*/
    {
