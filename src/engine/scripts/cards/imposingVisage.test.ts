// `Imposing Visage` - cast on Grizzly Bears it attaches and the enchanted creature has menace;
...
import { IMPOSING_VISAGE_SCRIPT } from './imposingVisage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Imposing Visage";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([IMPOSING_VISAGE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([IMPOSING_VISAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Imposing Visage", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has menace; Cyclops of One-Eyed Pass is untouched", () => {
