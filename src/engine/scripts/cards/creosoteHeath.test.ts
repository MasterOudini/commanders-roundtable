// `Creosote Heath` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CREOSOTE_HEATH_SCRIPT } from './creosoteHeath';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Creosote Heath";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Creosote Heath"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CREOSOTE_HEATH_SCRIPT]),
  });
  holdEverywhere(g);
