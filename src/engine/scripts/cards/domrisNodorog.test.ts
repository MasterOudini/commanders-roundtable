// `Domri's Nodorog` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Domri's Nodorog", "Domri, City Smasher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DOMRIS_NODOROG_SCRIPT]),
...
describe("Domri's Nodorog", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library and/or graveyard for a card named Domri, City Smasher, reveal it, and put it into your hand. If you search your library this way, shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Domri, City Smasher" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Domri, City Smasher");
      expect(found, "Domri, City Smasher is not in the library for the search to find").toBeDefined();
