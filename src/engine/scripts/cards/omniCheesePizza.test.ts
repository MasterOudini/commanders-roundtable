// `Omni-Cheese Pizza` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OMNI_CHEESE_PIZZA_SCRIPT } from './omniCheesePizza';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Omni-Cheese Pizza";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Omni-Cheese Pizza"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OMNI_CHEESE_PIZZA_SCRIPT]),
  });
  holdEverywhere(g);
