// `Exorcist` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { EXORCIST_SCRIPT } from './exorcist';
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Exorcist"], ["Vampire Nighthawk", "Grizzly Bears"]],
    scripts: createRegistry([EXORCIST_SCRIPT]),
  });
  holdEverywhere(g);
