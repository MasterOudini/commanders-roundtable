// `Footfall Crater` - the permanent it is attached to HAS a quoted
...
import { FOOTFALL_CRATER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(FOOTFALL_CRATER, "Enchant land\nEnchanted land has \"{T}: Target creature gains trample and haste until end of turn.\"\nCycling {1} ({1}, Discard this card: Draw a card.)");
const LINES = PRINTED.split('\n');

const GRANT = grantedActivated("{T}: Target creature gains trample and haste until end of turn.", `${FOOTFALL_CRATER.oracleId}#g1`, FOOTFALL_CRATER.name);
const VOCAB = vocabularyEffects("Target creature gains trample and haste until end of turn.", FOOTFALL_CRATER.name);
const VOCAB_T = vocabularyTargets("Target creature gains trample and haste until end of turn.");

export const FOOTFALL_CRATER_SCRIPT: CardScript = {
  oracleId: FOOTFALL_CRATER.oracleId,
  name: FOOTFALL_CRATER.name,
  statics: [
    {
      abilityId: 'grant-1',
