// `Wanderbrine Rootcutters` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WANDERBRINE_ROOTCUTTERS_SCRIPT } from './wanderbrineRootcutters';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wanderbrine Rootcutters";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wanderbrine Rootcutters", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Norwood Ranger"]],
    scripts: createRegistry([WANDERBRINE_ROOTCUTTERS_SCRIPT]),
...
describe("Wanderbrine Rootcutters", () => {
  test("This creature can't be blocked by green creatures.: the block by green creatures is refused", () => {
