// `Thrun, the Last Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THRUN_THE_LAST_TROLL_SCRIPT } from './thrunTheLastTroll';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thrun, the Last Troll";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thrun, the Last Troll", "Infernal Grasp"], ["Cyclops of One-Eyed Pass", "Counterspell"]],
    scripts: createRegistry([THRUN_THE_LAST_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
