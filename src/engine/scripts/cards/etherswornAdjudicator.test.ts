// `Ethersworn Adjudicator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ETHERSWORN_ADJUDICATOR_SCRIPT } from './etherswornAdjudicator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ethersworn Adjudicator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ethersworn Adjudicator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ETHERSWORN_ADJUDICATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ethersworn Adjudicator", () => {
  test("{1}{W}{B}, {T}: the vocabulary resolves \"Destroy target creature or enchantment.\"", () => {
