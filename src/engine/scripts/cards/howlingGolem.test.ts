// `Howling Golem` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOWLING_GOLEM_SCRIPT } from './howlingGolem';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Howling Golem";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Howling Golem"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOWLING_GOLEM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Howling Golem", () => {
  test("Whenever this creature attacks or blocks: the vocabulary resolves \"Each player draws a card.\" [attacks]", () => {
  ...
  test("Whenever this creature attacks or blocks: the vocabulary resolves \"Each player draws a card.\" [blocks]", () => {
