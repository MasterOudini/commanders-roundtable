// `Mythic Proportions` - cast on Grizzly Bears it attaches and the enchanted creature gets +8/+8 and has trample;
...
import { MYTHIC_PROPORTIONS_SCRIPT } from './mythicProportions';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mythic Proportions";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MYTHIC_PROPORTIONS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([MYTHIC_PROPORTIONS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mythic Proportions", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +8/+8 and has trample; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([10, 10]);
