// `Environmental Scientist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ENVIRONMENTAL_SCIENTIST_SCRIPT } from './environmentalScientist';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Environmental Scientist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Environmental Scientist", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ENVIRONMENTAL_SCIENTIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Environmental Scientist", () => {
