// `Giant Ladybug` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Giant Ladybug", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GIANT_LADYBUG_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Giant Ladybug", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a basic land card, reveal it, then shuffle and put that card on top.\"", () => {
    const { g } = armed(0);
