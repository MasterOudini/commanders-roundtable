// `Howling Wolf` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Howling Wolf", "Howling Wolf"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOWLING_WOLF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Howling Wolf", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for up to three cards named ~, reveal them, put them into your hand, then shuffle.\"", () => {
    ...
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Howling Wolf" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Howling Wolf");
      expect(found, "Howling Wolf is not in the library for the search to find").toBeDefined();
