// `Squire's Devotion` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+1 and has lifelink; (on entering: 1 token);
...
import { SQUIRES_DEVOTION_SCRIPT } from './squiresDevotion';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Squire's Devotion";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SQUIRES_DEVOTION_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SQUIRES_DEVOTION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Squire's Devotion", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+1 and has lifelink; (on entering: 1 token); Cyclops of One-Eyed Pass is untouched", () => {
