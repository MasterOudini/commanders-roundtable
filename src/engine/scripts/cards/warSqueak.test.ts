// `War Squeak` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WAR_SQUEAK_SCRIPT } from './warSqueak';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "War Squeak";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WAR_SQUEAK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["War Squeak", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WAR_SQUEAK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("War Squeak", () => {
  test("When this Aura enters: the vocabulary resolves \"Target creature an opponent controls can't block this turn.\"", () => {
    const { g, no } = armed(0);
  ...
  test("Enchanted creature gets +1/+1 and has haste.: the creature it is attached to reads it", () => {
