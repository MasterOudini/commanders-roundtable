// `Dryad's Favor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRYADS_FAVOR_SCRIPT } from './dryadsFavor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dryad's Favor";
...
function lw(g: Game, id: InstanceId): readonly string[] {
  const d = deps(createRegistry([DRYADS_FAVOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dryad's Favor", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRYADS_FAVOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dryad's Favor", () => {
  test("Enchanted creature has forestwalk. (It can't be blocked as long as defending player controls a Forest.): the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(lw(g, bearsB)).toContain("Forest");
  });
