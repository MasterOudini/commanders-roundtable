// `Kraven's Cats` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KRAVENS_CATS_SCRIPT } from './kravensCats';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kraven's Cats";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KRAVENS_CATS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kraven's Cats", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KRAVENS_CATS_SCRIPT]),
  });
  holdEverywhere(g);
