// `Desperate Sentry` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DESPERATE_SENTRY_SCRIPT } from './desperateSentry';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Desperate Sentry";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DESPERATE_SENTRY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Desperate Sentry", "Grizzly Bears", "Forest", "Sol Ring", "Hissing Miasma"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DESPERATE_SENTRY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Desperate Sentry", () => {
  test("When this creature dies: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 - 1);
  });

  test("`Desperate Sentry` - as long as there are four or more card types among cards in your graveyard: absent while it is unmet, read once it holds", () => {
