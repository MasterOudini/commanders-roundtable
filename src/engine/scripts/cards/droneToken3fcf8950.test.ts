...
import { DRONE_TOKEN3FCF8950_SCRIPT } from './droneToken3fcf8950';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([DRONE_TOKEN3FCF8950_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: DRONE_3FCF8950_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === DRONE_3FCF8950_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
...
describe("Drone", () => {
  test("This token can block only creatures with flying.: the block is refused", () => {
