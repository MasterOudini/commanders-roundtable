// `Venerated Stormsinger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VENERATED_STORMSINGER_SCRIPT } from './veneratedStormsinger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Venerated Stormsinger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Venerated Stormsinger", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VENERATED_STORMSINGER_SCRIPT]),
...
describe("Venerated Stormsinger", () => {
  test("Whenever this creature or another creature you control dies: each opponent loses 1 life and 1 is gained [dies]", () => {
    const { g, life0, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 - 1);
    expect(g.state.players.p1?.life).toBe(life0 + 1);
  });

  test("Whenever this creature or another creature you control dies: each opponent loses 1 life and 1 is gained [anotherCreatureDies]", () => {
