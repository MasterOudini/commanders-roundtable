// `Binding Grasp` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BINDING_GRASP_SCRIPT } from './bindingGrasp';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Binding Grasp";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; bearsB: InstanceId; payL1_p1_0: InstanceId; payL1_p1_1: InstanceId };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BINDING_GRASP_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Binding Grasp", "Grizzly Bears", "Island", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BINDING_GRASP_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const payL1_p1_0 = put(g, 'p1', "Island");
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, bearsB, payL1_p1_0, payL1_p1_1 };
}

describe("Binding Grasp", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this Aura unless you pay {1}{U}.\" - the price is declined", () => {
    const { g, self, board0, payL1_p1_0, payL1_p1_1 } = armed(0);
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this Aura unless you pay {1}{U}.\" - the price is paid", () => {
    const { g, self, payL1_p1_0, payL1_p1_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
  ...
  test("Enchanted creature gets +0/+1.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([2, 3]);
  });
