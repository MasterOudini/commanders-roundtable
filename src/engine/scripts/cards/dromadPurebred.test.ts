// `Dromad Purebred` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DROMAD_PUREBRED_SCRIPT } from './dromadPurebred';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dromad Purebred";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dromad Purebred", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([DROMAD_PUREBRED_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dromad Purebred", () => {
  test("Whenever this creature is dealt damage: 1 life is gained", () => {
