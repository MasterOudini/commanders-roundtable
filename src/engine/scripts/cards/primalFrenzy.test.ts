// `Primal Frenzy` - cast on Grizzly Bears it attaches and the enchanted creature has trample;
...
import { PRIMAL_FRENZY_SCRIPT } from './primalFrenzy';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Primal Frenzy";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([PRIMAL_FRENZY_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([PRIMAL_FRENZY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Primal Frenzy", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has trample; Cyclops of One-Eyed Pass is untouched", () => {
