// `Vigilance` - cast on Grizzly Bears it attaches and the enchanted creature has vigilance;
...
import { VIGILANCE_SCRIPT } from './vigilance';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vigilance";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([VIGILANCE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([VIGILANCE_SCRIPT]),
  });
  holdEverywhere(g);
