// `Capricious Sorcerer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAPRICIOUS_SORCERER_SCRIPT } from './capriciousSorcerer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Capricious Sorcerer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Capricious Sorcerer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAPRICIOUS_SORCERER_SCRIPT]),
  });
  holdEverywhere(g);
