// `Prosperous Pirates` — two Treasures, two DISTINCT ids.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { PROSPEROUS_PIRATES_SCRIPT } from './prosperousPirates';
import { advanceUntil, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';

function settle(g: Game): void {
  advanceUntil(g, (s) => s.stack.length === 0 && s.pendingTriggers.length === 0, 20_000);
}

function docked(): Game {
  const g = startedGame({
    players: 2,
    decks: [['Prosperous Pirates'], []],
    scripts: createRegistry([PROSPEROUS_PIRATES_SCRIPT]),
  });
  put(g, 'p1', 'Prosperous Pirates');
  settle(g);
  return g;
}

describe('Prosperous Pirates', () => {
  test('entering mints two distinct Treasures', () => {
    const g = docked();
    const treasures = g.state.zones.battlefield.filter((id) => nameOf(g, id) === 'Treasure');
    expect(treasures).toHaveLength(2);
    expect(new Set(treasures).size).toBe(2);
  });

  test('replays to the same hash', () => {
    const g = docked();
    advanceUntil(g, (s) => s.turn.turnNumber >= 3, 20_000);
    expect(stateHash(replay(g.log, g.seed))).toBe(g.hash());
  });
});
