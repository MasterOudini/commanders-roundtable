// `Shambling Ghast` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHAMBLING_GHAST_SCRIPT } from './shamblingGhast';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shambling Ghast";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHAMBLING_GHAST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shambling Ghast"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHAMBLING_GHAST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shambling Ghast", () => {
  test("When this creature dies: choosing \"Target creature an opponent controls gets -1/-1 until end of turn.\", the vocabulary resolves \"Target creature an opponent controls gets -1/-1 until end of turn.\"", () => {
    const { g, no } = armed(0);
    expect(pt(g, no)).toEqual([4, 1]);
  });

  test("When this creature dies: choosing \"Create a Treasure token.\", token", () => {
