// `Stoneforge Mystic` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stoneforge Mystic", "Accorder's Shield", "Accorder's Shield", "Accorder's Shield"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STONEFORGE_MYSTIC_SCRIPT]),
  ...
  if (which === 1) {
    ...
    settle(g);
    { const inHand = (g.state.zones.hand.p1 ?? []).some((id) => nameOf(g, id) === "Accorder's Shield");
      const inLib = (g.state.zones.library.p1 ?? []).find((id) => nameOf(g, id) === "Accorder's Shield");
      if (!inHand) { expect(inLib, "Accorder's Shield is dealt for the hand put").toBeDefined(); must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: inLib as InstanceId, to: { kind: 'hand', player: 'p1' } })); } }
...
describe("Stoneforge Mystic", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for an Equipment card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Accorder's Shield" && g.state.cards[id as InstanceId]?.zone.kind === 'hand' && g.state.cards[id as InstanceId]?.zone.player === 'p1');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Accorder's Shield");
      expect(found, "Accorder's Shield is not in the library for the search to find").toBeDefined();
  ...
  test("{1}{W}, {T}: the vocabulary resolves \"You may put an Equipment card from your hand onto the battlefield.\"", () => {
    const { g, self, hand0, board0 } = armed(1);
    ...
    { const ask = g.state.priority.awaiting;
      ...
      const found = (g.state.zones.hand.p1 ?? []).find((id) => nameOf(g, id) === "Accorder's Shield");
      expect(found, "Accorder's Shield is in hand for the put").toBeDefined();
