// `Crazed Goblin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRAZED_GOBLIN_SCRIPT } from './crazedGoblin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crazed Goblin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crazed Goblin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRAZED_GOBLIN_SCRIPT]),
  });
  holdEverywhere(g);
