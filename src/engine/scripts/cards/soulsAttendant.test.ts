// `Soul's Attendant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOULS_ATTENDANT_SCRIPT } from './soulsAttendant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Soul's Attendant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soul's Attendant", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOULS_ATTENDANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Soul's Attendant", () => {
  test("Whenever another creature enters: 1 life is gained", () => {
