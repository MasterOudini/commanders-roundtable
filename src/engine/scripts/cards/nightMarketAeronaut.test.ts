// `Night Market Aeronaut` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIGHT_MARKET_AERONAUT_SCRIPT } from './nightMarketAeronaut';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Night Market Aeronaut";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NIGHT_MARKET_AERONAUT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Night Market Aeronaut", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIGHT_MARKET_AERONAUT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Night Market Aeronaut", () => {
  test("Revolt — This creature enters with a +1/+1 counter on it if a permanent left the battlefield under your control this turn.: cast, it enters with 1 +1/+1 counter", () => {
