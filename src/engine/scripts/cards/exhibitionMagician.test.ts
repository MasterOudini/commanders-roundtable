// `Exhibition Magician` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EXHIBITION_MAGICIAN_SCRIPT } from './exhibitionMagician';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Exhibition Magician";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Exhibition Magician"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EXHIBITION_MAGICIAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Exhibition Magician", () => {
  test("When this creature enters: choosing \"Create a 1/1 green and white Citizen creature token.\", token", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("When this creature enters: choosing \"Create a Treasure token.\", token", () => {
