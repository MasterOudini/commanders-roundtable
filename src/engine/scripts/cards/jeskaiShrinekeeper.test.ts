// `Jeskai Shrinekeeper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JESKAI_SHRINEKEEPER_SCRIPT } from './jeskaiShrinekeeper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jeskai Shrinekeeper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jeskai Shrinekeeper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JESKAI_SHRINEKEEPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Jeskai Shrinekeeper", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"You gain 1 life and draw a card.\"", () => {
