// `Infernal Idol` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INFERNAL_IDOL_SCRIPT } from './infernalIdol';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Infernal Idol";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Infernal Idol"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INFERNAL_IDOL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Infernal Idol", () => {
  test("{1}{B}{B}, {T}, Sacrifice this artifact: the vocabulary resolves \"You draw two cards and you lose 2 life.\"", () => {
