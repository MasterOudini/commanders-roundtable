// `Rivendell` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIVENDELL_SCRIPT } from './rivendell';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rivendell";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rivendell", "Ant-Man, Scott Lang"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIVENDELL_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  settle(g);
  const cond0_0_0 = put(g, 'p1', "Ant-Man, Scott Lang");
...
describe("Rivendell", () => {
  test("{1}{U}, {T}: scry 2 asks, and the cards stay on top - refused unless if you control a legendary creature", () => {
    const { g, self, lib0, condRefused } = armed(0);
