// `Dromosaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DROMOSAUR_SCRIPT } from './dromosaur';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dromosaur";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DROMOSAUR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dromosaur"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DROMOSAUR_SCRIPT]),
  });
  holdEverywhere(g);
