// `Glorious Anthem` - the anthem reaches the permanent its scope names and not the other;
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GLORIOUS_ANTHEM_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Glorious Anthem", "Coral Eel"], ["Crimson Kobolds"]],
    scripts: createRegistry([GLORIOUS_ANTHEM_SCRIPT]),
  });
  holdEverywhere(g);
