// `Unchained Berserker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNCHAINED_BERSERKER_SCRIPT } from './unchainedBerserker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unchained Berserker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([UNCHAINED_BERSERKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Unchained Berserker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNCHAINED_BERSERKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Unchained Berserker", () => {
  test("`Unchained Berserker` - as long as it's attacking: absent while it is unmet, read once it holds", () => {
