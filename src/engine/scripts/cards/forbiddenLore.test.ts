// `Forbidden Lore` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { FORBIDDEN_LORE_SCRIPT } from './forbiddenLore';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Forbidden Lore";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Forbidden Lore","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FORBIDDEN_LORE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Forbidden Lore", () => {
  test("grants {T}: Target creature gets +2/+1 until end of turn. to Forest, and to nothing outside the scope", () => {
