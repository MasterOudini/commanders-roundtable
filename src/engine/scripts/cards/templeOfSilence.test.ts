// `Temple of Silence` — both printed entry rules: tapped, and the scry that asks.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { TEMPLE_OF_SILENCE_SCRIPT } from './templeOfSilence';
import { advanceUntil, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const TEMPLE = 'Temple of Silence';
...
function entered(): { g: Game; temple: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [[TEMPLE], []],
    scripts: createRegistry([TEMPLE_OF_SILENCE_SCRIPT]),
  });
