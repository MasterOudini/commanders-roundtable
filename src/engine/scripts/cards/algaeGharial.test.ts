// `Algae Gharial` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ALGAE_GHARIAL_SCRIPT } from './algaeGharial';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Algae Gharial";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Algae Gharial"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ALGAE_GHARIAL_SCRIPT]),
  });
  holdEverywhere(g);
