// `Rural Recruit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RURAL_RECRUIT_SCRIPT } from './ruralRecruit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rural Recruit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rural Recruit"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RURAL_RECRUIT_SCRIPT]),
  });
  holdEverywhere(g);
