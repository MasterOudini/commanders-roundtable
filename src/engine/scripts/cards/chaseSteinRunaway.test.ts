// `Chase Stein, Runaway` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHASE_STEIN_RUNAWAY_SCRIPT } from './chaseSteinRunaway';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chase Stein, Runaway";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chase Stein, Runaway", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHASE_STEIN_RUNAWAY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chase Stein, Runaway", () => {
  test("{T}, Discard a card: the vocabulary resolves \"Exile the top card of your library. Until the end of your next turn, you may play that card.\"", () => {
