// `Spirit of the Night` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIRIT_OF_THE_NIGHT_SCRIPT } from './spiritOfTheNight';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spirit of the Night";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPIRIT_OF_THE_NIGHT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spirit of the Night"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPIRIT_OF_THE_NIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spirit of the Night", () => {
  test("`Spirit of the Night` - as long as it's attacking: absent while it is unmet, read once it holds", () => {
