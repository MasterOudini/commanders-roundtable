// `Necrotic Sliver` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { NECROTIC_SLIVER_SCRIPT } from './necroticSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Necrotic Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Necrotic Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NECROTIC_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Necrotic Sliver", () => {
  test("grants {3}, Sacrifice this permanent: Destroy target permanent. to Metallic Sliver, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and runs the payload', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{3}, Sacrifice this permanent");
