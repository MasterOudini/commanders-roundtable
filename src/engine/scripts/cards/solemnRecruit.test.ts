// `Solemn Recruit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOLEMN_RECRUIT_SCRIPT } from './solemnRecruit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Solemn Recruit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Solemn Recruit", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOLEMN_RECRUIT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Solemn Recruit", () => {
  test("Revolt — At the beginning of your end step: it gets 1 +1/+1 counter", () => {
