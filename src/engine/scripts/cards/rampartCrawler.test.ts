// `Rampart Crawler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAMPART_CRAWLER_SCRIPT } from './rampartCrawler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rampart Crawler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rampart Crawler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Steel Wall"]],
    scripts: createRegistry([RAMPART_CRAWLER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const gift2 = put(g, 'p2', "Steel Wall");
  settle(g);
...
describe("Rampart Crawler", () => {
  test("This creature can't be blocked by Walls.: the block by Walls is refused", () => {
