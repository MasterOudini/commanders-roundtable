// `Murder of Crows` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MURDER_OF_CROWS_SCRIPT } from './murderOfCrows';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Murder of Crows";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Murder of Crows"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MURDER_OF_CROWS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Murder of Crows", () => {
  test("Whenever another creature dies: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(0);
