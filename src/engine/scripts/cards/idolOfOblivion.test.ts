// `Idol of Oblivion` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Idol of Oblivion"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IDOL_OF_OBLIVION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Idol of Oblivion", () => {
  test("{T}: a card is drawn - refused unless if you created a token this turn", () => {
  ...
  test("{8}, {T}, Sacrifice this artifact: 1 token made", () => {
