// `Auratog` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AURATOG_SCRIPT } from './auratog';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([AURATOG_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Auratog", "Hissing Miasma"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AURATOG_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Auratog", () => {
  test("Sacrifice an enchantment: it gets +2/+2 until end of turn", () => {
