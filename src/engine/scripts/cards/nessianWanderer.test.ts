// `Nessian Wanderer` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nessian Wanderer", "Grizzly Bears", "Nyxborn Courser", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NESSIAN_WANDERER_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const lookPicksL0_creatureEnters = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Forest" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL0_creatureEnters.length, "Forest is dealt for the look").toBeGreaterThan(0);
    for (const id of lookPicksL0_creatureEnters) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: id, to: { kind: 'library', player: 'p1' }, placement: 'top' }));
    bears = put(g, 'p1', "Nyxborn Courser");
    }
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, bears };
}

describe("Nessian Wanderer", () => {
  test("Constellation — Whenever an enchantment you control enters: the vocabulary resolves \"Look at the top three cards of your library. You may reveal a land card from among them and put that card into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
