// `Burnished Hart` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Burnished Hart", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BURNISHED_HART_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Burnished Hart", () => {
  test("{3}, Sacrifice this creature: the vocabulary resolves \"Search your library for up to two basic land cards, put them onto the battlefield tapped, then shuffle.\"", () => {
