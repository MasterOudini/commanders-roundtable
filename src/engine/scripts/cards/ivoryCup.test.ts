// `Ivory Cup` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IVORY_CUP_SCRIPT } from './ivoryCup';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ivory Cup";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ivory Cup", "Grizzly Bears", "Devoted Hero", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IVORY_CUP_SCRIPT]),
  });
  holdEverywhere(g);
