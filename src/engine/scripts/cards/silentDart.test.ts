// `Silent Dart` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SILENT_DART_SCRIPT } from './silentDart';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Silent Dart";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Silent Dart"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SILENT_DART_SCRIPT]),
  });
  holdEverywhere(g);
