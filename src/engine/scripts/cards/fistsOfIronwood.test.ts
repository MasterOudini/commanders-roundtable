// `Fists of Ironwood` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: 2 tokens); has trample;
...
import { FISTS_OF_IRONWOOD_SCRIPT } from './fistsOfIronwood';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fists of Ironwood";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([FISTS_OF_IRONWOOD_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FISTS_OF_IRONWOOD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fists of Ironwood", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: 2 tokens); has trample; Cyclops of One-Eyed Pass is untouched", () => {
