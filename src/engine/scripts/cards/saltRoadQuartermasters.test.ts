// `Salt Road Quartermasters` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SALT_ROAD_QUARTERMASTERS_SCRIPT } from './saltRoadQuartermasters';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Salt Road Quartermasters";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SALT_ROAD_QUARTERMASTERS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Salt Road Quartermasters"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SALT_ROAD_QUARTERMASTERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Salt Road Quartermasters", () => {
  ...
  test("{2}{G}, Remove a +1/+1 counter from this creature: the declared creature gets 1 +1/+1 counter", () => {
