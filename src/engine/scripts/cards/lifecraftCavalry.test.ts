// `Lifecraft Cavalry` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIFECRAFT_CAVALRY_SCRIPT } from './lifecraftCavalry';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lifecraft Cavalry";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LIFECRAFT_CAVALRY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lifecraft Cavalry", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIFECRAFT_CAVALRY_SCRIPT]),
  });
  holdEverywhere(g);
