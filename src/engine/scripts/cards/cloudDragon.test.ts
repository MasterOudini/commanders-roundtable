// `Cloud Dragon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLOUD_DRAGON_SCRIPT } from './cloudDragon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cloud Dragon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cloud Dragon", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLOUD_DRAGON_SCRIPT]),
  });
  holdEverywhere(g);
