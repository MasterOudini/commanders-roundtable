// `Wilson, Refined Grizzly` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WILSON_REFINED_GRIZZLY_SCRIPT } from './wilsonRefinedGrizzly';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wilson, Refined Grizzly";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wilson, Refined Grizzly"], ["Cyclops of One-Eyed Pass", "Counterspell"]],
    scripts: createRegistry([WILSON_REFINED_GRIZZLY_SCRIPT]),
