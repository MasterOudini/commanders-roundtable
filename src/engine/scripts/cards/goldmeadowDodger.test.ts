// `Goldmeadow Dodger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOLDMEADOW_DODGER_SCRIPT } from './goldmeadowDodger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goldmeadow Dodger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goldmeadow Dodger", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOLDMEADOW_DODGER_SCRIPT]),
  });
  holdEverywhere(g);
