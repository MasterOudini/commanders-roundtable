// `Ultima Weapon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ULTIMA_WEAPON_SCRIPT } from './ultimaWeapon';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ultima Weapon";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ULTIMA_WEAPON_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ultima Weapon", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ULTIMA_WEAPON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ultima Weapon", () => {
  test("Whenever equipped creature attacks: the vocabulary resolves \"Destroy target creature an opponent controls.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
  });

  test("Equipped creature gets +7/+7.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([9, 9]);
  });
