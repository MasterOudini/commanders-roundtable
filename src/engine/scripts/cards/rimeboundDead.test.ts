// `Rimebound Dead` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIMEBOUND_DEAD_SCRIPT } from './rimeboundDead';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rimebound Dead";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rimebound Dead", "Lightning Bolt", "Snow-Covered Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIMEBOUND_DEAD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rimebound Dead", () => {
  test("{S}: it regenerates from the Bolt", () => {
