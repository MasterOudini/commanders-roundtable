// `Flesh-Eater Imp` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLESH_EATER_IMP_SCRIPT } from './fleshEaterImp';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flesh-Eater Imp";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FLESH_EATER_IMP_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flesh-Eater Imp", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLESH_EATER_IMP_SCRIPT]),
  });
  holdEverywhere(g);
