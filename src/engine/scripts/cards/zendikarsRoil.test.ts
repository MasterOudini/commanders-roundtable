// `Zendikar's Roil` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zendikar's Roil"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZENDIKARS_ROIL_SCRIPT]),
  });
  holdEverywhere(g);
