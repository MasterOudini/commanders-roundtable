// `Fiery Mantle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIERY_MANTLE_SCRIPT } from './fieryMantle';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fiery Mantle";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FIERY_MANTLE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fiery Mantle", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIERY_MANTLE_SCRIPT]),
  });
  holdEverywhere(g);
