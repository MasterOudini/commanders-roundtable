// `Cast Out` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAST_OUT_SCRIPT } from './castOut';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cast Out"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAST_OUT_SCRIPT]),
  });
  holdEverywhere(g);
