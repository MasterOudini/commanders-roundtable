// `Lamplighter of Selhoff` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LAMPLIGHTER_OF_SELHOFF_SCRIPT } from './lamplighterOfSelhoff';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lamplighter of Selhoff";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lamplighter of Selhoff", "Walking Corpse"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LAMPLIGHTER_OF_SELHOFF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lamplighter of Selhoff", () => {
  test("When this creature enters: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(0);
