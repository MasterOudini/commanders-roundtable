// `Charging Paladin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHARGING_PALADIN_SCRIPT } from './chargingPaladin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Charging Paladin";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHARGING_PALADIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Charging Paladin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHARGING_PALADIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Charging Paladin", () => {
  test("Whenever this creature attacks: it gets +0/+3 until end of turn", () => {
