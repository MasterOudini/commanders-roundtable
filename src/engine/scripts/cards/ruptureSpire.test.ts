// `Rupture Spire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUPTURE_SPIRE_SCRIPT } from './ruptureSpire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rupture Spire";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rupture Spire", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUPTURE_SPIRE_SCRIPT]),
  });
  holdEverywhere(g);
