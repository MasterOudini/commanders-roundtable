// `Tapestry of the Ages` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TAPESTRY_OF_THE_AGES_SCRIPT } from './tapestryOfTheAges';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tapestry of the Ages";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tapestry of the Ages", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TAPESTRY_OF_THE_AGES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Tapestry of the Ages", () => {
  test("{2}, {T}: a card is drawn - refused unless if you've cast a noncreature spell this turn", () => {
