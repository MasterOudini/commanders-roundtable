// `Corrupted Harvester` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CORRUPTED_HARVESTER_SCRIPT } from './corruptedHarvester';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Corrupted Harvester";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Corrupted Harvester", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CORRUPTED_HARVESTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Corrupted Harvester", () => {
  test("{B}, Sacrifice a creature: it regenerates from the Bolt", () => {
