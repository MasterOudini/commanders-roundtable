// `Iron Man, Tony Stark` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IRON_MAN_TONY_STARK_SCRIPT } from './ironManTonyStark';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Iron Man, Tony Stark";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([IRON_MAN_TONY_STARK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Iron Man, Tony Stark", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IRON_MAN_TONY_STARK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Iron Man, Tony Stark", () => {
  test("Attacking creatures you control get +1/+0.: only the attacking creature reads it", () => {
    const { g, no, yes } = armed(0);
    expect(pt(g, yes)).toEqual([3, 2]);
    expect(pt(g, no)).toEqual([5, 2]);
  });

  test("Whenever you cast a red spell: 1 token made", () => {
