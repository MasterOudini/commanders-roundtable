// `Furtive Analyst` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FURTIVE_ANALYST_SCRIPT } from './furtiveAnalyst';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Furtive Analyst";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Furtive Analyst"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FURTIVE_ANALYST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Furtive Analyst", () => {
  test("{2}, {T}: a card is drawn, then one discarded", () => {
