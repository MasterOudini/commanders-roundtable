// `Mnemonic Sliver` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { MNEMONIC_SLIVER_SCRIPT } from './mnemonicSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mnemonic Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Mnemonic Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MNEMONIC_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mnemonic Sliver", () => {
  test("grants {2}, Sacrifice this permanent: Draw a card. to Metallic Sliver, and to nothing outside the scope", () => {
