// `Dynavolt Tower` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DYNAVOLT_TOWER_SCRIPT } from './dynavoltTower';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dynavolt Tower";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dynavolt Tower", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DYNAVOLT_TOWER_SCRIPT]),
...
describe("Dynavolt Tower", () => {
  test("Whenever you cast an instant or sorcery spell: the vocabulary resolves \"You get {E}{E}.\"", () => {
    const { g, energy0 } = armed(0);
    expect(g.state.players.p1?.energy).toBe(energy0 + 2);
  });

  test("{T}, Pay {E}{E}{E}{E}{E}: it deals 3 damage to the declared creature", () => {
    const { g, self, no, energy0 } = armed(1);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
    expect(g.state.cards[self]?.tapped).toBe(true);
    expect(g.state.players.p1?.energy).toBe(energy0 - 5);
  });
