// `Thalakos Drifters` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THALAKOS_DRIFTERS_SCRIPT } from './thalakosDrifters';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thalakos Drifters";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([THALAKOS_DRIFTERS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thalakos Drifters", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THALAKOS_DRIFTERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thalakos Drifters", () => {
  test("Discard a card: it gains shadow until end of turn", () => {
  ...
  test('the pump ends at cleanup', () => {
    ...
    expect(pt(g, self)).toEqual([3, 3]);
    expect(kw(g, self).has("shadow")).toBe(false);
  });
