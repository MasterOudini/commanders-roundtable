// `Embraal Gear-Smasher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EMBRAAL_GEAR_SMASHER_SCRIPT } from './embraalGearSmasher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Embraal Gear-Smasher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Embraal Gear-Smasher", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EMBRAAL_GEAR_SMASHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Embraal Gear-Smasher", () => {
  test("{T}, Sacrifice an artifact: it deals 2 damage to each opponent", () => {
    const { g, self, p2life0, fodder0 } = armed(0);
