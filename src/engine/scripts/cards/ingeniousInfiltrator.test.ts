// `Ingenious Infiltrator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INGENIOUS_INFILTRATOR_SCRIPT } from './ingeniousInfiltrator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ingenious Infiltrator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ingenious Infiltrator", "Fang of Shigeki", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INGENIOUS_INFILTRATOR_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const stillFix0 = put(g, 'p1', "Fang of Shigeki");
  settle(g);
...
describe("Ingenious Infiltrator", () => {
  test("Whenever a Ninja you control deals combat damage to a player: a card is drawn", () => {
