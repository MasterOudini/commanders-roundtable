// `Heartstabber Mosquito` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HEARTSTABBER_MOSQUITO_SCRIPT } from './heartstabberMosquito';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Heartstabber Mosquito";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Heartstabber Mosquito"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HEARTSTABBER_MOSQUITO_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Heartstabber Mosquito", () => {
  test("When this creature enters: the vocabulary resolves \"Destroy target creature.\"", () => {
    const { g, no } = armed(0);
