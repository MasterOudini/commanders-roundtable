// `Cephalid Pathmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CEPHALID_PATHMAGE_SCRIPT } from './cephalidPathmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cephalid Pathmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cephalid Pathmage", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CEPHALID_PATHMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cephalid Pathmage", () => {
  test("This creature can't be blocked.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("{T}, Sacrifice this creature: the vocabulary resolves \"Target creature can't be blocked this turn.\"", () => {
