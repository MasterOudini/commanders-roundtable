// `Improbable Alliance` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IMPROBABLE_ALLIANCE_SCRIPT } from './improbableAlliance';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Improbable Alliance";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Improbable Alliance", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IMPROBABLE_ALLIANCE_SCRIPT]),
  });
  holdEverywhere(g);
