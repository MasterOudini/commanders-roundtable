// `Spontaneous Artist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPONTANEOUS_ARTIST_SCRIPT } from './spontaneousArtist';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spontaneous Artist";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPONTANEOUS_ARTIST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spontaneous Artist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPONTANEOUS_ARTIST_SCRIPT]),
...
describe("Spontaneous Artist", () => {
  test("When this creature enters: the vocabulary resolves \"You get {E}.\"", () => {
    const { g, energy0 } = armed(0);
    expect(g.state.players.p1?.energy).toBe(energy0 + 1);
  });

  test("Pay {E}: the declared creature is pumped until end of turn", () => {
    const { g, no, energy0 } = armed(1);
