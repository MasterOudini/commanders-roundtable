// `Kor Cartographer` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kor Cartographer", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KOR_CARTOGRAPHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kor Cartographer", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a Plains card, put it onto the battlefield tapped, then shuffle.\"", () => {
