// `Trigon of Infestation` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRIGON_OF_INFESTATION_SCRIPT } from './trigonOfInfestation';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trigon of Infestation";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TRIGON_OF_INFESTATION_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trigon of Infestation"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRIGON_OF_INFESTATION_SCRIPT]),
...
describe("Trigon of Infestation", () => {
  ...
  test("{G}{G}, {T}: it gets 1 charge counter", () => {
  ...
  test("{2}, {T}, Remove a charge counter from this artifact: 1 token made", () => {
    const { g, self, board0, cnt0 } = armed(2);
