// `Banisher Priest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BANISHER_PRIEST_SCRIPT } from './banisherPriest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Banisher Priest";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Banisher Priest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BANISHER_PRIEST_SCRIPT]),
  });
  holdEverywhere(g);
