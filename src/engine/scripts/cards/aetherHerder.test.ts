// `Aether Herder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AETHER_HERDER_SCRIPT } from './aetherHerder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aether Herder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aether Herder"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AETHER_HERDER_SCRIPT]),
