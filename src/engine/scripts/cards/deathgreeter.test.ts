// `Deathgreeter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEATHGREETER_SCRIPT } from './deathgreeter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deathgreeter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deathgreeter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEATHGREETER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deathgreeter", () => {
  test("Whenever another creature dies: 1 life is gained", () => {
