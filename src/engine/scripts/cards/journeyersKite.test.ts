// `Journeyer's Kite` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Journeyer's Kite", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JOURNEYERS_KITE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Journeyer's Kite", () => {
  test("{3}, {T}: the vocabulary resolves \"Search your library for a basic land card, reveal it, put it into your hand, then shuffle.\"", () => {
