// `Brimstone Trebuchet` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRIMSTONE_TREBUCHET_SCRIPT } from './brimstoneTrebuchet';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brimstone Trebuchet";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brimstone Trebuchet", "Grizzly Bears", "Knight Errant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRIMSTONE_TREBUCHET_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Brimstone Trebuchet", () => {
  ...
  test("Whenever a Knight you control enters: it untaps", () => {
