// `Hidden Stockpile` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HIDDEN_STOCKPILE_SCRIPT } from './hiddenStockpile';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hidden Stockpile";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hidden Stockpile", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HIDDEN_STOCKPILE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hidden Stockpile", () => {
  test("Revolt — At the beginning of your end step: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("{1}, Sacrifice a creature: scry 1 asks, and the cards stay on top", () => {
    const { g, lib0, fodder0 } = armed(1);
