// `Pathmaker Initiate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PATHMAKER_INITIATE_SCRIPT } from './pathmakerInitiate';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pathmaker Initiate";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pathmaker Initiate"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([PATHMAKER_INITIATE_SCRIPT]),
  });
  holdEverywhere(g);
