// `Totem Speaker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TOTEM_SPEAKER_SCRIPT } from './totemSpeaker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Totem Speaker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Totem Speaker", "Grizzly Bears", "Jibbirik Omnivore"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TOTEM_SPEAKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Totem Speaker", () => {
  test("Whenever a Beast enters: 3 life is gained", () => {
