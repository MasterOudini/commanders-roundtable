// `Iridescent Vinelasher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IRIDESCENT_VINELASHER_SCRIPT } from './iridescentVinelasher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Iridescent Vinelasher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Iridescent Vinelasher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IRIDESCENT_VINELASHER_SCRIPT]),
...
describe("Iridescent Vinelasher", () => {
  test("Landfall — Whenever a land you control enters: it deals 1 damage to the declared player", () => {
