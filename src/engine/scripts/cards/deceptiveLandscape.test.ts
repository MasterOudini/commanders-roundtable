// `Deceptive Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deceptive Landscape", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DECEPTIVE_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deceptive Landscape", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Plains, Swamp, or Forest card, put it onto the battlefield tapped, then shuffle.\"", () => {
