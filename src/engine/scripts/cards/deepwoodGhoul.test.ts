// `Deepwood Ghoul` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEEPWOOD_GHOUL_SCRIPT } from './deepwoodGhoul';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deepwood Ghoul";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deepwood Ghoul", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEEPWOOD_GHOUL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deepwood Ghoul", () => {
  test("Pay 2 life: it regenerates from the Bolt", () => {
