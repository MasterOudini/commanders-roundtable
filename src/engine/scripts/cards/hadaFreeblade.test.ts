// `Hada Freeblade` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HADA_FREEBLADE_SCRIPT } from './hadaFreeblade';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hada Freeblade";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hada Freeblade"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HADA_FREEBLADE_SCRIPT]),
  });
  holdEverywhere(g);
