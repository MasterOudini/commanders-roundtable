// `Krosan Cloudscraper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KROSAN_CLOUDSCRAPER_SCRIPT } from './krosanCloudscraper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Krosan Cloudscraper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Krosan Cloudscraper", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KROSAN_CLOUDSCRAPER_SCRIPT]),
  });
  holdEverywhere(g);
