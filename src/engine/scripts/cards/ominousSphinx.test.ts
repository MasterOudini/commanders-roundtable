// `Ominous Sphinx` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OMINOUS_SPHINX_SCRIPT } from './ominousSphinx';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ominous Sphinx";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([OMINOUS_SPHINX_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ominous Sphinx"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OMINOUS_SPHINX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ominous Sphinx", () => {
  test("Whenever you cycle or discard a card: the vocabulary resolves \"Target creature an opponent controls gets -2/-0 until end of turn.\"", () => {
    const { g, no } = armed(0);
