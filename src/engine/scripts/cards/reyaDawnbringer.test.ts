// `Reya Dawnbringer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { REYA_DAWNBRINGER_SCRIPT } from './reyaDawnbringer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Reya Dawnbringer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Reya Dawnbringer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([REYA_DAWNBRINGER_SCRIPT]),
  });
  holdEverywhere(g);
