// `Valkyrie Harbinger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VALKYRIE_HARBINGER_SCRIPT } from './valkyrieHarbinger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Valkyrie Harbinger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Valkyrie Harbinger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VALKYRIE_HARBINGER_SCRIPT]),
  ...
  if (which === 0) {
    ...
    must(g.submit({ t: 'ManualSetLife', player: 'p1', target: 'p1', delta: 4 }));
      settle(g);
  }
