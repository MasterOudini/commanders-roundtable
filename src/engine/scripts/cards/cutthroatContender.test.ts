// `Cutthroat Contender` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CUTTHROAT_CONTENDER_SCRIPT } from './cutthroatContender';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cutthroat Contender";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CUTTHROAT_CONTENDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cutthroat Contender", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CUTTHROAT_CONTENDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cutthroat Contender", () => {
  test("Pay 1 life: it gets +1/+0 until end of turn", () => {
    const { g, self, life0, refused } = armed(0);
