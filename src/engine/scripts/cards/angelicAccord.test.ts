// `Angelic Accord` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ANGELIC_ACCORD_SCRIPT } from './angelicAccord';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Angelic Accord";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Angelic Accord"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ANGELIC_ACCORD_SCRIPT]),
  });
  holdEverywhere(g);
