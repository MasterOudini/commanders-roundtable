// `Troll Ascetic` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TROLL_ASCETIC_SCRIPT } from './trollAscetic';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Troll Ascetic";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Troll Ascetic", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TROLL_ASCETIC_SCRIPT]),
  });
  holdEverywhere(g);
