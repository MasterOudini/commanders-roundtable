// `Windbrisk Raptor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WINDBRISK_RAPTOR_SCRIPT } from './windbriskRaptor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Windbrisk Raptor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WINDBRISK_RAPTOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Windbrisk Raptor", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WINDBRISK_RAPTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Windbrisk Raptor", () => {
  test("Attacking creatures you control have lifelink.: only the attacking creature reads it", () => {
