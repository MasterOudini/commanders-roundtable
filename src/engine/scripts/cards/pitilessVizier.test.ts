// `Pitiless Vizier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PITILESS_VIZIER_SCRIPT } from './pitilessVizier';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pitiless Vizier";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PITILESS_VIZIER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pitiless Vizier"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PITILESS_VIZIER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pitiless Vizier", () => {
  test("Whenever you cycle or discard a card: it gains indestructible until end of turn", () => {
