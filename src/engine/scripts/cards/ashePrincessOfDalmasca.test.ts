// `Ashe, Princess of Dalmasca` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASHE_PRINCESS_OF_DALMASCA_SCRIPT } from './ashePrincessOfDalmasca';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ashe, Princess of Dalmasca";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ashe, Princess of Dalmasca", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASHE_PRINCESS_OF_DALMASCA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ashe, Princess of Dalmasca", () => {
  test("Whenever Ashe attacks: the vocabulary resolves \"Look at the top five cards of your library. You may reveal an artifact card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
