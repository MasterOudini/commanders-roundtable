// `Elusive Krasis` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ELUSIVE_KRASIS_SCRIPT } from './elusiveKrasis';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Elusive Krasis";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elusive Krasis", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELUSIVE_KRASIS_SCRIPT]),
  });
  holdEverywhere(g);
