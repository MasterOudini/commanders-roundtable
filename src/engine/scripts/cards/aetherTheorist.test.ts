// `Aether Theorist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AETHER_THEORIST_SCRIPT } from './aetherTheorist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aether Theorist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aether Theorist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AETHER_THEORIST_SCRIPT]),
...
describe("Aether Theorist", () => {
  ...
  test("{T}, Pay {E}: scry 1 asks, and the cards stay on top", () => {
    const { g, self, energy0, lib0 } = armed(1);
