// `Centaur Veteran` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CENTAUR_VETERAN_SCRIPT } from './centaurVeteran';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Centaur Veteran";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Centaur Veteran", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CENTAUR_VETERAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Centaur Veteran", () => {
  test("{G}, Discard a card: it regenerates from the Bolt", () => {
