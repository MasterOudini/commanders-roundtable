// `Rootwalla` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROOTWALLA_SCRIPT } from './rootwalla';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rootwalla";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ROOTWALLA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rootwalla", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROOTWALLA_SCRIPT]),
  });
  holdEverywhere(g);
