// `Primal Druid` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Primal Druid", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRIMAL_DRUID_SCRIPT]),
  });
  holdEverywhere(g);
