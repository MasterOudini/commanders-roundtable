// `Kor Halberd` - equipped to Grizzly Bears the creature gets +1/+1 and has vigilance; the Cyclops is untouched;
...
import { KOR_HALBERD_SCRIPT } from './korHalberd';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kor Halberd";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KOR_HALBERD_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([KOR_HALBERD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kor Halberd", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+1 and has vigilance; the Cyclops is untouched", () => {
