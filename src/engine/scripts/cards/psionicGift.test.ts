// `Psionic Gift` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { PSIONIC_GIFT_SCRIPT } from './psionicGift';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Psionic Gift";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Psionic Gift","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PSIONIC_GIFT_SCRIPT]),
  });
  holdEverywhere(g);
