// `Moon-Boy, Dino Rider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOON_BOY_DINO_RIDER_SCRIPT } from './moonBoyDinoRider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Moon-Boy, Dino Rider";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MOON_BOY_DINO_RIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Moon-Boy, Dino Rider", "Raptor Companion"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOON_BOY_DINO_RIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Moon-Boy, Dino Rider", () => {
  test("Whenever Moon-Boy attacks while you control a Dinosaur: it gets +1/+1 until end of turn", () => {
