// `Royal Assassin` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ROYAL_ASSASSIN_SCRIPT } from './royalAssassin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Royal Assassin";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Royal Assassin"], ["Grizzly Bears", "Grizzly Bears"]],
    scripts: createRegistry([ROYAL_ASSASSIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Royal Assassin", () => {
  test("Grizzly Bears (tapped) is a legal target and it is destroyed", () => {
