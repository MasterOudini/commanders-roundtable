// `Novice Dissector` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NOVICE_DISSECTOR_SCRIPT } from './noviceDissector';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Novice Dissector";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Novice Dissector", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NOVICE_DISSECTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Novice Dissector", () => {
  test("{1}, Sacrifice another creature: the declared creature gets 1 +1/+1 counter", () => {
