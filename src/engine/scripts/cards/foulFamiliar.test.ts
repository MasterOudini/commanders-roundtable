// `Foul Familiar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FOUL_FAMILIAR_SCRIPT } from './foulFamiliar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Foul Familiar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Foul Familiar", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOUL_FAMILIAR_SCRIPT]),
  });
  holdEverywhere(g);
