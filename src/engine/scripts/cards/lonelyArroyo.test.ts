// `Lonely Arroyo` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LONELY_ARROYO_SCRIPT } from './lonelyArroyo';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lonely Arroyo";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lonely Arroyo"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LONELY_ARROYO_SCRIPT]),
  });
  holdEverywhere(g);
