// `Bastion of Remembrance` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BASTION_OF_REMEMBRANCE_SCRIPT } from './bastionOfRemembrance';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bastion of Remembrance";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bastion of Remembrance", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BASTION_OF_REMEMBRANCE_SCRIPT]),
  });
  holdEverywhere(g);
