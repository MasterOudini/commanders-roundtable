// `Griffin Guide` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRIFFIN_GUIDE_SCRIPT } from './griffinGuide';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Griffin Guide";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GRIFFIN_GUIDE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Griffin Guide", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRIFFIN_GUIDE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Griffin Guide", () => {
  test("Enchanted creature gets +2/+2 and has flying.: the creature it is attached to reads it", () => {
  ...
  test("When enchanted creature dies: 1 token made", () => {
