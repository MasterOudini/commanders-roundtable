// `Selfless Spirit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SELFLESS_SPIRIT_SCRIPT } from './selflessSpirit';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Selfless Spirit";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SELFLESS_SPIRIT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Selfless Spirit", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SELFLESS_SPIRIT_SCRIPT]),
  });
  holdEverywhere(g);
