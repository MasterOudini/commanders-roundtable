// `Launch` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LAUNCH_SCRIPT } from './launch';
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([LAUNCH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Launch", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LAUNCH_SCRIPT]),
  });
  holdEverywhere(g);
