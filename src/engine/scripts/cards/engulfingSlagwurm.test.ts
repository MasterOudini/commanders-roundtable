// `Engulfing Slagwurm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ENGULFING_SLAGWURM_SCRIPT } from './engulfingSlagwurm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Engulfing Slagwurm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Engulfing Slagwurm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ENGULFING_SLAGWURM_SCRIPT]),
...
describe("Engulfing Slagwurm", () => {
  test("Whenever this creature blocks or becomes blocked by a creature: the vocabulary resolves \"Destroy target creature. You gain life equal to that creature's toughness.\"", () => {
