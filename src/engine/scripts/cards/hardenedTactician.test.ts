// `Hardened Tactician` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HARDENED_TACTICIAN_SCRIPT } from './hardenedTactician';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';
import { TOKEN_TABLE } from '../../../data/tokenTable';

const CARD = "Hardened Tactician";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hardened Tactician"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HARDENED_TACTICIAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hardened Tactician", () => {
  test("{1}, Sacrifice a token: a card is drawn", () => {
