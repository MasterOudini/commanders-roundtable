// `Skemfar Elderhall` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKEMFAR_ELDERHALL_SCRIPT } from './skemfarElderhall';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skemfar Elderhall";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skemfar Elderhall"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKEMFAR_ELDERHALL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skemfar Elderhall", () => {
  test("{2}{B}{B}{G}, {T}, Sacrifice this land: the vocabulary resolves \"Up to one target creature you don't control gets -2/-2 until end of turn. Create two 1/1 green Elf Warrior creature tokens.\"", () => {
    const { g, self, no, board0 } = armed(0);
