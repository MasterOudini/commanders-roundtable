// `Gold Pan` - equipped to Grizzly Bears the creature (on entering: 1 token); gets +1/+1; the Cyclops is untouched;
...
import { GOLD_PAN_SCRIPT } from './goldPan';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOLD_PAN_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GOLD_PAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gold Pan", () => {
  ...
  test("entering: (on entering: 1 token)", () => {
    const g = startedGame({ players: 2, decks: [[CARD, "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]], scripts: createRegistry([GOLD_PAN_SCRIPT]) });
    holdEverywhere(g);
