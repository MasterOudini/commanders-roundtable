// `Goblin Smuggler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_SMUGGLER_SCRIPT } from './goblinSmuggler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Smuggler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Smuggler"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([GOBLIN_SMUGGLER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Smuggler", () => {
  test("{T}: the vocabulary resolves \"Another target creature with power 2 or less can't be blocked this turn.\"", () => {
