// `Ephara's Radiance` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { EPHARAS_RADIANCE_SCRIPT } from './epharasRadiance';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ephara's Radiance";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Ephara's Radiance","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EPHARAS_RADIANCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ephara's Radiance", () => {
  test("grants {1}{W}, {T}: You gain 3 life. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and runs the payload', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{1}{W}, {T}");
