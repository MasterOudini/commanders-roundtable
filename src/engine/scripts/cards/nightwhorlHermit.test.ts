// `Nightwhorl Hermit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIGHTWHORL_HERMIT_SCRIPT } from './nightwhorlHermit';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nightwhorl Hermit";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NIGHTWHORL_HERMIT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nightwhorl Hermit", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIGHTWHORL_HERMIT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Nightwhorl Hermit", () => {
  test("Threshold — As long as there are seven or more cards in your graveyard, this creature gets +1/+0 and can't be blocked.: with seven cards in the graveyard it reads the pump", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([2, 4]);
  });

  test("Threshold — As long as there are seven or more cards in your graveyard, this creature gets +1/+0 and can't be blocked.: the block is refused", () => {
