// `Lightning Dragon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIGHTNING_DRAGON_SCRIPT } from './lightningDragon';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LIGHTNING_DRAGON_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lightning Dragon", "Forest", "Forest", "Mountain", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIGHTNING_DRAGON_SCRIPT]),
  });
  holdEverywhere(g);
