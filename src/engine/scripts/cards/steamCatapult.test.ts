// `Steam Catapult` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STEAM_CATAPULT_SCRIPT } from './steamCatapult';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Steam Catapult";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Steam Catapult"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STEAM_CATAPULT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Steam Catapult", () => {
  test("{T}: the vocabulary resolves \"Destroy target tapped creature.\" - refused unless during your turn, before attackers are declared", () => {
