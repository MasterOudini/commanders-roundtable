// `Mardu Roughrider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARDU_ROUGHRIDER_SCRIPT } from './marduRoughrider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mardu Roughrider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mardu Roughrider"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARDU_ROUGHRIDER_SCRIPT]),
  });
  holdEverywhere(g);
