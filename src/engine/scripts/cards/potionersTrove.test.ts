// `Potioner's Trove` - every printed ability proven in its own game: the cost's mark, the pump
...
import { POTIONERS_TROVE_SCRIPT } from './potionersTrove';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Potioner's Trove";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Potioner's Trove", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([POTIONERS_TROVE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Potioner's Trove", () => {
  test("{T}: 2 life is gained - refused unless if you've cast an instant or sorcery spell this turn", () => {
