// `Gatecreeper Vine` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gatecreeper Vine", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GATECREEPER_VINE_SCRIPT]),
...
describe("Gatecreeper Vine", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a basic land card or a Gate card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
