// `Shriekgeist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHRIEKGEIST_SCRIPT } from './shriekgeist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shriekgeist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shriekgeist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHRIEKGEIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shriekgeist", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Target player mills two cards.\"", () => {
