// `Deadeye Rig-Hauler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEADEYE_RIG_HAULER_SCRIPT } from './deadeyeRigHauler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deadeye Rig-Hauler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deadeye Rig-Hauler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEADEYE_RIG_HAULER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deadeye Rig-Hauler", () => {
  test("Raid — When this creature enters: the vocabulary resolves \"Return target creature to its owner's hand.\"", () => {
    const { g, no } = armed(0);
