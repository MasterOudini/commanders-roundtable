// `Escape Artist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ESCAPE_ARTIST_SCRIPT } from './escapeArtist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Escape Artist";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; refused: boolean; bearsB: InstanceId; disc0: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Escape Artist", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ESCAPE_ARTIST_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, refused, bearsB, disc0 };
}

describe("Escape Artist", () => {
  test("This creature can't be blocked.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("{U}, Discard a card: it returns to its owner's hand", () => {
