// `Hero's Resolve` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+5;
...
import { HEROS_RESOLVE_SCRIPT } from './herosResolve';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hero's Resolve";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HEROS_RESOLVE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([HEROS_RESOLVE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hero's Resolve", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+5; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([3, 7]);
