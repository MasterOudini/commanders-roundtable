// `Headhunter` - a combatDamagePlayer trigger vocab
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { HEADHUNTER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(HEADHUNTER, "Whenever this creature deals combat damage to a player, that player discards a card.\nMorph {B} (You may cast this card face down as a 2/2 creature for {3}. Turn it face up any time for its morph cost.)");
const LINES = PRINTED.split('\n');

const VOCAB_L0 = vocabularyEffects("Target player discards a card.", HEADHUNTER.name);
const VOCAB_T_L0 = vocabularyTargets("Target player discards a card.");

export const HEADHUNTER_SCRIPT: CardScript = {
  oracleId: HEADHUNTER.oracleId,
  ...
  triggers: [
    {
      ...
      label: () => "Headhunter - Target player discards a card.",
