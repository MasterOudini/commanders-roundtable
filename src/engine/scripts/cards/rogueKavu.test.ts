// `Rogue Kavu` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROGUE_KAVU_SCRIPT } from './rogueKavu';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rogue Kavu";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ROGUE_KAVU_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rogue Kavu"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROGUE_KAVU_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rogue Kavu", () => {
  test("Whenever this creature attacks alone: it gets +2/+0 until end of turn", () => {
