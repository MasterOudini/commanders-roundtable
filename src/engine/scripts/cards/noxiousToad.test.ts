// `Noxious Toad` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NOXIOUS_TOAD_SCRIPT } from './noxiousToad';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Noxious Toad";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Noxious Toad"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NOXIOUS_TOAD_SCRIPT]),
  });
  holdEverywhere(g);
