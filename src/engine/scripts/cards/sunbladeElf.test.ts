// `Sunblade Elf` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUNBLADE_ELF_SCRIPT } from './sunbladeElf';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunblade Elf";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUNBLADE_ELF_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sunblade Elf", "Coral Eel", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUNBLADE_ELF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sunblade Elf", () => {
  test("`Sunblade Elf` - as long as you control a Plains: absent while it is unmet, read once it holds", () => {
