// `Carrion Feeder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CARRION_FEEDER_SCRIPT } from './carrionFeeder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Carrion Feeder";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; refused: boolean; fodder0: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Carrion Feeder", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CARRION_FEEDER_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, refused, fodder0 };
}
...
describe("Carrion Feeder", () => {
  ...
  test("Sacrifice a creature: it gets 1 +1/+1 counter", () => {
