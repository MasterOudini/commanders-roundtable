// `Raging Gorilla` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAGING_GORILLA_SCRIPT } from './ragingGorilla';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Raging Gorilla";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RAGING_GORILLA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Raging Gorilla"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAGING_GORILLA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Raging Gorilla", () => {
  test("Whenever this creature blocks or becomes blocked: it gets +2/-2 until end of turn", () => {
