// `Artificer's Intuition` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Artificer's Intuition", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARTIFICERS_INTUITION_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const disc0 = [put(g, 'p1', "Sol Ring", 'hand')];
  settle(g);
...
describe("Artificer's Intuition", () => {
  test("{U}, Discard an artifact card: the vocabulary resolves \"Search your library for an artifact card with mana value 1 or less, reveal it, put it into your hand, then shuffle.\"", () => {
