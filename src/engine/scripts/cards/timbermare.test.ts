// `Timbermare` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Timbermare", "Forest", "Forest", "Forest", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TIMBERMARE_SCRIPT]),
...
describe("Timbermare", () => {
  test("When this creature enters: the vocabulary resolves \"Tap all other creatures.\"", () => {
    const { g, no } = armed(0);
