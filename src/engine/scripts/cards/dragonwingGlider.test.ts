// `Dragonwing Glider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRAGONWING_GLIDER_SCRIPT } from './dragonwingGlider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dragonwing Glider";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DRAGONWING_GLIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dragonwing Glider", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRAGONWING_GLIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dragonwing Glider", () => {
  test("For Mirrodin! (When this Equipment enters: a token is made and it attaches to it", () => {
    ...
    if (germ !== null) expect(pt(g, germ)).toEqual([4, 4]);
  });

  test("Equipped creature gets +2/+2 and has flying and haste.: the creature it is attached to reads it", () => {
