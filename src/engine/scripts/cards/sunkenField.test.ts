// `Sunken Field` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUNKEN_FIELD_SCRIPT } from './sunkenField';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { legalActions } from '../../legal';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunken Field";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; bearsB: InstanceId; vtG1_0: InstanceId; payG1_p2_0: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sunken Field", "Forest"], ["Cyclops of One-Eyed Pass", "Divination", "Forest"]],
    scripts: createRegistry([SUNKEN_FIELD_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const vtG1_0 = put(g, 'p2', "Divination", 'hand');
  const payG1_p2_0 = put(g, 'p2', "Forest");
  ...
  if (which === 1) {
    ...
    must(g.submit({ t: 'CastSpell', player: 'p2', card: vtG1_0 }));
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, bearsB, vtG1_0, payG1_p2_0 };
}

describe("Sunken Field", () => {
  ...
  test("Enchanted land has \"{T}: the vocabulary resolves \"Counter target spell unless its controller pays {1}.\" - the price is declined", () => {
    const { g, bearsB, payG1_p2_0 } = armed(1);
    ...
    settle(g);
    expect(g.state.cards[payG1_p2_0]?.tapped, 'declining costs nothing').toBe(false);
  ...
  test("Enchanted land has \"{T}: the vocabulary resolves \"Counter target spell unless its controller pays {1}.\" - the price is paid", () => {
    const { g, bearsB, payG1_p2_0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
    ...
    settle(g);
    expect(g.state.cards[payG1_p2_0]?.tapped, 'the payment taps what it spends').toBe(true);
