// `Hotfoot Gnome` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOTFOOT_GNOME_SCRIPT } from './hotfootGnome';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hotfoot Gnome";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HOTFOOT_GNOME_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hotfoot Gnome"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOTFOOT_GNOME_SCRIPT]),
  });
  holdEverywhere(g);
