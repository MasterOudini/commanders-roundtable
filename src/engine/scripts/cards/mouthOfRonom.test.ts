// `Mouth of Ronom` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOUTH_OF_RONOM_SCRIPT } from './mouthOfRonom';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mouth of Ronom";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mouth of Ronom", "Snow-Covered Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOUTH_OF_RONOM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mouth of Ronom", () => {
  test("{4}{S}, {T}, Sacrifice this land: it deals 4 damage to the declared creature", () => {
