// `Zhao Zilong, Tiger General` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZHAO_ZILONG_TIGER_GENERAL_SCRIPT } from './zhaoZilongTigerGeneral';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zhao Zilong, Tiger General";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ZHAO_ZILONG_TIGER_GENERAL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zhao Zilong, Tiger General"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZHAO_ZILONG_TIGER_GENERAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zhao Zilong, Tiger General", () => {
  test("Whenever Zhao Zilong blocks: it gets +1/+1 until end of turn", () => {
