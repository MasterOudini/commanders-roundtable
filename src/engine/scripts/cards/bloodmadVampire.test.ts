// `Bloodmad Vampire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODMAD_VAMPIRE_SCRIPT } from './bloodmadVampire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodmad Vampire";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodmad Vampire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODMAD_VAMPIRE_SCRIPT]),
