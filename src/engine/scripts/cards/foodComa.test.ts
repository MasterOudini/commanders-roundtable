// `Food Coma` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FOOD_COMA_SCRIPT } from './foodComa';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Food Coma";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Food Coma"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOOD_COMA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Food Coma", () => {
  test("When this enchantment enters: the vocabulary resolves \"Exile target creature an opponent controls until this enchantment leaves the battlefield. Create a Food token.\"", () => {
