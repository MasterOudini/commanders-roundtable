// `Dimir Cutpurse` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DIMIR_CUTPURSE_SCRIPT } from './dimirCutpurse';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dimir Cutpurse";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dimir Cutpurse"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DIMIR_CUTPURSE_SCRIPT]),
...
describe("Dimir Cutpurse", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Target player discards a card and you draw a card.\"", () => {
