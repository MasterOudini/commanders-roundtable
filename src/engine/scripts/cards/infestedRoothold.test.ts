// `Infested Roothold` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INFESTED_ROOTHOLD_SCRIPT } from './infestedRoothold';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Infested Roothold";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Infested Roothold"], ["Cyclops of One-Eyed Pass", "Memnite", "Grizzly Bears"]],
    scripts: createRegistry([INFESTED_ROOTHOLD_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsP2 = put(g, 'p2', "Memnite", 'hand');
  settle(g);
...
describe("Infested Roothold", () => {
  test("Whenever an opponent casts an artifact spell: 1 token made", () => {
