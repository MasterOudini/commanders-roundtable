// `Daring Fiendbonder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DARING_FIENDBONDER_SCRIPT } from './daringFiendbonder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Daring Fiendbonder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Daring Fiendbonder", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DARING_FIENDBONDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Daring Fiendbonder", () => {
  ...
  test("{1}{B}, Exile this card from your graveyard: the declared creature gets 1 indestructible counter", () => {
    const { g, self, no } = armed(1);
    expect(g.state.cards[no]?.counters["indestructible"] ?? 0).toBe(1);
