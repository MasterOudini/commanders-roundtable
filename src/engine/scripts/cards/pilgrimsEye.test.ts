// `Pilgrim's Eye` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pilgrim's Eye", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PILGRIMS_EYE_SCRIPT]),
  });
  holdEverywhere(g);
