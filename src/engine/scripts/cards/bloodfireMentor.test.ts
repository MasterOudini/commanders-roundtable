// `Bloodfire Mentor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODFIRE_MENTOR_SCRIPT } from './bloodfireMentor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodfire Mentor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodfire Mentor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODFIRE_MENTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bloodfire Mentor", () => {
  test("{2}{U}, {T}: a card is drawn, then one discarded", () => {
