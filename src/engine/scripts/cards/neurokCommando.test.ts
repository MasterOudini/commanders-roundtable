// `Neurok Commando` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NEUROK_COMMANDO_SCRIPT } from './neurokCommando';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Neurok Commando";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Neurok Commando"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NEUROK_COMMANDO_SCRIPT]),
  });
  holdEverywhere(g);
