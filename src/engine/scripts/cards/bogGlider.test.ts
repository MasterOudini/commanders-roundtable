// `Bog Glider` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bog Glider", "Forest", "Rampart Crawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOG_GLIDER_SCRIPT]),
...
describe("Bog Glider", () => {
  test("{T}, Sacrifice a land: the vocabulary resolves \"Search your library for a Mercenary permanent card with mana value 2 or less, put it onto the battlefield, then shuffle.\"", () => {
