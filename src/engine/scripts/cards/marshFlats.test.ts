// `Marsh Flats` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Marsh Flats", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARSH_FLATS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Marsh Flats", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for a Plains or Swamp card, put it onto the battlefield, then shuffle.\"", () => {
