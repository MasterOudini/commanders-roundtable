// `Exiled Boggart` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EXILED_BOGGART_SCRIPT } from './exiledBoggart';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Exiled Boggart";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Exiled Boggart"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EXILED_BOGGART_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Exiled Boggart", () => {
  test("When this creature dies: the vocabulary resolves \"Discard a card.\"", () => {
