// `Funnel-Web Recluse` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FUNNEL_WEB_RECLUSE_SCRIPT } from './funnelWebRecluse';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Funnel-Web Recluse";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Funnel-Web Recluse", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FUNNEL_WEB_RECLUSE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Funnel-Web Recluse", () => {
  test("Morbid — When this creature enters: 1 token made", () => {
