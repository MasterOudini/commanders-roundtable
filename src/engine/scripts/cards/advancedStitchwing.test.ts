// `Advanced Stitchwing` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ADVANCED_STITCHWING_SCRIPT } from './advancedStitchwing';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Advanced Stitchwing";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Advanced Stitchwing", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ADVANCED_STITCHWING_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const disc0 = [put(g, 'p1', "Grizzly Bears", 'hand'), put(g, 'p1', "Grizzly Bears", 'hand')];
  settle(g);
...
describe("Advanced Stitchwing", () => {
  test("{2}{U}, Discard two cards: it returns from the graveyard to the battlefield tapped", () => {
