// `Lightning Berserker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIGHTNING_BERSERKER_SCRIPT } from './lightningBerserker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning Berserker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LIGHTNING_BERSERKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lightning Berserker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIGHTNING_BERSERKER_SCRIPT]),
