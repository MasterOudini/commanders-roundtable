// `Terramorphic Expanse` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Terramorphic Expanse", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TERRAMORPHIC_EXPANSE_SCRIPT]),
  });
  holdEverywhere(g);
