// `Malcolm, the Eyes` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MALCOLM_THE_EYES_SCRIPT } from './malcolmTheEyes';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Malcolm, the Eyes";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Malcolm, the Eyes", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MALCOLM_THE_EYES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Malcolm, the Eyes", () => {
  test("Whenever you cast your second spell each turn: 1 token made", () => {
