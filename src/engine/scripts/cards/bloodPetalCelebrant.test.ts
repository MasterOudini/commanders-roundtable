// `Blood Petal Celebrant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOOD_PETAL_CELEBRANT_SCRIPT } from './bloodPetalCelebrant';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blood Petal Celebrant";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLOOD_PETAL_CELEBRANT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blood Petal Celebrant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOOD_PETAL_CELEBRANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Blood Petal Celebrant", () => {
  test("`Blood Petal Celebrant` - as long as it's attacking: absent while it is unmet, read once it holds", () => {
