// `Phyrexian Monitor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHYREXIAN_MONITOR_SCRIPT } from './phyrexianMonitor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phyrexian Monitor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phyrexian Monitor", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHYREXIAN_MONITOR_SCRIPT]),
  });
  holdEverywhere(g);
