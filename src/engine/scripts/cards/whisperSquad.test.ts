// `Whisper Squad` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Whisper Squad", "Whisper Squad"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WHISPER_SQUAD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Whisper Squad", () => {
  test("{1}{B}: the vocabulary resolves \"Search your library for a card named ~, put it onto the battlefield tapped, then shuffle.\"", () => {
    const { g, board0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Whisper Squad" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Whisper Squad");
      expect(found, "Whisper Squad is not in the library for the search to find").toBeDefined();
