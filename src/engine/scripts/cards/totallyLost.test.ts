// `Totally Lost` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { TOTALLY_LOST_SCRIPT } from './totallyLost';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Totally Lost";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Totally Lost"], ["Grizzly Bears", "Forest"]],
    scripts: createRegistry([TOTALLY_LOST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Totally Lost", () => {
  test('the named target is accepted and the effect lands', () => {
    ...
    settle(g);
    expect(g.state.cards[targets[0] as InstanceId]?.zone.kind).toBe('library');
  });
