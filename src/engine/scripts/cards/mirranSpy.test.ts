// `Mirran Spy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MIRRAN_SPY_SCRIPT } from './mirranSpy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mirran Spy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mirran Spy", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIRRAN_SPY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mirran Spy", () => {
  test("Whenever you cast an artifact spell: the vocabulary resolves \"Untap target creature.\"", () => {
    const { g, no } = armed(0);
