// `Thoughtrender Lamia` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THOUGHTRENDER_LAMIA_SCRIPT } from './thoughtrenderLamia';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thoughtrender Lamia";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thoughtrender Lamia", "Hissing Miasma"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THOUGHTRENDER_LAMIA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thoughtrender Lamia", () => {
  test("Constellation — Whenever this creature or another enchantment you control enters: the vocabulary resolves \"Each opponent discards a card.\"", () => {
    const { g } = armed(0);
    let qdL0 = 0;
