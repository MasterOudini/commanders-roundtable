// `Mask of Memory` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MASK_OF_MEMORY_SCRIPT } from './maskOfMemory';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mask of Memory";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mask of Memory", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MASK_OF_MEMORY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mask of Memory", () => {
  test("Whenever equipped creature deals combat damage to a player: the vocabulary resolves \"Draw two cards. If you do, discard a card.\"", () => {
    const { g } = armed(0);
