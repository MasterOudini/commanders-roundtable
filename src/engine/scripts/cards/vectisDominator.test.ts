// `Vectis Dominator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VECTIS_DOMINATOR_SCRIPT } from './vectisDominator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vectis Dominator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vectis Dominator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VECTIS_DOMINATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vectis Dominator", () => {
  test("{T}: the vocabulary resolves \"Tap target creature unless its controller pays 2 life.\" - the price is declined", () => {
  ...
  test("{T}: the vocabulary resolves \"Tap target creature unless its controller pays 2 life.\" - the price is paid", () => {
    const { g, self, no, p2life0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
