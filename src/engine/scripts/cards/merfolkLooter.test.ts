// `Merfolk Looter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERFOLK_LOOTER_SCRIPT } from './merfolkLooter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Merfolk Looter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merfolk Looter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERFOLK_LOOTER_SCRIPT]),
  });
  holdEverywhere(g);
