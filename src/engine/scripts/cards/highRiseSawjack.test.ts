// `High-Rise Sawjack` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HIGH_RISE_SAWJACK_SCRIPT } from './highRiseSawjack';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "High-Rise Sawjack";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HIGH_RISE_SAWJACK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["High-Rise Sawjack"], ["Cyclops of One-Eyed Pass", "Aven Skirmisher"]],
    scripts: createRegistry([HIGH_RISE_SAWJACK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("High-Rise Sawjack", () => {
  test("Whenever this creature blocks a creature with flying: it gets +2/+0 until end of turn", () => {
