// `Battered Golem` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BATTERED_GOLEM_SCRIPT } from './batteredGolem';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Battered Golem";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Battered Golem", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BATTERED_GOLEM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Battered Golem", () => {
  ...
  test("Whenever an artifact enters: it untaps", () => {
