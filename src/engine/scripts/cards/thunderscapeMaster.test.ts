// `Thunderscape Master` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THUNDERSCAPE_MASTER_SCRIPT } from './thunderscapeMaster';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thunderscape Master";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([THUNDERSCAPE_MASTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thunderscape Master", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THUNDERSCAPE_MASTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thunderscape Master", () => {
  test("{B}{B}, {T}: the vocabulary resolves \"Target player loses 2 life and you gain 2 life.\"", () => {
  ...
  test("{G}{G}, {T}: its controller's creatures get +2/+2 until end of turn", () => {
