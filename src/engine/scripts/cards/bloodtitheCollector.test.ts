// `Bloodtithe Collector` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODTITHE_COLLECTOR_SCRIPT } from './bloodtitheCollector';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodtithe Collector";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodtithe Collector"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODTITHE_COLLECTOR_SCRIPT]),
  });
  holdEverywhere(g);
