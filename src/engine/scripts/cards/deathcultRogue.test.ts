// `Deathcult Rogue` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEATHCULT_ROGUE_SCRIPT } from './deathcultRogue';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deathcult Rogue";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deathcult Rogue", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEATHCULT_ROGUE_SCRIPT]),
...
describe("Deathcult Rogue", () => {
  test("This creature can't be blocked except by Rogues.: the block by a creature outside the exception (Rogues) is refused", () => {
