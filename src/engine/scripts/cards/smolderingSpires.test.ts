// `Smoldering Spires` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SMOLDERING_SPIRES_SCRIPT } from './smolderingSpires';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Smoldering Spires";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Smoldering Spires"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SMOLDERING_SPIRES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Smoldering Spires", () => {
  test("When this land enters: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
    const { g, no } = armed(0);
