// `Surge Conductor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SURGE_CONDUCTOR_SCRIPT } from './surgeConductor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Surge Conductor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Surge Conductor", "Grizzly Bears", "Memnite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SURGE_CONDUCTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Surge Conductor", () => {
  test("Whenever another nontoken artifact you control enters: the vocabulary resolves \"Proliferate.\"", () => {
