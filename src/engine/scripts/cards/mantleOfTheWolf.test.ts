// `Mantle of the Wolf` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MANTLE_OF_THE_WOLF_SCRIPT } from './mantleOfTheWolf';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mantle of the Wolf";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MANTLE_OF_THE_WOLF_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mantle of the Wolf", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MANTLE_OF_THE_WOLF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mantle of the Wolf", () => {
  test("Enchanted creature gets +4/+4.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([6, 6]);
  });

  test("When this Aura is put into a graveyard from the battlefield: 2 tokens made", () => {
