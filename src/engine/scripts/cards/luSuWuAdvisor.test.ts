// `Lu Su, Wu Advisor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LU_SU_WU_ADVISOR_SCRIPT } from './luSuWuAdvisor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lu Su, Wu Advisor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lu Su, Wu Advisor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LU_SU_WU_ADVISOR_SCRIPT]),
  });
  holdEverywhere(g);
