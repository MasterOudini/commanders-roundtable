// `Centaur Safeguard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CENTAUR_SAFEGUARD_SCRIPT } from './centaurSafeguard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Centaur Safeguard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Centaur Safeguard"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CENTAUR_SAFEGUARD_SCRIPT]),
  });
  holdEverywhere(g);
