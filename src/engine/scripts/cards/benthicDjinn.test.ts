// `Benthic Djinn` - each of its controller's upkeeps: its controller loses 2 life; replay equal.
// Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { BENTHIC_DJINN_SCRIPT } from './benthicDjinn';
import { advanceUntil, holdEverywhere, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Benthic Djinn";
...
function fired(): { g: Game; self: InstanceId; life0: number } {
  const g = startedGame({ players: 2, decks: [[CARD], ['Grizzly Bears']], scripts: createRegistry([BENTHIC_DJINN_SCRIPT]) });
  holdEverywhere(g);
...
describe("Benthic Djinn", () => {
  test("its controller's upkeep costs 2 life", () => {
