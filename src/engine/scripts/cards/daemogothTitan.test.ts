// `Daemogoth Titan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAEMOGOTH_TITAN_SCRIPT } from './daemogothTitan';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Daemogoth Titan";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Daemogoth Titan", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAEMOGOTH_TITAN_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const queueFodderL0_attacks = put(g, 'p1', "Grizzly Bears");
  const queueFodderL0_blocks = put(g, 'p1', "Grizzly Bears");
  settle(g);
...
describe("Daemogoth Titan", () => {
  test("Whenever this creature attacks or blocks: the vocabulary resolves \"Sacrifice a creature.\" [attacks]", () => {
  ...
  test("Whenever this creature attacks or blocks: the vocabulary resolves \"Sacrifice a creature.\" [blocks]", () => {
