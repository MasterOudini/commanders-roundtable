// `Shizo, Death's Storehouse` - the named target is accepted and the pump lands; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SHIZO_DEATHS_STOREHOUSE_SCRIPT } from './shizoDeathsStorehouse';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shizo, Death's Storehouse";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Shizo, Death's Storehouse", "Lady Caleria", "Grizzly Bears"], ["Grizzly Bears"]],
    scripts: createRegistry([SHIZO_DEATHS_STOREHOUSE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shizo, Death's Storehouse", () => {
  test("Lady Caleria is a legal target and the pump lands", () => {
    ...
    settle(g);
    const d = deps(createRegistry([SHIZO_DEATHS_STOREHOUSE_SCRIPT]));
    const got = derive(g.state, d.oracle, d.scripts, target);
    expect([got.power, got.toughness]).toEqual([3, 6]);
    expect(got.keywords.has("fear")).toBe(true);
