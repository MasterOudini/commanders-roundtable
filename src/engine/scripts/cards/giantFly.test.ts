// `Giant Fly` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GIANT_FLY_SCRIPT } from './giantFly';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Giant Fly";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([GIANT_FLY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Giant Fly", "Lotus Petal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GIANT_FLY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Giant Fly", () => {
  test("Whenever you sacrifice another permanent: it gets +1/+0 until end of turn", () => {
