// `Umara Raptor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UMARA_RAPTOR_SCRIPT } from './umaraRaptor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Umara Raptor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Umara Raptor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UMARA_RAPTOR_SCRIPT]),
  });
  holdEverywhere(g);
