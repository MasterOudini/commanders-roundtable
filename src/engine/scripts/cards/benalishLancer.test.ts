// `Benalish Lancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BENALISH_LANCER_SCRIPT } from './benalishLancer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Benalish Lancer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BENALISH_LANCER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Benalish Lancer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BENALISH_LANCER_SCRIPT]),
  ...
  if (which === 0) {
    ...
    if (kw(g, self).has("firstStrike")) throw new Error('the kicked keyword held on an unkicked cast');
...
describe("Benalish Lancer", () => {
  test("If this creature was kicked, it enters with two +1/+1 counters on it and with first strike.: cast, it enters with 2 +1/+1 counters", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.zone.kind).toBe('battlefield');
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(2);
    expect(kw(g, self).has("firstStrike"), 'the kick granted firstStrike').toBe(true);
