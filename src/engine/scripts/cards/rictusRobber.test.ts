// `Rictus Robber` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RICTUS_ROBBER_SCRIPT } from './rictusRobber';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rictus Robber";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rictus Robber", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RICTUS_ROBBER_SCRIPT]),
