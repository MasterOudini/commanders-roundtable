// `Misty Rainforest` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Misty Rainforest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MISTY_RAINFOREST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Misty Rainforest", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for a Forest or Island card, put it onto the battlefield, then shuffle.\"", () => {
