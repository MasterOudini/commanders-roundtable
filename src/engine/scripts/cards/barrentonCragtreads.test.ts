// `Barrenton Cragtreads` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BARRENTON_CRAGTREADS_SCRIPT } from './barrentonCragtreads';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Barrenton Cragtreads";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Barrenton Cragtreads", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Crimson Kobolds"]],
    scripts: createRegistry([BARRENTON_CRAGTREADS_SCRIPT]),
...
describe("Barrenton Cragtreads", () => {
  test("This creature can't be blocked by red creatures.: the block by red creatures is refused", () => {
