// `Root Spider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROOT_SPIDER_SCRIPT } from './rootSpider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Root Spider";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ROOT_SPIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Root Spider"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROOT_SPIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Root Spider", () => {
  test("Whenever this creature blocks: it gets +1/+0 and gains firstStrike until end of turn", () => {
