// `Bone Shredder` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bone Shredder", "Forest", "Forest", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BONE_SHREDDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bone Shredder", () => {
  test("When this creature enters: the vocabulary resolves \"Destroy target nonartifact, nonblack creature.\"", () => {
    const { g, no } = armed(0);
