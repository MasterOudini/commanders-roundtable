// `Vengeful Rebel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VENGEFUL_REBEL_SCRIPT } from './vengefulRebel';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vengeful Rebel";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vengeful Rebel", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VENGEFUL_REBEL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vengeful Rebel", () => {
  test("Revolt — When this creature enters: the vocabulary resolves \"Target creature an opponent controls gets -3/-3 until end of turn.\"", () => {
    const { g, no } = armed(0);
