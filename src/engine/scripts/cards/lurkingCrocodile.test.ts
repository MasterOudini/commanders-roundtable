// `Lurking Crocodile` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LURKING_CROCODILE_SCRIPT } from './lurkingCrocodile';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lurking Crocodile";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LURKING_CROCODILE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lurking Crocodile", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LURKING_CROCODILE_SCRIPT]),
  });
  holdEverywhere(g);
