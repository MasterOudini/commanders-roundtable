// `Vedalken Heretic` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VEDALKEN_HERETIC_SCRIPT } from './vedalkenHeretic';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vedalken Heretic";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vedalken Heretic"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VEDALKEN_HERETIC_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vedalken Heretic", () => {
  test("Whenever this creature deals damage to an opponent: a card is drawn", () => {
