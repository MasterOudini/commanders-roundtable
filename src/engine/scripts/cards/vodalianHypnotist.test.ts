// `Vodalian Hypnotist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VODALIAN_HYPNOTIST_SCRIPT } from './vodalianHypnotist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vodalian Hypnotist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vodalian Hypnotist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VODALIAN_HYPNOTIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vodalian Hypnotist", () => {
  test("{2}{B}, {T}: the vocabulary resolves \"Target player discards a card.\"", () => {
