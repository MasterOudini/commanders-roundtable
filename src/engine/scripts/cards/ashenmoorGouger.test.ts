// `Ashenmoor Gouger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASHENMOOR_GOUGER_SCRIPT } from './ashenmoorGouger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ashenmoor Gouger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ashenmoor Gouger", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASHENMOOR_GOUGER_SCRIPT]),
  });
  holdEverywhere(g);
