// `Bloodline Pretender` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODLINE_PRETENDER_SCRIPT } from './bloodlinePretender';
import { advanceUntil, holdEverywhere, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodline Pretender";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodline Pretender", "Grizzly Bears", "Balduvian Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODLINE_PRETENDER_SCRIPT]),
  ...
  if (which === 0) {
    bears = put(g, 'p1', "Balduvian Bears");
...
describe("Bloodline Pretender", () => {
  test("Whenever another creature you control of the chosen type enters: it gets 1 +1/+1 counter", () => {
