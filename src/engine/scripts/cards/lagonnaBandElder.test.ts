// `Lagonna-Band Elder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LAGONNA_BAND_ELDER_SCRIPT } from './lagonnaBandElder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lagonna-Band Elder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lagonna-Band Elder", "Hissing Miasma"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LAGONNA_BAND_ELDER_SCRIPT]),
  });
  holdEverywhere(g);
