// `Reckless Pangolin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RECKLESS_PANGOLIN_SCRIPT } from './recklessPangolin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Reckless Pangolin";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RECKLESS_PANGOLIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Reckless Pangolin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RECKLESS_PANGOLIN_SCRIPT]),
  });
  holdEverywhere(g);
