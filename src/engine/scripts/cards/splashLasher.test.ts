// `Splash Lasher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPLASH_LASHER_SCRIPT } from './splashLasher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Splash Lasher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Splash Lasher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPLASH_LASHER_SCRIPT]),
...
describe("Splash Lasher", () => {
  test("When this creature enters: the vocabulary resolves \"Tap up to one target creature and put a stun counter on ~.\"", () => {
    const { g, no } = armed(0);
