// `Snarespinner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SNARESPINNER_SCRIPT } from './snarespinner';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Snarespinner";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SNARESPINNER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Snarespinner"], ["Cyclops of One-Eyed Pass", "Aven Skirmisher"]],
    scripts: createRegistry([SNARESPINNER_SCRIPT]),
  });
  holdEverywhere(g);
