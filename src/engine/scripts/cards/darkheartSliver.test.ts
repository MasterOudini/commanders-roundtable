// `Darkheart Sliver` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { DARKHEART_SLIVER_SCRIPT } from './darkheartSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Darkheart Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Darkheart Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DARKHEART_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Darkheart Sliver", () => {
  test("grants Sacrifice this permanent: You gain 3 life. to Metallic Sliver, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and runs the payload', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("Sacrifice this permanent");
