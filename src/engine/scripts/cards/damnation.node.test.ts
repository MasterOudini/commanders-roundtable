  ...
  test('no engine source implements or consults regeneration', () => {
    ...
    const walk = (d: string): void => {
      for (const entry of readdirSync(d, { withFileTypes: true })) {
        ...
        // Meek · D205 Crumble · D208 Devour in Shadow).
        if (['damnation.ts', 'terminate.ts', 'wrathOfGod.ts', 'consumeTheMeek.ts', 'crumble.ts', 'devourInShadow.ts'].includes(entry.name)) continue;
