  ...
  test('no engine source implements or consults regeneration', () => {
    ...
    const walk = (d: string): void => {
      for (const entry of readdirSync(d, { withFileTypes: true })) {
        ...
        // D214 Flesh to Dust · D221 Jokulhaups · D231 Oxidize).
        if (['damnation.ts', 'terminate.ts', 'wrathOfGod.ts', 'consumeTheMeek.ts', 'crumble.ts', 'devourInShadow.ts', 'fissure.ts', 'fleshToDust.ts', 'jokulhaups.ts', 'oxidize.ts'].includes(entry.name)) continue;
