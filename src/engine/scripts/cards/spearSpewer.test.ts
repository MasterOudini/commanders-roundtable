// `Spear Spewer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPEAR_SPEWER_SCRIPT } from './spearSpewer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spear Spewer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spear Spewer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPEAR_SPEWER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spear Spewer", () => {
  test("{T}: the vocabulary resolves \"~ deals 1 damage to each player.\"", () => {
