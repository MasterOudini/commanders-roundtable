// `Flight` - cast on Grizzly Bears it attaches and the enchanted creature has flying;
...
import { FLIGHT_SCRIPT } from './flight';
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([FLIGHT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FLIGHT_SCRIPT]),
  });
  holdEverywhere(g);
