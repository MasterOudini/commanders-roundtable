// `Goblin Glider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_GLIDER_SCRIPT } from './goblinGlider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Glider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Glider", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_GLIDER_SCRIPT]),
  });
  holdEverywhere(g);
