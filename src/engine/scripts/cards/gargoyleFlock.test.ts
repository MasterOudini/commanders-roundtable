// `Gargoyle Flock` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GARGOYLE_FLOCK_SCRIPT } from './gargoyleFlock';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gargoyle Flock";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gargoyle Flock", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GARGOYLE_FLOCK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gargoyle Flock", () => {
  test("Skyswarm — At the beginning of your end step: 1 token made", () => {
