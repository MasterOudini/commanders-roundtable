// `Glassdust Hulk` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GLASSDUST_HULK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glassdust Hulk", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLASSDUST_HULK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Glassdust Hulk", () => {
  test("Whenever another artifact you control enters: it gets +1/+1 until end of turn and can't be blocked this turn", () => {
