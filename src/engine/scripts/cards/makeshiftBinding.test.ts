// `Makeshift Binding` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAKESHIFT_BINDING_SCRIPT } from './makeshiftBinding';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Makeshift Binding";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Makeshift Binding"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAKESHIFT_BINDING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Makeshift Binding", () => {
  test("When this enchantment enters: the vocabulary resolves \"Exile target creature an opponent controls until this enchantment leaves the battlefield. You gain 2 life.\"", () => {
