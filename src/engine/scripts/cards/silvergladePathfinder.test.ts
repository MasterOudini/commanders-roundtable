// `Silverglade Pathfinder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SILVERGLADE_PATHFINDER_SCRIPT } from './silvergladePathfinder';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Silverglade Pathfinder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Silverglade Pathfinder", "Grizzly Bears", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SILVERGLADE_PATHFINDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Silverglade Pathfinder", () => {
  test("{1}{G}, {T}, Discard a card: the vocabulary resolves \"Search your library for a basic land card, put that card onto the battlefield tapped, then shuffle.\"", () => {
