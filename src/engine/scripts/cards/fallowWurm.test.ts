// `Fallow Wurm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FALLOW_WURM_SCRIPT } from './fallowWurm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fallow Wurm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fallow Wurm", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FALLOW_WURM_SCRIPT]),
  });
  holdEverywhere(g);
