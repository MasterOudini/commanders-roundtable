// `Akroma's Devoted` - the grant reaches the permanent its scope names and not the other;
// it ends when the source leaves; replay equal (D300). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { AKROMAS_DEVOTED_SCRIPT } from './akromasDevoted';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Akroma's Devoted";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([AKROMAS_DEVOTED_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Akroma's Devoted"], ["Ancestor's Prophet", "Coral Eel"]],
    scripts: createRegistry([AKROMAS_DEVOTED_SCRIPT]),
  });
  holdEverywhere(g);
  const yes = put(g, 'p2', "Ancestor's Prophet");
...
describe("Akroma's Devoted", () => {
  test("Ancestor's Prophet is reached, Coral Eel is not", () => {
