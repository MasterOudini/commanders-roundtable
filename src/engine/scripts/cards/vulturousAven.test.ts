// `Vulturous Aven` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VULTUROUS_AVEN_SCRIPT } from './vulturousAven';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vulturous Aven";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vulturous Aven"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VULTUROUS_AVEN_SCRIPT]),
...
describe("Vulturous Aven", () => {
  test("When this creature exploits a creature: the vocabulary resolves \"You draw two cards and you lose 2 life.\"", () => {
