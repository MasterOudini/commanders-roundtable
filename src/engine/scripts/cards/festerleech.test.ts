// `Festerleech` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FESTERLEECH_SCRIPT } from './festerleech';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Festerleech";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FESTERLEECH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Festerleech", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FESTERLEECH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Festerleech", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"You mill two cards.\"", () => {
    const { g, gy0 } = armed(0);
    expect((g.state.zones.graveyard.p1 ?? []).length).toBe(gy0 + 2);
  });

  test("{1}{B}: it gets +2/+2 until end of turn", () => {
