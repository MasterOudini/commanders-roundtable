// `Fretwork Colony` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FRETWORK_COLONY_SCRIPT } from './fretworkColony';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fretwork Colony";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fretwork Colony", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FRETWORK_COLONY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fretwork Colony", () => {
  test("This creature can't block.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("At the beginning of your upkeep: the vocabulary resolves \"Put a +1/+1 counter on this creature and you lose 1 life.\"", () => {
