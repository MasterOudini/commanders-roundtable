// `Complex Automaton` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COMPLEX_AUTOMATON_SCRIPT } from './complexAutomaton';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Complex Automaton";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Complex Automaton", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COMPLEX_AUTOMATON_SCRIPT]),
...
describe("Complex Automaton", () => {
  test("At the beginning of your upkeep: it returns to its owner's hand", () => {
