// `Sphinx Summoner` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sphinx Summoner", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPHINX_SUMMONER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sphinx Summoner", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for an artifact creature card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Memnite" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
