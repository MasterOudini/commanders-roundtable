// `Firefiend Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIREFIEND_ELEMENTAL_SCRIPT } from './firefiendElemental';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Firefiend Elemental";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FIREFIEND_ELEMENTAL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Firefiend Elemental"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIREFIEND_ELEMENTAL_SCRIPT]),
  });
  holdEverywhere(g);
