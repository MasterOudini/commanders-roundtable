// `Sidisi's Faithful` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SIDISIS_FAITHFUL_SCRIPT } from './sidisisFaithful';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sidisi's Faithful";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sidisi's Faithful"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIDISIS_FAITHFUL_SCRIPT]),
...
describe("Sidisi's Faithful", () => {
  test("When this creature exploits a creature: the vocabulary resolves \"Return target creature to its owner's hand.\"", () => {
    const { g, no } = armed(0);
