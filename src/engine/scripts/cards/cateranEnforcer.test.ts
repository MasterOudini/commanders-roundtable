// `Cateran Enforcer` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cateran Enforcer", "Rampart Crawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CATERAN_ENFORCER_SCRIPT]),
...
describe("Cateran Enforcer", () => {
  test("{4}, {T}: the vocabulary resolves \"Search your library for a Mercenary permanent card with mana value 4 or less, put it onto the battlefield, then shuffle.\"", () => {
