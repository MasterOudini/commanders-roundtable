// `Kamber, the Plunderer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KAMBER_THE_PLUNDERER_SCRIPT } from './kamberThePlunderer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kamber, the Plunderer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kamber, the Plunderer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KAMBER_THE_PLUNDERER_SCRIPT]),
...
describe("Kamber, the Plunderer", () => {
  test("Whenever a creature an opponent controls dies: the vocabulary resolves \"You gain 1 life and create a Blood token.\"", () => {
