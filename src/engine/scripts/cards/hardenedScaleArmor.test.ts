// `Hardened-Scale Armor` - cast on Grizzly Bears it attaches and the enchanted creature gets +3/+3;
...
import { HARDENED_SCALE_ARMOR_SCRIPT } from './hardenedScaleArmor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hardened-Scale Armor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HARDENED_SCALE_ARMOR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([HARDENED_SCALE_ARMOR_SCRIPT]),
  });
  holdEverywhere(g);
