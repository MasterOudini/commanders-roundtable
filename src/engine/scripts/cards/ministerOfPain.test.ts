// `Minister of Pain` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MINISTER_OF_PAIN_SCRIPT } from './ministerOfPain';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Minister of Pain";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MINISTER_OF_PAIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Minister of Pain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MINISTER_OF_PAIN_SCRIPT]),
...
describe("Minister of Pain", () => {
  test("When this creature exploits a creature: the vocabulary resolves \"Creatures your opponents control get -1/-1 until end of turn.\"", () => {
    const { g, no } = armed(0);
