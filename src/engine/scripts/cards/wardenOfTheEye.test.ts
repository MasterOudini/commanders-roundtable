// `Warden of the Eye` - the named target is accepted and it returns to hand; a permanent the adjective excludes
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Warden of the Eye", "Lightning Bolt", "Grizzly Bears"], ["Grizzly Bears"]],
    scripts: createRegistry([WARDEN_OF_THE_EYE_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD, 'graveyard');
  const target = put(g, 'p1', "Lightning Bolt", 'graveyard');
...
describe("Warden of the Eye", () => {
  test("Lightning Bolt is a legal target and it returns to hand", () => {
