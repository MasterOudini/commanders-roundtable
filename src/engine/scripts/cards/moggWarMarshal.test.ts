// `Mogg War Marshal` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mogg War Marshal", "Forest", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOGG_WAR_MARSHAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mogg War Marshal", () => {
  test("When this creature enters or dies: 1 token made [etb]", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("When this creature enters or dies: 1 token made [dies]", () => {
