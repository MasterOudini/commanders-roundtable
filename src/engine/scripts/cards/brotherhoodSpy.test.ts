// `Brotherhood Spy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BROTHERHOOD_SPY_SCRIPT } from './brotherhoodSpy';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brotherhood Spy";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BROTHERHOOD_SPY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brotherhood Spy", "Black Widow, Natasha Romanoff"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BROTHERHOOD_SPY_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    ...
    put(g, 'p1', "Black Widow, Natasha Romanoff");
      settle(g);
  }
...
describe("Brotherhood Spy", () => {
  test("At the beginning of combat on your turn: it gets +1/+0 until end of turn and can't be blocked this turn", () => {
