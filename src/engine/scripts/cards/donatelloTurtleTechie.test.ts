// `Donatello, Turtle Techie` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DONATELLO_TURTLE_TECHIE_SCRIPT } from './donatelloTurtleTechie';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Donatello, Turtle Techie";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Donatello, Turtle Techie", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DONATELLO_TURTLE_TECHIE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Donatello, Turtle Techie", () => {
  test("When Donatello enters: a card is drawn", () => {
