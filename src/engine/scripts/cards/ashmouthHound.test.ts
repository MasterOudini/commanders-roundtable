// `Ashmouth Hound` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASHMOUTH_HOUND_SCRIPT } from './ashmouthHound';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ashmouth Hound";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ashmouth Hound"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASHMOUTH_HOUND_SCRIPT]),
