// `Silhana Ledgewalker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SILHANA_LEDGEWALKER_SCRIPT } from './silhanaLedgewalker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Silhana Ledgewalker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Silhana Ledgewalker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SILHANA_LEDGEWALKER_SCRIPT]),
...
describe("Silhana Ledgewalker", () => {
  test("This creature can't be blocked except by creatures with flying.: the block by a creature outside the exception (creatures with flying) is refused", () => {
