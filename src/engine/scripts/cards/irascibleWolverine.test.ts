// `Irascible Wolverine` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IRASCIBLE_WOLVERINE_SCRIPT } from './irascibleWolverine';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Irascible Wolverine";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Irascible Wolverine"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IRASCIBLE_WOLVERINE_SCRIPT]),
...
describe("Irascible Wolverine", () => {
  test("When this creature enters: the vocabulary resolves \"Exile the top card of your library. Until end of turn, you may play that card.\"", () => {
    const { g } = armed(0);
