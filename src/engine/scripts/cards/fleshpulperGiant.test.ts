// `Fleshpulper Giant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLESHPULPER_GIANT_SCRIPT } from './fleshpulperGiant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fleshpulper Giant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fleshpulper Giant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLESHPULPER_GIANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fleshpulper Giant", () => {
  test("When this creature enters: the vocabulary resolves \"Destroy target creature with toughness 2 or less.\"", () => {
    const { g, no } = armed(0);
