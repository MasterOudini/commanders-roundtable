// `Shipwreck Looter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHIPWRECK_LOOTER_SCRIPT } from './shipwreckLooter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shipwreck Looter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shipwreck Looter", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHIPWRECK_LOOTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shipwreck Looter", () => {
  test("Raid — When this creature enters: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(0);
