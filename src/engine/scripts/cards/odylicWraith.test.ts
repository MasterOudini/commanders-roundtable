// `Odylic Wraith` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ODYLIC_WRAITH_SCRIPT } from './odylicWraith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Odylic Wraith";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Odylic Wraith"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ODYLIC_WRAITH_SCRIPT]),
  });
  holdEverywhere(g);
