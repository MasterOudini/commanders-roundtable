// `Spider-Girl, Legacy Hero` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIDER_GIRL_LEGACY_HERO_SCRIPT } from './spiderGirlLegacyHero';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spider-Girl, Legacy Hero";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPIDER_GIRL_LEGACY_HERO_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spider-Girl, Legacy Hero"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPIDER_GIRL_LEGACY_HERO_SCRIPT]),
...
describe("Spider-Girl, Legacy Hero", () => {
  test("`Spider-Girl, Legacy Hero` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
  ...
  test("When Spider-Girl leaves the battlefield: 1 token made", () => {
