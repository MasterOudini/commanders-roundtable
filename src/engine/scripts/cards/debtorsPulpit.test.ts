// `Debtor's Pulpit` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { DEBTORS_PULPIT_SCRIPT } from './debtorsPulpit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Debtor's Pulpit";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Debtor's Pulpit","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEBTORS_PULPIT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Debtor's Pulpit", () => {
  test("grants {T}: Tap target creature. to Forest, and to nothing outside the scope", () => {
