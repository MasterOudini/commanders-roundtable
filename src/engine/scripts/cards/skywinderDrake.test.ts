// `Skywinder Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKYWINDER_DRAKE_SCRIPT } from './skywinderDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skywinder Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skywinder Drake", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKYWINDER_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
