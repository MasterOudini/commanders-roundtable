// `Germinating Wurm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GERMINATING_WURM_SCRIPT } from './germinatingWurm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Germinating Wurm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Germinating Wurm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GERMINATING_WURM_SCRIPT]),
