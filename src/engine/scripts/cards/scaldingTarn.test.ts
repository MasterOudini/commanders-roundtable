// `Scalding Tarn` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scalding Tarn", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCALDING_TARN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Scalding Tarn", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for an Island or Mountain card, put it onto the battlefield, then shuffle.\"", () => {
