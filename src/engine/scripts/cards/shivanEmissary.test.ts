// `Shivan Emissary` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHIVAN_EMISSARY_SCRIPT } from './shivanEmissary';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shivan Emissary";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shivan Emissary"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHIVAN_EMISSARY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shivan Emissary", () => {
  test("When this creature enters: the vocabulary resolves \"Destroy target nonblack creature. It can't be regenerated.\"", () => {
    const { g, no } = armed(0);
