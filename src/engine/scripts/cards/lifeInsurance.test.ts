// `Life Insurance` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIFE_INSURANCE_SCRIPT } from './lifeInsurance';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Life Insurance";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Life Insurance", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIFE_INSURANCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Life Insurance", () => {
  test("Whenever a nontoken creature dies: the vocabulary resolves \"You lose 1 life and create a Treasure token.\"", () => {
