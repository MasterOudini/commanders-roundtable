// `Excavating Anurid` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EXCAVATING_ANURID_SCRIPT } from './excavatingAnurid';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Excavating Anurid";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([EXCAVATING_ANURID_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Excavating Anurid", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EXCAVATING_ANURID_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Excavating Anurid", () => {
  test("When this creature enters: the vocabulary resolves \"You may sacrifice a land. If you do, draw a card.\" - the price is declined", () => {
  ...
  test("When this creature enters: the vocabulary resolves \"You may sacrifice a land. If you do, draw a card.\" - the price is paid", () => {
  ...
  test("Threshold — As long as there are seven or more cards in your graveyard, this creature gets +1/+1 and has vigilance.: with seven cards in the graveyard it reads the pump", () => {
