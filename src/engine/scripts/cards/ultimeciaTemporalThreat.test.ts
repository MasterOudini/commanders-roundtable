// `Ultimecia, Temporal Threat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ULTIMECIA_TEMPORAL_THREAT_SCRIPT } from './ultimeciaTemporalThreat';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ultimecia, Temporal Threat";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ultimecia, Temporal Threat", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ULTIMECIA_TEMPORAL_THREAT_SCRIPT]),
...
describe("Ultimecia, Temporal Threat", () => {
  test("When Ultimecia enters: the vocabulary resolves \"Tap all creatures your opponents control.\"", () => {
    const { g, no } = armed(0);
