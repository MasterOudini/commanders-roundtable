// `Famished Paladin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FAMISHED_PALADIN_SCRIPT } from './famishedPaladin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Famished Paladin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Famished Paladin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FAMISHED_PALADIN_SCRIPT]),
  });
  holdEverywhere(g);
