// `Deep Goblin Skulltaker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEEP_GOBLIN_SKULLTAKER_SCRIPT } from './deepGoblinSkulltaker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deep Goblin Skulltaker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deep Goblin Skulltaker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEEP_GOBLIN_SKULLTAKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deep Goblin Skulltaker", () => {
