// `Sabotender` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sabotender"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SABOTENDER_SCRIPT]),
  });
  holdEverywhere(g);
