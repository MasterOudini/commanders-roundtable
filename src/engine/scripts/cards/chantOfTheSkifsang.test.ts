// `Chant of the Skifsang` - cast on Grizzly Bears it attaches and the enchanted creature gets -13/+0;
...
import { CHANT_OF_THE_SKIFSANG_SCRIPT } from './chantOfTheSkifsang';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chant of the Skifsang";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHANT_OF_THE_SKIFSANG_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([CHANT_OF_THE_SKIFSANG_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chant of the Skifsang", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets -13/+0; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([-11, 2]);
