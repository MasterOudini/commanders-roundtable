// `Rampaging Baloths` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rampaging Baloths"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAMPAGING_BALOTHS_SCRIPT]),
  });
  holdEverywhere(g);
