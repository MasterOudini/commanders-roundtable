// `Mogg Fanatic` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOGG_FANATIC_SCRIPT } from './moggFanatic';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mogg Fanatic";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mogg Fanatic"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOGG_FANATIC_SCRIPT]),
  });
  holdEverywhere(g);
