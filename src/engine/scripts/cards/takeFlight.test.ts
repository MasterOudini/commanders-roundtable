// `Take Flight` - the granted TRIGGER proven end to end (D368): the recipient HAS the
...
import { TAKE_FLIGHT_SCRIPT } from './takeFlight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Take Flight";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Take Flight","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TAKE_FLIGHT_SCRIPT]),
  });
  holdEverywhere(g);
