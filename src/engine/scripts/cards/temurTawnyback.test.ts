// `Temur Tawnyback` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEMUR_TAWNYBACK_SCRIPT } from './temurTawnyback';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Temur Tawnyback";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Temur Tawnyback"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEMUR_TAWNYBACK_SCRIPT]),
  });
  holdEverywhere(g);
