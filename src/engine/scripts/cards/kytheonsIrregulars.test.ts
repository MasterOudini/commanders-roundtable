// `Kytheon's Irregulars` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KYTHEONS_IRREGULARS_SCRIPT } from './kytheonsIrregulars';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kytheon's Irregulars";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KYTHEONS_IRREGULARS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kytheon's Irregulars", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KYTHEONS_IRREGULARS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kytheon's Irregulars", () => {
  ...
  test("{W}{W}: the declared creature is tapped", () => {
    const { g, no } = armed(1);
