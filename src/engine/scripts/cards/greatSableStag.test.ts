// `Great Sable Stag` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GREAT_SABLE_STAG_SCRIPT } from './greatSableStag';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Great Sable Stag";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Great Sable Stag"], ["Cyclops of One-Eyed Pass", "Counterspell"]],
    scripts: createRegistry([GREAT_SABLE_STAG_SCRIPT]),
  });
  holdEverywhere(g);
