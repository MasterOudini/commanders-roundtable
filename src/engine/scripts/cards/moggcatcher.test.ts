// `Moggcatcher` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Moggcatcher", "Mons's Goblin Raiders"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOGGCATCHER_SCRIPT]),
...
describe("Moggcatcher", () => {
  test("{3}, {T}: the vocabulary resolves \"Search your library for a Goblin permanent card, put it onto the battlefield, then shuffle.\"", () => {
    const { g, self, board0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Mons's Goblin Raiders" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Mons's Goblin Raiders");
      expect(found, "Mons's Goblin Raiders is not in the library for the search to find").toBeDefined();
