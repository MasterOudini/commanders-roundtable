// `Frost Walker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FROST_WALKER_SCRIPT } from './frostWalker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frost Walker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frost Walker", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FROST_WALKER_SCRIPT]),
  });
  holdEverywhere(g);
