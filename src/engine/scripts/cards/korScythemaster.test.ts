// `Kor Scythemaster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KOR_SCYTHEMASTER_SCRIPT } from './korScythemaster';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kor Scythemaster";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KOR_SCYTHEMASTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kor Scythemaster"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KOR_SCYTHEMASTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kor Scythemaster", () => {
  test("`Kor Scythemaster` - as long as it's attacking: absent while it is unmet, read once it holds", () => {
