// `Citanul Hierophants` - the granted MANA ability proven end to end (D372): the recipient HAS the
...
import { CITANUL_HIEROPHANTS_SCRIPT } from './citanulHierophants';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Citanul Hierophants";
const QUOTED = "{T}: Add {G}.";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Citanul Hierophants","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CITANUL_HIEROPHANTS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Citanul Hierophants", () => {
  test("grants {T}: Add {G}. to Grizzly Bears, and to nothing outside the scope", () => {
