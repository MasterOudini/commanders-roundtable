// `Seedguide Ash` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seedguide Ash", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEEDGUIDE_ASH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Seedguide Ash", () => {
  test("When this creature dies: the vocabulary resolves \"Search your library for up to three Forest cards, put them onto the battlefield tapped, then shuffle.\"", () => {
