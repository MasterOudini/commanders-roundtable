// `Benthic Infiltrator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BENTHIC_INFILTRATOR_SCRIPT } from './benthicInfiltrator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Benthic Infiltrator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Benthic Infiltrator", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BENTHIC_INFILTRATOR_SCRIPT]),
  });
  holdEverywhere(g);
