// `Sun-Spider, Nimble Webber` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUN_SPIDER_NIMBLE_WEBBER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sun-Spider, Nimble Webber", "Accorder's Shield"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUN_SPIDER_NIMBLE_WEBBER_SCRIPT]),
...
describe("Sun-Spider, Nimble Webber", () => {
  test("`Sun-Spider, Nimble Webber` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
  ...
  test("When Sun-Spider enters: the vocabulary resolves \"Search your library for an Aura or Equipment card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Accorder's Shield" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
