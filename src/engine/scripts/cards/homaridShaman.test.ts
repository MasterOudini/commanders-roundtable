// `Homarid Shaman` - the named target is accepted and it is tapped; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { HOMARID_SHAMAN_SCRIPT } from './homaridShaman';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Homarid Shaman";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Homarid Shaman"], ["Grizzly Bears", "Vampire Nighthawk"]],
    scripts: createRegistry([HOMARID_SHAMAN_SCRIPT]),
  });
  holdEverywhere(g);
