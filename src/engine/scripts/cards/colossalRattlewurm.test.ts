// `Colossal Rattlewurm` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([COLOSSAL_RATTLEWURM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Colossal Rattlewurm", "Bucolic Ranch", "Bucolic Ranch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COLOSSAL_RATTLEWURM_SCRIPT]),
  ...
  let condOff = false;
  if (which === 0) {
    condOff = !kw(g, self).has("flash");
    // D398 - stage two: the condition is met.
    put(g, 'p1', "Bucolic Ranch");
    settle(g);
    }
  if (which === 1) {
...
describe("Colossal Rattlewurm", () => {
  test("`Colossal Rattlewurm` - as long as you control a Desert: absent while it is unmet, read once it holds", () => {
    const { g, self, condOff } = armed(0);
    expect(pt(g, self)).toEqual([6, 5]);
    expect(kw(g, self).has("flash")).toBe(true);
    expect(condOff, 'the static is absent while the condition is unmet').toBe(true);
  });

  test("{1}{G}, Exile this card from your graveyard: the vocabulary resolves \"Search your library for a Desert card, put it onto the battlefield tapped, then shuffle.\"", () => {
    const { g, self, board0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Bucolic Ranch" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Bucolic Ranch");
      expect(found, "Bucolic Ranch is not in the library for the search to find").toBeDefined();
