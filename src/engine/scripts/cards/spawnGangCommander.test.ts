// `Spawn-Gang Commander` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPAWN_GANG_COMMANDER_SCRIPT } from './spawnGangCommander';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spawn-Gang Commander";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spawn-Gang Commander", "Prophet of Distortion"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPAWN_GANG_COMMANDER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = [put(g, 'p1', "Prophet of Distortion")];
  settle(g);
...
describe("Spawn-Gang Commander", () => {
  test("When you cast this spell: the vocabulary resolves \"Create three 0/1 colorless Eldrazi Spawn creature tokens with \\\"Sacrifice this token: Add {C}.\\\"\"", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 3 + 1);
  });

  test("{1}{C}, Sacrifice an Eldrazi: it deals 2 damage to the declared creature", () => {
