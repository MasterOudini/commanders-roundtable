// `Frostling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FROSTLING_SCRIPT } from './frostling';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frostling";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frostling"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FROSTLING_SCRIPT]),
  });
  holdEverywhere(g);
