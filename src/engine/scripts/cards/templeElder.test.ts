// `Temple Elder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEMPLE_ELDER_SCRIPT } from './templeElder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Temple Elder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Temple Elder"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEMPLE_ELDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Temple Elder", () => {
  test("{T}: 1 life is gained - refused unless during your turn, before attackers are declared", () => {
