// `Serum Raker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SERUM_RAKER_SCRIPT } from './serumRaker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Serum Raker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Serum Raker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SERUM_RAKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Serum Raker", () => {
  test("When this creature dies: the vocabulary resolves \"Each player discards a card.\"", () => {
