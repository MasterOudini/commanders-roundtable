// `Vona, Butcher of Magan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VONA_BUTCHER_OF_MAGAN_SCRIPT } from './vonaButcherOfMagan';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vona, Butcher of Magan";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vona, Butcher of Magan"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VONA_BUTCHER_OF_MAGAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vona, Butcher of Magan", () => {
  test("{T}, Pay 7 life: the vocabulary resolves \"Destroy target nonland permanent.\" - refused unless during your turn", () => {
    const { g, self, no, life0, condRefused } = armed(0);
