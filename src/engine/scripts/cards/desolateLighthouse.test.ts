// `Desolate Lighthouse` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DESOLATE_LIGHTHOUSE_SCRIPT } from './desolateLighthouse';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Desolate Lighthouse";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Desolate Lighthouse"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DESOLATE_LIGHTHOUSE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Desolate Lighthouse", () => {
  test("{1}{U}{R}, {T}: a card is drawn, then one discarded", () => {
