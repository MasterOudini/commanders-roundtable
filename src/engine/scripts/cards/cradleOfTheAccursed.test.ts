// `Cradle of the Accursed` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRADLE_OF_THE_ACCURSED_SCRIPT } from './cradleOfTheAccursed';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cradle of the Accursed";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cradle of the Accursed"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRADLE_OF_THE_ACCURSED_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cradle of the Accursed", () => {
  test("{3}, {T}, Sacrifice this land: 1 token made", () => {
