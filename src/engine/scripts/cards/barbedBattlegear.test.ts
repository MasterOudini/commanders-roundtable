// `Barbed Battlegear` - equipped to Grizzly Bears the creature gets +4/-1; the Cyclops is untouched;
...
import { BARBED_BATTLEGEAR_SCRIPT } from './barbedBattlegear';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Barbed Battlegear";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BARBED_BATTLEGEAR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([BARBED_BATTLEGEAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Barbed Battlegear", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +4/-1; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([6, 1]);
