// `Patchwork Gnomes` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PATCHWORK_GNOMES_SCRIPT } from './patchworkGnomes';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Patchwork Gnomes";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Patchwork Gnomes", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PATCHWORK_GNOMES_SCRIPT]),
  });
  holdEverywhere(g);
