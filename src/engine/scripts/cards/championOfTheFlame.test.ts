// `Champion of the Flame` - the pump is COUNTED: zero of them is no bonus, and each one moves the
// derived power and toughness by the printed delta. Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { CHAMPION_OF_THE_FLAME_SCRIPT } from './championOfTheFlame';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Champion of the Flame";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHAMPION_OF_THE_FLAME_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; subject: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([CHAMPION_OF_THE_FLAME_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Champion of the Flame", () => {
  test("each Swiftfoot Boots attached counted moves the P/T by [2, 2], and one leaving takes it back", () => {
