// `Devouring Strossus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEVOURING_STROSSUS_SCRIPT } from './devouringStrossus';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Devouring Strossus";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; queueFodderL1_upkeep: InstanceId; fodder0: InstanceId[] };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Devouring Strossus", "Grizzly Bears", "Infernal Grasp", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEVOURING_STROSSUS_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, queueFodderL1_upkeep, fodder0 };
}

describe("Devouring Strossus", () => {
  ...
  test("Sacrifice a creature: it regenerates from the Bolt", () => {
