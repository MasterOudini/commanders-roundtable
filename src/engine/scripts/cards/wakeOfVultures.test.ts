// `Wake of Vultures` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WAKE_OF_VULTURES_SCRIPT } from './wakeOfVultures';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wake of Vultures";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wake of Vultures", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WAKE_OF_VULTURES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wake of Vultures", () => {
  test("{1}{B}, Sacrifice a creature: it regenerates from the Bolt", () => {
