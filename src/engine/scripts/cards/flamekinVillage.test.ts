// `Flamekin Village` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLAMEKIN_VILLAGE_SCRIPT } from './flamekinVillage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flamekin Village";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FLAMEKIN_VILLAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flamekin Village"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLAMEKIN_VILLAGE_SCRIPT]),
  });
  holdEverywhere(g);
