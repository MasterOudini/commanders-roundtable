// `White Auracite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WHITE_AURACITE_SCRIPT } from './whiteAuracite';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "White Auracite";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["White Auracite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WHITE_AURACITE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("White Auracite", () => {
  test("When this artifact enters: the vocabulary resolves \"Exile target nonland permanent an opponent controls until this artifact leaves the battlefield.\"", () => {
