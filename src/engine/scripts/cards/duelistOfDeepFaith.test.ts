// `Duelist of Deep Faith` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DUELIST_OF_DEEP_FAITH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Duelist of Deep Faith"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUELIST_OF_DEEP_FAITH_SCRIPT]),
...
describe("Duelist of Deep Faith", () => {
  test("`Duelist of Deep Faith` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
