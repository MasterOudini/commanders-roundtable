// `Cave of Temptation` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAVE_OF_TEMPTATION_SCRIPT } from './caveOfTemptation';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cave of Temptation";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cave of Temptation"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAVE_OF_TEMPTATION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cave of Temptation", () => {
  test("{4}, {T}, Sacrifice this land: the declared creature gets 2 +1/+1 counters", () => {
