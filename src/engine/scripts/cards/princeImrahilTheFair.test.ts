// `Prince Imrahil the Fair` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PRINCE_IMRAHIL_THE_FAIR_SCRIPT } from './princeImrahilTheFair';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prince Imrahil the Fair";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prince Imrahil the Fair", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRINCE_IMRAHIL_THE_FAIR_SCRIPT]),
  });
  holdEverywhere(g);
