// `Fog Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FOG_ELEMENTAL_SCRIPT } from './fogElemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fog Elemental";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fog Elemental"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([FOG_ELEMENTAL_SCRIPT]),
