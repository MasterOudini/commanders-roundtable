// `Fanatical Devotion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FANATICAL_DEVOTION_SCRIPT } from './fanaticalDevotion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fanatical Devotion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fanatical Devotion", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FANATICAL_DEVOTION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fanatical Devotion", () => {
  test("Sacrifice a creature: the declared creature regenerates from the Bolt", () => {
