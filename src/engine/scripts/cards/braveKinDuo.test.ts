// `Brave-Kin Duo` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRAVE_KIN_DUO_SCRIPT } from './braveKinDuo';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brave-Kin Duo";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BRAVE_KIN_DUO_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brave-Kin Duo"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRAVE_KIN_DUO_SCRIPT]),
  });
  holdEverywhere(g);
