// `Huang Zhong, Shu General` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HUANG_ZHONG_SHU_GENERAL_SCRIPT } from './huangZhongShuGeneral';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Huang Zhong, Shu General";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Huang Zhong, Shu General", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([HUANG_ZHONG_SHU_GENERAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Huang Zhong, Shu General", () => {
  test("Huang Zhong can't be blocked by more than one creature.: the block is refused", () => {
