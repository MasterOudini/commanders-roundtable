// `Casey Jones, Jury-Rig Justiciar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CASEY_JONES_JURY_RIG_JUSTICIAR_SCRIPT } from './caseyJonesJuryRigJusticiar';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Casey Jones, Jury-Rig Justiciar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Casey Jones, Jury-Rig Justiciar", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CASEY_JONES_JURY_RIG_JUSTICIAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Casey Jones, Jury-Rig Justiciar", () => {
  test("When Casey Jones enters: the vocabulary resolves \"Look at the top four cards of your library. You may reveal an artifact card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
