// `Viridian Longbow` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { VIRIDIAN_LONGBOW_SCRIPT } from './viridianLongbow';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Viridian Longbow";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Viridian Longbow","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIRIDIAN_LONGBOW_SCRIPT]),
  });
  holdEverywhere(g);
