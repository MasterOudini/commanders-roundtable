// `Frostwalla` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FROSTWALLA_SCRIPT } from './frostwalla';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frostwalla";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FROSTWALLA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frostwalla", "Grizzly Bears", "Snow-Covered Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FROSTWALLA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Frostwalla", () => {
  test("{S}: it gets +2/+2 until end of turn", () => {
