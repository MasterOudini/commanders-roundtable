// `Stronghold Machinist` - counters a held spell of the named colour and refuses one of another:
// the adjective is the parser's and the validator's (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { STRONGHOLD_MACHINIST_SCRIPT } from './strongholdMachinist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stronghold Machinist";
...
function armed(which: 'right' | 'wrong'): { g: Game; self: InstanceId; spell: InstanceId; stackId: string; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Stronghold Machinist", "Grizzly Bears"], ["Pyretic Ritual", "Grizzly Bears"]],
    scripts: createRegistry([STRONGHOLD_MACHINIST_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const spell = put(g, 'p2', which === 'right' ? "Pyretic Ritual" : "Grizzly Bears", 'hand');
  settle(g);
  ...
  const fodder = put(g, 'p1', "Grizzly Bears", 'hand');
  must(g.submit({ t: 'ActivateAbility', player: 'p1', card: self, abilityIndex: 0, discard: [fodder] }));
  return { g, self, spell, stackId, life0 };
}

describe("Stronghold Machinist", () => {
  test("the held Pyretic Ritual is a legal target and is countered", () => {
    const { g, self, spell, stackId } = armed('right');
