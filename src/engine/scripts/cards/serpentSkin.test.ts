// `Serpent Skin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SERPENT_SKIN_SCRIPT } from './serpentSkin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Serpent Skin";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SERPENT_SKIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Serpent Skin", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SERPENT_SKIN_SCRIPT]),
  });
  holdEverywhere(g);
