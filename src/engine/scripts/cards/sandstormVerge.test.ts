// `Sandstorm Verge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SANDSTORM_VERGE_SCRIPT } from './sandstormVerge';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sandstorm Verge";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sandstorm Verge"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SANDSTORM_VERGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sandstorm Verge", () => {
  test("{3}, {T}: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
