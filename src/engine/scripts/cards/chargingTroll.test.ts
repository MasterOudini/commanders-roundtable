// `Charging Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHARGING_TROLL_SCRIPT } from './chargingTroll';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Charging Troll";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Charging Troll", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHARGING_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
