// `Sarcomite Myr` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SARCOMITE_MYR_SCRIPT } from './sarcomiteMyr';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sarcomite Myr";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SARCOMITE_MYR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sarcomite Myr"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SARCOMITE_MYR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sarcomite Myr", () => {
  test("{2}: it gains flying until end of turn", () => {
