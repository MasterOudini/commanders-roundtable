// `Stormsplitter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORMSPLITTER_SCRIPT } from './stormsplitter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stormsplitter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stormsplitter", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORMSPLITTER_SCRIPT]),
...
describe("Stormsplitter", () => {
  test("Whenever you cast an instant or sorcery spell: the vocabulary resolves \"Create a token that's a copy of this creature. Exile that token at the beginning of the next end step.\"", () => {
