// `Slag Strider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLAG_STRIDER_SCRIPT } from './slagStrider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slag Strider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slag Strider", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLAG_STRIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Slag Strider", () => {
  test("{1}, Sacrifice an artifact: it deals 1 damage to the declared creature", () => {
