// `Infernal Captor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INFERNAL_CAPTOR_SCRIPT } from './infernalCaptor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Infernal Captor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([INFERNAL_CAPTOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Infernal Captor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INFERNAL_CAPTOR_SCRIPT]),
...
describe("Infernal Captor", () => {
  test("When this creature exploits a creature: the vocabulary resolves \"Gain control of target artifact or creature until end of turn. Untap that permanent. It gains haste until end of turn.\"", () => {
