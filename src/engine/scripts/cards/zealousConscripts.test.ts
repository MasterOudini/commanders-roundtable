// `Zealous Conscripts` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZEALOUS_CONSCRIPTS_SCRIPT } from './zealousConscripts';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zealous Conscripts";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ZEALOUS_CONSCRIPTS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zealous Conscripts"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZEALOUS_CONSCRIPTS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zealous Conscripts", () => {
  test("When this creature enters: the vocabulary resolves \"Gain control of target permanent until end of turn. Untap that permanent. It gains haste until end of turn.\"", () => {
