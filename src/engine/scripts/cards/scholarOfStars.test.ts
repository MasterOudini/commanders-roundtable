// `Scholar of Stars` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCHOLAR_OF_STARS_SCRIPT } from './scholarOfStars';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scholar of Stars";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scholar of Stars", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCHOLAR_OF_STARS_SCRIPT]),
  });
  holdEverywhere(g);
