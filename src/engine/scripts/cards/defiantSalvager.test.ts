// `Defiant Salvager` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEFIANT_SALVAGER_SCRIPT } from './defiantSalvager';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Defiant Salvager";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Defiant Salvager", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEFIANT_SALVAGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Defiant Salvager", () => {
  test("Sacrifice an artifact or creature: it gets 1 +1/+1 counter", () => {
