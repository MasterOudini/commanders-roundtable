// `Demonic Appetite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEMONIC_APPETITE_SCRIPT } from './demonicAppetite';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Demonic Appetite";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; bearsB: InstanceId; queueFodderL2_upkeep: InstanceId };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DEMONIC_APPETITE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Demonic Appetite", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEMONIC_APPETITE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, bearsB, queueFodderL2_upkeep };
}

describe("Demonic Appetite", () => {
  test("Enchanted creature gets +3/+3.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([5, 5]);
  });

  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice a creature.\"", () => {
    const { g, board0, queueFodderL2_upkeep } = armed(1);
