// `Necropede` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NECROPEDE_SCRIPT } from './necropede';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Necropede";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Necropede"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NECROPEDE_SCRIPT]),
  });
  holdEverywhere(g);
