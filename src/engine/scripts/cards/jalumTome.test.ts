// `Jalum Tome` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JALUM_TOME_SCRIPT } from './jalumTome';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jalum Tome";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jalum Tome"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JALUM_TOME_SCRIPT]),
  });
  holdEverywhere(g);
