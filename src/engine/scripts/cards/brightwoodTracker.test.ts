// `Brightwood Tracker` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brightwood Tracker", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRIGHTWOOD_TRACKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Brightwood Tracker", () => {
  test("{5}{G}, {T}: the vocabulary resolves \"Look at the top four cards of your library. You may reveal a creature card from among them and put it into your hand. Put the rest on the bottom of your library in a random order.\"", () => {
