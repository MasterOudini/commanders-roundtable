// `Tamiyo's Immobilizer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TAMIYOS_IMMOBILIZER_SCRIPT } from './tamiyosImmobilizer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tamiyo's Immobilizer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TAMIYOS_IMMOBILIZER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tamiyo's Immobilizer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TAMIYOS_IMMOBILIZER_SCRIPT]),
...
describe("Tamiyo's Immobilizer", () => {
  test("This artifact enters with four oil counters on it.: cast, it enters with 4 oil counters", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.zone.kind).toBe('battlefield');
    expect(g.state.cards[self]?.counters["oil"] ?? 0).toBe(4);
  });

  test("{T}, Remove an oil counter from this artifact: the vocabulary resolves \"Tap target artifact or creature.\"", () => {
