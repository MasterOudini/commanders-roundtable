// `Quirion Trailblazer` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Quirion Trailblazer", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([QUIRION_TRAILBLAZER_SCRIPT]),
  });
  holdEverywhere(g);
