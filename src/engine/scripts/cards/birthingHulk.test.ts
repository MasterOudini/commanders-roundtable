// `Birthing Hulk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BIRTHING_HULK_SCRIPT } from './birthingHulk';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Birthing Hulk";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Birthing Hulk", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BIRTHING_HULK_SCRIPT]),
...
describe("Birthing Hulk", () => {
  test("When this creature enters: the vocabulary resolves \"Create two 1/1 colorless Eldrazi Scion creature tokens. They have \\\"Sacrifice this token: Add {C}.\\\"\"", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 2 + 1);
  });

  test("{1}{C}: it regenerates from the Bolt", () => {
