// `Skeletal Kathari` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKELETAL_KATHARI_SCRIPT } from './skeletalKathari';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skeletal Kathari";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skeletal Kathari", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKELETAL_KATHARI_SCRIPT]),
  });
  holdEverywhere(g);
