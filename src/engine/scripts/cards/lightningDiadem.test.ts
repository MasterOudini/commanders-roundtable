// `Lightning Diadem` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIGHTNING_DIADEM_SCRIPT } from './lightningDiadem';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning Diadem";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LIGHTNING_DIADEM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lightning Diadem", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIGHTNING_DIADEM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lightning Diadem", () => {
  test("When this Aura enters: it deals 2 damage to the declared creature", () => {
    const { g, no } = armed(0);
