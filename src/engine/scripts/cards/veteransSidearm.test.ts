// `Veteran's Sidearm` - equipped to Grizzly Bears the creature gets +1/+1; the Cyclops is untouched;
...
import { VETERANS_SIDEARM_SCRIPT } from './veteransSidearm';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Veteran's Sidearm";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VETERANS_SIDEARM_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([VETERANS_SIDEARM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Veteran's Sidearm", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+1; the Cyclops is untouched", () => {
