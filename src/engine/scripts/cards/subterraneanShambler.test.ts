// `Subterranean Shambler` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Subterranean Shambler", "Forest", "Forest", "Forest", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUBTERRANEAN_SHAMBLER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Subterranean Shambler", () => {
  test("When this creature enters or leaves the battlefield: the vocabulary resolves \"~ deals 1 damage to each creature without flying.\" [etb]", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.damage).toBe(1);
  });

  test("When this creature enters or leaves the battlefield: the vocabulary resolves \"~ deals 1 damage to each creature without flying.\" [leavesBattlefield]", () => {
    const { g, no } = armed(1);
    expect(g.state.cards[no]?.damage).toBe(1);
  });
