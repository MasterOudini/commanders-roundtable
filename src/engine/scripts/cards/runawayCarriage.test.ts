// `Runaway Carriage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUNAWAY_CARRIAGE_SCRIPT } from './runawayCarriage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Runaway Carriage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Runaway Carriage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUNAWAY_CARRIAGE_SCRIPT]),
