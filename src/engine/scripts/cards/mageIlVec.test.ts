// `Mage il-Vec` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAGE_IL_VEC_SCRIPT } from './mageIlVec';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mage il-Vec";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mage il-Vec"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAGE_IL_VEC_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mage il-Vec", () => {
  test("{T}, Discard a card at random: it deals 1 damage to the declared creature", () => {
