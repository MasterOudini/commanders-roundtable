// `Fuming Effigy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FUMING_EFFIGY_SCRIPT } from './fumingEffigy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fuming Effigy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fuming Effigy", "Badlands"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FUMING_EFFIGY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fuming Effigy", () => {
  test("Whenever one or more cards leave your graveyard: it deals 1 damage to each opponent", () => {
