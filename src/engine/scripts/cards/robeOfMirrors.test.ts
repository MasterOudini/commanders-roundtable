// `Robe of Mirrors` - cast on Grizzly Bears it attaches and the enchanted creature has shroud;
...
import { ROBE_OF_MIRRORS_SCRIPT } from './robeOfMirrors';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Robe of Mirrors";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([ROBE_OF_MIRRORS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ROBE_OF_MIRRORS_SCRIPT]),
  });
  holdEverywhere(g);
