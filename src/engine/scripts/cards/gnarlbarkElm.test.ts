// `Gnarlbark Elm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GNARLBARK_ELM_SCRIPT } from './gnarlbarkElm';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gnarlbark Elm";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GNARLBARK_ELM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gnarlbark Elm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GNARLBARK_ELM_SCRIPT]),
...
describe("Gnarlbark Elm", () => {
  ...
  test("{2}{B}, Remove two counters from this creature: the declared creature is pumped until end of turn", () => {
