// `Basalt Gargoyle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BASALT_GARGOYLE_SCRIPT } from './basaltGargoyle';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BASALT_GARGOYLE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Basalt Gargoyle", "Forest", "Forest", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BASALT_GARGOYLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Basalt Gargoyle", () => {
  test("{R}: it gets +0/+1 until end of turn", () => {
