// `Species Specialist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPECIES_SPECIALIST_SCRIPT } from './speciesSpecialist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Species Specialist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Species Specialist", "Grizzly Bears", "Balduvian Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPECIES_SPECIALIST_SCRIPT]),
  ...
  if (which === 0) {
    const bears = put(g, 'p1', "Balduvian Bears");
...
describe("Species Specialist", () => {
  test("Whenever a creature of the chosen type dies: a card is drawn", () => {
