// `Storm Fleet Pyromancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORM_FLEET_PYROMANCER_SCRIPT } from './stormFleetPyromancer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Storm Fleet Pyromancer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Storm Fleet Pyromancer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORM_FLEET_PYROMANCER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Storm Fleet Pyromancer", () => {
  test("Raid — When this creature enters: it deals 2 damage to the declared creature", () => {
    const { g, no } = armed(0);
