// `Chitinous Cloak` - equipped to Grizzly Bears the creature gets +2/+2 and has menace; the Cyclops is untouched;
...
import { CHITINOUS_CLOAK_SCRIPT } from './chitinousCloak';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chitinous Cloak";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHITINOUS_CLOAK_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([CHITINOUS_CLOAK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Chitinous Cloak", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +2/+2 and has menace; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([4, 4]);
    expect(kw(g, host).has("menace")).toBe(true);
    expect(pt(g, other)).toEqual([5, 2]);
    expect(kw(g, other).has("menace")).toBe(false);
  });
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("menace")).toBe(false);
  });
