// `Battle Mastery` - cast on Grizzly Bears it attaches and the enchanted creature has doubleStrike;
...
import { BATTLE_MASTERY_SCRIPT } from './battleMastery';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Battle Mastery";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([BATTLE_MASTERY_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([BATTLE_MASTERY_SCRIPT]),
  });
  holdEverywhere(g);
