// `Trapped in the Screen` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRAPPED_IN_THE_SCREEN_SCRIPT } from './trappedInTheScreen';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trapped in the Screen";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trapped in the Screen"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRAPPED_IN_THE_SCREEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Trapped in the Screen", () => {
  test("When this enchantment enters: the vocabulary resolves \"Exile target artifact, creature, or enchantment an opponent controls until this enchantment leaves the battlefield.\"", () => {
