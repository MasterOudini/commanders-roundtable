// `Haunted Library` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HAUNTED_LIBRARY_SCRIPT } from './hauntedLibrary';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Haunted Library";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Haunted Library", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HAUNTED_LIBRARY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Haunted Library", () => {
  test("Whenever a creature an opponent controls dies: the vocabulary resolves \"You may pay {1}. If you do, create a 1/1 white Spirit creature token with flying.\" - the price is declined", () => {
  ...
  test("Whenever a creature an opponent controls dies: the vocabulary resolves \"You may pay {1}. If you do, create a 1/1 white Spirit creature token with flying.\" - the price is paid", () => {
