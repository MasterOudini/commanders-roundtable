// `Dining Room` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DINING_ROOM_SCRIPT } from './diningRoom';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dining Room";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dining Room"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DINING_ROOM_SCRIPT]),
  });
  holdEverywhere(g);
