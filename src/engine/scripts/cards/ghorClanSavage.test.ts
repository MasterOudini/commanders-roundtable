// `Ghor-Clan Savage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHOR_CLAN_SAVAGE_SCRIPT } from './ghorClanSavage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghor-Clan Savage";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GHOR_CLAN_SAVAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghor-Clan Savage", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHOR_CLAN_SAVAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ghor-Clan Savage", () => {
  test("Bloodthirst 3 (If an opponent was dealt damage this turn, this creature enters with three +1/+1 counters on it.): cast, it enters with 3 +1/+1 counters", () => {
