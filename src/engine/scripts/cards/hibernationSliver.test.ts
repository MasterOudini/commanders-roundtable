// `Hibernation Sliver` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { HIBERNATION_SLIVER_SCRIPT } from './hibernationSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hibernation Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Hibernation Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HIBERNATION_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hibernation Sliver", () => {
  test("grants Pay 2 life: Return this permanent to its owner's hand. to Metallic Sliver, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and the body hits the RECIPIENT', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("Pay 2 life");
    ...
    settle(g);
    expect(g.state.cards[host]?.zone.kind).toBe('hand');
