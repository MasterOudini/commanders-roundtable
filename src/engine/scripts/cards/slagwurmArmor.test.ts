// `Slagwurm Armor` - equipped to Grizzly Bears the creature gets +0/+6; the Cyclops is untouched;
...
import { SLAGWURM_ARMOR_SCRIPT } from './slagwurmArmor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slagwurm Armor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SLAGWURM_ARMOR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SLAGWURM_ARMOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Slagwurm Armor", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +0/+6; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([2, 8]);
