// `Unflinching Courage` - an Aura (Enchant creature): the enchanted creature gets +2/+2 and has trample, lifelink.
...
import { UNFLINCHING_COURAGE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(UNFLINCHING_COURAGE, "Enchant creature\nEnchanted creature gets +2/+2 and has trample and lifelink. (Damage dealt by the creature also causes its controller to gain that much life.)");
const LINES = PRINTED.split('\n');

export const UNFLINCHING_COURAGE_SCRIPT: CardScript = {
  oracleId: UNFLINCHING_COURAGE.oracleId,
  name: UNFLINCHING_COURAGE.name,
  statics: [
    {
