// `Spineless Thug` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPINELESS_THUG_SCRIPT } from './spinelessThug';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spineless Thug";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spineless Thug", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPINELESS_THUG_SCRIPT]),
  });
  holdEverywhere(g);
