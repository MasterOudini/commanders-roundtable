// `Soltari Lancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOLTARI_LANCER_SCRIPT } from './soltariLancer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Soltari Lancer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SOLTARI_LANCER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soltari Lancer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOLTARI_LANCER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Soltari Lancer", () => {
  test("`Soltari Lancer` - as long as it's attacking: absent while it is unmet, read once it holds", () => {
