// `Sun-Crowned Hunters` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUN_CROWNED_HUNTERS_SCRIPT } from './sunCrownedHunters';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sun-Crowned Hunters";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sun-Crowned Hunters", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([SUN_CROWNED_HUNTERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sun-Crowned Hunters", () => {
  test("Enrage — Whenever this creature is dealt damage: the vocabulary resolves \"~ deals 3 damage to target opponent or planeswalker.\" [isDealtCombatDamage]", () => {
    const { g, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 + -3);
  });

  test("Enrage — Whenever this creature is dealt damage: the vocabulary resolves \"~ deals 3 damage to target opponent or planeswalker.\" [isDealtNoncombatDamage]", () => {
