// `Scrap Compactor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCRAP_COMPACTOR_SCRIPT } from './scrapCompactor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scrap Compactor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scrap Compactor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCRAP_COMPACTOR_SCRIPT]),
...
describe("Scrap Compactor", () => {
  test("{3}, {T}, Sacrifice this artifact: it deals 3 damage to the declared creature", () => {
  ...
  test("{6}, {T}, Sacrifice this artifact: the vocabulary resolves \"Destroy target creature or Vehicle.\"", () => {
