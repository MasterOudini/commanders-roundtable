// `Goldvein Pick` - equipped to Grizzly Bears the creature gets +1/+1; (on the equipped creature dealing combat damage to a player: 1 token); the Cyclops is untouched;
...
import { GOLDVEIN_PICK_SCRIPT } from './goldveinPick';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goldvein Pick";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOLDVEIN_PICK_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GOLDVEIN_PICK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goldvein Pick", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +1/+1; (on the equipped creature dealing combat damage to a player: 1 token); the Cyclops is untouched", () => {
  ...
  test("the equipped creature dealing combat damage to a player: (on the equipped creature dealing combat damage to a player: 1 token)", () => {
    const { g, self, host } = board();
    equip(g, self, host);
