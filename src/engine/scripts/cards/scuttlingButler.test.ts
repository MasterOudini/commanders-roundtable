// `Scuttling Butler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCUTTLING_BUTLER_SCRIPT } from './scuttlingButler';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scuttling Butler";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SCUTTLING_BUTLER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scuttling Butler", "Watchwolf", "Watchwolf"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCUTTLING_BUTLER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Scuttling Butler", () => {
  test("At the beginning of combat on your turn: it gains doubleStrike until end of turn", () => {
