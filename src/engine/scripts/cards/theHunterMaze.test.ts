// `The Hunter Maze` — enters tapped, and the sac-draw eats the land for a card.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { THE_HUNTER_MAZE_SCRIPT } from './theHunterMaze';
import { advanceUntil, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const SPHERE = 'The Hunter Maze';
...
function game(): { g: Game; sphere: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [[SPHERE], []],
    scripts: createRegistry([THE_HUNTER_MAZE_SCRIPT]),
  });
...
/** Straightens the land (it entered tapped) and funds the {1}{G}. */
