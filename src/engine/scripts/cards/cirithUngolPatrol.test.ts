// `Cirith Ungol Patrol` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CIRITH_UNGOL_PATROL_SCRIPT } from './cirithUngolPatrol';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cirith Ungol Patrol";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cirith Ungol Patrol", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CIRITH_UNGOL_PATROL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cirith Ungol Patrol", () => {
  test("{1}, {T}, Sacrifice another creature: the vocabulary resolves \"Draw a card, then create a Food token.\"", () => {
