// `Dire Wolf Prowler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DIRE_WOLF_PROWLER_SCRIPT } from './direWolfProwler';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dire Wolf Prowler";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DIRE_WOLF_PROWLER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dire Wolf Prowler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DIRE_WOLF_PROWLER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dire Wolf Prowler", () => {
  test("{1}{G}: it gets +2/+2 and gains haste until end of turn", () => {
