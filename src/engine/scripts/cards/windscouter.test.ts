// `Windscouter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WINDSCOUTER_SCRIPT } from './windscouter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Windscouter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Windscouter"], ["Cyclops of One-Eyed Pass", "Grizzly Bears"]],
    scripts: createRegistry([WINDSCOUTER_SCRIPT]),
