// `Noxious Field` - the carrier proven end to end (D367/D368) with a D384 payload: the recipient
...
import { NOXIOUS_FIELD_SCRIPT } from './noxiousField';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Noxious Field";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Noxious Field","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NOXIOUS_FIELD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Noxious Field", () => {
  test("grants {T}: This land deals 1 damage to each creature and each player. to Forest, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and the body hits the RECIPIENT', () => {
    ...
    settle(g);
    expect(g.state.cards[no]?.damage ?? 0).toBeGreaterThan(0);
