// `Kird Ape` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KIRD_APE_SCRIPT } from './kirdApe';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KIRD_APE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kird Ape", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KIRD_APE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kird Ape", () => {
  test("`Kird Ape` - as long as you control a Forest: absent while it is unmet, read once it holds", () => {
