// `Noggle Bandit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NOGGLE_BANDIT_SCRIPT } from './noggleBandit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Noggle Bandit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Noggle Bandit", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NOGGLE_BANDIT_SCRIPT]),
...
describe("Noggle Bandit", () => {
  test("This creature can't be blocked except by creatures with defender.: the block by a creature outside the exception (creatures with defender) is refused", () => {
