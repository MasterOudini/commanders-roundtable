// `Diversion Specialist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DIVERSION_SPECIALIST_SCRIPT } from './diversionSpecialist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Diversion Specialist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Diversion Specialist", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DIVERSION_SPECIALIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Diversion Specialist", () => {
  test("{1}, Sacrifice another creature or enchantment: the vocabulary resolves \"Exile the top card of your library. You may play it this turn.\"", () => {
