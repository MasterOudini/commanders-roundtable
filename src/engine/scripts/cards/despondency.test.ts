// `Despondency` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DESPONDENCY_SCRIPT } from './despondency';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Despondency";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DESPONDENCY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Despondency", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DESPONDENCY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Despondency", () => {
  test("Enchanted creature gets -2/-0.: the creature it is attached to reads it", () => {
    ...
    expect(pt(g, bearsB)).toEqual([0, 2]);
  });
