// `Rhox Veteran` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RHOX_VETERAN_SCRIPT } from './rhoxVeteran';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rhox Veteran";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RHOX_VETERAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rhox Veteran", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RHOX_VETERAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rhox Veteran", () => {
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"Tap target creature an opponent controls.\"", () => {
    const { g, no } = armed(1);
