// `Triton Waverider` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TRITON_WAVERIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Triton Waverider", "Grizzly Bears", "Nyxborn Courser"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRITON_WAVERIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Triton Waverider", () => {
  test("Constellation — Whenever an enchantment you control enters: it gains flying until end of turn", () => {
