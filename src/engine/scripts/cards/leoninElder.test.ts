// `Leonin Elder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LEONIN_ELDER_SCRIPT } from './leoninElder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Leonin Elder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Leonin Elder", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LEONIN_ELDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Leonin Elder", () => {
  test("Whenever an artifact enters: 1 life is gained", () => {
