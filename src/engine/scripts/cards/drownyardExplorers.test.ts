// `Drownyard Explorers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DROWNYARD_EXPLORERS_SCRIPT } from './drownyardExplorers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drownyard Explorers";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drownyard Explorers"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DROWNYARD_EXPLORERS_SCRIPT]),
  });
  holdEverywhere(g);
