// `Smoldering Tar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SMOLDERING_TAR_SCRIPT } from './smolderingTar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Smoldering Tar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Smoldering Tar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SMOLDERING_TAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Smoldering Tar", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Target player loses 1 life.\"", () => {
    const { g, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 + -1);
  });

  test("Sacrifice this enchantment: it deals 4 damage to the declared creature", () => {
