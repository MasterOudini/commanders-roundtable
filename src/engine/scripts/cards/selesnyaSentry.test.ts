// `Selesnya Sentry` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SELESNYA_SENTRY_SCRIPT } from './selesnyaSentry';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Selesnya Sentry";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Selesnya Sentry", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SELESNYA_SENTRY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Selesnya Sentry", () => {
  test("{5}{G}: it regenerates from the Bolt", () => {
