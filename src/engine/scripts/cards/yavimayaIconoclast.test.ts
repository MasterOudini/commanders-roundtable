// `Yavimaya Iconoclast` - every printed ability proven in its own game: the cost's mark, the pump
...
import { YAVIMAYA_ICONOCLAST_SCRIPT } from './yavimayaIconoclast';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Yavimaya Iconoclast";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([YAVIMAYA_ICONOCLAST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Yavimaya Iconoclast"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([YAVIMAYA_ICONOCLAST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Yavimaya Iconoclast", () => {
  test("When this creature enters: it gets +1/+1 and gains haste until end of turn", () => {
