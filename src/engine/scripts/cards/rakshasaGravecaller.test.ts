// `Rakshasa Gravecaller` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAKSHASA_GRAVECALLER_SCRIPT } from './rakshasaGravecaller';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rakshasa Gravecaller";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rakshasa Gravecaller"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAKSHASA_GRAVECALLER_SCRIPT]),
...
describe("Rakshasa Gravecaller", () => {
  test("When this creature exploits a creature: 2 tokens made", () => {
