// `Goblin Boom Keg` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_BOOM_KEG_SCRIPT } from './goblinBoomKeg';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Boom Keg";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Boom Keg", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_BOOM_KEG_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Boom Keg", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this artifact.\"", () => {
    const { g, self, board0 } = armed(0);
    expect(g.state.cards[self]?.zone.kind).toBe('graveyard');
    expect(onBoard(g)).toBe(board0 + -1);
  });

  test("When this artifact is put into a graveyard from the battlefield: it deals 3 damage to the declared creature", () => {
    const { g, no } = armed(1);
