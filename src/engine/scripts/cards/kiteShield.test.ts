// `Kite Shield` - equipped to Grizzly Bears the creature gets +0/+3; the Cyclops is untouched;
...
import { KITE_SHIELD_SCRIPT } from './kiteShield';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kite Shield";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KITE_SHIELD_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([KITE_SHIELD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kite Shield", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +0/+3; the Cyclops is untouched", () => {
