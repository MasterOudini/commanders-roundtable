// `Wall of Bone` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WALL_OF_BONE_SCRIPT } from './wallOfBone';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wall of Bone";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wall of Bone", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WALL_OF_BONE_SCRIPT]),
  });
  holdEverywhere(g);
