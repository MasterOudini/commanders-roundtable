// `Dauntless Cathar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAUNTLESS_CATHAR_SCRIPT } from './dauntlessCathar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dauntless Cathar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dauntless Cathar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAUNTLESS_CATHAR_SCRIPT]),
  });
  holdEverywhere(g);
