// `Ecologist's Terrarium` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ecologist's Terrarium", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ECOLOGISTS_TERRARIUM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ecologist's Terrarium", () => {
  test("When this artifact enters: the vocabulary resolves \"Search your library for a basic land card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
  ...
  test("{2}, {T}, Sacrifice this artifact: the declared creature gets 1 +1/+1 counter", () => {
