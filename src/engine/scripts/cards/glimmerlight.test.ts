// `Glimmerlight` - equipped to Grizzly Bears the creature (on entering: 1 token); gets +1/+1; the Cyclops is untouched;
...
import { GLIMMERLIGHT_SCRIPT } from './glimmerlight';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Glimmerlight";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GLIMMERLIGHT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GLIMMERLIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Glimmerlight", () => {
  test("equipped to Grizzly Bears: attached, and the creature (on entering: 1 token); gets +1/+1; the Cyclops is untouched", () => {
  ...
  test("entering: (on entering: 1 token)", () => {
    const g = startedGame({ players: 2, decks: [[CARD, "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]], scripts: createRegistry([GLIMMERLIGHT_SCRIPT]) });
    holdEverywhere(g);
