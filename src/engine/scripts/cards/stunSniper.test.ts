// `Stun Sniper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STUN_SNIPER_SCRIPT } from './stunSniper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stun Sniper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stun Sniper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STUN_SNIPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stun Sniper", () => {
  test("{1}, {T}: the vocabulary resolves \"~ deals 1 damage to target creature. Tap that creature.\"", () => {
