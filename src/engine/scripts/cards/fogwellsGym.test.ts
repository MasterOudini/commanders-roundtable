// `Fogwell's Gym` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FOGWELLS_GYM_SCRIPT } from './fogwellsGym';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fogwell's Gym";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fogwell's Gym", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOGWELLS_GYM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fogwell's Gym", () => {
  test("{2}{R}, {T}, Discard a card: a card is drawn", () => {
