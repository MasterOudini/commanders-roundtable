// `Wolfir Avenger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WOLFIR_AVENGER_SCRIPT } from './wolfirAvenger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wolfir Avenger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wolfir Avenger", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WOLFIR_AVENGER_SCRIPT]),
  });
  holdEverywhere(g);
