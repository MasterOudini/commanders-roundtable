// `Necrotic Sliver` - every permanent in its scope HAS a quoted
...
import { NECROTIC_SLIVER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(NECROTIC_SLIVER, "All Slivers have \"{3}, Sacrifice this permanent: Destroy target permanent.\"");

const GRANT = grantedActivated("{3}, Sacrifice this permanent: Destroy target permanent.", `${NECROTIC_SLIVER.oracleId}#g0`, NECROTIC_SLIVER.name);
const VOCAB = vocabularyEffects("Destroy target permanent.", NECROTIC_SLIVER.name);
const VOCAB_T = vocabularyTargets("Destroy target permanent.");

export const NECROTIC_SLIVER_SCRIPT: CardScript = {
  oracleId: NECROTIC_SLIVER.oracleId,
  name: NECROTIC_SLIVER.name,
  statics: [
    {
      abilityId: 'grant-0',
      text: PRINTED,
