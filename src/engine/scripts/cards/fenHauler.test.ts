// `Fen Hauler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FEN_HAULER_SCRIPT } from './fenHauler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fen Hauler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fen Hauler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Memnite"]],
    scripts: createRegistry([FEN_HAULER_SCRIPT]),
