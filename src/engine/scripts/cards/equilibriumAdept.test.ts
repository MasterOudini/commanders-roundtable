// `Equilibrium Adept` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EQUILIBRIUM_ADEPT_SCRIPT } from './equilibriumAdept';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Equilibrium Adept";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([EQUILIBRIUM_ADEPT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Equilibrium Adept", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EQUILIBRIUM_ADEPT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Equilibrium Adept", () => {
  test("When this creature enters: the vocabulary resolves \"Exile the top card of your library. Until the end of your next turn, you may play that card.\"", () => {
    const { g } = armed(0);
  ...
  test("Flurry — Whenever you cast your second spell each turn: it gains doubleStrike until end of turn", () => {
