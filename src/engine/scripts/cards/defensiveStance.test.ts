// `Defensive Stance` - cast on Grizzly Bears it attaches and the enchanted creature gets -1/+1;
...
import { DEFENSIVE_STANCE_SCRIPT } from './defensiveStance';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Defensive Stance";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DEFENSIVE_STANCE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([DEFENSIVE_STANCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Defensive Stance", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets -1/+1; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([1, 3]);
