// `Mantle of Webs` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+3 and has reach;
...
import { MANTLE_OF_WEBS_SCRIPT } from './mantleOfWebs';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mantle of Webs";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MANTLE_OF_WEBS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([MANTLE_OF_WEBS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mantle of Webs", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+3 and has reach; Cyclops of One-Eyed Pass is untouched", () => {
