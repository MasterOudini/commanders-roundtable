// `Prodigious Growth` - cast on Grizzly Bears it attaches and the enchanted creature gets +7/+7 and has trample;
...
import { PRODIGIOUS_GROWTH_SCRIPT } from './prodigiousGrowth';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prodigious Growth";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PRODIGIOUS_GROWTH_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([PRODIGIOUS_GROWTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Prodigious Growth", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +7/+7 and has trample; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([9, 9]);
