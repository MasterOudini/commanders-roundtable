// `Mesa Enchantress` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MESA_ENCHANTRESS_SCRIPT } from './mesaEnchantress';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mesa Enchantress";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mesa Enchantress", "Grizzly Bears", "Nyxborn Courser"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MESA_ENCHANTRESS_SCRIPT]),
  });
  holdEverywhere(g);
