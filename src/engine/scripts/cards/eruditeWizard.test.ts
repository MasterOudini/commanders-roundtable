// `Erudite Wizard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ERUDITE_WIZARD_SCRIPT } from './eruditeWizard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Erudite Wizard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Erudite Wizard", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ERUDITE_WIZARD_SCRIPT]),
  });
  holdEverywhere(g);
