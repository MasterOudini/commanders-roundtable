// `Havengul Vampire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HAVENGUL_VAMPIRE_SCRIPT } from './havengulVampire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Havengul Vampire";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Havengul Vampire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HAVENGUL_VAMPIRE_SCRIPT]),
  });
  holdEverywhere(g);
