// `Dreadwurm` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DREADWURM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dreadwurm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DREADWURM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dreadwurm", () => {
  test("Landfall — Whenever a land you control enters: it gains indestructible until end of turn", () => {
