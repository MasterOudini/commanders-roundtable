// `Bereavement` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BEREAVEMENT_SCRIPT } from './bereavement';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bereavement";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bereavement", "Grizzly Bears", "Dryad Arbor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BEREAVEMENT_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const bears = put(g, 'p1', "Dryad Arbor");
...
describe("Bereavement", () => {
  test("Whenever a green creature dies: the vocabulary resolves \"Target player discards a card.\"", () => {
    const { g } = armed(0);
