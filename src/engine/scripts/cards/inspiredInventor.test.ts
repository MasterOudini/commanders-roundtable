// `Inspired Inventor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INSPIRED_INVENTOR_SCRIPT } from './inspiredInventor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Inspired Inventor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Inspired Inventor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INSPIRED_INVENTOR_SCRIPT]),
...
describe("Inspired Inventor", () => {
  test("When this creature enters: choosing \"You get {E}{E}{E}                        .\", the vocabulary resolves \"You get {E}{E}{E}.\" [etb 1]", () => {
    const { g, energy0 } = armed(0);
    expect(g.state.players.p1?.energy).toBe(energy0 + 3);
  });

  test("When this creature enters: choosing \"Put a +1/+1 counter on target creature.\", counterOnTarget [etb 2]", () => {
    const { g, no } = armed(1);
    expect(g.state.cards[no]?.counters["+1/+1"] ?? 0).toBe(1);
  });

  test("When this creature enters: choosing \"Create a 1/1 colorless Servo artifact creature token.\", token [etb 3]", () => {
