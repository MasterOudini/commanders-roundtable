// `Chaos Imps` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHAOS_IMPS_SCRIPT } from './chaosImps';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chaos Imps";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHAOS_IMPS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chaos Imps"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHAOS_IMPS_SCRIPT]),
...
describe("Chaos Imps", () => {
  test("`Chaos Imps` - as long as it has a +1/+1 counter on it: absent while it is unmet, read once it holds", () => {
