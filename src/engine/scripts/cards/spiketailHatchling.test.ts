// `Spiketail Hatchling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIKETAIL_HATCHLING_SCRIPT } from './spiketailHatchling';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spiketail Hatchling";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spiketail Hatchling"], ["Cyclops of One-Eyed Pass", "Divination", "Forest"]],
    scripts: createRegistry([SPIKETAIL_HATCHLING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spiketail Hatchling", () => {
  test("Sacrifice this creature: the vocabulary resolves \"Counter target spell unless its controller pays {1}.\" - the price is declined", () => {
  ...
  test("Sacrifice this creature: the vocabulary resolves \"Counter target spell unless its controller pays {1}.\" - the price is paid", () => {
    const { g, self, payA0_p2_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
