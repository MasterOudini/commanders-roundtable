// `Scavenged Weaponry` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: draw 1); gets +1/+1;
...
import { SCAVENGED_WEAPONRY_SCRIPT } from './scavengedWeaponry';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scavenged Weaponry";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SCAVENGED_WEAPONRY_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SCAVENGED_WEAPONRY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Scavenged Weaponry", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: draw 1); gets +1/+1; Cyclops of One-Eyed Pass is untouched", () => {
