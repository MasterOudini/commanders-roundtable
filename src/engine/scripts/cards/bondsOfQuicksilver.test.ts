// `Bonds of Quicksilver` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BONDS_OF_QUICKSILVER_SCRIPT } from './bondsOfQuicksilver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bonds of Quicksilver";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bonds of Quicksilver", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BONDS_OF_QUICKSILVER_SCRIPT]),
  });
  holdEverywhere(g);
