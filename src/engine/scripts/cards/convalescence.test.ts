// `Convalescence` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONVALESCENCE_SCRIPT } from './convalescence';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Convalescence";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Convalescence"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONVALESCENCE_SCRIPT]),
  ...
  if (which === 0) {
    ...
    must(g.submit({ t: 'ManualSetLife', player: 'p1', target: 'p1', delta: 10 - (g.state.players.p1?.life ?? 0) }));
      settle(g);
...
describe("Convalescence", () => {
  test("At the beginning of your upkeep: 1 life is gained", () => {
