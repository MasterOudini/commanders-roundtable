// `Benthic Criminologists` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BENTHIC_CRIMINOLOGISTS_SCRIPT } from './benthicCriminologists';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Benthic Criminologists";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Benthic Criminologists", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BENTHIC_CRIMINOLOGISTS_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const priceL0_p1_0 = put(g, 'p1', "Sol Ring");
  settle(g);
...
describe("Benthic Criminologists", () => {
  test("Whenever this creature enters or attacks: the vocabulary resolves \"You may sacrifice an artifact. If you do, draw a card.\" - the price is declined [etb]", () => {
  ...
  test("Whenever this creature enters or attacks: the vocabulary resolves \"You may sacrifice an artifact. If you do, draw a card.\" - the price is paid [etb]", () => {
  ...
  test("Whenever this creature enters or attacks: the vocabulary resolves \"You may sacrifice an artifact. If you do, draw a card.\" - the price is declined [attacks]", () => {
  ...
  test("Whenever this creature enters or attacks: the vocabulary resolves \"You may sacrifice an artifact. If you do, draw a card.\" - the price is paid [attacks]", () => {
    const { g, board0, priceL0_p1_0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
    must(g.submit({ t: 'AnswerPayMana', player: "p1", pay: true, picks: [priceL0_p1_0] }));
