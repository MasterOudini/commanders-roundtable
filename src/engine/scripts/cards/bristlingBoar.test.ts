// `Bristling Boar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRISTLING_BOAR_SCRIPT } from './bristlingBoar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bristling Boar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bristling Boar", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([BRISTLING_BOAR_SCRIPT]),
  });
  holdEverywhere(g);
