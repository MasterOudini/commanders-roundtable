// `Plant Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PLANT_ELEMENTAL_SCRIPT } from './plantElemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Plant Elemental";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Plant Elemental", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PLANT_ELEMENTAL_SCRIPT]),
  });
  holdEverywhere(g);
