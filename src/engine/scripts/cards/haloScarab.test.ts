// `Halo Scarab` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HALO_SCARAB_SCRIPT } from './haloScarab';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Halo Scarab";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Halo Scarab"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HALO_SCARAB_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Halo Scarab", () => {
  test("{2}, Exile this card from your graveyard: 1 token made", () => {
