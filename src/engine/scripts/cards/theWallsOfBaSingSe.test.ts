// `The Walls of Ba Sing Se` - the grant reaches the permanent its scope names and not the other;
// it ends when the source leaves; replay equal (D300). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { THE_WALLS_OF_BA_SING_SE_SCRIPT } from './theWallsOfBaSingSe';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "The Walls of Ba Sing Se";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([THE_WALLS_OF_BA_SING_SE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["The Walls of Ba Sing Se", "Coral Eel"], ["Crimson Kobolds"]],
    scripts: createRegistry([THE_WALLS_OF_BA_SING_SE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("The Walls of Ba Sing Se", () => {
