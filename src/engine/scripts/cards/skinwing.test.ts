// `Skinwing` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKINWING_SCRIPT } from './skinwing';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SKINWING_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skinwing", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKINWING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skinwing", () => {
  ...
  test("Equipped creature gets +2/+2 and has flying.: the creature it is attached to reads it", () => {
