// `Horned Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HORNED_TROLL_SCRIPT } from './hornedTroll';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Horned Troll";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Horned Troll", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HORNED_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
