// `Siege-Gang Lieutenant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SIEGE_GANG_LIEUTENANT_SCRIPT } from './siegeGangLieutenant';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Siege-Gang Lieutenant";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; tok0: readonly InstanceId[]; fodder0: InstanceId[] };
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([SIEGE_GANG_LIEUTENANT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Siege-Gang Lieutenant", "Raging Goblin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIEGE_GANG_LIEUTENANT_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = [put(g, 'p1', "Raging Goblin")];
  settle(g);
  ...
  if (which === 0) {
    { advanceUntil(g, (s) => s.turn.turnNumber === 5 && s.turn.phase === 'precombatMain' && s.priority.player === 'p1' && s.priority.awaiting === null, 40_000);
      ...
      advanceUntil(g, (s) => s.turn.turnNumber === 5 && s.turn.step === 'beginCombat', 60000);
      settle(g);
    ...
    const cmd = (g.state.players.p1?.commanderIds ?? [])[0] as InstanceId;
    must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: cmd, to: { kind: 'battlefield', player: 'p1' } }));
      settle(g);
  }
  ...
  if (which === 0) {
    advanceUntil(g, (s) => s.turn.turnNumber === 7 && s.turn.step === 'beginCombat', 60000);
    settle(g);
    }
  if (which === 1) {
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, tok0, fodder0 };
}

describe("Siege-Gang Lieutenant", () => {
  test("Lieutenant — At the beginning of combat on your turn: the vocabulary resolves \"Create two 1/1 red Goblin creature tokens. Those tokens gain haste until end of turn.\"", () => {
    const { g, board0, tok0 } = armed(0);
    const made = tokensMade(g, tok0);
    expect(made, 'the tokens the fire made').toHaveLength(2);
    for (const id of made) expect(kw(g, id).has("haste"), "gained haste").toBe(true);
    expect(onBoard(g)).toBe(board0 + 2);
  });

  test("{2}, Sacrifice a Goblin: it deals 1 damage to the declared creature", () => {
    const { g, no, fodder0 } = armed(1);
