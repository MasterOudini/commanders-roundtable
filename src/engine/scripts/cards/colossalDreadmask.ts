// `Colossal Dreadmask` - a etb trigger germ, a static attachedStatic
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { COLOSSAL_DREADMASK, PHYREXIAN_GERM_TOKEN } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(COLOSSAL_DREADMASK, "Living weapon (When this Equipment enters, create a 0/0 black Phyrexian Germ creature token, then attach this to it.)\nEquipped creature gets +6/+6 and has trample.\nEquip {3}{G}{G}");
const LINES = PRINTED.split('\n');

export const COLOSSAL_DREADMASK_SCRIPT: CardScript = {
  oracleId: COLOSSAL_DREADMASK.oracleId,
  name: COLOSSAL_DREADMASK.name,
  triggers: [
    {
      abilityId: 'etb-0',
