// `Thundering Spineback` - a static anthem, an activation token
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { THUNDERING_SPINEBACK } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(THUNDERING_SPINEBACK, "Other Dinosaurs you control get +1/+1.\n{5}{G}: Create a 3/3 green Dinosaur creature token with trample. (It can deal excess combat damage to the player or planeswalker it's attacking.)");
const LINES = PRINTED.split('\n');
const TOKEN_0 = tokenRef("Dinosaur|3/3|G|Creature|trample");

export const THUNDERING_SPINEBACK_SCRIPT: CardScript = {
  oracleId: THUNDERING_SPINEBACK.oracleId,
  name: THUNDERING_SPINEBACK.name,
  activated: [
    {
      ref: `${THUNDERING_SPINEBACK.oracleId}#a0`,
