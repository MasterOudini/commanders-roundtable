// `Spawning Bed` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPAWNING_BED_SCRIPT } from './spawningBed';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spawning Bed";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spawning Bed"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPAWNING_BED_SCRIPT]),
...
describe("Spawning Bed", () => {
  test("{6}, {T}, Sacrifice this land: the vocabulary resolves \"Create three 1/1 colorless Eldrazi Scion creature tokens. They have \\\"Sacrifice this token: Add {C}.\\\"\"", () => {
