// `Necrosavant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NECROSAVANT_SCRIPT } from './necrosavant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Necrosavant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Necrosavant", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NECROSAVANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Necrosavant", () => {
  test("{3}{B}{B}, Sacrifice a creature: it returns from the graveyard to the battlefield - refused unless during your upkeep", () => {
