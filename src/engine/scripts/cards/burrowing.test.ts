// `Burrowing` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BURROWING_SCRIPT } from './burrowing';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Burrowing";
...
function lw(g: Game, id: InstanceId): readonly string[] {
  const d = deps(createRegistry([BURROWING_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Burrowing", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BURROWING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Burrowing", () => {
  test("Enchanted creature has mountainwalk. (It can't be blocked as long as defending player controls a Mountain.): the creature it is attached to reads it", () => {
