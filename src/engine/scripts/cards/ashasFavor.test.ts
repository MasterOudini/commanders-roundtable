// `Asha's Favor` - cast on Grizzly Bears it attaches and the enchanted creature has flying, firstStrike, vigilance;
...
import { ASHAS_FAVOR_SCRIPT } from './ashasFavor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Asha's Favor";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([ASHAS_FAVOR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ASHAS_FAVOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Asha's Favor", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has flying, firstStrike, vigilance; Cyclops of One-Eyed Pass is untouched", () => {
