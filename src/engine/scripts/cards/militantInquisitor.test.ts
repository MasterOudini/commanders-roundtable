// `Militant Inquisitor` - the pump is COUNTED: zero of them is no bonus, and each one moves the
// derived power and toughness by the printed delta. Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { MILITANT_INQUISITOR_SCRIPT } from './militantInquisitor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Militant Inquisitor";
const COUNTED = "Accorder's Shield";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MILITANT_INQUISITOR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; subject: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([MILITANT_INQUISITOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Militant Inquisitor", () => {
  test("each Accorder's Shield counted moves the P/T by [1, 0], and one leaving takes it back", () => {
