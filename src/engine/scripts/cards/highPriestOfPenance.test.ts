// `High Priest of Penance` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HIGH_PRIEST_OF_PENANCE_SCRIPT } from './highPriestOfPenance';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "High Priest of Penance";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["High Priest of Penance", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([HIGH_PRIEST_OF_PENANCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("High Priest of Penance", () => {
  test("Whenever this creature is dealt damage: the vocabulary resolves \"Destroy target nonland permanent.\"", () => {
    const { g, no } = armed(0);
