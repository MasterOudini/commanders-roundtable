// `Kemuri-Onna` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KEMURI_ONNA_SCRIPT } from './kemuriOnna';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kemuri-Onna";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kemuri-Onna", "Cloud Spirit"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KEMURI_ONNA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kemuri-Onna", () => {
  test("When this creature enters: the vocabulary resolves \"Target player discards a card.\"", () => {
    const { g } = armed(0);
