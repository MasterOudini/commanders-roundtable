// `Rancor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RANCOR_SCRIPT } from './rancor';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RANCOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rancor", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RANCOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rancor", () => {
  test("Enchanted creature gets +2/+0 and has trample.: the creature it is attached to reads it", () => {
