// `Dauntless Veteran` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAUNTLESS_VETERAN_SCRIPT } from './dauntlessVeteran';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dauntless Veteran";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DAUNTLESS_VETERAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dauntless Veteran", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAUNTLESS_VETERAN_SCRIPT]),
  });
  holdEverywhere(g);
