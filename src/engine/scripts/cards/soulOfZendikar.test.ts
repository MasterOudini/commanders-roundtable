// `Soul of Zendikar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOUL_OF_ZENDIKAR_SCRIPT } from './soulOfZendikar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Soul of Zendikar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soul of Zendikar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOUL_OF_ZENDIKAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Soul of Zendikar", () => {
  test("{3}{G}{G}: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("{3}{G}{G}, Exile this card from your graveyard: 1 token made", () => {
