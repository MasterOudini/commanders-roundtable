// `Summon: Anima` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUMMON_ANIMA_SCRIPT } from './summonAnima';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Summon: Anima";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Summon: Anima"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUMMON_ANIMA_SCRIPT]),
...
describe("Summon: Anima", () => {
  test("I: a card is drawn and 1 life lost", () => {
    const { g, life0, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1);
    expect(g.state.players.p1?.life).toBe(life0 - 1);
  });

  test("IV — Oblivion — Each opponent sacrifices a creature of their choice and loses 3 life.: the vocabulary resolves \"Each opponent sacrifices a creature of their choice and loses 3 life.\"", () => {
