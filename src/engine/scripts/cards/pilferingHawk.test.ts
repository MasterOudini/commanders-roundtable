// `Pilfering Hawk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PILFERING_HAWK_SCRIPT } from './pilferingHawk';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pilfering Hawk";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pilfering Hawk", "Snow-Covered Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PILFERING_HAWK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pilfering Hawk", () => {
  test("{S}, {T}: a card is drawn, then one discarded", () => {
