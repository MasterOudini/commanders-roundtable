// `Seeker of Insight` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEEKER_OF_INSIGHT_SCRIPT } from './seekerOfInsight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Seeker of Insight";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seeker of Insight", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEEKER_OF_INSIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Seeker of Insight", () => {
  test("{T}: a card is drawn, then one discarded - refused unless if you've cast a noncreature spell this turn", () => {
    const { g, self, hand0, gy0, condRefused } = armed(0);
