// `Spiketail Drakeling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIKETAIL_DRAKELING_SCRIPT } from './spiketailDrakeling';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spiketail Drakeling";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; vtA0_0: InstanceId; payA0_p2_0: InstanceId; payA0_p2_1: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spiketail Drakeling"], ["Cyclops of One-Eyed Pass", "Divination", "Forest", "Forest"]],
    scripts: createRegistry([SPIKETAIL_DRAKELING_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, vtA0_0, payA0_p2_0, payA0_p2_1 };
}

describe("Spiketail Drakeling", () => {
  test("Sacrifice this creature: the vocabulary resolves \"Counter target spell unless its controller pays {2}.\" - the price is declined", () => {
    const { g, self, payA0_p2_0, payA0_p2_1 } = armed(0);
  ...
  test("Sacrifice this creature: the vocabulary resolves \"Counter target spell unless its controller pays {2}.\" - the price is paid", () => {
    const { g, self, payA0_p2_0, payA0_p2_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
