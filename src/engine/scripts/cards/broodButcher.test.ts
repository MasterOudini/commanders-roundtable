// `Brood Butcher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BROOD_BUTCHER_SCRIPT } from './broodButcher';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brood Butcher";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BROOD_BUTCHER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brood Butcher", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BROOD_BUTCHER_SCRIPT]),
...
describe("Brood Butcher", () => {
  test("When this creature enters: the vocabulary resolves \"Create a 1/1 colorless Eldrazi Scion creature token. It has \\\"Sacrifice this token: Add {C}.\\\"\"", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("{B}{G}, Sacrifice a creature: the declared creature is pumped until end of turn", () => {
