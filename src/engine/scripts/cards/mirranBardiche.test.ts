// `Mirran Bardiche` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MIRRAN_BARDICHE_SCRIPT } from './mirranBardiche';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mirran Bardiche";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MIRRAN_BARDICHE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mirran Bardiche", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIRRAN_BARDICHE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mirran Bardiche", () => {
  test("For Mirrodin! (When this Equipment enters: a token is made and it attaches to it", () => {
    ...
    if (germ !== null) expect(pt(g, germ)).toEqual([4, 3]);
  });

  test("Equipped creature gets +2/+1 and has vigilance.: the creature it is attached to reads it", () => {
