// `Fledgling Mawcor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLEDGLING_MAWCOR_SCRIPT } from './fledglingMawcor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fledgling Mawcor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fledgling Mawcor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLEDGLING_MAWCOR_SCRIPT]),
  });
  holdEverywhere(g);
