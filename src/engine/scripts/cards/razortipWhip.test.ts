// `Razortip Whip` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAZORTIP_WHIP_SCRIPT } from './razortipWhip';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Razortip Whip";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Razortip Whip"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAZORTIP_WHIP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Razortip Whip", () => {
  test("{1}, {T}: the vocabulary resolves \"~ deals 1 damage to target opponent or planeswalker.\"", () => {
