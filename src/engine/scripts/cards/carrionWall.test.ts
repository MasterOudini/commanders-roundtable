// `Carrion Wall` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CARRION_WALL_SCRIPT } from './carrionWall';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Carrion Wall";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Carrion Wall", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CARRION_WALL_SCRIPT]),
  });
  holdEverywhere(g);
