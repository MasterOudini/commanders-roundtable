// `Gateway Plaza` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GATEWAY_PLAZA_SCRIPT } from './gatewayPlaza';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gateway Plaza";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gateway Plaza", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GATEWAY_PLAZA_SCRIPT]),
  });
  holdEverywhere(g);
