// `Lat-Nam Adept` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LAT_NAM_ADEPT_SCRIPT } from './latNamAdept';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lat-Nam Adept";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lat-Nam Adept", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LAT_NAM_ADEPT_SCRIPT]),
  });
  holdEverywhere(g);
