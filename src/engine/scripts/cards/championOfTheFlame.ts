// `Champion of the Flame` - a VARIABLE PUMP (D387): the delta is COUNTED at each derive
...
import { CHAMPION_OF_THE_FLAME } from '../../../data/fixtures/engineCards';
...
function countOf(ctx: ScriptCtx, self: InstanceId, applied: InstanceId): number {
  ...
  for (const inst of Object.values(ctx.state.cards)) {
    ...
    if (!face) continue;
    if (!(face.typeLine.subtypes.includes("Aura") || face.typeLine.subtypes.includes("Equipment"))) continue;
    n++;
  }
  return n;
}

export const CHAMPION_OF_THE_FLAME_SCRIPT: CardScript = {
  oracleId: CHAMPION_OF_THE_FLAME.oracleId,
  name: CHAMPION_OF_THE_FLAME.name,
  statics: [
    {
      abilityId: 'pump-1',
