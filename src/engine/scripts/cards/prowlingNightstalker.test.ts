// `Prowling Nightstalker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PROWLING_NIGHTSTALKER_SCRIPT } from './prowlingNightstalker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prowling Nightstalker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prowling Nightstalker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PROWLING_NIGHTSTALKER_SCRIPT]),
...
describe("Prowling Nightstalker", () => {
  test("This creature can't be blocked except by black creatures.: the block by a creature outside the exception (black creatures) is refused", () => {
