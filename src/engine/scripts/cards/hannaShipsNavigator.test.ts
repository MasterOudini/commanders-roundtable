// `Hanna, Ship's Navigator` - the named target is accepted and it returns to hand; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { HANNA_SHIPS_NAVIGATOR_SCRIPT } from './hannaShipsNavigator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hanna, Ship's Navigator";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Hanna, Ship's Navigator", "Sol Ring", "Grizzly Bears"], ["Grizzly Bears"]],
    scripts: createRegistry([HANNA_SHIPS_NAVIGATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hanna, Ship's Navigator", () => {
