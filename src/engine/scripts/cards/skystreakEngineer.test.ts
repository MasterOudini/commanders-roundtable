// `Skystreak Engineer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKYSTREAK_ENGINEER_SCRIPT } from './skystreakEngineer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skystreak Engineer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skystreak Engineer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKYSTREAK_ENGINEER_SCRIPT]),
...
describe("Skystreak Engineer", () => {
  test("Exhaust — {4}{U}: it gets 2 +1/+1 counters", () => {
