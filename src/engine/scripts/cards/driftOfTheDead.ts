// `Drift of the Dead` - a static cdaCount
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { DRIFT_OF_THE_DEAD } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(DRIFT_OF_THE_DEAD, "Defender (This creature can't attack.)\nDrift of the Dead's power and toughness are each equal to the number of snow lands you control.");
const LINES = PRINTED.split('\n');

// "snow lands you control", read off the printed faces (a count is not a characteristic, CR 604.3).
...
export const DRIFT_OF_THE_DEAD_SCRIPT: CardScript = {
  oracleId: DRIFT_OF_THE_DEAD.oracleId,
  name: DRIFT_OF_THE_DEAD.name,
  statics: [
    {
      abilityId: 'cda-1',
