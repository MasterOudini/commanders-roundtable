// `Erdwal Ripper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ERDWAL_RIPPER_SCRIPT } from './erdwalRipper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Erdwal Ripper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Erdwal Ripper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ERDWAL_RIPPER_SCRIPT]),
  });
  holdEverywhere(g);
