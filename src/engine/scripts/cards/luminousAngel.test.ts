// `Luminous Angel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LUMINOUS_ANGEL_SCRIPT } from './luminousAngel';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Luminous Angel";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Luminous Angel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LUMINOUS_ANGEL_SCRIPT]),
  });
  holdEverywhere(g);
