// `Teferi, Timebender` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEFERI_TIMEBENDER_SCRIPT } from './teferiTimebender';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Teferi, Timebender";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Teferi, Timebender", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEFERI_TIMEBENDER_SCRIPT]),
...
describe("Teferi, Timebender", () => {
  test("+2: the vocabulary resolves \"Untap up to one target artifact or creature.\"", () => {
  ...
  test("−3: the vocabulary resolves \"You gain 2 life and draw two cards.\"", () => {
    const { g, self, life0, hand0, refused } = armed(1);
  ...
  test("−9: the vocabulary resolves \"Take an extra turn after this one.\"", () => {
