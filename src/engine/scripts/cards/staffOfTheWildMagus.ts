// Staff of the Wild Magus - the green/Forest member of the five-Staff
// family (see staffOfTheDeathMagus.ts). D252.

import { STAFF_OF_THE_WILD_MAGUS } from '../../../data/fixtures/engineCards';
...
function landQualifies(ctx: ScriptCtx, self: InstanceId, entrant: InstanceId): boolean {
  ...
  return ctx.derive(entrant).typeLine.subtypes.includes('Forest');
}
...
export const STAFF_OF_THE_WILD_MAGUS_SCRIPT: CardScript = {
  oracleId: STAFF_OF_THE_WILD_MAGUS.oracleId,
  name: STAFF_OF_THE_WILD_MAGUS.name,
  triggers: [
    {
      abilityId: 'cast',
      text: TEXT,
      ...
      matches: (ctx, self, ev) => {
        ...
        if (!oc) return false;
        return faceOf(oc, ev.obj.faceIndex).colors.includes('G');
      },
      label: () => 'Staff of the Wild Magus — you gain 1 life',
