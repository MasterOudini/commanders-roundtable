// `Tunnel Rats` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TUNNEL_RATS_SCRIPT } from './tunnelRats';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tunnel Rats";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tunnel Rats"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TUNNEL_RATS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Tunnel Rats", () => {
  test("{4}{B}: it returns from the graveyard to the battlefield tapped", () => {
