// `Lifespinner` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lifespinner", "Bile Urchin", "Bile Urchin", "Bile Urchin", "Kataki, War's Wage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIFESPINNER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = [put(g, 'p1', "Bile Urchin"), put(g, 'p1', "Bile Urchin"), put(g, 'p1', "Bile Urchin")];
  settle(g);
...
describe("Lifespinner", () => {
  test("{T}, Sacrifice three Spirits: the vocabulary resolves \"Search your library for a legendary Spirit permanent card, put it onto the battlefield, then shuffle.\"", () => {
    const { g, self, board0, fodder0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Kataki, War's Wage" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Kataki, War's Wage");
      expect(found, "Kataki, War's Wage is not in the library for the search to find").toBeDefined();
