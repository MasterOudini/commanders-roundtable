// `Settlement Blacksmith` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SETTLEMENT_BLACKSMITH_SCRIPT } from './settlementBlacksmith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Settlement Blacksmith";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Settlement Blacksmith", "Swiftfoot Boots"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SETTLEMENT_BLACKSMITH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Settlement Blacksmith", () => {
