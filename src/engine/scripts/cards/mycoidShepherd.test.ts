// `Mycoid Shepherd` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MYCOID_SHEPHERD_SCRIPT } from './mycoidShepherd';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mycoid Shepherd";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mycoid Shepherd", "Grizzly Bears", "Kalakscion, Hunger Tyrant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MYCOID_SHEPHERD_SCRIPT]),
  ...
  if (which === 1) {
    const bears = put(g, 'p1', "Kalakscion, Hunger Tyrant");
...
describe("Mycoid Shepherd", () => {
  test("Whenever this creature or another creature you control with power 5 or greater dies: 5 life is gained [dies]", () => {
    const { g, life0 } = armed(0);
    expect(g.state.players.p1?.life).toBe(life0 + 5);
  });

  test("Whenever this creature or another creature you control with power 5 or greater dies: 5 life is gained [anotherCreatureDies]", () => {
