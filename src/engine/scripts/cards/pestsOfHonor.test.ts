// `Pests of Honor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PESTS_OF_HONOR_SCRIPT } from './pestsOfHonor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pests of Honor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pests of Honor", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PESTS_OF_HONOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pests of Honor", () => {
  test("Celebration — At the beginning of combat on your turn: it gets 1 +1/+1 counter", () => {
