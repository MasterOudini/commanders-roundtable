// `Elven Warhounds` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ELVEN_WARHOUNDS_SCRIPT } from './elvenWarhounds';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Elven Warhounds";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elven Warhounds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELVEN_WARHOUNDS_SCRIPT]),
...
describe("Elven Warhounds", () => {
  test("Whenever this creature becomes blocked by a creature: the vocabulary resolves \"Put target creature on top of its owner's library.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.zone.kind).toBe('library');
  });
