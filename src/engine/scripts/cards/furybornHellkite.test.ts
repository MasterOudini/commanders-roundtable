// `Furyborn Hellkite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FURYBORN_HELLKITE_SCRIPT } from './furybornHellkite';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Furyborn Hellkite";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FURYBORN_HELLKITE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Furyborn Hellkite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FURYBORN_HELLKITE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Furyborn Hellkite", () => {
  test("Bloodthirst 6 (If an opponent was dealt damage this turn, this creature enters with six +1/+1 counters on it.): cast, it enters with 6 +1/+1 counters", () => {
