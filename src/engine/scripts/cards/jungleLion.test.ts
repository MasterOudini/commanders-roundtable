// `Jungle Lion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JUNGLE_LION_SCRIPT } from './jungleLion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jungle Lion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jungle Lion", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JUNGLE_LION_SCRIPT]),
  });
  holdEverywhere(g);
