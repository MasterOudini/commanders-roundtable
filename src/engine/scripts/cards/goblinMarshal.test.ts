// `Goblin Marshal` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Marshal", "Forest", "Forest", "Forest", "Forest", "Mountain", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_MARSHAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Marshal", () => {
  test("When this creature enters or dies: 2 tokens made [etb]", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 2 + 1);
  });

  test("When this creature enters or dies: 2 tokens made [dies]", () => {
