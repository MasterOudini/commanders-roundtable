// `Greatsword` - equipped to Grizzly Bears the creature gets +3/+0; the Cyclops is untouched;
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GREATSWORD_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GREATSWORD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Greatsword", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +3/+0; the Cyclops is untouched", () => {
