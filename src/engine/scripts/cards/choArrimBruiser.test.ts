// `Cho-Arrim Bruiser` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHO_ARRIM_BRUISER_SCRIPT } from './choArrimBruiser';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cho-Arrim Bruiser";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cho-Arrim Bruiser"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHO_ARRIM_BRUISER_SCRIPT]),
...
describe("Cho-Arrim Bruiser", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Tap up to two target creatures.\"", () => {
    const { g, no } = armed(0);
