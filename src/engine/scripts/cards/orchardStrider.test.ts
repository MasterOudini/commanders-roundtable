// `Orchard Strider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORCHARD_STRIDER_SCRIPT } from './orchardStrider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Orchard Strider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Orchard Strider"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORCHARD_STRIDER_SCRIPT]),
  });
  holdEverywhere(g);
