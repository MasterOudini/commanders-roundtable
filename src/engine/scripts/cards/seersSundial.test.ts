// `Seer's Sundial` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEERS_SUNDIAL_SCRIPT } from './seersSundial';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Seer's Sundial";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seer's Sundial", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEERS_SUNDIAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Seer's Sundial", () => {
  test("Landfall — Whenever a land you control enters: the vocabulary resolves \"You may pay {2}. If you do, draw a card.\" - the price is declined", () => {
  ...
  test("Landfall — Whenever a land you control enters: the vocabulary resolves \"You may pay {2}. If you do, draw a card.\" - the price is paid", () => {
    const { g, payL0_p1_0, payL0_p1_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
