// `Battle-Mad Ronin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BATTLE_MAD_RONIN_SCRIPT } from './battleMadRonin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Battle-Mad Ronin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Battle-Mad Ronin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BATTLE_MAD_RONIN_SCRIPT]),
  });
  holdEverywhere(g);
