// `Retreat to Emeria` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RETREAT_TO_EMERIA_SCRIPT } from './retreatToEmeria';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Retreat to Emeria";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RETREAT_TO_EMERIA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Retreat to Emeria", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RETREAT_TO_EMERIA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Retreat to Emeria", () => {
  test("Landfall — Whenever a land you control enters: choosing \"Create a 1/1 white Kor Ally creature token.\", token", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("Landfall — Whenever a land you control enters: choosing \"Creatures you control get +1/+1 until end of turn.\", mass", () => {
