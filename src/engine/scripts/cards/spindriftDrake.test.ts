// `Spindrift Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPINDRIFT_DRAKE_SCRIPT } from './spindriftDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spindrift Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spindrift Drake", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPINDRIFT_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
