// `Chromatic Star` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHROMATIC_STAR_SCRIPT } from './chromaticStar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Chromatic Star";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Chromatic Star", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHROMATIC_STAR_SCRIPT]),
  });
  holdEverywhere(g);
