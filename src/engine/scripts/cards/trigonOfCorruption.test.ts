// `Trigon of Corruption` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRIGON_OF_CORRUPTION_SCRIPT } from './trigonOfCorruption';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trigon of Corruption";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TRIGON_OF_CORRUPTION_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trigon of Corruption"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRIGON_OF_CORRUPTION_SCRIPT]),
...
describe("Trigon of Corruption", () => {
  test("This artifact enters with three charge counters on it.: cast, it enters with 3 charge counters", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.zone.kind).toBe('battlefield');
    expect(g.state.cards[self]?.counters["charge"] ?? 0).toBe(3);
  });

  test("{B}{B}, {T}: it gets 1 charge counter", () => {
    const { g, self, cnt0 } = armed(1);
    expect(g.state.cards[self]?.counters["charge"] ?? 0).toBe(cnt0 + 1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{2}, {T}, Remove a charge counter from this artifact: the declared creature gets 1 -1/-1 counter", () => {
    const { g, self, no, cnt0 } = armed(2);
