// `Bloodhall Ooze` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODHALL_OOZE_SCRIPT } from './bloodhallOoze';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodhall Ooze";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodhall Ooze", "Muck Rats", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODHALL_OOZE_SCRIPT]),
