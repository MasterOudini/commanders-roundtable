// `Burden of Guilt` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BURDEN_OF_GUILT_SCRIPT } from './burdenOfGuilt';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Burden of Guilt";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Burden of Guilt", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BURDEN_OF_GUILT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Burden of Guilt", () => {
  test("{1}: the enchanted creature is tapped", () => {
