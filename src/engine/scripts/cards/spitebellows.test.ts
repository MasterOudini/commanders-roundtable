// `Spitebellows` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPITEBELLOWS_SCRIPT } from './spitebellows';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spitebellows";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spitebellows"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPITEBELLOWS_SCRIPT]),
...
describe("Spitebellows", () => {
  test("When this creature leaves the battlefield: it deals 6 damage to the declared creature", () => {
    const { g, no } = armed(0);
