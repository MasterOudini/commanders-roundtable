// `War Horn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WAR_HORN_SCRIPT } from './warHorn';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WAR_HORN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["War Horn", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WAR_HORN_SCRIPT]),
  });
  holdEverywhere(g);
