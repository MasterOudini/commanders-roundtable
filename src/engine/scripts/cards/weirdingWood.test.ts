// `Weirding Wood` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WEIRDING_WOOD_SCRIPT } from './weirdingWood';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Weirding Wood";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Weirding Wood", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WEIRDING_WOOD_SCRIPT]),
...
describe("Weirding Wood", () => {
  test("When this Aura enters: 1 token made", () => {
  ...
  test("Enchanted land has \"{T}: the creature it is attached to reads it", () => {
    ...
    { const d = deps(createRegistry([WEIRDING_WOOD_SCRIPT])); const prod = derive(g.state, d.oracle, d.scripts, bearsB).producesMana.find((p) => p.text === "{T}: Add two mana of any one color."); expect(prod, 'the host has the granted mana ability').toBeDefined(); const pool0 = poolTotal(g); must(g.submit({ t: 'TapForMana', player: 'p1', card: bearsB, abilityIndex: prod?.abilityIndex ?? -1, outputChoice: 0 })); expect(g.state.cards[bearsB]?.tapped).toBe(true); expect(poolTotal(g)).toBeGreaterThan(pool0); }
  });
