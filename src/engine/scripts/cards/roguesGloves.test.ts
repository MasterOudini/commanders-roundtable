// `Rogue's Gloves` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROGUES_GLOVES_SCRIPT } from './roguesGloves';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rogue's Gloves";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rogue's Gloves", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROGUES_GLOVES_SCRIPT]),
  });
  holdEverywhere(g);
