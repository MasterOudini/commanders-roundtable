// `Renegade Map` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Renegade Map", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RENEGADE_MAP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Renegade Map", () => {
  test("{T}, Sacrifice this artifact: the vocabulary resolves \"Search your library for a basic land card, reveal it, put it into your hand, then shuffle.\"", () => {
