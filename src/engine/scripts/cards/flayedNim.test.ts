// `Flayed Nim` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLAYED_NIM_SCRIPT } from './flayedNim';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flayed Nim";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flayed Nim", "Lightning Bolt", "Hill Giant"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([FLAYED_NIM_SCRIPT]),
...
describe("Flayed Nim", () => {
  test("Whenever this creature deals combat damage to a creature: the vocabulary resolves \"Target player loses that much life.\"", () => {
    const { g, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 + -2);
  });

  test("{2}{B}: it regenerates from the Bolt", () => {
