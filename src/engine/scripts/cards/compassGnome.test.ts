// `Compass Gnome` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Compass Gnome", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COMPASS_GNOME_SCRIPT]),
...
describe("Compass Gnome", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a basic land card or Cave card, reveal it, then shuffle and put that card on top.\"", () => {
    const { g } = armed(0);
