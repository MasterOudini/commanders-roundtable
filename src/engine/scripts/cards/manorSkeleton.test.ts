// `Manor Skeleton` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MANOR_SKELETON_SCRIPT } from './manorSkeleton';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Manor Skeleton";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Manor Skeleton", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MANOR_SKELETON_SCRIPT]),
  });
  holdEverywhere(g);
