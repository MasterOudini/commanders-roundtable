// `Thriving Ibex` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THRIVING_IBEX_SCRIPT } from './thrivingIbex';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thriving Ibex";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thriving Ibex"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THRIVING_IBEX_SCRIPT]),
