// `Slash, Reptile Rampager` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLASH_REPTILE_RAMPAGER_SCRIPT } from './slashReptileRampager';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slash, Reptile Rampager";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slash, Reptile Rampager", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLASH_REPTILE_RAMPAGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Slash, Reptile Rampager", () => {
  test("Alliance — Whenever another creature you control enters: it deals 2 damage to each opponent", () => {
    const { g, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 - 2);
  });

  test("Whenever Slash attacks: 1 token made", () => {
