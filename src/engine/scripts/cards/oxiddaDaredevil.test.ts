// `Oxidda Daredevil` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OXIDDA_DAREDEVIL_SCRIPT } from './oxiddaDaredevil';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Oxidda Daredevil";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([OXIDDA_DAREDEVIL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Oxidda Daredevil", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OXIDDA_DAREDEVIL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Oxidda Daredevil", () => {
  test("Sacrifice an artifact: it gains haste until end of turn", () => {
