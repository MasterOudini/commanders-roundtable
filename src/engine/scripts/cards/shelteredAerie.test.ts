// `Sheltered Aerie` - the granted MANA ability proven end to end (D372): the recipient HAS the
...
import { SHELTERED_AERIE_SCRIPT } from './shelteredAerie';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sheltered Aerie";
const QUOTED = "{T}: Add two mana of any one color.";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Sheltered Aerie","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHELTERED_AERIE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sheltered Aerie", () => {
  test("grants {T}: Add two mana of any one color. to Forest, and to nothing outside the scope", () => {
