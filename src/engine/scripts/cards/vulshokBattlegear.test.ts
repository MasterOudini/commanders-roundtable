// `Vulshok Battlegear` - equipped to Grizzly Bears the creature gets +3/+3; the Cyclops is untouched;
...
import { VULSHOK_BATTLEGEAR_SCRIPT } from './vulshokBattlegear';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vulshok Battlegear";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VULSHOK_BATTLEGEAR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([VULSHOK_BATTLEGEAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vulshok Battlegear", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +3/+3; the Cyclops is untouched", () => {
