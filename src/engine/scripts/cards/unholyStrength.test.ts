// `Unholy Strength` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+1;
...
import { UNHOLY_STRENGTH_SCRIPT } from './unholyStrength';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unholy Strength";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([UNHOLY_STRENGTH_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([UNHOLY_STRENGTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Unholy Strength", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+1; Cyclops of One-Eyed Pass is untouched", () => {
