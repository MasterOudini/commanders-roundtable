// `Sanguinary Priest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SANGUINARY_PRIEST_SCRIPT } from './sanguinaryPriest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sanguinary Priest";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sanguinary Priest", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SANGUINARY_PRIEST_SCRIPT]),
...
describe("Sanguinary Priest", () => {
  test("Blood Chalice — Whenever another creature you control dies: it deals 1 damage to the declared creature", () => {
    const { g, no } = armed(0);
