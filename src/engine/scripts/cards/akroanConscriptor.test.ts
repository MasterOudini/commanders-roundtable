// `Akroan Conscriptor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AKROAN_CONSCRIPTOR_SCRIPT } from './akroanConscriptor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Akroan Conscriptor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([AKROAN_CONSCRIPTOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Akroan Conscriptor", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AKROAN_CONSCRIPTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Akroan Conscriptor", () => {
  test("Heroic — Whenever you cast a spell that targets this creature: the vocabulary resolves \"Gain control of another target creature until end of turn. Untap that creature. It gains haste until end of turn.\"", () => {
