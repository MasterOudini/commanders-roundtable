// `Pyknite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PYKNITE_SCRIPT } from './pyknite';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pyknite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PYKNITE_SCRIPT]),
  });
  holdEverywhere(g);
