// `Foratog` - the printed cost buys the pump until end of turn; it ends at cleanup;
// replay equal (D301). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { FORATOG_SCRIPT } from './foratog';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FORATOG_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Foratog", "Forest"], ["Grizzly Bears"]],
    scripts: createRegistry([FORATOG_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Foratog", () => {
  test("{G}, Sacrifice a Forest: it gets +2/+2 until end of turn", () => {
