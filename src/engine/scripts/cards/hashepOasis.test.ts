// `Hashep Oasis` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HASHEP_OASIS_SCRIPT } from './hashepOasis';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hashep Oasis";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HASHEP_OASIS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hashep Oasis", "Bucolic Ranch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HASHEP_OASIS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hashep Oasis", () => {
  test("{1}{G}{G}, {T}, Sacrifice a Desert: the declared creature is pumped until end of turn", () => {
