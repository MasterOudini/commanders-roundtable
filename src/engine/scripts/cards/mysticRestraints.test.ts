// `Mystic Restraints` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MYSTIC_RESTRAINTS_SCRIPT } from './mysticRestraints';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mystic Restraints";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mystic Restraints", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MYSTIC_RESTRAINTS_SCRIPT]),
  });
  holdEverywhere(g);
