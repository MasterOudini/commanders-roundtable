// `Wu Longbowman` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WU_LONGBOWMAN_SCRIPT } from './wuLongbowman';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wu Longbowman";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wu Longbowman"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WU_LONGBOWMAN_SCRIPT]),
  });
  holdEverywhere(g);
