// `Whirler Virtuoso` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WHIRLER_VIRTUOSO_SCRIPT } from './whirlerVirtuoso';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Whirler Virtuoso";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Whirler Virtuoso"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WHIRLER_VIRTUOSO_SCRIPT]),
