// `Indebted Samurai` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INDEBTED_SAMURAI_SCRIPT } from './indebtedSamurai';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Indebted Samurai";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Indebted Samurai", "Grizzly Bears", "Devoted Retainer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INDEBTED_SAMURAI_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const bears = put(g, 'p1', "Devoted Retainer");
...
describe("Indebted Samurai", () => {
  test("Whenever a Samurai you control dies: it gets 1 +1/+1 counter", () => {
