// `Flaming Sword` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+0 and has firstStrike;
...
import { FLAMING_SWORD_SCRIPT } from './flamingSword';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flaming Sword";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FLAMING_SWORD_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FLAMING_SWORD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Flaming Sword", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+0 and has firstStrike; Cyclops of One-Eyed Pass is untouched", () => {
