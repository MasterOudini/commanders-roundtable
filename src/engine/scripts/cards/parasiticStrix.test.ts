// `Parasitic Strix` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PARASITIC_STRIX_SCRIPT } from './parasiticStrix';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Parasitic Strix";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Parasitic Strix", "Muck Rats"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PARASITIC_STRIX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Parasitic Strix", () => {
  test("When this creature enters: the vocabulary resolves \"Target player loses 2 life and you gain 2 life.\"", () => {
