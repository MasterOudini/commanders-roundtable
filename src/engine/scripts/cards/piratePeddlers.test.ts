// `Pirate Peddlers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PIRATE_PEDDLERS_SCRIPT } from './piratePeddlers';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pirate Peddlers";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([PIRATE_PEDDLERS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pirate Peddlers", "Lotus Petal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PIRATE_PEDDLERS_SCRIPT]),
  });
  holdEverywhere(g);
