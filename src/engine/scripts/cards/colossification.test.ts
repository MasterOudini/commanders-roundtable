// `Colossification` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COLOSSIFICATION_SCRIPT } from './colossification';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Colossification";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([COLOSSIFICATION_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Colossification", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COLOSSIFICATION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Colossification", () => {
  ...
  test("Enchanted creature gets +20/+20.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([22, 22]);
  });
