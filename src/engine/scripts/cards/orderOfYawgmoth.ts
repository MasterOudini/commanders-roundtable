// `Order of Yawgmoth` - a dealsDamagePlayer trigger vocab
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { ORDER_OF_YAWGMOTH } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(ORDER_OF_YAWGMOTH, "Fear (This creature can't be blocked except by artifact creatures and/or black creatures.)\nWhenever this creature deals damage to a player, that player discards a card.");
const LINES = PRINTED.split('\n');

const VOCAB_L1 = vocabularyEffects("Target player discards a card.", ORDER_OF_YAWGMOTH.name);
const VOCAB_T_L1 = vocabularyTargets("Target player discards a card.");

export const ORDER_OF_YAWGMOTH_SCRIPT: CardScript = {
  oracleId: ORDER_OF_YAWGMOTH.oracleId,
  name: ORDER_OF_YAWGMOTH.name,
  triggers: [
    {
      ...
      label: () => "Order of Yawgmoth - Target player discards a card.",
