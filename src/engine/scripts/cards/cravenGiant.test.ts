// `Craven Giant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRAVEN_GIANT_SCRIPT } from './cravenGiant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Craven Giant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Craven Giant", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRAVEN_GIANT_SCRIPT]),
  });
  holdEverywhere(g);
