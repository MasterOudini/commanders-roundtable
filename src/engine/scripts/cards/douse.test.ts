...
function armed(which: 'right' | 'wrong'): { g: Game; self: InstanceId; spell: InstanceId; stackId: string; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Douse"], ["Raging Goblin", "Grizzly Bears"]],
    scripts: createRegistry([DOUSE_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const spell = put(g, 'p2', which === 'right' ? "Raging Goblin" : "Grizzly Bears", 'hand');
  settle(g);
...
describe("Douse", () => {
  test("the held Raging Goblin is a legal target and is countered", () => {
