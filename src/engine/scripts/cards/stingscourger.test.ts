// `Stingscourger` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stingscourger", "Forest", "Forest", "Forest", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STINGSCOURGER_SCRIPT]),
  });
  holdEverywhere(g);
