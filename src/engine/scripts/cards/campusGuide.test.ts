// `Campus Guide` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Campus Guide", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAMPUS_GUIDE_SCRIPT]),
  });
  holdEverywhere(g);
