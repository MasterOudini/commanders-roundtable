// `Havoc` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HAVOC_SCRIPT } from './havoc';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Havoc"], ["Cyclops of One-Eyed Pass", "Devoted Hero", "Grizzly Bears"]],
    scripts: createRegistry([HAVOC_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Havoc", () => {
  test("Whenever an opponent casts a white spell: the vocabulary resolves \"Target player loses 2 life.\"", () => {
