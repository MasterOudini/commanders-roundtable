// `Strix Lookout` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STRIX_LOOKOUT_SCRIPT } from './strixLookout';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Strix Lookout";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Strix Lookout"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STRIX_LOOKOUT_SCRIPT]),
  });
  holdEverywhere(g);
