// `Murasa Rootgrazer` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Murasa Rootgrazer", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MURASA_ROOTGRAZER_SCRIPT]),
...
describe("Murasa Rootgrazer", () => {
  test("{T}: the vocabulary resolves \"You may put a basic land card from your hand onto the battlefield.\"", () => {
  ...
  test("{T}: the vocabulary resolves \"Return target basic land you control to its owner's hand.\"", () => {
    const { g, self, hand0, board0, vtA1_0 } = armed(1);
