// `Dusk Rose Reliquary` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DUSK_ROSE_RELIQUARY_SCRIPT } from './duskRoseReliquary';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dusk Rose Reliquary";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dusk Rose Reliquary"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUSK_ROSE_RELIQUARY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dusk Rose Reliquary", () => {
  test("When this artifact enters: the vocabulary resolves \"Exile target artifact or creature an opponent controls until this artifact leaves the battlefield.\"", () => {
