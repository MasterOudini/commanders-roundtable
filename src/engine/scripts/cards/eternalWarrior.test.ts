// `Eternal Warrior` - cast on Grizzly Bears it attaches and the enchanted creature has vigilance;
...
import { ETERNAL_WARRIOR_SCRIPT } from './eternalWarrior';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Eternal Warrior";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([ETERNAL_WARRIOR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ETERNAL_WARRIOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Eternal Warrior", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has vigilance; Cyclops of One-Eyed Pass is untouched", () => {
