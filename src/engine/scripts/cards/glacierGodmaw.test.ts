// `Glacier Godmaw` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GLACIER_GODMAW_SCRIPT } from './glacierGodmaw';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Glacier Godmaw";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GLACIER_GODMAW_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glacier Godmaw", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLACIER_GODMAW_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Glacier Godmaw", () => {
  test("When this creature enters: the vocabulary resolves \"Create a Lander token.\"", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("Landfall — Whenever a land you control enters: its controller's creatures get +1/+1 and gain vigilance and haste until end of turn", () => {
