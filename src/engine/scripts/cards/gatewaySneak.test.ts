// `Gateway Sneak` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GATEWAY_SNEAK_SCRIPT } from './gatewaySneak';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gateway Sneak";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gateway Sneak", "Grizzly Bears", "Azorius Guildgate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GATEWAY_SNEAK_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    bears = put(g, 'p1', "Azorius Guildgate");
    settle(g);
    }
  if (which === 1) {
...
describe("Gateway Sneak", () => {
  test("Whenever a Gate you control enters: the vocabulary resolves \"~ can't be blocked this turn.\"", () => {
    const {  } = armed(0);
  });
