// `Hoverguard Observer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOVERGUARD_OBSERVER_SCRIPT } from './hoverguardObserver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hoverguard Observer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hoverguard Observer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOVERGUARD_OBSERVER_SCRIPT]),
  });
  holdEverywhere(g);
