// `Wild Aesthir` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WILD_AESTHIR_SCRIPT } from './wildAesthir';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wild Aesthir";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WILD_AESTHIR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wild Aesthir", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WILD_AESTHIR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wild Aesthir", () => {
  test("{W}{W}: it gets +2/+0 until end of turn", () => {
