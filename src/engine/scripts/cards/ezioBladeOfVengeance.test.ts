// `Ezio, Blade of Vengeance` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EZIO_BLADE_OF_VENGEANCE_SCRIPT } from './ezioBladeOfVengeance';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ezio, Blade of Vengeance";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ezio, Blade of Vengeance", "Black Widow, Natasha Romanoff", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EZIO_BLADE_OF_VENGEANCE_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const stillFix0 = put(g, 'p1', "Black Widow, Natasha Romanoff");
  settle(g);
...
describe("Ezio, Blade of Vengeance", () => {
  test("Whenever an Assassin you control deals combat damage to a player: a card is drawn", () => {
