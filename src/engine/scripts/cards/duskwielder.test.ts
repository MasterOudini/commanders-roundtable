// `Duskwielder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DUSKWIELDER_SCRIPT } from './duskwielder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Duskwielder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Duskwielder", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUSKWIELDER_SCRIPT]),
...
describe("Duskwielder", () => {
  test("Boast — {1}: the vocabulary resolves \"Target opponent loses 1 life and you gain 1 life.\" - refused unless this creature attacked this turn", () => {
    const { g, life0, p2life0, refused, condRefused } = armed(0);
