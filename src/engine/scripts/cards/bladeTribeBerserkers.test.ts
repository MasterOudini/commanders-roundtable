// `Blade-Tribe Berserkers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLADE_TRIBE_BERSERKERS_SCRIPT } from './bladeTribeBerserkers';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blade-Tribe Berserkers";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLADE_TRIBE_BERSERKERS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blade-Tribe Berserkers", "Sol Ring", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLADE_TRIBE_BERSERKERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Blade-Tribe Berserkers", () => {
  test("Metalcraft — When this creature enters: it gets +3/+3 and gains haste until end of turn", () => {
