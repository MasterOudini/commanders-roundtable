// `Furnace Scamp` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FURNACE_SCAMP_SCRIPT } from './furnaceScamp';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Furnace Scamp";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Furnace Scamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FURNACE_SCAMP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Furnace Scamp", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"You may sacrifice it. If you do, this creature deals 3 damage to target player.\" - the price is declined", () => {
  ...
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"You may sacrifice it. If you do, this creature deals 3 damage to target player.\" - the price is paid", () => {
