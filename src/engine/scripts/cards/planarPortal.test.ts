// `Planar Portal` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Planar Portal", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PLANAR_PORTAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Planar Portal", () => {
  test("{6}, {T}: the vocabulary resolves \"Search your library for a card, put that card into your hand, then shuffle.\"", () => {
