// `Kavu Scout` - the pump is COUNTED: zero of them is no bonus, and each one moves the
// derived power and toughness by the printed delta. Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { KAVU_SCOUT_SCRIPT } from './kavuScout';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kavu Scout";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KAVU_SCOUT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; subject: InstanceId; other: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [[CARD, 'Forest', 'Forest', 'Mountain'], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KAVU_SCOUT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kavu Scout", () => {
  test("each basic land type counted moves the P/T by [1, 0], and one leaving takes it back", () => {
