// `Aesthir Glider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AESTHIR_GLIDER_SCRIPT } from './aesthirGlider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aesthir Glider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aesthir Glider", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AESTHIR_GLIDER_SCRIPT]),
  });
  holdEverywhere(g);
