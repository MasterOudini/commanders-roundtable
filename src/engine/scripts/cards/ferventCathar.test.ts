// `Fervent Cathar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FERVENT_CATHAR_SCRIPT } from './ferventCathar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fervent Cathar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fervent Cathar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FERVENT_CATHAR_SCRIPT]),
  });
  holdEverywhere(g);
