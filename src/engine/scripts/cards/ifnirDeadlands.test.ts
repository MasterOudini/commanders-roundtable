// `Ifnir Deadlands` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IFNIR_DEADLANDS_SCRIPT } from './ifnirDeadlands';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ifnir Deadlands";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ifnir Deadlands", "Bucolic Ranch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IFNIR_DEADLANDS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ifnir Deadlands", () => {
  test("{2}{B}{B}, {T}, Sacrifice a Desert: the vocabulary resolves \"Put two -1/-1 counters on target creature an opponent controls.\"", () => {
