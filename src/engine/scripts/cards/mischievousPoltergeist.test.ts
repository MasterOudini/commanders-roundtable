// `Mischievous Poltergeist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MISCHIEVOUS_POLTERGEIST_SCRIPT } from './mischievousPoltergeist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mischievous Poltergeist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mischievous Poltergeist", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MISCHIEVOUS_POLTERGEIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mischievous Poltergeist", () => {
  test("Pay 1 life: it regenerates from the Bolt", () => {
