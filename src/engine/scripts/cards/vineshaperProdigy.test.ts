// `Vineshaper Prodigy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VINESHAPER_PRODIGY_SCRIPT } from './vineshaperProdigy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vineshaper Prodigy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vineshaper Prodigy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VINESHAPER_PRODIGY_SCRIPT]),
  });
  holdEverywhere(g);
