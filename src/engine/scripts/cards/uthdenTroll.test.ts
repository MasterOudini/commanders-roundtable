// `Uthden Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UTHDEN_TROLL_SCRIPT } from './uthdenTroll';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Uthden Troll";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Uthden Troll", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UTHDEN_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
