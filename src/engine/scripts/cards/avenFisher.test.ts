// `Aven Fisher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AVEN_FISHER_SCRIPT } from './avenFisher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aven Fisher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aven Fisher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AVEN_FISHER_SCRIPT]),
  });
  holdEverywhere(g);
