// `Utvara Scalper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UTVARA_SCALPER_SCRIPT } from './utvaraScalper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Utvara Scalper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Utvara Scalper", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UTVARA_SCALPER_SCRIPT]),
  });
  holdEverywhere(g);
