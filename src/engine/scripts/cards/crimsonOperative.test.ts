// `Crimson Operative` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRIMSON_OPERATIVE_SCRIPT } from './crimsonOperative';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crimson Operative";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crimson Operative"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRIMSON_OPERATIVE_SCRIPT]),
  });
  holdEverywhere(g);
