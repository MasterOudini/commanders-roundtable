// `Rigging Runner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIGGING_RUNNER_SCRIPT } from './riggingRunner';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rigging Runner";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RIGGING_RUNNER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rigging Runner", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIGGING_RUNNER_SCRIPT]),
  });
  holdEverywhere(g);
