// `Cephalid Looter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CEPHALID_LOOTER_SCRIPT } from './cephalidLooter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cephalid Looter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cephalid Looter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CEPHALID_LOOTER_SCRIPT]),
  });
  holdEverywhere(g);
