// `Fireshrieker` - an Equipment: the equipped creature has doubleStrike.
...
import { FIRESHRIEKER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(FIRESHRIEKER, "Equipped creature has double strike. (It deals both first-strike and regular combat damage.)\nEquip {2} ({2}: Attach to target creature you control. Equip only as a sorcery.)");
const LINES = PRINTED.split('\n');

export const FIRESHRIEKER_SCRIPT: CardScript = {
  oracleId: FIRESHRIEKER.oracleId,
  name: FIRESHRIEKER.name,
  statics: [
    {
