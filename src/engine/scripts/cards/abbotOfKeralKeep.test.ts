// `Abbot of Keral Keep` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ABBOT_OF_KERAL_KEEP_SCRIPT } from './abbotOfKeralKeep';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Abbot of Keral Keep";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Abbot of Keral Keep"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ABBOT_OF_KERAL_KEEP_SCRIPT]),
  });
  holdEverywhere(g);
