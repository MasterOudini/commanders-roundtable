// `Seal from Existence` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEAL_FROM_EXISTENCE_SCRIPT } from './sealFromExistence';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Seal from Existence";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seal from Existence"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEAL_FROM_EXISTENCE_SCRIPT]),
  });
  holdEverywhere(g);
