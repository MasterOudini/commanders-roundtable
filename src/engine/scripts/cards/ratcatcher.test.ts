// `Ratcatcher` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ratcatcher", "Muck Rats"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RATCATCHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ratcatcher", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Search your library for a Rat card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Muck Rats" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
