// `Drossclaw` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DROSSCLAW_SCRIPT } from './drossclaw';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drossclaw";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DROSSCLAW_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drossclaw", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DROSSCLAW_SCRIPT]),
  });
  holdEverywhere(g);
