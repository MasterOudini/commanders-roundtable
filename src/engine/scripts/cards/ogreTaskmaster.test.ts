// `Ogre Taskmaster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OGRE_TASKMASTER_SCRIPT } from './ogreTaskmaster';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ogre Taskmaster";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ogre Taskmaster", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OGRE_TASKMASTER_SCRIPT]),
  });
  holdEverywhere(g);
