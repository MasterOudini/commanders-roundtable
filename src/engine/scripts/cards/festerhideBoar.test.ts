// `Festerhide Boar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FESTERHIDE_BOAR_SCRIPT } from './festerhideBoar';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Festerhide Boar";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FESTERHIDE_BOAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Festerhide Boar", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FESTERHIDE_BOAR_SCRIPT]),
  });
  holdEverywhere(g);
