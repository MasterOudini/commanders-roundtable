// `Assassin Den` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ASSASSIN_DEN_SCRIPT } from './assassinDen';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Assassin Den";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Assassin Den", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ASSASSIN_DEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Assassin Den", () => {
  test("{3}{U}: the vocabulary resolves \"Put a +1/+1 counter on target creature you control. It can't be blocked this turn.\"", () => {
