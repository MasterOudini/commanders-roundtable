// `Shattered Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shattered Landscape", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHATTERED_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shattered Landscape", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Mountain, Plains, or Swamp card, put it onto the battlefield tapped, then shuffle.\"", () => {
