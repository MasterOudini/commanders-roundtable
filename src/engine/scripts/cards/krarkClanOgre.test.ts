// `Krark-Clan Ogre` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KRARK_CLAN_OGRE_SCRIPT } from './krarkClanOgre';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Krark-Clan Ogre";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Krark-Clan Ogre", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KRARK_CLAN_OGRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Krark-Clan Ogre", () => {
  test("{R}, Sacrifice an artifact: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
