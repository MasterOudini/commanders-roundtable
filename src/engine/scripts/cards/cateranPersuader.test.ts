// `Cateran Persuader` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cateran Persuader", "Rampart Crawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CATERAN_PERSUADER_SCRIPT]),
...
describe("Cateran Persuader", () => {
  test("{1}, {T}: the vocabulary resolves \"Search your library for a Mercenary permanent card with mana value 1 or less, put it onto the battlefield, then shuffle.\"", () => {
