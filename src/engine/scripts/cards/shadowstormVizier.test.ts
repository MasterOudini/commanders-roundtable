// `Shadowstorm Vizier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHADOWSTORM_VIZIER_SCRIPT } from './shadowstormVizier';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shadowstorm Vizier";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHADOWSTORM_VIZIER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shadowstorm Vizier"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHADOWSTORM_VIZIER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shadowstorm Vizier", () => {
  test("Whenever you cycle or discard a card: it gets +1/+1 until end of turn", () => {
