// `Venomous Dragonfly` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VENOMOUS_DRAGONFLY_SCRIPT } from './venomousDragonfly';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Venomous Dragonfly";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Venomous Dragonfly"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VENOMOUS_DRAGONFLY_SCRIPT]),
...
describe("Venomous Dragonfly", () => {
  test("Whenever this creature blocks or becomes blocked by a creature: the vocabulary resolves \"Destroy target creature at end of combat.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.delayedTriggers.length, 'the payload is armed, not run').toBe(1);
    {
      ...
      settle(g);
      expect(g.state.cards[no]?.zone.kind, "destroy at the step").toBe('graveyard');
    }
