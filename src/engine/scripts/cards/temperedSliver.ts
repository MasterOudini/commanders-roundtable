// `Tempered Sliver` - every permanent in its scope HAS a quoted
...
import { TEMPERED_SLIVER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(TEMPERED_SLIVER, "Sliver creatures you control have \"Whenever this creature deals combat damage to a player, put a +1/+1 counter on it.\"");

const REF = grantedTriggerRef(`${TEMPERED_SLIVER.oracleId}#gt0`, TEMPERED_SLIVER.name);
const VOCAB = vocabularyEffects("put a +1/+1 counter on it.", TEMPERED_SLIVER.name);
const VOCAB_T = vocabularyTargets("put a +1/+1 counter on it.");

export const TEMPERED_SLIVER_SCRIPT: CardScript = {
  oracleId: TEMPERED_SLIVER.oracleId,
  name: TEMPERED_SLIVER.name,
  statics: [
    {
      abilityId: 'grant-0',
      text: PRINTED,
  ...
  triggers: [
    {
      ...
      label: () => "Tempered Sliver - put a +1/+1 counter on it.",
