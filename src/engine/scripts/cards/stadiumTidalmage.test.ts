// `Stadium Tidalmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STADIUM_TIDALMAGE_SCRIPT } from './stadiumTidalmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stadium Tidalmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stadium Tidalmage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STADIUM_TIDALMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stadium Tidalmage", () => {
  test("Whenever this creature enters or attacks: the vocabulary resolves \"Draw a card. If you do, discard a card.\" [etb]", () => {
    const { g } = armed(0);
  ...
  test("Whenever this creature enters or attacks: the vocabulary resolves \"Draw a card. If you do, discard a card.\" [attacks]", () => {
    const { g } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'chooseFromZone', 20_000);
    { const dh = [...(g.state.zones.hand.p1 ?? [])];
      expect(dh.length).toBeGreaterThanOrEqual(1);
