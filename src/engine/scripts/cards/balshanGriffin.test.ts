// `Balshan Griffin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BALSHAN_GRIFFIN_SCRIPT } from './balshanGriffin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Balshan Griffin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Balshan Griffin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BALSHAN_GRIFFIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Balshan Griffin", () => {
  test("{1}{U}, Discard a card: it returns to its owner's hand", () => {
