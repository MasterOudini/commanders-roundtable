// `Vodalian Mage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VODALIAN_MAGE_SCRIPT } from './vodalianMage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vodalian Mage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vodalian Mage"], ["Cyclops of One-Eyed Pass", "Divination", "Forest"]],
    scripts: createRegistry([VODALIAN_MAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vodalian Mage", () => {
  test("{U}, {T}: the vocabulary resolves \"Counter target spell unless its controller pays {1}.\" - the price is declined", () => {
  ...
  test("{U}, {T}: the vocabulary resolves \"Counter target spell unless its controller pays {1}.\" - the price is paid", () => {
    const { g, self, payA0_p2_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
