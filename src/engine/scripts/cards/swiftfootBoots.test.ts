// `Swiftfoot Boots` - equipped to Grizzly Bears the creature has hexproof, haste; the Cyclops is untouched;
...
import { SWIFTFOOT_BOOTS_SCRIPT } from './swiftfootBoots';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Swiftfoot Boots";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([SWIFTFOOT_BOOTS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SWIFTFOOT_BOOTS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Swiftfoot Boots", () => {
  test("equipped to Grizzly Bears: attached, and the creature has hexproof, haste; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(kw(g, host).has("hexproof")).toBe(true);
    expect(kw(g, host).has("haste")).toBe(true);
    expect(kw(g, other).has("hexproof")).toBe(false);
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("hexproof")).toBe(false);
