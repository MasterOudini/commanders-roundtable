// `Maze Glider` - the grant reaches the permanent its scope names and not the other;
// it ends when the source leaves; replay equal (D300). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { MAZE_GLIDER_SCRIPT } from './mazeGlider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Maze Glider";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([MAZE_GLIDER_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Maze Glider", "Armada Wurm", "Coral Eel"], ["Grizzly Bears"]],
    scripts: createRegistry([MAZE_GLIDER_SCRIPT]),
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Armada Wurm");
...
describe("Maze Glider", () => {
  test("Armada Wurm is reached, Coral Eel is not", () => {
