// `Bloodhall Priest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODHALL_PRIEST_SCRIPT } from './bloodhallPriest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodhall Priest";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodhall Priest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODHALL_PRIEST_SCRIPT]),
  ...
  if (which === 0) {
    // the payload's gate: you have 0 cards in hand
...
describe("Bloodhall Priest", () => {
  test("Whenever this creature enters or attacks: the vocabulary resolves \"If you have no cards in hand, this creature deals 2 damage to any target.\" [etb]", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
  });

  test("Whenever this creature enters or attacks: the vocabulary resolves \"If you have no cards in hand, this creature deals 2 damage to any target.\" [attacks]", () => {
    const { g, no } = armed(1);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
  });
