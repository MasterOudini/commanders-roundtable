// `Vivisection Evangelist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VIVISECTION_EVANGELIST_SCRIPT } from './vivisectionEvangelist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vivisection Evangelist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vivisection Evangelist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIVISECTION_EVANGELIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vivisection Evangelist", () => {
  test("Corrupted — When this creature enters: the vocabulary resolves \"Destroy target creature or planeswalker an opponent controls.\"", () => {
    const { g, no } = armed(0);
