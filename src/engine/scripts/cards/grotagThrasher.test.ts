// `Grotag Thrasher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GROTAG_THRASHER_SCRIPT } from './grotagThrasher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grotag Thrasher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grotag Thrasher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GROTAG_THRASHER_SCRIPT]),
  });
  holdEverywhere(g);
