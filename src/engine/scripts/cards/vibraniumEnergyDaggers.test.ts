// `Vibranium Energy Daggers` - equipped to Grizzly Bears the creature gets +2/+2; the Cyclops is untouched;
...
import { VIBRANIUM_ENERGY_DAGGERS_SCRIPT } from './vibraniumEnergyDaggers';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vibranium Energy Daggers";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VIBRANIUM_ENERGY_DAGGERS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([VIBRANIUM_ENERGY_DAGGERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vibranium Energy Daggers", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +2/+2; the Cyclops is untouched", () => {
