// `Mardu Hordechief` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARDU_HORDECHIEF_SCRIPT } from './marduHordechief';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mardu Hordechief";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mardu Hordechief", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARDU_HORDECHIEF_SCRIPT]),
  });
  holdEverywhere(g);
