// `Deathgazer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEATHGAZER_SCRIPT } from './deathgazer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deathgazer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deathgazer"], ["Cyclops of One-Eyed Pass", "Staunch Shieldmate"]],
    scripts: createRegistry([DEATHGAZER_SCRIPT]),
