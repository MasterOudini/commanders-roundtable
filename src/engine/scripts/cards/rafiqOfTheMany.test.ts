// `Rafiq of the Many` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAFIQ_OF_THE_MANY_SCRIPT } from './rafiqOfTheMany';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rafiq of the Many";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RAFIQ_OF_THE_MANY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rafiq of the Many"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAFIQ_OF_THE_MANY_SCRIPT]),
...
describe("Rafiq of the Many", () => {
  test("Whenever a creature you control attacks alone: the vocabulary resolves \"Target creature gains double strike until end of turn.\"", () => {
