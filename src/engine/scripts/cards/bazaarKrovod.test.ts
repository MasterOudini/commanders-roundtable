// `Bazaar Krovod` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BAZAAR_KROVOD_SCRIPT } from './bazaarKrovod';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bazaar Krovod";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BAZAAR_KROVOD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bazaar Krovod", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BAZAAR_KROVOD_SCRIPT]),
...
describe("Bazaar Krovod", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Another target attacking creature gets +0/+2 until end of turn. Untap that creature.\"", () => {
    const { g, vtL0_attacks_0 } = armed(0);
    expect(pt(g, vtL0_attacks_0)).toEqual([2, 4]);
    expect(g.state.cards[vtL0_attacks_0]?.tapped).toBe(false);
  });
