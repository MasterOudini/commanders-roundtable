// `Carrier Thrall` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CARRIER_THRALL_SCRIPT } from './carrierThrall';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Carrier Thrall";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Carrier Thrall"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CARRIER_THRALL_SCRIPT]),
...
describe("Carrier Thrall", () => {
  test("When this creature dies: the vocabulary resolves \"Create a 1/1 colorless Eldrazi Scion creature token. It has \\\"Sacrifice this token: Add {C}.\\\"\"", () => {
