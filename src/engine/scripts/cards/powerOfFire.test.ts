// `Power of Fire` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { POWER_OF_FIRE_SCRIPT } from './powerOfFire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Power of Fire";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Power of Fire","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([POWER_OF_FIRE_SCRIPT]),
  });
  holdEverywhere(g);
