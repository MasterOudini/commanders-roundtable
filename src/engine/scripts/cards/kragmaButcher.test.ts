// `Kragma Butcher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KRAGMA_BUTCHER_SCRIPT } from './kragmaButcher';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kragma Butcher";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KRAGMA_BUTCHER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kragma Butcher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KRAGMA_BUTCHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kragma Butcher", () => {
  test("Inspired — Whenever this creature becomes untapped: it gets +2/+0 until end of turn", () => {
