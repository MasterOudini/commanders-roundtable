// `Stoneforge Acolyte` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stoneforge Acolyte", "Expedition Envoy", "Accorder's Shield", "Accorder's Shield"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STONEFORGE_ACOLYTE_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const tapper0 = [put(g, 'p1', "Expedition Envoy")];
  settle(g);
  ...
  if (which === 0) {
    ...
    settle(g);
    const lookPicksA0 = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Accorder's Shield" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksA0.length, "Accorder's Shield is dealt for the look").toBeGreaterThan(0);
...
describe("Stoneforge Acolyte", () => {
  test("Cohort — {T}, Tap an untapped Ally you control: the vocabulary resolves \"Look at the top four cards of your library. You may reveal an Equipment card from among them and put it into your hand. Put the rest on the bottom of your library in any order.\"", () => {
    const { g, self, tapper0 } = armed(0);
