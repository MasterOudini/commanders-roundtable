// `Flying Octobot` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flying Octobot", "Grizzly Bears", "Black Tom Cassidy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLYING_OCTOBOT_SCRIPT]),
...
describe("Flying Octobot", () => {
  test("Whenever another Villain you control enters: it gets 1 +1/+1 counter", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(1);
  });

  test('the once-per-turn rider rides the def (D492)', () => {
    expect(FLYING_OCTOBOT_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("anotherCreatureEnters-1") && t.oncePerTurn === true), 'the def of line 1 carries oncePerTurn').toBeDefined();
  });
