// `Vile Entomber` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vile Entomber", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VILE_ENTOMBER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vile Entomber", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a card, put that card into your graveyard, then shuffle.\"", () => {
    const { g } = armed(0);
