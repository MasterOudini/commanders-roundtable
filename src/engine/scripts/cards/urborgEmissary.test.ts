// `Urborg Emissary` - every printed ability proven in its own game: the cost's mark, the pump
...
import { URBORG_EMISSARY_SCRIPT } from './urborgEmissary';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Urborg Emissary";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Urborg Emissary"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([URBORG_EMISSARY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Urborg Emissary", () => {
  test("When this creature enters: the vocabulary resolves \"Return target permanent to its owner's hand.\"", () => {
    const { g, no } = armed(0);
