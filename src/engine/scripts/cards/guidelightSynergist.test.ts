// `Guidelight Synergist` - the pump is COUNTED: zero of them is no bonus, and each one moves the
// derived power and toughness by the printed delta. Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { GUIDELIGHT_SYNERGIST_SCRIPT } from './guidelightSynergist';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Guidelight Synergist";
const COUNTED = "Sol Ring";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GUIDELIGHT_SYNERGIST_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; subject: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GUIDELIGHT_SYNERGIST_SCRIPT]),
  });
  holdEverywhere(g);
...
function plus(a: [number | null, number | null], n: number): [number | null, number | null] {
  return [a[0] === null ? null : a[0] + 1 * n, a[1] === null ? null : a[1] + 0 * n];
}

describe("Guidelight Synergist", () => {
  test("each Sol Ring counted moves the P/T by [1, 0], and one leaving takes it back", () => {
