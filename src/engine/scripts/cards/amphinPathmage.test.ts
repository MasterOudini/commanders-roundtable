// `Amphin Pathmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AMPHIN_PATHMAGE_SCRIPT } from './amphinPathmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Amphin Pathmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Amphin Pathmage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AMPHIN_PATHMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Amphin Pathmage", () => {
  test("{2}{U}: the vocabulary resolves \"Target creature can't be blocked this turn.\"", () => {
    const { g, no } = armed(0);
