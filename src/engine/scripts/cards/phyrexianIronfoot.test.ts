// `Phyrexian Ironfoot` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHYREXIAN_IRONFOOT_SCRIPT } from './phyrexianIronfoot';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phyrexian Ironfoot";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phyrexian Ironfoot", "Snow-Covered Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHYREXIAN_IRONFOOT_SCRIPT]),
  });
  holdEverywhere(g);
