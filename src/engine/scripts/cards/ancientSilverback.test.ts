// `Ancient Silverback` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ANCIENT_SILVERBACK_SCRIPT } from './ancientSilverback';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ancient Silverback";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ancient Silverback", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ANCIENT_SILVERBACK_SCRIPT]),
  });
  holdEverywhere(g);
