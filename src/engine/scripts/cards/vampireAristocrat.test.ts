// `Vampire Aristocrat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VAMPIRE_ARISTOCRAT_SCRIPT } from './vampireAristocrat';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vampire Aristocrat";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VAMPIRE_ARISTOCRAT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vampire Aristocrat", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VAMPIRE_ARISTOCRAT_SCRIPT]),
  });
  holdEverywhere(g);
