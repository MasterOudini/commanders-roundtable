// `Kazuul Warlord` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KAZUUL_WARLORD_SCRIPT } from './kazuulWarlord';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kazuul Warlord";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kazuul Warlord", "Makindi Aeronaut"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KAZUUL_WARLORD_SCRIPT]),
...
describe("Kazuul Warlord", () => {
  test("Whenever this creature or another Ally you control enters: each of its controller's creatures gets 1 +1/+1 counter", () => {
