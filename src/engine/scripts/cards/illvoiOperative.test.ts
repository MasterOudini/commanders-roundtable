// `Illvoi Operative` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ILLVOI_OPERATIVE_SCRIPT } from './illvoiOperative';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Illvoi Operative";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Illvoi Operative", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ILLVOI_OPERATIVE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Illvoi Operative", () => {
  test("Whenever you cast your second spell each turn: it gets 1 +1/+1 counter", () => {
