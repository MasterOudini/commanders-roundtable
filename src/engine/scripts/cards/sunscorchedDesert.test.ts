// `Sunscorched Desert` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUNSCORCHED_DESERT_SCRIPT } from './sunscorchedDesert';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunscorched Desert";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sunscorched Desert"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUNSCORCHED_DESERT_SCRIPT]),
  });
  holdEverywhere(g);
