// `Stromkirk Patrol` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STROMKIRK_PATROL_SCRIPT } from './stromkirkPatrol';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stromkirk Patrol";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stromkirk Patrol"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STROMKIRK_PATROL_SCRIPT]),
  });
  holdEverywhere(g);
