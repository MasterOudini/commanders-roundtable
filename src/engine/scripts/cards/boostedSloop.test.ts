// `Boosted Sloop` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BOOSTED_SLOOP_SCRIPT } from './boostedSloop';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Boosted Sloop";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Boosted Sloop", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOOSTED_SLOOP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Boosted Sloop", () => {
  test("Whenever you attack: a card is drawn, then one discarded", () => {
