// `Bog Badger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BOG_BADGER_SCRIPT } from './bogBadger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bog Badger";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BOG_BADGER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bog Badger", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BOG_BADGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bog Badger", () => {
  test("When this creature enters: its controller's creatures gain menace until end of turn", () => {
