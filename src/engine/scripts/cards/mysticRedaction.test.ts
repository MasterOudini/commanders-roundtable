// `Mystic Redaction` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MYSTIC_REDACTION_SCRIPT } from './mysticRedaction';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mystic Redaction";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mystic Redaction"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MYSTIC_REDACTION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mystic Redaction", () => {
  ...
  test("Whenever you discard a card: the vocabulary resolves \"Each opponent mills two cards.\"", () => {
