// `Wings of Hope` - cast on Grizzly Bears it attaches and the enchanted creature gets +1/+3 and has flying;
...
import { WINGS_OF_HOPE_SCRIPT } from './wingsOfHope';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wings of Hope";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WINGS_OF_HOPE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([WINGS_OF_HOPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wings of Hope", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +1/+3 and has flying; Cyclops of One-Eyed Pass is untouched", () => {
