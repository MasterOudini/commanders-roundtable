// `Whispering Wizard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WHISPERING_WIZARD_SCRIPT } from './whisperingWizard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Whispering Wizard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Whispering Wizard", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WHISPERING_WIZARD_SCRIPT]),
...
describe("Whispering Wizard", () => {
  ...
  test('the once-per-turn rider rides the def (D492)', () => {
    expect(WHISPERING_WIZARD_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("castNoncreature-0") && t.oncePerTurn === true), 'the def of line 0 carries oncePerTurn').toBeDefined();
  });
