// `Siegecraft` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+4;
...
import { SIEGECRAFT_SCRIPT } from './siegecraft';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Siegecraft";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SIEGECRAFT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SIEGECRAFT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Siegecraft", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+4; Cyclops of One-Eyed Pass is untouched", () => {
    const { g, self, host, other } = board();
    cast(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([4, 6]);
