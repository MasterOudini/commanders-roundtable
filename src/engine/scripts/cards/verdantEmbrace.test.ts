// `Verdant Embrace` - the granted TRIGGER proven end to end (D368): the recipient HAS the
...
import { VERDANT_EMBRACE_SCRIPT } from './verdantEmbrace';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Verdant Embrace";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Verdant Embrace","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VERDANT_EMBRACE_SCRIPT]),
  });
  holdEverywhere(g);
