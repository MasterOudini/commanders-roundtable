// `Rescue Leopard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RESCUE_LEOPARD_SCRIPT } from './rescueLeopard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rescue Leopard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rescue Leopard", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RESCUE_LEOPARD_SCRIPT]),
  });
  holdEverywhere(g);
