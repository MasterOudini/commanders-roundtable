// `Oppression` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OPPRESSION_SCRIPT } from './oppression';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Oppression";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Oppression", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OPPRESSION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Oppression", () => {
  test("Whenever a player casts a spell: the vocabulary resolves \"Target player discards a card.\"", () => {
    const { g } = armed(0);
