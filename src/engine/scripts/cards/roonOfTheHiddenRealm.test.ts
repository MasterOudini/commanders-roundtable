// `Roon of the Hidden Realm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROON_OF_THE_HIDDEN_REALM_SCRIPT } from './roonOfTheHiddenRealm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Roon of the Hidden Realm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Roon of the Hidden Realm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROON_OF_THE_HIDDEN_REALM_SCRIPT]),
...
describe("Roon of the Hidden Realm", () => {
  test("{2}, {T}: the vocabulary resolves \"Exile another target creature. Return that card to the battlefield under its owner's control at the beginning of the next end step.\"", () => {
