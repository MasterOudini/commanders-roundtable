// `Blighted Agent` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLIGHTED_AGENT_SCRIPT } from './blightedAgent';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blighted Agent";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blighted Agent", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLIGHTED_AGENT_SCRIPT]),
  });
  holdEverywhere(g);
