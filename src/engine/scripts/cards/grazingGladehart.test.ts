// `Grazing Gladehart` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRAZING_GLADEHART_SCRIPT } from './grazingGladehart';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grazing Gladehart";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grazing Gladehart"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRAZING_GLADEHART_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Grazing Gladehart", () => {
  test("Landfall — Whenever a land you control enters: 2 life is gained", () => {
