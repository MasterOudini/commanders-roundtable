// `Urborg Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { URBORG_DRAKE_SCRIPT } from './urborgDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Urborg Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Urborg Drake", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([URBORG_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
