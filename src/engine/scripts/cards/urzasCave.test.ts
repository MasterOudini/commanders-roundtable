// `Urza's Cave` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Urza's Cave", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([URZAS_CAVE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Urza's Cave", () => {
  test("{3}, {T}, Sacrifice this land: the vocabulary resolves \"Search your library for a land card, put it onto the battlefield tapped, then shuffle.\"", () => {
