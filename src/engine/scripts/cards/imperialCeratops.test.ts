// `Imperial Ceratops` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IMPERIAL_CERATOPS_SCRIPT } from './imperialCeratops';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Imperial Ceratops";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Imperial Ceratops", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([IMPERIAL_CERATOPS_SCRIPT]),
  });
  holdEverywhere(g);
