// `Grixis Slavedriver` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRIXIS_SLAVEDRIVER_SCRIPT } from './grixisSlavedriver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grixis Slavedriver";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grixis Slavedriver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRIXIS_SLAVEDRIVER_SCRIPT]),
