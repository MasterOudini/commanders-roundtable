// `Ajani, Adversary of Tyrants Emblem` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([AJANI_ADVERSARY_OF_TYRANTS_EMBLEM1C97E5B2_SCRIPT]),
  ...
  if (![null][which]) {
    must(g.submit({ t: 'ManualCreateEmblem', player: 'p1', printingId: AJANI_ADVERSARY_OF_TYRANTS_EMBLEM.scryfallId }));
    self = (g.state.zones.command.p1 ?? []).filter((c) => g.state.cards[c]?.printingId === AJANI_ADVERSARY_OF_TYRANTS_EMBLEM.scryfallId).pop() as InstanceId;
    settle(g);
  }
...
describe("Ajani, Adversary of Tyrants Emblem", () => {
  test("At the beginning of your end step: 3 tokens made", () => {
