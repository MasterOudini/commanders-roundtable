// `Wose Pathfinder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WOSE_PATHFINDER_SCRIPT } from './wosePathfinder';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wose Pathfinder";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WOSE_PATHFINDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wose Pathfinder"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WOSE_PATHFINDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wose Pathfinder", () => {
  test("{6}{G}, {T}: the vocabulary resolves \"Another target creature gets +3/+3 and gains trample until end of turn.\"", () => {
