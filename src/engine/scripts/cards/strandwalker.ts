// `Strandwalker` - a etb trigger germ, a static attachedStatic
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { STRANDWALKER, PHYREXIAN_GERM_TOKEN } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(STRANDWALKER, "Living weapon (When this Equipment enters, create a 0/0 black Phyrexian Germ creature token, then attach this to it.)\nEquipped creature gets +2/+4 and has reach.\nEquip {4}");
const LINES = PRINTED.split('\n');

export const STRANDWALKER_SCRIPT: CardScript = {
  oracleId: STRANDWALKER.oracleId,
  name: STRANDWALKER.name,
  triggers: [
    {
      abilityId: 'etb-0',
