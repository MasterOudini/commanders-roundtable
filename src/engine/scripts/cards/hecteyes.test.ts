// `Hecteyes` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HECTEYES_SCRIPT } from './hecteyes';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hecteyes"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HECTEYES_SCRIPT]),
  });
  holdEverywhere(g);
