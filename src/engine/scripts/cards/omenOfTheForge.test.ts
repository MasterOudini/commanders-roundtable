// `Omen of the Forge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OMEN_OF_THE_FORGE_SCRIPT } from './omenOfTheForge';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Omen of the Forge";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Omen of the Forge"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OMEN_OF_THE_FORGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Omen of the Forge", () => {
  test("When this enchantment enters: it deals 2 damage to the declared creature", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
  });

  test("{2}{R}, Sacrifice this enchantment: scry 2 asks, and the cards stay on top", () => {
