// `King's Assassin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KINGS_ASSASSIN_SCRIPT } from './kingsAssassin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "King's Assassin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["King's Assassin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KINGS_ASSASSIN_SCRIPT]),
  });
  holdEverywhere(g);
