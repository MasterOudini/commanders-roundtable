// `Festering Mummy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FESTERING_MUMMY_SCRIPT } from './festeringMummy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Festering Mummy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Festering Mummy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FESTERING_MUMMY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Festering Mummy", () => {
  test("When this creature dies: the vocabulary resolves \"Put a -1/-1 counter on target creature.\"", () => {
    const { g, no } = armed(0);
