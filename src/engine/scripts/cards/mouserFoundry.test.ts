// `Mouser Foundry` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOUSER_FOUNDRY_SCRIPT } from './mouserFoundry';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mouser Foundry";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mouser Foundry"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOUSER_FOUNDRY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mouser Foundry", () => {
  test("When this artifact enters or leaves the battlefield: 1 token made [etb]", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("When this artifact enters or leaves the battlefield: 1 token made [leavesBattlefield]", () => {
    const { g, board0 } = armed(1);
    expect(onBoard(g)).toBe(board0 + 1 - 1);
  });

  test("{4}{R}, Sacrifice this artifact: it deals 3 damage to the declared creature", () => {
