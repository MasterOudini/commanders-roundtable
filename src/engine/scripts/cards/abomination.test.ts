// `Abomination` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ABOMINATION_SCRIPT } from './abomination';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Abomination";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Abomination"], ["Cyclops of One-Eyed Pass", "Staunch Shieldmate"]],
    scripts: createRegistry([ABOMINATION_SCRIPT]),
...
describe("Abomination", () => {
  test("Whenever this creature blocks or becomes blocked by a green or white creature: the vocabulary resolves \"Destroy target creature at end of combat.\"", () => {
