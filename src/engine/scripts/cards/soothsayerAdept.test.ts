// `Soothsayer Adept` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOOTHSAYER_ADEPT_SCRIPT } from './soothsayerAdept';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Soothsayer Adept";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soothsayer Adept"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOOTHSAYER_ADEPT_SCRIPT]),
  });
  holdEverywhere(g);
