// `Pouncing Kavu` - every printed ability proven in its own game: the cost's mark, the pump
...
import { POUNCING_KAVU_SCRIPT } from './pouncingKavu';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pouncing Kavu";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([POUNCING_KAVU_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pouncing Kavu"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([POUNCING_KAVU_SCRIPT]),
...
describe("Pouncing Kavu", () => {
  test("If this creature was kicked, it enters with two +1/+1 counters on it and with haste.: cast, it enters with 2 +1/+1 counters", () => {
