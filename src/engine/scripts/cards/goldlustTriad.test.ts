// `Goldlust Triad` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOLDLUST_TRIAD_SCRIPT } from './goldlustTriad';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goldlust Triad";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goldlust Triad"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOLDLUST_TRIAD_SCRIPT]),
