// `Dimir House Guard` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DIMIR_HOUSE_GUARD_SCRIPT } from './dimirHouseGuard';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dimir House Guard";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dimir House Guard", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DIMIR_HOUSE_GUARD_SCRIPT]),
