// `Crystal Rod` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRYSTAL_ROD_SCRIPT } from './crystalRod';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crystal Rod";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crystal Rod", "Grizzly Bears", "Aegis Turtle", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRYSTAL_ROD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Crystal Rod", () => {
  test("Whenever a player casts a blue spell: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is declined", () => {
  ...
  test("Whenever a player casts a blue spell: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is paid", () => {
