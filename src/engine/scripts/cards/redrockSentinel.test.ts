// `Redrock Sentinel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { REDROCK_SENTINEL_SCRIPT } from './redrockSentinel';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Redrock Sentinel";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Redrock Sentinel", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([REDROCK_SENTINEL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Redrock Sentinel", () => {
  test("{2}, {T}, Sacrifice a land: the vocabulary resolves \"Draw a card and create a Treasure token.\"", () => {
