// `Ranger's Longbow` - equipped to Grizzly Bears the creature gets +2/+1 and has reach; the Cyclops is untouched;
...
import { RANGERS_LONGBOW_SCRIPT } from './rangersLongbow';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ranger's Longbow";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RANGERS_LONGBOW_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([RANGERS_LONGBOW_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ranger's Longbow", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +2/+1 and has reach; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([4, 3]);
    expect(kw(g, host).has("reach")).toBe(true);
    expect(pt(g, other)).toEqual([5, 2]);
    expect(kw(g, other).has("reach")).toBe(false);
  });
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("reach")).toBe(false);
  });
