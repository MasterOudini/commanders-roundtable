// `Bloodmark Mentor` - a layer-6 grant: "Red creatures you control have first strike". A StaticDef in the shape of the
...
import { BLOODMARK_MENTOR } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(BLOODMARK_MENTOR, "Red creatures you control have first strike.");
const TEXT = PRINTED;

export const BLOODMARK_MENTOR_SCRIPT: CardScript = {
  oracleId: BLOODMARK_MENTOR.oracleId,
  name: BLOODMARK_MENTOR.name,
  statics: [
    {
      abilityId: 'grant',
      text: TEXT,
      ...
      appliesTo: (ctx, self, candidate, chars) => {
        ...
        if (!chars.colors.includes("R")) return false;
        return true;
      },
      modify: (chars) => {
