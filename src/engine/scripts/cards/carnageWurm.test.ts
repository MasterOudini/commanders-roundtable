// `Carnage Wurm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CARNAGE_WURM_SCRIPT } from './carnageWurm';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Carnage Wurm";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CARNAGE_WURM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Carnage Wurm", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CARNAGE_WURM_SCRIPT]),
  });
  holdEverywhere(g);
