// `Serra Redeemer` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Serra Redeemer", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SERRA_REDEEMER_SCRIPT]),
...
describe("Serra Redeemer", () => {
  test("Whenever another creature you control with power 2 or less enters: the vocabulary resolves \"Put two +1/+1 counters on target creature.\"", () => {
