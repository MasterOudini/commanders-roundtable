// `Solemn Simulacrum` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Solemn Simulacrum", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOLEMN_SIMULACRUM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Solemn Simulacrum", () => {
  ...
  test("When this creature dies: a card is drawn", () => {
