// `Deadly Riposte` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { DEADLY_RIPOSTE_SCRIPT } from './deadlyRiposte';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deadly Riposte";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Deadly Riposte"], ["Grizzly Bears", "Grizzly Bears"]],
    scripts: createRegistry([DEADLY_RIPOSTE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deadly Riposte", () => {
  test('the named target is accepted and the effect lands', () => {
    ...
    expect(g.state.players.p1?.life).toBe(life0.p1 + (2));
  });
