// `Mage-Ring Bully` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAGE_RING_BULLY_SCRIPT } from './mageRingBully';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mage-Ring Bully";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mage-Ring Bully", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAGE_RING_BULLY_SCRIPT]),
  });
  holdEverywhere(g);
