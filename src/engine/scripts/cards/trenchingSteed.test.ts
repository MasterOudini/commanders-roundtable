// `Trenching Steed` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRENCHING_STEED_SCRIPT } from './trenchingSteed';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trenching Steed";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TRENCHING_STEED_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trenching Steed", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRENCHING_STEED_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Trenching Steed", () => {
  test("Sacrifice a land: it gets +0/+3 until end of turn", () => {
    const { g, self, fodder0 } = armed(0);
    expect(pt(g, self)).toEqual([2, 6]);
