// `Sidisi, Undead Vizier` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sidisi, Undead Vizier", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIDISI_UNDEAD_VIZIER_SCRIPT]),
...
describe("Sidisi, Undead Vizier", () => {
  test("When Sidisi exploits a creature: the vocabulary resolves \"Search your library for a card, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
