// `Henchfiend of Ukor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HENCHFIEND_OF_UKOR_SCRIPT } from './henchfiendOfUkor';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HENCHFIEND_OF_UKOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Henchfiend of Ukor", "Forest", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HENCHFIEND_OF_UKOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Henchfiend of Ukor", () => {
  test("{B/R}: it gets +1/+0 until end of turn", () => {
