// `Shock Troops` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHOCK_TROOPS_SCRIPT } from './shockTroops';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shock Troops";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shock Troops"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHOCK_TROOPS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shock Troops", () => {
  test("Sacrifice this creature: it deals 2 damage to the declared creature", () => {
