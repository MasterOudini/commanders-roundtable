// `Arcane Teachings` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { ARCANE_TEACHINGS_SCRIPT } from './arcaneTeachings';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Arcane Teachings";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Arcane Teachings","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARCANE_TEACHINGS_SCRIPT]),
  });
  holdEverywhere(g);
