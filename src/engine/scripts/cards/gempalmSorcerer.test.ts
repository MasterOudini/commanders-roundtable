// `Gempalm Sorcerer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GEMPALM_SORCERER_SCRIPT } from './gempalmSorcerer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gempalm Sorcerer";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([GEMPALM_SORCERER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gempalm Sorcerer", "Fugitive Wizard"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GEMPALM_SORCERER_SCRIPT]),
...
describe("Gempalm Sorcerer", () => {
  test("When you cycle this card: Wizard creatures gain flying until end of turn", () => {
  ...
  test('the pump ends at cleanup', () => {
    ...
    expect(pt(g, yes)).toEqual([1, 1]);
    expect(kw(g, yes).has("flying")).toBe(false);
  });
