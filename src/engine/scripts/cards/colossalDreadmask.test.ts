// `Colossal Dreadmask` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COLOSSAL_DREADMASK_SCRIPT } from './colossalDreadmask';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Colossal Dreadmask";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([COLOSSAL_DREADMASK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Colossal Dreadmask", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COLOSSAL_DREADMASK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Colossal Dreadmask", () => {
  ...
  test("Equipped creature gets +6/+6 and has trample.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([8, 8]);
