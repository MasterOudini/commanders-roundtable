// `Troll of Khazad-dûm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TROLL_OF_KHAZAD_DM_SCRIPT } from './trollOfKhazadDM';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Troll of Khazad-dûm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Troll of Khazad-dûm", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([TROLL_OF_KHAZAD_DM_SCRIPT]),
  });
  holdEverywhere(g);
