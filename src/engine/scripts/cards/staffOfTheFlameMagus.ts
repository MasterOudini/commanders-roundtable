// Staff of the Flame Magus - the red/Mountain member of the five-Staff
// family (see staffOfTheDeathMagus.ts). D252.

import { STAFF_OF_THE_FLAME_MAGUS } from '../../../data/fixtures/engineCards';
...
function landQualifies(ctx: ScriptCtx, self: InstanceId, entrant: InstanceId): boolean {
  ...
  return ctx.derive(entrant).typeLine.subtypes.includes('Mountain');
}
...
export const STAFF_OF_THE_FLAME_MAGUS_SCRIPT: CardScript = {
  oracleId: STAFF_OF_THE_FLAME_MAGUS.oracleId,
  name: STAFF_OF_THE_FLAME_MAGUS.name,
  triggers: [
    {
      abilityId: 'cast',
      text: TEXT,
      ...
      label: () => 'Staff of the Flame Magus — you gain 1 life',
