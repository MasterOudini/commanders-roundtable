// `Witches' Eye` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { WITCHES_EYE_SCRIPT } from './witchesEye';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Witches' Eye";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Witches' Eye","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WITCHES_EYE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Witches' Eye", () => {
  test("grants {1}, {T}: Scry 1. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and runs the payload', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{1}, {T}");
    ...
    { const top = [...(g.state.zones.library.p1 ?? [])].slice(-1).reverse();
