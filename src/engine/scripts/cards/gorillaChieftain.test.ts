// `Gorilla Chieftain` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GORILLA_CHIEFTAIN_SCRIPT } from './gorillaChieftain';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gorilla Chieftain";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gorilla Chieftain", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GORILLA_CHIEFTAIN_SCRIPT]),
  });
  holdEverywhere(g);
