// `Rot-Tide Gargantua` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROT_TIDE_GARGANTUA_SCRIPT } from './rotTideGargantua';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rot-Tide Gargantua";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rot-Tide Gargantua"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROT_TIDE_GARGANTUA_SCRIPT]),
...
describe("Rot-Tide Gargantua", () => {
  test("When this creature exploits a creature: the vocabulary resolves \"Each opponent sacrifices a creature of their choice.\"", () => {
    const { g } = armed(0);
