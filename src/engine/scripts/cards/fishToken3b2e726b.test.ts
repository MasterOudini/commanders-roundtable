// `Fish` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FISH_TOKEN3B2E726B_SCRIPT } from './fishToken3b2e726b';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([FISH_TOKEN3B2E726B_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: FISH_3B2E726B_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === FISH_3B2E726B_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
