// `Spiteful Prankster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPITEFUL_PRANKSTER_SCRIPT } from './spitefulPrankster';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spiteful Prankster";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPITEFUL_PRANKSTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spiteful Prankster"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPITEFUL_PRANKSTER_SCRIPT]),
...
describe("Spiteful Prankster", () => {
  test("`Spiteful Prankster` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
  ...
  test("Whenever another creature dies: it deals 1 damage to the declared player", () => {
