// `Restoration Gearsmith` - the named target is accepted and it returns to hand; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { RESTORATION_GEARSMITH_SCRIPT } from './restorationGearsmith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Restoration Gearsmith";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Restoration Gearsmith", "Grizzly Bears", "Pacifism"], ["Grizzly Bears"]],
    scripts: createRegistry([RESTORATION_GEARSMITH_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD, 'graveyard');
  const target = put(g, 'p1', "Grizzly Bears", 'graveyard');
  const wrong = put(g, 'p1', "Pacifism", 'graveyard');
  settle(g);
...
describe("Restoration Gearsmith", () => {
  test("Grizzly Bears is a legal target and it returns to hand", () => {
