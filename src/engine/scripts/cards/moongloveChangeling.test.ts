// `Moonglove Changeling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOONGLOVE_CHANGELING_SCRIPT } from './moongloveChangeling';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Moonglove Changeling";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MOONGLOVE_CHANGELING_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Moonglove Changeling"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOONGLOVE_CHANGELING_SCRIPT]),
  });
  holdEverywhere(g);
