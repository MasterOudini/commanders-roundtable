...
import { BIRD_TOKENA0F7B2F0_SCRIPT } from './birdTokena0f7b2f0';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { BIRD_A0F7B2F0_TOKEN } from '../../../data/fixtures/engineCards';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([BIRD_TOKENA0F7B2F0_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: BIRD_A0F7B2F0_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === BIRD_A0F7B2F0_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
