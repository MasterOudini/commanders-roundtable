// `Sustainer of the Realm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUSTAINER_OF_THE_REALM_SCRIPT } from './sustainerOfTheRealm';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sustainer of the Realm";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUSTAINER_OF_THE_REALM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sustainer of the Realm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUSTAINER_OF_THE_REALM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sustainer of the Realm", () => {
  test("Whenever this creature blocks: it gets +0/+2 until end of turn", () => {
