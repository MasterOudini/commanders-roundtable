// `Gnawing Zombie` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GNAWING_ZOMBIE_SCRIPT } from './gnawingZombie';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gnawing Zombie";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gnawing Zombie", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GNAWING_ZOMBIE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gnawing Zombie", () => {
  test("{1}{B}, Sacrifice a creature: the vocabulary resolves \"Target player loses 1 life and you gain 1 life.\"", () => {
