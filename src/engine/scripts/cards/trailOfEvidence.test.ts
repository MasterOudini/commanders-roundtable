// `Trail of Evidence` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRAIL_OF_EVIDENCE_SCRIPT } from './trailOfEvidence';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trail of Evidence";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trail of Evidence", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRAIL_OF_EVIDENCE_SCRIPT]),
  });
  holdEverywhere(g);
