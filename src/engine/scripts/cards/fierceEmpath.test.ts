// `Fierce Empath` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fierce Empath", "Alpha Tyrranax"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIERCE_EMPATH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fierce Empath", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a creature card with mana value 6 or greater, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Alpha Tyrranax" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Alpha Tyrranax");
      expect(found, "Alpha Tyrranax is not in the library for the search to find").toBeDefined();
