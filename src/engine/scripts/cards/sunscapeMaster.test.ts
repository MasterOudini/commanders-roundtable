// `Sunscape Master` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUNSCAPE_MASTER_SCRIPT } from './sunscapeMaster';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunscape Master";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUNSCAPE_MASTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sunscape Master", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUNSCAPE_MASTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sunscape Master", () => {
  ...
  test("{U}{U}, {T}: the declared creature returns to its owner's hand", () => {
