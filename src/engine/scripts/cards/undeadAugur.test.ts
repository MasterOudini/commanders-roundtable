// `Undead Augur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNDEAD_AUGUR_SCRIPT } from './undeadAugur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Undead Augur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Undead Augur", "Grizzly Bears", "Gutter Skulk"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNDEAD_AUGUR_SCRIPT]),
...
describe("Undead Augur", () => {
  test("Whenever this creature or another Zombie you control dies: a card is drawn and 1 life lost [dies]", () => {
    const { g, life0, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1);
    expect(g.state.players.p1?.life).toBe(life0 - 1);
  });

  test("Whenever this creature or another Zombie you control dies: a card is drawn and 1 life lost [anotherCreatureDies]", () => {
