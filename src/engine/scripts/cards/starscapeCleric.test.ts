// `Starscape Cleric` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STARSCAPE_CLERIC_SCRIPT } from './starscapeCleric';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Starscape Cleric";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Starscape Cleric", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STARSCAPE_CLERIC_SCRIPT]),
...
describe("Starscape Cleric", () => {
  test("This creature can't block.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("Whenever you gain life: each opponent loses 1 life", () => {
