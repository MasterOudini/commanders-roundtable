// `Undercity Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNDERCITY_TROLL_SCRIPT } from './undercityTroll';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Undercity Troll";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([UNDERCITY_TROLL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Undercity Troll", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNDERCITY_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Undercity Troll", () => {
  ...
  test("{2}{G}: it regenerates from the Bolt", () => {
