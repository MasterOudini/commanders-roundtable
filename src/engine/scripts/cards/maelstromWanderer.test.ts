// `Maelstrom Wanderer` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MAELSTROM_WANDERER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Maelstrom Wanderer", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAELSTROM_WANDERER_SCRIPT]),
