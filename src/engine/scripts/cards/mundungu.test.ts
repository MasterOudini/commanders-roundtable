// `Mundungu` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MUNDUNGU_SCRIPT } from './mundungu';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mundungu"], ["Cyclops of One-Eyed Pass", "Divination", "Forest"]],
    scripts: createRegistry([MUNDUNGU_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mundungu", () => {
  test("{T}: the vocabulary resolves \"Counter target spell unless its controller pays {1} and 1 life.\" - the price is declined", () => {
  ...
  test("{T}: the vocabulary resolves \"Counter target spell unless its controller pays {1} and 1 life.\" - the price is paid", () => {
    const { g, self, p2life0, payA0_p2_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
