// `Vivid Flying Fish` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VIVID_FLYING_FISH_SCRIPT } from './vividFlyingFish';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vivid Flying Fish";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([VIVID_FLYING_FISH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vivid Flying Fish"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIVID_FLYING_FISH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vivid Flying Fish", () => {
  test("`Vivid Flying Fish` - as long as it's attacking: absent while it is unmet, read once it holds", () => {
