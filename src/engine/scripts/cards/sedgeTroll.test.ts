// `Sedge Troll` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEDGE_TROLL_SCRIPT } from './sedgeTroll';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sedge Troll";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SEDGE_TROLL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sedge Troll", "Lightning Bolt", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEDGE_TROLL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sedge Troll", () => {
  test("`Sedge Troll` - as long as you control a Swamp: absent while it is unmet, read once it holds", () => {
