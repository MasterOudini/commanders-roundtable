// `Ubul Sar Gatekeepers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UBUL_SAR_GATEKEEPERS_SCRIPT } from './ubulSarGatekeepers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ubul Sar Gatekeepers";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ubul Sar Gatekeepers", "Azorius Guildgate", "Azorius Guildgate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UBUL_SAR_GATEKEEPERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ubul Sar Gatekeepers", () => {
  test("When this creature enters: the vocabulary resolves \"Target creature an opponent controls gets -2/-2 until end of turn.\"", () => {
    const { g, no } = armed(0);
