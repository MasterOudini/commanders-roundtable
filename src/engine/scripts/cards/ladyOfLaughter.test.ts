// `Lady of Laughter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LADY_OF_LAUGHTER_SCRIPT } from './ladyOfLaughter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lady of Laughter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lady of Laughter", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LADY_OF_LAUGHTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lady of Laughter", () => {
  test("Celebration — At the beginning of your end step: a card is drawn", () => {
