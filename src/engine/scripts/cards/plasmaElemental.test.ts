// `Plasma Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PLASMA_ELEMENTAL_SCRIPT } from './plasmaElemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Plasma Elemental";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Plasma Elemental", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PLASMA_ELEMENTAL_SCRIPT]),
  });
  holdEverywhere(g);
