// `Azimaet Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AZIMAET_DRAKE_SCRIPT } from './azimaetDrake';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Azimaet Drake";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([AZIMAET_DRAKE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Azimaet Drake", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AZIMAET_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
