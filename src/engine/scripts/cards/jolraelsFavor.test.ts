// `Jolrael's Favor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JOLRAELS_FAVOR_SCRIPT } from './jolraelsFavor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jolrael's Favor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jolrael's Favor", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JOLRAELS_FAVOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Jolrael's Favor", () => {
  test("{1}{G}: the enchanted creature regenerates from the Bolt", () => {
