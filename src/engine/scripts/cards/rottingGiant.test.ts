// `Rotting Giant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROTTING_GIANT_SCRIPT } from './rottingGiant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rotting Giant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rotting Giant", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROTTING_GIANT_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const priceL0_p1_0 = put(g, 'p1', "Grizzly Bears", 'graveyard');
  settle(g);
...
describe("Rotting Giant", () => {
  test("Whenever this creature attacks or blocks: the vocabulary resolves \"Sacrifice it unless you exile a card from your graveyard.\" - the price is declined [attacks]", () => {
    ...
    settle(g);
    expect(g.state.cards[priceL0_p1_0]?.zone.kind, 'declining costs nothing').toBe('graveyard');
  ...
  test("Whenever this creature attacks or blocks: the vocabulary resolves \"Sacrifice it unless you exile a card from your graveyard.\" - the price is paid [attacks]", () => {
    ...
    settle(g);
    expect(g.state.cards[priceL0_p1_0]?.zone.kind, 'the price took what it named').toBe('exile');
    expect(g.state.cards[self]?.zone.kind, 'the price was paid, so it stays').toBe('battlefield');
  });

  test("Whenever this creature attacks or blocks: the vocabulary resolves \"Sacrifice it unless you exile a card from your graveyard.\" - the price is declined [blocks]", () => {
    const { g, self, board0, priceL0_p1_0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
    must(g.submit({ t: 'AnswerPayMana', player: "p1", pay: false }));
  ...
  test("Whenever this creature attacks or blocks: the vocabulary resolves \"Sacrifice it unless you exile a card from your graveyard.\" - the price is paid [blocks]", () => {
    const { g, self, priceL0_p1_0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
    must(g.submit({ t: 'AnswerPayMana', player: "p1", pay: true, picks: [priceL0_p1_0] }));
