// `Amrou Kithkin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AMROU_KITHKIN_SCRIPT } from './amrouKithkin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Amrou Kithkin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Amrou Kithkin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AMROU_KITHKIN_SCRIPT]),
  });
  holdEverywhere(g);
