// `Senate Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SENATE_GUILDMAGE_SCRIPT } from './senateGuildmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Senate Guildmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Senate Guildmage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SENATE_GUILDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
