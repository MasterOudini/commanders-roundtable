// `Festering Evil` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FESTERING_EVIL_SCRIPT } from './festeringEvil';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Festering Evil";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Festering Evil"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FESTERING_EVIL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Festering Evil", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"This enchantment deals 1 damage to each creature and each player.\"", () => {
    const { g, no, life0, p2life0 } = armed(0);
  ...
  test("{B}{B}, Sacrifice this enchantment: the vocabulary resolves \"~ deals 3 damage to each creature and each player.\"", () => {
    const { g, self, no, life0, p2life0 } = armed(1);
