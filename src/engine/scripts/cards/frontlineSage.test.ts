// `Frontline Sage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FRONTLINE_SAGE_SCRIPT } from './frontlineSage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frontline Sage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frontline Sage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FRONTLINE_SAGE_SCRIPT]),
  });
  holdEverywhere(g);
