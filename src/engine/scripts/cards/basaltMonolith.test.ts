// `Basalt Monolith` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BASALT_MONOLITH_SCRIPT } from './basaltMonolith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Basalt Monolith";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Basalt Monolith"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BASALT_MONOLITH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Basalt Monolith", () => {
  test("This artifact doesn't untap during your untap step.: it stays tapped through its untap step", () => {
