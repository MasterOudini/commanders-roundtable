// `Glarewielder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GLAREWIELDER_SCRIPT } from './glarewielder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Glarewielder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glarewielder"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLAREWIELDER_SCRIPT]),
