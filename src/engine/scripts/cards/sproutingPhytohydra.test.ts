// `Sprouting Phytohydra` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPROUTING_PHYTOHYDRA_SCRIPT } from './sproutingPhytohydra';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sprouting Phytohydra";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; gob: InstanceId };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sprouting Phytohydra", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([SPROUTING_PHYTOHYDRA_SCRIPT]),
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, gob };
}

describe("Sprouting Phytohydra", () => {
  test("Whenever this creature is dealt damage: the vocabulary resolves \"Create a token that's a copy of this creature.\" [isDealtCombatDamage]", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 - 1);
  });

  test("Whenever this creature is dealt damage: the vocabulary resolves \"Create a token that's a copy of this creature.\" [isDealtNoncombatDamage]", () => {
