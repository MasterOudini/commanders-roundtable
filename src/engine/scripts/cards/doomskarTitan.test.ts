// `Doomskar Titan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DOOMSKAR_TITAN_SCRIPT } from './doomskarTitan';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Doomskar Titan";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DOOMSKAR_TITAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Doomskar Titan", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DOOMSKAR_TITAN_SCRIPT]),
