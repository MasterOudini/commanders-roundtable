// `Cephalid Retainer` — {U}{U} taps their ground creature; the flyer is
// refused ("without flying", D289).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { CEPHALID_RETAINER_SCRIPT } from './cephalidRetainer';
import { advanceUntil, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = 'Cephalid Retainer';
...
function placed(): { g: Game; hawk: InstanceId; bears: InstanceId } {
  const g = startedGame({ players: 2, decks: [[CARD], [HAWK, BEARS]], scripts: createRegistry([CEPHALID_RETAINER_SCRIPT]) });
