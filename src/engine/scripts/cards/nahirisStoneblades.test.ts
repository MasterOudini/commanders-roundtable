// `Nahiri's Stoneblades` — two targets get +2/+0 until cleanup; zero targets
// resolves.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry, SHIPPED_REGISTRY } from '../registry';
import { NAHIRIS_STONEBLADES_SCRIPT } from './nahirisStoneblades';
...
const SPELL = "Nahiri's Stoneblades";
...
function pt(g: Game, id: InstanceId): { power: number | null; toughness: number | null } {
  const d = deps(createRegistry([NAHIRIS_STONEBLADES_SCRIPT]));
...
function aimed(): { g: Game; spell: InstanceId; bears: InstanceId; hawk: InstanceId; logAt: number } {
  const g = startedGame({
    ...
    scripts: createRegistry([NAHIRIS_STONEBLADES_SCRIPT]),
  });
  settle(g);
  holdEverywhere(g);
...
describe("Nahiri's Stoneblades (up to two targets)", () => {
  test('two targets: both get +2/+0 until cleanup', () => {
    ...
    expect(pt(g, hawk)).toEqual({ power: 4, toughness: 3 });
  ...
  test('the suppression predicate holds (D187)', () => {
    const text = NAHIRI_S_STONEBLADES.faces[0]?.oracleText ?? '';
    expect(parseEffects(text, NAHIRI_S_STONEBLADES.name, true).mode).not.toBe('auto');
    expect(SHIPPED_REGISTRY.spell(NAHIRI_S_STONEBLADES.oracleId)).toBeDefined();
  });
