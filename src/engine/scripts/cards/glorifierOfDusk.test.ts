// `Glorifier of Dusk` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GLORIFIER_OF_DUSK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glorifier of Dusk"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLORIFIER_OF_DUSK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Glorifier of Dusk", () => {
  ...
  test("Pay 2 life: it gains vigilance until end of turn", () => {
