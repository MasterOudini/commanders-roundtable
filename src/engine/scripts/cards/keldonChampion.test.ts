// `Keldon Champion` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Keldon Champion", "Forest", "Forest", "Mountain", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KELDON_CHAMPION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Keldon Champion", () => {
  test("When this creature enters: it deals 3 damage to the declared player", () => {
