// `Sigardian Priest` - the named target is accepted and it is tapped; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SIGARDIAN_PRIEST_SCRIPT } from './sigardianPriest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sigardian Priest";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Sigardian Priest"], ["Grizzly Bears", "Thraben Standard Bearer"]],
    scripts: createRegistry([SIGARDIAN_PRIEST_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const target = put(g, 'p2', "Grizzly Bears");
  const wrong = put(g, 'p2', "Thraben Standard Bearer");
  settle(g);
...
describe("Sigardian Priest", () => {
  test("Grizzly Bears is a legal target and it is tapped", () => {
  ...
  test("Thraben Standard Bearer is refused (D294)", () => {
    const { g, wrong } = armed();
