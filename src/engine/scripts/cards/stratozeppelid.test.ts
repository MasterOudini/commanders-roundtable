// `Stratozeppelid` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STRATOZEPPELID_SCRIPT } from './stratozeppelid';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stratozeppelid";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stratozeppelid", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STRATOZEPPELID_SCRIPT]),
  });
  holdEverywhere(g);
