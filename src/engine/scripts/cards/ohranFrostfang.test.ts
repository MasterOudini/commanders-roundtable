// `Ohran Frostfang` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OHRAN_FROSTFANG_SCRIPT } from './ohranFrostfang';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ohran Frostfang";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; yes: InstanceId; bearsB: InstanceId };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([OHRAN_FROSTFANG_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ohran Frostfang", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OHRAN_FROSTFANG_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, yes, bearsB };
}

describe("Ohran Frostfang", () => {
  test("Attacking creatures you control have deathtouch.: only the attacking creature reads it", () => {
