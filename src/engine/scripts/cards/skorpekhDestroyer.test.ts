// `Skorpekh Destroyer` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SKORPEKH_DESTROYER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skorpekh Destroyer", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKORPEKH_DESTROYER_SCRIPT]),
...
describe("Skorpekh Destroyer", () => {
  test("Hyperphase Threshers — Whenever an artifact you control enters: it gains firstStrike until end of turn", () => {
