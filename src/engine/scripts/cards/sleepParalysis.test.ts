// `Sleep Paralysis` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLEEP_PARALYSIS_SCRIPT } from './sleepParalysis';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sleep Paralysis";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sleep Paralysis", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLEEP_PARALYSIS_SCRIPT]),
  });
  holdEverywhere(g);
