// `Dark-Dweller Oracle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DARK_DWELLER_ORACLE_SCRIPT } from './darkDwellerOracle';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dark-Dweller Oracle";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dark-Dweller Oracle", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DARK_DWELLER_ORACLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dark-Dweller Oracle", () => {
  test("{1}, Sacrifice a creature: the vocabulary resolves \"Exile the top card of your library. You may play that card this turn.\"", () => {
