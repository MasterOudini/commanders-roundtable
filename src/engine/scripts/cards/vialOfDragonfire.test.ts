// `Vial of Dragonfire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VIAL_OF_DRAGONFIRE_SCRIPT } from './vialOfDragonfire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vial of Dragonfire";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vial of Dragonfire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIAL_OF_DRAGONFIRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vial of Dragonfire", () => {
  test("{2}, {T}, Sacrifice this artifact: it deals 2 damage to the declared creature", () => {
