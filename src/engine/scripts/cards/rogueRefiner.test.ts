// `Rogue Refiner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROGUE_REFINER_SCRIPT } from './rogueRefiner';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rogue Refiner";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rogue Refiner"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROGUE_REFINER_SCRIPT]),
...
describe("Rogue Refiner", () => {
  test("When this creature enters: the vocabulary resolves \"Draw a card and you get {E}{E}.\"", () => {
    const { g, energy0, hand0 } = armed(0);
