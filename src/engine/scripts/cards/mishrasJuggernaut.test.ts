// `Mishra's Juggernaut` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MISHRAS_JUGGERNAUT_SCRIPT } from './mishrasJuggernaut';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mishra's Juggernaut";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mishra's Juggernaut", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MISHRAS_JUGGERNAUT_SCRIPT]),
