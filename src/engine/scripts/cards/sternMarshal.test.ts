// `Stern Marshal` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STERN_MARSHAL_SCRIPT } from './sternMarshal';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stern Marshal";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STERN_MARSHAL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stern Marshal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STERN_MARSHAL_SCRIPT]),
  });
  holdEverywhere(g);
