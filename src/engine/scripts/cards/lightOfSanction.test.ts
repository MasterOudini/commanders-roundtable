// `Light of Sanction` - the continuous prevention effect proven both ways (D385): the damage the line
// covers never arrives and nothing is spent; damage outside the line lands. Generated from one row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { LIGHT_OF_SANCTION_SCRIPT } from './lightOfSanction';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Light of Sanction";
...
function armed(): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Light of Sanction", "Grizzly Bears", "Spark Spray"], ["Cyclops of One-Eyed Pass", "Spark Spray"]],
    scripts: createRegistry([LIGHT_OF_SANCTION_SCRIPT]),
  });
  holdEverywhere(g);
