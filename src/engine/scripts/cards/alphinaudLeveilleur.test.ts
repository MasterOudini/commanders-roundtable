// `Alphinaud Leveilleur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ALPHINAUD_LEVEILLEUR_SCRIPT } from './alphinaudLeveilleur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Alphinaud Leveilleur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Alphinaud Leveilleur", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ALPHINAUD_LEVEILLEUR_SCRIPT]),
...
describe("Alphinaud Leveilleur", () => {
  test("Eukrasia — Whenever you cast your second spell each turn: a card is drawn", () => {
