// `Feather of Flight` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: draw 1); gets +1/+0 and has flying;
...
import { FEATHER_OF_FLIGHT_SCRIPT } from './featherOfFlight';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Feather of Flight";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FEATHER_OF_FLIGHT_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FEATHER_OF_FLIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Feather of Flight", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: draw 1); gets +1/+0 and has flying; Cyclops of One-Eyed Pass is untouched", () => {
