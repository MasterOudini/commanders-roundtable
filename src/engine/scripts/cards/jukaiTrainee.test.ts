// `Jukai Trainee` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JUKAI_TRAINEE_SCRIPT } from './jukaiTrainee';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jukai Trainee";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([JUKAI_TRAINEE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jukai Trainee"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JUKAI_TRAINEE_SCRIPT]),
  });
  holdEverywhere(g);
