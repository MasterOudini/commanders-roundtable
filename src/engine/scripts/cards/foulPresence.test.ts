// `Foul Presence` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { FOUL_PRESENCE_SCRIPT } from './foulPresence';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Foul Presence";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Foul Presence","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOUL_PRESENCE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Foul Presence", () => {
  test("grants {T}: Target creature gets -1/-1 until end of turn. to Grizzly Bears, and to nothing outside the scope", () => {
