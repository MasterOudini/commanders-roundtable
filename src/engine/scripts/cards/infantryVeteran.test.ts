// `Infantry Veteran` — pump on my ATTACKING creature: the right target is
// accepted, a creature that stayed home is refused (D291). Generated from one table row (D292).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { INFANTRY_VETERAN_SCRIPT } from './infantryVeteran';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Infantry Veteran";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([INFANTRY_VETERAN_SCRIPT]),
  });
...
describe("Infantry Veteran", () => {
  test('the attacking creature is a legal target and the effect lands', () => {
    ...
    settle(g);
    const d = deps(createRegistry([INFANTRY_VETERAN_SCRIPT]));
