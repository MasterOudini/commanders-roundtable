// `Twinblade Slasher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TWINBLADE_SLASHER_SCRIPT } from './twinbladeSlasher';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Twinblade Slasher";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TWINBLADE_SLASHER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Twinblade Slasher", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TWINBLADE_SLASHER_SCRIPT]),
  });
  holdEverywhere(g);
