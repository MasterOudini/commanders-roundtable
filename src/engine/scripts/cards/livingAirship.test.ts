// `Living Airship` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LIVING_AIRSHIP_SCRIPT } from './livingAirship';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Living Airship";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Living Airship", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LIVING_AIRSHIP_SCRIPT]),
  });
  holdEverywhere(g);
