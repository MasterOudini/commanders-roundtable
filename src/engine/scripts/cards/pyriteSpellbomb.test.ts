// `Pyrite Spellbomb` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PYRITE_SPELLBOMB_SCRIPT } from './pyriteSpellbomb';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pyrite Spellbomb";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pyrite Spellbomb"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PYRITE_SPELLBOMB_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pyrite Spellbomb", () => {
  test("{R}, Sacrifice this artifact: it deals 2 damage to the declared creature", () => {
