// `Guac & Marshmallow Pizza` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GUAC_MARSHMALLOW_PIZZA_SCRIPT } from './guacMarshmallowPizza';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Guac & Marshmallow Pizza";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GUAC_MARSHMALLOW_PIZZA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Guac & Marshmallow Pizza"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GUAC_MARSHMALLOW_PIZZA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Guac & Marshmallow Pizza", () => {
  test("When this artifact enters: the vocabulary resolves \"Target creature gets +2/+2 until end of turn. Untap it.\"", () => {
    const { g, no } = armed(0);
    expect(pt(g, no)).toEqual([7, 4]);
    expect(g.state.cards[no]?.tapped).toBe(false);
  });

  test("{2}, {T}, Sacrifice this artifact: 3 life is gained", () => {
