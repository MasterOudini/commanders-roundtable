// `Falkenrath Marauders` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FALKENRATH_MARAUDERS_SCRIPT } from './falkenrathMarauders';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Falkenrath Marauders";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Falkenrath Marauders"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FALKENRATH_MARAUDERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Falkenrath Marauders", () => {
  test("Whenever this creature deals combat damage to a player: it gets 2 +1/+1 counters", () => {
