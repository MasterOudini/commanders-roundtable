// `Avarax` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AVARAX_SCRIPT } from './avarax';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([AVARAX_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Avarax", "Avarax"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AVARAX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Avarax", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a card named ~, reveal it, put it into your hand, then shuffle.\"", () => {
    ...
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Avarax" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Avarax");
      expect(found, "Avarax is not in the library for the search to find").toBeDefined();
  ...
  test("{1}{R}: it gets +1/+0 until end of turn", () => {
