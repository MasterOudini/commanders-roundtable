// `Colossus of Sardia` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COLOSSUS_OF_SARDIA_SCRIPT } from './colossusOfSardia';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Colossus of Sardia";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Colossus of Sardia"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COLOSSUS_OF_SARDIA_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 1) {
    must(g.submit({ t: 'ManualAddMana', player: 'p1', target: 'p1', symbol: 'C', amount: 9 }));
...
describe("Colossus of Sardia", () => {
  ...
  test("{9}: it untaps - refused unless during your upkeep", () => {
