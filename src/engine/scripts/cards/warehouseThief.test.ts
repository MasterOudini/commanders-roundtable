// `Warehouse Thief` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WAREHOUSE_THIEF_SCRIPT } from './warehouseThief';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Warehouse Thief";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Warehouse Thief", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WAREHOUSE_THIEF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Warehouse Thief", () => {
  test("{2}, {T}, Sacrifice an artifact or creature: the vocabulary resolves \"Exile the top card of your library. Until the end of your next turn, you may play that card.\"", () => {
