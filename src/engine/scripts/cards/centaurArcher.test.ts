// `Centaur Archer` — its own {T} waits for turn 3 (summoning sickness), then
// pings their flyer for 1; a ground creature is refused.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { CENTAUR_ARCHER_SCRIPT } from './centaurArcher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = 'Centaur Archer';
...
function placed(): { g: Game; self: InstanceId; hawk: InstanceId; bears: InstanceId } {
  const g = startedGame({ players: 2, decks: [[CARD], [HAWK, BEARS]], scripts: createRegistry([CENTAUR_ARCHER_SCRIPT]) });
