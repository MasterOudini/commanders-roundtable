// `Blood-Chin Rager` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOOD_CHIN_RAGER_SCRIPT } from './bloodChinRager';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blood-Chin Rager";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLOOD_CHIN_RAGER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blood-Chin Rager", "Cliffhaven Sell-Sword"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOOD_CHIN_RAGER_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Cliffhaven Sell-Sword");
...
describe("Blood-Chin Rager", () => {
  test("Whenever this creature attacks: Warrior creatures you control gain menace until end of turn", () => {
