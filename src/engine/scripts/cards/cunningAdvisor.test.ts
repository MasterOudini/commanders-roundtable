// `Cunning Advisor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CUNNING_ADVISOR_SCRIPT } from './cunningAdvisor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cunning Advisor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cunning Advisor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CUNNING_ADVISOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cunning Advisor", () => {
  test("{T}: the vocabulary resolves \"Target opponent discards a card.\" - refused unless during your turn, before attackers are declared", () => {
