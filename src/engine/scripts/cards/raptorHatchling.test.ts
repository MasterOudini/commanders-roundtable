// `Raptor Hatchling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAPTOR_HATCHLING_SCRIPT } from './raptorHatchling';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Raptor Hatchling";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Raptor Hatchling", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([RAPTOR_HATCHLING_SCRIPT]),
  });
  holdEverywhere(g);
