// `Gibbering Barricade` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GIBBERING_BARRICADE_SCRIPT } from './gibberingBarricade';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gibbering Barricade";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gibbering Barricade", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GIBBERING_BARRICADE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gibbering Barricade", () => {
  test("{2}{B}, Sacrifice a creature: the vocabulary resolves \"You gain 1 life and draw a card.\"", () => {
    const { g, life0, hand0, fodder0 } = armed(0);
