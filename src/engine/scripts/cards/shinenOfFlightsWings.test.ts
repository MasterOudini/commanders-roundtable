// `Shinen of Flight's Wings` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHINEN_OF_FLIGHTS_WINGS_SCRIPT } from './shinenOfFlightsWings';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shinen of Flight's Wings";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHINEN_OF_FLIGHTS_WINGS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shinen of Flight's Wings"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHINEN_OF_FLIGHTS_WINGS_SCRIPT]),
...
describe("Shinen of Flight's Wings", () => {
  test("Channel — {U}, Discard this card: the declared creature is pumped until end of turn", () => {
