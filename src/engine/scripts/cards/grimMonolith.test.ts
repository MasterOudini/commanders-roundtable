// `Grim Monolith` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRIM_MONOLITH_SCRIPT } from './grimMonolith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grim Monolith";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grim Monolith"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRIM_MONOLITH_SCRIPT]),
  });
  holdEverywhere(g);
