// `Topan Freeblade` - a renownDamage trigger renown
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { TOPAN_FREEBLADE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(TOPAN_FREEBLADE, "Vigilance (Attacking doesn't cause this creature to tap.)\nRenown 1 (When this creature deals combat damage to a player, if it isn't renowned, put a +1/+1 counter on it and it becomes renowned.)");
const LINES = PRINTED.split('\n');

export const TOPAN_FREEBLADE_SCRIPT: CardScript = {
  oracleId: TOPAN_FREEBLADE.oracleId,
  name: TOPAN_FREEBLADE.name,
  triggers: [
    {
