// `Emeria Angel` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EMERIA_ANGEL_SCRIPT } from './emeriaAngel';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Emeria Angel";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Emeria Angel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EMERIA_ANGEL_SCRIPT]),
  });
  holdEverywhere(g);
