// `Pang Tong, "Young Phoenix"` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PANG_TONG_YOUNG_PHOENIX_SCRIPT } from './pangTongYoungPhoenix';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pang Tong, \"Young Phoenix\"";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PANG_TONG_YOUNG_PHOENIX_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pang Tong, \"Young Phoenix\""], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PANG_TONG_YOUNG_PHOENIX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pang Tong, \"Young Phoenix\"", () => {
  test("{T}: the declared creature is pumped until end of turn - refused unless during your turn, before attackers are declared", () => {
    const { g, self, no, condRefused } = armed(0);
    expect(pt(g, no)).toEqual([5, 4]);
