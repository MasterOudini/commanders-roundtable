// `Ravenous Skirge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAVENOUS_SKIRGE_SCRIPT } from './ravenousSkirge';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ravenous Skirge";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RAVENOUS_SKIRGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ravenous Skirge"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAVENOUS_SKIRGE_SCRIPT]),
  });
  holdEverywhere(g);
