// `Mysterio's Phantasm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MYSTERIOS_PHANTASM_SCRIPT } from './mysteriosPhantasm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mysterio's Phantasm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mysterio's Phantasm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MYSTERIOS_PHANTASM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mysterio's Phantasm", () => {
  test("Whenever this creature attacks: 1 card milled", () => {
    const { g, gy0 } = armed(0);
