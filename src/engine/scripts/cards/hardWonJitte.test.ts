// `Hard-Won Jitte` - equipped to Grizzly Bears the creature has doubleStrike; the Cyclops is untouched;
...
import { HARD_WON_JITTE_SCRIPT } from './hardWonJitte';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hard-Won Jitte";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([HARD_WON_JITTE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([HARD_WON_JITTE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hard-Won Jitte", () => {
  test("equipped to Grizzly Bears: attached, and the creature has doubleStrike; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(kw(g, host).has("doubleStrike")).toBe(true);
    expect(kw(g, other).has("doubleStrike")).toBe(false);
  });
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("doubleStrike")).toBe(false);
  });
