// `Pin to the Earth` - cast on Grizzly Bears it attaches and the enchanted creature gets -6/+0;
...
import { PIN_TO_THE_EARTH_SCRIPT } from './pinToTheEarth';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pin to the Earth";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PIN_TO_THE_EARTH_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([PIN_TO_THE_EARTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pin to the Earth", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets -6/+0; Cyclops of One-Eyed Pass is untouched", () => {
