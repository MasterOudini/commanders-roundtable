// `Haazda Exonerator` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { HAAZDA_EXONERATOR_SCRIPT } from './haazdaExonerator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Haazda Exonerator";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Haazda Exonerator", "Pacifism"], ["Sol Ring", "Grizzly Bears"]],
    scripts: createRegistry([HAAZDA_EXONERATOR_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const host = put(g, 'p2', "Grizzly Bears");
  const target = put(g, 'p1', "Pacifism", 'hand');
  const wrong = put(g, 'p2', "Sol Ring");
  settle(g);
  // Cast the Aura onto its host on p1's first main phase, so it is attached (CR 704.5m).
  ...
  must(g.submit({ t: 'CastSpell', player: 'p1', card: target }));
...
describe("Haazda Exonerator", () => {
  test("Pacifism is a legal target and it is destroyed", () => {
