// `Kathari Remnant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KATHARI_REMNANT_SCRIPT } from './kathariRemnant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kathari Remnant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kathari Remnant", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KATHARI_REMNANT_SCRIPT]),
