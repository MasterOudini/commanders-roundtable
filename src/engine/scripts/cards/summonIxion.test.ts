// `Summon: Ixion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUMMON_IXION_SCRIPT } from './summonIxion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Summon: Ixion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Summon: Ixion", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUMMON_IXION_SCRIPT]),
...
describe("Summon: Ixion", () => {
  test("I — Aerospark — Exile target creature an opponent controls until this Saga leaves the battlefield.: the vocabulary resolves \"Exile target creature an opponent controls until ~ leaves the battlefield.\"", () => {
  ...
  test("II: the vocabulary resolves \"Put a +1/+1 counter on each of up to two target creatures you control. You gain 2 life.\"", () => {
    const { g, life0, vtL2_chapter_0 } = armed(1);
    expect(g.state.cards[vtL2_chapter_0]?.counters["+1/+1"] ?? 0).toBe(1);
