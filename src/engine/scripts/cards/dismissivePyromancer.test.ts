// `Dismissive Pyromancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DISMISSIVE_PYROMANCER_SCRIPT } from './dismissivePyromancer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dismissive Pyromancer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dismissive Pyromancer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DISMISSIVE_PYROMANCER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dismissive Pyromancer", () => {
  test("{R}, {T}, Discard a card: a card is drawn", () => {
  ...
  test("{2}{R}, {T}, Sacrifice this creature: it deals 4 damage to the declared creature", () => {
