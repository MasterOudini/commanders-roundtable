// `Backlash` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { BACKLASH_SCRIPT } from './backlash';
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Backlash"], ["Grizzly Bears", "Grizzly Bears"]],
    scripts: createRegistry([BACKLASH_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  settle(g);
  must(g.submit({ t: 'ManualSetTapped', player: 'p2', cards: [wrong], tapped: true }));
