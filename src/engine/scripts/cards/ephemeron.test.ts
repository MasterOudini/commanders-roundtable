// `Ephemeron` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EPHEMERON_SCRIPT } from './ephemeron';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ephemeron";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ephemeron", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EPHEMERON_SCRIPT]),
  });
  holdEverywhere(g);
