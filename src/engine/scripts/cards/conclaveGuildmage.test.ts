// `Conclave Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONCLAVE_GUILDMAGE_SCRIPT } from './conclaveGuildmage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Conclave Guildmage";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CONCLAVE_GUILDMAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Conclave Guildmage", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONCLAVE_GUILDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Conclave Guildmage", () => {
  test("{G}, {T}: its controller's creatures gain trample until end of turn", () => {
