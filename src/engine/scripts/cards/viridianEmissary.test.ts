// `Viridian Emissary` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Viridian Emissary", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIRIDIAN_EMISSARY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Viridian Emissary", () => {
  test("When this creature dies: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
