// `Qasali Pridemage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { QASALI_PRIDEMAGE_SCRIPT } from './qasaliPridemage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Qasali Pridemage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Qasali Pridemage"], ["Cyclops of One-Eyed Pass", "Sol Ring"]],
    scripts: createRegistry([QASALI_PRIDEMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Qasali Pridemage", () => {
  test("{1}, Sacrifice this creature: the vocabulary resolves \"Destroy target artifact or enchantment.\"", () => {
