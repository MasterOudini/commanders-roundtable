// `Childhood Horror` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHILDHOOD_HORROR_SCRIPT } from './childhoodHorror';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Childhood Horror";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CHILDHOOD_HORROR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Childhood Horror", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHILDHOOD_HORROR_SCRIPT]),
  });
  holdEverywhere(g);
