// `Mirrormere Guardian` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MIRRORMERE_GUARDIAN_SCRIPT } from './mirrormereGuardian';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mirrormere Guardian";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mirrormere Guardian"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIRRORMERE_GUARDIAN_SCRIPT]),
...
describe("Mirrormere Guardian", () => {
  test("When this creature dies: the vocabulary resolves \"The Ring tempts you.\"", () => {
