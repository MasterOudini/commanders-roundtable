// `Rocky Tar Pit` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rocky Tar Pit", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROCKY_TAR_PIT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rocky Tar Pit", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a Swamp or Mountain card, put it onto the battlefield, then shuffle.\"", () => {
