// `Fortitude` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FORTITUDE_SCRIPT } from './fortitude';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fortitude";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fortitude", "Forest", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FORTITUDE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fortitude", () => {
  test("Sacrifice a Forest: the enchanted creature regenerates from the Bolt", () => {
    const { g, bearsB, fodder0 } = armed(0);
