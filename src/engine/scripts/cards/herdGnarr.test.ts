// `Herd Gnarr` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HERD_GNARR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Herd Gnarr", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HERD_GNARR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Herd Gnarr", () => {
  test("Whenever another creature you control enters: it gets +2/+2 until end of turn", () => {
