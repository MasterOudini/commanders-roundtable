// `Patrol Signaler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PATROL_SIGNALER_SCRIPT } from './patrolSignaler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Patrol Signaler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Patrol Signaler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PATROL_SIGNALER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Patrol Signaler", () => {
  test("{1}{W}, {Q}: 1 token made", () => {
