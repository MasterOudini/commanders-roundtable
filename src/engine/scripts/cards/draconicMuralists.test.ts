// `Draconic Muralists` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Draconic Muralists", "Shivan Devastator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRACONIC_MURALISTS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Draconic Muralists", () => {
  test("When this creature dies: the vocabulary resolves \"Search your library for a Dragon card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
