// `Charging Rhino` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHARGING_RHINO_SCRIPT } from './chargingRhino';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Charging Rhino";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Charging Rhino", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([CHARGING_RHINO_SCRIPT]),
  });
  holdEverywhere(g);
