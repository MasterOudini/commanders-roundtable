// `Acrobatic Maneuver` — my creature leaves for exile and comes straight
// back under its OWNER's control, and I draw; an opponent's creature is not
// a legal target.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry, SHIPPED_REGISTRY } from '../registry';
import { ACROBATIC_MANEUVER_SCRIPT } from './acrobaticManeuver';
...
const SPELL = 'Acrobatic Maneuver';
...
function aimed(): { g: Game; mine: InstanceId; theirs: InstanceId; logAt: number } {
  const g = startedGame({
    ...
    scripts: createRegistry([ACROBATIC_MANEUVER_SCRIPT]),
  });
...
describe('Acrobatic Maneuver', () => {
  ...
  test('the suppression predicate holds (D187)', () => {
    const text = ACROBATIC_MANEUVER.faces[0]?.oracleText ?? '';
    expect(parseEffects(text, ACROBATIC_MANEUVER.name, true).mode).not.toBe('auto');
    expect(SHIPPED_REGISTRY.spell(ACROBATIC_MANEUVER.oracleId)).toBeDefined();
  });
