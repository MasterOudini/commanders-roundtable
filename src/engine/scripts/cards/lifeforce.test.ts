// `Lifeforce` - counters a held spell of the named colour and refuses one of another:
// the adjective is the parser's and the validator's (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { LIFEFORCE_SCRIPT } from './lifeforce';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lifeforce";
...
function armed(which: 'right' | 'wrong'): { g: Game; self: InstanceId; spell: InstanceId; stackId: string; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Lifeforce"], ["Vampire Nighthawk", "Grizzly Bears"]],
    scripts: createRegistry([LIFEFORCE_SCRIPT]),
  });
  holdEverywhere(g);
