// `Gorehorn Minotaurs` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOREHORN_MINOTAURS_SCRIPT } from './gorehornMinotaurs';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gorehorn Minotaurs";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOREHORN_MINOTAURS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gorehorn Minotaurs", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOREHORN_MINOTAURS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gorehorn Minotaurs", () => {
  test("Bloodthirst 2 (If an opponent was dealt damage this turn, this creature enters with two +1/+1 counters on it.): cast, it enters with 2 +1/+1 counters", () => {
