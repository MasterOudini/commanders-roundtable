// `Reckless Imp` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RECKLESS_IMP_SCRIPT } from './recklessImp';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Reckless Imp";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Reckless Imp", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RECKLESS_IMP_SCRIPT]),
