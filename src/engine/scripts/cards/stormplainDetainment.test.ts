// `Stormplain Detainment` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORMPLAIN_DETAINMENT_SCRIPT } from './stormplainDetainment';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stormplain Detainment";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stormplain Detainment"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORMPLAIN_DETAINMENT_SCRIPT]),
  });
  holdEverywhere(g);
