// `Adventurer's Airship` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ADVENTURERS_AIRSHIP_SCRIPT } from './adventurersAirship';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Adventurer's Airship";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Adventurer's Airship", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ADVENTURERS_AIRSHIP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Adventurer's Airship", () => {
  test("Whenever this Vehicle attacks: a card is drawn, then one discarded", () => {
