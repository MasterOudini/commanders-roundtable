// `Fire Nation Raider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIRE_NATION_RAIDER_SCRIPT } from './fireNationRaider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fire Nation Raider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fire Nation Raider", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIRE_NATION_RAIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fire Nation Raider", () => {
  test("Raid — When this creature enters: 1 token made", () => {
