// `Brimstone Roundup` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRIMSTONE_ROUNDUP_SCRIPT } from './brimstoneRoundup';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brimstone Roundup";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brimstone Roundup", "Pyretic Ritual", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRIMSTONE_ROUNDUP_SCRIPT]),
...
describe("Brimstone Roundup", () => {
  test("Whenever you cast your second spell each turn: the vocabulary resolves \"Create a 1/1 red Mercenary creature token with \\\"{T}: Target creature you control gets +1/+0 until end of turn. Activate only as a sorcery.\\\"\"", () => {
