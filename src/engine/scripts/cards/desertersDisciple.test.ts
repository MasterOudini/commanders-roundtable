// `Deserter's Disciple` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DESERTERS_DISCIPLE_SCRIPT } from './desertersDisciple';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deserter's Disciple";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deserter's Disciple", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DESERTERS_DISCIPLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deserter's Disciple", () => {
  test("{T}: the vocabulary resolves \"Another target creature you control with power 2 or less can't be blocked this turn.\"", () => {
