// `Emissary of the Sleepless` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EMISSARY_OF_THE_SLEEPLESS_SCRIPT } from './emissaryOfTheSleepless';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Emissary of the Sleepless";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Emissary of the Sleepless", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EMISSARY_OF_THE_SLEEPLESS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Emissary of the Sleepless", () => {
