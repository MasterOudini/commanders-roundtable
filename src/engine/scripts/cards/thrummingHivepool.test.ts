// `Thrumming Hivepool` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THRUMMING_HIVEPOOL_SCRIPT } from './thrummingHivepool';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thrumming Hivepool";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([THRUMMING_HIVEPOOL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thrumming Hivepool", "Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THRUMMING_HIVEPOOL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thrumming Hivepool", () => {
  test("Slivers you control have double strike and haste.: only the Sliver reads it", () => {
  ...
  test("At the beginning of your upkeep: 2 tokens made", () => {
