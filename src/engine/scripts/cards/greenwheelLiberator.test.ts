// `Greenwheel Liberator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GREENWHEEL_LIBERATOR_SCRIPT } from './greenwheelLiberator';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Greenwheel Liberator";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GREENWHEEL_LIBERATOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Greenwheel Liberator", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GREENWHEEL_LIBERATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Greenwheel Liberator", () => {
  test("Revolt — This creature enters with two +1/+1 counters on it if a permanent left the battlefield under your control this turn.: cast, it enters with 2 +1/+1 counters", () => {
