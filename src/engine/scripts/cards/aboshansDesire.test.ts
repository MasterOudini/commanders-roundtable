// `Aboshan's Desire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ABOSHANS_DESIRE_SCRIPT } from './aboshansDesire';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aboshan's Desire";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([ABOSHANS_DESIRE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aboshan's Desire", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ABOSHANS_DESIRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aboshan's Desire", () => {
  ...
  test("Threshold — Enchanted creature has shroud as long as there are seven or more cards in your graveyard. (It can't be the target of spells or abilities.): the creature it is attached to reads it", () => {
