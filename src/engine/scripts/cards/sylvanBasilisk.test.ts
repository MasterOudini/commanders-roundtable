// `Sylvan Basilisk` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SYLVAN_BASILISK_SCRIPT } from './sylvanBasilisk';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sylvan Basilisk";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sylvan Basilisk"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SYLVAN_BASILISK_SCRIPT]),
...
describe("Sylvan Basilisk", () => {
  test("Whenever this creature becomes blocked by a creature: the vocabulary resolves \"Destroy target creature.\"", () => {
    const { g, no } = armed(0);
