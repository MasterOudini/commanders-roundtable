// `Akki Underling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AKKI_UNDERLING_SCRIPT } from './akkiUnderling';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Akki Underling";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([AKKI_UNDERLING_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Akki Underling"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AKKI_UNDERLING_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    ...
    settle(g);
    condOff = pt(g, self)[0] === 2 && pt(g, self)[1] === 1 && !kw(g, self).has("firstStrike");
...
describe("Akki Underling", () => {
  test("`Akki Underling` - as long as you have seven or more cards in hand: absent while it is unmet, read once it holds", () => {
