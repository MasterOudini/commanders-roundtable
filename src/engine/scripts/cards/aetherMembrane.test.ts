// `Aether Membrane` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AETHER_MEMBRANE_SCRIPT } from './aetherMembrane';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aether Membrane";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aether Membrane"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AETHER_MEMBRANE_SCRIPT]),
