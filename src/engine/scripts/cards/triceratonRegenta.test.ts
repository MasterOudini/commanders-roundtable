// `Triceraton Regenta` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRICERATON_REGENTA_SCRIPT } from './triceratonRegenta';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Triceraton Regenta";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Triceraton Regenta"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRICERATON_REGENTA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Triceraton Regenta", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Untap up to one other target creature.\"", () => {
    const { g, no } = armed(0);
