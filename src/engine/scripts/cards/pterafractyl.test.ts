// `Pterafractyl` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PTERAFRACTYL_SCRIPT } from './pterafractyl';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pterafractyl";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PTERAFRACTYL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pterafractyl"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PTERAFRACTYL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pterafractyl", () => {
  ...
  test("When this creature enters: 2 life is gained", () => {
