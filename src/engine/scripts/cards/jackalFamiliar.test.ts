// `Jackal Familiar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JACKAL_FAMILIAR_SCRIPT } from './jackalFamiliar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jackal Familiar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jackal Familiar", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JACKAL_FAMILIAR_SCRIPT]),
  });
  holdEverywhere(g);
