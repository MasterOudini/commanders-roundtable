// `Infinity Formula` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INFINITY_FORMULA_SCRIPT } from './infinityFormula';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Infinity Formula";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([INFINITY_FORMULA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Infinity Formula", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INFINITY_FORMULA_SCRIPT]),
...
describe("Infinity Formula", () => {
  ...
  test("Equipped creature gets +1/+2 and has \"Whenever this creature attacks, you gain 2 life.\": the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(1);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([3, 4]);
  });

  test("Equipped creature gets +1/+2 and has \"Whenever this creature attacks: 2 life is gained [attacks]", () => {
