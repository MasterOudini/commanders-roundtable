// `Dreadwing Scavenger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DREADWING_SCAVENGER_SCRIPT } from './dreadwingScavenger';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dreadwing Scavenger";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DREADWING_SCAVENGER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dreadwing Scavenger"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DREADWING_SCAVENGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dreadwing Scavenger", () => {
  test("Whenever this creature enters or attacks: a card is drawn, then one discarded", () => {
  ...
  test("Whenever this creature enters or attacks: a card is drawn, then one discarded", () => {
    const { g, hand0, gy0 } = armed(1);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0);
    expect((g.state.zones.graveyard.p1 ?? []).length).toBe(gy0 + 1);
  });

  test("Threshold — This creature gets +1/+1 and has deathtouch as long as there are seven or more cards in your graveyard.: with seven cards in the graveyard it reads the pump", () => {
