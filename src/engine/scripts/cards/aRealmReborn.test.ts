// `A Realm Reborn` - the granted MANA ability proven end to end (D372): the recipient HAS the
...
import { A_REALM_REBORN_SCRIPT } from './aRealmReborn';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "A Realm Reborn";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["A Realm Reborn","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([A_REALM_REBORN_SCRIPT]),
  });
  holdEverywhere(g);
