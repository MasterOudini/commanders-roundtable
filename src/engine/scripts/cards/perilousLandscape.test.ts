// `Perilous Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Perilous Landscape", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PERILOUS_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Perilous Landscape", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Island, Mountain, or Plains card, put it onto the battlefield tapped, then shuffle.\"", () => {
