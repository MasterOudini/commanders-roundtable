// `Grovetender Druids` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GROVETENDER_DRUIDS_SCRIPT } from './grovetenderDruids';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grovetender Druids";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grovetender Druids", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GROVETENDER_DRUIDS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Grovetender Druids", () => {
  test("Rally — Whenever this creature or another Ally you control enters: the vocabulary resolves \"You may pay {1}. If you do, create a 1/1 green Plant creature token.\" - the price is declined", () => {
  ...
  test("Rally — Whenever this creature or another Ally you control enters: the vocabulary resolves \"You may pay {1}. If you do, create a 1/1 green Plant creature token.\" - the price is paid", () => {
