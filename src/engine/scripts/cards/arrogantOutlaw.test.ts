// `Arrogant Outlaw` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ARROGANT_OUTLAW_SCRIPT } from './arrogantOutlaw';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Arrogant Outlaw";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Arrogant Outlaw"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARROGANT_OUTLAW_SCRIPT]),
  });
  holdEverywhere(g);
