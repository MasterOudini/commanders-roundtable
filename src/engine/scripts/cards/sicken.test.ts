// `Sicken` - cast on Grizzly Bears it attaches and the enchanted creature gets -1/-1;
...
import { SICKEN_SCRIPT } from './sicken';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SICKEN_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SICKEN_SCRIPT]),
  });
  holdEverywhere(g);
