// `Coastal Hornclaw` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COASTAL_HORNCLAW_SCRIPT } from './coastalHornclaw';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Coastal Hornclaw";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([COASTAL_HORNCLAW_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Coastal Hornclaw", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COASTAL_HORNCLAW_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Coastal Hornclaw", () => {
  test("Sacrifice a land: it gains flying until end of turn", () => {
