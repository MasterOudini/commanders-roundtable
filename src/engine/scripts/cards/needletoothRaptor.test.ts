// `Needletooth Raptor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NEEDLETOOTH_RAPTOR_SCRIPT } from './needletoothRaptor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Needletooth Raptor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Needletooth Raptor", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([NEEDLETOOTH_RAPTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Needletooth Raptor", () => {
  test("Enrage — Whenever this creature is dealt damage: the vocabulary resolves \"~ deals 5 damage to target creature an opponent controls.\"", () => {
    const { g, no } = armed(0);
