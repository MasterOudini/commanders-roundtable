// `Cult Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CULT_GUILDMAGE_SCRIPT } from './cultGuildmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cult Guildmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cult Guildmage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CULT_GUILDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cult Guildmage", () => {
  test("{3}{B}, {T}: the vocabulary resolves \"Target player discards a card.\"", () => {
  ...
  test("{R}, {T}: the vocabulary resolves \"~ deals 1 damage to target opponent or planeswalker.\"", () => {
