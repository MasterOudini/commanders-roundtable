// `Lightning Greaves` - equipped to Grizzly Bears the creature has haste, shroud; the Cyclops is untouched;
...
import { LIGHTNING_GREAVES_SCRIPT } from './lightningGreaves';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lightning Greaves";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([LIGHTNING_GREAVES_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([LIGHTNING_GREAVES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lightning Greaves", () => {
  test("equipped to Grizzly Bears: attached, and the creature has haste, shroud; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(kw(g, host).has("haste")).toBe(true);
    expect(kw(g, host).has("shroud")).toBe(true);
    expect(kw(g, other).has("haste")).toBe(false);
    expect(kw(g, other).has("shroud")).toBe(false);
  });
  ...
  test('equipping again moves it', () => {
    ...
    expect(kw(g, host).has("shroud")).toBe(false);
  });
