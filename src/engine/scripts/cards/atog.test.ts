// `Atog` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ATOG_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Atog", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ATOG_SCRIPT]),
  });
  holdEverywhere(g);
