// `Shacklegeist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHACKLEGEIST_SCRIPT } from './shacklegeist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shacklegeist";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; refused: boolean; tapper0: InstanceId[] };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shacklegeist", "Bile Urchin", "Grizzly Bears", "Bile Urchin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHACKLEGEIST_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const tapper0 = [put(g, 'p1', "Bile Urchin"), put(g, 'p1', "Bile Urchin")];
  settle(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, refused, tapper0 };
}

describe("Shacklegeist", () => {
  test("This creature can block only creatures with flying.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("Tap two untapped Spirits you control: the vocabulary resolves \"Tap target creature you don't control.\"", () => {
