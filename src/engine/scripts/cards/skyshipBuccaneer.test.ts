// `Skyship Buccaneer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKYSHIP_BUCCANEER_SCRIPT } from './skyshipBuccaneer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skyship Buccaneer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skyship Buccaneer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKYSHIP_BUCCANEER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skyship Buccaneer", () => {
  test("Raid — When this creature enters: a card is drawn", () => {
