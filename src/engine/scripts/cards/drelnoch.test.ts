// `Drelnoch` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRELNOCH_SCRIPT } from './drelnoch';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drelnoch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRELNOCH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Drelnoch", () => {
  test("Whenever this creature becomes blocked: 2 cards are drawn", () => {
