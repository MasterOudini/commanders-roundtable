// `Mardu Skullhunter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARDU_SKULLHUNTER_SCRIPT } from './marduSkullhunter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mardu Skullhunter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mardu Skullhunter", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARDU_SKULLHUNTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mardu Skullhunter", () => {
  test("Raid — When this creature enters: the vocabulary resolves \"Target opponent discards a card.\"", () => {
    const { g } = armed(0);
