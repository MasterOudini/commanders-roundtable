// `Canyon Crawler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CANYON_CRAWLER_SCRIPT } from './canyonCrawler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Canyon Crawler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Canyon Crawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CANYON_CRAWLER_SCRIPT]),
  });
  holdEverywhere(g);
