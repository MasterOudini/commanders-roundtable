// `Pirate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PIRATE_TOKEND5F108E7_SCRIPT } from './pirateTokend5f108e7';
...
function armed(which: number): Armed {
  const g = startedGame({
    ...
    scripts: createRegistry([PIRATE_TOKEND5F108E7_SCRIPT]),
  ...
  settle(g);
  let self: InstanceId;
  must(g.submit({ t: 'ManualCreateToken', player: 'p1', printingId: PIRATE_D5F108E7_TOKEN.scryfallId, count: 1 }));
  self = Object.values(g.state.cards).filter((c) => c.isToken && c.controller === 'p1' && c.printingId === PIRATE_D5F108E7_TOKEN.scryfallId).map((c) => c.id).pop() as InstanceId;
  settle(g);
