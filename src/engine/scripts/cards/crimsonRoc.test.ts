// `Crimson Roc` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRIMSON_ROC_SCRIPT } from './crimsonRoc';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crimson Roc";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CRIMSON_ROC_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crimson Roc"], ["Cyclops of One-Eyed Pass", "Memnite"]],
    scripts: createRegistry([CRIMSON_ROC_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Crimson Roc", () => {
  test("Whenever this creature blocks a creature without flying: it gets +1/+0 and gains firstStrike until end of turn", () => {
