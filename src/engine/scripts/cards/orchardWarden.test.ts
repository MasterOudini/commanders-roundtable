// `Orchard Warden` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORCHARD_WARDEN_SCRIPT } from './orchardWarden';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Orchard Warden";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Orchard Warden", "Grizzly Bears", "Indomitable Ancients"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORCHARD_WARDEN_SCRIPT]),
  ...
  if (which === 0) {
    bears = put(g, 'p1', "Indomitable Ancients");
...
describe("Orchard Warden", () => {
  test("Whenever another Treefolk creature you control enters: the vocabulary resolves \"Gain life equal to target creature's toughness.\"", () => {
