// `Zhentarim Bandit` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZHENTARIM_BANDIT_SCRIPT } from './zhentarimBandit';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zhentarim Bandit";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zhentarim Bandit"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZHENTARIM_BANDIT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zhentarim Bandit", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"You may pay 1 life. If you do, create a Treasure token.\" - the price is declined", () => {
    const { g } = armed(0);
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"You may pay 1 life. If you do, create a Treasure token.\" - the price is paid", () => {
    const { g, life0, board0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
