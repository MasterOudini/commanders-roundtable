// `Cathar's Call` - the permanent it is attached to HAS a quoted
...
import { CATHAR_S_CALL } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(CATHAR_S_CALL, "Enchant creature\nEnchanted creature has vigilance and \"At the beginning of your end step, create a 1/1 white Human creature token.\"");
const LINES = PRINTED.split('\n');

const REF = grantedTriggerRef(`${CATHAR_S_CALL.oracleId}#gt1`, CATHAR_S_CALL.name);
const VOCAB = vocabularyEffects("create a 1/1 white Human creature token.", CATHAR_S_CALL.name);
const VOCAB_T = vocabularyTargets("create a 1/1 white Human creature token.");

export const CATHARS_CALL_SCRIPT: CardScript = {
  oracleId: CATHAR_S_CALL.oracleId,
  name: CATHAR_S_CALL.name,
  statics: [
    {
      abilityId: 'grant-1',
  ...
  triggers: [
    {
      ...
      label: () => "Cathar's Call - create a 1/1 white Human creature token.",
