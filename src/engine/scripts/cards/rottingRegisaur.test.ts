// `Rotting Regisaur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROTTING_REGISAUR_SCRIPT } from './rottingRegisaur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rotting Regisaur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rotting Regisaur"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROTTING_REGISAUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rotting Regisaur", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Discard a card.\"", () => {
