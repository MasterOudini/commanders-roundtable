// `Big Game Hunter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BIG_GAME_HUNTER_SCRIPT } from './bigGameHunter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Big Game Hunter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Big Game Hunter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BIG_GAME_HUNTER_SCRIPT]),
...
describe("Big Game Hunter", () => {
  test("When this creature enters: the vocabulary resolves \"Destroy target creature with power 4 or greater. It can't be regenerated.\"", () => {
    const { g, no } = armed(0);
