// `Rhox Maulers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RHOX_MAULERS_SCRIPT } from './rhoxMaulers';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rhox Maulers";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RHOX_MAULERS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rhox Maulers"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RHOX_MAULERS_SCRIPT]),
  });
  holdEverywhere(g);
