// `Thing from the Deep` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THING_FROM_THE_DEEP_SCRIPT } from './thingFromTheDeep';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thing from the Deep";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thing from the Deep", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THING_FROM_THE_DEEP_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const priceL0_p1_0 = put(g, 'p1', "Island");
  settle(g);
...
describe("Thing from the Deep", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Sacrifice it unless you sacrifice an Island.\" - the price is declined", () => {
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"Sacrifice it unless you sacrifice an Island.\" - the price is paid", () => {
    const { g, self, board0, priceL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
