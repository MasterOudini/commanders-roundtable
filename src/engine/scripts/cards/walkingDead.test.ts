// `Walking Dead` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WALKING_DEAD_SCRIPT } from './walkingDead';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Walking Dead";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Walking Dead", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WALKING_DEAD_SCRIPT]),
  });
  holdEverywhere(g);
