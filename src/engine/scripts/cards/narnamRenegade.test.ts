// `Narnam Renegade` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NARNAM_RENEGADE_SCRIPT } from './narnamRenegade';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Narnam Renegade";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([NARNAM_RENEGADE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Narnam Renegade", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NARNAM_RENEGADE_SCRIPT]),
  });
  holdEverywhere(g);
