// `Graceful Cat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRACEFUL_CAT_SCRIPT } from './gracefulCat';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Graceful Cat";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GRACEFUL_CAT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Graceful Cat"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRACEFUL_CAT_SCRIPT]),
  });
  holdEverywhere(g);
