// `Brazen Wolves` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRAZEN_WOLVES_SCRIPT } from './brazenWolves';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brazen Wolves";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BRAZEN_WOLVES_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brazen Wolves"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRAZEN_WOLVES_SCRIPT]),
  });
  holdEverywhere(g);
