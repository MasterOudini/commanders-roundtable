// `Separatist Voidmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEPARATIST_VOIDMAGE_SCRIPT } from './separatistVoidmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Separatist Voidmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Separatist Voidmage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEPARATIST_VOIDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
