// `Grixis Battlemage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRIXIS_BATTLEMAGE_SCRIPT } from './grixisBattlemage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grixis Battlemage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grixis Battlemage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRIXIS_BATTLEMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Grixis Battlemage", () => {
  test("{U}, {T}: a card is drawn, then one discarded", () => {
  ...
  test("{R}, {T}: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
