// `Wirewood Hivemaster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WIREWOOD_HIVEMASTER_SCRIPT } from './wirewoodHivemaster';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wirewood Hivemaster";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wirewood Hivemaster", "Grizzly Bears", "Devoted Hero"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WIREWOOD_HIVEMASTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wirewood Hivemaster", () => {
  test("Whenever another nontoken Elf enters: 1 token made", () => {
