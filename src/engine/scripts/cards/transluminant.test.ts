// `Transluminant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRANSLUMINANT_SCRIPT } from './transluminant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Transluminant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Transluminant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRANSLUMINANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Transluminant", () => {
  test("{W}, Sacrifice this creature: the vocabulary resolves \"Create a 1/1 white Spirit creature token with flying at the beginning of the next end step.\"", () => {
    const { g, self, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 0 - 1);
    ...
    { const boardD = onBoard(g);
      ...
      settle(g);
      expect(onBoard(g), 'the delayed token came at the step').toBe(boardD + 1);
    }
