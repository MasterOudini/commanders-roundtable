// `Delivery Moogle` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Delivery Moogle", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DELIVERY_MOOGLE_SCRIPT]),
...
describe("Delivery Moogle", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library and/or graveyard for an artifact card with mana value 2 or less, reveal it, and put it into your hand. If you search your library this way, shuffle.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Sol Ring" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
