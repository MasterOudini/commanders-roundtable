// `Cadaverous Knight` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CADAVEROUS_KNIGHT_SCRIPT } from './cadaverousKnight';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cadaverous Knight";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cadaverous Knight", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CADAVEROUS_KNIGHT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cadaverous Knight", () => {
  test("{1}{B}{B}: it regenerates from the Bolt", () => {
