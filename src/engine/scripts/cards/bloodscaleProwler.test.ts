// `Bloodscale Prowler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODSCALE_PROWLER_SCRIPT } from './bloodscaleProwler';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodscale Prowler";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLOODSCALE_PROWLER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodscale Prowler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODSCALE_PROWLER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bloodscale Prowler", () => {
  test("Bloodthirst 1 (If an opponent was dealt damage this turn, this creature enters with a +1/+1 counter on it.): cast, it enters with 1 +1/+1 counter", () => {
