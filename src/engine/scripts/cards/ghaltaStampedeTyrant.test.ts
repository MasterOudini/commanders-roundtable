// `Ghalta, Stampede Tyrant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHALTA_STAMPEDE_TYRANT_SCRIPT } from './ghaltaStampedeTyrant';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghalta, Stampede Tyrant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghalta, Stampede Tyrant", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHALTA_STAMPEDE_TYRANT_SCRIPT]),
...
describe("Ghalta, Stampede Tyrant", () => {
  test("When Ghalta enters: the vocabulary resolves \"Put any number of creature cards from your hand onto the battlefield.\"", () => {
