// `Mardu Heart-Piercer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MARDU_HEART_PIERCER_SCRIPT } from './marduHeartPiercer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mardu Heart-Piercer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mardu Heart-Piercer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARDU_HEART_PIERCER_SCRIPT]),
  });
  holdEverywhere(g);
