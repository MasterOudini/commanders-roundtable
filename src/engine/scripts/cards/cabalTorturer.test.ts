// `Cabal Torturer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CABAL_TORTURER_SCRIPT } from './cabalTorturer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cabal Torturer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CABAL_TORTURER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cabal Torturer", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CABAL_TORTURER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cabal Torturer", () => {
  test("{B}, {T}: the declared creature is pumped until end of turn", () => {
    const { g, self, no } = armed(0);
    expect(pt(g, no)).toEqual([4, 1]);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("Threshold — {3}{B}{B}, {T}: the declared creature is pumped until end of turn - refused unless if there are seven or more cards in your graveyard", () => {
