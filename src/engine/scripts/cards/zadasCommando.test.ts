// `Zada's Commando` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ZADAS_COMMANDO_SCRIPT } from './zadasCommando';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zada's Commando";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zada's Commando", "Expedition Envoy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZADAS_COMMANDO_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zada's Commando", () => {
  test("Cohort — {T}, Tap an untapped Ally you control: the vocabulary resolves \"~ deals 1 damage to target opponent or planeswalker.\"", () => {
