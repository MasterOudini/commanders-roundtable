// `Undercity Scrounger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNDERCITY_SCROUNGER_SCRIPT } from './undercityScrounger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Undercity Scrounger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Undercity Scrounger", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNDERCITY_SCROUNGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Undercity Scrounger", () => {
  test("{T}: 1 token made - refused unless if a creature died this turn", () => {
