// `Flameborn Hellion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLAMEBORN_HELLION_SCRIPT } from './flamebornHellion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flameborn Hellion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flameborn Hellion", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLAMEBORN_HELLION_SCRIPT]),
  });
  holdEverywhere(g);
