// `Liliana's Elite` - a VARIABLE PUMP (D386): the delta is COUNTED at each derive
...
import { LILIANA_S_ELITE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(LILIANA_S_ELITE, "This creature gets +1/+1 for each creature card in your graveyard.");
...
function countOf(ctx: ScriptCtx, self: InstanceId, _applied: InstanceId): number {
  ...
  for (const inst of Object.values(ctx.state.cards)) {
    if (inst.zone.kind !== 'graveyard') continue;
...
export const LILIANAS_ELITE_SCRIPT: CardScript = {
  oracleId: LILIANA_S_ELITE.oracleId,
  name: LILIANA_S_ELITE.name,
  statics: [
    {
      abilityId: 'pump-0',
