// `Study` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STUDY_SCRIPT } from './study';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Study"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STUDY_SCRIPT]),
  });
  holdEverywhere(g);
