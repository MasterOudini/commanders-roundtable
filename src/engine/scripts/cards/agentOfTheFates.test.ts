// `Agent of the Fates` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AGENT_OF_THE_FATES_SCRIPT } from './agentOfTheFates';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Agent of the Fates";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Agent of the Fates", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AGENT_OF_THE_FATES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Agent of the Fates", () => {
  test("Heroic — Whenever you cast a spell that targets this creature: the vocabulary resolves \"Each opponent sacrifices a creature of their choice.\"", () => {
    const { g } = armed(0);
