// `Hopeless Nightmare` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOPELESS_NIGHTMARE_SCRIPT } from './hopelessNightmare';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hopeless Nightmare";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hopeless Nightmare", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOPELESS_NIGHTMARE_SCRIPT]),
...
describe("Hopeless Nightmare", () => {
  test("When this enchantment enters: the vocabulary resolves \"Each opponent discards a card and loses 2 life.\"", () => {
    const { g, p2life0, p2hand0 } = armed(0);
    let qdL0 = 0;
    ...
    expect(qdL0, 'p1 was not in the scope').toBe(0);
  ...
  test("{2}{B}: the vocabulary resolves \"Sacrifice this enchantment.\"", () => {
