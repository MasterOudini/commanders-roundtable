// `Gwyllion Hedge-Mage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GWYLLION_HEDGE_MAGE_SCRIPT } from './gwyllionHedgeMage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gwyllion Hedge-Mage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gwyllion Hedge-Mage", "Plains", "Plains", "Swamp", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GWYLLION_HEDGE_MAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gwyllion Hedge-Mage", () => {
  test("When this creature enters: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("When this creature enters: the vocabulary resolves \"Put a -1/-1 counter on target creature.\"", () => {
    const { g, no } = armed(1);
