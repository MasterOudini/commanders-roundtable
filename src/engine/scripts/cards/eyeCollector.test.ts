// `Eye Collector` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EYE_COLLECTOR_SCRIPT } from './eyeCollector';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Eye Collector";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Eye Collector"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EYE_COLLECTOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Eye Collector", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Each player mills a card.\"", () => {
