// `Elfsworn Giant` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elfsworn Giant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELFSWORN_GIANT_SCRIPT]),
  });
  holdEverywhere(g);
