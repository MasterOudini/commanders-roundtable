// `Stillmoon Cavalier` - the printed cost buys the pump until end of turn; it ends at cleanup;
// replay equal (D301). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { STILLMOON_CAVALIER_SCRIPT } from './stillmoonCavalier';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stillmoon Cavalier";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STILLMOON_CAVALIER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stillmoon Cavalier"], ["Grizzly Bears"]],
    scripts: createRegistry([STILLMOON_CAVALIER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stillmoon Cavalier", () => {
  test("{W/B}: it gains flying until end of turn", () => {
    const { g, self } = armed(0);
    expect(pt(g, self)).toEqual([2, 1]);
    expect(kw(g, self).has("flying")).toBe(true);
  });

  test("{W/B}: it gains firstStrike until end of turn", () => {
    const { g, self } = armed(1);
    expect(pt(g, self)).toEqual([2, 1]);
    expect(kw(g, self).has("firstStrike")).toBe(true);
  });

  test("{W/B}{W/B}: it gets +1/+0 until end of turn", () => {
