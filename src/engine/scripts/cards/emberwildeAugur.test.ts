// `Emberwilde Augur` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EMBERWILDE_AUGUR_SCRIPT } from './emberwildeAugur';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Emberwilde Augur";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Emberwilde Augur"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EMBERWILDE_AUGUR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Emberwilde Augur", () => {
  test("Sacrifice this creature: it deals 3 damage to the declared player - refused unless during your upkeep", () => {
