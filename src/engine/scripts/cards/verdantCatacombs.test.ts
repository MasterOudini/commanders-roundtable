// `Verdant Catacombs` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Verdant Catacombs", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VERDANT_CATACOMBS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Verdant Catacombs", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for a Swamp or Forest card, put it onto the battlefield, then shuffle.\"", () => {
