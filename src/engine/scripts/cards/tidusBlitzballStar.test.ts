// `Tidus, Blitzball Star` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TIDUS_BLITZBALL_STAR_SCRIPT } from './tidusBlitzballStar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tidus, Blitzball Star";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tidus, Blitzball Star", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TIDUS_BLITZBALL_STAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Tidus, Blitzball Star", () => {
  test("Whenever an artifact you control enters: it gets 1 +1/+1 counter", () => {
    const { g, self } = armed(0);
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(1);
  });

  test("Whenever Tidus attacks: the vocabulary resolves \"Tap target creature an opponent controls.\"", () => {
    const { g, no } = armed(1);
