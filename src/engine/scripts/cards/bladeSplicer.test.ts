// `Blade Splicer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLADE_SPLICER_SCRIPT } from './bladeSplicer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blade Splicer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLADE_SPLICER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blade Splicer", "Gilded Sentinel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLADE_SPLICER_SCRIPT]),
  });
  holdEverywhere(g);
