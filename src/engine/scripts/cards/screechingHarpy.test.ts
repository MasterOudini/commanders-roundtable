// `Screeching Harpy` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCREECHING_HARPY_SCRIPT } from './screechingHarpy';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Screeching Harpy";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Screeching Harpy", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCREECHING_HARPY_SCRIPT]),
  });
  holdEverywhere(g);
