// `Jhessian Thief` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JHESSIAN_THIEF_SCRIPT } from './jhessianThief';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jhessian Thief";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jhessian Thief"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JHESSIAN_THIEF_SCRIPT]),
  });
  holdEverywhere(g);
