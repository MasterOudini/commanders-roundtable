// `Neurok Invisimancer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NEUROK_INVISIMANCER_SCRIPT } from './neurokInvisimancer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Neurok Invisimancer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Neurok Invisimancer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NEUROK_INVISIMANCER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Neurok Invisimancer", () => {
  test("This creature can't be blocked.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("When this creature enters: the vocabulary resolves \"Target creature can't be blocked this turn.\"", () => {
    const { g, no } = armed(1);
