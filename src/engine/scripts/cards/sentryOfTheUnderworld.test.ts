// `Sentry of the Underworld` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SENTRY_OF_THE_UNDERWORLD_SCRIPT } from './sentryOfTheUnderworld';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sentry of the Underworld";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sentry of the Underworld", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SENTRY_OF_THE_UNDERWORLD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sentry of the Underworld", () => {
  test("{W}{B}, Pay 3 life: it regenerates from the Bolt", () => {
