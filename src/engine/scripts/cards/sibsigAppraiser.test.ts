// `Sibsig Appraiser` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SIBSIG_APPRAISER_SCRIPT } from './sibsigAppraiser';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sibsig Appraiser";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sibsig Appraiser"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIBSIG_APPRAISER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sibsig Appraiser", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top two cards of your library. Put one of them into your hand and the other into your graveyard.\"", () => {
    const { g } = armed(0);
