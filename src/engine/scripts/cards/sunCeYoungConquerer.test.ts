// `Sun Ce, Young Conquerer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SUN_CE_YOUNG_CONQUERER_SCRIPT } from './sunCeYoungConquerer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sun Ce, Young Conquerer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sun Ce, Young Conquerer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SUN_CE_YOUNG_CONQUERER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sun Ce, Young Conquerer", () => {
  test("When Sun Ce enters: the vocabulary resolves \"Return target creature to its owner's hand.\"", () => {
    const { g, no } = armed(0);
