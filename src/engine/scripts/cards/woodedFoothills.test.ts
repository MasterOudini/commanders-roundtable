// `Wooded Foothills` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wooded Foothills", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WOODED_FOOTHILLS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wooded Foothills", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for a Mountain or Forest card, put it onto the battlefield, then shuffle.\"", () => {
