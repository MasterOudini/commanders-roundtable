// `Lunk Errant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LUNK_ERRANT_SCRIPT } from './lunkErrant';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lunk Errant";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LUNK_ERRANT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lunk Errant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LUNK_ERRANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lunk Errant", () => {
  test("Whenever this creature attacks alone: it gets +1/+1 and gains trample until end of turn", () => {
