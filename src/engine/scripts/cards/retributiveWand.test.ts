// `Retributive Wand` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RETRIBUTIVE_WAND_SCRIPT } from './retributiveWand';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Retributive Wand";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Retributive Wand", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RETRIBUTIVE_WAND_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Retributive Wand", () => {
  test("{3}, {T}: it deals 1 damage to the declared creature", () => {
    const { g, self, no } = armed(0);
    expect(g.state.cards[no]?.damage).toBe(1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("When this artifact is put into a graveyard from the battlefield: it deals 5 damage to the declared creature", () => {
    const { g, no } = armed(1);
