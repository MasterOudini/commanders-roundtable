// `Bloodstained Mire` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodstained Mire", "Swamp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODSTAINED_MIRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bloodstained Mire", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for a Swamp or Mountain card, put it onto the battlefield, then shuffle.\"", () => {
    const { g, self, life0, board0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Swamp" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Swamp");
      expect(found, "Swamp is not in the library for the search to find").toBeDefined();
