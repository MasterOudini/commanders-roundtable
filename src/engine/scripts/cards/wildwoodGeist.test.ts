// `Wildwood Geist` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WILDWOOD_GEIST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Wildwood Geist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WILDWOOD_GEIST_SCRIPT]),
...
describe("Wildwood Geist", () => {
  test("`Wildwood Geist` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
