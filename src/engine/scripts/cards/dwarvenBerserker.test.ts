// `Dwarven Berserker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DWARVEN_BERSERKER_SCRIPT } from './dwarvenBerserker';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dwarven Berserker";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DWARVEN_BERSERKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dwarven Berserker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DWARVEN_BERSERKER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dwarven Berserker", () => {
  test("Whenever this creature becomes blocked: it gets +3/+0 and gains trample until end of turn", () => {
