// `Tower Geist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TOWER_GEIST_SCRIPT } from './towerGeist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tower Geist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tower Geist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TOWER_GEIST_SCRIPT]),
  });
  holdEverywhere(g);
