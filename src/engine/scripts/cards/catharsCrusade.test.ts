// `Cathars' Crusade` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cathars' Crusade", "Coral Eel", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CATHARS_CRUSADE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cathars' Crusade", () => {
  test("Whenever a creature you control enters: each of its controller's creatures gets 1 +1/+1 counter", () => {
