// `Insectoid Exterminator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INSECTOID_EXTERMINATOR_SCRIPT } from './insectoidExterminator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Insectoid Exterminator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Insectoid Exterminator", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INSECTOID_EXTERMINATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Insectoid Exterminator", () => {
  test("Disappear — At the beginning of your end step: scry 1 asks, and the cards stay on top", () => {
