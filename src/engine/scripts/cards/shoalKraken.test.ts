// `Shoal Kraken` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHOAL_KRAKEN_SCRIPT } from './shoalKraken';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shoal Kraken";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shoal Kraken", "Grizzly Bears", "Nyxborn Courser"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHOAL_KRAKEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shoal Kraken", () => {
  test("Constellation — Whenever an enchantment you control enters: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(0);
