// `Secret Passage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SECRET_PASSAGE_SCRIPT } from './secretPassage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Secret Passage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Secret Passage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SECRET_PASSAGE_SCRIPT]),
  });
  holdEverywhere(g);
