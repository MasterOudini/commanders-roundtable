// `Blisterpod` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLISTERPOD_SCRIPT } from './blisterpod';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blisterpod";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blisterpod"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLISTERPOD_SCRIPT]),
