// `Mudbrawler Raiders` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MUDBRAWLER_RAIDERS_SCRIPT } from './mudbrawlerRaiders';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mudbrawler Raiders";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mudbrawler Raiders", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Aegis Turtle"]],
    scripts: createRegistry([MUDBRAWLER_RAIDERS_SCRIPT]),
