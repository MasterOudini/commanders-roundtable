// `Malamet Brawler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MALAMET_BRAWLER_SCRIPT } from './malametBrawler';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Malamet Brawler";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MALAMET_BRAWLER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Malamet Brawler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MALAMET_BRAWLER_SCRIPT]),
...
describe("Malamet Brawler", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Target attacking creature gains trample until end of turn.\"", () => {
