// `Judge's Familiar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JUDGES_FAMILIAR_SCRIPT } from './judgesFamiliar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Judge's Familiar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Judge's Familiar"], ["Cyclops of One-Eyed Pass", "Divination", "Forest"]],
    scripts: createRegistry([JUDGES_FAMILIAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Judge's Familiar", () => {
  test("Sacrifice this creature: the vocabulary resolves \"Counter target instant or sorcery spell unless its controller pays {1}.\" - the price is declined", () => {
  ...
  test("Sacrifice this creature: the vocabulary resolves \"Counter target instant or sorcery spell unless its controller pays {1}.\" - the price is paid", () => {
    const { g, self, payA0_p2_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p2"); }
