// `Numai Outcast` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NUMAI_OUTCAST_SCRIPT } from './numaiOutcast';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Numai Outcast";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Numai Outcast", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NUMAI_OUTCAST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Numai Outcast", () => {
  test("{B}, Pay 5 life: it regenerates from the Bolt", () => {
