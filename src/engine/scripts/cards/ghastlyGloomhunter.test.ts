// `Ghastly Gloomhunter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHASTLY_GLOOMHUNTER_SCRIPT } from './ghastlyGloomhunter';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghastly Gloomhunter";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GHASTLY_GLOOMHUNTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghastly Gloomhunter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHASTLY_GLOOMHUNTER_SCRIPT]),
  });
  holdEverywhere(g);
