// `Acidic Sliver` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { ACIDIC_SLIVER_SCRIPT } from './acidicSliver';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Acidic Sliver";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Acidic Sliver","Metallic Sliver"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ACIDIC_SLIVER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Acidic Sliver", () => {
  test("grants {2}, Sacrifice this permanent: This permanent deals 2 damage to any target. to Metallic Sliver, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and runs the payload', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{2}, Sacrifice this permanent");
    ...
    // The recipient IS the source, so "Sacrifice this permanent" eats the recipient.
    expect(g.state.cards[host]?.zone.kind).toBe('graveyard');
    settle(g);
    expect(g.state.players.p2?.life).toBe(p2life0 + -2);
    expect(Object.values(g.state.cards).filter((c) => c.zone.kind === 'battlefield' && c.controller === 'p1').length).toBe(board0 + -1);
