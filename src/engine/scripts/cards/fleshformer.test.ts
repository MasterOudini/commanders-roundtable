// `Fleshformer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLESHFORMER_SCRIPT } from './fleshformer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fleshformer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fleshformer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLESHFORMER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fleshformer", () => {
  test("{W}{U}{B}{R}{G}: the vocabulary resolves \"~ gets +2/+2 and gains fear until end of turn. Target creature gets -2/-2 until end of turn.\" - refused unless during your turn", () => {
    const { g, no, condRefused } = armed(0);
