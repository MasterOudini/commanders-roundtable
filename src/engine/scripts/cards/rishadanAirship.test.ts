// `Rishadan Airship` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RISHADAN_AIRSHIP_SCRIPT } from './rishadanAirship';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rishadan Airship";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rishadan Airship", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RISHADAN_AIRSHIP_SCRIPT]),
  });
  holdEverywhere(g);
