// `Prosperous Thief` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PROSPEROUS_THIEF_SCRIPT } from './prosperousThief';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Prosperous Thief";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Prosperous Thief", "Triton Shorethief", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PROSPEROUS_THIEF_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const stillFix0 = put(g, 'p1', "Triton Shorethief");
  settle(g);
...
describe("Prosperous Thief", () => {
  test("Whenever one or more Ninja or Rogue creatures you control deal combat damage to a player: 1 token made", () => {
