// `Pouncing Lynx` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([POUNCING_LYNX_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pouncing Lynx"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([POUNCING_LYNX_SCRIPT]),
...
describe("Pouncing Lynx", () => {
  test("`Pouncing Lynx` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
