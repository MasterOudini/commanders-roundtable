// `Water Wurm` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WATER_WURM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Water Wurm"], ["Cyclops of One-Eyed Pass", "Island"]],
    scripts: createRegistry([WATER_WURM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Water Wurm", () => {
  test("`Water Wurm` - as long as an opponent controls an Island: absent while it is unmet, read once it holds", () => {
