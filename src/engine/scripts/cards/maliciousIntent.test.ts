// `Malicious Intent` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MALICIOUS_INTENT_SCRIPT } from './maliciousIntent';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { legalActions } from '../../legal';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Malicious Intent";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Malicious Intent", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MALICIOUS_INTENT_SCRIPT]),
...
describe("Malicious Intent", () => {
  ...
  test("Enchanted creature has \"{T}: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
