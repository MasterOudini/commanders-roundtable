// `Warlord's Axe` - equipped to Grizzly Bears the creature gets +3/+1; the Cyclops is untouched;
...
import { WARLORDS_AXE_SCRIPT } from './warlordsAxe';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Warlord's Axe";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WARLORDS_AXE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([WARLORDS_AXE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Warlord's Axe", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +3/+1; the Cyclops is untouched", () => {
