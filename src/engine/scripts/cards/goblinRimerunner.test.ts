// `Goblin Rimerunner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_RIMERUNNER_SCRIPT } from './goblinRimerunner';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Rimerunner";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOBLIN_RIMERUNNER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Rimerunner", "Snow-Covered Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_RIMERUNNER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Rimerunner", () => {
  test("{T}: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
  ...
  test("{S}: it gains haste until end of turn", () => {
