// `Dutiful Thrull` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DUTIFUL_THRULL_SCRIPT } from './dutifulThrull';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dutiful Thrull";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dutiful Thrull", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUTIFUL_THRULL_SCRIPT]),
  });
  holdEverywhere(g);
