// `Deity of Scars` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEITY_OF_SCARS_SCRIPT } from './deityOfScars';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deity of Scars";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DEITY_OF_SCARS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deity of Scars", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEITY_OF_SCARS_SCRIPT]),
...
describe("Deity of Scars", () => {
  ...
  test("{B/G}, Remove a -1/-1 counter from this creature: it regenerates from the Bolt", () => {
