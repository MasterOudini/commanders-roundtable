// `Forerunner of the Heralds` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FORERUNNER_OF_THE_HERALDS_SCRIPT } from './forerunnerOfTheHeralds';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Forerunner of the Heralds";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Forerunner of the Heralds", "Grizzly Bears", "Merfolk of the Pearl Trident", "Merfolk of the Pearl Trident"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FORERUNNER_OF_THE_HERALDS_SCRIPT]),
...
describe("Forerunner of the Heralds", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a Merfolk card, reveal it, then shuffle and put that card on top.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Merfolk of the Pearl Trident" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Merfolk of the Pearl Trident");
      expect(found, "Merfolk of the Pearl Trident is not in the library for the search to find").toBeDefined();
      must(g.submit({ t: 'AnswerSearchLibrary', player: 'p1', cards: [found as InstanceId], declined: false }));
      settle(g);
      expect(g.state.cards[found as InstanceId]?.zone).toEqual({ kind: 'library', player: 'p1' });
  ...
  test("Whenever another Merfolk you control enters: it gets 1 +1/+1 counter", () => {
