// `Endless Wurm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ENDLESS_WURM_SCRIPT } from './endlessWurm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Endless Wurm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Endless Wurm", "Hissing Miasma"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ENDLESS_WURM_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const priceL1_p1_0 = put(g, 'p1', "Hissing Miasma");
  settle(g);
...
describe("Endless Wurm", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you sacrifice an enchantment.\" - the price is declined", () => {
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you sacrifice an enchantment.\" - the price is paid", () => {
    const { g, self, board0, priceL1_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
