// `Impetuous Sunchaser` - every printed ability proven in its own game: the cost's mark, the pump
...
import { IMPETUOUS_SUNCHASER_SCRIPT } from './impetuousSunchaser';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Impetuous Sunchaser";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Impetuous Sunchaser", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([IMPETUOUS_SUNCHASER_SCRIPT]),
  });
  holdEverywhere(g);
