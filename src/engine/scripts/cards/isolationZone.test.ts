// `Isolation Zone` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ISOLATION_ZONE_SCRIPT } from './isolationZone';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Isolation Zone";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Isolation Zone"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ISOLATION_ZONE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Isolation Zone", () => {
  test("When this enchantment enters: the vocabulary resolves \"Exile target creature or enchantment an opponent controls until this enchantment leaves the battlefield.\"", () => {
