// `Kongming, "Sleeping Dragon"` - the anthem reaches the permanent its scope names and not the other;
// it ends when the source leaves; replay equal (D300). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { KONGMING_SLEEPING_DRAGON_SCRIPT } from './kongmingSleepingDragon';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kongming, \"Sleeping Dragon\"";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KONGMING_SLEEPING_DRAGON_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Kongming, \"Sleeping Dragon\"", "Coral Eel"], ["Crimson Kobolds"]],
    scripts: createRegistry([KONGMING_SLEEPING_DRAGON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kongming, \"Sleeping Dragon\"", () => {
