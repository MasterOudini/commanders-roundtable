// `Bloodrage Vampire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODRAGE_VAMPIRE_SCRIPT } from './bloodrageVampire';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodrage Vampire";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLOODRAGE_VAMPIRE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodrage Vampire", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODRAGE_VAMPIRE_SCRIPT]),
  });
  holdEverywhere(g);
