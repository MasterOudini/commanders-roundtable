// `Voldaren Duelist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VOLDAREN_DUELIST_SCRIPT } from './voldarenDuelist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Voldaren Duelist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Voldaren Duelist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VOLDAREN_DUELIST_SCRIPT]),
  });
  holdEverywhere(g);
