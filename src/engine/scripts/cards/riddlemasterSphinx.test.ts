// `Riddlemaster Sphinx` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RIDDLEMASTER_SPHINX_SCRIPT } from './riddlemasterSphinx';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Riddlemaster Sphinx";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Riddlemaster Sphinx"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RIDDLEMASTER_SPHINX_SCRIPT]),
  });
  holdEverywhere(g);
