// `Fire Shrine Keeper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIRE_SHRINE_KEEPER_SCRIPT } from './fireShrineKeeper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fire Shrine Keeper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fire Shrine Keeper"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIRE_SHRINE_KEEPER_SCRIPT]),
...
describe("Fire Shrine Keeper", () => {
  test("{7}{R}, {T}, Sacrifice this creature: the vocabulary resolves \"~ deals 3 damage to each of up to two target creatures.\"", () => {
