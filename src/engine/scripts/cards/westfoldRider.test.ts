// `Westfold Rider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WESTFOLD_RIDER_SCRIPT } from './westfoldRider';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Westfold Rider";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Westfold Rider"], ["Cyclops of One-Eyed Pass", "Sol Ring"]],
    scripts: createRegistry([WESTFOLD_RIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Westfold Rider", () => {
  test("Sacrifice this creature: the vocabulary resolves \"Destroy target artifact or enchantment.\"", () => {
