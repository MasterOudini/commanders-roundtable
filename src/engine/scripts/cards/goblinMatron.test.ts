// `Goblin Matron` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Matron", "Gobland"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_MATRON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Matron", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for a Goblin card, reveal that card, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
