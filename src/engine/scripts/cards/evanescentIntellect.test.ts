// `Evanescent Intellect` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EVANESCENT_INTELLECT_SCRIPT } from './evanescentIntellect';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { legalActions } from '../../legal';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Evanescent Intellect";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Evanescent Intellect", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EVANESCENT_INTELLECT_SCRIPT]),
...
describe("Evanescent Intellect", () => {
  test("Enchanted creature has \"{1}{U}, {T}: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
  });

  test("Enchanted creature has \"{1}{U}, {T}: the vocabulary resolves \"Target player mills three cards.\"", () => {
