// `Enslaved Dwarf` - the named target is accepted and the pump lands; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ENSLAVED_DWARF_SCRIPT } from './enslavedDwarf';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Enslaved Dwarf";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Enslaved Dwarf"], ["Vampire Nighthawk", "Grizzly Bears"]],
    scripts: createRegistry([ENSLAVED_DWARF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Enslaved Dwarf", () => {
  test("Vampire Nighthawk is a legal target and the pump lands", () => {
    ...
    settle(g);
    const d = deps(createRegistry([ENSLAVED_DWARF_SCRIPT]));
