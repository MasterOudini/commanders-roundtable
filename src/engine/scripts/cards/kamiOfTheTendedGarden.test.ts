// `Kami of the Tended Garden` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KAMI_OF_THE_TENDED_GARDEN_SCRIPT } from './kamiOfTheTendedGarden';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kami of the Tended Garden";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kami of the Tended Garden", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KAMI_OF_THE_TENDED_GARDEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kami of the Tended Garden", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you pay {G}.\" - the price is declined", () => {
    const { g, self, board0, payL0_p1_0 } = armed(0);
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you pay {G}.\" - the price is paid", () => {
    const { g, self, payL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
