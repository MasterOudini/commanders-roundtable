// `Elvish Rejuvenator` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elvish Rejuvenator", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELVISH_REJUVENATOR_SCRIPT]),
...
describe("Elvish Rejuvenator", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top five cards of your library. You may put a land card from among them onto the battlefield tapped. Put the rest on the bottom of your library in a random order.\"", () => {
    const { g } = armed(0);
