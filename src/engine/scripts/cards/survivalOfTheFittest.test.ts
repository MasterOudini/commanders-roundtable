// `Survival of the Fittest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SURVIVAL_OF_THE_FITTEST_SCRIPT } from './survivalOfTheFittest';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Survival of the Fittest";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Survival of the Fittest", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SURVIVAL_OF_THE_FITTEST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Survival of the Fittest", () => {
  test("{G}, Discard a creature card: the vocabulary resolves \"Search your library for a creature card, reveal that card, put it into your hand, then shuffle.\"", () => {
    const { g, disc0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Grizzly Bears" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
