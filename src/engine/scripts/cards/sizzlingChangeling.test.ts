// `Sizzling Changeling` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SIZZLING_CHANGELING_SCRIPT } from './sizzlingChangeling';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sizzling Changeling";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sizzling Changeling"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIZZLING_CHANGELING_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sizzling Changeling", () => {
  test("When this creature dies: the vocabulary resolves \"Exile the top card of your library. Until the end of your next turn, you may play that card.\"", () => {
    const { g } = armed(0);
