// `Unworthy Dead` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNWORTHY_DEAD_SCRIPT } from './unworthyDead';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Unworthy Dead";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Unworthy Dead", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNWORTHY_DEAD_SCRIPT]),
  });
  holdEverywhere(g);
