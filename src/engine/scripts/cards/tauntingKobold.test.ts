// `Taunting Kobold` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TAUNTING_KOBOLD_SCRIPT } from './tauntingKobold';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Taunting Kobold";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Taunting Kobold"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TAUNTING_KOBOLD_SCRIPT]),
...
describe("Taunting Kobold", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Goad target creature an opponent controls.\"", () => {
    const { g, no } = armed(0);
