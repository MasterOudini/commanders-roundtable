// `Pyreheart Wolf` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PYREHEART_WOLF_SCRIPT } from './pyreheartWolf';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pyreheart Wolf";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PYREHEART_WOLF_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pyreheart Wolf", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PYREHEART_WOLF_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pyreheart Wolf", () => {
  test("Whenever this creature attacks: its controller's creatures gain menace until end of turn", () => {
