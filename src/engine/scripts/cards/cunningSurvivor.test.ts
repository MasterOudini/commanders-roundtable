// `Cunning Survivor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CUNNING_SURVIVOR_SCRIPT } from './cunningSurvivor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cunning Survivor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CUNNING_SURVIVOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cunning Survivor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CUNNING_SURVIVOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cunning Survivor", () => {
  test("Whenever you cycle or discard a card: it gets +1/+0 until end of turn and can't be blocked this turn", () => {
