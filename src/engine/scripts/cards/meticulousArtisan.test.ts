// `Meticulous Artisan` - every printed ability proven in its own game: the cost's mark, the pump
...
import { METICULOUS_ARTISAN_SCRIPT } from './meticulousArtisan';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Meticulous Artisan";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Meticulous Artisan"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([METICULOUS_ARTISAN_SCRIPT]),
  });
  holdEverywhere(g);
