// `Elvish Lyrist` — Druid Lyrist's text on its own oracle id, proven on its
// own.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ELVISH_LYRIST_SCRIPT } from './elvishLyrist';
import { advanceUntil, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const LYRIST = 'Elvish Lyrist';
...
function armed(): { g: Game; lyrist: InstanceId; theirs: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ELVISH_LYRIST_SCRIPT]),
  });
