// `Glimmer Bairn` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GLIMMER_BAIRN_SCRIPT } from './glimmerBairn';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GLIMMER_BAIRN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glimmer Bairn"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLIMMER_BAIRN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Glimmer Bairn", () => {
  test("Sacrifice a token: it gets +2/+2 until end of turn", () => {
