// `Merciless Eternal` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERCILESS_ETERNAL_SCRIPT } from './mercilessEternal';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Merciless Eternal";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MERCILESS_ETERNAL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merciless Eternal", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERCILESS_ETERNAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Merciless Eternal", () => {
  test("{2}{B}, Discard a card: it gets +2/+2 until end of turn", () => {
