// `Manifold Key` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MANIFOLD_KEY_SCRIPT } from './manifoldKey';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Manifold Key";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Manifold Key"], ["Cyclops of One-Eyed Pass", "Sol Ring"]],
    scripts: createRegistry([MANIFOLD_KEY_SCRIPT]),
  });
  holdEverywhere(g);
