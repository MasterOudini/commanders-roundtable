// `Monastery Mentor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MONASTERY_MENTOR_SCRIPT } from './monasteryMentor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Monastery Mentor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Monastery Mentor", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MONASTERY_MENTOR_SCRIPT]),
  });
  holdEverywhere(g);
