// `Tel-Jilad Exile` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEL_JILAD_EXILE_SCRIPT } from './telJiladExile';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tel-Jilad Exile";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tel-Jilad Exile", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEL_JILAD_EXILE_SCRIPT]),
  });
  holdEverywhere(g);
