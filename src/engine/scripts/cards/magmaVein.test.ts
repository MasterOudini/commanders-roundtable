// `Magma Vein` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAGMA_VEIN_SCRIPT } from './magmaVein';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Magma Vein";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Magma Vein", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAGMA_VEIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Magma Vein", () => {
  test("{R}, Sacrifice a land: the vocabulary resolves \"~ deals 1 damage to each creature without flying.\"", () => {
