// `Seasoned Marshal` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEASONED_MARSHAL_SCRIPT } from './seasonedMarshal';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Seasoned Marshal";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seasoned Marshal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEASONED_MARSHAL_SCRIPT]),
  });
  holdEverywhere(g);
