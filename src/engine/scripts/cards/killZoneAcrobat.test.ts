// `Kill-Zone Acrobat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KILL_ZONE_ACROBAT_SCRIPT } from './killZoneAcrobat';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kill-Zone Acrobat";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([KILL_ZONE_ACROBAT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kill-Zone Acrobat", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KILL_ZONE_ACROBAT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Kill-Zone Acrobat", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"You may sacrifice another creature or artifact. If you do, this creature gains flying until end of turn.\" - the price is declined", () => {
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"You may sacrifice another creature or artifact. If you do, this creature gains flying until end of turn.\" - the price is paid", () => {
