// `Undead Hand Ninja` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNDEAD_HAND_NINJA_SCRIPT } from './undeadHandNinja';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Undead Hand Ninja";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Undead Hand Ninja", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNDEAD_HAND_NINJA_SCRIPT]),
  });
  holdEverywhere(g);
