// `Enterprising Scallywag` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ENTERPRISING_SCALLYWAG_SCRIPT } from './enterprisingScallywag';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Enterprising Scallywag";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Enterprising Scallywag", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ENTERPRISING_SCALLYWAG_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Enterprising Scallywag", () => {
