// `Shalai's Acolyte` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHALAIS_ACOLYTE_SCRIPT } from './shalaisAcolyte';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shalai's Acolyte";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SHALAIS_ACOLYTE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shalai's Acolyte"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHALAIS_ACOLYTE_SCRIPT]),
  });
  holdEverywhere(g);
