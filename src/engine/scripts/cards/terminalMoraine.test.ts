// `Terminal Moraine` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Terminal Moraine", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TERMINAL_MORAINE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Terminal Moraine", () => {
  test("{2}, {T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic land card, put that card onto the battlefield tapped, then shuffle.\"", () => {
