// `Zephid's Embrace` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2 and has flying, shroud;
...
import { ZEPHIDS_EMBRACE_SCRIPT } from './zephidsEmbrace';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Zephid's Embrace";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ZEPHIDS_EMBRACE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ZEPHIDS_EMBRACE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Zephid's Embrace", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+2 and has flying, shroud; Cyclops of One-Eyed Pass is untouched", () => {
