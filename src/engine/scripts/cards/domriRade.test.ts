// `Domri Rade` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Domri Rade", "Grizzly Bears", "Cyclops of One-Eyed Pass", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DOMRI_RADE_SCRIPT]),
...
describe("Domri Rade", () => {
  test("+1: the vocabulary resolves \"Look at the top card of your library. If it's a creature card, you may reveal it and put it into your hand.\"", () => {
  ...
  test("−2: the vocabulary resolves \"Target creature you control fights another target creature.\"", () => {
    const { g, self, no, board0, refused, vtA1_0 } = armed(1);
  ...
  test("−7: the vocabulary resolves \"You get an emblem with \\\"Creatures you control have double strike, trample, hexproof, and haste.\\\"\"", () => {
    const { g, self, refused } = armed(2);
    expect((g.state.zones.command.p1 ?? []).filter((c) => g.state.cards[c]?.printingId === "1612f546-1bdb-4861-966c-4eeb7d1e65e8").length, 'the emblem is in the command zone').toBe(1);
    expect(refused).toBe(true);
    expect(g.state.cards[self]?.counters['loyalty'] ?? 0).toBe(1);
  });
