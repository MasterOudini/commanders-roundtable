// `Welkin Tern` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WELKIN_TERN_SCRIPT } from './welkinTern';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Welkin Tern";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Welkin Tern", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WELKIN_TERN_SCRIPT]),
  });
  holdEverywhere(g);
