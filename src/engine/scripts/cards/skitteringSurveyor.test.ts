// `Skittering Surveyor` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skittering Surveyor", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKITTERING_SURVEYOR_SCRIPT]),
  });
  holdEverywhere(g);
