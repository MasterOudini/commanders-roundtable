// `Skeletal Grimace` - the carrier proven end to end (D367/D368) with a D373 payload: the recipient
...
import { SKELETAL_GRIMACE_SCRIPT } from './skeletalGrimace';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skeletal Grimace";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Skeletal Grimace","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKELETAL_GRIMACE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skeletal Grimace", () => {
  test("grants {B}: Regenerate this creature. to Grizzly Bears, and to nothing outside the scope", () => {
