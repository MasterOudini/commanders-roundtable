// `Bonecaller Cleric` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BONECALLER_CLERIC_SCRIPT } from './bonecallerCleric';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bonecaller Cleric";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bonecaller Cleric", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BONECALLER_CLERIC_SCRIPT]),
  });
  holdEverywhere(g);
