// `Skeleton Key` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SKELETON_KEY_SCRIPT } from './skeletonKey';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Skeleton Key";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([SKELETON_KEY_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Skeleton Key", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SKELETON_KEY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Skeleton Key", () => {
  test("Equipped creature has skulk. (It can't be blocked by creatures with greater power.): the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(kw(g, bearsB).has("skulk")).toBe(true);
  });

  test("Whenever equipped creature deals combat damage to a player: the vocabulary resolves \"Draw a card. If you do, discard a card.\"", () => {
    const { g } = armed(1);
