// `Mechan Navigator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MECHAN_NAVIGATOR_SCRIPT } from './mechanNavigator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mechan Navigator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mechan Navigator"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MECHAN_NAVIGATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mechan Navigator", () => {
  test("Whenever this creature becomes tapped: a card is drawn, then one discarded", () => {
