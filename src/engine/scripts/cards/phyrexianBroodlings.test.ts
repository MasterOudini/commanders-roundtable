// `Phyrexian Broodlings` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PHYREXIAN_BROODLINGS_SCRIPT } from './phyrexianBroodlings';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phyrexian Broodlings";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phyrexian Broodlings", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHYREXIAN_BROODLINGS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Phyrexian Broodlings", () => {
  test("{1}, Sacrifice a creature: it gets 1 +1/+1 counter", () => {
