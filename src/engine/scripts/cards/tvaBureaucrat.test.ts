// `TVA Bureaucrat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TVA_BUREAUCRAT_SCRIPT } from './tvaBureaucrat';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "TVA Bureaucrat";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TVA_BUREAUCRAT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["TVA Bureaucrat", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TVA_BUREAUCRAT_SCRIPT]),
  });
  holdEverywhere(g);
