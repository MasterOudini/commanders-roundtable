// `Destructor Dragon` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { DESTRUCTOR_DRAGON_SCRIPT } from './destructorDragon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Destructor Dragon";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Destructor Dragon"], ["Forest", "Grizzly Bears"]],
    scripts: createRegistry([DESTRUCTOR_DRAGON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Destructor Dragon", () => {
  test("Forest is a legal target and it is destroyed", () => {
