// `Panic Spellbomb` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PANIC_SPELLBOMB_SCRIPT } from './panicSpellbomb';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Panic Spellbomb";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Panic Spellbomb", "Grizzly Bears", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PANIC_SPELLBOMB_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Panic Spellbomb", () => {
  test("{T}, Sacrifice this artifact: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
  ...
  test("When this artifact is put into a graveyard from the battlefield: the vocabulary resolves \"You may pay {R}. If you do, draw a card.\" - the price is declined", () => {
  ...
  test("When this artifact is put into a graveyard from the battlefield: the vocabulary resolves \"You may pay {R}. If you do, draw a card.\" - the price is paid", () => {
    const { g, payL1_p1_0 } = armed(1);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
