// `Dimir Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DIMIR_GUILDMAGE_SCRIPT } from './dimirGuildmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dimir Guildmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dimir Guildmage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DIMIR_GUILDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dimir Guildmage", () => {
  test("{3}{U}: the vocabulary resolves \"Target player draws a card.\"", () => {
  ...
  test("{3}{B}: the vocabulary resolves \"Target player discards a card.\"", () => {
