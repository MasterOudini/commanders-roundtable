// `Cinderhaze Wretch` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CINDERHAZE_WRETCH_SCRIPT } from './cinderhazeWretch';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cinderhaze Wretch";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; cnt0: number; pt0: [number | null, number | null]; condRefused: boolean };
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CINDERHAZE_WRETCH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cinderhaze Wretch"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CINDERHAZE_WRETCH_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, cnt0, pt0, condRefused };
}

describe("Cinderhaze Wretch", () => {
  test("{T}: the vocabulary resolves \"Target player discards a card.\" - refused unless during your turn", () => {
  ...
  test("Put a -1/-1 counter on this creature: it untaps", () => {
