// `Gorgon's Head` - equipped to Grizzly Bears the creature has deathtouch; the Cyclops is untouched;
...
import { GORGONS_HEAD_SCRIPT } from './gorgonsHead';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gorgon's Head";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([GORGONS_HEAD_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([GORGONS_HEAD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Gorgon's Head", () => {
  test("equipped to Grizzly Bears: attached, and the creature has deathtouch; the Cyclops is untouched", () => {
