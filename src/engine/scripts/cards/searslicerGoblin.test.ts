// `Searslicer Goblin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEARSLICER_GOBLIN_SCRIPT } from './searslicerGoblin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Searslicer Goblin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Searslicer Goblin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEARSLICER_GOBLIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Searslicer Goblin", () => {
  test("Raid — At the beginning of your end step: 1 token made", () => {
