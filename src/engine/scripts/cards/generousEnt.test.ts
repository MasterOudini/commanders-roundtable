// `Generous Ent` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GENEROUS_ENT_SCRIPT } from './generousEnt';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Generous Ent";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Generous Ent"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GENEROUS_ENT_SCRIPT]),
  });
  holdEverywhere(g);
