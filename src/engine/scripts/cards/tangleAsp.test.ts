// `Tangle Asp` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TANGLE_ASP_SCRIPT } from './tangleAsp';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tangle Asp";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tangle Asp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TANGLE_ASP_SCRIPT]),
