// `Spartan Veteran` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPARTAN_VETERAN_SCRIPT } from './spartanVeteran';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spartan Veteran";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPARTAN_VETERAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spartan Veteran"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPARTAN_VETERAN_SCRIPT]),
...
describe("Spartan Veteran", () => {
  test("`Spartan Veteran` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
