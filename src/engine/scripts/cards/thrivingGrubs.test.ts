// `Thriving Grubs` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THRIVING_GRUBS_SCRIPT } from './thrivingGrubs';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thriving Grubs";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thriving Grubs"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THRIVING_GRUBS_SCRIPT]),
