// `Sightless Ghoul` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SIGHTLESS_GHOUL_SCRIPT } from './sightlessGhoul';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sightless Ghoul";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sightless Ghoul", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIGHTLESS_GHOUL_SCRIPT]),
  });
  holdEverywhere(g);
