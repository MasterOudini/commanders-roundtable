// `Godtracker of Jund` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GODTRACKER_OF_JUND_SCRIPT } from './godtrackerOfJund';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Godtracker of Jund";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Godtracker of Jund", "Grizzly Bears", "Kalakscion, Hunger Tyrant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GODTRACKER_OF_JUND_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Godtracker of Jund", () => {
  test("Whenever a creature you control with power 5 or greater enters: it gets 1 +1/+1 counter", () => {
