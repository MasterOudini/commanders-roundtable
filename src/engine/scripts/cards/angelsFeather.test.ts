// `Angel's Feather` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ANGELS_FEATHER_SCRIPT } from './angelsFeather';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Angel's Feather";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Angel's Feather", "Grizzly Bears", "Devoted Hero"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ANGELS_FEATHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Angel's Feather", () => {
  test("Whenever a player casts a white spell: 1 life is gained", () => {
