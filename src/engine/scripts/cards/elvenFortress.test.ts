// `Elven Fortress` — pump on their BLOCKING creature: the right target is
// accepted, a creature that is attacking, not blocking, is refused (D291). Generated from one table row (D292).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ELVEN_FORTRESS_SCRIPT } from './elvenFortress';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Elven Fortress";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([ELVEN_FORTRESS_SCRIPT]),
  });
  ...
  return { g, self, target: theirs, wrong: att };
}

describe("Elven Fortress", () => {
  test('the blocking creature is a legal target and the effect lands', () => {
    const { g, self, target } = armed();
    must(g.submit({ t: 'ChooseTargets', player: 'p1', targets: [{ kind: 'card', id: target }] }));
    settle(g);
    const d = deps(createRegistry([ELVEN_FORTRESS_SCRIPT]));
    const got = derive(g.state, d.oracle, d.scripts, target);
    expect([got.power, got.toughness]).toEqual([2, 3]);
