// `Blade Historian` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLADE_HISTORIAN_SCRIPT } from './bladeHistorian';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blade Historian";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BLADE_HISTORIAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blade Historian", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLADE_HISTORIAN_SCRIPT]),
  });
  holdEverywhere(g);
