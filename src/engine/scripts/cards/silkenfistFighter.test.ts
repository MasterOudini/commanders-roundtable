// `Silkenfist Fighter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SILKENFIST_FIGHTER_SCRIPT } from './silkenfistFighter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Silkenfist Fighter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Silkenfist Fighter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SILKENFIST_FIGHTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Silkenfist Fighter", () => {
  test("Whenever this creature becomes blocked: it untaps", () => {
