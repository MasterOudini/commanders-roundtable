// `Ocular Halo` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OCULAR_HALO_SCRIPT } from './ocularHalo';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([OCULAR_HALO_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ocular Halo", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OCULAR_HALO_SCRIPT]),
...
describe("Ocular Halo", () => {
  ...
  test("{W}: the enchanted creature is pumped until end of turn", () => {
  ...
  test("Enchanted creature has \"{T}: the vocabulary resolves \"Draw a card.\"", () => {
    const { g, hand0, bearsB } = armed(2);
