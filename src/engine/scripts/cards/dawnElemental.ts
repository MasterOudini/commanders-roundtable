// `Dawn Elemental` - a CONTINUOUS prevention effect (CR 615, D385): a `PreventionDef` the
...
import { DAWN_ELEMENTAL } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(DAWN_ELEMENTAL, "Flying\nPrevent all damage that would be dealt to this creature.");
const LINES = PRINTED.split(String.fromCharCode(10));

export const DAWN_ELEMENTAL_SCRIPT: CardScript = {
  oracleId: DAWN_ELEMENTAL.oracleId,
  name: DAWN_ELEMENTAL.name,
  prevention: [
    {
      abilityId: 'prevent-1',
