// `Havoc Jester` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HAVOC_JESTER_SCRIPT } from './havocJester';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Havoc Jester";
...
function sacManaIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([HAVOC_JESTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Havoc Jester", "Lotus Petal"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HAVOC_JESTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Havoc Jester", () => {
  test("Whenever you sacrifice a permanent: it deals 1 damage to the declared creature", () => {
    const { g, no } = armed(0);
