// `Hookblade Veteran` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HOOKBLADE_VETERAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hookblade Veteran"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOOKBLADE_VETERAN_SCRIPT]),
...
describe("Hookblade Veteran", () => {
  test("`Hookblade Veteran` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
