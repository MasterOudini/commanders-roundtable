// `Sleeper's Guile` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLEEPERS_GUILE_SCRIPT } from './sleepersGuile';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sleeper's Guile";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([SLEEPERS_GUILE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sleeper's Guile", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLEEPERS_GUILE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sleeper's Guile", () => {
  test("Enchanted creature has fear. (It can't be blocked except by artifact creatures and/or black creatures.): the creature it is attached to reads it", () => {
