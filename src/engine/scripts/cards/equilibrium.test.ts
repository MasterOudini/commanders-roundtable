// `Equilibrium` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EQUILIBRIUM_SCRIPT } from './equilibrium';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Equilibrium";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Equilibrium", "Grizzly Bears", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EQUILIBRIUM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Equilibrium", () => {
  test("Whenever you cast a creature spell: the vocabulary resolves \"You may pay {1}. If you do, return target creature to its owner's hand.\" - the price is declined", () => {
  ...
  test("Whenever you cast a creature spell: the vocabulary resolves \"You may pay {1}. If you do, return target creature to its owner's hand.\" - the price is paid", () => {
    const { g, no, payL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
