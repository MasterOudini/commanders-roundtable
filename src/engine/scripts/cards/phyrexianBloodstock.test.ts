// `Phyrexian Bloodstock` - the named target is accepted and it is destroyed; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { PHYREXIAN_BLOODSTOCK_SCRIPT } from './phyrexianBloodstock';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Phyrexian Bloodstock";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Phyrexian Bloodstock"], ["Thraben Standard Bearer", "Grizzly Bears"]],
    scripts: createRegistry([PHYREXIAN_BLOODSTOCK_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const target = put(g, 'p2', "Thraben Standard Bearer");
...
describe("Phyrexian Bloodstock", () => {
  test("Thraben Standard Bearer is a legal target and it is destroyed", () => {
