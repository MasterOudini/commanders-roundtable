// `Ghost-Lit Nourisher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHOST_LIT_NOURISHER_SCRIPT } from './ghostLitNourisher';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghost-Lit Nourisher";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GHOST_LIT_NOURISHER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghost-Lit Nourisher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHOST_LIT_NOURISHER_SCRIPT]),
...
describe("Ghost-Lit Nourisher", () => {
  test("{2}{G}, {T}: the declared creature is pumped until end of turn", () => {
    const { g, self, no } = armed(0);
    expect(pt(g, no)).toEqual([7, 4]);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("Channel — {3}{G}, Discard this card: the declared creature is pumped until end of turn", () => {
