// `Magma Phoenix` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAGMA_PHOENIX_SCRIPT } from './magmaPhoenix';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Magma Phoenix";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Magma Phoenix"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAGMA_PHOENIX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Magma Phoenix", () => {
  test("When this creature dies: the vocabulary resolves \"~ deals 3 damage to each creature and each player.\"", () => {
  ...
  test("{3}{R}{R}: it returns from the graveyard to hand", () => {
