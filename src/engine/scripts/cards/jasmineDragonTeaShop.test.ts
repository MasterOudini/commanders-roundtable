// `Jasmine Dragon Tea Shop` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JASMINE_DRAGON_TEA_SHOP_SCRIPT } from './jasmineDragonTeaShop';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jasmine Dragon Tea Shop";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jasmine Dragon Tea Shop"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JASMINE_DRAGON_TEA_SHOP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Jasmine Dragon Tea Shop", () => {
  test("{5}, {T}: 1 token made", () => {
