// `Sandsteppe Outcast` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SANDSTEPPE_OUTCAST_SCRIPT } from './sandsteppeOutcast';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sandsteppe Outcast";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sandsteppe Outcast"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SANDSTEPPE_OUTCAST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sandsteppe Outcast", () => {
  ...
  test("When this creature enters: choosing \"Create a 1/1 white Spirit creature token with flying.\", token", () => {
