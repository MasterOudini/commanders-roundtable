// `Fire Imp` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIRE_IMP_SCRIPT } from './fireImp';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fire Imp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIRE_IMP_SCRIPT]),
  });
  holdEverywhere(g);
