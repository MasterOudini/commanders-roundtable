// `Sunrise Sovereign` - the anthem reaches the permanent its scope names and not the other;
// it ends when the source leaves; replay equal (D300). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { SUNRISE_SOVEREIGN_SCRIPT } from './sunriseSovereign';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sunrise Sovereign";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SUNRISE_SOVEREIGN_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; yes: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Sunrise Sovereign", "Bulwark Giant", "Coral Eel"], ["Grizzly Bears"]],
    scripts: createRegistry([SUNRISE_SOVEREIGN_SCRIPT]),
  });
  holdEverywhere(g);
  const yes = put(g, 'p1', "Bulwark Giant");
...
describe("Sunrise Sovereign", () => {
  test("Bulwark Giant is reached, Coral Eel is not", () => {
    const { g, yes, no } = board();
    expect(pt(g, yes)).toEqual([5, 8]);
