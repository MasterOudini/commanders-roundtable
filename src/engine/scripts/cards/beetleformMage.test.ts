// `Beetleform Mage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BEETLEFORM_MAGE_SCRIPT } from './beetleformMage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Beetleform Mage";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BEETLEFORM_MAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Beetleform Mage", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BEETLEFORM_MAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Beetleform Mage", () => {
  test("{G}{U}: it gets +2/+2 and gains flying until end of turn", () => {
