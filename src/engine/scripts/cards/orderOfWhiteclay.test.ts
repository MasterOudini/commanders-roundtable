// `Order of Whiteclay` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORDER_OF_WHITECLAY_SCRIPT } from './orderOfWhiteclay';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Order of Whiteclay";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Order of Whiteclay", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORDER_OF_WHITECLAY_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Order of Whiteclay", () => {
  test("{1}{W}{W}, {Q}: the vocabulary resolves \"Return target creature card with mana value 3 or less from your graveyard to the battlefield.\"", () => {
