// `Rust Monster` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RUST_MONSTER_SCRIPT } from './rustMonster';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rust Monster";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([RUST_MONSTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rust Monster", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RUST_MONSTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Rust Monster", () => {
  test("Sacrifice an artifact: it gets +2/+0 until end of turn", () => {
