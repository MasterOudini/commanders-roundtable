// `Phalanx Vanguard` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PHALANX_VANGUARD_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Phalanx Vanguard", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PHALANX_VANGUARD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Phalanx Vanguard", () => {
  test("Whenever an artifact you control enters: it gets +1/+0 until end of turn", () => {
