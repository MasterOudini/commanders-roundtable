// `Usher of the Fallen` - every printed ability proven in its own game: the cost's mark, the pump
...
import { USHER_OF_THE_FALLEN_SCRIPT } from './usherOfTheFallen';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Usher of the Fallen";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Usher of the Fallen", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([USHER_OF_THE_FALLEN_SCRIPT]),
...
describe("Usher of the Fallen", () => {
  test("Boast — {1}{W}: 1 token made - refused unless this creature attacked this turn", () => {
    const { g, board0, refused, condRefused } = armed(0);
