// `Snapping Sailback` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SNAPPING_SAILBACK_SCRIPT } from './snappingSailback';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Snapping Sailback";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Snapping Sailback", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([SNAPPING_SAILBACK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Snapping Sailback", () => {
  test("Enrage — Whenever this creature is dealt damage: it gets 1 +1/+1 counter", () => {
