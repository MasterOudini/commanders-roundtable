// `Gurmag Drowner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GURMAG_DROWNER_SCRIPT } from './gurmagDrowner';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gurmag Drowner";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gurmag Drowner"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GURMAG_DROWNER_SCRIPT]),
...
describe("Gurmag Drowner", () => {
  test("When this creature exploits a creature: the vocabulary resolves \"Look at the top four cards of your library. Put one of them into your hand and the rest into your graveyard.\"", () => {
    const { g } = armed(0);
