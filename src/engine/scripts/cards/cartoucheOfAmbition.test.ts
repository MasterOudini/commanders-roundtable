// `Cartouche of Ambition` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CARTOUCHE_OF_AMBITION_SCRIPT } from './cartoucheOfAmbition';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cartouche of Ambition";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CARTOUCHE_OF_AMBITION_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cartouche of Ambition", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CARTOUCHE_OF_AMBITION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cartouche of Ambition", () => {
  test("When this Aura enters: the vocabulary resolves \"Put a -1/-1 counter on target creature.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.counters["-1/-1"] ?? 0).toBe(1);
  });

  test("Enchanted creature gets +1/+1 and has lifelink.: the creature it is attached to reads it", () => {
