// `Virus Beetle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VIRUS_BEETLE_SCRIPT } from './virusBeetle';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Virus Beetle";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Virus Beetle"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VIRUS_BEETLE_SCRIPT]),
  });
  holdEverywhere(g);
