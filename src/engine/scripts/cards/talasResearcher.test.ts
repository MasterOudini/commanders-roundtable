// `Talas Researcher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TALAS_RESEARCHER_SCRIPT } from './talasResearcher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Talas Researcher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Talas Researcher"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TALAS_RESEARCHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Talas Researcher", () => {
  test("{T}: a card is drawn - refused unless during your turn, before attackers are declared", () => {
