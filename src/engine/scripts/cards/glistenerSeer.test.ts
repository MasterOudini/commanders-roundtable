// `Glistener Seer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GLISTENER_SEER_SCRIPT } from './glistenerSeer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Glistener Seer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GLISTENER_SEER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glistener Seer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GLISTENER_SEER_SCRIPT]),
...
describe("Glistener Seer", () => {
  test("This creature enters with three oil counters on it.: cast, it enters with 3 oil counters", () => {
    ...
    expect(pt(g, self)).toEqual([0, 3]);
  });

  test("{T}, Remove an oil counter from this creature: scry 1 asks, and the cards stay on top", () => {
    const { g, self, lib0, cnt0 } = armed(1);
