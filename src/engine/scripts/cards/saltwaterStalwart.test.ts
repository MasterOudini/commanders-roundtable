// `Saltwater Stalwart` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SALTWATER_STALWART_SCRIPT } from './saltwaterStalwart';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Saltwater Stalwart";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Saltwater Stalwart"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SALTWATER_STALWART_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Saltwater Stalwart", () => {
  test("Whenever this creature deals damage to an opponent: the vocabulary resolves \"Target player draws a card.\"", () => {
