// `Weldfast Wingsmith` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WELDFAST_WINGSMITH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Weldfast Wingsmith", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WELDFAST_WINGSMITH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Weldfast Wingsmith", () => {
  test("Whenever an artifact you control enters: it gains flying until end of turn", () => {
