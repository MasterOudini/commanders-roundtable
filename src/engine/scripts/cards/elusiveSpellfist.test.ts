// `Elusive Spellfist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ELUSIVE_SPELLFIST_SCRIPT } from './elusiveSpellfist';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Elusive Spellfist";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ELUSIVE_SPELLFIST_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elusive Spellfist", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELUSIVE_SPELLFIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Elusive Spellfist", () => {
  test("Whenever you cast a noncreature spell: it gets +1/+0 until end of turn and can't be blocked this turn", () => {
