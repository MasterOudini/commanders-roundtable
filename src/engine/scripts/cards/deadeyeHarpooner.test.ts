// `Deadeye Harpooner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DEADEYE_HARPOONER_SCRIPT } from './deadeyeHarpooner';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deadeye Harpooner";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Deadeye Harpooner", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DEADEYE_HARPOONER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Deadeye Harpooner", () => {
  test("Revolt — When this creature enters: the vocabulary resolves \"Destroy target tapped creature an opponent controls.\"", () => {
    const { g, no } = armed(0);
