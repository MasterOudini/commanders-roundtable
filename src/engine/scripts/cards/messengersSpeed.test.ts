// `Messenger's Speed` - cast on Grizzly Bears it attaches and the enchanted creature has trample, haste;
...
import { MESSENGERS_SPEED_SCRIPT } from './messengersSpeed';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Messenger's Speed";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([MESSENGERS_SPEED_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([MESSENGERS_SPEED_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Messenger's Speed", () => {
  test("on Grizzly Bears: attached, and the enchanted creature has trample, haste; Cyclops of One-Eyed Pass is untouched", () => {
