// `Obsessive Stitcher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OBSESSIVE_STITCHER_SCRIPT } from './obsessiveStitcher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Obsessive Stitcher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Obsessive Stitcher", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OBSESSIVE_STITCHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Obsessive Stitcher", () => {
  test("{T}: a card is drawn, then one discarded", () => {
    const { g, self, hand0, gy0 } = armed(0);
  ...
  test("{2}{U}{B}, {T}, Sacrifice this creature: the vocabulary resolves \"Return target creature card from your graveyard to the battlefield.\"", () => {
    const { g, self, board0, vtA1_0 } = armed(1);
