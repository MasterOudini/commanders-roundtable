// `Llanowar Stalker` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LLANOWAR_STALKER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Llanowar Stalker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LLANOWAR_STALKER_SCRIPT]),
  });
  holdEverywhere(g);
