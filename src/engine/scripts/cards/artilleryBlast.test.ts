// `Artillery Blast` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { ARTILLERY_BLAST_SCRIPT } from './artilleryBlast';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Artillery Blast";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Artillery Blast", "Forest", "Island"], ["Grizzly Bears", "Grizzly Bears"]],
    scripts: createRegistry([ARTILLERY_BLAST_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  extras.push(put(g, 'p1', "Forest"));
  extras.push(put(g, 'p1', "Island"));
  settle(g);
  must(g.submit({ t: 'ManualSetTapped', player: 'p2', cards: [targets[0] as InstanceId], tapped: true }));
