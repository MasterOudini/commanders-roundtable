// `Gelectrode` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GELECTRODE_SCRIPT } from './gelectrode';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gelectrode";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Gelectrode", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GELECTRODE_SCRIPT]),
  });
  holdEverywhere(g);
