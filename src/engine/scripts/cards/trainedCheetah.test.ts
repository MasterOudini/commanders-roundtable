// `Trained Cheetah` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRAINED_CHEETAH_SCRIPT } from './trainedCheetah';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trained Cheetah";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TRAINED_CHEETAH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trained Cheetah"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRAINED_CHEETAH_SCRIPT]),
  });
  holdEverywhere(g);
