// `Mischievous Mystic` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MISCHIEVOUS_MYSTIC_SCRIPT } from './mischievousMystic';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mischievous Mystic";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mischievous Mystic", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MISCHIEVOUS_MYSTIC_SCRIPT]),
  });
  holdEverywhere(g);
