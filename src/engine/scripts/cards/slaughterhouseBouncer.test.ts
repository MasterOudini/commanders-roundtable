// `Slaughterhouse Bouncer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLAUGHTERHOUSE_BOUNCER_SCRIPT } from './slaughterhouseBouncer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slaughterhouse Bouncer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slaughterhouse Bouncer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLAUGHTERHOUSE_BOUNCER_SCRIPT]),
...
describe("Slaughterhouse Bouncer", () => {
  test("Hellbent — When this creature dies: the vocabulary resolves \"If you have no cards in hand, target creature gets -3/-3 until end of turn.\"", () => {
    const { g, no } = armed(0);
