...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; forest: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [[CARD, BEARS, BEARS, 'Vampire Nighthawk', 'Forest', 'Thraben Standard Bearer'], [BEARS, BEARS]],
    scripts: createRegistry([ELVEN_PALISADE_SCRIPT]),
  });
  ...
  return { g, self, target: att, wrong: home, forest };
}

describe("Elven Palisade", () => {
  test('the attacking creature is a legal target and the effect lands', () => {
    const { g, target, forest } = armed();
    must(g.submit({ t: 'ChooseTargets', player: 'p1', targets: [{ kind: 'card', id: target }] }));
    settle(g);
    const d = deps(createRegistry([ELVEN_PALISADE_SCRIPT]));
