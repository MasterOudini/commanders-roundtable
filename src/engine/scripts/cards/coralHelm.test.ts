// `Coral Helm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CORAL_HELM_SCRIPT } from './coralHelm';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Coral Helm";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CORAL_HELM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Coral Helm"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CORAL_HELM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Coral Helm", () => {
  test("{3}, Discard a card at random: the declared creature is pumped until end of turn", () => {
