// `Contaminated Landscape` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CONTAMINATED_LANDSCAPE_SCRIPT } from './contaminatedLandscape';
import { advanceUntil, holdEverywhere, must, nameOf, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Contaminated Landscape";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Contaminated Landscape", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CONTAMINATED_LANDSCAPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Contaminated Landscape", () => {
  test("{T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Plains, Island, or Swamp card, put it onto the battlefield tapped, then shuffle.\"", () => {
