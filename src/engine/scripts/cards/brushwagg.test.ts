// `Brushwagg` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRUSHWAGG_SCRIPT } from './brushwagg';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brushwagg";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([BRUSHWAGG_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brushwagg"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRUSHWAGG_SCRIPT]),
  });
  holdEverywhere(g);
