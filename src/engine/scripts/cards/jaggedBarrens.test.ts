// `Jagged Barrens` - every printed ability proven in its own game: the cost's mark, the pump
...
import { JAGGED_BARRENS_SCRIPT } from './jaggedBarrens';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Jagged Barrens";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Jagged Barrens"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([JAGGED_BARRENS_SCRIPT]),
  });
  holdEverywhere(g);
