// `Cloud Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLOUD_ELEMENTAL_SCRIPT } from './cloudElemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cloud Elemental";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cloud Elemental", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLOUD_ELEMENTAL_SCRIPT]),
  });
  holdEverywhere(g);
