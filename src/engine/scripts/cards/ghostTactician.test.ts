// `Ghost Tactician` - the printed cost buys the pump until end of turn; it ends at cleanup;
// replay equal (D301). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { GHOST_TACTICIAN_SCRIPT } from './ghostTactician';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghost Tactician";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GHOST_TACTICIAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghost Tactician", "Coral Eel", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHOST_TACTICIAN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ghost Tactician", () => {
  test("{W}, {T}, Discard a card: its controller's creatures get +1/+0 until end of turn", () => {
