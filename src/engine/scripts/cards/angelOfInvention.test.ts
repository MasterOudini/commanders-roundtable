// `Angel of Invention` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ANGEL_OF_INVENTION_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Angel of Invention", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ANGEL_OF_INVENTION_SCRIPT]),
