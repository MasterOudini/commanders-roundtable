// `Courageous Goblin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COURAGEOUS_GOBLIN_SCRIPT } from './courageousGoblin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Courageous Goblin";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([COURAGEOUS_GOBLIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Courageous Goblin", "Tyrox, Saurid Tyrant"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COURAGEOUS_GOBLIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Courageous Goblin", () => {
  test("Whenever this creature attacks while you control a creature with power 4 or greater: it gets +1/+0 and gains menace until end of turn", () => {
