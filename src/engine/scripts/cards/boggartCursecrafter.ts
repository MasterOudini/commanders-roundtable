// `Boggart Cursecrafter` - a anotherCreatureDies trigger damageOpponents
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { BOGGART_CURSECRAFTER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(BOGGART_CURSECRAFTER, "Deathtouch\nWhenever another Goblin you control dies, this creature deals 1 damage to each opponent.");
const LINES = PRINTED.split('\n');

export const BOGGART_CURSECRAFTER_SCRIPT: CardScript = {
  oracleId: BOGGART_CURSECRAFTER.oracleId,
  name: BOGGART_CURSECRAFTER.name,
  triggers: [
    {
      ...
      matches: (ctx, self, ev) =>
        ev.t === 'CardsMoved' &&
        ev.moves.some(
          (m) => m.card !== self && m.from.kind === 'battlefield' && m.to.kind === 'graveyard' && ctx.state.cards[m.card]?.controller === ctx.query.controllerOf(self) && ctx.derive(m.card).typeLine.subtypes.includes('Goblin'),
        ),
      label: () => "Boggart Cursecrafter - damageOpponents",
