// `Amok` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Amok"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AMOK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Amok", () => {
  test("{1}, Discard a card at random: the declared creature gets 1 +1/+1 counter", () => {
