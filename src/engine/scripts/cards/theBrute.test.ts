// `The Brute` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THE_BRUTE_SCRIPT } from './theBrute';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "The Brute";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([THE_BRUTE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["The Brute", "Grizzly Bears", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THE_BRUTE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("The Brute", () => {
  ...
  test("{R}{R}{R}: the enchanted creature regenerates from the Bolt", () => {
