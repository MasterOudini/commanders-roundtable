// `Navigator's Ruin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NAVIGATORS_RUIN_SCRIPT } from './navigatorsRuin';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Navigator's Ruin";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Navigator's Ruin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NAVIGATORS_RUIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Navigator's Ruin", () => {
  test("Raid — At the beginning of your end step: the vocabulary resolves \"Target opponent mills four cards.\"", () => {
