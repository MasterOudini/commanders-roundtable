// `Frightcrawler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FRIGHTCRAWLER_SCRIPT } from './frightcrawler';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frightcrawler";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FRIGHTCRAWLER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frightcrawler", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FRIGHTCRAWLER_SCRIPT]),
  });
  holdEverywhere(g);
