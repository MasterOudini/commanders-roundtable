// `Siren Song Lyre` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { SIREN_SONG_LYRE_SCRIPT } from './sirenSongLyre';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Siren Song Lyre";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Siren Song Lyre","Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SIREN_SONG_LYRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Siren Song Lyre", () => {
  test("grants {2}, {T}: Tap target creature. to Grizzly Bears, and to nothing outside the scope", () => {
  ...
  test('the recipient is offered it, and activating it charges the RECIPIENT and runs the payload', () => {
    ...
    expect(offer?.t === 'ActivateAbility' && offer.costText).toBe("{2}, {T}");
