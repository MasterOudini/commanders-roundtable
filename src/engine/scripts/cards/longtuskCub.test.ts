// `Longtusk Cub` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LONGTUSK_CUB_SCRIPT } from './longtuskCub';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Longtusk Cub";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Longtusk Cub"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LONGTUSK_CUB_SCRIPT]),
...
describe("Longtusk Cub", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"You get {E}{E}.\"", () => {
    const { g, energy0 } = armed(0);
    expect(g.state.players.p1?.energy).toBe(energy0 + 2);
  });

  test("Pay {E}{E}: it gets 1 +1/+1 counter", () => {
