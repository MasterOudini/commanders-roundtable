// `Flight-Deck Coordinator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FLIGHT_DECK_COORDINATOR_SCRIPT } from './flightDeckCoordinator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Flight-Deck Coordinator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flight-Deck Coordinator", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLIGHT_DECK_COORDINATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Flight-Deck Coordinator", () => {
  test("At the beginning of your end step: 2 life is gained", () => {
