// `Primeval Herald` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Primeval Herald", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRIMEVAL_HERALD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Primeval Herald", () => {
  test("Whenever this creature enters or attacks: the vocabulary resolves \"Search your library for a basic land card, put it onto the battlefield tapped, then shuffle.\"", () => {
