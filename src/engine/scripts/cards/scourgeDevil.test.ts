// `Scourge Devil` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCOURGE_DEVIL_SCRIPT } from './scourgeDevil';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scourge Devil";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SCOURGE_DEVIL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scourge Devil", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCOURGE_DEVIL_SCRIPT]),
...
describe("Scourge Devil", () => {
  test("When this creature enters: its controller's creatures get +1/+0 until end of turn", () => {
