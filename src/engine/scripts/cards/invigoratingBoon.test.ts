// `Invigorating Boon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INVIGORATING_BOON_SCRIPT } from './invigoratingBoon';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Invigorating Boon";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([INVIGORATING_BOON_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Invigorating Boon"], ["Cyclops of One-Eyed Pass", "Lonely Sandbar"]],
    scripts: createRegistry([INVIGORATING_BOON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Invigorating Boon", () => {
  test("Whenever a player cycles a card: the vocabulary resolves \"Put a +1/+1 counter on target creature.\"", () => {
    const { g, no } = armed(0);
