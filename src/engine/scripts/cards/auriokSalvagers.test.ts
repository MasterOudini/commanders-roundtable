// `Auriok Salvagers` - the named target is accepted and it returns to hand; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { AURIOK_SALVAGERS_SCRIPT } from './auriokSalvagers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Auriok Salvagers";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Auriok Salvagers", "Sol Ring", "Lightning Greaves"], ["Grizzly Bears"]],
    scripts: createRegistry([AURIOK_SALVAGERS_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const target = put(g, 'p1', "Sol Ring", 'graveyard');
  const wrong = put(g, 'p1', "Lightning Greaves", 'graveyard');
  settle(g);
...
describe("Auriok Salvagers", () => {
  ...
  test("Lightning Greaves is refused (D294)", () => {
    const { g, wrong } = armed();
