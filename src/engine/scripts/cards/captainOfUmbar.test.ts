// `Captain of Umbar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CAPTAIN_OF_UMBAR_SCRIPT } from './captainOfUmbar';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Captain of Umbar";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Captain of Umbar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CAPTAIN_OF_UMBAR_SCRIPT]),
  });
  holdEverywhere(g);
