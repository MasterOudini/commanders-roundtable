// `Wolfkin Bond` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: 1 token); gets +2/+2;
...
import { WOLFKIN_BOND_SCRIPT } from './wolfkinBond';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Wolfkin Bond";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WOLFKIN_BOND_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([WOLFKIN_BOND_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Wolfkin Bond", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: 1 token); gets +2/+2; Cyclops of One-Eyed Pass is untouched", () => {
