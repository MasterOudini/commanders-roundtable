// `Diabolic Machine` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DIABOLIC_MACHINE_SCRIPT } from './diabolicMachine';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Diabolic Machine";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Diabolic Machine", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DIABOLIC_MACHINE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Diabolic Machine", () => {
  test("{3}: it regenerates from the Bolt", () => {
