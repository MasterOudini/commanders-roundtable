// `Rictus Robber` - a etb trigger token
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { RICTUS_ROBBER } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(RICTUS_ROBBER, "When this creature enters, if a creature died this turn, create a 2/2 blue and black Zombie Rogue creature token.\nPlot {2}{B} (You may pay {2}{B} and exile this card from your hand. Cast it as a sorcery on a later turn without paying its mana cost. Plot only as a sorcery.)");
const LINES = PRINTED.split('\n');
const TOKEN_L0 = tokenRef("Zombie Rogue|2/2|BU|Creature|");
...
export const RICTUS_ROBBER_SCRIPT: CardScript = {
  oracleId: RICTUS_ROBBER.oracleId,
  name: RICTUS_ROBBER.name,
  triggers: [
    {
      abilityId: 'etb-0',
