// `Spidersilk Net` - equipped to Grizzly Bears the creature gets +0/+2 and has reach; the Cyclops is untouched;
...
import { SPIDERSILK_NET_SCRIPT } from './spidersilkNet';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spidersilk Net";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPIDERSILK_NET_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SPIDERSILK_NET_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spidersilk Net", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +0/+2 and has reach; the Cyclops is untouched", () => {
    const { g, self, host, other } = board();
    equip(g, self, host);
    expect(g.state.cards[self]?.attachedTo).toBe(host);
    expect(pt(g, host)).toEqual([2, 4]);
