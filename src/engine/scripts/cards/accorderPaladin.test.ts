// `Accorder Paladin` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ACCORDER_PALADIN_SCRIPT } from './accorderPaladin';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Accorder Paladin";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ACCORDER_PALADIN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Accorder Paladin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ACCORDER_PALADIN_SCRIPT]),
  });
  holdEverywhere(g);
