// `Orcish Hellraiser` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Orcish Hellraiser", "Mountain"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORCISH_HELLRAISER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Orcish Hellraiser", () => {
  test("When this creature dies: it deals 2 damage to the declared player", () => {
