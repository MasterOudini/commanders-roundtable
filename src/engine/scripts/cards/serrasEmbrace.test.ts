// `Serra's Embrace` - cast on Grizzly Bears it attaches and the enchanted creature gets +2/+2 and has flying, vigilance;
...
import { SERRAS_EMBRACE_SCRIPT } from './serrasEmbrace';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Serra's Embrace";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SERRAS_EMBRACE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SERRAS_EMBRACE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Serra's Embrace", () => {
  test("on Grizzly Bears: attached, and the enchanted creature gets +2/+2 and has flying, vigilance; Cyclops of One-Eyed Pass is untouched", () => {
