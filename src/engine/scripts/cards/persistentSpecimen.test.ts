// `Persistent Specimen` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PERSISTENT_SPECIMEN_SCRIPT } from './persistentSpecimen';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Persistent Specimen";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Persistent Specimen"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PERSISTENT_SPECIMEN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Persistent Specimen", () => {
  test("{2}{B}: it returns from the graveyard to the battlefield tapped", () => {
