// `Foe-liage` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Foe-liage", "Grizzly Bears", "Badlands"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FOE_LIAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Foe-liage", () => {
  test("Whenever a land enters during your turn: it gets 1 +1/+1 counter", () => {
