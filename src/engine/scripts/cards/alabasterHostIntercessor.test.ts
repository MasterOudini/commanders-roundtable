// `Alabaster Host Intercessor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ALABASTER_HOST_INTERCESSOR_SCRIPT } from './alabasterHostIntercessor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Alabaster Host Intercessor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Alabaster Host Intercessor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ALABASTER_HOST_INTERCESSOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Alabaster Host Intercessor", () => {
  test("When this creature enters: the vocabulary resolves \"Exile target creature an opponent controls until this creature leaves the battlefield.\"", () => {
