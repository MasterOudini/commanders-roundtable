// `Elite Skirmisher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ELITE_SKIRMISHER_SCRIPT } from './eliteSkirmisher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Elite Skirmisher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elite Skirmisher", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELITE_SKIRMISHER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Elite Skirmisher", () => {
  test("Heroic — Whenever you cast a spell that targets this creature: the vocabulary resolves \"Tap target creature.\"", () => {
    const { g, no } = armed(0);
