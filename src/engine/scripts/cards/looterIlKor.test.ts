// `Looter il-Kor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOOTER_IL_KOR_SCRIPT } from './looterIlKor';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Looter il-Kor";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Looter il-Kor"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOOTER_IL_KOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Looter il-Kor", () => {
  test("Whenever this creature deals damage to an opponent: a card is drawn, then one discarded", () => {
