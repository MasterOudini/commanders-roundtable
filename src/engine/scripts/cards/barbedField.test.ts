// `Barbed Field` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { BARBED_FIELD_SCRIPT } from './barbedField';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Barbed Field";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Barbed Field","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BARBED_FIELD_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Barbed Field", () => {
  test("grants {T}: This land deals 1 damage to any target. to Forest, and to nothing outside the scope", () => {
