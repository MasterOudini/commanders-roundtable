// `Nightshade Stinger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIGHTSHADE_STINGER_SCRIPT } from './nightshadeStinger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Nightshade Stinger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Nightshade Stinger", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIGHTSHADE_STINGER_SCRIPT]),
  });
  holdEverywhere(g);
