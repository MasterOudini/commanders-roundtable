// `Clockwork Percussionist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLOCKWORK_PERCUSSIONIST_SCRIPT } from './clockworkPercussionist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Clockwork Percussionist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Clockwork Percussionist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLOCKWORK_PERCUSSIONIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Clockwork Percussionist", () => {
  test("When this creature dies: the vocabulary resolves \"Exile the top card of your library. You may play it until the end of your next turn.\"", () => {
    const { g } = armed(0);
