// `Hulking Ogre` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HULKING_OGRE_SCRIPT } from './hulkingOgre';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hulking Ogre";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hulking Ogre", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HULKING_OGRE_SCRIPT]),
  });
  holdEverywhere(g);
