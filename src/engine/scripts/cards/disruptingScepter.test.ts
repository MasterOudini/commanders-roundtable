// `Disrupting Scepter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DISRUPTING_SCEPTER_SCRIPT } from './disruptingScepter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Disrupting Scepter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Disrupting Scepter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DISRUPTING_SCEPTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Disrupting Scepter", () => {
  test("{3}, {T}: the vocabulary resolves \"Target player discards a card.\" - refused unless during your turn", () => {
