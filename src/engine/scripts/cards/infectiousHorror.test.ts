// `Infectious Horror` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INFECTIOUS_HORROR_SCRIPT } from './infectiousHorror';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Infectious Horror";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Infectious Horror"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INFECTIOUS_HORROR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Infectious Horror", () => {
  test("Whenever this creature attacks: each opponent loses 2 life", () => {
