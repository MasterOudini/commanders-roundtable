// `Harrier Strix` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HARRIER_STRIX_SCRIPT } from './harrierStrix';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Harrier Strix";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Harrier Strix"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HARRIER_STRIX_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Harrier Strix", () => {
  test("When this creature enters: the vocabulary resolves \"Tap target permanent.\"", () => {
    const { g, no } = armed(0);
    expect(g.state.cards[no]?.tapped).toBe(true);
  });

  test("{2}{U}: a card is drawn, then one discarded", () => {
