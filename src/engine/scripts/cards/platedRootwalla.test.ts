// `Plated Rootwalla` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PLATED_ROOTWALLA_SCRIPT } from './platedRootwalla';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Plated Rootwalla";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([PLATED_ROOTWALLA_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Plated Rootwalla", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PLATED_ROOTWALLA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Plated Rootwalla", () => {
  test("{2}{G}: it gets +3/+3 until end of turn", () => {
