// `Moon-Blessed Cleric` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Moon-Blessed Cleric", "Valgavoth's Lair"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOON_BLESSED_CLERIC_SCRIPT]),
...
describe("Moon-Blessed Cleric", () => {
  test("Divine Intervention — When this creature enters: the vocabulary resolves \"Search your library for an enchantment card, reveal it, then shuffle and put that card on top.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Valgavoth's Lair" && g.state.cards[id as InstanceId]?.zone.kind === 'hand');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Valgavoth's Lair");
      expect(found, "Valgavoth's Lair is not in the library for the search to find").toBeDefined();
