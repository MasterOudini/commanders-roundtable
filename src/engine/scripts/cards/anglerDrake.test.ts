// `Angler Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ANGLER_DRAKE_SCRIPT } from './anglerDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Angler Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Angler Drake"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ANGLER_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
