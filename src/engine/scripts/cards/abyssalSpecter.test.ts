// `Abyssal Specter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ABYSSAL_SPECTER_SCRIPT } from './abyssalSpecter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Abyssal Specter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Abyssal Specter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ABYSSAL_SPECTER_SCRIPT]),
  });
  holdEverywhere(g);
