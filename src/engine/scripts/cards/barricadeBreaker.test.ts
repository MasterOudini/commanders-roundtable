// `Barricade Breaker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BARRICADE_BREAKER_SCRIPT } from './barricadeBreaker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Barricade Breaker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Barricade Breaker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BARRICADE_BREAKER_SCRIPT]),
  });
  holdEverywhere(g);
