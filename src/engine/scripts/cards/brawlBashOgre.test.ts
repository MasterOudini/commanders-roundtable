// `Brawl-Bash Ogre` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BRAWL_BASH_OGRE_SCRIPT } from './brawlBashOgre';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Brawl-Bash Ogre";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Brawl-Bash Ogre", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRAWL_BASH_OGRE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Brawl-Bash Ogre", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"You may sacrifice another creature. If you do, this creature gets +2/+2 until end of turn.\" - the price is declined", () => {
  ...
  test("Whenever this creature attacks: the vocabulary resolves \"You may sacrifice another creature. If you do, this creature gets +2/+2 until end of turn.\" - the price is paid", () => {
