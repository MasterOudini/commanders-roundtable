// `Lunatic Pandora` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LUNATIC_PANDORA_SCRIPT } from './lunaticPandora';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lunatic Pandora";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lunatic Pandora"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LUNATIC_PANDORA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lunatic Pandora", () => {
  ...
  test("{6}, {T}, Sacrifice Lunatic Pandora: the vocabulary resolves \"Destroy target nonland permanent.\"", () => {
