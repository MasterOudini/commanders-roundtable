// `Sickleslicer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SICKLESLICER_SCRIPT } from './sickleslicer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sickleslicer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SICKLESLICER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sickleslicer", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SICKLESLICER_SCRIPT]),
  });
  holdEverywhere(g);
