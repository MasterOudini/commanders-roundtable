// `Siren Song Lyre` - the permanent it is attached to HAS a quoted
...
import { SIREN_SONG_LYRE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(SIREN_SONG_LYRE, "Equipped creature has \"{2}, {T}: Tap target creature.\"\nEquip {2}");
const LINES = PRINTED.split('\n');

const GRANT = grantedActivated("{2}, {T}: Tap target creature.", `${SIREN_SONG_LYRE.oracleId}#g0`, SIREN_SONG_LYRE.name);
const VOCAB = vocabularyEffects("Tap target creature.", SIREN_SONG_LYRE.name);
const VOCAB_T = vocabularyTargets("Tap target creature.");

export const SIREN_SONG_LYRE_SCRIPT: CardScript = {
  oracleId: SIREN_SONG_LYRE.oracleId,
  name: SIREN_SONG_LYRE.name,
  statics: [
    {
      abilityId: 'grant-0',
