// `Raven's Run Dragoon` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAVENS_RUN_DRAGOON_SCRIPT } from './ravensRunDragoon';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Raven's Run Dragoon";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Raven's Run Dragoon", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Muck Rats"]],
    scripts: createRegistry([RAVENS_RUN_DRAGOON_SCRIPT]),
    options: { maxHandSize: null },
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const bearsB = put(g, 'p1', "Grizzly Bears");
  const gift2 = put(g, 'p2', "Muck Rats");
  settle(g);
...
describe("Raven's Run Dragoon", () => {
  test("This creature can't be blocked by black creatures.: the block by black creatures is refused", () => {
