// `Foul Presence` - the permanent it is attached to HAS a quoted
...
import { FOUL_PRESENCE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(FOUL_PRESENCE, "Enchant creature\nEnchanted creature gets -1/-1 and has \"{T}: Target creature gets -1/-1 until end of turn.\"");
const LINES = PRINTED.split('\n');

const GRANT = grantedActivated("{T}: Target creature gets -1/-1 until end of turn.", `${FOUL_PRESENCE.oracleId}#g1`, FOUL_PRESENCE.name);
const VOCAB = vocabularyEffects("Target creature gets -1/-1 until end of turn.", FOUL_PRESENCE.name);
const VOCAB_T = vocabularyTargets("Target creature gets -1/-1 until end of turn.");

export const FOUL_PRESENCE_SCRIPT: CardScript = {
  oracleId: FOUL_PRESENCE.oracleId,
  name: FOUL_PRESENCE.name,
  statics: [
    {
