// `Crossway Vampire` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CROSSWAY_VAMPIRE_SCRIPT } from './crosswayVampire';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crossway Vampire";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crossway Vampire"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CROSSWAY_VAMPIRE_SCRIPT]),
  });
  holdEverywhere(g);
