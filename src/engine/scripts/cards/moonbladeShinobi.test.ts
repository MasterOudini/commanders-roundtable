// `Moonblade Shinobi` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOONBLADE_SHINOBI_SCRIPT } from './moonbladeShinobi';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Moonblade Shinobi";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Moonblade Shinobi"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOONBLADE_SHINOBI_SCRIPT]),
...
describe("Moonblade Shinobi", () => {
  test("Whenever this creature deals combat damage to a player: 1 token made", () => {
