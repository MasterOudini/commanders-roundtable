// `Latch Seeker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LATCH_SEEKER_SCRIPT } from './latchSeeker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Latch Seeker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Latch Seeker", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LATCH_SEEKER_SCRIPT]),
  });
  holdEverywhere(g);
