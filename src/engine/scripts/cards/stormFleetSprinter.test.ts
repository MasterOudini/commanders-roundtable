// `Storm Fleet Sprinter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STORM_FLEET_SPRINTER_SCRIPT } from './stormFleetSprinter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Storm Fleet Sprinter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Storm Fleet Sprinter", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STORM_FLEET_SPRINTER_SCRIPT]),
  });
  holdEverywhere(g);
