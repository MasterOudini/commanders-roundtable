// `Undead Leotau` - every printed ability proven in its own game: the cost's mark, the pump
...
import { UNDEAD_LEOTAU_SCRIPT } from './undeadLeotau';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Undead Leotau";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([UNDEAD_LEOTAU_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Undead Leotau"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([UNDEAD_LEOTAU_SCRIPT]),
