// `Effortless Master` - a static entersWithCounters
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { EFFORTLESS_MASTER } from '../../../data/fixtures/engineCards';
...
export const EFFORTLESS_MASTER_SCRIPT: CardScript = {
  oracleId: EFFORTLESS_MASTER.oracleId,
  name: EFFORTLESS_MASTER.name,
  replacements: [
    {
      ...
      applies: (ctx, self, ev) =>
        ifCond2Of(ctx, self) && ev.t === 'CardsMoved' && ev.moves.some((m) => m.card === self && m.to.kind === 'battlefield' && m.from.kind !== 'battlefield'),
