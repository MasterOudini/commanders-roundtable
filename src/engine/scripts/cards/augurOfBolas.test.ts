// `Augur of Bolas` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Augur of Bolas", "Ancestral Vision", "Ancestral Vision"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AUGUR_OF_BOLAS_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 0) {
    const lookPicksL0_etb = (Object.keys(g.state.cards) as InstanceId[]).filter((id) => nameOf(g, id) === "Ancestral Vision" && (g.state.cards[id]?.zone.kind === 'library' || g.state.cards[id]?.zone.kind === 'hand'));
    expect(lookPicksL0_etb.length, "Ancestral Vision is dealt for the look").toBeGreaterThan(0);
...
describe("Augur of Bolas", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top three cards of your library. You may reveal an instant or sorcery card from among them and put it into your hand. Put the rest on the bottom of your library in any order.\"", () => {
    const { g } = armed(0);
    ...
    { const shown = (g.state.zones.library.p1 ?? []).filter((id) => g.state.cards[id]?.revealedTo.includes('p1'));
      const found = shown.find((id) => nameOf(g, id) === "Ancestral Vision");
      expect(found, "Ancestral Vision is not among the revealed cards").toBeDefined();
