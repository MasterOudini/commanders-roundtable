// `Regal Bloodlord` - every printed ability proven in its own game: the cost's mark, the pump
...
import { REGAL_BLOODLORD_SCRIPT } from './regalBloodlord';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Regal Bloodlord";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Regal Bloodlord"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([REGAL_BLOODLORD_SCRIPT]),
  });
  holdEverywhere(g);
