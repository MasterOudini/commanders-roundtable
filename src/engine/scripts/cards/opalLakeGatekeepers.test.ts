// `Opal Lake Gatekeepers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OPAL_LAKE_GATEKEEPERS_SCRIPT } from './opalLakeGatekeepers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Opal Lake Gatekeepers";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Opal Lake Gatekeepers", "Azorius Guildgate", "Azorius Guildgate"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OPAL_LAKE_GATEKEEPERS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Opal Lake Gatekeepers", () => {
