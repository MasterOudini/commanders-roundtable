// `Voidwielder` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VOIDWIELDER_SCRIPT } from './voidwielder';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Voidwielder";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Voidwielder"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VOIDWIELDER_SCRIPT]),
  });
  holdEverywhere(g);
