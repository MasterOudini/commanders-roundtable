// `Glóin, Dwarf Emissary` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GL_IN_DWARF_EMISSARY_SCRIPT } from './glInDwarfEmissary';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';
import { TOKEN_TABLE } from '../../../data/tokenTable';

const CARD = "Glóin, Dwarf Emissary";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Glóin, Dwarf Emissary", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GL_IN_DWARF_EMISSARY_SCRIPT]),
...
describe("Glóin, Dwarf Emissary", () => {
  test("Whenever you cast a historic spell: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("{T}, Sacrifice a Treasure: the vocabulary resolves \"Goad target creature.\"", () => {
  ...
  test('the once-per-turn rider rides the def (D492)', () => {
    expect(GL_IN_DWARF_EMISSARY_SCRIPT.triggers?.find((t) => t.abilityId.startsWith("castSpell-0") && t.oncePerTurn === true), 'the def of line 0 carries oncePerTurn').toBeDefined();
  });
