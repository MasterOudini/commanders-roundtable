// `Hostile Realm` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOSTILE_REALM_SCRIPT } from './hostileRealm';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { legalActions } from '../../legal';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hostile Realm";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hostile Realm", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOSTILE_REALM_SCRIPT]),
...
describe("Hostile Realm", () => {
  ...
  test("Enchanted land has \"{T}: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
