// `Argentum Armor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ARGENTUM_ARMOR_SCRIPT } from './argentumArmor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Argentum Armor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ARGENTUM_ARMOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Argentum Armor", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARGENTUM_ARMOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Argentum Armor", () => {
  test("Equipped creature gets +6/+6.: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(0);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
    expect(pt(g, bearsB)).toEqual([8, 8]);
  });

  test("Whenever equipped creature attacks: the vocabulary resolves \"Destroy target permanent.\"", () => {
    const { g, no } = armed(1);
