// `Orthion, Hero of Lavabrink` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORTHION_HERO_OF_LAVABRINK_SCRIPT } from './orthionHeroOfLavabrink';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Orthion, Hero of Lavabrink";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; p2hand0: number; gy0: number; p2gy0: number; lib0: number; tok0: readonly InstanceId[]; vtA0_0: InstanceId; vtA1_0: InstanceId };
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([ORTHION_HERO_OF_LAVABRINK_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Orthion, Hero of Lavabrink", "Cyclops of One-Eyed Pass", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORTHION_HERO_OF_LAVABRINK_SCRIPT]),
  ...
  return { g, self, no, life0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0, tok0, vtA0_0, vtA1_0 };
}

describe("Orthion, Hero of Lavabrink", () => {
  test("{1}{R}, {T}: the vocabulary resolves \"Create a token that's a copy of another target creature you control. It gains haste. Sacrifice it at the beginning of the next end step.\"", () => {
    const { g, self, board0, tok0 } = armed(0);
  ...
  test("{6}{R}{R}{R}, {T}: the vocabulary resolves \"Create five tokens that are copies of another target creature you control. They gain haste. Sacrifice them at the beginning of the next end step.\"", () => {
    const { g, self, board0, tok0 } = armed(1);
    const made = tokensMade(g, tok0);
    expect(made, 'the tokens the fire made').toHaveLength(5);
    for (const id of made) expect(kw(g, id).has("haste"), "gained haste").toBe(true);
    for (const id of made) expect(g.state.cards[id]?.gained, 'without a printed duration the grant rides the object').toContain("haste");
    expect(onBoard(g)).toBe(board0 + 5);
    expect(g.state.delayedTriggers.length, 'the payload is armed, not run').toBe(1);
    { const boardD = onBoard(g);
      advanceUntil(g, (s) => s.turn.turnNumber === 3 && s.turn.step === "end", 40_000);
      settle(g);
      expect(onBoard(g), "the board after the step").toBe(boardD + -5);
      for (const id of made) expect(g.state.cards[id]?.zone.kind, "sacrificeObj at the step").not.toBe('battlefield');
    }
    expect(g.state.delayedTriggers.length, 'every entry fired').toBe(0);
    expect(g.state.cards[self]?.tapped).toBe(true);
