// `Perilous Voyage` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { PERILOUS_VOYAGE_SCRIPT } from './perilousVoyage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Perilous Voyage";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Perilous Voyage", "Grizzly Bears"], ["Grizzly Bears"]],
    scripts: createRegistry([PERILOUS_VOYAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Perilous Voyage", () => {
  test('the named target is accepted and the effect lands', () => {
    ...
    must(g.submit({ t: 'AnswerScry', player: 'p1', toTop: lib.slice(lib.length - 2).reverse(), toBottom: [] }));
    settle(g);
