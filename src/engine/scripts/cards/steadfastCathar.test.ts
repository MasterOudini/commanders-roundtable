// `Steadfast Cathar` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STEADFAST_CATHAR_SCRIPT } from './steadfastCathar';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Steadfast Cathar";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([STEADFAST_CATHAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Steadfast Cathar"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([STEADFAST_CATHAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Steadfast Cathar", () => {
  test("Whenever this creature attacks: it gets +0/+2 until end of turn", () => {
