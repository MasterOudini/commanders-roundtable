// `Deathgrip` - counters a held spell of the named colour and refuses one of another:
// the adjective is the parser's and the validator's (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { DEATHGRIP_SCRIPT } from './deathgrip';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Deathgrip";
...
function armed(which: 'right' | 'wrong'): { g: Game; self: InstanceId; spell: InstanceId; stackId: string; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Deathgrip"], ["Grizzly Bears", "Raging Goblin"]],
    scripts: createRegistry([DEATHGRIP_SCRIPT]),
  });
  holdEverywhere(g);
  const self = put(g, 'p1', CARD);
  const spell = put(g, 'p2', which === 'right' ? "Grizzly Bears" : "Raging Goblin", 'hand');
  settle(g);
...
describe("Deathgrip", () => {
  test("the held Grizzly Bears is a legal target and is countered", () => {
  ...
  test("a held Raging Goblin is refused (D294)", () => {
