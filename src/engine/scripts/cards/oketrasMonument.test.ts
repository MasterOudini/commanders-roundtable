// `Oketra's Monument` - every printed ability proven in its own game: the cost's mark, the pump
...
import { OKETRAS_MONUMENT_SCRIPT } from './oketrasMonument';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Oketra's Monument";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Oketra's Monument", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([OKETRAS_MONUMENT_SCRIPT]),
  });
  holdEverywhere(g);
