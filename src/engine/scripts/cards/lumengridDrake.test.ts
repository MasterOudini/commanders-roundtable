// `Lumengrid Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LUMENGRID_DRAKE_SCRIPT } from './lumengridDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lumengrid Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lumengrid Drake", "Sol Ring", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LUMENGRID_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lumengrid Drake", () => {
  test("Metalcraft — When this creature enters: the vocabulary resolves \"Return target creature to its owner's hand.\"", () => {
    const { g, no } = armed(0);
