// `Thraben Inspector` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THRABEN_INSPECTOR_SCRIPT } from './thrabenInspector';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thraben Inspector";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thraben Inspector"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THRABEN_INSPECTOR_SCRIPT]),
  });
  holdEverywhere(g);
