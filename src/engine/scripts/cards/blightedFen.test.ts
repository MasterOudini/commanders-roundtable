// `Blighted Fen` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLIGHTED_FEN_SCRIPT } from './blightedFen';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blighted Fen";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blighted Fen"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLIGHTED_FEN_SCRIPT]),
...
describe("Blighted Fen", () => {
  test("{4}{B}, {T}, Sacrifice this land: the vocabulary resolves \"Target opponent sacrifices a creature of their choice.\"", () => {
