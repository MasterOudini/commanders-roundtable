// `Child of the Volcano` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CHILD_OF_THE_VOLCANO_SCRIPT } from './childOfTheVolcano';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Child of the Volcano";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Child of the Volcano", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CHILD_OF_THE_VOLCANO_SCRIPT]),
  });
  holdEverywhere(g);
