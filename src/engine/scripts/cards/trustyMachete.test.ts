// `Trusty Machete` - equipped to Grizzly Bears the creature gets +2/+1; the Cyclops is untouched;
...
import { TRUSTY_MACHETE_SCRIPT } from './trustyMachete';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trusty Machete";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TRUSTY_MACHETE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([TRUSTY_MACHETE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Trusty Machete", () => {
  test("equipped to Grizzly Bears: attached, and the creature gets +2/+1; the Cyclops is untouched", () => {
