// `Criminal Enterprise` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRIMINAL_ENTERPRISE_SCRIPT } from './criminalEnterprise';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Criminal Enterprise";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Criminal Enterprise", "Grizzly Bears", "Black Tom Cassidy"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRIMINAL_ENTERPRISE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 1) {
    const bears = put(g, 'p1', "Black Tom Cassidy");
...
describe("Criminal Enterprise", () => {
  test("When this enchantment enters: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("Whenever a Villain you control dies: the vocabulary resolves \"This enchantment deals 1 damage to each opponent and you gain 1 life.\"", () => {
