// `Thriving Rhino` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THRIVING_RHINO_SCRIPT } from './thrivingRhino';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thriving Rhino";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thriving Rhino"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THRIVING_RHINO_SCRIPT]),
