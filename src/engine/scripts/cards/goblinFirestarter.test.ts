// `Goblin Firestarter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_FIRESTARTER_SCRIPT } from './goblinFirestarter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Firestarter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Firestarter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_FIRESTARTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goblin Firestarter", () => {
  test("Sacrifice this creature: it deals 1 damage to the declared creature - refused unless during your turn, before attackers are declared", () => {
