// `Hulking Cyclops` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HULKING_CYCLOPS_SCRIPT } from './hulkingCyclops';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hulking Cyclops";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hulking Cyclops", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HULKING_CYCLOPS_SCRIPT]),
  });
  holdEverywhere(g);
