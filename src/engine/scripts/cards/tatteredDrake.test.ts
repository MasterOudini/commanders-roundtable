// `Tattered Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TATTERED_DRAKE_SCRIPT } from './tatteredDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tattered Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tattered Drake", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TATTERED_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
