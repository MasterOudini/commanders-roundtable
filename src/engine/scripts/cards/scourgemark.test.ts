// `Scourgemark` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: draw 1); gets +1/+0;
...
import { SCOURGEMARK_SCRIPT } from './scourgemark';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scourgemark";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SCOURGEMARK_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([SCOURGEMARK_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Scourgemark", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: draw 1); gets +1/+0; Cyclops of One-Eyed Pass is untouched", () => {
