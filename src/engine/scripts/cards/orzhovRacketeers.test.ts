// `Orzhov Racketeers` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ORZHOV_RACKETEERS_SCRIPT } from './orzhovRacketeers';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Orzhov Racketeers";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Orzhov Racketeers"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ORZHOV_RACKETEERS_SCRIPT]),
  });
  holdEverywhere(g);
