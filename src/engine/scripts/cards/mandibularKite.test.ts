// `Mandibular Kite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MANDIBULAR_KITE_SCRIPT } from './mandibularKite';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mandibular Kite";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MANDIBULAR_KITE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mandibular Kite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MANDIBULAR_KITE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mandibular Kite", () => {
  ...
  test("Equipped creature gets +1/+1 and has flying.: the creature it is attached to reads it", () => {
