// `Niall Silvain` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIALL_SILVAIN_SCRIPT } from './niallSilvain';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Niall Silvain";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Niall Silvain", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIALL_SILVAIN_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Niall Silvain", () => {
  test("{G}{G}{G}{G}, {T}: the declared creature regenerates from the Bolt", () => {
