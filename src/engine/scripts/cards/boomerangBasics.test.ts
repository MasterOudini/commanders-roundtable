// `Boomerang Basics` - the named targets are accepted and the riders land; a permanent the clause
// excludes is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { BOOMERANG_BASICS_SCRIPT } from './boomerangBasics';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Boomerang Basics";
...
function armed(): { g: Game; targets: InstanceId[]; wrong: InstanceId; extras: InstanceId[]; handBefore: Record<'p1' | 'p2', number>; life0: Record<'p1' | 'p2', number> } {
  const g = startedGame({
    players: 2,
    decks: [["Boomerang Basics", "Grizzly Bears"], ["Forest"]],
    scripts: createRegistry([BOOMERANG_BASICS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Boomerang Basics", () => {
  test('the named target is accepted and the effect lands', () => {
    ...
    expect((g.state.zones.hand.p1 ?? []).length - handBefore.p1).toBe(2);
  });
