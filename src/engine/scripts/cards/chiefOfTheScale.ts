// `Chief of the Scale` - a layer-6 anthem: "Other Warrior creatures you control get +0/+1". A StaticDef in the shape of the
...
import { CHIEF_OF_THE_SCALE } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(CHIEF_OF_THE_SCALE, "Other Warrior creatures you control get +0/+1.");
const TEXT = PRINTED;

export const CHIEF_OF_THE_SCALE_SCRIPT: CardScript = {
  oracleId: CHIEF_OF_THE_SCALE.oracleId,
  name: CHIEF_OF_THE_SCALE.name,
  statics: [
    {
      abilityId: 'anthem',
      text: TEXT,
      ...
      appliesTo: (ctx, self, candidate, chars) => {
        ...
        if (!chars.typeLine.subtypes.includes("Warrior")) return false;
        return true;
      },
      modify: (chars) => {
