// `Exit Specialist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EXIT_SPECIALIST_SCRIPT } from './exitSpecialist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Exit Specialist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Exit Specialist", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EXIT_SPECIALIST_SCRIPT]),
...
describe("Exit Specialist", () => {
  test("This creature can't be blocked by creatures with power 3 or greater.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("When this creature is turned face up: the vocabulary resolves \"Return another target creature to its owner's hand.\"", () => {
    const { g, no } = armed(1);
