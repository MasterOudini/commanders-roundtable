// `Dawn of Hope` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAWN_OF_HOPE_SCRIPT } from './dawnOfHope';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dawn of Hope";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dawn of Hope", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAWN_OF_HOPE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dawn of Hope", () => {
  test("Whenever you gain life: the vocabulary resolves \"You may pay {2}. If you do, draw a card.\" - the price is declined", () => {
  ...
  test("Whenever you gain life: the vocabulary resolves \"You may pay {2}. If you do, draw a card.\" - the price is paid", () => {
    const { g, payL0_p1_0, payL0_p1_1 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
