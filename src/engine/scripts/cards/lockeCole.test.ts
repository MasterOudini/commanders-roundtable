// `Locke Cole` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOCKE_COLE_SCRIPT } from './lockeCole';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Locke Cole";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Locke Cole"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOCKE_COLE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Locke Cole", () => {
  test("Whenever Locke Cole deals combat damage to a player: a card is drawn, then one discarded", () => {
