// `Crackling Club` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRACKLING_CLUB_SCRIPT } from './cracklingClub';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Crackling Club";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CRACKLING_CLUB_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Crackling Club", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRACKLING_CLUB_SCRIPT]),
  });
  holdEverywhere(g);
