// `Ichorclaw Myr` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ICHORCLAW_MYR_SCRIPT } from './ichorclawMyr';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ichorclaw Myr";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([ICHORCLAW_MYR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ichorclaw Myr"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ICHORCLAW_MYR_SCRIPT]),
  });
  holdEverywhere(g);
