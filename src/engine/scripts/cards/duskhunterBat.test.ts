// `Duskhunter Bat` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DUSKHUNTER_BAT_SCRIPT } from './duskhunterBat';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Duskhunter Bat";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DUSKHUNTER_BAT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Duskhunter Bat", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DUSKHUNTER_BAT_SCRIPT]),
  });
  holdEverywhere(g);
