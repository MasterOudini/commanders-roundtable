// `Sultai Monument` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sultai Monument", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SULTAI_MONUMENT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Sultai Monument", () => {
  test("When this artifact enters: the vocabulary resolves \"Search your library for a basic Swamp, Forest, or Island card, reveal it, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
  ...
  test("{2}{B}{G}{U}, {T}, Sacrifice this artifact: 2 tokens made", () => {
