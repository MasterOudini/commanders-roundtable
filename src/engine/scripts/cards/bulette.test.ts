// `Bulette` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BULETTE_SCRIPT } from './bulette';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bulette", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BULETTE_SCRIPT]),
  });
  holdEverywhere(g);
