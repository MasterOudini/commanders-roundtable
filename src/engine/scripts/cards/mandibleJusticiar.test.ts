// `Mandible Justiciar` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MANDIBLE_JUSTICIAR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mandible Justiciar", "Grizzly Bears", "Memnite"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MANDIBLE_JUSTICIAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mandible Justiciar", () => {
  test("Whenever another artifact you control enters: it gets +1/+1 until end of turn", () => {
