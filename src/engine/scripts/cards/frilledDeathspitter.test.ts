// `Frilled Deathspitter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FRILLED_DEATHSPITTER_SCRIPT } from './frilledDeathspitter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frilled Deathspitter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Frilled Deathspitter", "Lightning Bolt"], ["Cyclops of One-Eyed Pass", "Raging Goblin"]],
    scripts: createRegistry([FRILLED_DEATHSPITTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Frilled Deathspitter", () => {
  test("Enrage — Whenever this creature is dealt damage: the vocabulary resolves \"~ deals 2 damage to target opponent or planeswalker.\" [isDealtCombatDamage]", () => {
    const { g, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 + -2);
  });

  test("Enrage — Whenever this creature is dealt damage: the vocabulary resolves \"~ deals 2 damage to target opponent or planeswalker.\" [isDealtNoncombatDamage]", () => {
