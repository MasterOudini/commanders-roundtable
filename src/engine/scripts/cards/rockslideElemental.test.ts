// `Rockslide Elemental` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ROCKSLIDE_ELEMENTAL_SCRIPT } from './rockslideElemental';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rockslide Elemental";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rockslide Elemental"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ROCKSLIDE_ELEMENTAL_SCRIPT]),
  });
  holdEverywhere(g);
