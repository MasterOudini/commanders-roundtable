// `Tin Street Market` - the carrier proven end to end (D367): the recipient HAS the quoted
...
import { TIN_STREET_MARKET_SCRIPT } from './tinStreetMarket';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tin Street Market";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; no: InstanceId } {
  const g = startedGame({
    players: 2,
    decks: [["Tin Street Market","Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TIN_STREET_MARKET_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Tin Street Market", () => {
  test("grants {T}, Discard a card: Draw a card. to Forest, and to nothing outside the scope", () => {
