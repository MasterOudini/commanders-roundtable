// `Saprazzan Heir` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SAPRAZZAN_HEIR_SCRIPT } from './saprazzanHeir';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Saprazzan Heir";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Saprazzan Heir"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SAPRAZZAN_HEIR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Saprazzan Heir", () => {
  test("Whenever this creature becomes blocked: 3 cards are drawn", () => {
