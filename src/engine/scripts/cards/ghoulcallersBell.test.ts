// `Ghoulcaller's Bell` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHOULCALLERS_BELL_SCRIPT } from './ghoulcallersBell';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghoulcaller's Bell";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghoulcaller's Bell"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHOULCALLERS_BELL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ghoulcaller's Bell", () => {
  test("{T}: the vocabulary resolves \"Each player mills a card.\"", () => {
    const { g, self, gy0, p2gy0 } = armed(0);
