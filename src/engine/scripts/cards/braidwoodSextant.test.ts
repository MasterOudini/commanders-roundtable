// `Braidwood Sextant` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Braidwood Sextant", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BRAIDWOOD_SEXTANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Braidwood Sextant", () => {
  test("{2}, {T}, Sacrifice this artifact: the vocabulary resolves \"Search your library for a basic land card, reveal that card, put it into your hand, then shuffle.\"", () => {
