// `Zoologist` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Zoologist", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ZOOLOGIST_SCRIPT]),
...
describe("Zoologist", () => {
  test("{3}{G}, {T}: the vocabulary resolves \"Reveal the top card of your library. If it's a creature card, put it onto the battlefield. Otherwise, put it into your graveyard.\"", () => {
