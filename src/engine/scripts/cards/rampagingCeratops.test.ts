// `Rampaging Ceratops` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RAMPAGING_CERATOPS_SCRIPT } from './rampagingCeratops';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Rampaging Ceratops";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rampaging Ceratops", "Grizzly Bears"], ["Cyclops of One-Eyed Pass", "Walking Corpse", "Walking Corpse"]],
    scripts: createRegistry([RAMPAGING_CERATOPS_SCRIPT]),
  });
  holdEverywhere(g);
