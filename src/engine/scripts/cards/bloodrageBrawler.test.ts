// `Bloodrage Brawler` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLOODRAGE_BRAWLER_SCRIPT } from './bloodrageBrawler';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bloodrage Brawler";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bloodrage Brawler"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLOODRAGE_BRAWLER_SCRIPT]),
  });
  holdEverywhere(g);
