// `Cold Case Cracker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { COLD_CASE_CRACKER_SCRIPT } from './coldCaseCracker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cold Case Cracker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cold Case Cracker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([COLD_CASE_CRACKER_SCRIPT]),
  });
  holdEverywhere(g);
