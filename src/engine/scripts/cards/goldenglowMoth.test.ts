// `Goldenglow Moth` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOLDENGLOW_MOTH_SCRIPT } from './goldenglowMoth';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goldenglow Moth";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goldenglow Moth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOLDENGLOW_MOTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Goldenglow Moth", () => {
  test("Whenever this creature blocks: 4 life is gained", () => {
