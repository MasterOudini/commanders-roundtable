// `Denethor, Stone Seer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DENETHOR_STONE_SEER_SCRIPT } from './denethorStoneSeer';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Denethor, Stone Seer";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Denethor, Stone Seer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DENETHOR_STONE_SEER_SCRIPT]),
  ...
  if (which === 1) {
    ...
    must(g.submit({ t: 'ChooseTargets', player: 'p1', targets: [{ kind: 'player', id: 'p2' }, { kind: 'card', id: no }] }));
    settle(g);
    }
  return { g, self, no, life0, energy0, hand0, board0, p2life0, p2hand0, gy0, p2gy0, lib0 };
}

describe("Denethor, Stone Seer", () => {
  test("When Denethor enters: scry 2 asks, and the cards stay on top", () => {
    const { g, lib0 } = armed(0);
    expect(g.state.priority.awaiting).toBeNull();
    expect((g.state.zones.library.p1 ?? []).length).toBe(lib0);
  });

  test("{3}{R}, {T}, Sacrifice Denethor: the vocabulary resolves \"Target player becomes the monarch. ~ deals 3 damage to any target.\"", () => {
    const { g, self, no } = armed(1);
    expect(g.state.monarch, 'the crown').toBe("p2");
