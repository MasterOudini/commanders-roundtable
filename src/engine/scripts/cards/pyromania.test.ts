// `Pyromania` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PYROMANIA_SCRIPT } from './pyromania';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Pyromania";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Pyromania"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PYROMANIA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Pyromania", () => {
  test("{1}{R}, Discard a card at random: it deals 1 damage to the declared creature", () => {
    const { g, no, hand0 } = armed(0);
    expect(g.state.cards[no]?.damage).toBe(1);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 - 1);
  });

  test("{1}{R}, Sacrifice this enchantment: it deals 1 damage to the declared creature", () => {
