// `Slumbering Cerberus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SLUMBERING_CERBERUS_SCRIPT } from './slumberingCerberus';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Slumbering Cerberus";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Slumbering Cerberus", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SLUMBERING_CERBERUS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Slumbering Cerberus", () => {
  ...
  test("Morbid — At the beginning of each end step: it untaps", () => {
