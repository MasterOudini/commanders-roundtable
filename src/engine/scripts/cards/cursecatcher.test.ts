// `Cursecatcher` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CURSECATCHER_SCRIPT } from './cursecatcher';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cursecatcher";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cursecatcher"], ["Cyclops of One-Eyed Pass", "Divination", "Forest"]],
    scripts: createRegistry([CURSECATCHER_SCRIPT]),
  });
  holdEverywhere(g);
