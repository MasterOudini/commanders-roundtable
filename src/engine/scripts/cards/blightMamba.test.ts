// `Blight Mamba` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLIGHT_MAMBA_SCRIPT } from './blightMamba';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blight Mamba";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blight Mamba", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLIGHT_MAMBA_SCRIPT]),
  });
  holdEverywhere(g);
