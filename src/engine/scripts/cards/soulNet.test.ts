// `Soul Net` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SOUL_NET_SCRIPT } from './soulNet';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Soul Net", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SOUL_NET_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Soul Net", () => {
  test("Whenever a creature dies: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is declined", () => {
  ...
  test("Whenever a creature dies: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is paid", () => {
