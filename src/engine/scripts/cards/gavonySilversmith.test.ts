// `Gavony Silversmith` - every declared pick is accepted and it gets a +1/+1 counter; a permanent the clause
// excludes is refused (D299). Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { GAVONY_SILVERSMITH_SCRIPT } from './gavonySilversmith';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Gavony Silversmith";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; target2: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Gavony Silversmith"], ["Grizzly Bears", "Vampire Nighthawk", "Forest"]],
    scripts: createRegistry([GAVONY_SILVERSMITH_SCRIPT]),
  });
  holdEverywhere(g);
