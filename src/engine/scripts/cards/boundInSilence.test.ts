// `Bound in Silence` - cast on Grizzly Bears it attaches and the enchanted creature cannot attack or block;
...
import { BOUND_IN_SILENCE_SCRIPT } from './boundInSilence';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bound in Silence";
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([BOUND_IN_SILENCE_SCRIPT]),
  });
  holdEverywhere(g);
