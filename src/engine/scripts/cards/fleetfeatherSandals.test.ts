// `Fleetfeather Sandals` - equipped to Grizzly Bears the creature has flying, haste; the Cyclops is untouched;
...
import { FLEETFEATHER_SANDALS_SCRIPT } from './fleetfeatherSandals';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fleetfeather Sandals";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([FLEETFEATHER_SANDALS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FLEETFEATHER_SANDALS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fleetfeather Sandals", () => {
  test("equipped to Grizzly Bears: attached, and the creature has flying, haste; the Cyclops is untouched", () => {
