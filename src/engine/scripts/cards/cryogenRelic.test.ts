// `Cryogen Relic` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRYOGEN_RELIC_SCRIPT } from './cryogenRelic';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cryogen Relic";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cryogen Relic"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CRYOGEN_RELIC_SCRIPT]),
...
describe("Cryogen Relic", () => {
  test("When this artifact enters or leaves the battlefield: a card is drawn [etb]", () => {
    const { g, hand0 } = armed(0);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1);
  });

  test("When this artifact enters or leaves the battlefield: a card is drawn [leavesBattlefield]", () => {
    const { g, hand0 } = armed(1);
    expect((g.state.zones.hand.p1 ?? []).length).toBe(hand0 + 1);
  });

  test("{1}{U}, Sacrifice this artifact: the vocabulary resolves \"Put a stun counter on up to one target tapped creature.\"", () => {
    const { g, self, no } = armed(2);
    expect(g.state.cards[no]?.counters["stun"] ?? 0).toBe(1);
