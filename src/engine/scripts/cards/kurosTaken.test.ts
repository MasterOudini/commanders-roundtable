// `Kuro's Taken` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KUROS_TAKEN_SCRIPT } from './kurosTaken';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kuro's Taken";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kuro's Taken", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KUROS_TAKEN_SCRIPT]),
  });
  holdEverywhere(g);
