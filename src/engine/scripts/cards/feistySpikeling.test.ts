// `Feisty Spikeling` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([FEISTY_SPIKELING_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Feisty Spikeling"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FEISTY_SPIKELING_SCRIPT]),
...
describe("Feisty Spikeling", () => {
  test("`Feisty Spikeling` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
