// `Flooded Strand` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Flooded Strand", "Plains"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FLOODED_STRAND_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Flooded Strand", () => {
  test("{T}, Pay 1 life, Sacrifice this land: the vocabulary resolves \"Search your library for a Plains or Island card, put it onto the battlefield, then shuffle.\"", () => {
