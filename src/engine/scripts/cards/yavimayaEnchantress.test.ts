// `Yavimaya Enchantress` - the pump is COUNTED: zero of them is no bonus, and each one moves the
// derived power and toughness by the printed delta. Generated from one table row.

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registryCore';
import { YAVIMAYA_ENCHANTRESS_SCRIPT } from './yavimayaEnchantress';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Yavimaya Enchantress";
const COUNTED = "Hissing Miasma";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([YAVIMAYA_ENCHANTRESS_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; subject: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([YAVIMAYA_ENCHANTRESS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Yavimaya Enchantress", () => {
  test("each Hissing Miasma counted moves the P/T by [1, 1], and one leaving takes it back", () => {
