// `Grixis Panorama` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grixis Panorama", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRIXIS_PANORAMA_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Grixis Panorama", () => {
  test("{1}, {T}, Sacrifice this land: the vocabulary resolves \"Search your library for a basic Island, Swamp, or Mountain card, put it onto the battlefield tapped, then shuffle.\"", () => {
