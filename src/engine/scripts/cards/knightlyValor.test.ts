// `Knightly Valor` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: 1 token); gets +2/+2 and has vigilance;
...
import { KNIGHTLY_VALOR_SCRIPT } from './knightlyValor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Knightly Valor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KNIGHTLY_VALOR_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([KNIGHTLY_VALOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Knightly Valor", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: 1 token); gets +2/+2 and has vigilance; Cyclops of One-Eyed Pass is untouched", () => {
