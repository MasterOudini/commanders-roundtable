// `Sedraxis Specter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SEDRAXIS_SPECTER_SCRIPT } from './sedraxisSpecter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sedraxis Specter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sedraxis Specter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SEDRAXIS_SPECTER_SCRIPT]),
