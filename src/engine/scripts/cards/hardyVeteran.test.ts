// `Hardy Veteran` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HARDY_VETERAN_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hardy Veteran"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HARDY_VETERAN_SCRIPT]),
...
describe("Hardy Veteran", () => {
  test("`Hardy Veteran` - as long as it's your turn: absent while it is unmet, read once it holds", () => {
