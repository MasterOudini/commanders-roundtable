// `Turtle Blimp` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TURTLE_BLIMP_SCRIPT } from './turtleBlimp';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Turtle Blimp";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Turtle Blimp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TURTLE_BLIMP_SCRIPT]),
  });
  holdEverywhere(g);
