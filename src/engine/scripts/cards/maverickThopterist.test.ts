// `Maverick Thopterist` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MAVERICK_THOPTERIST_SCRIPT } from './maverickThopterist';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Maverick Thopterist";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Maverick Thopterist"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MAVERICK_THOPTERIST_SCRIPT]),
  });
  holdEverywhere(g);
