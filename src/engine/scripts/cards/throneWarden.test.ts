// `Throne Warden` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THRONE_WARDEN_SCRIPT } from './throneWarden';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Throne Warden";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Throne Warden"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THRONE_WARDEN_SCRIPT]),
