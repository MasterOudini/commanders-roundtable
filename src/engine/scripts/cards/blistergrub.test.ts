// `Blistergrub` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BLISTERGRUB_SCRIPT } from './blistergrub';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Blistergrub";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blistergrub"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLISTERGRUB_SCRIPT]),
  });
  holdEverywhere(g);
