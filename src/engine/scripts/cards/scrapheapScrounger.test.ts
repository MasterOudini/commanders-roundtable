// `Scrapheap Scrounger` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCRAPHEAP_SCROUNGER_SCRIPT } from './scrapheapScrounger';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scrapheap Scrounger";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scrapheap Scrounger", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCRAPHEAP_SCROUNGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Scrapheap Scrounger", () => {
  test("This creature can't block.: the block is refused", () => {
    const { refused } = armed(0);
    expect(refused).toBe(true);
  });

  test("{1}{B}, Exile another creature card from your graveyard: it returns from the graveyard to the battlefield", () => {
