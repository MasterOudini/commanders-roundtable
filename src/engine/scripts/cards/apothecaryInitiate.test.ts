// `Apothecary Initiate` - every printed ability proven in its own game: the cost's mark, the pump
...
import { APOTHECARY_INITIATE_SCRIPT } from './apothecaryInitiate';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Apothecary Initiate";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Apothecary Initiate", "Grizzly Bears", "Devoted Hero", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([APOTHECARY_INITIATE_SCRIPT]),
  });
  holdEverywhere(g);
