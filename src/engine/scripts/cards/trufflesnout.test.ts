// `Trufflesnout` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRUFFLESNOUT_SCRIPT } from './trufflesnout';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trufflesnout";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trufflesnout"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRUFFLESNOUT_SCRIPT]),
  });
  holdEverywhere(g);
