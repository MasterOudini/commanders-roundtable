// `Greta, Sweettooth Scourge` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GRETA_SWEETTOOTH_SCOURGE_SCRIPT } from './gretaSweettoothScourge';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';
import { TOKEN_TABLE } from '../../../data/tokenTable';

const CARD = "Greta, Sweettooth Scourge";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Greta, Sweettooth Scourge"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GRETA_SWEETTOOTH_SCOURGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Greta, Sweettooth Scourge", () => {
  test("When Greta enters: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("{G}, Sacrifice a Food: the declared creature gets 1 +1/+1 counter", () => {
  ...
  test("{1}{B}, Sacrifice a Food: a card is drawn and 1 life lost", () => {
    const { g, life0, hand0, fodder0 } = armed(2);
