// `Holy Armor` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HOLY_ARMOR_SCRIPT } from './holyArmor';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Holy Armor";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([HOLY_ARMOR_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Holy Armor", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HOLY_ARMOR_SCRIPT]),
  });
  holdEverywhere(g);
