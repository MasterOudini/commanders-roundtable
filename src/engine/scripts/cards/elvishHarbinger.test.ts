// `Elvish Harbinger` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Elvish Harbinger", "Devoted Hero"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ELVISH_HARBINGER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Elvish Harbinger", () => {
  test("When this creature enters: the vocabulary resolves \"Search your library for an Elf card, reveal it, then shuffle and put that card on top.\"", () => {
    const { g } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'searchLibrary', 20_000);
    { const stray = Object.keys(g.state.cards).find((id) => nameOf(g, id as InstanceId) === "Devoted Hero" && g.state.cards[id as InstanceId]?.zone.kind !== 'library');
      if (stray) must(g.submit({ t: 'ManualMoveCard', player: 'p1', card: stray as InstanceId, to: { kind: 'library', player: 'p1' } }));
      const lib = [...(g.state.zones.library.p1 ?? [])];
      const found = lib.find((id) => nameOf(g, id) === "Devoted Hero");
      expect(found, "Devoted Hero is not in the library for the search to find").toBeDefined();
