// `Quakefoot Cyclops` - every printed ability proven in its own game: the cost's mark, the pump
...
import { QUAKEFOOT_CYCLOPS_SCRIPT } from './quakefootCyclops';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Quakefoot Cyclops";
...
function cyclingIndex(g: Game, id: InstanceId): number {
  const d = deps(createRegistry([QUAKEFOOT_CYCLOPS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Quakefoot Cyclops"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([QUAKEFOOT_CYCLOPS_SCRIPT]),
...
describe("Quakefoot Cyclops", () => {
  test("When this creature enters: the vocabulary resolves \"Up to two target creatures can't block this turn.\"", () => {
    const { g, no } = armed(0);
  ...
  test("When you cycle this card: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
    const { g, no } = armed(1);
    if (g.state.cards[no]?.zone.kind === 'battlefield') expect(g.state.untilEndOfTurn.some((m) => m.card === no && m.cantBlock === true), 'cannot block this turn').toBe(true);
  });
