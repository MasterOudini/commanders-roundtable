// `Debilitating Injury` - cast on Grizzly Bears it attaches and the enchanted creature gets -2/-2;
...
import { DEBILITATING_INJURY_SCRIPT } from './debilitatingInjury';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Debilitating Injury";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DEBILITATING_INJURY_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([DEBILITATING_INJURY_SCRIPT]),
  });
  holdEverywhere(g);
