// `Spined Fluke` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPINED_FLUKE_SCRIPT } from './spinedFluke';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spined Fluke";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spined Fluke", "Lightning Bolt", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPINED_FLUKE_SCRIPT]),
  });
  holdEverywhere(g);
