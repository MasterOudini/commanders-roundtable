// `Ringing Strike Mastery` - every printed ability proven in its own game: the cost's mark, the pump
...
import { RINGING_STRIKE_MASTERY_SCRIPT } from './ringingStrikeMastery';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { legalActions } from '../../legal';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ringing Strike Mastery";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ringing Strike Mastery", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RINGING_STRIKE_MASTERY_SCRIPT]),
...
describe("Ringing Strike Mastery", () => {
  ...
  test("Enchanted creature has \"{5}: the creature it is attached to reads it", () => {
    const { g, self, bearsB } = armed(2);
    expect(g.state.cards[self]?.attachedTo).toBe(bearsB);
  });

  test("Enchanted creature has \"{5}: the vocabulary resolves \"Untap this creature.\"", () => {
