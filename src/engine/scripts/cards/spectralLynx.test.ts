// `Spectral Lynx` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPECTRAL_LYNX_SCRIPT } from './spectralLynx';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spectral Lynx";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spectral Lynx", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPECTRAL_LYNX_SCRIPT]),
  });
  holdEverywhere(g);
