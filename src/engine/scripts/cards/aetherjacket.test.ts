// `Aetherjacket` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AETHERJACKET_SCRIPT } from './aetherjacket';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aetherjacket";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aetherjacket"], ["Cyclops of One-Eyed Pass", "Sol Ring"]],
    scripts: createRegistry([AETHERJACKET_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aetherjacket", () => {
  test("{2}, {T}, Sacrifice this creature: the vocabulary resolves \"Destroy another target artifact.\"", () => {
