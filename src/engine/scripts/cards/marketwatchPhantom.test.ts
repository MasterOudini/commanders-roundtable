// `Marketwatch Phantom` - every printed ability proven in its own game: the cost's mark, the pump
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MARKETWATCH_PHANTOM_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Marketwatch Phantom", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MARKETWATCH_PHANTOM_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Marketwatch Phantom", () => {
  test("Whenever another creature you control with power 2 or less enters: it gains flying until end of turn", () => {
