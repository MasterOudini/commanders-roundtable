// `Vorosh, the Hunter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VOROSH_THE_HUNTER_SCRIPT } from './voroshTheHunter';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Vorosh, the Hunter";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Vorosh, the Hunter", "Forest", "Forest", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VOROSH_THE_HUNTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Vorosh, the Hunter", () => {
  test("Whenever Vorosh deals combat damage to a player: the vocabulary resolves \"You may pay {2}{G}. If you do, put six +1/+1 counters on ~.\" - the price is declined", () => {
  ...
  test("Whenever Vorosh deals combat damage to a player: the vocabulary resolves \"You may pay {2}{G}. If you do, put six +1/+1 counters on ~.\" - the price is paid", () => {
    const { g, self, payL1_p1_0, payL1_p1_1, payL1_p1_2 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
    ...
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(6);
  });
