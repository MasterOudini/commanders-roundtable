// `Frog Tongue` - cast on Grizzly Bears it attaches and the enchanted creature (on entering: draw 1); has reach;
...
import { FROG_TONGUE_SCRIPT } from './frogTongue';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Frog Tongue";
...
function kw(g: Game, id: InstanceId): ReadonlySet<string> {
  const d = deps(createRegistry([FROG_TONGUE_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([FROG_TONGUE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Frog Tongue", () => {
  test("on Grizzly Bears: attached, and the enchanted creature (on entering: draw 1); has reach; Cyclops of One-Eyed Pass is untouched", () => {
