// `Thalakos Scout` - every printed ability proven in its own game: the cost's mark, the pump
...
import { THALAKOS_SCOUT_SCRIPT } from './thalakosScout';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Thalakos Scout";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Thalakos Scout", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([THALAKOS_SCOUT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Thalakos Scout", () => {
  test("Discard a card: it returns to its owner's hand", () => {
