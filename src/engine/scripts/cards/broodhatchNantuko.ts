// `Broodhatch Nantuko` - a isDealtCombatDamage trigger vocab, a isDealtNoncombatDamage trigger vocab
// until end of turn where it pumps (D194's carrier, D301). Generated from one table row.

import { BROODHATCH_NANTUKO } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(BROODHATCH_NANTUKO, "Whenever this creature is dealt damage, you may create that many 1/1 green Insect creature tokens.\nMorph {2}{G} (You may cast this card face down as a 2/2 creature for {3}. Turn it face up any time for its morph cost.)");
const LINES = PRINTED.split('\n');

const VOCAB_L0 = vocabularyEffects("Create that many 1/1 green Insect creature tokens.", BROODHATCH_NANTUKO.name, { memo: true });
const VOCAB_T_L0 = vocabularyTargets("Create that many 1/1 green Insect creature tokens.");

export const BROODHATCH_NANTUKO_SCRIPT: CardScript = {
  oracleId: BROODHATCH_NANTUKO.oracleId,
  name: BROODHATCH_NANTUKO.name,
  triggers: [
    {
      ...
      label: () => "Broodhatch Nantuko - Create that many 1/1 green Insect creature tokens.",
