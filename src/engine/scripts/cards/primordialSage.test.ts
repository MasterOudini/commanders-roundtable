// `Primordial Sage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { PRIMORDIAL_SAGE_SCRIPT } from './primordialSage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Primordial Sage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Primordial Sage", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([PRIMORDIAL_SAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Primordial Sage", () => {
  test("Whenever you cast a creature spell: a card is drawn", () => {
