// `EPF Point Squad` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["EPF Point Squad", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EPF_POINT_SQUAD_SCRIPT]),
  });
  holdEverywhere(g);
