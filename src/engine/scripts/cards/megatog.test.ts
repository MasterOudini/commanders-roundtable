// `Megatog` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MEGATOG_SCRIPT } from './megatog';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MEGATOG_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Megatog", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MEGATOG_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Megatog", () => {
  test("Sacrifice an artifact: it gets +3/+3 and gains trample until end of turn", () => {
