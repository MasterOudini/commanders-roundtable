// `Shi'ar Soldier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SHIAR_SOLDIER_SCRIPT } from './shiarSoldier';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Shi'ar Soldier";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Shi'ar Soldier", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SHIAR_SOLDIER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Shi'ar Soldier", () => {
  test("{U}, {T}: the vocabulary resolves \"Return another target permanent you control to its owner's hand.\" - refused unless during your turn", () => {
    const { g, self, hand0, board0, condRefused, vtA0_0 } = armed(0);
