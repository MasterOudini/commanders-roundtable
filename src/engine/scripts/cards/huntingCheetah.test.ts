// `Hunting Cheetah` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hunting Cheetah", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HUNTING_CHEETAH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hunting Cheetah", () => {
  test("Whenever this creature deals damage to an opponent: the vocabulary resolves \"Search your library for a Forest card, reveal that card, put it into your hand, then shuffle.\"", () => {
    const { g } = armed(0);
