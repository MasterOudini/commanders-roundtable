// `Inspired Sprite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INSPIRED_SPRITE_SCRIPT } from './inspiredSprite';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Inspired Sprite";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Inspired Sprite", "Grizzly Bears", "Fugitive Wizard"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INSPIRED_SPRITE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  const bearsSpell = put(g, 'p1', "Fugitive Wizard", 'hand');
...
describe("Inspired Sprite", () => {
  test("Whenever you cast a Wizard spell: it untaps", () => {
