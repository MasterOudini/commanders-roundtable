// `Dawnfeather Eagle` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DAWNFEATHER_EAGLE_SCRIPT } from './dawnfeatherEagle';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dawnfeather Eagle";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DAWNFEATHER_EAGLE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dawnfeather Eagle", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DAWNFEATHER_EAGLE_SCRIPT]),
  });
  holdEverywhere(g);
