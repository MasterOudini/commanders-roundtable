// `Novice Inspector` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NOVICE_INSPECTOR_SCRIPT } from './noviceInspector';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Novice Inspector";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Novice Inspector"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NOVICE_INSPECTOR_SCRIPT]),
  });
  holdEverywhere(g);
