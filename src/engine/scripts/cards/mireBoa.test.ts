// `Mire Boa` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MIRE_BOA_SCRIPT } from './mireBoa';
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mire Boa", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MIRE_BOA_SCRIPT]),
  });
  holdEverywhere(g);
