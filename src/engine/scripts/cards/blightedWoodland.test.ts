// `Blighted Woodland` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Blighted Woodland", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BLIGHTED_WOODLAND_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Blighted Woodland", () => {
  test("{3}{G}, {T}, Sacrifice this land: the vocabulary resolves \"Search your library for up to two basic land cards, put them onto the battlefield tapped, then shuffle.\"", () => {
