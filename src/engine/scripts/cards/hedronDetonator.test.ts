// `Hedron Detonator` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HEDRON_DETONATOR_SCRIPT } from './hedronDetonator';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Hedron Detonator";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Hedron Detonator", "Sol Ring", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HEDRON_DETONATOR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Hedron Detonator", () => {
  test("Whenever an artifact you control enters: it deals 1 damage to the declared player", () => {
    const { g, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 - 1);
  });

  test("{T}, Sacrifice two artifacts: the vocabulary resolves \"Exile the top card of your library. You may play that card this turn.\"", () => {
