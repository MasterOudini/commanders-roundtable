// `Goblin Boarders` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_BOARDERS_SCRIPT } from './goblinBoarders';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Boarders";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([GOBLIN_BOARDERS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Boarders", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_BOARDERS_SCRIPT]),
  });
  holdEverywhere(g);
