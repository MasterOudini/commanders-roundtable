// `Lofty Dreams` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOFTY_DREAMS_SCRIPT } from './loftyDreams';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Lofty Dreams";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([LOFTY_DREAMS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lofty Dreams", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOFTY_DREAMS_SCRIPT]),
  });
  holdEverywhere(g);
