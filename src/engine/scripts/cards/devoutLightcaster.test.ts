// `Devout Lightcaster` - the named target is accepted and it is exiled; a permanent the adjective excludes
// is refused (D294). Generated from one table row (D295).

import { describe, expect, test } from 'vitest';
import { replay, stateHash } from '../../log';
import { createRegistry } from '../registry';
import { DEVOUT_LIGHTCASTER_SCRIPT } from './devoutLightcaster';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Devout Lightcaster";
...
function armed(): { g: Game; self: InstanceId; target: InstanceId; wrong: InstanceId; life0: number } {
  const g = startedGame({
    players: 2,
    decks: [["Devout Lightcaster"], ["Vampire Nighthawk", "Grizzly Bears"]],
    scripts: createRegistry([DEVOUT_LIGHTCASTER_SCRIPT]),
  });
  holdEverywhere(g);
