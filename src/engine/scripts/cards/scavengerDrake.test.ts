// `Scavenger Drake` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SCAVENGER_DRAKE_SCRIPT } from './scavengerDrake';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Scavenger Drake";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Scavenger Drake"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SCAVENGER_DRAKE_SCRIPT]),
  });
  holdEverywhere(g);
