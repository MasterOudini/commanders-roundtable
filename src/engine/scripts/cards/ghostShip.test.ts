// `Ghost Ship` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GHOST_SHIP_SCRIPT } from './ghostShip';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Ghost Ship";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Ghost Ship", "Infernal Grasp"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GHOST_SHIP_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Ghost Ship", () => {
  test("{U}{U}{U}: it regenerates from the Bolt", () => {
