// `Bontu's Monument` - every printed ability proven in its own game: the cost's mark, the pump
...
import { BONTUS_MONUMENT_SCRIPT } from './bontusMonument';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Bontu's Monument";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Bontu's Monument", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([BONTUS_MONUMENT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Bontu's Monument", () => {
  test("Whenever you cast a creature spell: each opponent loses 1 life and 1 is gained", () => {
