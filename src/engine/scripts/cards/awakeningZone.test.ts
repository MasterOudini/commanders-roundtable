// `Awakening Zone` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AWAKENING_ZONE_SCRIPT } from './awakeningZone';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Awakening Zone";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Awakening Zone"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AWAKENING_ZONE_SCRIPT]),
...
describe("Awakening Zone", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Create a 0/1 colorless Eldrazi Spawn creature token. It has \\\"Sacrifice this token: Add {C}.\\\"\"", () => {
