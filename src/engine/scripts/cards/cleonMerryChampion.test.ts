// `Cleon, Merry Champion` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLEON_MERRY_CHAMPION_SCRIPT } from './cleonMerryChampion';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cleon, Merry Champion";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cleon, Merry Champion", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLEON_MERRY_CHAMPION_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cleon, Merry Champion", () => {
  test("Heroic — Whenever you cast a spell that targets Cleon: the vocabulary resolves \"Exile the top card of your library. You may play that card until the end of your next turn.\"", () => {
    const { g } = armed(0);
