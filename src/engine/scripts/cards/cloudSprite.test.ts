// `Cloud Sprite` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLOUD_SPRITE_SCRIPT } from './cloudSprite';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cloud Sprite";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cloud Sprite", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLOUD_SPRITE_SCRIPT]),
  });
  holdEverywhere(g);
