// `Spike Soldier` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPIKE_SOLDIER_SCRIPT } from './spikeSoldier';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spike Soldier";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SPIKE_SOLDIER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spike Soldier"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPIKE_SOLDIER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spike Soldier", () => {
  ...
  test("Remove a +1/+1 counter from this creature: it gets +2/+2 until end of turn", () => {
    const { g, self, cnt0, pt0 } = armed(2);
    expect(pt(g, self)).toEqual([(pt0[0] ?? 0) + 2 - 1, (pt0[1] ?? 0) + 2 - 1]);
    expect(g.state.cards[self]?.counters["+1/+1"] ?? 0).toBe(cnt0 - 1);
  });
