// `Sergeant-at-Arms` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SERGEANT_AT_ARMS_SCRIPT } from './sergeantAtArms';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Sergeant-at-Arms";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Sergeant-at-Arms"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SERGEANT_AT_ARMS_SCRIPT]),
  });
  holdEverywhere(g);
