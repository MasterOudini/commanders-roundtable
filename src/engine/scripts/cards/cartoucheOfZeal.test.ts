// `Cartouche of Zeal` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CARTOUCHE_OF_ZEAL_SCRIPT } from './cartoucheOfZeal';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cartouche of Zeal";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CARTOUCHE_OF_ZEAL_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cartouche of Zeal", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CARTOUCHE_OF_ZEAL_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cartouche of Zeal", () => {
  test("When this Aura enters: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
    const { g, no } = armed(0);
  ...
  test("Enchanted creature gets +1/+1 and has haste. (It can attack and {T} no matter when it came under your control.): the creature it is attached to reads it", () => {
