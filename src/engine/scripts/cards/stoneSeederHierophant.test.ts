// `Stone-Seeder Hierophant` - every printed ability proven in its own game: the cost's mark, the pump
...
import { STONE_SEEDER_HIEROPHANT_SCRIPT } from './stoneSeederHierophant';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Stone-Seeder Hierophant";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Stone-Seeder Hierophant"], ["Cyclops of One-Eyed Pass", "Forest"]],
    scripts: createRegistry([STONE_SEEDER_HIEROPHANT_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Stone-Seeder Hierophant", () => {
  test("Landfall — Whenever a land you control enters: it untaps", () => {
