// `Merrow Grimeblotter` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MERROW_GRIMEBLOTTER_SCRIPT } from './merrowGrimeblotter';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Merrow Grimeblotter";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MERROW_GRIMEBLOTTER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Merrow Grimeblotter"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MERROW_GRIMEBLOTTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Merrow Grimeblotter", () => {
  test("{1}{U/B}, {Q}: the declared creature is pumped until end of turn", () => {
