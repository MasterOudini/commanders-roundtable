// `Goblin Warrens` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GOBLIN_WARRENS_SCRIPT } from './goblinWarrens';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Goblin Warrens";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Goblin Warrens", "Raging Goblin", "Raging Goblin"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GOBLIN_WARRENS_SCRIPT]),
  });
  holdEverywhere(g);
  const no = put(g, 'p2', "Cyclops of One-Eyed Pass");
  const fodder0 = [put(g, 'p1', "Raging Goblin"), put(g, 'p1', "Raging Goblin")];
  settle(g);
...
describe("Goblin Warrens", () => {
  test("{2}{R}, Sacrifice two Goblins: 3 tokens made", () => {
    const { g, board0, fodder0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 3 - 2);
