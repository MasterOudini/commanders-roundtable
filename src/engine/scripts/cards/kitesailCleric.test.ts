// `Kitesail Cleric` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KITESAIL_CLERIC_SCRIPT } from './kitesailCleric';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kitesail Cleric";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kitesail Cleric"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KITESAIL_CLERIC_SCRIPT]),
...
describe("Kitesail Cleric", () => {
  test("When this creature enters: the vocabulary resolves \"Tap up to two target creatures.\"", () => {
    const { g, no } = armed(0);
