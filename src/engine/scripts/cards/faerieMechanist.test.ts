// `Faerie Mechanist` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Faerie Mechanist", "Sol Ring", "Sol Ring"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FAERIE_MECHANIST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Faerie Mechanist", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top three cards of your library. You may reveal an artifact card from among them and put it into your hand. Put the rest on the bottom of your library in any order.\"", () => {
    const { g } = armed(0);
