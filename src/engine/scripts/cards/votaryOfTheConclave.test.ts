// `Votary of the Conclave` - every printed ability proven in its own game: the cost's mark, the pump
...
import { VOTARY_OF_THE_CONCLAVE_SCRIPT } from './votaryOfTheConclave';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Votary of the Conclave";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Votary of the Conclave", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VOTARY_OF_THE_CONCLAVE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Votary of the Conclave", () => {
