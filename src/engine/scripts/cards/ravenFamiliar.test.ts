// `Raven Familiar` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Raven Familiar", "Forest", "Forest", "Island"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAVEN_FAMILIAR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Raven Familiar", () => {
  test("When this creature enters: the vocabulary resolves \"Look at the top three cards of your library. Put one of them into your hand and the rest on the bottom of your library in any order.\"", () => {
    const { g } = armed(0);
