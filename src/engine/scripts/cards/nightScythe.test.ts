// `Night Scythe` - every printed ability proven in its own game: the cost's mark, the pump
...
import { NIGHT_SCYTHE_SCRIPT } from './nightScythe';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Night Scythe";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Night Scythe"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([NIGHT_SCYTHE_SCRIPT]),
...
describe("Night Scythe", () => {
  test("Invasion Beams — When this Vehicle enters: 1 token made", () => {
