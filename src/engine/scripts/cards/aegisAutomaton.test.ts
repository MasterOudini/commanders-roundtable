// `Aegis Automaton` - every printed ability proven in its own game: the cost's mark, the pump
...
import { AEGIS_AUTOMATON_SCRIPT } from './aegisAutomaton';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Aegis Automaton";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Aegis Automaton", "Cyclops of One-Eyed Pass"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([AEGIS_AUTOMATON_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Aegis Automaton", () => {
  test("{4}{W}: the vocabulary resolves \"Return another target creature you control to its owner's hand.\"", () => {
