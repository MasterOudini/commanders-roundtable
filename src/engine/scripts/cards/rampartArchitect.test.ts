// `Rampart Architect` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Rampart Architect", "Grizzly Bears", "Dragon's Eye Sentry", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([RAMPART_ARCHITECT_SCRIPT]),
  ...
  if (which === 2) {
    const bears = put(g, 'p1', "Dragon's Eye Sentry");
...
describe("Rampart Architect", () => {
  test("Whenever this creature enters or attacks: 1 token made [etb]", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("Whenever this creature enters or attacks: 1 token made [attacks]", () => {
    const { g, board0 } = armed(1);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("Whenever a creature you control with defender dies: the vocabulary resolves \"Search your library for a basic land card, put that card onto the battlefield tapped, then shuffle.\"", () => {
