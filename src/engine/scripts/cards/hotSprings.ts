// `Hot Springs` - the permanent it is attached to HAS a quoted
...
import { HOT_SPRINGS } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(HOT_SPRINGS, "Enchant land you control\nEnchanted land has \"{T}: Prevent the next 1 damage that would be dealt to any target this turn.\"");
const LINES = PRINTED.split("\n");

const GRANT = grantedActivated("{T}: Prevent the next 1 damage that would be dealt to any target this turn.", `${HOT_SPRINGS.oracleId}#g1`, HOT_SPRINGS.name);
const VOCAB = vocabularyEffects("Prevent the next 1 damage that would be dealt to any target this turn.", HOT_SPRINGS.name);
const VOCAB_T = vocabularyTargets("Prevent the next 1 damage that would be dealt to any target this turn.");

export const HOT_SPRINGS_SCRIPT: CardScript = {
  oracleId: HOT_SPRINGS.oracleId,
  name: HOT_SPRINGS.name,
  statics: [
    {
      abilityId: 'grant-1',
