// `Incisor Glider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { INCISOR_GLIDER_SCRIPT } from './incisorGlider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Incisor Glider";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([INCISOR_GLIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Incisor Glider", "Coral Eel"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([INCISOR_GLIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Incisor Glider", () => {
  test("Corrupted — Whenever this creature attacks: its controller's creatures get +1/+1 until end of turn", () => {
