// `Arena Athlete` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ARENA_ATHLETE_SCRIPT } from './arenaAthlete';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Arena Athlete";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Arena Athlete", "Giant Growth"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARENA_ATHLETE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Arena Athlete", () => {
  test("Heroic — Whenever you cast a spell that targets this creature: the vocabulary resolves \"Target creature an opponent controls can't block this turn.\"", () => {
    const { g, no } = armed(0);
