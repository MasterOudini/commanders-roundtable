// `Spark Reaper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SPARK_REAPER_SCRIPT } from './sparkReaper';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Spark Reaper";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Spark Reaper", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SPARK_REAPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Spark Reaper", () => {
  test("{3}, Sacrifice a creature or planeswalker: the vocabulary resolves \"You gain 1 life and draw a card.\"", () => {
