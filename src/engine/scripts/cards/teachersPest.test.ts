// `Teacher's Pest` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TEACHERS_PEST_SCRIPT } from './teachersPest';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Teacher's Pest";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Teacher's Pest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TEACHERS_PEST_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Teacher's Pest", () => {
  test("Whenever this creature attacks: 1 life is gained", () => {
    const { g, life0 } = armed(0);
    expect(g.state.players.p1?.life).toBe(life0 + 1);
  });

  test("{B}{G}: it returns from the graveyard to the battlefield tapped", () => {
