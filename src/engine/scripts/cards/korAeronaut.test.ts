// `Kor Aeronaut` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KOR_AERONAUT_SCRIPT } from './korAeronaut';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Kor Aeronaut";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KOR_AERONAUT_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Kor Aeronaut"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KOR_AERONAUT_SCRIPT]),
  });
  holdEverywhere(g);
