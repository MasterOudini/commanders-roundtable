// `Loan Shark` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LOAN_SHARK_SCRIPT } from './loanShark';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Loan Shark";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Loan Shark", "Divination", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LOAN_SHARK_SCRIPT]),
