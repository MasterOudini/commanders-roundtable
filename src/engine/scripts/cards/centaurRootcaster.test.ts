// `Centaur Rootcaster` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Centaur Rootcaster", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CENTAUR_ROOTCASTER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Centaur Rootcaster", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Search your library for a basic land card, put that card onto the battlefield tapped, then shuffle.\"", () => {
