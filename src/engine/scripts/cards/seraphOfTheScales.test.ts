// `Seraph of the Scales` - every printed ability proven in its own game: the cost's mark, the pump
...
import { SERAPH_OF_THE_SCALES_SCRIPT } from './seraphOfTheScales';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Seraph of the Scales";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([SERAPH_OF_THE_SCALES_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Seraph of the Scales"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([SERAPH_OF_THE_SCALES_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Seraph of the Scales", () => {
  test("{W}: it gains vigilance until end of turn", () => {
