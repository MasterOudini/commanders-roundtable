// `Trigon of Rage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TRIGON_OF_RAGE_SCRIPT } from './trigonOfRage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Trigon of Rage";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([TRIGON_OF_RAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Trigon of Rage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TRIGON_OF_RAGE_SCRIPT]),
...
describe("Trigon of Rage", () => {
  ...
  test("{R}{R}, {T}: it gets 1 charge counter", () => {
  ...
  test("{2}, {T}, Remove a charge counter from this artifact: the declared creature is pumped until end of turn", () => {
    const { g, self, no, cnt0 } = armed(2);
    expect(pt(g, no)).toEqual([8, 2]);
    expect(g.state.cards[self]?.counters["charge"] ?? 0).toBe(cnt0 - 1);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });
