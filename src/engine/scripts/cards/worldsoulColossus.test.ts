// `Worldsoul Colossus` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WORLDSOUL_COLOSSUS_SCRIPT } from './worldsoulColossus';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Worldsoul Colossus";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WORLDSOUL_COLOSSUS_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Worldsoul Colossus"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([WORLDSOUL_COLOSSUS_SCRIPT]),
  });
  holdEverywhere(g);
