// `Clamor Shaman` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLAMOR_SHAMAN_SCRIPT } from './clamorShaman';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Clamor Shaman";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Clamor Shaman"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLAMOR_SHAMAN_SCRIPT]),
...
describe("Clamor Shaman", () => {
  test("Whenever this creature attacks: the vocabulary resolves \"Target creature an opponent controls can't block this turn.\"", () => {
    const { g, no } = armed(0);
