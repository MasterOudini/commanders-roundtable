// `Cragplate Baloth` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CRAGPLATE_BALOTH_SCRIPT } from './cragplateBaloth';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Cragplate Baloth";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CRAGPLATE_BALOTH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Cragplate Baloth"], ["Cyclops of One-Eyed Pass", "Counterspell"]],
    scripts: createRegistry([CRAGPLATE_BALOTH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Cragplate Baloth", () => {
  ...
  test("If this creature was kicked, it enters with four +1/+1 counters on it.: cast, it enters with 4 +1/+1 counters", () => {
    const { g, self } = armed(1);
    expect(g.state.cards[self]?.zone.kind).toBe('battlefield');
    expect(g.state.cards[self]?.counters['+1/+1'] ?? 0).toBe(4);
    expect(pt(g, self)).toEqual([10, 10]);
  });
