// `Kitesail` - equipped to Grizzly Bears the creature gets +1/+0 and has flying; the Cyclops is untouched;
...
import { KITESAIL_SCRIPT } from './kitesail';
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KITESAIL_SCRIPT]));
...
function board(): { g: Game; self: InstanceId; host: InstanceId; second: InstanceId; other: InstanceId } {
  const g = startedGame({
    ...
    scripts: createRegistry([KITESAIL_SCRIPT]),
  });
  holdEverywhere(g);
