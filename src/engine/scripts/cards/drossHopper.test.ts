// `Dross Hopper` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DROSS_HOPPER_SCRIPT } from './drossHopper';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dross Hopper";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([DROSS_HOPPER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dross Hopper", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DROSS_HOPPER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Dross Hopper", () => {
  test("Sacrifice a creature: it gains flying until end of turn", () => {
