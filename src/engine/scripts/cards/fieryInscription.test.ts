// `Fiery Inscription` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FIERY_INSCRIPTION_SCRIPT } from './fieryInscription';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fiery Inscription";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fiery Inscription", "Pyretic Ritual"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FIERY_INSCRIPTION_SCRIPT]),
...
describe("Fiery Inscription", () => {
  ...
  test("Whenever you cast an instant or sorcery spell: the vocabulary resolves \"This enchantment deals 2 damage to each opponent.\"", () => {
