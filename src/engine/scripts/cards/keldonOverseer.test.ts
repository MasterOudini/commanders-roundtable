// `Keldon Overseer` - every printed ability proven in its own game: the cost's mark, the pump
...
import { KELDON_OVERSEER_SCRIPT } from './keldonOverseer';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Keldon Overseer";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([KELDON_OVERSEER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Keldon Overseer"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([KELDON_OVERSEER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Keldon Overseer", () => {
  test("When this creature enters: the vocabulary resolves \"Gain control of target creature until end of turn. Untap that creature. It gains haste until end of turn.\"", () => {
