// `Bad Moon` - a layer-6 anthem: "Black creatures get +1/+1". A StaticDef in the shape of the
...
import { BAD_MOON } from '../../../data/fixtures/engineCards';
...
const PRINTED = printed(BAD_MOON, "Black creatures get +1/+1.");
const TEXT = PRINTED;

export const BAD_MOON_SCRIPT: CardScript = {
  oracleId: BAD_MOON.oracleId,
  name: BAD_MOON.name,
  statics: [
    {
      abilityId: 'anthem',
      ...
      appliesTo: (ctx, self, candidate, chars) => {
        ...
        if (!chars.colors.includes("B")) return false;
        return true;
      },
      modify: (chars) => {
