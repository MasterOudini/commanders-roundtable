// `Voracious Vermin` - every printed ability proven in its own game: the cost's mark, the pump
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Voracious Vermin", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([VORACIOUS_VERMIN_SCRIPT, RAT_TOKEN1E0205F2_SCRIPT]),
...
describe("Voracious Vermin", () => {
  test("When this creature enters: the vocabulary resolves \"Create a 1/1 black Rat creature token with \\\"This token can't block.\\\"\"", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1 + 1);
  });

  test("Whenever another creature you control dies: it gets 1 +1/+1 counter", () => {
