// `Archangel of Wrath` - every printed ability proven in its own game: the cost's mark, the pump
...
import { ARCHANGEL_OF_WRATH_SCRIPT } from './archangelOfWrath';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Archangel of Wrath";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Archangel of Wrath"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([ARCHANGEL_OF_WRATH_SCRIPT]),
  ...
  if (which === 1) {
    ...
    must(g.submit({ t: 'CastSpell', player: 'p1', card: self, kicked: 2, kickedWith: [0,1] }));
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'chooseTargets', 20_000);
    must(g.submit({ t: 'ChooseTargets', player: 'p1', targets: [{ kind: 'card', id: no }] }));
    settle(g);
    }
