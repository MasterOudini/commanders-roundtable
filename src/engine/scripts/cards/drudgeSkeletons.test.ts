// `Drudge Skeletons` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DRUDGE_SKELETONS_SCRIPT } from './drudgeSkeletons';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Drudge Skeletons";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Drudge Skeletons", "Lightning Bolt"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DRUDGE_SKELETONS_SCRIPT]),
  });
  holdEverywhere(g);
