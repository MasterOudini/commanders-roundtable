// `Mystic Skyfish` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MYSTIC_SKYFISH_SCRIPT } from './mysticSkyfish';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Mystic Skyfish";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([MYSTIC_SKYFISH_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Mystic Skyfish", "Divination"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MYSTIC_SKYFISH_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Mystic Skyfish", () => {
  test("Whenever you draw your second card each turn: it gains flying until end of turn", () => {
