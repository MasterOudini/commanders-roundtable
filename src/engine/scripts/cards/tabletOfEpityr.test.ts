// `Tablet of Epityr` - every printed ability proven in its own game: the cost's mark, the pump
...
import { TABLET_OF_EPITYR_SCRIPT } from './tabletOfEpityr';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Tablet of Epityr";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Tablet of Epityr", "Memnite", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([TABLET_OF_EPITYR_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Tablet of Epityr", () => {
  test("Whenever an artifact you control is put into a graveyard from the battlefield: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is declined", () => {
  ...
  test("Whenever an artifact you control is put into a graveyard from the battlefield: the vocabulary resolves \"You may pay {1}. If you do, you gain 1 life.\" - the price is paid", () => {
    const { g, life0, payL0_p1_0 } = armed(0);
    advanceUntil(g, (s) => s.priority.awaiting?.kind === 'payMana', 20_000);
    { const ask = g.state.priority.awaiting;
      expect(ask?.kind === 'payMana' && ask.player, 'the payer is the one the clause names').toBe("p1"); }
