// `Uruk-hai Berserker` - every printed ability proven in its own game: the cost's mark, the pump
...
import { URUK_HAI_BERSERKER_SCRIPT } from './urukHaiBerserker';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Uruk-hai Berserker";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Uruk-hai Berserker"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([URUK_HAI_BERSERKER_SCRIPT]),
