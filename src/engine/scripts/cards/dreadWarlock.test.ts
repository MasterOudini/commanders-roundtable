// `Dread Warlock` - every printed ability proven in its own game: the cost's mark, the pump
...
import { DREAD_WARLOCK_SCRIPT } from './dreadWarlock';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Dread Warlock";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Dread Warlock", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([DREAD_WARLOCK_SCRIPT]),
