// `Molten-Tail Masticore` - every printed ability proven in its own game: the cost's mark, the pump
...
import { MOLTEN_TAIL_MASTICORE_SCRIPT } from './moltenTailMasticore';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Molten-Tail Masticore";

type Armed = { g: Game; self: InstanceId; no: InstanceId; life0: number; hand0: number; board0: number; p2life0: number; gy0: number; lib0: number; priceL0_p1_0: InstanceId; exg0: InstanceId[] };
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Molten-Tail Masticore", "Infernal Grasp", "Grizzly Bears", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([MOLTEN_TAIL_MASTICORE_SCRIPT]),
  });
  holdEverywhere(g);
  ...
  if (which === 2) {
    ...
    must(g.submit({ t: 'CastSpell', player: 'p1', card: grasp, targets: [{ kind: 'card', id: self }] }));
    settle(g);
    }
  return { g, self, no, life0, hand0, board0, p2life0, gy0, lib0, priceL0_p1_0, exg0 };
}

describe("Molten-Tail Masticore", () => {
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you discard a card.\" - the price is declined", () => {
  ...
  test("At the beginning of your upkeep: the vocabulary resolves \"Sacrifice this creature unless you discard a card.\" - the price is paid", () => {
  ...
  test("{4}, Exile a creature card from your graveyard: it deals 4 damage to the declared creature", () => {
    const { g, no, exg0 } = armed(1);
    expect(g.state.cards[no]?.zone.kind).toBe('graveyard');
    for (const c of exg0) expect(g.state.cards[c]?.zone.kind).toBe('exile');
  });

  test("{2}: it regenerates from the Bolt", () => {
