// `Fanatical Firebrand` - every printed ability proven in its own game: the cost's mark, the pump
...
import { FANATICAL_FIREBRAND_SCRIPT } from './fanaticalFirebrand';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Fanatical Firebrand";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Fanatical Firebrand"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([FANATICAL_FIREBRAND_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Fanatical Firebrand", () => {
  test("{T}, Sacrifice this creature: it deals 1 damage to the declared creature", () => {
