// `Woolly Spider` - every printed ability proven in its own game: the cost's mark, the pump
...
import { WOOLLY_SPIDER_SCRIPT } from './woollySpider';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Woolly Spider";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([WOOLLY_SPIDER_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Woolly Spider"], ["Cyclops of One-Eyed Pass", "Aven Skirmisher"]],
    scripts: createRegistry([WOOLLY_SPIDER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Woolly Spider", () => {
  test("Whenever this creature blocks a creature with flying: it gets +0/+2 until end of turn", () => {
