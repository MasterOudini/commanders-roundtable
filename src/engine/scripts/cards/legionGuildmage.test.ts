// `Legion Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LEGION_GUILDMAGE_SCRIPT } from './legionGuildmage';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Legion Guildmage";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Legion Guildmage"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LEGION_GUILDMAGE_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Legion Guildmage", () => {
  test("{5}{R}, {T}: it deals 3 damage to each opponent", () => {
    const { g, self, p2life0 } = armed(0);
    expect(g.state.players.p2?.life).toBe(p2life0 - 3);
    expect(g.state.cards[self]?.tapped).toBe(true);
  });

  test("{2}{W}, {T}: the vocabulary resolves \"Tap another target creature.\"", () => {
