// `Clan Guildmage` - every printed ability proven in its own game: the cost's mark, the pump
...
import { CLAN_GUILDMAGE_SCRIPT } from './clanGuildmage';
import { advanceUntil, deps, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import { derive } from '../../derive';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Clan Guildmage";
...
function pt(g: Game, id: InstanceId): [number | null, number | null] {
  const d = deps(createRegistry([CLAN_GUILDMAGE_SCRIPT]));
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Clan Guildmage", "Forest"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([CLAN_GUILDMAGE_SCRIPT]),
...
describe("Clan Guildmage", () => {
  test("{1}{R}, {T}: the vocabulary resolves \"Target creature can't block this turn.\"", () => {
  ...
  test("{2}{G}, {T}: the vocabulary resolves \"Target land you control becomes a 4/4 Elemental creature with haste until end of turn. It's still a land.\"", () => {
