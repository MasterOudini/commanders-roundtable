// `Emmessi Tome` - every printed ability proven in its own game: the cost's mark, the pump
...
import { EMMESSI_TOME_SCRIPT } from './emmessiTome';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Emmessi Tome";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Emmessi Tome"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([EMMESSI_TOME_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Emmessi Tome", () => {
  test("{5}, {T}: the vocabulary resolves \"Draw two cards, then discard a card.\"", () => {
