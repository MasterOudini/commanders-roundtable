// `Lord Dregg, Insect Invader` - every printed ability proven in its own game: the cost's mark, the pump
...
import { LORD_DREGG_INSECT_INVADER_SCRIPT } from './lordDreggInsectInvader';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';
import { TOKEN_TABLE } from '../../../data/tokenTable';

const CARD = "Lord Dregg, Insect Invader";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Lord Dregg, Insect Invader", "Grizzly Bears"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([LORD_DREGG_INSECT_INVADER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Lord Dregg, Insect Invader", () => {
  test("Disappear — At the beginning of your end step: 1 token made", () => {
    const { g, board0 } = armed(0);
    expect(onBoard(g)).toBe(board0 + 1);
  });

  test("{3}{G}, Sacrifice a token: a card is drawn", () => {
