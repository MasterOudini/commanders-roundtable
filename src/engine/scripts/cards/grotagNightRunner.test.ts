// `Grotag Night-Runner` - every printed ability proven in its own game: the cost's mark, the pump
...
import { GROTAG_NIGHT_RUNNER_SCRIPT } from './grotagNightRunner';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Grotag Night-Runner";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Grotag Night-Runner"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([GROTAG_NIGHT_RUNNER_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Grotag Night-Runner", () => {
  test("Whenever this creature deals combat damage to a player: the vocabulary resolves \"Exile the top card of your library. You may play that card this turn.\"", () => {
    const { g } = armed(0);
