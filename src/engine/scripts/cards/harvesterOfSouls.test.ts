// `Harvester of Souls` - every printed ability proven in its own game: the cost's mark, the pump
...
import { HARVESTER_OF_SOULS_SCRIPT } from './harvesterOfSouls';
import { advanceUntil, holdEverywhere, must, put, startedGame } from '../../testing/harness';
import type { Game } from '../../game';
import type { InstanceId } from '../../types/ids';

const CARD = "Harvester of Souls";
...
function armed(which: number): Armed {
  const g = startedGame({
    players: 2,
    decks: [["Harvester of Souls", "Grizzly Bears", "Crimson Kobolds"], ["Cyclops of One-Eyed Pass"]],
    scripts: createRegistry([HARVESTER_OF_SOULS_SCRIPT]),
  });
  holdEverywhere(g);
...
describe("Harvester of Souls", () => {
  test("Whenever another nontoken creature dies: a card is drawn", () => {
