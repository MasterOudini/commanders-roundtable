...
const WANTED = [
  ...
  // D300 - the static seam's wave (table rows, token rows, hand modules, helper creatures).
  'Aang, Air Nomad',
  'Aggressive Mammoth',
  "Akroma's Devoted",
  'Anthem of Champions',
  ...
  'Crimson Kobolds',
  'Straw Soldiers',
  // D300 part C - the 16 the classifier offered beyond the layer-6 probe.
  'Sinew Sliver',
  'Might Sliver',
  ...
  'Aven Brigadier',
  'Myr Matrix',
  // D301 - the one-shot seam's wave (activated self / mass pumps) and its two proof spells.
  'Drifting Shade',
  'Fiery Hellhound',
  'Granite Gargoyle',
  ...
  'Charge',
  'Overrun',
  // D301 part B - the one-shot rows the classifier offered beyond the probe (activated + library triggers).
  'Ampryn Tactician',
  'Angel of the Dawn',
  'Ankle Shanker',
  'Atog',
  ...
  'Phantatog',
  'Thaumatog',
  // D302 - the triggered one-shot self-pumps (eleven library heads).
  'Aven Wind Mage',
  'Beast-Kin Ranger',
  'Benalish Veteran',
  'Borderland Marauder',
  ...
  'Fortress Cyclops',
  // D303 - the counter one-shots (self and mass +1/+1 / -1/-1 counters).
  'Ant-Man, Scott Lang',
  "Aron, Benalia's Ruin",
  'Boar-q-pine',
  "Cathars' Crusade",
  ...
  'Web-Warriors',
  // D303 part B - the counter one-shots the self-word fix and two heads opened.
  'Archangel of Thune',
  'Blood Researcher',
  'Bloodbond Vampire',
  ...
  'Unholy Officiant',
  'Verdant Automaton',
  // D303 part C - the mana-line lands and the draw-payload trigger.
  'Gavony Township',
  'Slith Strider',
  // D303 - two hand modules over dies triggers (a nontoken creature, a countered creature).
  'Abzan Ascendancy',
  'Skyclave Shadowcat',
  // D304 - the Aura seam test cards (an Enchant player Aura, an Enchant creature you control one).
  'Unholy Strength',
  'Curse of Shallow Graves',
  'Emblem of the Warmind',
  // D304 - the Aura rows (Enchant creature + the statics and restrictions of the enchanted creature).
  "Alexi's Cloak",
  'Arcane Flight',
  "Asha's Favor",
  'Aspect of Gorgon',
  ...
  "Zephid's Embrace",
  'Zephyr Net',
  // D304 part B - the Aura rows the widened shapes opened (enchant variants, the ETB one-shots).
  'Angelic Gift',
  ...
  "Wizard's Retort",
  'Rover Blades',
  // D313 - the trigger-head rows (seven more heads, six more payloads, optional triggers).
  'Unruly Mob',
  'Forsaken Drifters',
  ...
  'Study',
  'Billiard Room',
  // D317 - the trigger-head rows, part 4 (nine more heads, the enters head on every permanent word, investigate).
  'Invisible Stalker',
  'Crusader of Odric',
  'Nezumi Cutthroat',
  'Cloud Elemental',
  ...
  'Despondency',
  'Reach for the Sky',
  // D323 - the trigger-head rows, part 4 (nine more heads, the enters head on every permanent word, investigate).
  'Shadow Lance',
  'Firebreathing',
  'Claustrophobia',
  "Shiv's Embrace",
  ...
  // D324 - the trigger-head rows, part 4 (nine more heads, the enters head on every permanent word, investigate).
  "Akroma's Memorial",
  // D325 - the trigger-head rows, part 4 (nine more heads, the enters head on every permanent word, investigate).
  'Rakdos Signet',
  'Mana Confluence',
  'Lotus Petal',
  'Springleaf Drum',
  ...
  'Caged Zombie',
  // D348 - THE TURN RECORD: the cards whose activation asks what this turn has already done.
  // D349 - the derived cost fixtures: a sacrifice or a tap cost the engine charges, and the cards its suites pay it with.
  'Disrupting Scepter',
  ...
  "Nature's Lore",
  'Wood Elves',
  // D358 - the derived cost fixtures: a sacrifice or a tap cost the engine charges, and the cards its suites pay it with.
  'Arid Mesa',
  'Bloodstained Mire',
  'Deceptive Landscape',
  'Evolving Wilds',
  ...
  'Treasure Mage',
  'Mystical Tutor',
  // D360 - the derived cost fixtures: a sacrifice or a tap cost the engine charges, and the cards its suites pay it with.
  'Abjure',
  ...
  // D387 - the variable pump, four counts wider: attached, domain, blockers, Equipment and Squirrels.
  'Goblin Gaveleer',
  'Myr Adapter',
  'Elvish Berserker',
  ...
  'Jungle Wurm',
  'Rabid Elephant',
  // D388 - the conditional static: the wave the condition reader made rowable.
  'Paradise Druid',
  'Tuinvale Guide',
  ...
  'Guul Draz Vampire',
  'Snapsail Glider',
  // D388 - the helpers the conditional suites deal (the fixtures a condition is met with).
  'Aven Envoy',
  'Gran Pulse Ochu',
  ...
  'Triton Shorethief',
  'Watchwolf',
  // D389 - the look with a filter: the seam's own proof cards (two typed reveals, the singular graveyard form, two refusals, one trigger).
  'Arcane Infusion',
  'Commune with Nature',
  "Ral's Outburst",
  ...
  'Talas Lookout',
  // D389 - the conditional static: the wave the condition reader made rowable.
  'Eclipsed Boggart',
  'Augur of Bolas',
  'Staunch Crewmate',
  'Nessian Wanderer',
  ...
  // D394 - the can't-block restriction: the seam's three proof spells (a referent after damage, a counted target, a counted plural referent) - Mugging is the fuzz canary staple.
  'Mugging',
  'Panic Attack',
  'Wrap in Flames',
  // D394 - the can't-block restriction: the wave the flag made rowable (a creature that can't block this turn under a head, or after the damage a payload deals it).
  'Bola Warrior',
  'Chainwhip Cyclops',
  'Crossway Vampire',
  'Fervent Cathar',
  ...
  'War Squeak',
  'Pia Nalaar',
  // D395 - the animate family: the seam's proof cards (a manland, a keyrune-style artifact - the fuzz canary staple - and the targeted spell).
  'Treetop Village',
  'Guardian Idol',
  'Hydroform',
  // D395 - the animate family: the wave the base P/T on the until-end-of-turn list made rowable (manlands, keyrunes and idols that become creatures until end of turn).
  'Alloy Animist',
  'Atarka Monument',
  'Azorius Keyrune',
  'Boros Keyrune',
  ...
  // D399 - the temporary unblockable grant: the seam's proof spells (Infiltrate is the fuzz staple).
  'Infiltrate',
  'Artful Dodge',
  'Stealth Mission',
  ...
  'Fevered Strength',
  'Transluminant',
  // D403 - kicker: the seam's proof cards (Goblin Bushwhacker is the fuzz staple).
  'Into the Roil',
  'Goblin Bushwhacker',
  'Skitter of Lizards',
  ...
  // D414 - the another qualifier: the seam's proof cards (Trained Condor is the fuzz staple).
  'Trained Condor',
  'Hardened Escort',
  ...
  // D401 - the conditional statics the classifier now admits (a self static, an anthem or an attached static gated by as long as, the condition in the closed set the row maker reads): the wave the widened classifier offered.
  'Basking Capybara',
  'Bellowing Elk',
  'Brightspear Zealot',
  'Echo of Dusk',
  ...
  // D403 - kicker (CR 702.33): the rows whose enters head or enters-with line is gated on the kick the cast announced.
  'Academy Drake',
  'Ardent Soldier',
  'Baloth Gorger',
  'Benalish Emissary',
  ...
  // D407 - the linked exile (CR 610.3): the rows whose enters head exiles a target until the permanent leaves the battlefield, and their kin.
  'Meteor Golem',
  'Annex Sentry',
  'Banisher Priest',
  'Banishing Light',
  ...
  // D414 - the another qualifier: the rows the selector offered once another target read, and their kin.
  'Ulvenwald Tracker',
  'Aegis Automaton',
  'Blooming Stinger',
  'Bonebind Orator',
  ...
  'Werefox Bodyguard',
  'Sonic Screwdriver',
  // D414 - the fixtures the wave's suites deal that the generator DERIVED (a look's keep, a cost's fodder): in no names list until the sweep names them (D347/D370/D374).
  // D415 - the verb price at resolution: the rows the selector offered once you-may-verb and unless-you-verb read, and their kin.
  'Academy Raider',
  'Aegis Sculptor',
  'Avatar of Discord',
  ...
  // D419 - the board condition: the rows the selector offered once if-you-control read under the enters and refire heads, and their kin.
  'Acclaimed Contender',
  'Apothecary Geist',
  ...
  'Sunstar Chaplain',
  // D419 - the fixtures the wave's suites deal that the generator DERIVED (a look's keep, a cost's fodder): in no names list until the sweep names them (D347/D370/D374).
  'Agent of Stromgald',
  'Black Widow, Natasha Romanoff',
  // D420 - the ability word: the rows the selector offered once the classifier read past a true ability word (Landfall, Magecraft, Constellation, Heroic, ...), and their kin.
  'Akroan Conscriptor',
  'Archmage Emeritus',
  'Arena Athlete',
  'Captivating Unicorn',
  ...
  'Equilibrium Adept',
  'Thraben Doomsayer',
  // D420 - the fixtures the wave's suites deal that the generator DERIVED (a look's keep, a cost's fodder): in no names list until the sweep names them (D347/D370/D374).
  // D421 - the self subject: the rows the selector offered once the classifier read a payload's self subject (this creature deals, it gets, on this creature) as the vocabulary does, and their kin.
  ...
  'Plague Spitter',
  "Warleader's Call",
  // D424 - the classifier reads as the row maker reads: the rows the selector offered once the optional trigger, the row kinds, the attached keywords and the named self head read, and the 49 stale ledger rows drained.
  'Agility',
  'Aphetto Runecaster',
  'Apothecary Initiate',
  'Arrogant Outlaw',
  ...
  'Tenured Concocter',
  'Yuki-Onna',
  // D424 - the fixtures the wave's suites deal that the generator DERIVED (a look's keep, a cost's fodder): in no names list until the sweep names them (D347/D370/D374).
  ...
  'Scryb Sprites',
  // D425 - the sacrificed self, the you scope and the opponent-or-planeswalker noun: the rows the selector offered once those read.
  'Aeolipile',
  'Barbarian Lunatic',
  'Barbarian Ring',
  'Bear Trap',
  ...
  'Syndicate Guildmage',
  'Unyaro Bees',
  // D425 - the fixtures the wave's suites deal that the generator DERIVED (a look's keep, a cost's fodder): in no names list until the sweep names them (D347/D370/D374).
  // D426 - the conjunction: the rows the selector offered once X and Y read as two clauses, and the eight stale ledger rows drained.
  'Bitterbloom Bearer',
  'Bitterblossom',
  ...
  // D429 - the wide run is the offer: the rows the row maker reads over the whole leftover (the classifier mirror drift, and the subjectless life gain).
  ...
  'Battered Golem',
  'Retreat to Valakut',
  // D429 - the fixtures the wave's suites deal that the generator DERIVED (a look's keep, a cost's fodder): in no names list until the sweep names them (D347/D370/D374).
  // D430 - the counted static: the rows the row maker reads once a static pump or a P/T CDA takes a count.
  'Enigma Drake',
  'Fleshformer',
  'Storm-Kiln Artist',
  'Entomber Exarch',
  ...
  // D434 - the mill vocabulary: the rows the row maker reads once a mill could be aimed at a player, walk a player scope or be the controller own.
  ...
  // D436 - the graveyard-card target: the rows the row maker reads once a card in a graveyard could be exiled or put under its owner library.
  'Reito Sentinel',
  'Honored Heirloom',
  'Crypt Creeper',
  'Junktroller',
  ...
  // D439 - the upkeep prices: an echo, a cumulative upkeep in mana, one in life, and the two prices the engine refuses (the gate).
  'Shivan Raptor',
  'Illusionary Forces',
  'Gallowbraid',
  'Deepcavern Imp',
  'Braid of Fire',
  // D439 - the upkeep prices and the scoped life loss: the rows the row maker reads once echo and cumulative upkeep are the engine's and each player or opponent can lose N life.
  "Urza's Blueprints",
  'Ghitu Slinger',
  ...
  // D445 - backup: the seam's own fixtures (three keyword grants, a Flash above the line, a static below it).
  'Boon-Bringer Valkyrie',
  'Saiba Cryptomancer',
  'Gloomfang Mauler',
  'Chomping Kavu',
  'Sigiled Sentinel',
  // D446 - the during-your-turn statics: the rows the row maker reads once the turn condition is in the closed reader.
  'Blood Burglar',
  'Jousting Lance',
  ...
  // D461 - the enters-or-turned-face-up compound head: split into its two library heads by the row maker (D380 way), the rows that read.
  'Ponyback Brigade',
  'Rakish Scoundrel',
  'Efreet Weaponmaster',
  'Offender at Large',
  ...
  'Balduvian Bears',
  // D466 - the blocker predicate: can't be blocked by / except by <subject> as a CombatDef over the filter reader, the rows that read.
  'Taoist Mystic',
  'Juggernaut',
  ...
  // D474 - the token printings rowed like cards: the makers whose quoted tokens now run (Pests, Rats, Mercenaries, Devils, Pilots, Constructs).
  'Digsite Engineer',
  "Mage's Attendant",
  'Chocobo Racetrack',
  'Harried Spearguard',
  ...
  // D478 - the search row restored: the rows whose search the vocabulary reads and the generator answers (the Masques recruiters, the fetch lands, the land tutors).
  'Rampart Architect',
  'Yavimaya Granger',
  'Rathi Fiend',
  'Bog Glider',
  ...
  'Cateran Overlord',
  'Ramosian Sergeant',
  // D478 - the fixtures the wave's suites deal that the generator DERIVED (a look's keep, a cost's fodder): in no names list until the sweep names them (D347/D370/D374).
  'Children of Korlis',
  "Dragon's Eye Sentry",
  "Kataki, War's Wage",
  // D479 - the tribal target: the rows whose payload aims at a subtype (target Knight you control, another target Vampire you control).
  'Venerable Knight',
  ...
  'Voracious Vampire',
  'Poison-Blade Mentor',
  // D479 - the fixtures the wave's suites deal that the generator DERIVED (a look's keep, a cost's fodder): in no names list until the sweep names them (D347/D370/D374).
  // D479 - the tribal target, the second port: the activated abilities aiming at a subtype (Regenerate target Beast, Target Soldier creature gets +1/+0), rowable once the target read.
  'Mad Auntie',
  'Otepec Huntmaster',
  'Snarling Undorak',
  'Merfolk Sovereign',
  ...
  'Hearthcage Giant',
  // D479 - the fixtures the wave's suites deal that the generator DERIVED (a look's keep, a cost's fodder): in no names list until the sweep names them (D347/D370/D374).
  // D479 - the fixtures the second port's suites deal that the generator DERIVED (a subtype's vanilla creature): in no names list until the sweep names them (D347/D370/D374).
  'Dwarven Trader',
  'Felhide Minotaur',
  ...
  'Swell of Growth',
  'Lessons from Life',
  // D509 - the controller word on every target noun: the seam tests' spells (Assassin's Trophy is a fixture since D507).
  'Eerie Interlude',
  "Domri's Ambush",
  'Vandalblast',
  ...
  'Jace Reawakened',
  // D509 - the controller word on every target noun: the rows the select offered once the effect parser admitted it (Basri's Acolyte, Demolition Field, Aethershield Artificer ...).
  ...
  'Street Urchin',
  'Immobilizing Ink',
  // D519 - ENERGY: the rows the whole-leftover row maker rowed once the engine tracked energy counters (the enters gains, the Pay {E} activations, the attack prompts).
  'Bristling Hydra',
...
const WANTED_TOKENS = [
  ...
  // D300: the tokens the static wave's combined defs create (Splicers, Hammer of Purphoros).
  { name: 'Phyrexian Golem', set: 'tblc', cn: '38', key: 'PHYREXIAN_GOLEM_3_3_TOKEN' },
  { name: 'Golem', set: 'j13', cn: '9', key: 'GOLEM_3_3_ENCHANTMENT_TOKEN' },
  { name: 'Myr', set: 'tm3c', cn: '25', key: 'MYR_1_1_TOKEN' },
  // D304 - the Aura wave's ETB tokens (Cartouche of Solidarity, Robotics Mastery),
  ...
  // D424 - the tokens the wave's rows create (the port's token-pin step, missing until D424).
  { name: 'Elemental', set: 'tmma', cn: '11', key: 'ELEMENTAL_4_4_G_CREATURE_TOKEN' },
  { name: 'Bird', set: 'totj', cn: '7', key: 'BIRD_1_1_U_CREATURE_FLYING_TOKEN' },
  ...
  // D474 - the token printings rowed like cards: the quoted descriptions' printings the engine did not run (Pests, Rats, Mercenaries, Devils, Pilots ...), each a fixture by set and number.
  { name: 'Mercenary', set: 'totj', cn: '10', key: 'MERCENARY_5F04607F_TOKEN' },
  { name: 'Rat', set: 'twoe', cn: '7', key: 'RAT_1E0205F2_TOKEN' },
  { name: 'Pest', set: 'plst', cn: 'TSTX-5', key: 'PEST_1AE25997_TOKEN' },
  { name: 'Devil', set: 'tmid', cn: '6', key: 'DEVIL_14F9C505_TOKEN' },
  { name: 'Pilot', set: 'tdft', cn: '1', key: 'PILOT_8672D795_TOKEN' },
  { name: 'Bird', set: 'tfin', cn: '16', key: 'BIRD_1FBC471D_TOKEN' },
  { name: 'Spirit', set: 'ttla', cn: '3', key: 'SPIRIT_F59EBA51_TOKEN' },
  { name: 'Wizard', set: 'tfin', cn: '35', key: 'WIZARD_04AE24BF_TOKEN' },
  { name: 'Construct', set: 'tc21', cn: '23', key: 'CONSTRUCT_008D3B2E_TOKEN' },
  { name: 'Pest', set: 'tsos', cn: '9', key: 'PEST_40B22872_TOKEN' },
  { name: 'Fish', set: 'tsnc', cn: '4', key: 'FISH_3B2E726B_TOKEN' },
  { name: 'Drone', set: 'teoe', cn: '3', key: 'DRONE_3FCF8950_TOKEN' },
  { name: 'Pilot', set: 'tneo', cn: '1', key: 'PILOT_BE84F259_TOKEN' },
  { name: 'Satyr', set: 'tcmm', cn: '29', key: 'SATYR_49BDAE00_TOKEN' },
  { name: 'Faerie', set: 'twoe', cn: '5', key: 'FAERIE_0F9A993F_TOKEN' },
  { name: 'Fungus', set: 'tlci', cn: '7', key: 'FUNGUS_73FF66E3_TOKEN' },
  { name: 'Skeleton', set: 'ta25', cn: '8', key: 'SKELETON_1ABB1AB0_TOKEN' },
  { name: 'Alien', set: 'tmsh', cn: '11', key: 'ALIEN_ECA87CBB_TOKEN' },
  { name: 'Assassin', set: 'twar', cn: '6', key: 'ASSASSIN_B7E997AC_TOKEN' },
  { name: 'Assassin', set: 'tc19', cn: '9', key: 'ASSASSIN_4401437C_TOKEN' },
  { name: 'Avatar', set: 'tstx', cn: '1', key: 'AVATAR_94A50ACD_TOKEN' },
  { name: 'Beast', set: 'tdsk', cn: '3', key: 'BEAST_F43E18D8_TOKEN' },
  { name: 'Bird', set: 'tltc', cn: '1', key: 'BIRD_561341F5_TOKEN' },
  { name: 'Bird', set: 'tdmu', cn: '6', key: 'BIRD_8DCE95D5_TOKEN' },
  { name: 'Bird', set: 'tmid', cn: '3', key: 'BIRD_A0F7B2F0_TOKEN' },
  { name: 'Boar', set: 'teld', cn: '9', key: 'BOAR_365B2234_TOKEN' },
  { name: 'Cleric', set: 'tgk2', cn: '4', key: 'CLERIC_88BB40E8_TOKEN' },
  { name: 'Dragon', set: 'tm14', cn: '6', key: 'DRAGON_0EFAA5B5_TOKEN' },
  { name: 'Dragon Spirit', set: 'tafc', cn: '9', key: 'DRAGON_SPIRIT_B997ACE9_TOKEN' },
  { name: 'Drone', set: 'tone', cn: '8', key: 'DRONE_CFACAC5F_TOKEN' },
  { name: 'Elemental', set: 'tncc', cn: '18', key: 'ELEMENTAL_512FEDF8_TOKEN' },
  { name: 'Goblin', set: 'tclb', cn: '34', key: 'GOBLIN_1DB51576_TOKEN' },
  { name: 'Goblin Shaman', set: 'tneo', cn: '8', key: 'GOBLIN_SHAMAN_0D9461C3_TOKEN' },
  { name: 'Horse', set: 'twho', cn: '4', key: 'HORSE_5E29C393_TOKEN' },
  { name: 'Human Knight', set: 'tpip', cn: '2', key: 'HUMAN_KNIGHT_546BDA77_TOKEN' },
  { name: 'Illusion', set: 'tafc', cn: '3', key: 'ILLUSION_42A6BC0B_TOKEN' },
  { name: 'Illusion', set: 'txln', cn: '2', key: 'ILLUSION_A10729A5_TOKEN' },
  { name: 'Illusion', set: 'trna', cn: '2', key: 'ILLUSION_CCC3D8DA_TOKEN' },
  { name: 'Imp', set: 'tmkm', cn: '7', key: 'IMP_47A1385B_TOKEN' },
  { name: 'Nightmare', set: 'tthb', cn: '12', key: 'NIGHTMARE_C528581F_TOKEN' },
  { name: 'Nightmare', set: 'twoe', cn: '6', key: 'NIGHTMARE_858428EE_TOKEN' },
  { name: 'Ninja', set: 'tneo', cn: '4', key: 'NINJA_14EB629E_TOKEN' },
  { name: 'Ooze', set: 'tpca', cn: '17', key: 'OOZE_18B9C83D_TOKEN' },
  { name: 'Pirate', set: 'twoc', cn: '12', key: 'PIRATE_D5F108E7_TOKEN' },
  { name: 'Robot', set: 't40k', cn: '23', key: 'ROBOT_319AAAAE_TOKEN' },
  { name: 'Spirit', set: 'tbro', cn: '1', key: 'SPIRIT_349E3241_TOKEN' },
  ...
  // D478 - the tokens the wave's rows create (the port's token-pin step, missing until D478).
  { name: 'Wall', set: 'ttdc', cn: '7', key: 'WALL_1_3_W_CREATURE_DEFENDER_TOKEN' },
...
async function main() {
  ...
  // D335 - the records go to engineCards.json (a parse for the loader, not a
  // 159,000-line object literal for the compiler); engineCards.ts is the typed
  // index that gives each record its export name. Same names, same order.
  const data = {};
  const exported = [];
  for (const name of WANTED) {
    const id = constName(name);
    exported.push(id);
    data[id] = found.get(name);
  }
  for (const t of WANTED_TOKENS) {
    exported.push(t.key);
    data[t.key] = foundTokens.get(t.key);
  }
  const dir = path.join(__dirname, '..', 'src', 'data', 'fixtures');
  fs.writeFileSync(path.join(dir, 'engineCards.json'), JSON.stringify(data, null, 2) + '\n', 'utf8');
  ...
  lines.push('// exercise the same text the app will. The records live in engineCards.json');
  lines.push('// beside this file, read as a string and parsed once (D335 measured a suite');
  lines.push('// importing the 159,000-line object literal at 1.8 s, the same records as a');
  lines.push('// JSON module at 1.6 s, and this at 0.4 s); the index gives each record its');
  lines.push('// typed name. Hand-editing a value silently turns a rules test into');
  lines.push('// a test of the edit — the D15b failure mode.');
  ...
  lines.push("import raw from './engineCards.json?raw';");
  lines.push('');
  lines.push('const F = JSON.parse(raw) as Readonly<Record<string, CardData>>;');
  lines.push('');
  for (const id of exported) lines.push(`export const ${id}: CardData = F.${id} as CardData;`);
  lines.push('');
  ...
  const out = path.join(dir, 'engineCards.ts');
  fs.writeFileSync(out, lines.join('\n'), 'utf8');
  console.log(`Wrote ${exported.length} cards → ${path.relative(process.cwd(), out)} (+ engineCards.json)`);
}
