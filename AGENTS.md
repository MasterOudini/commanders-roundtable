...
- [x] **M6.4cv — Nineteen landed: "damage" is not "combat damage"
      (2026-08-23):** **3,535 of 31,692 Commander-legal cards now execute
      completely, up from 3,516.** `SHIPPED_SCRIPTS` 1,619 → 1,638; ledger
      580 (+6, ONE new class). ZERO new tokens or support bodies. **Sliver
      Queen reaches 3,486 from 78 legendaries.** Decisions in **D259**.
      ⚠️⚠️ **The headline is a reading the arc almost copied wrong.** Thieving
      Otter and Thieving Magpie print "deals DAMAGE", not "combat damage" —
      Scroll Thief (D244) says the latter and watches `CombatDamageDealt`
      alone. Copying that would have under-fired on every ping with nothing
      failing. **MEASURED: the two events are DISJOINT** (`loop.ts` emits
      combat, `effects.ts` emits the rest, neither emits the other), so two
      defs cover the card safely. The noncombat half is proven by COMPOSITION
      — casting the shipped `Soul's Fire` with the Otter as its source.
      ⚠️⚠️ **A probe pulled a card before a line of it was written:**
      Thistledown Players' "target nonland permanent" parses confident with
      `unenforced: ['nonland']` — the aim would untap a LAND the card
      forbids (D161's Angelic Page, verbatim). NEW class **'negated-type
      target qualifier'**; only the colour directions had been probed.
      ⚠️ **A third harness fact: `settle()` RETURNS BEFORE COMBAT DAMAGE.**
      Declaring attackers leaves the stack empty, so both connect tests read
      zero draws against correct scripts — the walk must reach
      `postcombatMain` (Scroll Thief's test already does).
      **Also:** Three Tree Scribe (the FIRST leaves-WITHOUT-DYING watcher —
      the destination filter is the whole card, and its own death pays
      nothing); Thran Vigil (graveyard-exit gated on "during your turn");
      Théoden (self-or-Human pair granting double strike); Thoughtweft Gambit;
      Their Name Is Death; Thermokarst; Thraxodemon; **The Surgical Bay — the
      FIFTH Sphere, generated by growing D258's table.**
      ⚠️ Théoden's const is `TH_ODEN_KING_OF_ROHAN` — the diacritic becomes
      an UNDERSCORE, not dropped (D222's Lothlórien shape).
      Fixtures 1,877 (89 tokens) · botPool creature 1,866 / instant 641 /
      sorcery 525 / enchantment 59 / land 326 · ladder [1518, 1617, 3410,
      5324, 6536] · batch.json 600.
      **Verified: `verify.cjs --full` — ALL FIVE GATES: 1,718
      files, 8,948 passed / 10 skipped · 500-seed gate
      747.1 s · build clean · probe 124/124 · battery 130/130.**
      ⚠️ **Reportables** (D259): sweep the shipped connect-triggers for
      "deals damage" without COMBAT — each is a two-def card; up-to-N's
...
- [x] **M6.4dc — Twenty-four landed: the first up-to-N card, and a record tied
      ...
      **Verified: `verify.cjs --full` — ALL FIVE GATES: 1,845 files, 9,621 passed / 10 skipped ·
      ...
      stay the named engine jobs; prior items stand.
- [x] **M6.4dd — Twenty landed: a batch that needed no probe at all
      (2026-08-23):** **3,682 of 31,692 Commander-legal cards now execute
      completely, up from 3,662.** `SHIPPED_SCRIPTS` 1,765 → 1,785; ledger 633
      (+5, ZERO new classes). ZERO new token pins, ZERO new support bodies.
      **Sliver Queen reaches 3,633 from 82 legendaries.** Decisions in **D267**.
      ⚠️ **ZERO PROBES OWED, and that is the finding.** Every shape already had
      a SHIPPED precedent, grep-checked before a line was written: 'target
      player or planeswalker' (Chandra's Magmutt); 'target creature an opponent
      controls' (Ambuscade); a TARGETED end-step trigger (Luminarch Aspirant,
      Blood Mist — `StepBegan` + `parseTargetClauses`, and `Step` already
      carries 'end'); 'Sacrifice a SUBTYPE' (Arms Dealer, Deadapult, Deranged
      Outcast, Bog Naughty); Domain (Allied Strategies); 'all other permanents
      with the same name' (Bile Blight, Echoing Calm); 'whenever an opponent
      casts a COLOUR spell' (Insight). **A batch needing no probe is what a
      mature aim layer looks like on a page of ordinary cards** — worth
      recording precisely because the previous six batches each turned up
      something.
      ⚠️ **The damage-and-gain family takes a FIFTH member.** Warleader's Helix
      (4/any) joins Vampiric Feast, Vampiric Touch (D265), Vicious Hunger and
      Void Helix (D266) — GENERATED by growing D266's generator with one row
      plus a MANA column (the first member not castable off one colour).
      Eighth time the idiom has paid.
      ⚠️ **`land.cjs` caught the apostrophe trap BEFORE it cost a gate.** It
      derives the export from the FILE name, so `warleadersHelix.ts` must
      export `WARLEADERS_HELIX_SCRIPT` while the fixture const is
      `WARLEADER_S_HELIX`. The generator had named the export after the
      fixture; land.cjs refused it BY NAME. ⚠️ The fix went into the
      **GENERATOR** (a separate EXPORT column) and the file was regenerated,
      never hand-edited — D215's trap, now paid structurally.
      ⚠️ **Two more measurements.** Derived `power`/`toughness` are
      **`number | null`** (a non-creature has neither), so a helper annotating
      them `number` does not compile; two did. And **the bot pool has a SECOND
      enchantment pin site** (`botPool.node.test.ts:286`) — Warmth takes it
      63 → 64; a label-anchored counted repin found it where a bare-number
      sweep would have moved one of the two and left the other.
      **Also:** Wake of Destruction (Echoing Calm's NAME match one type over —
      my own Islands go with theirs, name read BEFORE the moves, fourth
      outing); Voltaic Servant (Voltaic Key's untap on a targeted end-step
      trigger); Wall of Mulch (it may eat ITSELF — 'a Wall' is not 'another' —
      with batch-mate Wall of Runes as the other Wall); Wandering Stream (TYPES
      not lands: two Islands are one type); War Report ('plus' not 'or' — an
      artifact creature counts TWICE); Volcanic Geyser (X=0 deals NOTHING, not
      a zero-amount entry); Voyaging Satyr + Vulshok Sorcerer (EXACT-TEXT TWINS
      of shipped Blossom Dryad and Cunning Sparkmage, written to the same shape
      so a twin sweep finds a pair); Volcanic Rambler (the planeswalker half
      asserts `damage`, never loyalty — D257); Voracious Varmint; Vulshok
      Heartstoker; Wakandan Drone Flock; Wakandan Shield Guard; Wanderbrine
      Preacher; Wanderwine Distracter; War Chariot; Warmth.
      ⚠️ **All 69 tests green FIRST run**, the fourth batch running.
      ⚠️ **Ledger hygiene, recorded in the ledger itself:** `ctx.random` and
      `ctx.random stub` are ONE gap under TWO names (Mnemonic Nexus and
      Reminisce under the first, Sway of the Stars and Timetwister under the
      second). Warp World joins the larger. Merge them next time that file is
      opened for its own sake.
      Fixtures 2,029 (1,937 by name + 92 tokens) · botPool artifact 131 /
      creature 1,928 / enchantment 64 / instant 666 / land 337 / sorcery 556 ·
      ladder [1371, 1470, 3263, 5177, 6389] · batch.json 400.
      **Verified: `verify.cjs --full` — ALL FIVE GATES: 1,865 files, 9,730 passed / 10 skipped ·
      ...
      ⚠️ **Reportables** (D267): unchanged from D266 and none of them moved,
      which is itself the report — up-to-N is PARTLY landable so the chooser's
      scope is the non-parsing forms only; the keyword-qualifier silent drop
      (three witnesses) stays the cheapest high-value aim-layer repair; the
      subtype noun list still needs a field where the mixed one does not; the
      undying carrier closes two ledger entries; `ctx.random` (now under two
      ledger names that want merging) and 'a resolve cannot see its own
      effects' stay the named engine jobs; prior items stand.
- [x] **M6.4de — Nineteen landed: up-to-N works in the plural, and one
      qualifier kind is enforced (2026-08-23):** **3,701 of 31,692
      Commander-legal cards now execute completely, up from 3,682.**
      `SHIPPED_SCRIPTS` 1,785 → 1,804; ledger 639 (+6, ZERO new classes). ONE
      new token pin, ZERO new support bodies. **Sliver Queen reaches 3,652 from
      82 legendaries.** Decisions in **D268**.
      ⚠️⚠️ **UP-TO-N WORKS IN THE PLURAL.** `Waterwhirl`'s "up to **two** target
      creatures" probes **min 0 / max 2**, confident, `unenforced: []`. D266
      measured the singular; this is the more valuable half, since most of the
      fifty-entry class is plural. Together they settle D262's question: **the
      class is a set of PARSE FAILURES, not one missing feature**, and the
      chooser is owed only for the forms that genuinely fail to parse. The test
      proves all three legal answers — two, one, and none.
      ⚠️⚠️ **The aim layer is weak on EXACTLY ONE KIND of qualifier.**
      `Whalebone Glider`'s "power 3 or less" returns a STRUCTURED
      `numeric: {attr:'power', cmp:'atMost', value:3}` with `unenforced: []` —
      disclosed AND enforced, so a 6/6 is refused at the aim. A KEYWORD
      qualifier is silently dropped (D261/D262/D265). That narrows D265's
      "cheapest high-value repair" from *all qualifiers* to *the keyword ones*.
      ⚠️⚠️ **`TOKEN_TABLE` and `WANTED_TOKENS` ARE DIFFERENT LISTS, and only the
      second is a pin.** This batch was classified "zero new tokens" on a grep
      of `tokenTable.ts` — the wrong file. TOKEN_TABLE gives a SCRIPT the
      printing to create; WANTED_TOKENS is what the TEST ORACLE knows. `Goblin
      Rogue` was in the first and not the second, so Weirding Shaman made two
      tokens `nameOf()` could not see. Fixed by looking the printing UP from
      the id TOKEN_TABLE already held (`tznc 5`). **Grepping tokenTable.ts is
      NOT a check that a token is pinned.**
      ⚠️ **A false diagnosis, corrected:** the first reading was "an activated
      ability does not resolve once its source has left" — plausible, since the
      Shaman may eat itself. Wrong: sacrificing ANOTHER Goblin failed
      identically. Isolate the variable before naming the cause.
      ⚠️ **THREE family cases — the richest batch yet for Phase 2.6.** Wary
      Thespian and Wary Watchdog have **byte-identical oracle text** on two ids
      in the same batch (a true exact-text twin pair, generated from one base);
      Watchful Giant twins D266's Voice of the Provinces; and **Waterfront
      District is the TWENTIETH member of a dual-sac-land family whose first
      nineteen were each hand-written** — generated, with the generator KEPT so
      the twenty-first is a table row, not a file.
      ⚠️ That generator's TEST was wrong first run and the engine named it: the
      land ENTERS TAPPED, so its `{T}` cost cannot be paid on arrival
      (`alreadyTapped`). Fixed IN THE GENERATOR, on the shipped family's own
      test shape (enter from the graveyard, assert tapped, untap, activate;
      draws counted from the LOG, sidestepping D232's put() trap).
      **Also:** Warpath (Fight to the Death's combat set with damage — an
      UNBLOCKED attacker takes nothing); Warren Soultrader (a TWO-part cost
      with NO mana at all, and never itself); Warteye Witch (self-or-other
      death in ONE def, looksBack carrying the self half); Wave Goodbye (each
      creature WITHOUT a +1/+1 counter — a counter is the only save); Weight of
      Spires (the TARGET controller's nonbasics, the two seats given different
      counts so both readings cannot pass); Welding Sparks (3 PLUS my
      artifacts, floor 3); Warped Physique (X read at resolution, spell already
      gone from hand); Watcher in the Mist; Watchful Automaton; Waterwind
      Scout; Weaponize the Monsters; Welkin Guide.
      ⚠️ **All 69 tests green, the fifth batch running** — two fixed mid-port,
      both because the engine reported the problem by name.
      Fixtures 2,049 (1,956 by name + 93 tokens) · botPool artifact 132 /
      creature 1,938 / enchantment 65 / instant 671 / land 338 / sorcery 557 ·
      ladder [1352, 1451, 3244, 5158, 6370] · batch.json 375.
      **Verified: `verify.cjs --full` — ALL FIVE GATES: 1,884 files, 9,837 passed / 10 skipped ·
      ...
      ⚠️ **Reportables** (D268): up-to-N's chooser is owed only for non-parsing
      forms (singular AND plural now measured working); the keyword-qualifier
      drop is now known to be the ONLY weak qualifier kind, a smaller and
      better-specified repair than D265 could state; the subtype noun list
      still needs a field where the mixed one does not; the undying carrier
      closes two ledger entries; `ctx.random` (two ledger names wanting a
      merge, D267) and 'a resolve cannot see its own effects' stay the named
...
- [x] **M6.4dk — Eight landed: the first optional trigger and the first
      graveyard watcher, both measured (2026-09-02):** **3,783 of 31,692
      Commander-legal cards now execute completely, up from 3,775.**
      `SHIPPED_SCRIPTS` 1,878 → 1,886; ledger 707 (+17, THREE new classes).
      ZERO new token pins, ZERO new support bodies. **Sliver Queen reaches
      3,734 from 84 legendaries.** Decisions in **D274**.
      ⚠️⚠️ **CREEPING CHILL OPENS TWO DOORS, MEASURED:** the FIRST script def
      with `optional: true` (D128's `optionalTrigger` prompt — accept exiles
      and chills, decline leaves it, a hand discard asks nothing) and the
      FIRST watcher active in the GRAVEYARD zone (post-event state, no
      `looksBack`). The accounting also accepted a SPELL def on line 1 beside
      a TRIGGER def on line 2. **Every "when X, you may Y" trigger in the
      ledger's prompt classes is worth re-reading against `optional: true`.**
      ⚠️ **Positional is exact when the re-check enforces it:** `Cunning
      Strike` reads its creature / player-or-planeswalker specs in SPEC ORDER
      — a planeswalker is a card too — and CR 608.2b's re-check (D255/D271)
      fizzles any other order before the resolve runs.
      **The guildmage generator grew by a row** (`Dega Disciple`; the three
      shipped members regenerated byte-identical — D258's growth path
      proven). **Also:** Dispatch (tap; Metalcraft exile at ≥3 artifacts);
      Dream Fracture (counter; ITS CONTROLLER draws, then I do); Drogskol
      Cavalry (the Spirit it makes is "another Spirit entering": a token AND
      2 life per activation); Disciple of Tevesh Szat (−1/−1 on the tap;
      −6/−6 for six mana, the tap and itself); Cremate (ANY graveyard).
      ⚠️ **One tsc catch, then every suite green on the first run** (third
      batch in a row; 8 files / 32 tests): `put()` refuses the library zone —
      stage a mill hand → top of library → graveyard by manual moves.
      **Refused 17, three new classes:** `graveyard-order condition` (Death
      Spark — no zone tracks ORDER), `freerunning mechanic` (Distract the
      Guards, Eagle Vision), `mayhem mechanic` (Electro's Bolt); plus
      Crashing Footfalls, Crippling Chill, Crookclaw Elder, Crystal Spray,
      Dark Dabbling, Dash Hopes, Deceive the Messenger, Devastation Tide,
      Disrupting Shoal, Divine Congregation, Dizzy Spell, Earthbrawn,
      Elemental Masterpiece.
      ⚠️ **The pool is now 225.**
      Fixtures 2,136 (2,031 by name + 98 tokens) · botPool artifact 137 /
      creature 1,974 / enchantment 69 / instant 690 / land 340 / sorcery 573 ·
      ladder [1270, 1369, 3162, 5076, 6288] · batch.json 225.
      **Verified: `verify.cjs --full` — ALL FIVE GATES: 1,966 files, 10,301 passed / 10 skipped · 500-seed gate 726.1 s ·
      build clean · probe 124/124 · battery 130/130.**
      ⚠️ **Reportables** (D274): `optional: true` is a LIVE seam — re-read
      the `script-raised prompt` class (76 entries) for "you may" TRIGGERS
...
- [x] **M6.4em — THE ONE-SHOT SEAM, part 2: the triggered self-pumps through
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 2,924 files, 15,209 passed / 11 skipped ·
      ...
      split; the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4en — THE COUNTER SEAM, part 1: +1/+1 and -1/-1 counters on the
      permanent itself or on each creature, behind a cost or a library head;
      two heads more; 65 scripts (2026-09-04):** **5,014 of
      31,692 Commander-legal cards now execute completely, up from 4,949
      (+65).** `SHIPPED_SCRIPTS` 2,897; ledger 870
      (+6: four or-typed sacrifice costs - Bartolome del Presidio, Hammerhead, Maggia Boss, Malevolent Noble, Dreadmalkin (Sacrifice another creature or artifact / or planeswalker, an artifact or another creature - no row charges an either-type sacrifice); an exile-from-graveyard cost (Tome Shredder); a conditional enters-tapped replacement (Abandoned Air Temple, enters tapped unless you control a basic land)). Fixtures 3,220 (3,106 by name + 107 tokens: the wave's 65 cards less the one already a fixture). ZERO token pins, ZERO
      support bodies. **Select pool 0.** Decisions in **D303**.
      ⚠️ **Measured first:** the ladder's `effect:counter` rung is +1,912;
      the probe: 170 cards carry a counter line as their ONLY blocker (26,743 keep another; 3,309 print one at all) - self counters under a trigger 126, behind a cost 28, mass counters under a trigger 8, spell mass counters 5, enters-with 3; by shape over the 3,309, self +1/+1 counters lead (trigger 194, activated 35), then charge 33, oil 12, mass +1/+1 on each creature you control 16, loyalty 5, the rest single digits; by head, you gain life 21 and deals combat damage to a player 16 lead the tail - both entered the library this decision.
      ⚠️ **The seam:** `primitives.ts` `oneShotCounterShape` (an activated
      effect or a library-head payload putting one to five +1/+1 / -1/-1
      counters on this permanent or on each creature - its controller's,
      each other, all; "it" is this permanent only under a self head) and
      two library heads more for the pump and counter shapes alike:
      "Whenever ~ deals combat damage to a player" (self head,
      `CombatDamageDealt`) and "Whenever you gain life" (other head,
      `LifeChanged`). `d301/gen-oneshot2.cjs` kinds `selfCounter` /
      `massCounter` (`CountersChanged`), a draw payload under a head, three
      rules: a mana line is no row but keeps its ability index (Blighted
      Cataract's precedent); a double striker's combat-damage trigger fires
      twice (Markov Blademaster); a -1/-1 mass counter reads off Grizzly
      Bears, not the 2/1 Eel it kills (Soul Snuffers). Two hand modules over
      dies watchers: Abzan Ascendancy (nontoken creature of yours -> Spirit),
      Skyclave Shadowcat (a countered creature of yours -> draw; `looksBack`).
      ⚠️ **The gap the first cut missed, again:** the activated branch read
      "on ~" only - `selfRef` never lowercases the "this creature" a cost's
      effect prints; the first pool read 24, the fixed shape 52 more (D302's
      lesson, one colon over). NOT this decision: "enters with N counters" (a
      ReplacementDef - none ships), the tail heads, other counter kinds, the
      per-item counter on an entering object. The fuzz list: the testing
      `Ajani's Pridemate` (D130) left it - the shipped row is the same
      oracleId (gate 154 caught it; 155 green). Refused by name: +6: four or-typed sacrifice costs - Bartolome del Presidio, Hammerhead, Maggia Boss, Malevolent Noble, Dreadmalkin (Sacrifice another creature or artifact / or planeswalker, an artifact or another creature - no row charges an either-type sacrifice); an exile-from-graveyard cost (Tome Shredder); a conditional enters-tapped replacement (Abandoned Air Temple, enters tapped unless you control a basic land).
      No spell def collided: a counter one-shot is no spell's text, so nothing was retired by D187's rule. Report `effect:auto` 3,572 → 3,572, `withUnenforced`
      237 → 237.
      **Landed:** 65 scripts and no auto flip (a counter one-shot needs a script). The wave in three parts and two hands: part A 19 rows off the first pool of 24, part B 42 off the 52 the printed self-words and the two heads opened, part C 2 (Gavony Township, Slith Strider - the mana line kept its index, a draw payload under a head), and two hand modules over dies watchers (Abzan Ascendancy, Skyclave Shadowcat). By head: behind a cost 24 (Ant-Man, Scott Lang, Aron, Benalia's Ruin, Jenara, Asura of War, Bloodflow Connoisseur ...); you gaining life 13 (Archangel of Thune, Ajani's Pridemate, Blood Researcher, Bloodbond Vampire ...); combat damage to a player 10 (Borborygmos, Erdwal Ripper, Falkenrath Marauders, Heirs of Stromkirk ...); a noncreature spell cast 6 (Boar-q-pine, Deeproot Champion, Pyroceratops, Spellgorger Weird ...); entering 6 (Katara, Heroic Healer, Ridgescale Tusker, Soul Snuffers, Supply Runners ...); an instant or sorcery cast 2 (Electrostatic Infantry, Pyre Hound); a creature entering 1 (Cathars' Crusade); attacking 1 (Leatherhead, Iron Gator); becoming blocked 1 (Slith Strider). Six refused by name. Tests: oneShotCounters.test.ts +
      oneShotTriggers.test.ts (the two heads) + 65 suites.
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 2,990 files, 15,482 passing / 11 skipped ·
      ...
      ⚠️ **Reportables** (D303): the tail heads by count ('you draw your second card each turn' 9, 'you cast a creature spell', 'another creature dies' ...) - a library entry each when its count earns it; 'enters with N +1/+1 counters' (a ReplacementDef); the per-item counter on the entering object; the vocabulary's massCounter for spells (5); the or-typed sacrifice cost (four names wait on it); the modal seam (42); the "another"
      split; the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4eo — THE AURA SEAM, part 1: the Enchant line is the engine's
      own, CR 704.5m's other half, the enchanted creature's statics and
      restrictions as derived rows; 112 scripts (2026-09-04):**
      **5,126 of 31,692 Commander-legal cards now execute completely, up
      from 5,014 (+112).** `SHIPPED_SCRIPTS` 3,009; ledger 900
      (+30: the Auras the pool offered whose other line is outside the row shapes - the Aura ETB and enchanted-dies heads with payloads the generator does not read yet (a counter on a target, a tap, discards, scry, surveil, graveyard tokens), the put-into-a-graveyard and attacks-or-blocks heads, two sacrifice-to-draw activations, an anthem on an Aura (Emblem of the Warmind), Enchant artifact / creature or Vehicle / Forest or Plains / creature with another Aura attached, one multi-face). Fixtures 3,335 (3,219 by name + 109 tokens: the wave's 112 Auras, the seam's four test cards (two already fixtures), and two token pins for the Aura ETB tokens - a vigilant Warrior and a flying Robot, the fuzz gate's own catch). Two token pins (the fuzz gate's own
      catch); three support bodies, one classifier shape. **Select pool 0.** Decisions
      in **D304**.
      ⚠️ **Measured first:** three gate-free probes - the unread sentences (the
      vocabulary's tail is long: the top 300 sentences free 2,018 cards, 14,794
      blocked cards carry a line no rule fixes), the structural families (an
      Aura's Enchant line 1,185 cards, "Enchant creature" the single largest
      structural line at 871), the Auras: 1,214 Commander-legal Auras, every one incomplete because the accounting refused the Enchant line itself; by their blocker lines, Enchant creature 871, an enchanted-creature P/T static 158, an Aura ETB 143, a P/T-and-keyword static 97, a keyword grant 55, Enchant creature you control 54, does-not-untap 48, Enchant player 42, enchanted-dies 41, you-control-enchanted 25, can-t-attack-or-block 20; 129 Auras whose every other line is the Enchant line plus one of the row statics, 134 with the restrictions.
      ⚠️ **The seam:** the engine already ran Enchant (the cast aims by the
      parsed spec, CR 303.4g attaches, SBA 704.5m drops) and the accounting
      refused the line. `engineComplete.ts` `enchantSpecRuns` /
      `enchantLineRuns` (kinds read, nothing unenforced, never a player);
      `targets.ts` `specAdmits` (the spec's half of `targetAllowed`, no
      hexproof / shroud / protection - they restrict targeting, not an
      attachment) + `candidateFor` (exported one-card body); `sba.ts` CR
      704.5m's other half (the Aura's own spec asked of its host every pass,
      plus protection); `primitives.ts` the enforced Enchant line is
      scriptable + `auraLineShape` (gets +N/+N [and has KW], has KW, can't
      attack / block). `d304/gen-aura.cjs`: StaticDefs / a CombatDef whose one
      candidate is the attached creature; suites cast on Grizzly Bears. The
      first CombatDefs to ship. NOT this decision: Enchant land / permanent /
      artifact rows, doesn't-untap, you-control-enchanted, the Aura heads,
      Curses, Equipment (next: Equip 504 lines). Refused by name: +30: the Auras the pool offered whose other line is outside the row shapes - the Aura ETB and enchanted-dies heads with payloads the generator does not read yet (a counter on a target, a tap, discards, scry, surveil, graveyard tokens), the put-into-a-graveyard and attacks-or-blocks heads, two sacrifice-to-draw activations, an anthem on an Aura (Emblem of the Warmind), Enchant artifact / creature or Vehicle / Forest or Plains / creature with another Aura attached, one multi-face.
      No spell def collided: an Aura's text is no spell's, so nothing was retired by D187's rule. Report `effect:auto` 3,572 → 3,572, `withUnenforced`
      237 → 280.
      **Landed:** 112 scripts and no auto flip (complete moved by exactly the wave: an Aura's other lines still need their defs). The wave in two parts: part A 91 rows off the pool of 142 the accounting opened (a P/T static 45 - Boon of Emrakul, Chant of the Skifsang, Clinging Darkness, Dead Weight ...; P/T and a keyword 40 - Arcane Flight, Aspect of Gorgon, Candlelight Vigil, Epic Proportions ...; a keyword grant 22 - Alexi's Cloak, Asha's Favor, Battle Mastery, Buoyancy ...; a combat restriction 5 - Bound in Silence, Detained by Legionnaires, Luminous Bonds, Pacifism, Pious Interdiction), part B 21 more once the shapes widened (Enchant creature you control 3, an opponent's creature 0, a green creature 0; the Aura's own ETB: a draw 11, life 3, tokens 6 - Angelic Gift, Cartouche of Knowledge, Cartouche of Solidarity, Chosen by Heliod, Dark Favor ...). Thirty refused by name. Tests: auraSeam.test.ts +
      auraAttachment.test.ts + 112 suites.
...
- [x] **M6.4eq — THE CYCLING SEAM: Cycling {N} is the engine's own activated
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,182 files, 16,662 passing / 11 skipped ·
      ...
      split; the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4er — THE FLASHBACK SEAM: Flashback {N} casts from the graveyard
      for that cost and exiles on leaving the stack, the engine's own;
      51 cards complete with no script (2026-09-04):** **5,359
      of 31,692 Commander-legal cards now execute completely, up from 5,308
      (+51).** `SHIPPED_SCRIPTS` 3,083; ledger 992
      (+38: the flashback spells the pool offered whose other sentences the vocabulary does not read yet - their flashback runs (Momentary Blink, Past in Flames, Devil's Play, Sever the Bloodline, Seize the Day, Echo of Eons ...)). Fixtures 3,412 (3,296 by name + 109 tokens: the seam's test cards (Feeling of Dread, Beast Attack, Deep Analysis; Counterspell was one already)). ZERO token pins; five
      support bodies and the vocabulary's exclusion, one classifier shape.
      **Select pool 0.** Decisions in **D307**.
      ⚠️ **Measured first:** D306's keyword probe - flashback 184 lines / 88
      pure (17 more with a dash cost). The pure 88 are spells whose bodies the vocabulary already reads - Feeling of Dread (tap up to two target creatures), Beast Attack (a 4/4 Beast) and their kin - and the fixtures' own Deep Analysis pays life for its flashback and stays where it was.
      ⚠️ **The seam:** `OracleFace.flashbackCost` (`parseFlashback`, mana
      only, instants and sorceries); `legal.ts` offers the cast from the
      GRAVEYARD at the flashback cost; `handlers.ts` `castSetup` admits the
      graveyard and prices it; `loop.ts` / `effects.ts` route a spell whose
      `castFrom` is the graveyard to EXILE on resolution, fizzle and counter;
      `effectParse.ts` drops the line from a spell's clauses;
      `engineComplete.ts` / `tier3.ts` / `primitives.ts` treat the line as
      the engine's own. No generator: the pure flashback spells complete by
      the accounting alone. NOT this decision: the dash costs (17), the other
      graveyard casts (Escape, Jump-start, Unearth, Encore). Refused by name:
      +38: the flashback spells the pool offered whose other sentences the vocabulary does not read yet - their flashback runs (Momentary Blink, Past in Flames, Devil's Play, Sever the Bloodline, Seize the Day, Echo of Eons ...).
      No spell def collided: a flashback line is no spell's sentence - the vocabulary drops it beside the cycling line - so nothing was retired by D187's rule. Report `effect:auto` 3,659 → 3,884, `withUnenforced`
      280 → 280.
      **Landed:** no script and 51 AUTO FLIPS - the flashback spells whose only unread line was their flashback line completed by the accounting alone, and 225 faces read auto once the flashback line is no clause (effect:auto 3,659 -> 3,884). No wave of rows: the seam's own flips are the landing (Feeling of Dread, Beast Attack, Momentary Blink's readable kin ...), and the bot's own deck took 22 instants and 29 sorceries more on the same sweep. Tests: flashbackSeam.test.ts +
      flashback.test.ts.
...
- [x] **M6.4es — THE KEYWORD-TRIGGER SEAM: prowess, exalted, bushido,
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,199 files, 16,744 passing / 11 skipped ·
      ...
      split; the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4et — THE MORPH SEAM: Morph {N} casts any card face down as a
      nameless colorless 2/2 for {3} and turns it face up for the cost, the
      engine's own; 38 cards complete with no script (2026-09-04):**
      **5,481 of 31,692 Commander-legal cards now execute completely, up
      from 5,443 (+38).** `SHIPPED_SCRIPTS` 3,097; ledger 1,035
      (+27: the morph creatures whose OTHER line the vocabulary does not read yet - their morph runs: 18 turned-face-up triggers (Aphetto Exterminator, Daru Sanctifier, Echo Tracer, Master of Pearls, Voidmage Apprentice, Skinthinner, Stratus Dancer ...), 7 activated lines (Whipcorder, Gravel Slinger, Fledgling Mawcor, Voidmage Prodigy ...), Grinning Demon's upkeep, Grim Haruspex's dies watcher, Lumithread Field's anthem). Fixtures 3,439 (3,323 by name + 109 tokens: the seam's four test cards (Woolly Loxodon, Glacial Stalker, Battering Craghorn, Aerie Bowmasters)). ZERO token pins; eleven
      support bodies, one intent. **Select pool 0.** Decisions in
      **D309**.
      ⚠️ **Measured first:** D306's keyword probe put morph first among the
      keyword yields still standing; measured again by d309/probe-morph.json: 184 blocked cards print a morph line - 141 Morph and 31 Megamorph with a mana cost, 12 with a dash cost - and 65 of them (12 megamorphs) have nothing else unread; 49 more carry a turned-face-up trigger.
      ⚠️ **The seam:** `OracleFace.morphCost` / `morphCostText` / `megamorph`
      (`parseMorph`, mana only, permanents); `legal.ts` offers every card in
      hand with a morph cost ALSO as a `CastSpell` with `faceDown: true` (a
      creature spell for {3}, sorcery speed, before the land check) and every
      face-down permanent you control as `TurnFaceUp` any time you have
      priority; `handlers.ts` `prepareCast` prices a face-down cast at {3} (no
      X, no targets, no identity), the move and the stack object carry
      `faceDown`, `turnFaceUp` pays the morph cost through the staged plan and
      validator a cast uses, `FaceDownSet` false, a +1/+1 counter for a
      megamorph, priority retained, no stack; `loop.ts` resolves the spell
      into a face-down permanent; `derive.ts` gives the face-down object to
      the STACK too and makes it `hasAbilities: false` — the one flag the
      trigger bus, the replacement funnel, the combat restrictions and the
      activated offers already honour — and skips a face-down static source;
      the accounting, the disclosure and the classifier treat the line as the
      engine's own; the fuzz gate casts face down and turns face up. No
      generator. NOT this decision: the dash morph costs (12), the
      turned-face-up triggers (49), the bot playing face down, manifest /
      disguise / cloak. Refused by name: +27: the morph creatures whose OTHER line the vocabulary does not read yet - their morph runs: 18 turned-face-up triggers (Aphetto Exterminator, Daru Sanctifier, Echo Tracer, Master of Pearls, Voidmage Apprentice, Skinthinner, Stratus Dancer ...), 7 activated lines (Whipcorder, Gravel Slinger, Fledgling Mawcor, Voidmage Prodigy ...), Grinning Demon's upkeep, Grim Haruspex's dies watcher, Lumithread Field's anthem.
      Nothing retired. Report `effect:auto` 3,884 → 3,884, `withUnenforced`
      280 → 280.
      **Landed:** 38 AUTO FLIPS with no script - the morph permanents whose morph line was the last line the accounting refused (the probe's 65 pure carriers less the 27 whose other line is scriptable but unshipped, ledgered by name). No wave of rows: the seam's own flips are the landing, and the bot's own deck took 37 creatures and a land (Zoetic Cavern) more on the same sweep. Tests: morphSeam.test.ts +
      morph.test.ts.
...
- [x] **M6.4eu — THE CHARACTERISTIC-DEFINING KEYWORDS: Changeling is every
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,214 files, 16,815 passing / 11 skipped ·
      ...
      split; the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4ev — THE CREW SEAM: Crew N taps creatures with total power N or
      more and the Vehicle is an artifact creature until end of turn, the
      engine's own ability; 18 cards complete with no script
      (2026-09-04):** **5,522 of 31,692 Commander-legal cards now execute
      completely, up from 5,504 (+18).** `SHIPPED_SCRIPTS`
      3,108; ledger 1,074 (+19: the Vehicles whose OTHER line the vocabulary does not read yet - their crew runs: 13 enters triggers (tokens, draws, a scry, a targeted pump: Enchanted Carriage, Hulldrifter, Bomat Bazaar Barge, Turtle Blimp, Spotcycle Scouter, Burner Rocket ...), 2 combat triggers (Careening Mine Cart, Silent Submersible), 4 other lines (Rangers' Aetherhive, Thopter Fabricator, Fire Nation Warship, Rover Blades)). Fixtures 3,456
      (3,340 by name + 109 tokens: the seam's test Vehicles (Sky Skiff, Consulate Dreadnought; Cultivator's Caravan was one already)). ZERO token pins; nine support bodies, no new intent.
      **Select pool 0.** Decisions in **D311**.
      ⚠️ **Measured first:** d311/probe-crew.json over the database: 172 blocked Vehicles print a Crew line (Crew 2 on 70, Crew 1 on 44, Crew 3 on 39, Crew 4 on 12, Crew 6 on 4, Crew 5 on 2, Crew 8 on 1), every one with a printed P/T, and 37 of them (Sky Skiff, Consulate Dreadnought, Cultivator Caravan, Aradara Express, Sleek Schooner ...) have nothing else unread; the rest print attack triggers and enters-crewed clauses next.
      ⚠️ **The seam:** `activatedParse.ts` synthesizes "Crew N" as an
      activated ability in print order — no mana, a tap cost over creatures
      with `powerAtLeast: N` (count 0), the effect "This Vehicle becomes an
      artifact creature until end of turn", `crew: {line, power}`;
      `legal.ts` offers it without a def when the untapped creatures can
      reach the power together (`tapPower` on the offer); `handlers.ts` sums
      the named taps' derived power; `loop.ts` resolves a
      `PtModifiedUntilEndOfTurn` with `types: [Artifact, Creature]` — the
      carrier, the reducer and the event learn `types`, `derive.ts` reads
      them at layer 4, the cleanup ends it; the fuzz gate crews by tapping
      every candidate; the accounting, the disclosure and the classifier
      treat the line as the engine's own. No generator. NOT this decision:
      the Vehicles' other lines, the bot choosing which creatures to crew
      with, saddle and station. Refused by name: +19: the Vehicles whose OTHER line the vocabulary does not read yet - their crew runs: 13 enters triggers (tokens, draws, a scry, a targeted pump: Enchanted Carriage, Hulldrifter, Bomat Bazaar Barge, Turtle Blimp, Spotcycle Scouter, Burner Rocket ...), 2 combat triggers (Careening Mine Cart, Silent Submersible), 4 other lines (Rangers' Aetherhive, Thopter Fabricator, Fire Nation Warship, Rover Blades).
      Nothing retired. Report `effect:auto` 3,884 → 3,884,
      `withUnenforced` 280 → 280.
      **Landed:** 18 AUTO FLIPS with no script - the Vehicles whose Crew line was the last line the accounting refused (the probe's 37 pure Vehicles less the 19 whose other line is scriptable but unshipped, ledgered by name). No wave of rows: the seam's own flips are the landing, and the bot's own deck took 18 artifacts more on the same sweep (Air Response Unit among them). Tests: crewSeam.test.ts +
      crew.test.ts.
...
- [x] **M6.4ex — THE TRIGGER HEADS, PART 2: seven more heads and six more
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,307 files, 17,204 passing / 11 skipped ·
      ...
      split; the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4ey — THE TRIGGER HEADS, PART 3: tokens and per-item counters on a
      payloads in the one-shot generator, a you-may payload an optional
      trigger; 9 cards land as generated rows (2026-09-04):**
      **5,660 of 31,692 Commander-legal cards now execute completely, up
      from 5,651 (+9).** `SHIPPED_SCRIPTS` 3,206; ledger
      1,093 (none this decision - the rows landed by name, none of them was in the ledger, and the pool stays empty). Fixtures 3,560 (3,443 by name + 110 tokens: the 9 rowed cards and the Merfolk Wizard token Stonybrook Schoolmaster makes (the first token pin on a trigger row)). ZERO token
      pins; no engine change. **Select pool 0.** Decisions in **D314**.
      ⚠️ **Measured first:** the D314 candidate probe over the database after D313: 20 blocked single-face permanents whose every unread line the extended grammar reads (the enters draw-and-lose 8, the upkeep mills 6 and counters 4, the end-step sacrifices 4, the upkeep tokens 2 ...); the mass behind them is other heads and payloads - not a trigger at all 11,438, an Aura enters 129, an Equipment enters 89, a modal choose-one 94, a land enters 58, when you cast this spell 42, each end step 41, enters or attacks 81, the monarch 35, energy 33, equipped creature attacks 32.
      ⚠️ **The rows:** `d314/make-rows4.cjs` + `gen-oneshot4.cjs` (from D313) — a
      token payload on a TRIGGER (its printing pinned as a fixture through
      `token-pins-needed-d314.json`), a +1/+1 counter on the entering creature
      (the per-item fan-out over the two enters heads, `obj.item`), heads each
      upkeep (`StepBegan`, any player) and turned face up (`FaceDownSet`),
      payloads draw-and-lose N, lose N life, untap itself. One module + one
      suite per row. Rows land BY NAME from the candidate probe; the classifier
      does not learn the heads yet, so the pool stays 0 and only `complete`
      moves. NOT this decision: the payloads and heads the generator still
      refuses, the classifier's head library. Refused by name: none this decision - the rows landed by name, none of them was in the ledger, and the pool stays empty.
      Nothing retired. Report `effect:auto` 3,918 → 3,918,
      `withUnenforced` 280 → 280.
      **Landed:** no auto flips (no engine change) and 9 generated rows - the 20 candidates less the 11 the row maker still refuses (three morph lines, two damage payloads, two untap-it under a block head, an exile and a discard cost, a land-graveyard head, a cost line). The wave IS the landing: 9 rows - the draw-and-lose cards (Phyrexian Arena's upkeep, Phyrexian Rager's and Dusk Legion Zealot's and Tithebearer Giant's enters, Audacious Thief's attack, Baleful Force's upkeep, Novice Occultist's death), the token triggers (Stonybrook Schoolmaster's Merfolk Wizard, Luminous Angel's Spirit); the bot's own deck took 8 creatures and an enchantment more. Tests: one generated suite per row.
...
- [x] **M6.4ez — THE TRIGGER HEADS, PART 4: nine more heads, the enters head on
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,333 files, 17,315 passing / 11 skipped ·
      ...
      split; the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4fa — THE MIXED ROWS: the candidate probe admits activated lines the
      row maker reads beside the trigger heads; 25 cards land as
      generated rows (2026-09-05):**
      **5,702 of 31,692 Commander-legal cards now execute completely, up
      from 5,677 (+25).** `SHIPPED_SCRIPTS` 3,248; ledger
      1,093 (none this decision - the rows landed by name, none of them was in the ledger, and the pool stays empty). Fixtures 3,602 (3,485 by name + 110 tokens: the 25 rowed cards; the Clue the twelve investigate rows make was already pinned). No engine
      change, no grammar change. **Select pool 0.** Decisions in **D316**.
      ⚠️ **Measured first:** the D316 candidate probe over the database after D315: 62 blocked single-face permanents whose every unread line is a trigger the D315 grammar reads or an activated line whose effect is a row payload - 46 activations only, 15 triggers only, one mixed (lines: the activated Clue tokens 13, drains 7, mills 7, self counters 7, draw-and-lose 5; the enters mills 18 and tokens 8, the upkeep mills 6); the mass behind them: an Aura enters 129, landfall 126, a modal choose-one 100, power and toughness equal to something 98, regenerate 97, an Equipment enters 89, any-colour mana 80, can not block 75, an additional cost 58, enchanted creature has 55, can not be blocked 50, convoke 50, can not be countered 46, enters with X counters 45, attacks each combat 44, when you cast this spell 42, enchant player 42, heroic 41, a Spirit or Arcane spell 40, loot 39.
      ⚠️ **The rows:** `d316/zz-probe-d316b.node.test.ts` passes a blocker line
      as a trigger (D315's heads) OR as an activated line whose effect is a row
      payload; `make-rows6.cjs` + `gen-oneshot6.cjs` are D315's renamed. Rows
      land BY NAME; the classifier does not learn the heads yet, so the pool
      stays 0 and only `complete` moves. NOT this decision: the costs the row maker cannot charge (tap two or three untapped creatures, an Ally, exile from the graveyard, exile this creature, sacrifice an artifact or creature), the target effects (a target creature's pump, trample, counters), the damage payloads, the ability-word activated lines (Boast, Exhaust, Cohort, Channel), the heads still outside (one or more cards put into the graveyard, sacrifice a token, deals combat damage to a creature); the mass behind - the Aura and Equipment enters heads, landfall, the modal choose-one, regenerate, any-colour mana, the can-not-block and can-not-be-blocked statics, convoke, enters with counters; the classifier learning the heads. Refused by
      name: none this decision - the rows landed by name, none of them was in the ledger, and the pool stays empty.
      Nothing retired. Report `effect:auto` 3,918 → 3,918,
      `withUnenforced` 280 → 280.
...
- [x] **M6.4fb — THE STATIC ROWS: the card's own combat restrictions, the count
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,509 files, 17,795 passing / 11 skipped ·
      ...
      the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4fc — THE TRIGGER HEADS, PART 5: eight more heads in the one-shot
      generator, and the entering card's own replacements (CR 614.12) for the
      enters-with-counters rows; 8 cards land as generated rows
      (2026-09-05):**
      **5,861 of 31,692 Commander-legal cards now execute completely, up
      from 5,853 (+8).** `SHIPPED_SCRIPTS` 3,407; ledger
      1,093 (none this decision - the rows landed by name, none of them was in the ledger, and the pool stays empty). Fixtures 3,761 (3,642 by name + 112 tokens: the 8 rowed cards; no token pin (Agent 13's Clue was already pinned)). ONE engine
      change: the replacement funnel offers a `CardsMoved` event to the
      entering cards' OWN replacement defs (CR 614.12; `applicableTo` in
      `src/engine/triggers.ts`; `src/engine/entersWith.test.ts`) — until now a
      `ReplacementDef` on the entering card never ran, so no shipped script
      carried one. **Select pool 0.** Decisions in **D318**.
      ⚠️ **Measured first:** the D318 candidate probe over the database after D317: 80 blocked single-face permanents whose every unread line the widened grammar reads - 39 with a static or combat line, 41 within the D316 grammar (the new heads' lines among them: a creature you control deals combat damage to a player 3, a player casts a spell 3, an opponent casts a spell 2, is dealt damage 1, attacks or blocks 1, a creature you control attacks alone 1; enters with counters 15, the count CDA 10, can not attack or block alone 5); the mass behind them: regenerate 99, any-colour mana 80, a modal choose-one 64, enchanted creature has 55, convoke 50, attacks each combat if able 46, can not be countered 46, enters with X counters 45, when you cast this spell 42, enchant player 42, heroic 41, a Spirit or Arcane spell 40, a look at the top card 39, loot 39, start your engines 38, the monarch 36, enchanted land has 35, energy 33, equipped creature attacks 32, threshold 32.
      ⚠️ **The rows:** `d318/make-rows8.cjs` + `gen-oneshot8.cjs` (from D317) —
      attacks and isn't blocked (`BlockersDeclared` against the combat record),
      deals combat damage to a creature, a creature you control deals combat
      damage to a player, is dealt damage as TWO defs on one line
      (`CombatDamageDealt` + `DamageDealt`: the engine emits combat and
      noncombat damage as two events), attacks or blocks as two defs, a
      creature you control attacks alone, a player / an opponent casts a spell
      (`SpellCast`); "~ enters with N +1/+1 counters on it" as a `ReplacementDef`
      whose test CASTS the card for its printed cost. A self payload under a
      damage head needs toughness 2 (the blocking Goblin) or 4 (the Bolt). Rows
      land BY NAME; the classifier does
      not learn the heads yet, so the pool stays 0 and only `complete` moves.
      NOT this decision: the remove-a-counter cost verb (the Spike family, ten rows waiting, and the ledger's eight), the variable enters-with-X counters (45), the tap-N-creatures and exile-from-graveyard costs, the target effects and damage payloads, the heads still outside (one or more cards put into a graveyard, sacrifice a token, a second card each turn); the mass behind - regenerate, any-colour mana, the modal choose-one, convoke, attacks each combat if able, can not be countered; the classifier learning the kinds. Refused by name: none this decision - the rows landed by name, none of them was in the ledger, and the pool stays empty.
      Nothing retired. Report `effect:auto` 3,918 → 3,918,
      `withUnenforced` 280 → 280.
      **Landed:** no auto flips (the engine change offers a def no shipped script carried) and 8 generated rows - the 80 candidates less the 72 the row maker still refuses (seventeen lines outside both grammars - enters with X counters, morph, can not attack or block alone, toxic, a cost line, an anthem; eleven remove-a-counter costs, the Spike family; the tap-N-creatures, exile-from-graveyard, sacrifice-a-choice and return-a-land costs; the target effects and damage payloads; eight heads outside the library; the untap-it, Lander, surveil and scry payloads). The wave IS the landing: 8 rows - Reconnaissance Mission's draw when a creature you control connects, Taurean Mauler's and Mogg Sentry's answers to an opponent's spell, Managorger Hydra's and Blind Creeper's to any player's, Agent 13's Clue when a creature attacks alone, and the first enters-with rows - Faithful Watchdog's one counter and Adaptive Shimmerer's three; the bot's own deck took 7 creatures and an enchantment more. Tests: one generated suite per row.
...
- [x] **M6.4fe — THE TRIGGER HEADS, PART 6: five more heads (one from the stack)
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,548 files, 18,233 passing / 11 skipped ·
      ...
      the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4ff — THE LEFTOVER ROWS: the row maker reads only the lines nothing
      runs; the leftover probe replaces the classifier's blocker list;
      42 cards land as generated rows (2026-09-05):**
      **5,932 of 31,692 Commander-legal cards now execute completely, up
      from 5,890 (+42).** `SHIPPED_SCRIPTS` 3,478; ledger
      1,060 (33 rows retired by name - cards the ledger held for a named gap that the leftover probe and the row grammar now read whole (Broodstar, Debris Beetle, Hystrodon, the morph creatures, Rumbleweed among them); the pool stays empty). Fixtures 3,838 (3,712 by name + 119 tokens: the 42 rowed cards and six token pins (Monastery Mentor's Monk, a Mouse, a Mutant, and - after the 60-seed canary caught them unpinned - the Bear, Citizen and Pegasus the activated rows make)). No engine
      change. **Select pool 0.** Decisions in **D321**.
      ⚠️ **Measured first:** the leftover probe over the database after D320: 154 blocked single-face permanents whose every LEFTOVER line the grammar reads (114 in the trigger and activated grammar, 32 with a static or combat line, 6 Auras, 2 Equipment) - against 97 under the classifier's blocker list, which counted the engine's own lines (a mana ability, a keyword, an equip, enters tapped) as blockers; the lines the covered cards carry: activated self pumps 27, tokens 20, draws 12, target pumps 10, the count CDA 10, the variable enters-with counters 9, enters tokens 7; the mass behind them: a modal choose-one 276 leftover lines, regenerate 100, any-colour mana 70, enchanted creature has 55, convoke 50, attacks each combat if able 46, enters with X counters 43, can not be countered 43, enchant player 42, a look at the top card 38, start your engines 38, your second card each turn 37, the monarch 35, loot 35, enchanted land has 34, energy 33, does not untap 31, enter as a copy 31, threshold 31, blocks or becomes blocked by 30.
      ⚠️ **The seam, in the tooling:** every probe since D313 took a card's
      blockers from the classifier (every line whose primitive is not
      "scriptable"), which counts the lines the ENGINE runs — a mana ability, a
      keyword, an equip, enters tapped — as blockers, and the row maker walked
      every printed line and refused what it could not read.
      `engineCompleteness(card).leftover` is the exact list of lines nothing
      runs: `d321/zz-probe-d321c.node.test.ts` reads those, writing each with
      its printed line index and the engine's ability index (matched against
      the parsed face) plus the Equip line beside; `make-rows11.cjs` rows ONLY
      those lines, the index taken from the probe — every earlier skip is gone
      with the walk that needed it. `gen-oneshot11.cjs` is D320's unchanged.
      Rows land BY NAME; the classifier does not learn the grammar yet, so the
      pool stays 0 and only `complete` moves. NOT this decision: the Aura and Equipment statics (enchanted or equipped creature gets +N/+N or has a keyword - 79 and 77 leftover lines, the largest readable mass now that the leftover probe sees them, with the attach test), the costs the row maker cannot charge (a random discard on ctx.random, {Q}, snow mana, exile-from-graveyard, the remove-a-counter chooser, sacrifice-a-choice, tap-N-creatures), the untap-it payloads, the counts outside the vocabulary (other creature you control, died this turn); the mass behind - the modal choose-one, regenerate, any-colour mana, convoke, attacks each combat if able, can not be countered; the classifier learning the grammar.
...
- [x] **M6.4fi — THE X COUNTERS AND THE ANTHEMS: enters with X counters, the anthem
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,702 files, 18,952 passing / 11 skipped ·
      ...
      the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4fj — THE MANA ABILITIES WITH AN EXTRA COST: the engine charges a mana
      ability's cost beside the {T} (mana, life, its own sacrifice) and the
      accounting counts the line as run; 296 cards land — 197 as
      generated rows, the rest with no script at all (2026-09-05):**
      **6,340 of 31,692 Commander-legal cards now execute completely, up
      from 6,044 (+296).** `SHIPPED_SCRIPTS` 3,787; ledger
      1,043 (2 rows added by name - the two cards the seam made offerable that the row maker still refuses, Heap Gate (a tap-a-Gate cost) and Urn of Godfire (a two-type destroy); none retired (none of the 197 was ledgered); the pool is empty). Fixtures 4,152 (4,022 by name + 123 tokens: the 197 rowed cards, the four cards the engine test reads (Rakdos Signet, Mana Confluence, Lotus Petal, Springleaf Drum) and the Sol Ring and Hissing Miasma the new heads put down; no token pin moved). An ENGINE change
      (five counted swaps) and the wave it uncovered (197 generated
      rows). **Select pool 0.** Decisions in **D325**.
      ⚠️ **Measured first:** the scoping probe over the blocked permanents before the change: 87 leftover lines were a mana ability with a piece beside the {T} the engine never charged, 56 cards would be complete on that alone (the shapes: a {N} beside the tap 59, pay 1 life 10, tap an untapped creature 8, a coloured symbol 4, pay 2 life 2); two other seams measured beside it - can not be countered would complete 11, attacks each combat if able 25, the monarch 0. Then the leftover probe over the database after the engine change: 326 blocked single-face permanents whose every leftover line the extended grammar reads (287 in the trigger and activated grammar, 32 with a static or combat line, 5 Auras, 2 Equipment) - the pingers first: an enters damage 28, a dies damage 8, an activated damage 7, damage to each opponent 8 activated and 11 triggered, loot 23 activated and 10 on entry, the activated self return 14, the activated untap 9, the artifact draws 6, the scry lands 6; the mass behind the candidates: a modal choose-one 276 leftover lines, regenerate 102, enchanted creature has 60, convoke 50, attacks each combat if able 46, can not be countered 43, enchant player 42, a look at the top card 38, start your engines 38, your second card each turn 37, the monarch 35, enchanted land has 34.
      ⚠️ **The seam:** `ManaProduction.extraCost` — the pieces beside the {T}
      the tap charges (mana from the pool, a life payment, the permanent's own
      sacrifice; `chargeablePieces()` in `oracleParse.ts`); any other piece
      keeps the line `conditional` as before. `manaSourcesOf` takes
      `includeCostly` (a costly source is NEVER a solver source — mana against
      mana is not priced); `extraCostSpend()` says payable-now and with what;
      `legalActions` offers a costly source only while payable; `tapForMana`
      charges (`ManaSpent`, `LifeChanged`, the `CardsMoved`) before the
      `ManaAdded` and refuses an unpayable one (`cannotAfford`). The accounting
      needed no change (a charged line is no longer conditional). Test:
      `src/engine/manaExtraCost.test.ts` (Rakdos Signet, Mana Confluence, Lotus
      Petal; Springleaf Drum stays conditional). ⚠️ **The wave:** the D326
      grammar brought forward (`d325/make-rows15.cjs` + `gen-oneshot15.cjs`) —
      the pingers ("~ deals N damage to any target / target creature / target
      player / each opponent" as a `DamageDealt` built like `effects.ts`; a
      targeted trigger declares `parseTargetClauses(line)`), counters of any
      kind, scry (the engine's `scryChoice` prompt raised by the resolve), loot
      (the draw, then the `chooseFromZone` prompt), the artifact tokens (Food,
      Treasure, Blood, Gold, Map), the activated self returns and untaps, the
      graveyard head on any permanent, seven heads (this or another creature
      enters, an artifact enters, cast an artifact spell, a creature you control
      dies, constellation, inspired, the enchanted creature's controller's
      upkeep) and the Enrage / Alliance words. TRAP: a line split into two
      triggers (Enrage) declares its token constant once. NOT this decision: can not be countered (a static flag the counter effect consults, 11 cards whole), attacks each combat if able (a declaration requirement, 25), the remaining mana pieces (tap an untapped creature you control 8, a discard 1 - chooser costs on a mana ability), the costs the row maker cannot charge (a random discard on ctx.random, snow mana, exile-from-graveyard, a counter from another creature, tap-N-creatures, a token, a Gate), the counts outside the vocabulary (died this turn, a named creature, subtypes without a fixture), the Vehicle attacks head (a crew in the test), the destroy payloads with a two-type target; the mass behind - the modal choose-one, regenerate, the quoted abilities enchanted creatures and lands have, convoke, the monarch and energy, the second card and second spell each turn.
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,900 files, 19,805 passing / 11 skipped ·
      ...
      ⚠️ **Reportables** (D325): can not be countered (a static flag the counter effect consults, 11 cards whole), attacks each combat if able (a declaration requirement, 25), the remaining mana pieces (tap an untapped creature you control 8, a discard 1 - chooser costs on a mana ability), the costs the row maker cannot charge (a random discard on ctx.random, snow mana, exile-from-graveyard, a counter from another creature, tap-N-creatures, a token, a Gate), the counts outside the vocabulary (died this turn, a named creature, subtypes without a fixture), the Vehicle attacks head (a crew in the test), the destroy payloads with a two-type target; the mass behind - the modal choose-one, regenerate, the quoted abilities enchanted creatures and lands have, convoke, the monarch and energy, the second card and second spell each turn; the modal seam; the "another" split;
      the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4fk — THE VEHICLES AND THE TARGETED: the Vehicle attacks head crewed in
      the test and the becomes-the-target heads, as generated rows; 25
      cards land as generated rows (2026-09-05):**
      **6,365 of 31,692 Commander-legal cards now execute completely, up
      from 6,340 (+25).** `SHIPPED_SCRIPTS` 3,812; ledger
      1,040 (3 rows retired by name - the targeted and Vehicle cards the ledger held for a head the rows now read (Tethered Skirge, Thorn Lieutenant, Careening Mine Cart); the pool stays empty). Fixtures 4,177 (4,047 by name + 123 tokens: the 25 rowed cards; no token pin moved). No engine
      change. **Select pool 0.** Decisions in **D326**.
      ⚠️ **Measured first:** the leftover probe over the database after D325: 160 blocked single-face permanents whose every leftover line the grammar reads (133 in the trigger and activated grammar, 26 with a static or combat line, 1 Aura); the lines the covered cards carry: activated tokens 30, activated self pumps 25, draws 14, activated damage 13, the sacrifice when targeted 13, the count CDA 10, target pumps 9, the variable enters-with counters 8, upkeep counters 7; the mass behind the candidates: a modal choose-one 276 leftover lines, regenerate 102, enchanted creature has 60, convoke 50, attacks each combat if able 46, can not be countered 43, enchant player 42, a look at the top card 38, start your engines 38, your second card each turn 37, the monarch 35, enchanted land has 34, energy 33, enter as a copy 31, threshold in its older wording 31, blocks or becomes blocked by 30, can not attack unless 28, the graveyard returns 27 and 26, your second spell each turn 26, restricted any-colour mana 25, activate only once each turn 18.
      ⚠️ **The rows:** `d326/make-rows16.cjs` + `gen-oneshot16.cjs` (from D325)
      — "Whenever this Vehicle attacks" (the Crew line's index and power off the
      probe; the test crews it with the Bears, and the Eel past two, through the
      engine's own Crew ability, then attacks) and "When(ever) this creature
      becomes the target of a spell or ability [an opponent controls]" (one line,
      TWO defs: `SpellCast` and `AbilityPutOnStack`, both reading the stack
      object's targets; the test aims a Giant Growth — the row's own, or the
      opponent's on its fourth turn; a self pump under it is refused as under
      heroic). TRAP: the test's own Growth resolves into the graveyard — a mill
      under the head reads one more. Rows land BY NAME from the leftover probe; the classifier does not
      learn the kinds yet, so the pool stays 0 and only `complete` moves. NOT
      this decision: the tap-N costs (tap two or three or five untapped creatures, Zombies, Kithkin, Elves, artifacts - the engine's tap chooser with a count, the deck carrying the fixtures N times) and the token fodder (sacrifice a Saproling, sacrifice a token - a token made by hand before the baseline), a +1/+1 counter on a Vehicle, the random discard cost (ctx.random in the cost), activate only once each turn (a per-turn memory on the turn state), can not be countered (a flag the counter effect consults), the counts outside the vocabulary (died this turn, a named creature, subtypes without a fixture), attacks or blocks alone and attacks each combat if able (declaration requirements); the mass behind - the modal choose-one, regenerate, the quoted abilities enchanted creatures and lands have, convoke, the monarch and energy, the second card and second spell each turn. Refused by name: 3 rows retired by name - the targeted and Vehicle cards the ledger held for a head the rows now read (Tethered Skirge, Thorn Lieutenant, Careening Mine Cart); the pool stays empty.
      Nothing retired. Report `effect:auto` 3,918 → 3,918,
      `withUnenforced` 280 → 280.
      **Landed:** no auto flips and 25 generated rows in one pass, one fix (the test's own Giant Growth lands in the graveyard beside a mill) - the 160 candidates less the 135 the row maker still refuses (the costs it cannot charge, 107: a random discard, a Saproling or a token, exile this card or a creature card or two or three cards from your graveyard, a counter from another creature, snow mana, return a land, tap N untapped creatures or Zombies or Kithkin or Elves or artifacts or a Gate; 15 counts outside the vocabulary; five lines the probe reads and the generator does not - attacks or blocks alone; two heroic self pumps beside the Growth; two self payloads the test damage kills; one Vehicle counter, one historic discard, one Aura payload after a sacrifice). The wave IS the landing: 25 rows - the Phantasmals and the skulkers that die when targeted (Phantasmal Bear, Phantasmal Dragon, Phantasmal Shieldback, Phantasmal Abomination, Gossamer Phantasm, Skulking Fugitive, Skulking Ghost, Tar Pit Warrior and their kin), the ones that leave or grow instead (Mirozel, Petrahydrox, Segmented Wurm, Mossdog, Tethered Skirge), the ones that draw or make a token when an opponent aims at them (Cactarantula, Warden of the Woods, Thorn Lieutenant), Cephalid Aristocrat's mill, and the Vehicles (Adventurer's Airship's loot, Ballista Charger's damage, Careening Mine Cart's token); the bot's own deck took 22 creatures and 3 artifacts more. Tests: one generated suite per row.
      Fixtures 4,177 · botPool artifact 357 / creature 3,540 / enchantment 323 / instant 947 / land 475 / sorcery 723 - auto 868 / assisted 1,840 / autoAnyFace 877 · ladder [1131, 1256, 2948, 4829, 6180] · batch.json
      25 · select pool 0.
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,925 files, 19,907 passing / 11 skipped ·
      500-seed gate, 6 shards, 383.2 s wall · build clean · probe 124/124 · battery 130/130.**
      ⚠️ **Reportables** (D326): the tap-N costs (tap two or three or five untapped creatures, Zombies, Kithkin, Elves, artifacts - the engine's tap chooser with a count, the deck carrying the fixtures N times) and the token fodder (sacrifice a Saproling, sacrifice a token - a token made by hand before the baseline), a +1/+1 counter on a Vehicle, the random discard cost (ctx.random in the cost), activate only once each turn (a per-turn memory on the turn state), can not be countered (a flag the counter effect consults), the counts outside the vocabulary (died this turn, a named creature, subtypes without a fixture), attacks or blocks alone and attacks each combat if able (declaration requirements); the mass behind - the modal choose-one, regenerate, the quoted abilities enchanted creatures and lands have, convoke, the monarch and energy, the second card and second spell each turn; the modal seam; the "another" split;
      the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4fl — THE TAPPERS AND THE FODDER TOKENS: the tap-N costs, the token
      fodder and the Vehicle counter, as generated rows; 14 cards land
      as generated rows (2026-09-05):**
      **6,379 of 31,692 Commander-legal cards now execute completely, up
      from 6,365 (+14).** `SHIPPED_SCRIPTS` 3,826; ledger
      1,038 (5 rows retired by name - the Thallids and tappers the ledger held for a cost the rows now charge (Ghirapur Aether Grid, Voice of the Woods among them) - and 3 added back: Fountainport, Glimmer Bairn and Hardened Tactician, whose Sacrifice a token cost the engine's sacrifice chooser does not read (no token predicate); the pool stays empty). Fixtures 4,193 (4,061 by name + 125 tokens: the 14 rowed cards, the Walking Corpse, Llanowar Elves and Raging Goblin the tap counts put, and two token pins (Grove of the Guardian's 8/8 Elemental, a 7/7 trampling Elemental)). No engine
      change. **Select pool 0.** Decisions in **D327**.
      ⚠️ **Measured first:** the leftover probe over the database after D326: 135 blocked single-face permanents whose every leftover line the grammar reads; the lines the covered cards carry: activated tokens 30, activated self pumps 25, draws 14, activated damage 13, the count CDA 10, target pumps 9, the variable enters-with counters 8, upkeep counters 7, and the costs this decision charges beneath them - tap N untapped creatures or Zombies or Elves or artifacts 8, sacrifice a Saproling 7, a +1/+1 counter on a Vehicle 1; the mass behind the candidates: a modal choose-one 276 leftover lines, regenerate 102, enchanted creature has 60, convoke 50, attacks each combat if able 46, can not be countered 43, enchant player 42, a look at the top card 38, start your engines 38, your second card each turn 37, the monarch 35, enchanted land has 34, energy 33, enter as a copy 31, threshold in its older wording 31, blocks or becomes blocked by 30, can not attack unless 28, your second spell each turn 26, activate only once each turn 18, a random discard 10.
      ⚠️ **The rows:** `d327/make-rows17.cjs` + `gen-oneshot17.cjs` (from D326)
      — "Tap two / three / four / five untapped creatures (Zombies, Elves,
      Goblins, artifacts) you control" as the engine's tap chooser with a count
      (the deck carries the fixture N times; `tapper<i>` is a list the test puts
      and reads tapped); "Sacrifice a Saproling" / "Sacrifice a token" as a
      `TOKEN:` fodder (the Saproling printing off `TOKEN_TABLE`, made by
      `ManualCreateToken` before the baseline, read gone after); a +1/+1 counter
      on a Vehicle. TRAPS: "Sacrifice a token" is not a cost the engine charges
      (no token predicate in the chooser) — three rows refused and ledgered; a
      count fixture a tap cost also puts N times needs one more deck copy. Rows
      land BY NAME from the leftover probe; the classifier
      does not learn the kinds yet, so the pool stays 0 and only `complete`
      moves. NOT this decision: activate only once each turn (a per-turn memory on the turn state, a parser flag, the handler and the offers - 18 lines), the random discard cost (the parser's atRandom, the handler drawing on the seeded rng at payment - 10), sacrifice a token (a token predicate in the sacrifice chooser - 3), can not be countered (a flag the counter effect consults, 11 cards), the exile-from-graveyard and return-a-land costs, the counts outside the vocabulary (died this turn, a named creature, subtypes without a fixture), attacks or blocks alone and attacks each combat if able (declaration requirements); the mass behind - the modal choose-one, regenerate, the quoted abilities enchanted creatures and lands have, convoke, the monarch and energy, the second card and second spell each turn. Refused by name: 5 rows retired by name - the Thallids and tappers the ledger held for a cost the rows now charge (Ghirapur Aether Grid, Voice of the Woods among them) - and 3 added back: Fountainport, Glimmer Bairn and Hardened Tactician, whose Sacrifice a token cost the engine's sacrifice chooser does not read (no token predicate); the pool stays empty.
      Nothing retired. Report `effect:auto` 3,918 → 3,918,
      `withUnenforced` 280 → 280.
      **Landed:** no auto flips and 14 generated rows in two passes - 17 on the grammar, then 14 once the row maker stopped reading Sacrifice a token (the engine's sacrifice chooser reads types, subtypes and colours, and a token is none of them: three rows the handler refused, back in the ledger by name) and the count fixture a tap cost also reads got its extra copy (Altar Golem: five Bears tapped, one more counted) - the 135 candidates less the 121 the row maker still refuses (the costs it cannot charge: a random discard, exile this card or a creature card or two or three cards from your graveyard, a counter from another creature, snow mana, return a land, a Gate; 15 counts outside the vocabulary; five lines the probe reads and the generator does not; two heroic self pumps beside the Growth; two self payloads the test damage kills; one historic discard; one Aura payload after a sacrifice). The wave IS the landing: 14 rows - the Thallids that spend their spore counters on Saprolings and the Saprolings on more (Thallid Germinator, Thallid Devourer, Deathspore Thallid, Vitaspore Thallid, Psychotrope Thallid, Mycologist, Elvish Farmer), the tap-N cards (Siege Zombie, Cryptbreaker, Altar Golem, Grove of the Guardian, Voice of the Woods, Ghirapur Aether Grid) and Untethered Express's counter; the bot's own deck took 11 creatures, 1 artifact, 1 enchantment and 1 land more. Tests: one generated suite per row.
      Fixtures 4,193 · botPool artifact 358 / creature 3,551 / enchantment 324 / instant 947 / land 476 / sorcery 723 - auto 868 / assisted 1,840 / autoAnyFace 877 · ladder [1129, 1254, 2946, 4819, 6169] · batch.json
      14 · select pool 0.
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 3,939 files, 19,980 passing / 11 skipped ·
      500-seed gate, 6 shards, 374.2 s wall · build clean · probe 124/124 · battery 130/130.**
      ⚠️ **Reportables** (D327): activate only once each turn (a per-turn memory on the turn state, a parser flag, the handler and the offers - 18 lines), the random discard cost (the parser's atRandom, the handler drawing on the seeded rng at payment - 10), sacrifice a token (a token predicate in the sacrifice chooser - 3), can not be countered (a flag the counter effect consults, 11 cards), the exile-from-graveyard and return-a-land costs, the counts outside the vocabulary (died this turn, a named creature, subtypes without a fixture), attacks or blocks alone and attacks each combat if able (declaration requirements); the mass behind - the modal choose-one, regenerate, the quoted abilities enchanted creatures and lands have, convoke, the monarch and energy, the second card and second spell each turn; the modal seam; the "another" split;
      the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4fm — ONCE EACH TURN, AT RANDOM, A TOKEN: the engine charges "activate
      only once each turn", a random discard and "sacrifice a token", and the rows
      that follow; 65 cards land, 65 as generated rows (2026-09-05):**
      **6,444 of 31,692 Commander-legal cards now execute completely, up
      from 6,379 (+65).** `SHIPPED_SCRIPTS` 3,891; ledger
      1,016 (13 rows retired by name - the once-each-turn pumps, the random discards and the three token sacrifices the ledger held for a cost or a limit the engine charges now (Fountainport, Glimmer Bairn, Hardened Tactician among them); the count is MEASURED this decision (the REFUSED map's size) - the carried figure of 1,038 had drifted 9 above the map). Fixtures 4,258 (4,125 by name + 126 tokens: the 65 rowed cards (64 new by name) and one token pin (Call the Bloodline's 1/1 lifelink Vampire Knight)). An ENGINE change
      (three seams, one patch) and the wave behind them (65 generated
      rows). **Select pool 0.** Decisions in **D328**.
      ⚠️ **Measured first:** the leftover probe over the database after D327, with the activation limits stripped off the payload (they were not part of it): 191 blocked single-face permanents whose every leftover line the grammar reads (135 before the strip); the row maker took 65 and refused 126 - 99 for costs it still cannot price (exile this card from your graveyard 15, a creature card from your graveyard 7, two or three cards from your graveyard 6 - an activation from the graveyard the engine does not offer; snow mana 9; remove a +1/+1 counter from a creature you control 5; tap an untapped Gate 3; return a land you control 3; sacrifice a Desert 3) and 15 for counts outside the vocabulary.
      ⚠️ **The seams:** `ActivatedAbility.oncePerTurn` + `TurnState.activations`
      (each `source|abilityRef` counted on `AbilityPutOnStack`, cleared by
      `TurnBegan`; the handler refuses a second activation, `legalActions`
      stops offering it); `discardCost.atRandom` (the activation names no card;
      `finishAbility` draws the discard off the seeded rng at payment, the rng
      on the accept as a coin flip's is); `PermanentPredicate.token`
      (`predicatesOf('token')`, the sacrifice chooser checks `isToken`). The row
      maker (`d328/make-rows18.cjs` + `gen-oneshot18.cjs`, from D327) strips the
      once-each-turn / sorcery-only sentence and marks the row, reads a random
      discard as a cost with no fixture, keeps the Saproling fodder for
      "sacrifice a token"; the suites prove the second activation refused, the
      hand one lighter, the token gone. NOT this decision: the graveyard-exile costs (exile this card / a creature card / two or three cards from your graveyard - 28 lines; an activation from the graveyard the engine does not offer), snow mana ({S} - 9), remove a counter from another permanent (6), tap an untapped Gate or Desert (4), return a land you control (3), the counts outside the vocabulary (15); then can not be countered (11 whole cards), the monarch, attacks each combat if able, the modal seam (42), the another split. Refused by
      name: 13 rows retired by name - the once-each-turn pumps, the random discards and the three token sacrifices the ledger held for a cost or a limit the engine charges now (Fountainport, Glimmer Bairn, Hardened Tactician among them); the count is MEASURED this decision (the REFUSED map's size) - the carried figure of 1,038 had drifted 9 above the map.
      Nothing retired. Report `effect:auto` 3,918 → 3,918,
      `withUnenforced` 280 → 280.
      **Landed:** no auto flips and 65 generated rows in two passes - 65 on the first with two suites red (Bloodsoaked Altar: one Grizzly Bears in the deck served both the discard and the sacrifice, so the discard's put moved the fodder off the battlefield; Ice Cream Kitty: another creature or token read as ONE token predicate), 65 green on the second (one more deck copy for a fixture a discard shares with a fodder or a tapper - fix-gen18-1; the token word read per or-alternative - apply-token-alternatives.cjs). The wave IS the landing: 65 rows - 33 activate-only-once-each-turn pumps and counters (Twinblade Slasher, Rootwalla, Putrid Leech, Savage Knuckleblade, Kraven's Cats, Wolfsbane, Highland Hero among them), 19 sorcery-speed activations the row maker had refused for the sentence alone (Bloodsoaked Altar, Ice Cream Kitty, Savageborn Hydra, Volrath's Gardens, Intrepid Tenderfoot), the 10 random discards (Mage il-Vec, Frenetic Ogre, Canyon Drake, Coral Helm, Pardic Swordsmith, Stormbind, Amok, Dwarven Strike Force, Ogre Shaman, Pardic Lancer) and the three token sacrifices D327 ledgered (Fountainport, Glimmer Bairn, Hardened Tactician); 41 of the 75 abilities are self pumps, then counters on a target (6), tokens (5), self counters (4), target pumps (4), damage (3), draws (3). Tests: the parser test; one generated
      suite per row.
...
- [x] **M6.4ft — THE GATE AT SCALE: the unit gate stops loading the whole library
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 4,203 files, 21,137 passing / 11 skipped ·
      ...
      the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4fu — THE TURN MEMORY AND THE UNCOUNTERABLE: per-turn spell and draw
      tallies behind "your second spell / your second card each turn", and "This
      spell can't be countered" read by the one funnel every counter passes
      through; 33 cards land, 33 as generated rows (2026-09-06):**
      **6,669 of 31,692 Commander-legal cards now execute completely, up
      from 6,636 (+33).** `SHIPPED_SCRIPTS` 4,116; ledger
      993 (5 rows retired - Mischievous Mystic, Mad Ratter, Prince Imrahil the Fair, Clarion Spirit and Jori En, Ruin Diver, the second-card and second-spell classes; measured: the REFUSED map's size). Fixtures 4,488 (4,350 by name + 131 tokens: the rowed cards, a Divination for the second-card tests, and two token pins (the Ape Villain, the Rat)). **Select pool
      0.** Decisions in **D336**.
      ⚠️ **The turn memory** (`d336/apply-turn-memory.cjs`): `TurnState.spellsCast`
      and `TurnState.cardsDrawn`, per player, empty as a turn begins; the reducer
      counts a `SpellCast` on its controller (an ability on the stack is not a
      spell) and a `DrewCards` by the cards it names. The second-spell head reads
      a tally of exactly 2 after the event; the second-card head a tally that
      crossed 2 with this event (a draw step is one card, a Divination two).
      ⚠️ **The uncounterable** (`CardScript.cantBeCountered`): a claim the
      accounting counts and the replacement funnel consults at its head - a
      `SpellCountered` whose spell claims the line becomes "X can't be countered."
      and the move off the stack is dropped with it; every counter in the engine,
      the resolver's `counter` effect and the twenty-odd scripts that emit the
      event themselves, passes through that funnel, which is why the rule lives
      there and nowhere else. Tests `turnMemory.test.ts`, `cantBeCountered.test.ts`.
      ⚠️ **The generator** (`d336/make-gen26.cjs`): the two heads and the static
      kind (in hand, cast, then Counterspell at the spell once the opponent holds
      priority). Lessons: helper spells go to hand BEFORE the baseline (`put()`
      finds an opening-hand copy and adds nothing); a second-card row enters
      early. Measured first: the leftover probe over the database after D335, with the two turn-memory heads and the uncounterable line as payloads: 161 blocked single-face permanents whose every leftover line the grammar reads - the 126 D335 left and 35 new (13 can't-be-countered whole cards, 17 second-card, 5 second-spell); the row maker took 33 and refused 128: the 126 as before, Pearl Lake Ancient (return three lands you control - an unpriced cost) and Toski, Bearer of Secrets (a must-attacker beside a head whose test attacks with another creature). Across the whole leftover database the line was the FIRST blocker of 43 cards and the two heads of 37 + 26; the rest carry other blockers. NOT this decision: snow mana ({S} - 12 across three cost forms; the snow source concept in the payment problem), remove a +1/+1 counter from a creature you control (5), sacrifice a Desert / tap an untapped Gate (6), return a land you control (3, and Pearl Lake Ancient's three), tap two untapped artifacts and/or creatures (3), the counts outside the vocabulary (15), can't attack or block alone (5; the second declaration requirement), a must-attacker beside an attacking head (Toski); then the quoted abilities enchanted creatures and lands have (60 + 34 first blockers), Enchant player (42), the look at the top card of your library (38), the modal seam (276), the another split. Refused by
      name: 5 rows retired - Mischievous Mystic, Mad Ratter, Prince Imrahil the Fair, Clarion Spirit and Jori En, Ruin Diver, the second-card and second-spell classes; measured: the REFUSED map's size.
      Nothing retired. Report `effect:auto` 3,918 → 3,918,
      `withUnenforced` 280 → 280.
      **Landed:** no auto flips; 33 generated rows, green after two generator corrections (the helper spells go to hand BEFORE the baseline is measured - put() finds a card already in the opening hand and moves it hand-to-hand, adding nothing, which Jori En's draw exposed; a second-card row enters early, so a combat kind beside it is past summoning sickness on turn 3 - Red Ghost) and one refusal (Toski, Bearer of Secrets: a must-attacker beside a head whose test attacks with another creature). The wave IS the landing: 33 rows - 11 spells that can't be countered (Tyrranax Rex, Mistcutter Hydra, Carnage Tyrant, Skylasher, Akroma, Angel of Fury, Scragnoth, Great Sable Stag, Terra Stomper, Blurred Mongoose, Thrun, the Last Troll, Hit-Monkey), 17 that read your second card each turn (Mystic Skyfish, Improbable Alliance, Faerie Vandal, Knights of Dol Amroth, Atlantean Cavalry, Irencrag Pyromancer, Mischievous Mystic, Bloodhaze Wolverine, Red Ghost, Intangible Genius, Mad Ratter, Steelgaze Griffin, Lat-Nam Adept, Prince Imrahil the Fair, Knowledge Seeker, Erudite Wizard, Loxodon Eavesdropper, Blue Marvel, Adam Brashear) and 5 that read your second spell (Thunder Drake, Jori En, Ruin Diver, Malcolm, the Eyes, Clarion Spirit, Illvoi Operative). Tests: the two engine suites; one
...
- [x] **M6.4fv — THE WIDER HEADS: "another creature dies" and "a creature enters"
      ...
      **Verified: `verify.cjs --full --scope` (sharded) — ALL FIVE GATES: scoped: 120 files outside the card suites, 10,095 passing / 11 skipped, plus the wave's 41 suites, 107 passing ·
      ...
      the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4fw — RAMPAGE, THE DEFENDER'S BOARD, AND THE BUSHIDO THAT WAS ALREADY
      THERE: Rampage as a trigger row, a combat restriction that reads the defender
      (CR 508.1c), and the accounting taught the numbered keyword the engine has run
      since D308; 39 cards land, 25 as generated rows (2026-09-06):**
      **6,749 of 31,692 Commander-legal cards now execute completely, up
      from 6,710 (+39).** `SHIPPED_SCRIPTS` 4,182; ledger
      991 (no rows retired - none of the 25 was in the ledger; measured: the REFUSED map's size). Fixtures 4,554 (4,416 by name + 131 tokens: the 25 rowed cards; the first port's 18 Bushido rows were unlanded, and the unland took Kitsune Blademaster's fixture with them although the D308 keyword-trigger test had it first (re-added; the fixture DB counts it once); no new token pins). **Select pool
      0.** Decisions in **D338**.
      ⚠️ **The defender's board** (`d338/apply-attack-defender.cjs`):
      `CombatDef.canAttackDefender?(ctx, self, candidate, defender)`, asked of each
      attacker/defender pair at the declaration after `canAttack` admitted the
      creature; the prompt's attackers list stays defender-free; the handler
      refuses the pair by name; a creature no legal defender admits is not able to
      attack, so no requirement asks it (the handler's check and the prompt's
      `required` filter by it). Test `attackDefender.test.ts`.
      ⚠️ **The Bushido that was already there:** the first port's 18 Bushido rows
      each DOUBLED the pump - the engine has run bushido from D308's keyword-trigger
      table; only the accounting missed its numbered line ("bushido 1" asked of
      `canonicalKeyword` whole). `d338/apply-bushido-accounting.cjs`:
      `NUMBERED_TRIGGER_KEYWORDS` (bushido) accounted like the bare keyword; those
      cards are complete with no script; the rows were unlanded.
      ⚠️ **The generator** (`d338/make-gen28.cjs`): Rampage as a becomes-blocked
      pump read off the combat state (+N per blocker beyond the first), tested
      blocked by two; the defender-read restriction as a combat kind over a
      per-module helper (subtype, flying, colour), tested refused then accepted
      once the defender's gift is on the board.
      Measured first: the leftover probe over the database after D337, with the defender-read restriction, the old combat keywords and the any-colour mana line keyed by its cost as payloads: 198 blocked single-face permanents whose every leftover line the grammar reads - the 136 D337 left and 62 new (can't attack unless defending player controls: an Island 11, a creature with flying 1, a blue permanent 1, an enchantment or an enchanted permanent 1; Bushido 18 whole of 33; Rampage 8; the any-colour line 24 whole - none with a bare {T}, those are engine-native already - behind a priced or chooser cost: tap an untapped creature you control 7, {1} 4, and a tail of one each); the row maker took 39 and refused 159 - the 136 as before, Godhunter Octopus (the enchantment predicate) and the any-colour costs. Then the port showed Bushido was engine-native (every row doubled the pump), and the second pass after the accounting fix read 184 candidates and took 25. NOT this decision: the any-colour mana line behind a priced or chooser cost (24 whole: tap an untapped creature you control 7, {1} 4, and a tail - the mana-ability half of the payment problem), deals damage to a player / an opponent (4; a def per damage event), blocks or becomes blocked by a colour or a type (4), the enchantment-or-enchanted defender predicate (1), snow mana ({S} - 12), remove a +1/+1 counter from a creature you control (5), sacrifice a Desert / tap an untapped Gate (6), return a land you control (3 + 3), tap two untapped artifacts and/or creatures (3), the counts outside the vocabulary (15), can't attack or block alone (5), a must-attacker beside an attacking head (1); then the quoted abilities enchanted creatures and lands have (60 + 34 first blockers), Enchant player (42), the look at the top card of your library (38), the modal seam (276), the another split. Refused by name:
      no rows retired - none of the 25 was in the ledger; measured: the REFUSED map's size.
      Nothing retired. Report `effect:auto` 3,918 → 3,918,
      `withUnenforced` 280 → 280.
      **Landed:** no auto flips; the first port's 39 rows were 18 Bushido doublings (unlanded - the engine had run the keyword all along) beside 21 green; the second port's 25 rows green in one pass. The wave IS the landing: 25 rows - 8 with Rampage (Craw Giant, Aerathi Berserker, Hunding Gjornersen, Balduvian War-Makers, Marhault Elsdragon, Frost Giant, Wolverine Pack, Horrible Hordes), 13 that can't attack unless the defending player controls a thing (an Island for eleven, a creature with flying, a blue permanent: Slipstream Eel, Sea Monster, Wu Warship, Armored Galleon, Hammerhead Shark, Ethereal Whiskergill, Lurking Green Dragon, Zhou Yu, Chief Commander, Whimwader, Red Cliffs Armada, Steam Frigate, Deep-Sea Serpent, Serpent of the Endless Sea) and 4 Bushido cards whose other line the grammar reads (Kuro's Taken, Battle-Mad Ronin, Numai Outcast, Cursed Ronin) - and 14 more cards complete with no script at all, their only leftover the Bushido line the accounting reads now. Tests: the engine suite; one generated
      suite per row.
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 4,305 files, 21,583 passing / 11 skipped ·
      ...
      ⚠️ **Reportables** (D338): the any-colour mana line behind a priced or chooser cost (24 whole: tap an untapped creature you control 7, {1} 4, and a tail - the mana-ability half of the payment problem), deals damage to a player / an opponent (4; a def per damage event), blocks or becomes blocked by a colour or a type (4), the enchantment-or-enchanted defender predicate (1), snow mana ({S} - 12), remove a +1/+1 counter from a creature you control (5), sacrifice a Desert / tap an untapped Gate (6), return a land you control (3 + 3), tap two untapped artifacts and/or creatures (3), the counts outside the vocabulary (15), can't attack or block alone (5), a must-attacker beside an attacking head (1); then the quoted abilities enchanted creatures and lands have (60 + 34 first blockers), Enchant player (42), the look at the top card of your library (38), the modal seam (276), the another split; the modal seam; the "another" split;
      the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4fx — THE THRESHOLD PHRASINGS AND BATTLE CRY: the Threshold read widened
      to "this creature" and the reversed order, and Battle cry as an attacks
      trigger row; a generator-only wave; 21 cards land, 21 as
      generated rows (2026-09-06):**
      **6,770 of 31,692 Commander-legal cards now execute completely, up
      from 6,749 (+21).** `SHIPPED_SCRIPTS` 4,203; ledger
      991 (no rows retired - none of the 21 was in the ledger; measured: the REFUSED map's size). Fixtures 4,576 (4,437 by name + 132 tokens: the 21 rowed cards and one new token pin (the Squirrel, for a Threshold card's token payload)). **Select pool
      0.** Decisions in **D339**.
      ⚠️ **The generator** (`d339/make-gen29.cjs`): the Threshold read accepts "this
      creature" as the subject and the reversed order (the same kind, def, fire
      and asserts as D335's); Battle cry is an attacks trigger row whose resolve
      pumps each other attacking creature +1/+0 off the combat state, tested
      attacking beside the Bears. No engine change; the gate ran the whole unit
      suite (4,326 files, 21,675 passing / 11 skipped).
      Measured first: the leftover probe over the database after D338, with Battle cry, every Threshold phrasing (its tail recorded), Raid's head and the Renown / Unleash / Mentor lines as payloads: 259 blocked single-face permanents whose every leftover line the grammar reads - the 159 D338 left and 100 new: Threshold lines the read missed 60 (29 with this creature as the subject, 4 in the reversed order, and a long tail of other bodies), Unleash 10, Renown 11, Mentor 8, Battle cry 4, Raid 7; the row maker took 21 (17 Threshold pumps and grants in the two phrasings, 4 Battle cry) and refused 238 - the 159 as before, 43 Threshold lines whose bodies are outside the read (activated abilities under Activate only if, enchanted-creature and anthem bodies, can't be blocked, a leftover sentence the printed line does not carry whole), and the four keyword mechanics measured for the decision after. NOT this decision: Renown (11 whole; a renowned flag on the instance), Unleash (10; an asked entry choice and a can't-block-with-a-counter def), Raid's enters head (7; an attacked-this-turn turn memory), Mentor (8; target attacking creature with lesser power), the other Threshold bodies (43), the any-colour mana line behind a priced or chooser cost (24), deals damage to a player / an opponent (4), blocks or becomes blocked by a colour or a type (4), snow mana ({S} - 12), remove a +1/+1 counter from a creature you control (5), sacrifice a Desert / tap an untapped Gate (6), the counts outside the vocabulary (15), can't attack or block alone (5); then the quoted abilities enchanted creatures and lands have (60 + 34 first blockers), Enchant player (42), the look at the top card of your library (38), the modal seam (276), the another split. Refused by name:
      no rows retired - none of the 21 was in the ledger; measured: the REFUSED map's size.
      Nothing retired. Report `effect:auto` 3,918 → 3,918,
      `withUnenforced` 280 → 280.
      **Landed:** no auto flips and 21 generated rows in one pass, all green. The wave IS the landing: 21 rows - 17 Threshold creatures whose line the read had missed (Springing Tiger, Werebear, Cabal Initiate, Mystic Enforcer, Mystic Zealot, Mystic Crusader, Nimble Mongoose, Anurid Barkripper, Krosan Beast, Mystic Visionary, Seton's Scout, Billowing Shriekmass, Battlewise Aven, Crashing Centaur, Dreadwing Scavenger, Metamorphic Wurm, Mystic Penitent) and 4 with Battle cry (Goblin Wardriver, Kuldotha Ringleader, Loxodon Partisan, Accorder Paladin). Tests: one generated suite per row.
...
- [x] **M6.4ga — THE ACTIVATION CONDITIONS: "Activate only <condition>" read by a
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 4,402 files, 21,998 passing / 11 skipped ·
      ...
      ⚠️ **Reportables** (D342): the turn-memory conditions (a creature died this turn 2 + 1, a noncreature spell / an instant or sorcery cast this turn 2 + 2, an opponent lost life this turn 2, you gained life / discarded / created a token this turn - a per-turn record beside D336's tallies), the keyword-predicate condition (a creature with flying 2; a keyword on PermanentPredicate), the exactly-N hand size (1), the counts outside the vocabulary (15), snow mana ({S} - 12), remove a +1/+1 counter from a creature you control (5), sacrifice a Desert / tap an untapped Gate (6), the other Threshold bodies (activated lines under other words, enchanted-creature and anthem bodies), Unleash (10; an asked entry choice), Goblin Banneret's Mentor; then the script-raised prompt seam itself - the modal spells (276), the exploits, the searches, the choices on entry - the quoted abilities enchanted creatures and lands have (60 + 34), Enchant player (42), the look at the top card of your library (38), the another split; the modal seam; the "another" split;
      the by-name sacrifice cost; the remaining cost verbs; the prompt
      continuation seam; prior items stand.
- [x] **M6.4gb — THE MODAL SEAM: a spell that says "Choose one —" reads mode by
      mode — the choice asked at the cast, the activation and the stacking of a
      trigger, a mode offered only while its targets can be chosen — and CR
      608.2b's other half at resolution; 80 cards land, 0 as
      generated rows (2026-09-07):**
      **6,920 of 31,692 Commander-legal cards now execute completely, up
      from 6,840 (+80).** `SHIPPED_SCRIPTS` 4,273; ledger
      991 (no rows retired - none of the 80 was in the ledger (d343/modal-ledger-check.cjs: 0 of the 80 names in REFUSED; a modal spell had been a SPELL_STRUCTURAL refusal at the classifier, never a drafter's verdict); measured: the REFUSED map's size). Fixtures 4,650 (4,510 by name + 133 tokens: the seam's three proof spells (Crushing Canopy, Dawn to Dusk, Blue Elemental Blast); no new token pins). **Select pool
      0.** Decisions in **D343**.
      ⚠️ **The modal face** (`d343/apply-modal-1.cjs`, `src/data/modalParse.ts`):
      a face whose WHOLE text is a "Choose one / two / three / one or both / one
      or more / any number —" head over bullet lines is a `ModalFace` — each
      mode its own target clauses and its own `parseEffects`, the face's own
      targets and effects empty, auto iff every mode is auto (D90: a head beside
      another sentence stays unread). ⚠️ **The choice is a PROMPT and it comes
      FIRST** (`-2`, `-3`): a cast stage `'modes'` ahead of X and the targets
      (CR 601.2b), the same `chooseModes` prompt for an activation (602.2b) and
      for a TRIGGER as it stacks (603.3c; `TriggerDef.modes` / `ActivatedDef.modes`
      ride the bus); `ModesChosen` / `StackModesSet` on the log; a mode is
      offered only while `minimumLegalTargets` fills its clauses, a spell only
      while a mode can be chosen (`castAction` — D102's livelock prevented at
      the offer), a trigger with too few modes removed as it would stack
      (603.3d); the chosen modes' clauses alone feed X, the targets and the
      re-check (`modeSpecs`, `modalEffects`). The twentieth `Awaiting` kind,
      answered by the bot, the harness and the net driver by policy, drawn by
      `PromptBar` as one button per mode. ⚠️ **CR 608.2b's OTHER HALF, found
      by the seam's own proof** (`-4`): the re-check decided fizzle and then
      ran the effects over EVERY declared target — `aimOf` admits any zone —
      so a two-target destroy "destroyed" a target exiled in response (a card
      in two zones, caught by the invariants); `withStillLegalPicks` drops the
      picks no longer legal for their clause before the vocabulary runs.
      ⚠️ **And gate 197's first run found a SECOND hole, pre-existing** (`-5`,
      seed 306): an ACTIVATED ability's re-check asked the trigger def's
      clauses and, with none, ran the ability on a dead target — Mage il-Vec
      pinged its own corpse, damage on a graveyard card; the re-check reads
      the parsed face's clauses now, and the gate's failure names the events
      that last touched the offending ids (`activatedFizzle.test.ts`, red
      without the fix).
      ⚠️ **The one sweep red was the DISCLOSURE:** `tier3.ts` re-derived the
      effect note over the whole text and told the player the app does not run
      a spell the engine runs — it asks `parseModalFace` first now (D122's
      rule, the fifth time that file has learned it). Fuzz: `Crushing Canopy`
      a canary staple feeding a `modeChoices` floor at gate size. Tests
      `modalSpell.test.ts` (13), `modalTrigger.test.ts` (4),
      `activatedFizzle.test.ts` (3).
      Measured first: the D343 probe (d343/zz-modal-spells.node.test.ts -> modal-spells.json) over the database: 404 incomplete single-face Commander-legal instants and sorceries print a Choose one / two / three / one or both / one or more / any number head; 59 print it beside another sentence (the head is not the whole text), 80 print it as the whole text with every mode inside the effect vocabulary and every target clause confident, and 265 carry a mode outside the vocabulary or an unconfident clause (modal-spells.json names each mode text). D294's probe had counted 355 modal spells and 42 all-modes-auto over the vocabulary of that day; the sentences D295-D301 taught the vocabulary since are the difference. The 80 are ZERO in the REFUSED ledger (d343/modal-ledger-check.cjs). The parse report says what the seam did across the database: a modal face's clauses are its modes' own now - target:modalUnion 2,751 -> 1,569, target:unparsedClause 1,166 -> 1,234 and target:unparsedCount 554 -> 610 (mode lines read one by one expose the clauses the whole-text union had blurred), facesWithSpecs 19,757 -> 18,575, specs 20,840 -> 19,658, free 3,328 -> 2,146; effect:auto 3,918 -> 4,209, effect:none 15,115 -> 14,849, effect:partial 5,298 -> 5,273; the bot pool's spell columns instant 947 -> 1,007 and sorcery 723 -> 743, auto 868 -> 948; tier3 silentAfter 7,150 -> 7,230 (the 80, silent because the disclosure asks the same reader now). NOT this decision: the modal PERMANENT lines (276 leftover lines on permanents whose first unread line is a choose-one: a trigger or an activation whose payload is modal - TriggerDef.modes / ActivatedDef.modes ship in this seam, so the row maker naming a mode per printed bullet is the wave), the modal spells beside another sentence (59) and the 265 with a mode outside the vocabulary or an unconfident clause, Unleash (10; an asked entry choice), the turn-memory conditions (a creature died this turn 2 + 1, a noncreature spell / an instant or sorcery cast this turn 2 + 2, an opponent lost life this turn 2, you gained life / discarded / created a token this turn - a per-turn record beside D336's tallies), the keyword-predicate condition (a creature with flying 2; a keyword on PermanentPredicate), the exactly-N hand size (1), the counts outside the vocabulary (15), snow mana ({S} - 12), remove a +1/+1 counter from a creature you control (5), sacrifice a Desert / tap an untapped Gate (6), the other Threshold bodies (activated lines under other words, enchanted-creature and anthem bodies), Goblin Banneret's Mentor; then the rest of the script-raised prompt seam - the exploits, the searches, the choices on entry - the quoted abilities enchanted creatures and lands have (60 + 34), Enchant player (42), the look at the top card of your library (38), the another split. Refused by name:
      no rows retired - none of the 80 was in the ledger (d343/modal-ledger-check.cjs: 0 of the 80 names in REFUSED; a modal spell had been a SPELL_STRUCTURAL refusal at the classifier, never a drafter's verdict); measured: the REFUSED map's size.
      Nothing retired: no shipped spell def sat on a modal face (SpellDef v1 refused the choice), so the vocabulary reading the 80 whole retires no script. Report `effect:auto` 3,918 → 4,209,
      `withUnenforced` 280 → 280.
      **Landed:** 80 AUTO FLIPS with no script - the whole-text modal spells whose every mode the vocabulary reads and whose every target clause reads confident, complete by the accounting alone once the face reads mode by mode (Crushing Canopy, Dawn to Dusk, Valorous Stance, Mouser Attack!, Blue Elemental Blast, Crush Contraband, Casualties of War, Unforgiving Aim, Subtle Strike, Feast or Famine, Ojutai's Command, Rip Apart, Fissure Vent, Ready to Rumble, Abrade ... - the 80 named in d343/modal-names.json); no generated rows, and the one sweep red was the DISCLOSURE, not a card: tier3.ts re-derived the effect note by asking parseEffects over the whole text, which reads a Choose one face as manual and told the player the app does not run a spell the engine runs - it asks parseModalFace first now (d343/apply-tier3-modal.cjs), the same reader oracleParse uses; and gate 197's first run went RED on the fuzz (seed 306: damage on a card in a graveyard) - a PRE-EXISTING hole the reshaped pool surfaced, an activated ability's CR 608.2b re-check with no clauses to ask (apply-modal-5.cjs; activatedFizzle.test.ts red without it, green with it); gate 198 is the landing's gate. No wave of rows: the seam's own flips are the landing, and the bot's own deck took 60 instants and 20 sorceries more on the same sweep (auto 868 -> 948; Abrade, a choose-one, displaced Alesha's Legacy; the commander reaches 6,861 cards from 6,781). Tests: the two engine suites; no
      generated rows.
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 4,405 files, 22,020 passing / 11 skipped ·
      ...
      ⚠️ **Reportables** (D343): the modal PERMANENT lines (276 leftover lines on permanents whose first unread line is a choose-one: a trigger or an activation whose payload is modal - TriggerDef.modes / ActivatedDef.modes ship in this seam, so the row maker naming a mode per printed bullet is the wave), the modal spells beside another sentence (59) and the 265 with a mode outside the vocabulary or an unconfident clause, Unleash (10; an asked entry choice), the turn-memory conditions (a creature died this turn 2 + 1, a noncreature spell / an instant or sorcery cast this turn 2 + 2, an opponent lost life this turn 2, you gained life / discarded / created a token this turn - a per-turn record beside D336's tallies), the keyword-predicate condition (a creature with flying 2; a keyword on PermanentPredicate), the exactly-N hand size (1), the counts outside the vocabulary (15), snow mana ({S} - 12), remove a +1/+1 counter from a creature you control (5), sacrifice a Desert / tap an untapped Gate (6), the other Threshold bodies (activated lines under other words, enchanted-creature and anthem bodies), Goblin Banneret's Mentor; then the rest of the script-raised prompt seam - the exploits, the searches, the choices on entry - the quoted abilities enchanted creatures and lands have (60 + 34), Enchant player (42), the look at the top card of your library (38), the another split; the "another" split; the by-name
      sacrifice cost; the remaining cost verbs; the prompt continuation seam;
      prior items stand.
- [x] **M6.4gc — THE VOCABULARY PAYLOAD: a generated row hands the engine its
      own effect vocabulary as the payload of a trigger or an activation — the
      printed sentence parsed once at module load, the clauses asked as the
      ability stacks, the picks still legal resolved through the executor a
      spell uses; 133 cards land, 133 as generated rows (2026-09-07):**
      **7,053 of 31,692 Commander-legal cards now execute completely, up
      from 6,920 (+133).** `SHIPPED_SCRIPTS` 4,406; ledger
      958 (35 rows retired by name (the port's remove-ledger-rows: the wave's cards whose refusal had been a payload the row kinds did not read), and two rows added back - Kithkin Spellduster (a self-sacrifice on a creature that returns: persist) and Failed Conversion (an attached static whose -4/-4 empties the suite's 2/2 Bears) - which the port's first run had landed and the generator now refuses by name; measured: the REFUSED map's size 991 -> 958). Fixtures 4,783 (4,643 by name + 133 tokens: the 133 rows' cards (4,510 -> 4,643); no new token pins (every token a payload creates was pinned by the spells that create it)). **Select pool
      0.** Decisions in **D344**.
      ⚠️ **The seam** (`d344/apply-vocab-1.cjs`, `-1b`,
      `src/engine/scripts/vocabulary.ts`): `ScriptCtx.vocabulary(obj, effects,
      targets)` runs `withStillLegalPicks` (CR 608.2b's other half, D343) then
      `effectEvents` — the executor a spell uses — over the ability's own
      targets, controller and source; the other two ctx builders carry a
      throwing stub (resolution-only). A def reaches it ONLY through
      `vocabularyEffects(payload, name)` / `vocabularyTargets(payload)`, which
      parse the printed payload once at module load and THROW BY NAME: not
      `auto`, "at random" (no RNG on a def's events), a clause that asks (the
      continuation seam), a self clause of an aim-needing kind, a clause not
      confident (D90). Proof `vocabularyPayload.test.ts` (7): the prompt as the
      trigger stacks, a no-clause payload, a pick exiled in response skipped
      while the other resolves, every pick gone, the refusals.
      ⚠️ **The generator** (`d344/make-gen33.cjs` + `gen33-vocab.cjs`): the row
      maker reads a payload outside the row kinds through the `-vocab` probe's
      dump as a `vocab` row kind (the parsed effects and clauses) and refuses
      what the suite cannot fixture or assert; the module declares the two
      helpers once per line and resolves through `ctx.vocabulary`; the suite
      puts ONE FIXTURE PER CLAUSE on the side the clause names, with a name
      the row does not already put there (`put` finds by name: Cauldron of
      Essence's graveyard Bears was carried off by a pinned Bears), waits for
      the prompt (a step head's fire no longer advances past it), and asserts
      each effect's mark, then the hand, the board and the life. ⚠️ The
      port's first run found the generator's own gaps with real rows (134
      rows, 11 red — an unused helper, one line two defs, a
      player-or-planeswalker clause, the step heads' prompt, a token row under
      a cast head, an Aura leaving the hand, PERSIST after a self-sacrifice,
      a -4/-4 on the 2/2 Bears, the stolen fixture); every one a generator
      change, no card hand-fixed.
      Measured first: the D344 probes over the database (d344/make-probe-d344.cjs -> zz-probe-d344c: D343's grammar over the 18,248 incomplete single-face permanents; make-probe-d344v.cjs -> zz-probe-d344v: the same with ONE fallback - a payload the row kinds do not read is offered to the engine's effect vocabulary): (1) the modal PERMANENT lines D343 named next - 276 print a modal head, 10 fully readable at the row kinds, 33 with the vocabulary fallback, 235 distinct first-blocker reasons behind the rest (modal-why.json): a wave of ten, so it waits; (2) EVERY incomplete card's kinds and first blocker dumped (all-cards.json), so a seam is sized offline before it is built - the first blockers are PAYLOADS by a wide margin (an activated payload 2,982, the enters head's 2,483, upkeep 483, attacks 356, the end step 206 ...; a static line outside every grammar 6,706; a head outside the library 2,181); (3) the vocabulary fallback: 274 distinct payload sentences the effect vocabulary reads whole with every clause confident (592 lines across the database; 18 ask, 2 use randomness, 221 target), and the probe's fully-covered cards 273 -> 601. D342's row maker over the 601 (make-rows33.cjs, unpatched): ZERO rowable - 88 cost verbs, 48 static lines, 39 activation conditions, 28 threshold or modal lines the accounting names but the card does not print; with the vocab kind: 172 rows (166 with a vocabulary ability) before the suite's own refusals, 134 after them (33 clauses with no fixture - combat-role 8, counted 7, tapped 3, nonbasic land 2, an Aura, a legendary card, a black card ...; 5 counter-a-spell payloads), 133 after the port's first run (Kithkin Spellduster's persist and Failed Conversion's -4/-4 refused by name; Xiahou Dun, the One-Eyed rowable once a graveyard clause with no card type reads as a creature card). NOT this decision: the vocabulary payloads the suite cannot fixture or assert yet (a counted clause - up to one, up to two - aimed at one fixture; a combat-role clause fired mid-combat; a tapped or nonbasic-land predicate; a spell clause for the 5 counter payloads; the 18 asking payloads - discard, scry, surveil, look at the top - once the continuation seam exists), the modal PERMANENT lines (276; 33 readable with the vocabulary), the cost verbs the probe's covered set names (121 - snow mana {S} 12, remove a counter from a creature you control 6, a Desert or a Gate to sacrifice or tap 7, return a land you control 6, tap two untapped artifacts and/or creatures 3, Blight, Exert, Mill four cards ...), the activation conditions (49 - the turn-memory ones first: an opponent lost life, a creature died, a noncreature or an instant or sorcery spell cast this turn), the static lines (58 - Unleash 10, the Threshold bodies 25, attacking creatures you control get +1/+0 or have double strike 9, All Slivers have 2), Add one mana of any color with a cost the engine does not charge (21), the counts outside the vocabulary (15), the 12 tokens outside TOKEN_TABLE; then the rest of D343's list - the modal spells beside another sentence (59) and the 265 with a mode outside the vocabulary, the exploits, the searches, the choices on entry, the quoted abilities enchanted creatures and lands have (60 + 34), Enchant player (42), the look at the top card of your library (38), the another split. Refused by name:
      35 rows retired by name (the port's remove-ledger-rows: the wave's cards whose refusal had been a payload the row kinds did not read), and two rows added back - Kithkin Spellduster (a self-sacrifice on a creature that returns: persist) and Failed Conversion (an attached static whose -4/-4 empties the suite's 2/2 Bears) - which the port's first run had landed and the generator now refuses by name; measured: the REFUSED map's size 991 -> 958.
      Nothing retired: no shipped def sat on any of the 133 - the vocabulary reads their payloads for the first time. Report `effect:auto` 4,209 → 4,209,
      `withUnenforced` 280 → 280.
      **Landed:** No second wave: the rows are the landing, and the bot's own deck took 106 creatures, 18 artifacts and 9 enchantments more on the same sweep (the commander reaches 6,994 cards from 6,861; 191 fully-executable legendary creatures from 186). Tests: the engine suite (7) and one generated
      suite per row (133 GENERATED ROWS with a vocabulary payload (the kinds the rows carry: destroy 36, returnFromGraveyard 23, reanimate 15, pump 15, untap 10, damage 9, tap 7, bounce 6, draw 5, putCounters 4, loseLife 3, createToken 3, noop 2, gainLife 1): Kor Entanglers, Noxious Dragon, Treasure Hunter, Eternal Witness, Sun Titan, Spine of Ish Sah, Riddlemaster Sphinx, Primeval Bounty, Auramancer, Gravedigger, Surly Farrier, Quarry Beetle, Runaway Boulder, Order of Whiteclay, Stoic Builder, Conclave Naturalists, Hell's Caretaker, Dreadbringer Lampads, Necromantic Thirst, Woodfall Primus, Cauldron of Essence, Graveshifter, Reclamation Sage, Clock of Omens ... - the 133 named in d344/generated-oneshot-d344-names.json, each with its own suite (one fixture per clause, the prompt answered as the ability stacks, every effect asserted)).
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 4,539 files, 22,604 passing / 11 skipped ·
      ...
      ⚠️ **Reportables** (D344): the vocabulary payloads the suite cannot fixture or assert yet (a counted clause - up to one, up to two - aimed at one fixture; a combat-role clause fired mid-combat; a tapped or nonbasic-land predicate; a spell clause for the 5 counter payloads; the 18 asking payloads - discard, scry, surveil, look at the top - once the continuation seam exists), the modal PERMANENT lines (276; 33 readable with the vocabulary), the cost verbs the probe's covered set names (121 - snow mana {S} 12, remove a counter from a creature you control 6, a Desert or a Gate to sacrifice or tap 7, return a land you control 6, tap two untapped artifacts and/or creatures 3, Blight, Exert, Mill four cards ...), the activation conditions (49 - the turn-memory ones first: an opponent lost life, a creature died, a noncreature or an instant or sorcery spell cast this turn), the static lines (58 - Unleash 10, the Threshold bodies 25, attacking creatures you control get +1/+0 or have double strike 9, All Slivers have 2), Add one mana of any color with a cost the engine does not charge (21), the counts outside the vocabulary (15), the 12 tokens outside TOKEN_TABLE; then the rest of D343's list - the modal spells beside another sentence (59) and the 265 with a mode outside the vocabulary, the exploits, the searches, the choices on entry, the quoted abilities enchanted creatures and lands have (60 + 34), Enchant player (42), the look at the top card of your library (38), the another split; the "another" split; the by-name
      sacrifice cost; the remaining cost verbs; the prompt continuation seam;
      prior items stand.
- [x] **M6.4gd — THE MODAL PERMANENT LINES: a trigger or an activation whose
      printed payload is "choose one —" over bullets becomes ONE def with modes,
      ...
      **7,077 of 31,692 Commander-legal cards now execute completely, up
      ...
      `targetParse.ts`): `parseSpellTargets` tested the modal head against the
      WHOLE text, so a permanent whose "choose one" sits on a triggered or an
      activated line carried a cast-time free spec for a cast that aims nothing
      — reported as the leftover "Choose a mode, then its target" on all 276
      modal permanents, which NO script could claim: D343 shipped the seam, the
      24 rows shipped defs with `modes`, and the disclosure still refused every
      one (said by name, 24 times, by `shippedScripts.node.test.ts`). The union
      is a SPELL line's; a modal head on a permanent's own ability is that
      def's, and the printed lines behind it are read now. `target:modalUnion`
      1,569 → 505, free specs 2,146 → 1,082.
      ⚠️ **The generator** (`d345/make-gen34.cjs` + `gen34-modal.cjs`): the row
      maker walks the leftover lines BY INDEX so a modal head takes its `• `
      bullets, and a head or an activated line whose payload is "choose one —"
      becomes ONE ABILITY PER MODE, each carrying that mode's parsed effect (a
      row kind or D344's vocabulary) — so every existing emitter reads it and
      only the def and the prompt are new. One def per modal LINE with `modes`
      (the PRINTED bullet as `text`, the clauses from the payload) and
      `modeChoice`, resolving by the chosen mode. **Choose-one only**: one mode
      resolves, so the declared targets are its own clauses and no index shift
      is needed, and an asking mode inside a multi-mode resolution cannot
      arise. A head that is two defs declares its modes on both. The suite
      fires the head, answers `chooseModes`, then its targets. A FLAVOUR NAME
      before a payload ("Heckle — Tap target creature.") is stripped to read it
      and kept in the printed text the accounting claims.
      ⚠️ The port's runs found five generator gaps (a per-mode token constant,
      two vocabulary modes colliding, an ACTIVATED modal def with no `modes` at
      all, a mode naming its own card, and a derive behind a PIPE swallowing
      its own failure so a whole round regenerated with the old generator); no
      card was hand-fixed.
      Measured first: the D345 probe (d345/make-probe-d345.cjs -> zz-probe-d345v, D344's vocabulary probe plus the FLAVOUR NAME a mode prints before its payload - "Heckle — Tap target creature.", "Cure Wounds — You gain 2 life." - which carries no rules meaning, stripped to READ the payload and kept in the printed line the accounting claims): of the 276 incomplete permanents printing a modal head, 33 have every mode inside the row kinds or the effect vocabulary, 36 with the flavour strip (the strip alone is 14 cards' first blocker; the rest of those 14 are blocked further on). The blockers behind the other 240 (modal-why.json): 102 a mode payload outside both readers (Proliferate, Cloak, a copy token, a search, an X count ...), 21 a head outside the library beside 18 modal heads outside it (whenever you proliferate, whenever a Swamp you control enters, when this creature enters or dies), 29 another line entirely, and the rest the payload classes every other wave meets. The row maker rowed 24 of the 36: three modes ask (a scry inside a resolution is the continuation seam), one head is a choose ONE OR BOTH (D345 is choose-one), Flash Thompson's 1..2 the same, and Titanium Man is a Vehicle whose crew the engine does not charge. NOT this decision: the wider modal heads (one or both, one or more, any number, choose two - the def needs modalEffects' index shift, which the engine already has for spells), the asking modes (a scry or a surveil inside an ability's resolution: the continuation seam), the 102 mode payloads outside both readers, the 21 + 18 heads outside the library (whenever you proliferate, whenever a Swamp you control enters, when this creature enters or dies), and then D344's own list unchanged: the vocabulary clauses the suite cannot fixture (a counted clause, a combat-role clause, a tapped or nonbasic-land predicate, a spell clause for the counter payloads), the cost verbs (121 - snow mana, remove a counter, a Desert or a Gate, return a land), the activation conditions (49 - the turn-memory ones first), the static lines (58 - Unleash 10, the Threshold bodies 25), Add one mana of any color with a cost the engine does not charge (21), the counts outside the vocabulary (16), the tokens outside TOKEN_TABLE (12); then the modal spells beside another sentence (59) and the 265 with a mode outside the vocabulary, the exploits, the searches, the choices on entry, the quoted abilities enchanted creatures and lands have (60 + 34), Enchant player (42), the look at the top card of your library (38), the another split. Refused by name:
      no rows retired and none added - not one of the 24 was in the REFUSED ledger: a modal permanent had been unreachable at the ACCOUNTING (its face carried a cast-time target spec no script could claim), never a drafter's verdict; measured: the REFUSED map's size, unchanged at 958.
      Nothing retired: no shipped def sat on any of the 24, and no ledger row named one. Report `effect:auto` 4,209 → 4,209,
      `withUnenforced` 280 → 280.
      **Landed:** No second wave: the seam and its 24 rows are the landing, and the bot's own deck took 20 creatures and 4 enchantments more on the same sweep (the commander reaches 7,018 cards from 6,994; Elder Gargaroth displaced Allied Strategies). Tests: one generated suite per row, one test per
      mode.
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 4,563 files, 22,733 passing / 11 skipped ·
      ...
      ⚠️ **Reportables** (D345): the wider modal heads (one or both, one or more, any number, choose two - the def needs modalEffects' index shift, which the engine already has for spells), the asking modes (a scry or a surveil inside an ability's resolution: the continuation seam), the 102 mode payloads outside both readers, the 21 + 18 heads outside the library (whenever you proliferate, whenever a Swamp you control enters, when this creature enters or dies), and then D344's own list unchanged: the vocabulary clauses the suite cannot fixture (a counted clause, a combat-role clause, a tapped or nonbasic-land predicate, a spell clause for the counter payloads), the cost verbs (121 - snow mana, remove a counter, a Desert or a Gate, return a land), the activation conditions (49 - the turn-memory ones first), the static lines (58 - Unleash 10, the Threshold bodies 25), Add one mana of any color with a cost the engine does not charge (21), the counts outside the vocabulary (16), the tokens outside TOKEN_TABLE (12); then the modal spells beside another sentence (59) and the 265 with a mode outside the vocabulary, the exploits, the searches, the choices on entry, the quoted abilities enchanted creatures and lands have (60 + 34), Enchant player (42), the look at the top card of your library (38), the another split; the "another" split; the by-name
      sacrifice cost; the remaining cost verbs; the prompt continuation seam;
      prior items stand.
- [x] **M6.4ge — THE SCOPED ANTHEMS AND GRANTS: a continuous line over a NARROWER
      set is the same static with one more conjunct, and a true ability word
      before a trigger head is read past; 19 cards land, 19 as
      generated rows (2026-09-07):**
      **7,096 of 31,692 Commander-legal cards now execute completely, up
      from 7,077 (+19).** `SHIPPED_SCRIPTS` 4,449; ledger
      958 (no rows retired and none added - not one of the 19 was in the REFUSED ledger: a scoped continuous line had been outside the ROW MAKER's grammar, never a drafter's verdict; measured: the REFUSED map's size, unchanged at 958). Fixtures 4,829 (4,688 by name + 134 tokens: the 19 rows' cards and the two fixtures their scopes are read on (Ornithopter, an artifact creature; Metallic Sliver)). **Select pool
      0.** Decisions in **D346**.
      ⚠️ **No engine change:** the `StaticDef` layers and `appliesTo` already
      carried it; the decision is the generator learning to read a sentence the
      engine could already run — the cheapest measured class on the board (the
      probe read those lines, the row maker did not).
      ⚠️ **The scoped line** (`d346/gen35-scope.cjs`, `make-rows35.cjs`): the row
      maker splits the scope phrase from the body and parses the BODY by the
      canonical anthem regex ("Creatures you control " + body), so the body
      vocabulary stays in ONE place and a scope cannot widen it; a generic
      plural is guarded (Lands you control is not an anthem over creatures).
      The conjuncts: attacking reads the LIVE combat, untapped reads the card,
      the type/supertype/subtype scopes read the derived type line; an
      "All <Subtype>" scope drops the controller check.
      ⚠️ **The suite reads it on its OWN fixture** (Grizzly Bears, Ornithopter,
      Ant-Man Scott Lang, Metallic Sliver — the last two new), attacks with it
      where the scope says attacking and asserts at the declare-blockers
      prompt, and keeps the opponent's Cyclops as the control. **Two anthems on
      one card both land on that fixture**, so every anthem assert adds the
      row's other anthems (Rising of the Day).
      ⚠️ **A true ability word before a head** (`Flurry — Whenever you cast your
      second spell…`) was never tried, because the prefix was a hand-written
      list; the library is asked on the line and then past the word, as the cost
      parser already does. The def's text stays the whole printed line.
      Measured first: the D345 probe re-run over a tree 24 cards further on (d346/make-probe-d346.cjs; the grammar is unchanged, the candidate set is not): 447 fully covered cards, down from 471 as D345's modal permanents became complete. The row maker refused 428 of them, and its own histogram is the map of what is left: 121 a COST verb it cannot charge (snow mana {S} 12, remove a counter 6, a Desert or a Gate to sacrifice or tap 7, return a land 6, and a tail of some fifty singletons), 58 a line that is neither an activated ability nor a library trigger, 52 a trigger payload outside both readers, 49 an activation condition, 47 an effect outside the row kinds (21 of them Add one mana of any color, whose blocker is the COST beside it), 36 a vocabulary clause the suite cannot fixture, 16 a count outside the vocabulary, 13 a token outside TOKEN_TABLE. Inside the 58: Unleash 10 and Exploit 8 (both an asked choice on entry), 14 Threshold bodies the static grammar does not read, and the 17 SCOPED continuous lines this decision takes - which the PROBE already read (static:scopedAnthem 11, static:scopedGrant 7) while the row maker did not, so they were complete-but-unrowed, the cheapest measured class on the board. NOT this decision: the remaining Threshold bodies (14: an enchanted-creature body, an anthem body, a can't-be-blocked body, a CDA, and the intervening-if triggers - "whenever this creature attacks, IF there are seven or more cards in your graveyard" - which need a condition on the trigger's matches, not just the ability word stripped), Unleash (10) and Exploit (8) - both an asked choice as the creature enters, which is the entry-prompt seam - the cost verbs (121, snow mana first at 12), the activation conditions (49, the turn-memory ones first), the trigger payloads outside both readers (52), the vocabulary clauses the suite cannot fixture (36: a counted clause, a combat-role clause, a tapped or nonbasic-land predicate, a spell clause), the counts outside the vocabulary (16), the tokens outside TOKEN_TABLE (13), "Enchanted creatures you control" (1, a scope that asks what is attached); then D345's list unchanged: the wider modal heads (one or both, one or more, any number), the asking modes, the 102 mode payloads outside both readers, the heads outside the library, the modal spells beside another sentence (59), the exploits, the searches, the quoted abilities enchanted creatures and lands have (60 + 34), Enchant player (42), the look at the top card of your library (38). Refused by name:
      no rows retired and none added - not one of the 19 was in the REFUSED ledger: a scoped continuous line had been outside the ROW MAKER's grammar, never a drafter's verdict; measured: the REFUSED map's size, unchanged at 958.
      Nothing retired: no shipped def sat on any of the 19, and no ledger row named one. Report `effect:auto` 4,209 → 4,209,
      `withUnenforced` 280 → 280.
      **Landed:** No second wave: the 19 rows are the landing, and the bot's own deck took 9 creatures, 8 enchantments and 2 artifacts more on the same sweep (the commander reaches 7,037 cards from 7,018). Tests: one generated suite per row.
...
- [x] **M6.4gg — THE TURN RECORD: an activation that asks what this turn has
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 4,611 files, 22,947 passing / 11 skipped ·
      ...
      sacrifice cost; the remaining cost verbs; the prompt continuation seam;
      prior items stand.
- [x] **M6.4gh — THE WIDER VOCABULARY PAYLOAD: a payload the engine's own
      vocabulary reads is emitted under a trigger head, and a clause that asks
      may be the payload's last effect; 48 cards land, 48 as
      generated rows (2026-09-07):**
      **7,172 of 31,692 Commander-legal cards now execute completely, up
      from 7,124 (+48).** `SHIPPED_SCRIPTS` 4,525; ledger
      940 (12 rows RETIRED by name - the cards the ledger held for a payload the row maker could not emit, which the vocabulary has read all along - and none added; measured: the REFUSED map's size 952 -> 940). Fixtures 4,912 (4,770 by name + 135 tokens: the 48 rows' cards; no new token pin (every token these payloads make was pinned already)). **Select pool
      0.** Decisions in **D349**.
      ⚠️ **ONE ENGINE LINE; the rest is the generator reaching what the engine
      already does.** No reading is new: every payload here is a sentence
      `parseEffects` has read since D344.
      ⚠️ **The trigger re-read:** `parseEffect` claims "Tap target creature."
      and its kin as ACTIVATED row kinds the TRIGGER emitter has no case for, so
      29 payloads the vocabulary reads were refused as "not a pump". A payload
      whose kind the trigger emitter cannot emit is offered to the vocabulary
      now, and the allowlist is a named SET so the re-read and the refusal
      cannot drift apart.
      ⚠️ **The ask-last rule:** `vocabularyEffects` threw on ANY asking clause,
      for a good reason — `effectEvents` stops at an `AwaitingSet`, so a clause
      after the prompt would be dropped in silence (D90). With the ask LAST
      there is nothing to drop: that is D195's rule for spells, and how D285's
      targeted discards and D278's Necrogen Spellbomb have run from a trigger
      and an activation ever since. An ask anywhere but last still throws by
      name.
      ⚠️ **The suite answers it**: `AnswerChooseFromZone` for the player the
      clause names, then the cards are read into that player's graveyard. Scry
      and surveil stay refused — three cards — because the scaffold has no
      answer for their prompt yet, which is the honest reason to leave them.
      ⚠️ **A refusal must name the CAUSE, not the kind.** The port's one real
      red was an ENCHANTMENT with an attack head: the suite declares the row's
      own card as the attacker. Widening the refusal by kind ("an optional
      trigger that also targets") dropped 17 GREEN rows before the measurement
      caught it; the refusal that shipped is an attack head on a card with no
      creature body, and it costs exactly one card.
      Measured first: the D348 probe re-run over a tree 14 cards further on (the grammar is unchanged, the candidate set is not): 400 fully covered cards, down from 414 as the turn-record rows became complete. The row maker refused all 400 before this decision, and its histogram named where they sit: a COST it cannot charge 103 (across 72 distinct verbs - a long tail, the biggest family snow mana at 13), a TRIGGER PAYLOAD it has no case for 52, an EFFECT outside its row kinds 48, a vocabulary clause the suite cannot fixture 36, a line that is neither an activated ability nor a library trigger 36, an activation condition 34. Reading the two payload rows against the vocabulary dump settled the decision: of the 52 trigger payloads, 29 are sentences the vocabulary reads whole with every clause confident and no prompt; of the 100 payload refusals in both rows, 24 more are read whole and refused ONLY because they ask (21 discards, 3 that scry or surveil). Nothing else on the board came close: the biggest cost verb is 13 cards. NOT this decision: the three asking payloads the suite cannot answer yet (a scry, a surveil and a scry beside damage - the answer is `AnswerScry`, which the scaffold has for its own scry rows and not for a vocabulary one), the 20 trigger payloads still outside both readers (a graveyard return under a head, a look at the top cards, a conditional discard, a regenerate), the cost verbs the derivation cannot pay (104 across 72 verbs, the largest families snow mana {S} 13, a return-to-hand cost 12, a remove-a-counter-from-another-permanent cost 8, a counted sacrifice 7, an exile-self cost 5), the activation conditions outside the vocabulary (34), the vocabulary clauses the suite cannot fixture (36: a counted clause, a combat-role clause, a tapped or nonbasic-land predicate, a spell clause), the lines that are neither an activated ability nor a library trigger (36: Unleash 10, Exploit 8, the Threshold bodies), Add one mana of any color behind a cost the engine does not charge (23), the counts outside the vocabulary (16), the tokens outside TOKEN_TABLE (14), an attack head on a card with no creature body (1 - a scaffold that attacks with a creature the row did not put down); then D348's list unchanged: the modal permanent lines, the wider modal heads, the asking modes, the heads outside the library, the modal spells beside another sentence, the quoted abilities enchanted creatures and lands have, Enchant player, the look at the top card of your library. Refused by name:
      12 rows RETIRED by name - the cards the ledger held for a payload the row maker could not emit, which the vocabulary has read all along - and none added; measured: the REFUSED map's size 952 -> 940.
      12 ledger rows retired by name: cards refused for a payload the row maker had no case for, which the vocabulary reads. Report `effect:auto` 4,209 → 4,209,
      `withUnenforced` 280 → 280.
      **Landed:** No second wave: the 48 rows are the landing, and the POOL took 39 creatures, 6 enchantments and 3 artifacts more on the same sweep - the commander's reach rose to 7,113 cards from 7,065, chosen from 196 fully-executable legendary creatures (193 before). Tests: one generated suite per row.
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 4,659 files, 23,146 passing / 11 skipped ·
      ...
      ⚠️ **Reportables** (D349): the three asking payloads the suite cannot answer yet (a scry, a surveil and a scry beside damage - the answer is `AnswerScry`, which the scaffold has for its own scry rows and not for a vocabulary one), the 20 trigger payloads still outside both readers (a graveyard return under a head, a look at the top cards, a conditional discard, a regenerate), the cost verbs the derivation cannot pay (104 across 72 verbs, the largest families snow mana {S} 13, a return-to-hand cost 12, a remove-a-counter-from-another-permanent cost 8, a counted sacrifice 7, an exile-self cost 5), the activation conditions outside the vocabulary (34), the vocabulary clauses the suite cannot fixture (36: a counted clause, a combat-role clause, a tapped or nonbasic-land predicate, a spell clause), the lines that are neither an activated ability nor a library trigger (36: Unleash 10, Exploit 8, the Threshold bodies), Add one mana of any color behind a cost the engine does not charge (23), the counts outside the vocabulary (16), the tokens outside TOKEN_TABLE (14), an attack head on a card with no creature body (1 - a scaffold that attacks with a creature the row did not put down); then D348's list unchanged: the modal permanent lines, the wider modal heads, the asking modes, the heads outside the library, the modal spells beside another sentence, the quoted abilities enchanted creatures and lands have, Enchant player, the look at the top card of your library; the "another" split; the by-name
      sacrifice cost; the remaining cost verbs; the prompt continuation seam;
      prior items stand.
- [x] **M6.4gi — THE CLAUSE FIXTURES: the suite learns to put and aim four
      clauses the engine already read; 15 cards land, 15 as
      generated rows (2026-09-07):**
      **7,187 of 31,692 Commander-legal cards now execute completely, up
      from 7,172 (+15).** `SHIPPED_SCRIPTS` 4,540; ledger
      935 (5 rows RETIRED by name - cards the ledger held for a clause the suite could not aim at - and none added; measured: the REFUSED map's size 940 -> 935). Fixtures 4,927 (4,785 by name + 135 tokens: the 15 rows' cards; no new token pin, and the four fixtures the clauses are aimed at (Metallic Sliver, Tundra, Accorder's Shield, Ant-Man, Scott Lang, Raging Goblin, Blaze) were pinned already). **Select pool
      0.** Decisions in **D350**.
      ⚠️ **No engine change and no reading changed.** Every one of these rows was
      refused because the SUITE had nothing to aim the clause at, not because
      anything could not be read or run.
      ⚠️ **A counted clause that takes AT MOST ONE pick** is aimed at exactly one
      fixture: declaring the one is a legal answer whether the minimum is 0 or 1,
      and every effect on it runs once. A clause that can take two still needs a
      fixture apiece, which the aim and the asserts do not carry yet.
      ⚠️ **A TAPPED restriction is not a property of the fixture table.** The
      suite arranges it before the fire, the way it already did for an untap
      payload; each fixture is tapped at most once, however many reasons it has.
      ⚠️ **Four fixtures, APPENDED so no existing clause moves its pick** (the
      picker takes the first that fits): Metallic Sliver, a 1/1 colorless
      artifact creature that answers both the colorless clause and the
      power-1-or-less one; Tundra, whose whole text is a reminder-text mana line,
      for the nonbasic land; Accorder's Shield for the Equipment; and for the
      GRAVEYARD lists Ant-Man Scott Lang (legendary), Raging Goblin (a Goblin)
      and Blaze (a red sorcery at mana value 1).
      ⚠️ **A graveyard clause that NAMES no card type may be aimed at any card.**
      The picker tried the CREATURE list and nothing else, so a Grizzly Bears was
      the only card it ever offered for "target card" or "target noncreature,
      nonland card". It tries every list now — and a clause that DOES name a type
      is still refused when that list has nothing, which is the half that keeps
      it honest.
      Measured first: the D349 probe re-run over a tree 48 cards further on: 352 fully covered cards, down from 400 as the vocabulary-payload rows became complete. The row maker's histogram put the clause fixtures third and made them the cheapest thing on the board: a COST it cannot charge 104 (72 distinct verbs - a tail), 'a vocabulary clause the suite has no fixture for' 36, 'a line that is neither an activated ability nor a library trigger' 36 (Unleash 10, Exploit 8, the Threshold bodies), an activation condition 34, an effect outside the row kinds 30 (23 of them Add one mana of any color, whose blocker is the cost beside it). Reading the 36 by sub-class named this decision: a combat-role clause 8, a counted clause 9, a tapped creature 4, a graveyard card the lists did not hold 6, a nonbasic land 2, a colorless or power-1 body 2, an Equipment 1, an Aura 1, alternatives 2, a spell 1 - and twelve of those need nothing but a fixture and a tap. NOT this decision: the clause fixtures still missing (24: a COMBAT-ROLE clause 8 - the suite must fire mid-combat with an attacking or blocking creature, which the attack heads already scaffold; a counted clause that takes TWO or three 5 - one fixture per pick, which the aim and the asserts do not carry yet; a clause whose alternatives differ 2; an Aura, an Equipment in a graveyard, a spell clause), the cost verbs the derivation cannot pay (104 across 72 verbs; the sacrifice and tap predicates the engine's own grammar cannot place lead them), the activation conditions outside the vocabulary (34), the lines that are neither an activated ability nor a library trigger (36: Unleash 10, Exploit 8, the Threshold bodies), the trigger payloads outside both readers (20), Add one mana of any color behind a cost the engine does not charge (23), the counts outside the vocabulary (16), the tokens outside TOKEN_TABLE (14); then D349's list unchanged: the three asking payloads the suite cannot answer (a scry, a surveil), the modal permanent lines, the wider modal heads, the asking modes, the heads outside the library, the modal spells beside another sentence, the quoted abilities enchanted creatures and lands have, Enchant player, the look at the top card of your library. Refused by name:
      5 rows RETIRED by name - cards the ledger held for a clause the suite could not aim at - and none added; measured: the REFUSED map's size 940 -> 935.
      5 ledger rows retired by name: cards refused for a clause the suite had nothing to aim at. Report `effect:auto` 4,209 → 4,209,
      `withUnenforced` 280 → 280.
      **Landed:** No second wave: the 15 rows are the landing, and the POOL took 13 creatures, a land and an artifact more on the same sweep - the commander's reach rose to 7,128 cards from 7,113, chosen from 197 fully-executable legendary creatures (196 before). Tests: one generated suite per row, all fifteen
      green on the first port run.
...
- [x] **M6.4gl — THE COUNTED SACRIFICE: the sacrifice chooser learns the count
      ...
      **Verified: `verify.cjs --full` (sharded) — ALL FIVE GATES: 4,715 files, 23,415 passed / 11 skipped ·
      ...
      sacrifice cost; the remaining cost verbs; the prompt continuation seam;
      prior items stand.
- [x] **M6.4gm — THE TRIBAL LORD: the scope's noun may be the subtype, and the
      first honest map of what is left; 32 cards land, 32 as
      generated rows (2026-09-07):**
      **7,258 of 31,692 Commander-legal cards now execute completely, up
      from 7,226 (+32).** `SHIPPED_SCRIPTS` 4,611; ledger
      932 (nine rows ADDED by name and none retired - four causes, none of them the scope grammar: a scope that is not a creature (Vehicles), a subtype with no inert member in the format (Citizens, Saprolings), a token outside TOKEN_TABLE (the Splicers' 3/3 Phyrexian Golem) and four trigger heads outside the library; measured: the REFUSED map's size 923 -> 932). Fixtures 5,022 (4,879 by name + 136 tokens: the 32 rows' cards and the 21 SUBTYPE FIXTURES the scoped suites put down - one creature per subtype a lord scopes over, vanilla where the format prints one and keyword-only where it does not; no new token pin). **Select pool
      0.** Decisions in **D354**.
      ⚠️ **The seam is one noun**: `staticRowShape` has demanded the literal
      words `creatures` or `permanents` since D300, so it read
      `Other Knight creatures you control get +1/+1.` — which no card prints —
      and refused `Other Knights you control get +1/+1.`, which is how every
      tribal lord in the format is printed. The noun may be the subtype now, the
      plural is LOOKED UP from the fixture table (Elf/Elves, Ally/Allies,
      Merfolk/Merfolk each break a different guess), and the emitter already
      composed `other` with a scope predicate.
      ⚠️⚠️ **THE MEASUREMENT IS THE DECISION.** The first dump of every
      incomplete card's whole remaining text (24,466 cards, 39,272 lines) named a
      160-card seam sharing ONE wording. Measured before it was built, it would
      have landed **zero**: `faceCompleteness` RETURNS EARLY on the target-spec
      gate, so a card whose reported leftover is a spec has never had its
      sentences read. Rebuilt on the union of specs and lines: **12,576 cards are
      one piece from landing across 11,089 wordings — 1.13 each**, so a wording
      table cannot finish this project and only a grammar can.
      ⚠️⚠️ **A SUBTYPE IS SPELLED LIKE ANY CAPITALISED WORD**, so the first
      widening read `You have hexproof.` as a tribal anthem and took five
      player-scoped statics with it. `You`, `Player` and `Opponent` joined the
      exclusion list; the yield is 31, not 36.
      ⚠️ **A TEST THAT CANNOT FAIL IS NOT A PROOF**: five subtypes have no
      vanilla member (every Angel, Faerie and Thopter flies), so the fixture
      table records what each fixture is PRINTED with and the row maker refuses a
      grant of it by name — beside a second guard, that a row whose scoped
      anthems need two different fixtures cannot be proven on one board.
      ⚠️ **A derive step that rebuilds a file must also re-extend it**:
      `make-gen43.cjs` reverted `helpers.json`'s 21 subtype fixtures on every
      run, including from inside `regen.sh`.
      ⚠️⚠️ **PIPELINE TRAP: re-running the candidate probe mid-port excludes the
      cards the port has just LANDED**, and `regen.sh` then unlands all of them
      as dropped rows — 26 in one go. Re-probe only from a fully unlanded tree.
      Measured first: the first dump of every incomplete card's whole remaining text - 24,466 cards, 39,272 leftover lines - and then the correction that made it usable. Read as reported, it said 13,264 cards were ONE line from landing, and named the densest seam ever measured in this arc: 160 modal spells whose single leftover is the same synthetic fragment. That seam was measured before it was built and would have landed EXACTLY ZERO, because `faceCompleteness` returns early on the target-spec gate and never examines the sentences behind it - so a card whose reported leftover is a spec has not been shown to be one step from anything. Rebuilt on the UNION of unread specs and unaccounted lines: exactly ONE card in the format is blocked by a target spec alone, 12,576 are one piece from landing, and they carry 11,089 distinct wordings - 1.13 cards per wording, with the top 100 wordings worth 764 cards. Grouped by the reader that would have to exist, the three densest families are a static grant to a scoped set (494 cards, 302 wordings), a bare keyword or ability word (274 / 81) and a mana ability with a drawback (181 / 121). The grant family was then sized exactly: 185 of it is a QUOTED granted ability, 55 is the plain grammar, and of those the classifier freed 31 with the scope widening alone - measured with the real `primitivesFor` over the whole database, not estimated. NOT this decision: the QUOTED granted ability (221 cards - 121 activated, 90 triggered, 9 static, 1 mixed - whose text `scrub` blanks, so the accounting reports a line ending at the verb and 113 outer grant lines select the ACTIVATED claim bucket because the classifier sees the colons inside the quotes), the grant vocabulary the engine already honours but the shapes do not (8 cards behind fear, intimidate, skulk, horsemanship, wither and exalted - and exalted is not a characteristic at all but a triggered ability `keywordTriggers.ts` runs, so it needs its own proof), a scope that is not a creature (Vehicles), the subtypes with no inert member in the format (Citizens, Saprolings), the 3/3 Phyrexian Golem outside TOKEN_TABLE, the four trigger heads outside the library (a head naming this creature OR another of a type, counted attackers, cast-from-exile, cards leaving a graveyard), the mana ability with a CHOOSER price (17 unsupported prices behind `Add one mana of any color`), snow mana ({S} - 14 cards across four cost forms), the remove-a-counter CHOOSER (11), the self exile (5), the activation conditions outside the vocabulary (34 across 33 wordings), the lines that are neither an activated ability nor a library trigger (20: Unleash 10, Exploit 8). Refused by name:
      nine rows ADDED by name and none retired - four causes, none of them the scope grammar: a scope that is not a creature (Vehicles), a subtype with no inert member in the format (Citizens, Saprolings), a token outside TOKEN_TABLE (the Splicers' 3/3 Phyrexian Golem) and four trigger heads outside the library; measured: the REFUSED map's size 923 -> 932.
      No ledger row retired: this decision adds nine and drains none, because the class it opened was never named in the ledger - the lords were never OFFERED at all. Report `effect:auto` 4,209 → 4,209,
      `withUnenforced` 280 → 280 — neither moves, because a scoped
      static is a PERMANENT's line. `layer6` falls 1,349 → 1,325 instead.
      **Landed:** No second wave: the 32 rows are the landing. The bot's own reach moved with them - the deck now draws from 7,199 fully-executable cards (7,167 before) and its commander is chosen from 203 fully-executable legendary creatures (201 before). Tests: one generated suite per row.
